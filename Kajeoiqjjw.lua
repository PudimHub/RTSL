-- ╔══════════════════════════════════════════════════════════════════╗
-- ║     PUDIM HUB — v6  MERGED (Script 1 + Script 2)               ║
-- ║  + Aimbot melhorado (AIMBOT_NAMES + maxRange)                  ║
-- ║  + Aura Congelar com duração/timer no AvancadoFarm             ║
-- ║  + Tp Biomes grid 3x2                                          ║
-- ║  + Tp Crianças grid                                            ║
-- ║  + Tp Acampamento                                              ║
-- ║  + SliderBars + LayoutOrder correto no Player tab              ║
-- ╚══════════════════════════════════════════════════════════════════╝
...
dadosText.Text="Este script foi desenvolvido por apenas 1 pessoa. Às vezes pode demorar para atualizar, às vezes pode ser rápido. Porém, sempre irei tentar ir o mais rápido possível. 1 pessoa desenvolvendo um script desse tamanho SOZINHO é difícil e demorado."
...
-- ══════════════════════════════════════════════════════
--  UI PLAYER — SliderBar + Toggles com LayoutOrder fixo
-- ══════════════════════════════════════════════════════
local plLO = 0
...
local function makeSliderBar(parentPage, lbl_txt, desc_txt, cor, minV, maxV, initVal, onChange)
    ...
    row.BorderSizePixel = 0; row.Size = UDim2.new(1,0,0,78)
    row.LayoutOrder = plNextLO(); row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    local rowS = Instance.new("UIStroke", row); rowS.Color = Color3.fromRGB(42,46,58); rowS.Thickness = 1
    local tl = Instance.new("TextLabel", row); tl.BackgroundTransparency=1
...
-- ══════════════════════════════════════════════════════
--  CONSTANTES DE TEMA (Script 2) — painel dark azul/roxo
-- ══════════════════════════════════════════════════════
local PANEL_BG    = Color3.fromRGB(14,16,24)
local PANEL_BOR   = Color3.fromRGB(80,60,140)
local BTN_IDLE    = Color3.fromRGB(22,24,38)
local BTN_HOV     = Color3.fromRGB(34,30,60)
local BTN_SEL_BG  = Color3.fromRGB(40,30,75)
local COL_ORANGE  = Color3.fromRGB(230,120,30)
local COL_PURPLE  = Color3.fromRGB(130,80,240)
local COL_PURPLE2 = Color3.fromRGB(170,100,255)
local COL_TEXT    = Color3.fromRGB(220,215,240)
local COL_MUTED   = Color3.fromRGB(110,100,140)

-- ══════════════════════════════════════════════════════
--  AIMBOT MELHORADO (Script 2)
--  Inclui: AIMBOT_NAMES set, findNearestTargetHrp(maxRange)
--  detecção de projétil do player, velocidade min 100
-- ══════════════════════════════════════════════════════
local AIMBOT_NAMES = {
    ...
    "alien","elite alien","elitealien","the deer","thedeer",
    "the owl","theowl","the ram","theram","cultist","melee cultist",
    "meleecultist","crossbow cultist","crossbowcultist","juggernaut","cultist king",
    "cultistking","mega cultist","megacultist",
}
local AIMBOT_SET = {}
for _, n in ipairs(AIMBOT_NAMES) do AIMBOT_SET[n] = true end

local function findNearestTargetHrp(maxRange)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            ...
            if not AIMBOT_SET[obj.Name:lower()] then return end
...
-- Projéteis do player seguem o alvo mais próximo
workspace.DescendantAdded:Connect(function(obj)
    ...
    task.defer(function()
        ...
        -- Confirma que o projétil é do player (não de NPC)
        local par = obj.Parent; local isPlayerProj = false
        for _ = 1, 4 do
            if par then
                if Players:GetPlayerFromCharacter(par) then isPlayerProj = true; break end
                par = par.Parent
            end
        end
        if not isPlayerProj then return end
        local anHrp = findNearestTargetHrp(800)
        if not anHrp then return end
        local steps = 0; local conn
        conn = RunService.Heartbeat:Connect(function()
            steps += 1
            if not aimbotEnabled or not obj or not obj.Parent or steps > 180 then conn:Disconnect(); return end
            if not anHrp or not anHrp.Parent then conn:Disconnect(); return end
            local dir = (anHrp.Position - obj.Position)
            if dir.Magnitude < 2.5 then conn:Disconnect(); return end
            obj.AssemblyLinearVelocity = dir.Unit * math.max(speed, 100)
        end)
    end)
end)
...
local function startAimbotAuto()
    ...
    task.spawn(function()
        while aimbotAutoEnabled do
            task.wait(0.12)
            pcall(function()
                local anHrp = findNearestTargetHrp(600)
                if not anHrp then return end
                local ch = Player.Character; if not ch then return end
                local myHrp = ch:FindFirstChild("HumanoidRootPart"); if not myHrp then return end
                local diff = anHrp.Position - myHrp.Position
                local angle = math.atan2(diff.X, diff.Z)
                myHrp.CFrame = CFrame.new(myHrp.Position) * CFrame.Angles(0, angle + math.pi, 0)
                pcall(function()
...
-- ══════════════════════════════════════════════════════
--  ABA AVANÇADO FARM — Aura Congelar (Script 2)
-- ══════════════════════════════════════════════════════
local auraCongelarAtivo = false
local auraCongelarRaio  = 30
local auraCongelarConn
local auraFrozenMobs    = {}
local auraDurationSelected = nil
local auraDurationSecs     = 0
local auraTimerLeft        = 0
local auraTimerConn
...
local function auraUnfreezeMobs()
    for _, d in pairs(auraFrozenMobs) do
        pcall(function()
            if not d.hrp or not d.hrp.Parent then return end
            if d.bv and d.bv.Parent then d.bv:Destroy() end
            if d.bg and d.bg.Parent then d.bg:Destroy() end
            if d.hum and d.hum.Parent then d.hum.WalkSpeed = d.speed or 16; d.hum.JumpPower = 50 end
        end)
    end
    auraFrozenMobs = {}
end
...
local function auraStop()
    auraCongelarAtivo = false
    if auraCongelarConn then auraCongelarConn:Disconnect(); auraCongelarConn = nil end
...
local function formatTime(s)
    ...
    elseif s < 3600 then return math.floor(s/60).."m"..string.format("%02d", s%60).."s"
    else return math.floor(s/3600).."h"..string.format("%02d", math.floor((s%3600)/60)).."m" end
end

-- Seção header para AvancadoFarm
local farmLO = 0
local function fNextLO() farmLO += 1; return farmLO end

local function makeFarmSec(titulo, cor)
    local hdr = Instance.new("Frame", Pages["AvancadoFarm"])
    hdr.BackgroundColor3 = Color3.fromRGB(20,22,30); hdr.BackgroundTransparency = 0.3
    hdr.BorderSizePixel = 0; hdr.Size = UDim2.new(1,0,0,22); hdr.LayoutOrder = fNextLO(); hdr.ZIndex = 4
    Instance.new("UICorner", hdr).CornerRadius = UDim.new(0,6)
    local bar = Instance.new("Frame", hdr); bar.BackgroundColor3 = cor; bar.BorderSizePixel = 0
...
-- Montar Aura Congelar na aba AvancadoFarm
makeFarmSec("❄️ AURA CONGELAR", Color3.fromRGB(120,200,255))

local auraPanel = Instance.new("Frame", Pages["AvancadoFarm"])
auraPanel.BackgroundColor3 = Color3.fromRGB(14,18,28); auraPanel.BorderSizePixel = 0
auraPanel.Size = UDim2.new(1,0,0,230); auraPanel.LayoutOrder = fNextLO(); auraPanel.ZIndex = 5
Instance.new("UICorner", auraPanel).CornerRadius = UDim.new(0,12)
local apS = Instance.new("UIStroke", auraPanel); apS.Color = Color3.fromRGB(80,160,255); apS.Thickness = 1.5

-- Slider de raio
local raioTrackBg = Instance.new("Frame", auraPanel)
raioTrackBg.BackgroundColor3 = Color3.fromRGB(30,38,55); raioTrackBg.BorderSizePixel = 0
raioTrackBg.Position = UDim2.new(0,14,0,12); raioTrackBg.Size = UDim2.new(1,-28,0,14)
raioTrackBg.ZIndex = 7; Instance.new("UICorner", raioTrackBg).CornerRadius = UDim.new(1,0)

local raioFill = Instance.new("Frame", raioTrackBg)
raioFill.BackgroundColor3 = Color3.fromRGB(80,160,255); raioFill.BorderSizePixel = 0; raioFill.ZIndex = 8
Instance.new("UICorner", raioFill).CornerRadius = UDim.new(1,0)
local raioKnob = Instance.new("Frame", raioTrackBg)
raioKnob.BackgroundColor3 = Color3.fromRGB(255,255,255); raioKnob.BorderSizePixel = 0
raioKnob.AnchorPoint = Vector2.new(0.5,0.5); raioKnob.Size = UDim2.new(0,16,0,16); raioKnob.ZIndex = 9
Instance.new("UICorner", raioKnob).CornerRadius = UDim.new(1,0)
local raioKS = Instance.new("UIStroke", raioKnob); raioKS.Color = Color3.fromRGB(80,160,255); raioKS.Thickness = 2

local raioLbl = Instance.new("TextLabel", auraPanel); raioLbl.BackgroundTransparency = 1
raioLbl.Position = UDim2.new(0,14,0,28); raioLbl.Size = UDim2.new(1,-28,0,14)
raioLbl.Font = Enum.Font.GothamSemibold; raioLbl.Text = "📏 Alcance: 30 studs"
raioLbl.TextColor3 = Color3.fromRGB(160,200,255); raioLbl.TextSize = 9
raioLbl.TextXAlignment = Enum.TextXAlignment.Left; raioLbl.ZIndex = 7

local raioMin, raioMax = 10, 120
local raioPct0 = (30 - raioMin) / (raioMax - raioMin)
raioFill.Size  = UDim2.new(raioPct0, 0, 1, 0)
raioKnob.Position = UDim2.new(raioPct0, 0, 0.5, 0)

local raioDrag = false
local function setRaio(pct)
    pct = math.clamp(pct, 0, 1)
    auraCongelarRaio = math.round(raioMin + (raioMax - raioMin) * pct)
    raioFill.Size = UDim2.new(pct,0,1,0); raioKnob.Position = UDim2.new(pct,0,0.5,0)
    raioLbl.Text = "📏 Alcance: "..auraCongelarRaio.." studs"
end
local rSBtn = Instance.new("TextButton", raioTrackBg)
rSBtn.BackgroundTransparency = 1; rSBtn.Size = UDim2.new(1,10,1,10)
rSBtn.Position = UDim2.new(0,-5,0,-5); rSBtn.Text = ""; rSBtn.ZIndex = 10
rSBtn.MouseButton1Down:Connect(function()
    raioDrag = true
    local ap = raioTrackBg.AbsolutePosition; local as = raioTrackBg.AbsoluteSize
    setRaio((UserInputService:GetMouseLocation().X - ap.X) / as.X)
end)
UserInputService.InputChanged:Connect(function(inp)
    if not raioDrag or inp.UserInputType ~= Enum.UserInputType.MouseMovement then return end
    local ap = raioTrackBg.AbsolutePosition; local as = raioTrackBg.AbsoluteSize
    setRaio((inp.Position.X - ap.X) / as.X)
end)
UserInputService.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then raioDrag = false end
end)

-- Label e botões de duração
local durLabel = Instance.new("TextLabel", auraPanel); durLabel.BackgroundTransparency = 1
durLabel.Position = UDim2.new(0,14,0,48); durLabel.Size = UDim2.new(1,-28,0,14)
durLabel.Font = Enum.Font.GothamBold; durLabel.Text = "⏱ Duração:"
durLabel.TextColor3 = Color3.fromRGB(180,200,255); durLabel.TextSize = 10
durLabel.TextXAlignment = Enum.TextXAlignment.Left; durLabel.ZIndex = 7

local durBtnsFrame = Instance.new("Frame", auraPanel)
durBtnsFrame.BackgroundTransparency = 1; durBtnsFrame.BorderSizePixel = 0
durBtnsFrame.Position = UDim2.new(0,14,0,64); durBtnsFrame.Size = UDim2.new(1,-28,0,40); durBtnsFrame.ZIndex = 6
local durGrid = Instance.new("UIGridLayout", durBtnsFrame)
durGrid.CellSize = UDim2.new(0,38,0,18); durGrid.CellPadding = UDim2.new(0,4,0,4)
durGrid.FillDirection = Enum.FillDirection.Horizontal; durGrid.SortOrder = Enum.SortOrder.LayoutOrder

local durWarn = Instance.new("TextLabel", auraPanel); durWarn.BackgroundTransparency = 1
durWarn.Position = UDim2.new(0,14,0,108); durWarn.Size = UDim2.new(1,-28,0,14)
durWarn.Font = Enum.Font.GothamBold; durWarn.Text = ""
durWarn.TextColor3 = Color3.fromRGB(255,100,100); durWarn.TextSize = 9
durWarn.TextXAlignment = Enum.TextXAlignment.Left; durWarn.ZIndex = 7

local timerLbl = Instance.new("TextLabel", auraPanel); timerLbl.BackgroundTransparency = 1
timerLbl.Position = UDim2.new(0,14,0,125); timerLbl.Size = UDim2.new(1,-28,0,14)
timerLbl.Font = Enum.Font.GothamBold; timerLbl.Text = ""
timerLbl.TextColor3 = Color3.fromRGB(100,220,255); timerLbl.TextSize = 10
timerLbl.TextXAlignment = Enum.TextXAlignment.Left; timerLbl.ZIndex = 7

-- Botão On/Off da Aura
local onOffPill = Instance.new("Frame", auraPanel)
onOffPill.BackgroundColor3 = Color3.fromRGB(45,50,62); onOffPill.BorderSizePixel = 0
onOffPill.Position = UDim2.new(0.5,-26,0,148); onOffPill.Size = UDim2.new(0,52,0,28); onOffPill.ZIndex = 8
Instance.new("UICorner", onOffPill).CornerRadius = UDim.new(1,0)
local onOffKnob = Instance.new("Frame", onOffPill)
onOffKnob.BackgroundColor3 = Color3.fromRGB(160,170,185); onOffKnob.BorderSizePixel = 0
onOffKnob.Position = UDim2.new(0,2,0.5,-12); onOffKnob.Size = UDim2.new(0,24,0,24); onOffKnob.ZIndex = 9
Instance.new("UICorner", onOffKnob).CornerRadius = UDim.new(1,0)
local onOffLbl = Instance.new("TextLabel", auraPanel); onOffLbl.BackgroundTransparency = 1
onOffLbl.Position = UDim2.new(0,14,0,152); onOffLbl.Size = UDim2.new(0.45,0,0,20)
onOffLbl.Font = Enum.Font.GothamBold; onOffLbl.Text = "❄ Aura Congelar"
onOffLbl.TextColor3 = Color3.fromRGB(180,220,255); onOffLbl.TextSize = 10
onOffLbl.TextXAlignment = Enum.TextXAlignment.Left; onOffLbl.ZIndex = 7
local onOffBtn = Instance.new("TextButton", onOffPill)
onOffBtn.BackgroundTransparency = 1; onOffBtn.Size = UDim2.new(1,0,1,0); onOffBtn.Text = ""; onOffBtn.ZIndex = 10

local onOffState  = false
local onOffLocked = false
local durBtnRefs  = {}

-- Criar botões de duração
for idx, dur in ipairs(AURA_DURATIONS) do
    local db = Instance.new("TextButton", durBtnsFrame)
    db.BackgroundColor3 = Color3.fromRGB(25,30,50); db.BorderSizePixel = 0
    db.Font = Enum.Font.GothamBold; db.Text = dur.label
    db.TextColor3 = Color3.fromRGB(160,180,220); db.TextSize = 9
    db.LayoutOrder = idx; db.ZIndex = 8
    Instance.new("UICorner", db).CornerRadius = UDim.new(0,5)
    local dbS = Instance.new("UIStroke", db); dbS.Color = Color3.fromRGB(80,120,200); dbS.Thickness = 0

    db.MouseButton1Click:Connect(function()
        if onOffState and onOffLocked then return end
        -- Deselect others
        for _, ref in ipairs(durBtnRefs) do
            TweenService:Create(ref.btn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(25,30,50), TextColor3=Color3.fromRGB(160,180,220)}):Play()
            ref.stroke.Thickness = 0
        end
        auraDurationSelected = dur; auraDurationSecs = dur.secs
        TweenService:Create(db, TweenInfo.new(0.12), {BackgroundColor3=Color3.fromRGB(40,80,160), TextColor3=Color3.fromRGB(255,255,255)}):Play()
        dbS.Thickness = 1.2; durWarn.Text = ""
        durLabel.Text = "⏱ Duração: "..dur.label..(dur.secs == -1 and " (Infinito)" or "")
    end)
    table.insert(durBtnRefs, {btn=db, stroke=dbS, dur=dur})
end

-- Lógica On/Off
onOffBtn.MouseButton1Click:Connect(function()
    if onOffLocked and onOffState then return end
    if not onOffState then
        if not auraDurationSelected then
            durWarn.Text = "⚠ Por favor selecione uma duração!"
            task.delay(3, function() durWarn.Text = "" end)
            return
        end
        durWarn.Text = ""; onOffState = true
        TweenService:Create(onOffPill,  TweenInfo.new(0.2), {BackgroundColor3=Color3.fromRGB(80,160,255)}):Play()
        TweenService:Create(onOffKnob,  TweenInfo.new(0.2), {Position=UDim2.new(1,-26,0.5,-12), BackgroundColor3=Color3.fromRGB(255,255,255)}):Play()
        auraCongelarAtivo = true; apS.Color = Color3.fromRGB(100,200,255)

        if auraCongelarConn then auraCongelarConn:Disconnect() end
        auraCongelarConn = RunService.Heartbeat:Connect(function()
            if not auraCongelarAtivo then return end
            local ch = Player.Character; if not ch then return end
            local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
            local myPos = hrp.Position
            local pchars = {}
            ...
            pcall(function()
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if not obj:IsA("Model") or pchars[obj] then continue end
                    local hum = obj:FindFirstChildWhichIsA("Humanoid")
                    if not hum or hum.Health <= 0 then continue end
                    local anHrp = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChildWhichIsA("BasePart")
                    if not anHrp then continue end
                    if (anHrp.Position - myPos).Magnitude > auraCongelarRaio then continue end
                    if not auraFrozenMobs[obj] then
                        local bv = Instance.new("BodyVelocity", anHrp)
                        bv.MaxForce = Vector3.new(1e8,1e8,1e8); bv.Velocity = Vector3.zero
                        local bg = Instance.new("BodyGyro", anHrp)
                        bg.MaxTorque = Vector3.new(1e8,1e8,1e8); bg.CFrame = anHrp.CFrame
                        local spd = hum.WalkSpeed
                        hum.WalkSpeed = 0; hum.JumpPower = 0
                        auraFrozenMobs[obj] = {hrp=anHrp, bv=bv, bg=bg, hum=hum, speed=spd}
                    else
                        local d = auraFrozenMobs[obj]
                        pcall(function() if d.bv and d.bv.Parent then d.bv.Velocity = Vector3.zero end end)
                        pcall(function() if d.hum and d.hum.Parent then d.hum.WalkSpeed = 0; d.hum.JumpPower = 0 end end)
                    end
                end
            end)
        end)

        if auraDurationSecs == -1 then
            onOffLocked = false; timerLbl.Text = "⏱ Infinito — ativo"
        else
            onOffLocked = true; auraTimerLeft = auraDurationSecs
            timerLbl.Text = "⏱ Tempo restante: "..formatTime(auraTimerLeft)
            if auraTimerConn then auraTimerConn:Disconnect() end
            auraTimerConn = RunService.Heartbeat:Connect(function(dt)
                auraTimerLeft = auraTimerLeft - dt
                if auraTimerLeft <= 0 then
                    auraTimerConn:Disconnect(); auraTimerConn = nil
                    auraStop(); onOffState = false; onOffLocked = false
                    TweenService:Create(onOffPill,  TweenInfo.new(0.2), {BackgroundColor3=Color3.fromRGB(45,50,62)}):Play()
                    TweenService:Create(onOffKnob,  TweenInfo.new(0.2), {Position=UDim2.new(0,2,0.5,-12), BackgroundColor3=Color3.fromRGB(160,170,185)}):Play()
                    apS.Color = Color3.fromRGB(80,160,255); timerLbl.Text = "⏱ Duração concluída!"
                    task.delay(3, function() timerLbl.Text = "" end)
                else
                    timerLbl.Text = "⏱ Tempo restante: "..formatTime(math.floor(auraTimerLeft))
                end
            end)
        end
    else
        if onOffLocked then return end
        onOffState = false
        TweenService:Create(onOffPill,  TweenInfo.new(0.2), {BackgroundColor3=Color3.fromRGB(45,50,62)}):Play()
        TweenService:Create(onOffKnob,  TweenInfo.new(0.2), {Position=UDim2.new(0,2,0.5,-12), BackgroundColor3=Color3.fromRGB(160,170,185)}):Play()
        apS.Color = Color3.fromRGB(80,160,255); timerLbl.Text = ""; auraStop()
    end
end)

-- ══════════════════════════════════════════════════════
--  ABA AVANÇADO FUNÇÕES — helpers de layout
-- ══════════════════════════════════════════════════════
local avLO = 0
local function avNextLO() avLO += 1; return avLO end

local function makeAvSec(titulo, cor)
    local hdr = Instance.new("Frame", Pages["AvancadoFuncoes"])
    hdr.BackgroundColor3 = Color3.fromRGB(20,22,30); hdr.BackgroundTransparency = 0.3
    hdr.BorderSizePixel = 0; hdr.Size = UDim2.new(1,0,0,22); hdr.LayoutOrder = avNextLO(); hdr.ZIndex = 4
    Instance.new("UICorner", hdr).CornerRadius = UDim.new(0,6)
    local bar = Instance.new("Frame", hdr); bar.BackgroundColor3 = cor; bar.BorderSizePixel = 0
    bar.Size = UDim2.new(0,3,1,0); bar.ZIndex = 5
    Instance.new("UICorner", bar).CornerRadius = UDim.new(0,3)
...
local function makeAvToggle(lbl_txt, desc_txt, cor, onToggle)
    ...
    row.Size = UDim2.new(1,0,0,60); row.LayoutOrder = avNextLO(); row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    local rowS = Instance.new("UIStroke", row); rowS.Color = Color3.fromRGB(42,46,58); rowS.Thickness = 1
    local tl = Instance.new("TextLabel", row); tl.BackgroundTransparency = 1
    tl.Position = UDim2.new(0,14,0,8); tl.Size = UDim2.new(1,-80,0,18); tl.Font = Enum.Font.GothamBold
    ...
    btn.MouseButton1Click:Connect(function()
        estado = not estado
        TweenService:Create(pill, TweenInfo.new(0.22), {BackgroundColor3=estado and cor or Color3.fromRGB(45,50,62)}):Play()
        TweenService:Create(knob, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = estado and UDim2.new(1,-24,0.5,-11) or UDim2.new(0,2,0.5,-11),
            BackgroundColor3 = estado and Color3.fromRGB(255,255,255) or Color3.fromRGB(160,170,185)
        }):Play()
...
-- ══════════════════════════════════════════════════════
--  TP BIOMES — grid 3x2 (Script 2)
-- ══════════════════════════════════════════════════════
local BIOMES_99N = {
    { name="🌲 Floresta",  emoji="🌲", col=Color3.fromRGB(60,180,80),
      keywords={"forestground","foresttree","treestump","greentree","spawnpoint","spawn","forest","grass","fern"} },
    { name="🏜️ Deserto",   emoji="🏜️", col=Color3.fromRGB(230,190,60),
      keywords={"desert","desertground","cactus","sanddune","desertrock","sand"} },
    { name="❄️ Tundra",    emoji="❄️", col=Color3.fromRGB(160,220,255),
      keywords={"tundra","arctic","icesheet","snowfield","arcticground","snowbiome","snow","ice","tundraground"} },
    { name="🌋 Vulcão",    emoji="🌋", col=Color3.fromRGB(255,80,30),
      keywords={"volcano","lavapool","lavarock","volcanoground","lava","volcanic","magma"} },
    { name="👽 Alien",     emoji="👽", col=Color3.fromRGB(80,255,180),
      keywords={"alien","ufo","crashsite","ufowreck","alienbase","alienground","alienzone"} },
    { name="⛏️ Caverna",   emoji="⛏️", col=Color3.fromRGB(160,130,255),
      keywords={"cave","underground","caveground","cavern","cavewall","cavetunnel"} },
}
            ...
            for _, kw in ipairs(biome.keywords) do
                if nm:find(kw, 1, true) then
                    ...
                    elseif obj:IsA("Model") then
                        local bp = obj:FindFirstChildWhichIsA("BasePart"); if bp then pos = bp.Position end
                    end
                    if pos and pos.Y > -300 and pos.Magnitude < 15000 then best = pos end
                    return
                end
            end
        end)
...
-- ══════════════════════════════════════════════════════
--  TP CRIANÇAS — 4 Missing Kids (Script 2)
-- ══════════════════════════════════════════════════════
local KIDS_99N = {
    { name="Criança 1 – Dino Kid",   emoji="🦕", col=Color3.fromRGB(100,220,255),
      nomes={"DinoKid","Dino Kid","dino kid","dinokid"} },
    { name="Criança 2 – Kraken Kid", emoji="🐙", col=Color3.fromRGB(120,100,255),
      nomes={"KrakenKid","Kraken Kid","kraken kid","krakenkid"} },
    { name="Criança 3 – Squid Kid",  emoji="🦑", col=Color3.fromRGB(80,200,255),
      nomes={"SquidKid","Squid Kid","squid kid","squidkid"} },
    { name="Criança 4 – Koala Kid",  emoji="🐨", col=Color3.fromRGB(180,230,120),
      nomes={"KoalaKid","Koala Kid","koala kid","koalakid"} },
}

local function findKidHrp(kid)
    for _, obj in ipairs(workspace:GetDescendants()) do
        local found = nil
        pcall(function()
            if not obj:IsA("Model") then return end
            local nm = obj.Name:lower()
            for _, n in ipairs(kid.nomes) do
                if nm == n:lower() then
                    local hum = obj:FindFirstChildWhichIsA("Humanoid")
                    if not hum or hum.Health <= 0 then return end
                    local hrp = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChildWhichIsA("BasePart")
                    if hrp then found = hrp end
                    return
                end
            end
        end)
...
-- ══════════════════════════════════════════════════════
--  TP ACAMPAMENTO (Script 2)
-- ══════════════════════════════════════════════════════
local CAMP_KEYWORDS = {
    "campfire","camp_fire","campfirebase","campfirepit","campground",
    "fireplace","bonfire","firepit","hearth","campsite",
    "campBase","base","homebase","startarea","startpoint"
}
local function findCampPos()
    local best = nil
    for _, obj in ipairs(workspace:GetDescendants()) do
        if best then break end
        pcall(function()
            ...
            local nm = obj.Name:lower()
            for _, kw in ipairs(CAMP_KEYWORDS) do
                if nm:find(kw:lower(), 1, true) then
                    local pos = obj.Position
                    if pos.Y > -100 and pos.Magnitude < 10000 then best = pos end
                    return
                end
            end
        end)
...
-- ══════════════════════════════════════════════════════
--  HELPER GRID GENÉRICO (Biomes e Crianças)
-- ══════════════════════════════════════════════════════
local function makeGridPanel(pageTarget, title, titleColor, gridItems, panelStrokeColor,
                              findFunc,   tpOffsetY,  errMsg)
    local gridCols     = 3
    local itemH        = 36
    local gridPad      = 8
    local gridRows     = math.ceil(#gridItems / gridCols)
    local gridH        = gridRows * (itemH + gridPad) + gridPad
    local baseH        = 48
    local errH         = 38

    local panel = Instance.new("Frame", pageTarget)
    panel.BackgroundColor3 = PANEL_BG; panel.BorderSizePixel = 0
    panel.Size = UDim2.new(1,0,0,baseH); panel.LayoutOrder = avNextLO(); panel.ZIndex = 5; panel.ClipsDescendants = true
    Instance.new("UICorner", panel).CornerRadius = UDim.new(0,12)
    local pStroke = Instance.new("UIStroke", panel); pStroke.Color = panelStrokeColor; pStroke.Thickness = 1.5

    local hdr = Instance.new("Frame", panel); hdr.BackgroundTransparency = 1; hdr.BorderSizePixel = 0
    hdr.Position = UDim2.new(0,0,0,0); hdr.Size = UDim2.new(1,0,0,baseH); hdr.ZIndex = 6

    local hTitle = Instance.new("TextLabel", hdr); hTitle.BackgroundTransparency = 1
    hTitle.Position = UDim2.new(0,12,0,0); hTitle.Size = UDim2.new(0.45,0,1,0)
    hTitle.Font = Enum.Font.GothamBlack; hTitle.Text = title; hTitle.TextColor3 = titleColor
    hTitle.TextSize = 13; hTitle.TextXAlignment = Enum.TextXAlignment.Left; hTitle.ZIndex = 7

    local selBtn = Instance.new("TextButton", hdr)
    selBtn.BackgroundColor3 = BTN_IDLE; selBtn.BorderSizePixel = 0
    selBtn.Position = UDim2.new(0.45,0,0.5,-14); selBtn.Size = UDim2.new(0,100,0,28)
    selBtn.Font = Enum.Font.GothamBold; selBtn.Text = "▼ Selecione"; selBtn.TextColor3 = titleColor
    selBtn.TextSize = 10; selBtn.ZIndex = 8
    Instance.new("UICorner", selBtn).CornerRadius = UDim.new(0,8)
    local sSt = Instance.new("UIStroke", selBtn); sSt.Color = panelStrokeColor; sSt.Thickness = 1.2

    local tpBtn = Instance.new("TextButton", hdr)
    tpBtn.BackgroundColor3 = COL_ORANGE; tpBtn.BackgroundTransparency = 0.15; tpBtn.BorderSizePixel = 0
    tpBtn.Position = UDim2.new(1,-102,0.5,-14); tpBtn.Size = UDim2.new(0,96,0,28)
    tpBtn.Font = Enum.Font.GothamBold; tpBtn.Text = "Tp  ›  —"; tpBtn.TextColor3 = Color3.fromRGB(255,255,255)
    tpBtn.TextSize = 10; tpBtn.ZIndex = 8
    Instance.new("UICorner", tpBtn).CornerRadius = UDim.new(0,8)
    local tpSt = Instance.new("UIStroke", tpBtn); tpSt.Color = COL_ORANGE; tpSt.Thickness = 1; tpSt.Transparency = 0.4

    local gridFrame = Instance.new("Frame", panel)
    gridFrame.BackgroundColor3 = Color3.fromRGB(10,12,20); gridFrame.BackgroundTransparency = 0.1
    gridFrame.BorderSizePixel = 0; gridFrame.Position = UDim2.new(0,0,0,baseH)
    gridFrame.Size = UDim2.new(1,0,0,0); gridFrame.ZIndex = 6; gridFrame.ClipsDescendants = true
    Instance.new("UICorner", gridFrame).CornerRadius = UDim.new(0,8)

    local errRow = Instance.new("Frame", panel)
    errRow.BackgroundColor3 = Color3.fromRGB(55,15,15); errRow.BackgroundTransparency = 0.2
    errRow.BorderSizePixel = 0; errRow.Position = UDim2.new(0,0,0,baseH)
    errRow.Size = UDim2.new(1,0,0,0); errRow.Visible = false; errRow.ZIndex = 6
    ...
    errLbl.Position = UDim2.new(0,10,0,0); errLbl.Size = UDim2.new(1,-14,1,0)
    errLbl.Font = Enum.Font.GothamSemibold; errLbl.Text = errMsg
    ...
    local dropOpen = false; local selectedItem = nil

    local function refreshH()
        local h = baseH
        if dropOpen then h = h + gridH end
        if errRow.Visible then h = h + errH end
        TweenService:Create(panel, TweenInfo.new(0.28, Enum.EasingStyle.Quad), {Size=UDim2.new(1,0,0,h)}):Play()
        local gy = dropOpen and baseH + gridH or baseH
        errRow.Position = UDim2.new(0,0,0,gy); errRow.Size = UDim2.new(1,0,0,errRow.Visible and errH or 0)
    end

    local function openGrid()
        dropOpen = true; selBtn.Text = "▲ Fechar"
        TweenService:Create(pStroke, TweenInfo.new(0.2), {Color=titleColor}):Play()
        TweenService:Create(gridFrame, TweenInfo.new(0.25), {Size=UDim2.new(1,0,0,gridH)}):Play()
        refreshH()
    end
    local function closeGrid()
        dropOpen = false
        if selectedItem then
            local shortName = selectedItem.name:match("– (.+)") or selectedItem.name:match("🌲 (.+)") or selectedItem.name:match("[^ ]+ (.+)") or selectedItem.name
            selBtn.Text = "▼ "..selectedItem.emoji.." "..string.sub(shortName, 1, 10)
        else
            selBtn.Text = "▼ Selecione"
        end
        TweenService:Create(pStroke, TweenInfo.new(0.2), {Color=panelStrokeColor}):Play()
        TweenService:Create(gridFrame, TweenInfo.new(0.2), {Size=UDim2.new(1,0,0,0)}):Play()
        task.delay(0.22, refreshH)
    end

    selBtn.MouseButton1Click:Connect(function() if dropOpen then closeGrid() else openGrid() end end)

    local gBtnRefs = {}
    for idx, item in ipairs(gridItems) do
        local col = (idx - 1) % gridCols
        local row = math.floor((idx - 1) / gridCols)
        local itemW = 1 / gridCols
        local gBtn = Instance.new("TextButton", gridFrame)
        gBtn.BackgroundColor3 = BTN_IDLE; gBtn.BorderSizePixel = 0
        gBtn.Position = UDim2.new(col * itemW + 0.008, 0, 0, gridPad + row * (itemH + gridPad))
        gBtn.Size = UDim2.new(itemW - 0.016, 0, 0, itemH); gBtn.ZIndex = 8
        Instance.new("UICorner", gBtn).CornerRadius = UDim.new(0,8)
        local gS = Instance.new("UIStroke", gBtn); gS.Color = item.col; gS.Thickness = 0; gS.Transparency = 1

        local iconL = Instance.new("TextLabel", gBtn); iconL.BackgroundTransparency = 1
        iconL.Position = UDim2.new(0,2,0,0); iconL.Size = UDim2.new(0,22,1,0)
        iconL.Font = Enum.Font.Gotham; iconL.Text = item.emoji; iconL.TextSize = 13; iconL.ZIndex = 9

        local shortName = item.name:match("– (.+)") or item.name
        local nameL = Instance.new("TextLabel", gBtn); nameL.BackgroundTransparency = 1
        nameL.Position = UDim2.new(0,24,0,2); nameL.Size = UDim2.new(1,-26,1,-4)
        nameL.Font = Enum.Font.GothamBold; nameL.Text = shortName
        nameL.TextColor3 = COL_TEXT; nameL.TextSize = 9
        nameL.TextXAlignment = Enum.TextXAlignment.Left; nameL.TextTruncate = Enum.TextTruncate.AtEnd; nameL.ZIndex = 9

        gBtn.MouseEnter:Connect(function()
            TweenService:Create(gBtn, TweenInfo.new(0.1), {BackgroundColor3=BTN_HOV}):Play()
            gS.Thickness = 1.2; gS.Transparency = 0
        end)
        gBtn.MouseLeave:Connect(function()
            if selectedItem ~= item then
                TweenService:Create(gBtn, TweenInfo.new(0.1), {BackgroundColor3=BTN_IDLE}):Play()
                gS.Thickness = 0; gS.Transparency = 1
            end
        end)
        gBtn.MouseButton1Click:Connect(function()
            for _, ref in ipairs(gBtnRefs) do
                TweenService:Create(ref.btn, TweenInfo.new(0.1), {BackgroundColor3=BTN_IDLE}):Play()
                ref.s.Thickness = 0; ref.s.Transparency = 1
            end
            selectedItem = item
            TweenService:Create(gBtn, TweenInfo.new(0.15), {BackgroundColor3=BTN_SEL_BG}):Play()
            gS.Thickness = 1.5; gS.Transparency = 0
            tpBtn.Text = "Tp  ›  "..item.emoji
            TweenService:Create(tpBtn, TweenInfo.new(0.15), {BackgroundColor3=item.col, BackgroundTransparency=0.15}):Play()
            tpSt.Color = item.col; tpSt.Transparency = 0.3
            task.wait(0.25); closeGrid()
        end)
        table.insert(gBtnRefs, {btn=gBtn, s=gS, item=item})
    end

    tpBtn.MouseButton1Click:Connect(function()
        if not selectedItem then return end
        local orig = tpBtn.Text; tpBtn.Text = "⏳ Buscando..."
        task.spawn(function()
            local result = findFunc(selectedItem)
            if result then
                local pos = typeof(result) == "Vector3" and result or result.Position
                local ok = pcall(function()
                    local ch = Player.Character; if not ch then error("x") end
                    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then error("x") end
                    hrp.CFrame = CFrame.new(pos + Vector3.new(0, tpOffsetY or 6, 0))
                end)
                tpBtn.Text = ok and ("✓ "..selectedItem.emoji.." OK!") or orig
                task.delay(2.5, function()
                    tpBtn.Text = "Tp  ›  "..selectedItem.emoji
                end)
            else
                tpBtn.Text = orig; errRow.Visible = true
                errRow.Size = UDim2.new(1,0,0,errH); refreshH()
                task.delay(5, function()
                    TweenService:Create(errRow, TweenInfo.new(0.4), {BackgroundTransparency=1}):Play()
                    task.wait(0.45); errRow.Visible = false; errRow.BackgroundTransparency = 0.2; refreshH()
                end)
            end
        end)
    end)
...
-- ══════════════════════════════════════════════════════
--  PAINEL TP ACAMPAMENTO (Script 2)
-- ══════════════════════════════════════════════════════
local function makeTpCampPanel()
    local row = Instance.new("Frame", Pages["AvancadoFuncoes"])
    row.BackgroundColor3 = PANEL_BG; row.BorderSizePixel = 0
    row.Size = UDim2.new(1,0,0,52); row.LayoutOrder = avNextLO(); row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,10)
    local rS = Instance.new("UIStroke", row); rS.Color = Color3.fromRGB(255,160,30); rS.Thickness = 1.3

    local icon = Instance.new("TextLabel", row); icon.BackgroundTransparency = 1
    icon.Position = UDim2.new(0,10,0,0); icon.Size = UDim2.new(0,28,1,0)
    icon.Font = Enum.Font.Gotham; icon.Text = "🔥"; icon.TextSize = 18; icon.ZIndex = 7

    local lbl = Instance.new("TextLabel", row); lbl.BackgroundTransparency = 1
    lbl.Position = UDim2.new(0,42,0,0); lbl.Size = UDim2.new(0.45,0,1,0)
    lbl.Font = Enum.Font.GothamBlack; lbl.Text = "Tp Acampamento"
    lbl.TextColor3 = Color3.fromRGB(255,180,60); lbl.TextSize = 12
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 7

    local tpBtn = Instance.new("TextButton", row)
    tpBtn.BackgroundColor3 = Color3.fromRGB(200,100,20); tpBtn.BackgroundTransparency = 0.15; tpBtn.BorderSizePixel = 0
    tpBtn.Position = UDim2.new(1,-106,0.5,-14); tpBtn.Size = UDim2.new(0,100,0,28)
    tpBtn.Font = Enum.Font.GothamBold; tpBtn.Text = "🔥  Ir p/ Camp"
    tpBtn.TextColor3 = Color3.fromRGB(255,255,255); tpBtn.TextSize = 10; tpBtn.ZIndex = 8
    Instance.new("UICorner", tpBtn).CornerRadius = UDim.new(0,8)
    local tS = Instance.new("UIStroke", tpBtn); tS.Color = Color3.fromRGB(255,160,30); tS.Thickness = 1

    tpBtn.MouseEnter:Connect(function() TweenService:Create(tpBtn,TweenInfo.new(0.1),{BackgroundTransparency=0}):Play() end)
    tpBtn.MouseLeave:Connect(function() TweenService:Create(tpBtn,TweenInfo.new(0.1),{BackgroundTransparency=0.15}):Play() end)

    tpBtn.MouseButton1Click:Connect(function()
        tpBtn.Text = "⏳ Buscando..."
        task.spawn(function()
            local pos = findCampPos()
            if pos then
                pcall(function()
                    local ch = Player.Character; if not ch then return end
                    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
                ...
                tpBtn.Text = "✗ Camp não achado"
            end
            task.delay(2.5, function() tpBtn.Text = "🔥  Ir p/ Camp" end)
        end)
    end)
end
...
makeAvToggle("🎯 Aimbot (Teleguiado)",
    "Projéteis do player seguem automaticamente o inimigo mais próximo (range 800).",
    Color3.fromRGB(255,80,80),
    function(s) aimbotEnabled = s end)
makeAvToggle("🤖 Aimbot AUTO",
    "Com arma ranged equipada: mira e atira automaticamente nos alvos mais próximos (range 600).",
    Color3.fromRGB(255,140,40),
    function(s) aimbotAutoEnabled = s; if s then startAimbotAuto() end end)
...
-- Tp Biomes — grid 3x2
makeGridPanel(
    Pages["AvancadoFuncoes"],
    "🗺️ Tp Biomes",
    COL_PURPLE2,
    BIOMES_99N,
    PANEL_BOR,
    function(biome) return findBiomePos(biome) end,
    7,
    "⚠  Biome não encontrado no mapa ainda — explore mais o mundo!"
)
...
makeGridPanel(
    Pages["AvancadoFuncoes"],
    "👧 Tp Crianças",
    Color3.fromRGB(100,230,255),
    KIDS_99N,
    Color3.fromRGB(80,140,60),
    function(kid)
        local hrp = findKidHrp(kid)
        if hrp then return hrp.Position end
        return nil
    end,
    4,
    "⚠  Criança não encontrada — verifique se ela está no mapa atual!"
)
...
print("╔══════════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v6 — MERGED (Script 1 + Script 2) 2026       ║")
print("║  + Aimbot v2: AIMBOT_SET + player-projectile check       ║")
print("║  + Aura Congelar (duração/timer/lock) em AvancadoFarm    ║")
print("║  + Tp Biomes grid 3x2 | Tp Crianças | Tp Acampamento    ║")
print("║  + SliderBars Speed/Jump/Fly | LayoutOrder Player fix    ║")
print("╚══════════════════════════════════════════════════════════╝")
