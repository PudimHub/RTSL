-- ╔══════════════════════════════════════════════════════════════════╗
-- ║          PUDIM HUB — v5 LITE  (Otimizado Mobile)               ║
-- ║  Compatível com Delta Mobile, Arceus X, Hydrogen e similares   ║
-- ║  ESP 20 cats | BRING 17 cats | Player | Teleporte | Biomas     ║
-- ║  SEM Aimbot/FOV — versão leve para executores mobile           ║
-- ║  Fevereiro 2026 — 99 Dias na Floresta                          ║
-- ╚══════════════════════════════════════════════════════════════════╝
...
bannerTitle.Text="🌲  PudimHub LITE"; bannerTitle.TextColor3=Color3.fromRGB(255,255,255)
...
infoStatusSub.Text="ID: "..tostring(game.PlaceId).."  •  Hub LITE"
...
local POOL_SIZE=30; local labelPool={}; local activeList={}
...
local entityCache={}; local itemCache={}; local cacheBuilding=false; local lastCache=0; local CACHE_INTER=8
...
local function buildCache()
    ...
    task.spawn(function()
        ...
        for _,obj in ipairs(descs) do
            ...
            end -- fecha: if obj and obj.Parent
        end
        entityCache=newEnt; itemCache=newItem; cacheBuilding=false
    end)
end

local dtAcc=0; local RENDER_I=1/10
...
local function executarBring(key)
    ...
    for _,obj in ipairs(descs) do
        ...
        pcall(function()
            ...
            count=count+1; table.insert(trazidos,{obj=obj,pos=target})
        end)
    end
    if #trazidos>0 then
        task.spawn(function()
...
local function makePlSec(titulo, cor)
    ...
    for _, c in pairs(Pages["Player"]:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextButton") then lo=lo+1 end
    end
...
-- [AIMBOT REMOVIDO — versão LITE mobile]
...
local function criarIconeAvancado(parent, key, cor)
    ...
    local function r(x,y,w,h,rad) return mkRect(ic,x,y,w,h,cor,rad) end
    local function c(x,y,rad)     return mkCircle(ic,x,y,rad,cor) end
    if key == "Aimbot" or key == "AimbotFOV" or key == "AimbotAuto" then
...
local avTabLO = 0
local function avLO() avTabLO=avTabLO+1; return avTabLO end
...
makeAvInfoBox(
    "⚠️  Aimbot, Aimbot FOV e Aimbot AUTO foram removidos nesta versão LITE.\n"..
    "Esta versão é otimizada para executores mobile (Delta, Arceus X, etc).",
...
-- ─── PAINEL BIOMES ───
local tpLO = 0
local function tplo() tpLO=tpLO+1; return tpLO end
...
print("╔══════════════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v5.0 LITE — MOBILE  Fev 2026                     ║")
print("╠══════════════════════════════════════════════════════════════╣")
print("║  ✓ InfZoom | CamLivre | FullBright | InfiniteJump           ║")
print("║  ✓ SlopeBoost | Aba Teleporte/Biomas | Notificações Toast    ║")
print("║  ✓ Player: Speed | JumpPower | Fly | Noclip | TpClick | ANC ║")
print("║  ✓ ESP: 20 cats | BRING: 17 cats | Wiki 2026                ║")
print("║  ✗ Aimbot/FOV removidos — versão LITE otimizada mobile      ║")
print("╚══════════════════════════════════════════════════════════════╝")
