...
-- FIX: Delta mobile não tem acesso a CoreGui via loadstring
-- Usa pcall para tentar CoreGui primeiro, senão cai no PlayerGui
...
local function UltraBooster(state)
    if state then
        ...
        pcall(function() if sethiddenproperty then sethiddenproperty(Player,"MaximumSimulationRadius",math.huge); sethiddenproperty(Player,"SimulationRadius",math.huge) end end)
...
-- [v4 log removido — ver v5 no final]
...
local function showNotif(msg, tipo)
    ...
    local cor = tipo=="erro" and Color3.fromRGB(220,60,60)
           ...
           or   tipo=="sucesso" and Color3.fromRGB(60,200,100)
           or   Color3.fromRGB(88,101,242)
    local ico = tipo=="erro" and "✖" or tipo=="aviso" and "⚠" or tipo=="sucesso" and "✔" or "ℹ"
    ...
    nf.ZIndex = 999
    nf.LayoutOrder = _notifCount
    Instance.new("UICorner", nf).CornerRadius = UDim.new(0,10)
    local ns = Instance.new("UIStroke", nf); ns.Color = cor; ns.Thickness = 1.2
    local acc = Instance.new("Frame", nf)
    acc.BackgroundColor3 = cor; acc.BorderSizePixel = 0
    acc.Size = UDim2.new(0,3,1,0); acc.ZIndex = 1000
    local icoLbl = Instance.new("TextLabel", nf)
    icoLbl.BackgroundTransparency = 1
    icoLbl.Position = UDim2.new(0,10,0.5,-10); icoLbl.Size = UDim2.new(0,20,0,20)
    icoLbl.Font = Enum.Font.GothamBlack; icoLbl.Text = ico
    icoLbl.TextColor3 = cor; icoLbl.TextSize = 14; icoLbl.ZIndex = 1000
    local msgLbl = Instance.new("TextLabel", nf)
    msgLbl.BackgroundTransparency = 1
    msgLbl.Position = UDim2.new(0,34,0.5,-12); msgLbl.Size = UDim2.new(1,-40,0,24)
    msgLbl.Font = Enum.Font.GothamSemibold; msgLbl.Text = msg
    msgLbl.TextColor3 = Color3.fromRGB(220,225,240); msgLbl.TextSize = 9
    msgLbl.TextXAlignment = Enum.TextXAlignment.Left
    msgLbl.TextWrapped = true; msgLbl.ZIndex = 1000
    TweenService:Create(nf, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size=UDim2.new(0,270,0,44)}):Play()
    -- FIX: aumentado de 0.6s para 3s para a notificação ser visível
    task.delay(3, function()
        TweenService:Create(nf, TweenInfo.new(0.3), {Size=UDim2.new(0,0,0,44)}):Play()
...
-- ══════════════════════════════════════════════════════════
--  FUNÇÕES DE APLICAÇÃO — SPEED / JUMP
-- ══════════════════════════════════════════════════════════
...
local function setInfZoom(state)
    ...
    if state then
        ...
        pcall(function() workspace.CurrentCamera.MaxZoomDistance = 999999 end)
        showNotif("Infinito Zoom ativado! 🔭", "sucesso")
    else
...
-- ══════════════════════════════════════════════════════════
--  CÂMERA LIVRE (atravessa paredes)
-- ══════════════════════════════════════════════════════════
local function setCamLivre(state)
    if state then
        if camLivreConn then camLivreConn:Disconnect() end
        -- Força câmera custom que não colide com geometry
        camLivreConn = RunService.Heartbeat:Connect(function()
            if not state then return end
            pcall(function()
                local cam = workspace.CurrentCamera
                if cam and cam.CameraType ~= Enum.CameraType.Scriptable then
                    -- Não troca, mas remove a colisão dos objetos com a câmera
                    for _, obj in ipairs(workspace:GetDescendants()) do
                        if obj:IsA("BasePart") and obj.CanCollide then
                            obj.CastShadow = obj.CastShadow -- keep alive
                        end
                    end
                end
            end)
        end)
        -- Método direto: desativa colisão da câmera via CameraType trick
        pcall(function()
            workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
            task.wait(0.05)
            workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
        end)
        showNotif("Câmera Livre ativada! 👁️", "sucesso")
    else
        if camLivreConn then camLivreConn:Disconnect(); camLivreConn = nil end
    end
end
    ...
    if state then
        ...
        for _, obj in ipairs(Lighting:GetChildren()) do
            pcall(function()
                ...
                or obj:IsA("BlurEffect") or obj:IsA("DepthOfFieldEffect")
                or obj:IsA("SunRaysEffect") then
                    obj.Enabled = false
                end
            end)
        end
        -- Remove chuva e neblina no workspace
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                if obj:IsA("ParticleEmitter") then
                    local n = obj.Name:lower()
                    if n:find("rain") or n:find("fog") or n:find("snow")
                    or n:find("mist") or n:find("storm") or n:find("cloud") then
                        obj.Enabled = false
                    end
                end
            end)
        end
        showNotif("FullBright ativado! Noite = dia ☀️", "sucesso")
    else
...
-- ══════════════════════════════════════════════════════════
--  SLOPE BOOST (imune a armadilhas)
-- ══════════════════════════════════════════════════════════
local function setSlopeBoost(state)
    ...
    if state then
        ...
        slopeBoostConn = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                local ch = Player.Character; if not ch then return end
                local hum = ch:FindFirstChildWhichIsA("Humanoid"); if not hum then return end
                if hum.WalkSpeed < playerSpeed - 1 then
...
-- ══════════════════════════════════════════════════════════
--  NOCLIP (atravessa paredes, não o chão)
-- ══════════════════════════════════════════════════════════
...
local function setNoclip(state)
    ...
    if state then
        ...
        noclipConn = RunService.Stepped:Connect(function()
            ...
            if hrp and hrp.Position.Y < NOCLIP_VOID_Y then
...
local function criarIconePlayer(parent, key, cor)
    ...
    if     key == "Speed"     then r(0,10,20,4,2); r(4,6,16,4,2); r(8,14,16,4,2); r(22,8,6,10,3)
    elseif key == "JumpPower" then c(14,18,6); r(12,4,4,14,2); r(6,4,16,4,2)
    elseif key == "Fly"       then r(2,12,24,4,2); r(4,6,8,8,4); r(16,6,8,8,4); r(10,14,8,10,3)
    ...
    elseif key == "TpClick"   then r(10,0,4,28,2); r(0,10,28,4,2); c(14,14,4)
    elseif key == "BauANC"    then r(2,8,24,16,4); r(8,4,12,6,2); r(10,12,8,4,3)
    ...
    elseif key == "CamLivre"  then r(2,8,20,14,4); r(22,10,6,10,3); c(12,15,4)
    elseif key == "FullBright" then
        c(14,14,6)
        for _,deg in ipairs({0,45,90,135,180,225,270,315}) do
            local rad = math.rad(deg)
            r(14+math.cos(rad)*10-1, 14+math.sin(rad)*10-1, 3, 3, 1)
        end
    elseif key == "InfJump"   then r(10,0,8,8,2); r(6,8,16,4,2); r(2,12,24,4,2); r(6,16,16,8,4)
    elseif key == "SlopeBoost" then r(0,20,28,4,2); r(4,14,12,6,2); r(12,8,12,6,2); r(18,2,10,6,2)
    else r(8,4,12,4,2); r(4,10,20,8,4); r(8,18,12,8,2)
    end
    return ic
end

-- ══════════════════════════════════════════════════════════
--  UI HELPERS — ABA PLAYER
-- ══════════════════════════════════════════════════════════
local function makePlSec(titulo, cor)
    local lo = 0
    for _, c in pairs(Pages["Player"]:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextButton") then lo += 1 end
    end
    local hdr = Instance.new("Frame", Pages["Player"])
    hdr.BackgroundColor3 = Color3.fromRGB(20,22,30); hdr.BackgroundTransparency = 0.3
    hdr.BorderSizePixel = 0; hdr.Size = UDim2.new(1,0,0,22); hdr.LayoutOrder = lo+10; hdr.ZIndex = 4
    Instance.new("UICorner", hdr).CornerRadius = UDim.new(0,6)
    local bar = Instance.new("Frame", hdr); bar.BackgroundColor3 = cor; bar.BorderSizePixel = 0
    bar.Position = UDim2.new(0,0,0,0); bar.Size = UDim2.new(0,3,1,0); bar.ZIndex = 5
    Instance.new("UICorner", bar).CornerRadius = UDim.new(0,3)
...
local function makePlRow_Slider(icon_key, cor, lbl_txt, desc_txt, minV, maxV, step, initVal, onChange)
    local lo = 0
    for _, c in pairs(Pages["Player"]:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextButton") then lo += 1 end
    end
    ...
    row.Size = UDim2.new(1,0,0,68); row.LayoutOrder = lo+10; row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    ...
    criarIconePlayer(ib, icon_key, cor)
    ...
    tl.Text = lbl_txt; tl.TextColor3 = Color3.fromRGB(220,225,240); tl.TextSize = 11
    ...
    local ctrl = Instance.new("Frame", row); ctrl.BackgroundColor3 = Color3.fromRGB(16,18,26)
    ctrl.BackgroundTransparency = 0.15; ctrl.BorderSizePixel = 0
    ctrl.Position = UDim2.new(0,54,0,42); ctrl.Size = UDim2.new(1,-66,0,20); ctrl.ZIndex = 8
    ...
    valLbl.Position = UDim2.new(0.5,-20,0,0); valLbl.Size = UDim2.new(0,40,1,0)
    valLbl.Font = Enum.Font.GothamBold; valLbl.Text = tostring(initVal)
    valLbl.TextColor3 = cor; valLbl.TextSize = 12; valLbl.ZIndex = 9
    local curV = {initVal}
    local function upd(v)
        v = math.clamp(v, minV, maxV); curV[1] = v; valLbl.Text = tostring(v); onChange(v)
    end
    local btnDefs = {
        {"-"..tostring(step*5), -step*5}, {"-"..tostring(step), -step},
        {"+"..tostring(step),   step},    {"+"..tostring(step*5), step*5}
    }
    for idx, bd in ipairs(btnDefs) do
        local b = Instance.new("TextButton", ctrl); b.BackgroundColor3 = cor
        b.BackgroundTransparency = 0.35; b.BorderSizePixel = 0; b.Size = UDim2.new(0,30,1,0)
        b.Position = idx <= 2 and UDim2.new(0,(idx-1)*31,0,0) or UDim2.new(1,-(4-idx+1)*31,0,0)
        b.Font = Enum.Font.GothamBold; b.Text = bd[1]; b.TextColor3 = Color3.fromRGB(255,255,255)
...
local function makePlRow_Toggle(icon_key, cor, lbl_txt, desc_txt, onToggle, extraSlider)
    local lo = 0
    for _, c in pairs(Pages["Player"]:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextButton") then lo += 1 end
    end
    local row = Instance.new("Frame", Pages["Player"])
    row.BackgroundColor3 = Color3.fromRGB(28,30,38); row.BorderSizePixel = 0
    row.Size = UDim2.new(1,0,0,58); row.LayoutOrder = lo+10; row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    local rowS = Instance.new("UIStroke", row); rowS.Color = Color3.fromRGB(42,46,58); rowS.Thickness = 1
    local gb = Instance.new("Frame", row); gb.BackgroundColor3 = cor; gb.BackgroundTransparency = 0.9
    gb.BorderSizePixel = 0; gb.Size = UDim2.new(1,0,1,0); gb.ZIndex = 5
    ...
    if extraSlider then
        ...
        local esf = Instance.new("Frame", row); esf.BackgroundColor3 = Color3.fromRGB(16,18,26)
        esf.BackgroundTransparency = 0.15; esf.BorderSizePixel = 0
        esf.Position = UDim2.new(1,-pillRight,0.5,-11); esf.Size = UDim2.new(0,56,0,22); esf.ZIndex = 9
        Instance.new("UICorner", esf).CornerRadius = UDim.new(0,6)
        Instance.new("UIStroke", esf).Color = cor
        local esm = Instance.new("TextButton", esf); esm.BackgroundTransparency = 1
        esm.Position = UDim2.new(0,2,0,0); esm.Size = UDim2.new(0,14,1,0)
        esm.Font = Enum.Font.GothamBlack; esm.Text = "-"; esm.TextColor3 = cor; esm.TextSize = 13; esm.ZIndex = 10
        local esv = Instance.new("TextLabel", esf); esv.BackgroundTransparency = 1
        esv.Position = UDim2.new(0,16,0,0); esv.Size = UDim2.new(1,-30,1,0)
        esv.Font = Enum.Font.GothamBold; esv.Text = tostring(esl.val[1])
        esv.TextColor3 = Color3.fromRGB(255,255,255); esv.TextSize = 9; esv.ZIndex = 10
        local esp = Instance.new("TextButton", esf); esp.BackgroundTransparency = 1
        esp.Position = UDim2.new(1,-14,0,0); esp.Size = UDim2.new(0,14,1,0)
        esp.Font = Enum.Font.GothamBlack; esp.Text = "+"; esp.TextColor3 = cor; esp.TextSize = 13; esp.ZIndex = 10
        esm.MouseButton1Click:Connect(function()
            esl.val[1] = math.max(esl.min, esl.val[1]-esl.step)
            esv.Text = tostring(esl.val[1])
            if esl.onChange then esl.onChange(esl.val[1]) end
        end)
        esp.MouseButton1Click:Connect(function()
            esl.val[1] = math.min(esl.max, esl.val[1]+esl.step)
            esv.Text = tostring(esl.val[1])
            if esl.onChange then esl.onChange(esl.val[1]) end
        end)
    end
    local pill = Instance.new("Frame", row); pill.BackgroundColor3 = Color3.fromRGB(45,50,62)
    pill.BorderSizePixel = 0; pill.Position = UDim2.new(1,-54,0.5,-12); pill.Size = UDim2.new(0,46,0,24); pill.ZIndex = 9
    Instance.new("UICorner", pill).CornerRadius = UDim.new(1,0)
    ...
    btn.Size = UDim2.new(1,0,1,0); btn.Text = ""; btn.ZIndex = 11
    btn.MouseEnter:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(33,36,46)}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(28,30,38)}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        estado = not estado
        TweenService:Create(pill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{
            BackgroundColor3=estado and cor or Color3.fromRGB(45,50,62)
        }):Play()
        TweenService:Create(knob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
...
makePlSec("🎮 CÂMERA & MUNDO", Color3.fromRGB(180,120,255))
makePlRow_Toggle("InfZoom",    Color3.fromRGB(180,120,255), "🔭 Infinito Zoom",
    "Zoom da câmera ilimitado — sem limite de distância",
    function(s) setInfZoom(s) end)
makePlRow_Toggle("CamLivre",   Color3.fromRGB(140,180,255), "👁️ Câmera Livre",
    "Câmera atravessa paredes e objetos",
    function(s) setCamLivre(s) end)
makePlRow_Toggle("FullBright", Color3.fromRGB(255,240,80),  "☀️ FullBright",
    "Noite vira dia — remove chuva, neblina e escuridão",
    function(s) setFullBright(s) end)

makePlSec("⚡ MOVIMENTO", Color3.fromRGB(100,220,160))
makePlRow_Toggle("InfJump",    Color3.fromRGB(100,220,160), "🚀 Infinite Jump",
    "Pula infinitamente no ar sem limite",
    function(s) setInfJump(s) end)
makePlRow_Toggle("SlopeBoost", Color3.fromRGB(255,140,40),  "🛡️ Slope Boost",
    "Imune a armadilhas de desaceleração — velocidade sempre constante",
    function(s) setSlopeBoost(s) end)
makePlRow_Slider("Speed",     Color3.fromRGB(255,180,30),  "⚡ Speed",
    "Velocidade | padrão:16 | min:1 | max:500", 1, 500, 1, 30,
    function(v) playerSpeed=v; applySpeed(v) end)
makePlRow_Slider("JumpPower", Color3.fromRGB(100,220,255), "🦘 Jump Power",
    "Força do pulo | padrão:50 | min:1 | max:999", 1, 999, 1, 80,
    function(v) playerJump=v; applyJump(v) end)

makePlSec("✈️ VOO & NOCLIP", Color3.fromRGB(80,180,255))
makePlRow_Toggle("Fly",    Color3.fromRGB(80,180,255),   "✈️ Fly",
    "W/A/S/D mover • Espaço=subir • Ctrl=descer • [-][+]=velocidade",
    function(s) setFly(s) end, flySpeedRef)
makePlRow_Toggle("Noclip", Color3.fromRGB(140,255,140),  "👻 Noclip",
...
makePlRow_Toggle("TpClick", Color3.fromRGB(255,220,60),  "⚡ TP Click",
    "Clique em qualquer lugar para teleportar instantaneamente",
    function(s) setTpClick(s) end)
makePlRow_Toggle("BauANC",  Color3.fromRGB(210,160,80),  "📦 Baú ANC",
...
local ANIMAL_NAMES = {
    ...
    "alien","elite alien","elitealien","cultist","mega cultist","megacultist",
    "cultist king","cultistking"
}
local ANIMAL_SET = {}
for _, n in ipairs(ANIMAL_NAMES) do ANIMAL_SET[n] = true end

local function findNearestAnimalHrp(fovRadius)
    ...
    local camCenter = Vector2.new(vp.X/2, vp.Y/2)
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            ...
            if fovRadius then
                local sp, onScreen = workspace.CurrentCamera:WorldToViewportPoint(anHrp.Position)
                if not onScreen then return end
                local distScreen = (Vector2.new(sp.X, sp.Y) - camCenter).Magnitude
                if distScreen > fovRadius then return end
            end
...
local function createFOVCircle()
    if aimbotFOVCircle then pcall(function() aimbotFOVCircle:Destroy() end) end
    local size = aimbotFOVRadius * 2
    local circle = Instance.new("Frame", ScreenGui)
    circle.Name = "PudimFOV"
    ...
    dot.BackgroundColor3 = Color3.fromRGB(255,60,60); dot.BorderSizePixel = 0
    dot.AnchorPoint = Vector2.new(0.5, 0.5)
    dot.Position = UDim2.new(0.5,0,0.5,0); dot.Size = UDim2.new(0,5,0,5); dot.ZIndex = 501
...
local function removeFOVCircle()
    if aimbotFOVCircle then
        pcall(function() aimbotFOVCircle:Destroy() end)
        aimbotFOVCircle = nil
    end
end
...
workspace.DescendantAdded:Connect(function(obj)
    ...
    task.defer(function()
        ...
        local fov = aimbotFOVEnabled and aimbotFOVRadius or nil
        local anHrp = findNearestAnimalHrp(fov)
        if not anHrp then return end
        local steps = 0
        local conn; conn = RunService.Heartbeat:Connect(function()
            steps += 1
            if steps > 120 or (not aimbotEnabled and not aimbotFOVEnabled) then
                conn:Disconnect(); return
            end
            if not obj or not obj.Parent or not anHrp or not anHrp.Parent then
                conn:Disconnect(); return
            end
...
local function startAimbotAuto()
    ...
    task.spawn(function()
        while aimbotAutoEnabled do
            task.wait(0.15)
            pcall(function()
                local tool = getEquippedRangedTool(); if not tool then return end
                local anHrp = findNearestAnimalHrp(nil); if not anHrp then return end
                local ch = Player.Character; if not ch then return end
...
local function criarIconeAvancado(parent, key, cor)
    ...
    local function c(x,y,rad)     return mkCircle(ic,x,y,rad,cor) end
    if key == "Aimbot" then
        c(14,14,10); local i1=mkCircle(ic,14,14,7,Color3.fromRGB(20,22,30)); i1.ZIndex=ic.ZIndex+2
        c(14,14,3); r(13,0,2,28,1); r(0,13,28,2,1)
    elseif key == "AimbotFOV" then
        c(14,14,12); local i1=mkCircle(ic,14,14,9,Color3.fromRGB(20,22,30)); i1.ZIndex=ic.ZIndex+2
        c(14,14,4); local i2=mkCircle(ic,14,14,2,Color3.fromRGB(20,22,30)); i2.ZIndex=ic.ZIndex+3
        r(13,1,2,6,1); r(13,21,2,6,1); r(1,13,6,2,1); r(21,13,6,2,1)
    elseif key == "AimbotAuto" then
        r(2,10,24,4,2); c(14,6,5); local i1=mkCircle(ic,14,6,3,Color3.fromRGB(20,22,30)); i1.ZIndex=ic.ZIndex+2
        r(8,14,12,10,4)
    else
        r(4,4,20,20,5); local i1=mkRect(ic,5,5,18,18,Color3.fromRGB(20,22,30),4); i1.ZIndex=ic.ZIndex+2
        r(10,10,8,8,3)
    end
    return ic
end
...
local function makeAvRow_Toggle(icon_key, cor, lbl_txt, desc_txt, onToggle)
    ...
    row.BackgroundColor3 = Color3.fromRGB(28,30,38); row.BorderSizePixel = 0
    row.Size = UDim2.new(1,0,0,68); row.LayoutOrder = avLO(); row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    local rowS = Instance.new("UIStroke", row); rowS.Color = Color3.fromRGB(42,46,58); rowS.Thickness = 1
    local gb = Instance.new("Frame", row); gb.BackgroundColor3 = cor; gb.BackgroundTransparency = 0.9
    gb.BorderSizePixel = 0; gb.Size = UDim2.new(1,0,1,0); gb.ZIndex = 5
    ...
    tl.Text = lbl_txt; tl.TextColor3 = Color3.fromRGB(225,230,248); tl.TextSize = 12
    tl.TextXAlignment = Enum.TextXAlignment.Left; tl.ZIndex = 7
    local td = Instance.new("TextLabel", row); td.BackgroundTransparency = 1
    ...
    statusDot.BorderSizePixel = 0; statusDot.Position = UDim2.new(1,-68,0.5,-4)
    statusDot.Size = UDim2.new(0,8,0,8); statusDot.ZIndex = 9
    Instance.new("UICorner", statusDot).CornerRadius = UDim.new(1,0)
    local pill = Instance.new("Frame", row); pill.BackgroundColor3 = Color3.fromRGB(45,50,62)
    pill.BorderSizePixel = 0; pill.Position = UDim2.new(1,-56,0.5,-12)
    pill.Size = UDim2.new(0,48,0,24); pill.ZIndex = 9
    Instance.new("UICorner", pill).CornerRadius = UDim.new(1,0)
    local knob = Instance.new("Frame", pill); knob.BackgroundColor3 = Color3.fromRGB(160,170,185)
    knob.BorderSizePixel = 0; knob.Position = UDim2.new(0,2,0.5,-10)
    knob.Size = UDim2.new(0,20,0,20); knob.ZIndex = 10
    Instance.new("UICorner", knob).CornerRadius = UDim.new(1,0)
    local estado = false
    local btn = Instance.new("TextButton", row); btn.BackgroundTransparency = 1
    btn.Size = UDim2.new(1,0,1,0); btn.Text = ""; btn.ZIndex = 11
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(statusDot,TweenInfo.new(0.2),{
            BackgroundColor3=estado and Color3.fromRGB(87,242,135) or Color3.fromRGB(80,90,110)
        }):Play()
        onToggle(estado)
    end)
end
...
local function makeAvInfoBox(txt, cor)
    ...
    ft.Font = Enum.Font.Gotham; ft.Text = txt
...
-- BUILD — Avançado Funções
makeAvSec("🎯 COMBATE AUTOMÁTICO", Color3.fromRGB(255,80,80))
makeAvRow_Toggle("Aimbot", Color3.fromRGB(255,80,80),
    "🎯 Aimbot (Teleguiado)",
    "Projéteis se movem automaticamente para o animal mais próximo. 100% acerto.",
    function(s) aimbotEnabled = s end)
makeAvRow_Toggle("AimbotFOV", Color3.fromRGB(255,50,120),
    "🔴 Aimbot FOV (Círculo)",
    "Círculo pequeno no centro da tela. Só mira mobs dentro do FOV. 100% precisão.",
    function(s)
        aimbotFOVEnabled = s
        if s then createFOVCircle() else removeFOVCircle() end
    end)
makeAvRow_Toggle("AimbotAuto", Color3.fromRGB(255,140,40),
    "🤖 Aimbot AUTO",
    "Com arma equipada: atira sozinho nos animais mais próximos sem precisar clicar.",
    function(s)
        aimbotAutoEnabled = s
        if s then startAimbotAuto() end
    end)
makeAvInfoBox(
    "ℹ️  AIMBOT: Funciona com todas as armas de projéteis (Revolver, Rifle, Crossbow, etc).\n"..
    "FOV: círculo vermelho no centro — mira só mobs dentro. Auto: requer VirtualInputManager.",
...
tpPage.Name = "TeleportPage"; tpPage.Size = UDim2.new(1,0,1,0)
tpPage.BackgroundTransparency = 1; tpPage.Visible = false
tpPage.ScrollBarThickness = 3; tpPage.ScrollBarImageColor3 = C_ACCENT
tpPage.BorderSizePixel = 0; tpPage.AutomaticCanvasSize = Enum.AutomaticSize.Y
tpPage.CanvasSize = UDim2.new(0,0,0,0); tpPage.ZIndex = 4
...
Instance.new("UIListLayout", tpPage).Padding = UDim.new(0,10)
Pages["Teleport"] = tpPage

-- Adicionar aba Teleporte na sidebar (grupo EXTRA)
do
    layoutOrder += 1
    local tpBg = Instance.new("Frame", SideBar)
    tpBg.Name = "TeleportTab"; tpBg.BackgroundColor3 = C_BG_HOV
    tpBg.BackgroundTransparency = 1; tpBg.BorderSizePixel = 0
    tpBg.Size = UDim2.new(1,0,0,36); tpBg.LayoutOrder = layoutOrder*100; tpBg.ZIndex = 4
    tpBg.ClipsDescendants = true
    Instance.new("UICorner", tpBg).CornerRadius = UDim.new(0,7)
    local tpBar = Instance.new("Frame", tpBg); tpBar.BackgroundColor3 = Color3.fromRGB(255,200,80)
    tpBar.BorderSizePixel = 0; tpBar.Position = UDim2.new(0,0,0.2,0)
    tpBar.Size = UDim2.new(0,3,0.6,0); tpBar.Visible = false; tpBar.ZIndex = 6
    Instance.new("UICorner", tpBar).CornerRadius = UDim.new(0,2)
    -- Ícone de mapa
    local icC = Instance.new("Frame", tpBg); icC.BackgroundTransparency = 1; icC.BorderSizePixel = 0
    icC.Position = UDim2.new(0,8,0.5,-10); icC.Size = UDim2.new(0,20,0,20); icC.ZIndex = tpBg.ZIndex+2
    mkRect(icC,8,0,4,20,C_ICON_IDLE,2); mkRect(icC,0,8,20,4,C_ICON_IDLE,2); mkCircle(icC,10,10,4,C_ICON_IDLE)
    local tpLabel = Instance.new("TextLabel", tpBg); tpLabel.BackgroundTransparency = 1
    tpLabel.Position = UDim2.new(0,37,0,0); tpLabel.Size = UDim2.new(1,-42,1,0)
    tpLabel.Font = Enum.Font.GothamSemibold; tpLabel.Text = "Teleporte"
    tpLabel.TextColor3 = C_TEXT_OFF; tpLabel.TextSize = 11; tpLabel.TextXAlignment = Enum.TextXAlignment.Left
    tpLabel.TextTruncate = Enum.TextTruncate.AtEnd; tpLabel.ZIndex = 6
    local tpBtn = Instance.new("TextButton", tpBg); tpBtn.BackgroundTransparency = 1
    tpBtn.Size = UDim2.new(1,0,1,0); tpBtn.Text = ""; tpBtn.ZIndex = 7
    local tpEntry = {key="Teleport", bg=tpBg, iconParts={}, label=tpLabel, bar=tpBar}
    table.insert(allTabs, tpEntry)
    tpBtn.MouseEnter:Connect(function()
        if currentTab~="Teleport" then TweenService:Create(tpBg,TweenInfo.new(0.15),{BackgroundTransparency=0.78}):Play() end
    end)
    tpBtn.MouseLeave:Connect(function()
        if currentTab~="Teleport" then TweenService:Create(tpBg,TweenInfo.new(0.15),{BackgroundTransparency=1}):Play() end
    end)
    tpBtn.MouseButton1Click:Connect(function() selectTab("Teleport") end)
end

-- Biomas do jogo (2026)
local BIOMES = {
    {nome="Floresta",  emoji="🌲", cor=Color3.fromRGB(60,180,80),   desc="Bioma inicial — lobos, ursos, recursos básicos",   searchNames={"Forest","Floresta","SpawnArea","Spawn","Green"}},
    {nome="Ártico",    emoji="❄️", cor=Color3.fromRGB(140,200,255), desc="Bioma de neve — mamutes, raposas árticas, ursos polares", searchNames={"Arctic","Artic","Snow","Ice","Tundra","Polar","Winter"}},
    {nome="Vulcão",    emoji="🌋", cor=Color3.fromRGB(255,100,30),  desc="Bioma vulcânico — hellephants, enguias de lava",   searchNames={"Volcano","Volcanic","Lava","Hell","Fire","Vulcao","Magma"}},
    {nome="Pântano",   emoji="🐸", cor=Color3.fromRGB(100,180,80),  desc="Bioma pântano — sapos coloridos, plantas raras",   searchNames={"Swamp","Pantano","Frog","Marsh","Bog","Wetland","Swampy"}},
}
...
local function tpToBiome(biome)
    ...
    if not ch then showNotif("Personagem não encontrado!", "erro"); return end
    local hrp = ch:FindFirstChild("HumanoidRootPart")
    if not hrp then showNotif("Erro de teleporte!", "erro"); return end
    local foundPos = nil
    ...
    if foundPos then
        ...
        showNotif("Bioma não encontrado — explore o mapa! 🗺️", "aviso")
    end
end
...
-- Header
local bHeader = Instance.new("Frame", tpPage)
bHeader.BackgroundColor3 = Color3.fromRGB(22,24,30); bHeader.BorderSizePixel = 0
bHeader.Size = UDim2.new(1,0,0,58); bHeader.LayoutOrder = tplo(); bHeader.ZIndex = 5
Instance.new("UICorner", bHeader).CornerRadius = UDim.new(0,12)
local bhStroke = Instance.new("UIStroke", bHeader); bhStroke.Color = Color3.fromRGB(255,200,80); bhStroke.Thickness = 1.5
local bhTitle = Instance.new("TextLabel", bHeader); bhTitle.BackgroundTransparency = 1
bhTitle.Position = UDim2.new(0,14,0,0); bhTitle.Size = UDim2.new(1,-14,0,30)
bhTitle.Font = Enum.Font.GothamBlack; bhTitle.Text = "🗺️  BIOMES"
bhTitle.TextColor3 = Color3.fromRGB(255,200,80); bhTitle.TextSize = 14
bhTitle.TextXAlignment = Enum.TextXAlignment.Left; bhTitle.ZIndex = 6
local bhSub = Instance.new("TextLabel", bHeader); bhSub.BackgroundTransparency = 1
bhSub.Position = UDim2.new(0,14,0,30); bhSub.Size = UDim2.new(1,-14,0,22)
bhSub.Font = Enum.Font.Gotham; bhSub.Text = "Selecione um bioma e teleporte instantaneamente"
bhSub.TextColor3 = Color3.fromRGB(100,115,135); bhSub.TextSize = 9
bhSub.TextXAlignment = Enum.TextXAlignment.Left; bhSub.ZIndex = 6

-- Linha de seleção
local selRow = Instance.new("Frame", tpPage)
selRow.BackgroundColor3 = Color3.fromRGB(28,30,38); selRow.BorderSizePixel = 0
selRow.Size = UDim2.new(1,0,0,52); selRow.LayoutOrder = tplo(); selRow.ZIndex = 5
Instance.new("UICorner", selRow).CornerRadius = UDim.new(0,10)
Instance.new("UIStroke", selRow).Color = Color3.fromRGB(50,55,70)
local selLabel = Instance.new("TextLabel", selRow); selLabel.BackgroundTransparency = 1
selLabel.Position = UDim2.new(0,14,0,0); selLabel.Size = UDim2.new(0,60,1,0)
selLabel.Font = Enum.Font.GothamSemibold; selLabel.Text = "Selecionar"
selLabel.TextColor3 = Color3.fromRGB(150,160,180); selLabel.TextSize = 10
selLabel.TextXAlignment = Enum.TextXAlignment.Left; selLabel.ZIndex = 6

-- Botão dropdown
local dropBtn = Instance.new("TextButton", selRow)
dropBtn.BackgroundColor3 = Color3.fromRGB(40,44,56); dropBtn.BorderSizePixel = 0
dropBtn.Position = UDim2.new(0,78,0.5,-16); dropBtn.Size = UDim2.new(1,-86,0,32)
dropBtn.Font = Enum.Font.GothamBold; dropBtn.Text = "🗺️  Biomes ▾"
dropBtn.TextColor3 = Color3.fromRGB(200,210,255); dropBtn.TextSize = 11; dropBtn.ZIndex = 7
...
dropMenu.BackgroundColor3 = Color3.fromRGB(20,22,30); dropMenu.BorderSizePixel = 0
dropMenu.Visible = false; dropMenu.ZIndex = 600; dropMenu.Size = UDim2.new(0,0,0,0)
...
dmList.Padding = UDim.new(0,4); dmList.HorizontalAlignment = Enum.HorizontalAlignment.Center
...
local tpBiomeBtn -- declarado aqui para o closure dos items funcionar

local function closeDropdown()
    dropOpen = false
    TweenService:Create(dropMenu,TweenInfo.new(0.18,Enum.EasingStyle.Quad),{Size=UDim2.new(0,200,0,0)}):Play()
    task.delay(0.19, function() dropMenu.Visible = false end)
    TweenService:Create(dropBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(40,44,56)}):Play()
end
...
local function openDropdown()
    ...
    dropMenu.Position = UDim2.new(0, abs.X, 0, abs.Y + absS.Y + 6)
    dropMenu.Size = UDim2.new(0,200,0,0); dropMenu.Visible = true
    local h = #BIOMES * 50 + 24
    TweenService:Create(dropMenu,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,200,0,h)}):Play()
    TweenService:Create(dropBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(55,60,80)}):Play()
end

-- Itens do dropdown
for _, biome in ipairs(BIOMES) do
    local item = Instance.new("TextButton", dropMenu)
    item.BackgroundColor3 = Color3.fromRGB(28,32,42); item.BorderSizePixel = 0
    item.Size = UDim2.new(1,0,0,42); item.Text = ""; item.ZIndex = 605
    Instance.new("UICorner", item).CornerRadius = UDim.new(0,8)
    local ibg = Instance.new("Frame", item); ibg.BackgroundColor3 = biome.cor
    ibg.BackgroundTransparency = 0.88; ibg.BorderSizePixel = 0; ibg.Size = UDim2.new(1,0,1,0); ibg.ZIndex = 601
    Instance.new("UICorner", ibg).CornerRadius = UDim.new(0,8)
    local ibar = Instance.new("Frame", item); ibar.BackgroundColor3 = biome.cor; ibar.BorderSizePixel = 0
    ibar.Position = UDim2.new(0,0,0.15,0); ibar.Size = UDim2.new(0,3,0.7,0); ibar.ZIndex = 603
    Instance.new("UICorner", ibar).CornerRadius = UDim.new(0,2)
    local emo = Instance.new("TextLabel", item); emo.BackgroundTransparency = 1
    emo.Position = UDim2.new(0,8,0,0); emo.Size = UDim2.new(0,28,1,0)
    emo.Font = Enum.Font.GothamBold; emo.Text = biome.emoji; emo.TextSize = 16; emo.ZIndex = 602
    emo.TextColor3 = Color3.fromRGB(255,255,255)
    local nm = Instance.new("TextLabel", item); nm.BackgroundTransparency = 1
    nm.Position = UDim2.new(0,38,0,4); nm.Size = UDim2.new(1,-46,0,18)
    nm.Font = Enum.Font.GothamBold; nm.Text = biome.nome; nm.TextColor3 = biome.cor; nm.TextSize = 12
    nm.TextXAlignment = Enum.TextXAlignment.Left; nm.ZIndex = 602
    local ds = Instance.new("TextLabel", item); ds.BackgroundTransparency = 1
    ds.Position = UDim2.new(0,38,0,22); ds.Size = UDim2.new(1,-46,0,14)
    ds.Font = Enum.Font.Gotham; ds.Text = biome.desc; ds.TextColor3 = Color3.fromRGB(100,115,135); ds.TextSize = 8
    ds.TextXAlignment = Enum.TextXAlignment.Left; ds.ZIndex = 602
    item.MouseEnter:Connect(function() TweenService:Create(item,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(35,40,55)}):Play() end)
    item.MouseLeave:Connect(function() TweenService:Create(item,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(28,32,42)}):Play() end)
    local bRef = biome
    item.MouseButton1Click:Connect(function()
        selectedBiome = bRef
        dropBtn.Text = bRef.emoji .. "  " .. bRef.nome .. " ▾"
        dropBtn.TextColor3 = Color3.fromRGB(255,255,255)
        TweenService:Create(dropBtn,TweenInfo.new(0.2),{BackgroundColor3=bRef.cor}):Play()
        if tpBiomeBtn then
            tpBiomeBtn.Text = "🚀  Tp> " .. bRef.nome
            TweenService:Create(tpBiomeBtn,TweenInfo.new(0.25,Enum.EasingStyle.Back),{BackgroundColor3=bRef.cor}):Play()
        end
        closeDropdown()
        showNotif(bRef.emoji .. " " .. bRef.nome .. " selecionado!", "info")
    end)
end
...
UserInputService.InputBegan:Connect(function(input)
    ...
    local inD = mp.X>=ap.X and mp.X<=ap.X+as.X and mp.Y>=ap.Y and mp.Y<=ap.Y+as.Y
    local inB = mp.X>=bp.X and mp.X<=bp.X+bs.X and mp.Y>=bp.Y and mp.Y<=bp.Y+bs.Y
    if not inD and not inB then closeDropdown() end
end)

-- Linha TP
local tpRow = Instance.new("Frame", tpPage)
tpRow.BackgroundColor3 = Color3.fromRGB(22,24,30); tpRow.BorderSizePixel = 0
tpRow.Size = UDim2.new(1,0,0,52); tpRow.LayoutOrder = tplo(); tpRow.ZIndex = 5
...
tpBiomeBtn.BackgroundColor3 = Color3.fromRGB(55,60,80); tpBiomeBtn.BorderSizePixel = 0
tpBiomeBtn.Position = UDim2.new(0,10,0.5,-18); tpBiomeBtn.Size = UDim2.new(1,-20,0,36)
tpBiomeBtn.Font = Enum.Font.GothamBlack; tpBiomeBtn.Text = "🚀  Selecione um Bioma acima ▲"
tpBiomeBtn.TextColor3 = Color3.fromRGB(180,190,220); tpBiomeBtn.TextSize = 12; tpBiomeBtn.ZIndex = 7
...
confirmFrame.BackgroundColor3 = Color3.fromRGB(0,0,0); confirmFrame.BackgroundTransparency = 0.45
...
confirmBox.BackgroundColor3 = Color3.fromRGB(22,24,30); confirmBox.BorderSizePixel = 0
confirmBox.AnchorPoint = Vector2.new(0.5,0.5); confirmBox.Position = UDim2.new(0.5,0,0.5,0)
confirmBox.Size = UDim2.new(0,300,0,140); confirmBox.ZIndex = 701
...
local cTitle = Instance.new("TextLabel", confirmBox); cTitle.BackgroundTransparency = 1
cTitle.Position = UDim2.new(0,0,0,16); cTitle.Size = UDim2.new(1,0,0,20)
cTitle.Font = Enum.Font.GothamBold; cTitle.Text = "Você deseja viajar para?"
cTitle.TextColor3 = Color3.fromRGB(180,185,200); cTitle.TextSize = 11; cTitle.ZIndex = 702

local cDest = Instance.new("TextLabel", confirmBox); cDest.BackgroundTransparency = 1
cDest.Position = UDim2.new(0,0,0,38); cDest.Size = UDim2.new(1,0,0,34)
cDest.Font = Enum.Font.GothamBlack; cDest.Text = "🌲 Floresta"
cDest.TextColor3 = Color3.fromRGB(255,200,80); cDest.TextSize = 20; cDest.ZIndex = 702

local cYes = Instance.new("TextButton", confirmBox)
cYes.BackgroundColor3 = Color3.fromRGB(50,190,90); cYes.BorderSizePixel = 0
cYes.Position = UDim2.new(0,16,1,-50); cYes.Size = UDim2.new(0.5,-22,0,34)
cYes.Font = Enum.Font.GothamBold; cYes.Text = "✔  Sim"; cYes.TextColor3 = Color3.fromRGB(255,255,255)
cYes.TextSize = 13; cYes.ZIndex = 703
Instance.new("UICorner", cYes).CornerRadius = UDim.new(0,9)

local cNo = Instance.new("TextButton", confirmBox)
cNo.BackgroundColor3 = Color3.fromRGB(200,55,55); cNo.BorderSizePixel = 0
cNo.Position = UDim2.new(0.5,6,1,-50); cNo.Size = UDim2.new(0.5,-22,0,34)
cNo.Font = Enum.Font.GothamBold; cNo.Text = "✖  Não"; cNo.TextColor3 = Color3.fromRGB(255,255,255)
cNo.TextSize = 13; cNo.ZIndex = 703
Instance.new("UICorner", cNo).CornerRadius = UDim.new(0,9)

local function showConfirm(biome)
    cDest.Text = biome.emoji .. "  " .. biome.nome
    cDest.TextColor3 = biome.cor
    cbStroke.Color = biome.cor
    cTitle.Text = "Você deseja viajar para " .. biome.nome .. "?"
...
local function hideConfirm()
    TweenService:Create(confirmBox,TweenInfo.new(0.2),{Size=UDim2.new(0,0,0,0)}):Play()
...
cNo.MouseButton1Click:Connect(function() hideConfirm() end)

tpBiomeBtn.MouseButton1Click:Connect(function()
    if not selectedBiome then showNotif("Selecione um bioma primeiro!", "aviso"); return end
    showConfirm(selectedBiome)
end)
tpBiomeBtn.MouseEnter:Connect(function()
    TweenService:Create(tpBiomeBtn,TweenInfo.new(0.12),{BackgroundTransparency=0.1}):Play()
end)
tpBiomeBtn.MouseLeave:Connect(function()
    TweenService:Create(tpBiomeBtn,TweenInfo.new(0.12),{BackgroundTransparency=0}):Play()
end)

-- Info box
local tpInfo = Instance.new("Frame", tpPage)
tpInfo.BackgroundColor3 = Color3.fromRGB(14,20,35); tpInfo.BackgroundTransparency = 0.5
tpInfo.BorderSizePixel = 0; tpInfo.Size = UDim2.new(1,0,0,68); tpInfo.LayoutOrder = tplo(); tpInfo.ZIndex = 4
Instance.new("UICorner", tpInfo).CornerRadius = UDim.new(0,8)
local tiStroke = Instance.new("UIStroke", tpInfo); tiStroke.Color = Color3.fromRGB(255,200,80); tiStroke.Thickness = 1
local tiLbl = Instance.new("TextLabel", tpInfo); tiLbl.BackgroundTransparency = 1
tiLbl.Position = UDim2.new(0,10,0,6); tiLbl.Size = UDim2.new(1,-20,1,-12)
tiLbl.Font = Enum.Font.Gotham
tiLbl.Text = "🗺️  BIOMAS 99 DIAS NA FLORESTA 2026\n🌲 Floresta • ❄️ Ártico • 🌋 Vulcão • 🐸 Pântano\n⚠️  Se 'Bioma não encontrado' — explore o mapa primeiro!"
tiLbl.TextColor3 = Color3.fromRGB(200,180,100); tiLbl.TextSize = 8; tiLbl.TextWrapped = true
tiLbl.TextXAlignment = Enum.TextXAlignment.Left; tiLbl.TextYAlignment = Enum.TextYAlignment.Top; tiLbl.ZIndex = 5

-- ══════════════════════════════════════════════════════════
--  ABA INICIAL + LOG FINAL
-- ══════════════════════════════════════════════════════════
task.wait(0.05)
selectTab("Info")

print("╔══════════════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v5.0 — COMPLETO  Fev 2026                        ║")
print("╠══════════════════════════════════════════════════════════════╣")
print("║  ✓ NOVO: InfZoom | CamLivre | FullBright | InfiniteJump     ║")
print("║  ✓ NOVO: SlopeBoost | Aimbot FOV | Aba Teleporte/Biomas     ║")
print("║  ✓ NOVO: Sistema de Notificações Toast                       ║")
print("║  ✓ Player: Speed | JumpPower | Fly | Noclip | TpClick | ANC ║")
