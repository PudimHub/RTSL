...
local function executarBring(key)
    ...
    -- Aguarda 3 segundos antes de iniciar (solicitado pelo usuário)
    task.wait(3)
    ...
    for i, entry in ipairs(eligiveis) do
        ...
        pcall(function()
            ...
            local radius = 2.5 + math.random() * 3.5
            local offsetX = math.cos(angle) * radius
            local offsetZ = math.sin(angle) * radius

            -- ── DETECÇÃO DO CHÃO REAL via Raycast ──────────────────
            -- Lança raio de 30 studs acima para baixo e encontra a
            -- superfície real (funciona em terreno irregular)
            local groundY = cf.Position.Y - 2.5  -- fallback
            pcall(function()
                ...
                local result = workspace:Raycast(rayOrigin, Vector3.new(0,-100,0), rayParamsBring)
                if result then
                    groundY = result.Position.Y
                end
            end)

            -- Coloca 1.5 studs ACIMA do chão → cai e pousa naturalmente
            ...
            -- Solta o item completamente livre no espaço
            pcall(function() targetPart.Anchored = false end)
            targetPart.CanCollide  = true
            targetPart.CFrame      = CFrame.new(target)
            pcall(function() targetPart.AssemblyLinearVelocity = Vector3.zero end)
            pcall(function() targetPart.Velocity = Vector3.zero end)

            -- Para modelos com múltiplas peças, mantém agrupadas
                ...
                for _,bp in ipairs(obj:GetDescendants()) do
                    if bp:IsA("BasePart") and bp ~= targetPart then
                        pcall(function()
                            ...
                            pcall(function() bp.AssemblyLinearVelocity = Vector3.zero end)
                        end)
                    end
                end
            end
...
do
    ...
    baBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            -- Delay de 3 segundos antes de trazer
            task.wait(3)
            ...
            local pchars={}; for _,pl in ipairs(Players:GetPlayers()) do if pl.Character then pchars[pl.Character]=true end end
            local ok,descs=pcall(function() return workspace:GetDescendants() end)
            -- RaycastParams para detecção de chão
            ...
            if ok then
                ...
                for idx, eEntry in ipairs(eligiveis) do
                    pcall(function()
                        ...
                        -- Raycast para detectar o chão real
                        local offsetX = math.cos(angle)*radius
                        local offsetZ = math.sin(angle)*radius
                        ...
                        -- Solta o item completamente livre
                        pcall(function() targetPart.Anchored = false end)
                        targetPart.CanCollide  = true
                        targetPart.CFrame      = CFrame.new(target)
                        pcall(function() targetPart.AssemblyLinearVelocity = Vector3.zero end)
...
local function setGodMod(state)
    ...
    if state then
        ...
        local function applyGodModToChar(ch)
            ...
            -- MaxHealth e HP altos
            pcall(function()
                hum.MaxHealth = 1e6
                hum.Health    = 1e6
            end)

            -- ── CAMADA 2: __newindex no Humanoid — bloqueia Health=0 ──────
            -- Intercepta atribuição direta de propriedade no Humanoid.
            -- Bloqueia qualquer tentativa de reduzir Health abaixo de MaxHealth/2.
            pcall(function()
                if not getrawmetatable or not setreadonly then return end
                godMtHum = getrawmetatable(hum)
                setreadonly(godMtHum, false)
                if not godOrigNewindex then
                    godOrigNewindex = godMtHum.__newindex
                end
                local origNI = godMtHum.__newindex
                godMtHum.__newindex = newcclosure(function(self, k, v)
                    if godModEnabled and k == "Health" then
                        local ch2 = Player.Character
                        if ch2 and self == ch2:FindFirstChildWhichIsA("Humanoid") then
                            if type(v) == "number" and v < self.MaxHealth * 0.5 then
                                return  -- bloqueia redução de HP
                            end
                        end
                    end
                    return origNI(self, k, v)
                end)
                setreadonly(godMtHum, true)
            end)

            -- Died → cancela estado de morte
            local diedConn
        ...
        -- ── CAMADA 3: Heartbeat ultra-rápido ──────────────────────────────
        -- Restaura HP a QUALQUER redução — MaxHealth = 1e9 absorve danos altos
        godModConn = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                -- Garante MaxHealth enorme a cada frame (servidor pode resetar)
                if h2.MaxHealth < 1e9 then h2.MaxHealth = 1e9 end
                -- Restaura instantaneamente qualquer redução de HP
                if h2.Health < 1e9 then h2.Health = 1e9 end
                -- Bloqueia estado de morte
        ...
        -- Restaura HP e configurações normais
        pcall(function()
            local ch = Player.Character; if not ch then return end
            local hum = ch:FindFirstChildWhichIsA("Humanoid"); if not hum then return end
...
buildPanelCard.Size = UDim2.new(1,0,0,260); buildPanelCard.LayoutOrder = tpNextLO(); buildPanelCard.ZIndex = 5
...
buildScroll.Size = UDim2.new(1,-12,0,204); buildScroll.ZIndex = 6
...
local function scanBuildings(isRefresh)
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(descs) do
            ...
            -- Filtro: armadilhas e BAÚS fora — não são construções
            if oNameLow:find("trap",1,true) or oNameLow:find("spike",1,true)
            or oNameLow:find("snare",1,true) or oNameLow:find("stake",1,true)
            or oNameLow:find("chest",1,true) or oNameLow:find("bau",1,true)
            or oNameLow:find("baú",1,true) then continue end
        ...
        buildPanelCard.Size = UDim2.new(1,0,0, math.clamp(48 + math.min(contentH, 212), 100, 260))
        buildScroll.Size    = UDim2.new(1,-12,0, math.clamp(math.min(contentH, 212), 40, 212))
        scanRunning = false
    end)
end
...
btnClear.MouseButton1Click:Connect(function()
    ...
    buildPanelCard.Size = UDim2.new(1,0,0,260)
    buildScroll.Size = UDim2.new(1,-12,0,204)
...
chestPanelCard.Size = UDim2.new(1,0,0,260); chestPanelCard.LayoutOrder = tpNextLO(); chestPanelCard.ZIndex = 5
...
chestScroll.Position = UDim2.new(0,6,0,48); chestScroll.Size = UDim2.new(1,-12,0,204); chestScroll.ZIndex = 6
...
local function scanChests(isRefresh)
    ...
    task.spawn(function()
        ...
        chestPanelCard.Size = UDim2.new(1,0,0, math.clamp(48 + math.min(contentH, 212), 100, 260))
        chestScroll.Size    = UDim2.new(1,-12,0, math.clamp(math.min(contentH, 212), 40, 212))
        chestScanRun = false
    end)
end
...
chestBtnClr.MouseButton1Click:Connect(function()
    ...
    chestPanelCard.Size = UDim2.new(1,0,0,260)
    chestScroll.Size    = UDim2.new(1,-12,0,204)
...
-- ══════════════════════════════════════════════════════════════
-- KILL AURA v4 — Sistema correto para 99 Nights in the Forest
-- ══════════════════════════════════════════════════════════════
--
-- POR QUE O v3 FALHOU:
--   "Bring Mob" e "Hitbox Expansion" no CLIENTE não replicam para
--   o SERVIDOR. O servidor ignora completamente essas mudanças.
--   O 99 Nights valida dano por colisão física SERVER-SIDE.
--
-- SOLUÇÃO REAL:
--   firetouchinterest(toolPart, mobPart, 0/1):
--     Simula uma colisão física entre toolPart e mobPart.
--     O evento Touched da tool dispara com o mob como argumento.
--     O jogo processa esse toque IGUAL a uma colisão real.
--     Isso é CLIENT-SIDE mas o handler do jogo roda localmente e
--     envia o dano ao servidor via RemoteEvent interno.
--
--   MODO AUTO (sem clicar):
--     Loop RunService.Heartbeat a cada 0.4s.
--     Pega automaticamente a tool equipada.
--     Simula toque com todos os mobs no range.
--     Você não precisa fazer nada além de equipar a ferramenta.
--
--   FALLBACK (sem firetouchinterest):
--     Ativa a tool automaticamente via :Activate() em loop.
--     Menos eficaz mas funciona em executores sem FTI.
-- ══════════════════════════════════════════════════════════════
...
-- Verifica se o executor suporta firetouchinterest
local hasFTI = false
pcall(function()
...
if not hasFTI then
    pcall(function()
        hasFTI = rawget(getfenv(0), "firetouchinterest") ~= nil
    end)
end

-- Pega a Part da tool que faz o dano (Handle é o padrão Roblox)
...
local function getMobsInRange(hrp, range)
    ...
    for _, src in ipairs({
        ...
        workspace:FindFirstChild("LiveObjects"),
        workspace:FindFirstChild("Enemies"),
        workspace,
    }) do
        if src then
            for _, obj in ipairs(src:GetChildren()) do pcall(tryAdd, obj) end
        end
    end
    return mobs
end

-- ── MÉTODO 1: firetouchinterest (PRINCIPAL) ─────────────────────
-- Simula colisão física entre Handle da tool e cada part do mob.
-- O handler Touched do jogo dispara naturalmente.
local function attackWithFTI(handle, mobs)
    for _, mob in ipairs(mobs) do
        pcall(function()
            -- Pega as partes principais do mob (HRP + Torso são suficientes)
            local parts = {}
            local hrp2 = mob:FindFirstChild("HumanoidRootPart")
            local torso = mob:FindFirstChild("Torso") or mob:FindFirstChild("UpperTorso")
            if hrp2 then table.insert(parts, hrp2) end
            if torso then table.insert(parts, torso) end
            -- Se não achou, pega todas as BaseParts
            if #parts == 0 then
            ...
            for _, part in ipairs(parts) do
                pcall(function() firetouchinterest(handle, part, 0) end)  -- begin touch
                pcall(function() firetouchinterest(handle, part, 1) end)  -- end touch
            end
        end)
    end
end

-- ── MÉTODO 2: Activate em loop (FALLBACK) ──────────────────────
-- Para executores sem FTI. Ativa a tool 3x em sequência rápida.
-- Combinado com o range, aumenta chance de acertar mobs próximos.
local function attackWithActivate(tool, mob)
...
local function startKillAura()
    ...
    local lastAttack = 0
    local interval   = 0.4  -- segundos entre ataques

    kaAutoLoop = RunService.Heartbeat:Connect(function()
        if not kaEnabled then return end
        local now = tick()
        if now - lastAttack < interval then return end
        lastAttack = now
        ...
        pcall(function()
            ...
            if hasFTI and handle then
                -- Método principal: firetouchinterest
                attackWithFTI(handle, mobs)
            else
                -- Fallback: Activate em loop
                for _, mob in ipairs(mobs) do
                    pcall(attackWithActivate, tool, mob)
                end
            end
        end)
    end)
    ...
    kaCharConn = Player.CharacterAdded:Connect(function()
        -- Sem ação necessária, o loop pega automaticamente o novo personagem
    end)
end
...
local function autoTriturador()
    ...
    -- Coleta jogadores para exclusão
    local pchars = {}
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl.Character then pchars[pl.Character] = true end
    end

    -- Varre workspace por itens elegíveis
    local encontrados = {}
    local alreadySeen = {}
    local ok, descs = pcall(function() return workspace:GetDescendants() end)
    if not ok then atRunning=false; return end
    ...
    for _, obj in ipairs(descs) do
        pcall(function()
            ...
            -- Exclui o próprio triturador
            if obj == shredder or (shredder:IsA("Model") and shredder:IsAncestorOf(obj)) then return end

            -- Só aceita itens da lista
    ...
    Notify.send({type="custom", icon="⚙️", accent=AT_COR,
        title="Auto Triturador",
        msg=string.format("Enviando %d item(s) para o triturador!", #encontrados),
        duration=4})

    local total   = #encontrados
    local ALTURA  = 6      -- studs acima do triturador
    local RAIO    = 1.0    -- raio pequeno: cai direto dentro do triturador

    -- Função auxiliar: destrói constraints e solta item, insere BodyVelocity
    local function soltarItem(part, obj, targetPos)
        -- 1. Remove constraints/forces que prendem o item
        ...
        -- 3. Destrói Welds/WeldConstraints que possam prender o item
        for _, v in ipairs(obj:GetDescendants()) do
            pcall(function()
        ...
        -- 4. Solta completamente
        pcall(function() part.Anchored = false end)
        part.CanCollide = true
        part.Massless   = false

        -- 5. Posiciona acima do triturador
        part.CFrame = CFrame.new(targetPos)

        -- 6. BodyVelocity temporário para forçar queda (funciona sem network ownership)
        pcall(function()
            local bv = Instance.new("BodyVelocity")
            bv.Velocity    = Vector3.new(0, -50, 0)  -- força forte para baixo
            bv.MaxForce    = Vector3.new(0, 1e6, 0)
            bv.P           = 1e4
            bv.Name        = "AT_Fall"
            bv.Parent      = part
            -- Remove após 1 segundo — física normal assume
            task.delay(1, function() pcall(function() bv:Destroy() end) end)
        end)

        -- 7. Multi-parte: move Model inteiro junto
        if obj:IsA("Model") then
            local offset = targetPos - part.Position
            for _, bp in ipairs(obj:GetDescendants()) do
                if bp:IsA("BasePart") and bp ~= part then
                    pcall(function()
                        bp.Anchored   = false
    ...
    for i, entry in ipairs(encontrados) do
        pcall(function()
            ...
            local angle    = (i / math.max(total, 1)) * math.pi * 2
            local dist     = math.random() * RAIO
            local dropPos  = Vector3.new(
                shredPos.X + math.cos(angle) * dist,
                shredPos.Y + ALTURA,
                shredPos.Z + math.sin(angle) * dist)

            soltarItem(part, obj, dropPos)
        end)

        if i % 10 == 0 then task.wait(0.03) end
    end

    Notify.send({type="custom", icon="✅", accent=Color3.fromRGB(87,242,135),
        title="Auto Triturador — Concluído!",
        msg=string.format("%d item(s) enviados para o triturador!", total),
        duration=5})
...
local function aeReadCampfireLevel()
    -- Tenta ler o nível diretamente do Model da fogueira
    local level = 0
    pcall(function()
        local camp = _campfirePosCache and workspace:FindFirstChild("Campfire",true)
                                        or workspace:FindFirstChild("MainCampfire",true)
                                        or workspace:FindFirstChild("Fogueira",true)
        -- Busca mais ampla se não achou pelo nome
        if not camp then
            for _, obj in ipairs(workspace:GetChildren()) do
                if obj:IsA("Model") then
                    local n = obj.Name:lower()
                    if n:find("campfire",1,true) or n:find("fogueira",1,true)
                    or n:find("mainfire",1,true) or n:find("camp_fire",1,true) then
                        camp = obj
                        break
                    end
                end
        ...
        local attrs = {"Level","FireLevel","CampfireLevel","Tier","campLevel","fire_level","Level_Number","CurrentLevel"}
...
local function aeComputeMaxRadius()
    ...
    -- FogEnd nos jogos 99 Nights: cada nível tem um valor específico
    -- Nível 1 ≈ 106   Nível 2 ≈ 188   Nível 3 ≈ 270
    -- Nível 4 ≈ 352   Nível 5 ≈ 423   Nível 6 ≈ 505  (ou 9e9 = sem névoa)
    -- Fator 0.85 → converte FogEnd → raio real explorado

    -- Identifica o nível pelo FogEnd com faixas
    local function fogToLevel(fe)
        if fe <= 0    then return 0 end
        if fe < 150   then return 1 end
        if fe < 230   then return 2 end
        if fe < 310   then return 3 end
        if fe < 390   then return 4 end
        if fe < 465   then return 5 end
        if fe < 9000  then return 6 end
        return 6  -- sem névoa = mapa completo
    end

    -- FONTE 1: FogEnd válido (entre 50 e 9000)
    if fogEnd >= 50 and fogEnd < 9e8 then
        local detectedLvl = fogToLevel(fogEnd)
        if detectedLvl >= 1 then
            -- Usa a tabela de raio real por nível (mais preciso que * fator)
            local radius = AE_LEVEL_RADIUS[detectedLvl]
            return radius, fogEnd, "FogEnd·Nv"..detectedLvl
        end
        -- FogEnd existe mas nível não detectado → usa proporção
        local radius = math.max(math.floor(fogEnd * AE_FOG_FACTOR), AE_LEVEL_RADIUS[1])
        return radius, fogEnd, "FogEnd·%"
    end

    -- FogEnd = 0 ou inválido → FONTE 2: atributo do model
    local lvl = aeReadCampfireLevel()
    if lvl >= 1 then
        return AE_LEVEL_RADIUS[math.clamp(lvl, 1, 6)], fogEnd, "CampLvl"..lvl
    end
...
local function startAutoExplore()
    ...
    -- Gera lista de waypoints na BORDA DA NÉVOA em anel
    -- Divide 360° em setores e visita cada ponto na borda
    -- Começa de 80% do raio (já gerado) e vai até 100% (borda da névoa)
    local AE_SECTORS   = 36         -- pontos ao redor (a cada 10°)
    local AE_RINGS     = 3          -- anéis concêntricos (80%, 90%, 100% do raio)
    local waypoints    = {}

    -- Monta waypoints do anel externo para o interno
    -- (explora borda primeiro, depois consolida)
    for ring = AE_RINGS, 1, -1 do
        local ringPct = 0.65 + (ring / AE_RINGS) * 0.35  -- 0.77 → 0.88 → 1.0
        local r = maxRadius * ringPct
        for s = 0, AE_SECTORS - 1 do
            -- Alterna sentido por anel para cobertura eficiente
            local sectorIdx = (ring % 2 == 0) and s or (AE_SECTORS - 1 - s)
            local angle = (sectorIdx / AE_SECTORS) * math.pi * 2
            table.insert(waypoints, Vector3.new(
                campPos.X + math.cos(angle) * r,
                aeHeight,
                campPos.Z + math.sin(angle) * r))
        end
    end
    ...
    aeBodyVel.MaxForce = Vector3.new(1e6,1e6,1e6)
    aeBodyVel.Velocity = Vector3.zero
    aeBodyVel.Name     = "AE_BodyVel"
    aeBodyGyro = Instance.new("BodyGyro", hrp)
    aeBodyGyro.MaxTorque = Vector3.new(1e6,1e6,1e6)
    aeBodyGyro.D = 50; aeBodyGyro.CFrame = hrp.CFrame; aeBodyGyro.Name = "AE_BodyGyro"
    ...
    Notify.send({type="custom", icon="🗺️", accent=AE_COR,
        ...
        msg=string.format("🔥 Nível %d  •  Raio: %d studs  •  %d pontos na borda da névoa",
            campLvl, maxRadius, totalWP),
        duration=5})

    -- Monitor FogEnd em tempo real (fogueira pode subir de nível durante exploração)
    aeFogConn = game:GetService("Lighting"):GetPropertyChangedSignal("FogEnd"):Connect(function()
        ...
        pcall(function()
            ...
            if newRadius > aeDynMaxRadius + 10 then
                local oldRadius = aeDynMaxRadius
                aeDynMaxRadius  = newRadius
                -- Recalcula waypoints para o novo raio
                waypoints = {}
                for ring = AE_RINGS, 1, -1 do
                    local ringPct = 0.65 + (ring / AE_RINGS) * 0.35
                    local r = newRadius * ringPct
                    for s = 0, AE_SECTORS - 1 do
                        local sectorIdx = (ring % 2 == 0) and s or (AE_SECTORS - 1 - s)
                        local angle = (sectorIdx / AE_SECTORS) * math.pi * 2
                        table.insert(waypoints, Vector3.new(
                ...
                Notify.send({type="custom", icon="🔥", accent=Color3.fromRGB(255,200,80),
                    ...
                    msg=string.format("Raio: %d → %d studs — Waypoints atualizados!", oldRadius, newRadius),
                    duration=5})
            end
        end)
    end)
    ...
    aeConn = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            -- Re-checa nível a cada 5 segundos
            if tick() - lastFogCheck > 5 then
                lastFogCheck = tick()
                aeRefreshLevel()
            end

            -- Waypoint atual
            if curWP > #waypoints then
                -- CONCLUÍDO
                ...
                Notify.send({type="custom", icon="✅", accent=Color3.fromRGB(87,242,135),
                    ...
                    msg=string.format("Nível %d explorado em %s! (%d pontos visitados)",
                        aeCampLevel, aeFormatTime(totalTime), totalWP),
                    duration=8})
                return
            end

            local targetPos = waypoints[curWP]
            local dir  = targetPos - hrp2.Position
            local dist = dir.Magnitude

            if dist > 4 then
                -- Voa em direção ao waypoint
                aeBodyVel.Velocity = dir.Unit * AE_SPEED
                -- Orienta o personagem na direção do movimento
                if dir.Magnitude > 1 then
                    aeBodyGyro.CFrame = CFrame.new(hrp2.Position,
            ...
            -- Progresso e timer
            local pct = math.clamp((curWP - 1) / math.max(totalWP, 1), 0, 1)
            aeUpdateUI(pct)
            pcall(function()
                if aeTimerLabel then
                    local el = math.floor(tick()-aeStartTime)
                    local m,s = math.floor(el/60), el%60
                    aeTimerLabel.Text = string.format("%d:%02d  %.0f%%  •  Ponto %d / %d",
                        m, s, pct*100, curWP, totalWP)
                end
            end)
        end)
    end)
...
-- Card principal (altura aumentada para 134px para caber o level label)
aeCard = Instance.new("Frame", Pages["Farm"])
aeCard.BackgroundColor3 = Color3.fromRGB(16,22,32); aeCard.BorderSizePixel = 0
aeCard.Size = UDim2.new(1,0,0,134); aeCard.LayoutOrder = fNextLO(); aeCard.ZIndex = 5
...
-- Linha 2: nível detectado (dinâmico)
aeLevelLbl = Instance.new("TextLabel",aeCard); aeLevelLbl.BackgroundTransparency=1
aeLevelLbl.Position=UDim2.new(0,14,0,34); aeLevelLbl.Size=UDim2.new(1,-28,0,14)
...
aeRadiusLbl.Position=UDim2.new(0,14,0,50); aeRadiusLbl.Size=UDim2.new(1,-28,0,12)
aeRadiusLbl.Font=Enum.Font.Gotham; aeRadiusLbl.Text="FogEnd: —  •  Vel: "..AE_SPEED.." st/s  •  Alt: +"..AE_HEIGHT_OFF
...
aeProgressBg.BorderSizePixel=0; aeProgressBg.Position=UDim2.new(0,14,0,68)
...
-- Linha 5: timer ao vivo
aeTimerLabel = Instance.new("TextLabel",aeCard); aeTimerLabel.BackgroundTransparency=1
aeTimerLabel.Position=UDim2.new(0,14,0,82); aeTimerLabel.Size=UDim2.new(1,-28,0,16)
...
aeDesc.Position=UDim2.new(0,14,0,102); aeDesc.Size=UDim2.new(1,-28,0,24)
aeDesc.Font=Enum.Font.Gotham; aeDesc.Text="Raio detectado via FogEnd em tempo real — sobe automático se a fogueira subir de nível"
