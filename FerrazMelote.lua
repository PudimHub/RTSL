...
-- ──────────────────────────────────────────────
-- Função: encontrar a FOGUEIRA REAL do acampamento
-- Prioridade: Campground > CampfireLevel > atributo Level > maior fogueira
-- ──────────────────────────────────────────────
local function getCampfirePos()
    local pos

    pcall(function()
        -- 1) Prioridade máxima: workspace.Campground.MainFire.Center
        ...
        if camp then
            ...
            if mf then
                ...
                if center and center:IsA("BasePart") then
                    pos = center.Position; return
                end
                -- Qualquer BasePart dentro de MainFire
                local bp = mf:FindFirstChildWhichIsA("BasePart")
                if bp then pos = bp.Position; return end
            end
            -- Qualquer BasePart dentro do Campground (não de fogueira decorativa)
            for _, obj in ipairs(camp:GetDescendants()) do
                if obj:IsA("BasePart") and obj.Name ~= "Decoration" then
                    pos = obj.Position; return
                end
            end
        end

        -- 2) Procura objeto com atributo "Level" (fogueira upgradável real)
        --    Fogueiras decorativas não têm Level
        for _, obj in ipairs(workspace:GetDescendants()) do
            local nm = obj.Name:lower()
            if (nm == "campfire" or nm == "mainfire" or nm == "camp fire") then
                local hasLevel = false
                pcall(function()
                    local lvl = obj:GetAttribute("Level") or obj:GetAttribute("CampfireLevel")
                    if lvl ~= nil then hasLevel = true end
                end)
                if hasLevel then
        ...
        -- 3) Fallback: pega o Campfire de maior Y (mais provável ser o real)
        --    e que esteja próximo de outros jogadores ou do spawn
        local bestFire, bestY = nil, -math.huge
        for _, obj in ipairs(workspace:GetDescendants()) do
            local nm = obj.Name:lower()
            -- Ignora nomes claramente decorativos
            if nm:find("decor") or nm:find("ambient") or nm:find("fake") then continue end
            if nm == "campfire" or nm == "mainfire" or nm == "fire" then
                local bp = obj:IsA("BasePart") and obj or obj:FindFirstChildWhichIsA("BasePart")
                if bp and bp.Position.Y > bestY and bp.Position.Y > -50 then
                    bestY = bp.Position.Y; bestFire = bp
                end
            end
        end
        if bestFire then pos = bestFire.Position end
    end)
        ...
        pcall(function()
            ...
            for _, obj in ipairs(descs) do
                ...
                if obj:IsA("Model") then
                    ...
                    -- ❌ FILTRO: ignora armadilhas (traps) — não são construções
                    local isTrap = oNameLow:find("trap",1,true)
                             or oNameLow:find("spike",1,true)
                             or oNameLow:find("armadilha",1,true)
                             or oNameLow:find("rope trap",1,true)
                             or oNameLow:find("bear trap",1,true)
                             or oNameLow:find("stake",1,true)
                             or oNameLow:find("snare",1,true)
                    if isTrap then continue end
...
local function makeChestBtn(name, pos, rarity, biomeLabel, biomeIcon, isNew)
    ...
    local function doTp()
        if entry.isNew then entry.isNew = false; tagBadge.Visible = false end
        ...
        pcall(function()
            ...
            for _, obj in ipairs(descs) do
                ...
                if obj:IsA("Model") then
                    ...
                    -- Detecta raridade e bioma
                    local rarity = getRarity(obj)
                    local biomeLabel, biomeIcon = getBiome(obj)
                    makeChestBtn(obj.Name, part.Position, rarity, biomeLabel, biomeIcon, isRefresh)
                    found += 1
