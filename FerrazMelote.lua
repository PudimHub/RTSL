...
-- ── Papel de parede (opcional, configurável em Configurações) ──
local Wallpaper = Instance.new("ImageLabel", MainFrame)
...
Wallpaper.Image = ""
Wallpaper.ImageTransparency = 0.55
Wallpaper.ZIndex = 1

-- ══════════════════════════════════════════════════════
--  CONFIG AUTOMÁTICA (salva/carrega sozinho, sem botão)
-- ══════════════════════════════════════════════════════
local PUDIM_CFG_FILE = "PudimHub_config.json"
local PudimCfg = {
    wallpaperId      = "",
    wallpaperOpacity = 45,
    uiOpacity        = 0,
    -- Estilo de Interface
    themeR = 105, themeG = 78, themeB = 158,
    textColorSet = false, textColorR = 255, textColorG = 255, textColorB = 255,
    textSizeValue = 5,
    borderNeon = false,
    rounded    = false,
    rgbActive  = false,
...
local function pudimSaveConfig()
    pcall(function()
        if not writefile then return end
        writefile(PUDIM_CFG_FILE, HttpService:JSONEncode(PudimCfg))
    end)
end

local function pudimLoadConfig()
    pcall(function()
        if not readfile or not isfile then return end
        if not isfile(PUDIM_CFG_FILE) then return end
        local raw = readfile(PUDIM_CFG_FILE)
        local data = HttpService:JSONDecode(raw)
        if type(data) ~= "table" then return end
        for k, v in pairs(data) do PudimCfg[k] = v end
    end)
end

pudimLoadConfig()

-- Aplica cedo o que já pode ser aplicado (idioma e notificações não
-- dependem de nenhuma UI já construída)
pcall(function()
    notifPosition = PudimCfg.notifPosition or "TR"
    NOTIF_CFG.DEFAULT_DURATION = PudimCfg.notifDuration or 3
    NOTIF_CFG.STYLE = PudimCfg.notifStyle or "card"
    for _, lang in ipairs(LANGUAGES) do
        if lang.code == PudimCfg.langCode then
            applyLanguage(lang)
            break
        end
    end
...
-- Aplica o wallpaper salvo direto (a opacidade/campo de texto são
-- sincronizados depois, quando a seção de Configurações é construída)
if PudimCfg.wallpaperId and PudimCfg.wallpaperId ~= "" then
    pcall(function()
        Wallpaper.Image = "rbxassetid://"..tostring(PudimCfg.wallpaperId)
        Wallpaper.ImageTransparency = 1 - ((PudimCfg.wallpaperOpacity or 45)/100)
    end)
end
...
local function getGameDay()
    local day = nil
    pcall(function()
        -- Tenta atributos em game, workspace, Lighting
        for _, root in ipairs({game, workspace, Lighting, Player}) do
            for _, n in ipairs({"Day","CurrentDay","GameDay","NightNumber","DayCount","DaysElapsed","DaysSurvived","DayNum"}) do
                local v = root:GetAttribute(n)
        ...
        -- Tenta NumberValues soltos no workspace
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("NumberValue") or obj:IsA("IntValue") then
                local nm = obj.Name:lower()
                if (nm:find("day") or nm:find("night") or nm:find("dia")) and obj.Value > 0 then
                    day = math.floor(obj.Value); return
                end
            end
        end
        -- Tenta via ReplicatedStorage (comum em alguns jogos)
        local rs = game:GetService("ReplicatedStorage")
        for _, n in ipairs({"Day","CurrentDay","GameDay"}) do
            local val = rs:FindFirstChild(n)
            if val and (val:IsA("IntValue") or val:IsA("NumberValue")) and val.Value>0 then
                day = math.floor(val.Value); return
            end
        end
...
local function getIsNight()
    ...
    pcall(function()
        ...
        -- Se ClockTime for 0 (não configurado), tenta atributo
        if hr == 0 then
            local attr = lighting:GetAttribute("NightTime") or game:GetAttribute("IsNight") or workspace:GetAttribute("IsNight")
            if attr ~= nil then isNight = (attr == true or attr == 1 or attr == "true") end
        end
    end)
    return isNight
end
...
local function getNightProgress()
    ...
    pcall(function()
        ...
        local hr = lighting.ClockTime
        if hr == 0 then
            -- Tenta atributo de progresso
            local attr = lighting:GetAttribute("CycleProgress") or game:GetAttribute("DayProgress") or workspace:GetAttribute("DayProgress")
            if attr and type(attr)=="number" then pct = math.clamp(attr, 0, 1); return end
        end
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
            ...
            local ping = 0
            pcall(function()
                -- Tenta via PerformanceStats (Roblox novo)
                local ps = game:GetService("Stats"):FindFirstChild("PerformanceStats")
                if ps then
                    local pv = ps:FindFirstChild("Ping")
                    if pv then ping = math.floor(pv:GetValue()) end
                end
                -- Fallback via Network Stats (Roblox antigo)
                if ping == 0 then
                ...
                -- Último fallback: atributo do jogador
                if ping == 0 then
                    local attr = Players.LocalPlayer:GetAttribute("Ping")
                    if attr and type(attr)=="number" then ping = math.floor(attr) end
                end
            end)
...
local function makeMultiDropdown(parent, yPos, cfg)
    ...
    local SEL      = cfg.sel
    local BTN_H    = 26
    local ITEM_H   = 32  -- altura item estilo foto
    local POP_W    = 180  -- largura popup estilo foto
    ...
    -- ── Botão trigger — usa a cor do próprio card (COR), fundo neutro ──
    local trigBtn = Instance.new("TextButton", parent)
    trigBtn.BackgroundColor3       = Color3.fromRGB(26, 22, 40)
    ...
    trigBtn.Size                   = UDim2.new(0, 120, 0, 24)
    trigBtn.Font                   = Enum.Font.GothamBold
    trigBtn.Text                   = buildLabel()
    trigBtn.TextColor3             = Color3.fromRGB(230, 225, 245)
    trigBtn.TextSize               = 10
    trigBtn.ZIndex                 = 8
    ...
    Instance.new("UICorner", trigBtn).CornerRadius = UDim.new(0, 12)
    -- Borda na cor do card
    local trigS = Instance.new("UIStroke", trigBtn)
    trigS.Color = COR; trigS.Thickness = 1; trigS.Transparency = 0.35
    ...
    trigArrow.Text                   = "⇅"
    trigArrow.TextColor3             = COR; trigArrow.TextSize = 11; trigArrow.ZIndex = 9

    -- ── Popup — fundo neutro (combina com o resto do hub), acento = COR ──
    local pop = Instance.new("Frame", ScreenGui)
    pop.BackgroundColor3       = Color3.fromRGB(18, 15, 28)
    ...
    popS.Color = COR; popS.Thickness = 1; popS.Transparency = 0.35
    popS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Barra de busca — topo do popup, levemente mais clara que o fundo
    local popSearch = Instance.new("Frame", pop)
    popSearch.BackgroundColor3 = Color3.fromRGB(26, 22, 40)
    ...
    popSearchFix.BackgroundColor3 = Color3.fromRGB(26, 22, 40)
    ...
    popSearchBox.PlaceholderColor3 = Color3.fromRGB(150, 140, 175); popSearchBox.Text = ""
    popSearchBox.TextColor3 = Color3.fromRGB(235, 230, 245); popSearchBox.TextSize = 11
    ...
    for _, item in ipairs(ITEMS) do
        ...
        nameLbl.TextColor3 = isSel and Color3.fromRGB(245,242,255) or Color3.fromRGB(190,185,205)
        nameLbl.TextSize = 11
        nameLbl.TextXAlignment = Enum.TextXAlignment.Left
        nameLbl.TextYAlignment = Enum.TextYAlignment.Center
        nameLbl.ZIndex   = 433

        -- Checkbox de verdade — anel vazio quando desmarcado, preenchido + ✓ quando marcado
        local circ = Instance.new("Frame", row)
        circ.BackgroundColor3       = COR
        circ.BackgroundTransparency = isSel and 0 or 1
        ...
        circ.ZIndex                 = 433
        Instance.new("UICorner", circ).CornerRadius = UDim.new(1, 0)
        local circRing = Instance.new("UIStroke", circ)
        circRing.Color = COR; circRing.Thickness = 1.3
        circRing.Transparency = isSel and 1 or 0.15
        circRing.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        local circCheck = Instance.new("TextLabel", circ)
        circCheck.BackgroundTransparency = 1
        circCheck.Size = UDim2.new(1,0,1,0)
        circCheck.Font = Enum.Font.GothamBlack
        circCheck.Text = "✓"
        circCheck.TextColor3 = Color3.fromRGB(255,255,255)
        circCheck.TextSize = 10
        circCheck.TextTransparency = isSel and 0 or 1
        circCheck.ZIndex = 434
        ...
        local function refreshRow(hover)
            ...
            TweenService:Create(row, TweenInfo.new(0.1), {
                BackgroundTransparency = on and 0.82 or (hover and 0.92 or 1),
            }):Play()
            TweenService:Create(circ, TweenInfo.new(0.15), {
                BackgroundTransparency = on and 0 or 1,
            }):Play()
            TweenService:Create(circRing, TweenInfo.new(0.15), {
                Transparency = on and 1 or 0.15,
            }):Play()
            TweenService:Create(circCheck, TweenInfo.new(0.15), {
                TextTransparency = on and 0 or 1,
            }):Play()
            nameLbl.TextColor3 = on and Color3.fromRGB(245,242,255) or (hover and Color3.fromRGB(220,215,235) or Color3.fromRGB(190,185,205))
...
-- ── Botão de ativar no estilo switch iOS de verdade (pill + bolinha branca) ──
-- Mesma interface de retorno do _accActivBtn (btn, stroke) + registro em
-- _btnStateMap, então serve de substituto direto nos cards que já usavam ele.
local function _accToggleSwitch(parent, y, icon, color)
    local iconStr = icon or "▶"
    local lbl=Instance.new("TextLabel",parent); lbl.BackgroundTransparency=1; lbl.BorderSizePixel=0
    lbl.Position=UDim2.new(0,10,0,y); lbl.Size=UDim2.new(1,-70,0,40)
    lbl.Font=Enum.Font.GothamBold; lbl.Text=iconStr.."  Ativar"
    ...
    pill.AnchorPoint=Vector2.new(1,0.5); pill.Position=UDim2.new(1,-10,0,y+20)
    pill.Size=UDim2.new(0,44,0,24); pill.ZIndex=8
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    pill.BackgroundColor3=C_OFF
    local s=Instance.new("UIStroke",pill); s.Color=color or Color3.fromRGB(80,140,255)
    s.Thickness=1.2; s.Transparency=0.45; s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

    -- Bolinha branca deslizante (o toque final estilo iOS)
    local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
    ...
    local btn=Instance.new("TextButton",parent); btn.BackgroundTransparency=1; btn.BorderSizePixel=0
    btn.Position=UDim2.new(0,0,0,y); btn.Size=UDim2.new(1,0,0,40); btn.Text=""; btn.ZIndex=11

    local COR_ON = color or Color3.fromRGB(50,120,255)
    _btnStateMap[btn] = function(on)
        TweenService:Create(pill,TweenInfo.new(0.2),{BackgroundColor3 = on and COR_ON or C_OFF}):Play()
        TweenService:Create(knob,TweenInfo.new(0.2,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
...
local function setTSlider(pct)
    ...
    _applyTransp(pct)
    PudimCfg.uiOpacity = math.floor(pct)
end
...
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 and tDrag then
        tDrag=false
        pudimSaveConfig()
    end
end)
-- Sincroniza com o que já foi salvo/carregado (se tiver)
if PudimCfg.uiOpacity and PudimCfg.uiOpacity > 0 then
    setTSlider(PudimCfg.uiOpacity)
end
end -- Transparência

-- ══════════════════════════════════════════════════════════════
-- PAPEL DE PAREDE — Configurações Tab
-- ══════════════════════════════════════════════════════════════
cfgMkSec("🖼️  PAPEL DE PAREDE", Color3.fromRGB(120,190,255))
do
local WPC = Color3.fromRGB(120,190,255)
local _, _wpCF = makeAccordionCard(Pages["Configuracoes"], cfg2NextLO, {
    icon="🖼️", title="Papel de Parede", color=WPC,
    summary="Cole o ID/link de uma imagem do Roblox pra usar como fundo do hub.",
    contentH = 36+9+40+10+56+10+38+14,
})
local _wy = 36+8
_accDivLine(_wpCF, _wy, WPC); _wy = _wy+9

-- Campo de texto (ID ou link)
local wpInputRow = Instance.new("Frame", _wpCF)
wpInputRow.BackgroundColor3 = Color3.fromRGB(26,22,40); wpInputRow.BorderSizePixel=0
wpInputRow.Position = UDim2.new(0,10,0,_wy); wpInputRow.Size = UDim2.new(1,-20,0,32); wpInputRow.ZIndex=7
Instance.new("UICorner", wpInputRow).CornerRadius = UDim.new(0,8)
local wpInputS = Instance.new("UIStroke", wpInputRow)
wpInputS.Color = WPC; wpInputS.Thickness = 1; wpInputS.Transparency = 0.55

local wpBox = Instance.new("TextBox", wpInputRow)
wpBox.BackgroundTransparency = 1
wpBox.Position = UDim2.new(0,10,0,0); wpBox.Size = UDim2.new(1,-20,1,0)
wpBox.Font = Enum.Font.Gotham; wpBox.PlaceholderText = "ID ou link do Roblox (ex: 123456789)"
wpBox.PlaceholderColor3 = Color3.fromRGB(140,135,165); wpBox.Text = ""
wpBox.TextColor3 = Color3.fromRGB(230,225,245); wpBox.TextSize = 11
wpBox.TextXAlignment = Enum.TextXAlignment.Left; wpBox.ClearTextOnFocus = false; wpBox.ZIndex=8
_wy = _wy + 40

-- Botões Aplicar / Remover
local wpApplyBtn = Instance.new("TextButton", _wpCF)
wpApplyBtn.BackgroundColor3 = WPC; wpApplyBtn.BackgroundTransparency = 0.2; wpApplyBtn.BorderSizePixel=0
wpApplyBtn.Position = UDim2.new(0,10,0,_wy); wpApplyBtn.Size = UDim2.new(0.5,-15,0,32); wpApplyBtn.ZIndex=7
wpApplyBtn.Font = Enum.Font.GothamBold; wpApplyBtn.Text = "✅ Aplicar"
wpApplyBtn.TextColor3 = Color3.fromRGB(15,15,25); wpApplyBtn.TextSize = 11
wpApplyBtn.AutoButtonColor = false
Instance.new("UICorner", wpApplyBtn).CornerRadius = UDim.new(0,8)

local wpRemoveBtn = Instance.new("TextButton", _wpCF)
wpRemoveBtn.BackgroundColor3 = Color3.fromRGB(60,45,80); wpRemoveBtn.BorderSizePixel=0
wpRemoveBtn.AnchorPoint = Vector2.new(1,0)
wpRemoveBtn.Position = UDim2.new(1,-10,0,_wy); wpRemoveBtn.Size = UDim2.new(0.5,-15,0,32); wpRemoveBtn.ZIndex=7
wpRemoveBtn.Font = Enum.Font.GothamBold; wpRemoveBtn.Text = "🗑️ Remover"
wpRemoveBtn.TextColor3 = Color3.fromRGB(220,210,235); wpRemoveBtn.TextSize = 11
wpRemoveBtn.AutoButtonColor = false
Instance.new("UICorner", wpRemoveBtn).CornerRadius = UDim.new(0,8)
_wy = _wy + 42

-- Extrai o ID numérico, seja de um link completo ou já um número puro
local function _wpExtractId(txt)
    if not txt or txt == "" then return nil end
    txt = txt:gsub("%s+", "")
    local id = txt:match("rbxassetid://(%d+)") or txt:match("assetId=(%d+)")
        or txt:match("/(%d+)/") or txt:match("^(%d+)$")
    return id
end

wpApplyBtn.MouseButton1Click:Connect(function()
    local id = _wpExtractId(wpBox.Text)
    if not id then
        Notify.warn("🖼️ Papel de Parede", "Não consegui achar um ID válido nesse texto.", 4)
        return
    end
    local ok = pcall(function()
        Wallpaper.Image = "rbxassetid://"..id
    end)
    if ok then
        PudimCfg.wallpaperId = id
        pudimSaveConfig()
        Notify.success("🖼️ Papel de Parede", "Aplicado!", 3)
    else
        Notify.error("🖼️ Papel de Parede", "Não consegui carregar essa imagem.", 4)
    end
end)

wpRemoveBtn.MouseButton1Click:Connect(function()
    Wallpaper.Image = ""
    wpBox.Text = ""
    PudimCfg.wallpaperId = ""
    pudimSaveConfig()
    Notify.info("🖼️ Papel de Parede", "Removido.", 3)
end)

-- Slider de opacidade do papel de parede
local wpSlRow = Instance.new("Frame", _wpCF)
wpSlRow.BackgroundColor3 = Color3.fromRGB(30,45,66); wpSlRow.BorderSizePixel=0
wpSlRow.Position = UDim2.new(0,10,0,_wy); wpSlRow.Size = UDim2.new(1,-20,0,56); wpSlRow.ZIndex=7
Instance.new("UICorner", wpSlRow).CornerRadius = UDim.new(0,12)

local wpSlLbl = Instance.new("TextLabel", wpSlRow)
wpSlLbl.BackgroundTransparency=1; wpSlLbl.Position=UDim2.new(0,14,0,0); wpSlLbl.Size=UDim2.new(0.45,0,1,0)
wpSlLbl.Font=Enum.Font.GothamBold; wpSlLbl.Text="🔅  Opacidade"
wpSlLbl.TextColor3=Color3.fromRGB(215,225,235); wpSlLbl.TextSize=11
wpSlLbl.TextXAlignment=Enum.TextXAlignment.Left; wpSlLbl.TextYAlignment=Enum.TextYAlignment.Center; wpSlLbl.ZIndex=8

local wpSlValLbl = Instance.new("TextLabel", wpSlRow)
wpSlValLbl.BackgroundTransparency=1; wpSlValLbl.Position=UDim2.new(0.47,0,0.5,-10); wpSlValLbl.Size=UDim2.new(0,42,0,20)
wpSlValLbl.Font=Enum.Font.GothamBold; wpSlValLbl.Text="45%"; wpSlValLbl.TextColor3=Color3.fromRGB(215,225,235)
wpSlValLbl.TextSize=12; wpSlValLbl.TextXAlignment=Enum.TextXAlignment.Left; wpSlValLbl.ZIndex=9

local wpTrack = Instance.new("Frame", wpSlRow)
wpTrack.BackgroundColor3=Color3.fromRGB(50,68,90); wpTrack.BorderSizePixel=0
wpTrack.Position=UDim2.new(0.47,48,0.5,-2); wpTrack.Size=UDim2.new(0.50,-58,0,4); wpTrack.ZIndex=9
Instance.new("UICorner",wpTrack).CornerRadius=UDim.new(1,0)

local wpFill=Instance.new("Frame",wpTrack); wpFill.BackgroundColor3=WPC; wpFill.BorderSizePixel=0
wpFill.Size=UDim2.new(0.45,0,1,0); wpFill.ZIndex=10
Instance.new("UICorner",wpFill).CornerRadius=UDim.new(1,0)

local wpKnob=Instance.new("Frame",wpTrack); wpKnob.BackgroundColor3=Color3.fromRGB(20,35,55); wpKnob.BorderSizePixel=0
wpKnob.AnchorPoint=Vector2.new(0.5,0.5); wpKnob.Position=UDim2.new(0.45,0,0.5,0); wpKnob.Size=UDim2.new(0,18,0,18); wpKnob.ZIndex=11
Instance.new("UICorner",wpKnob).CornerRadius=UDim.new(1,0)
local wpKS=Instance.new("UIStroke",wpKnob); wpKS.Color=WPC; wpKS.Thickness=1.5; wpKS.Transparency=0.3; wpKS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

local wpDrag=false
local function setWpSlider(pct)
    pct=math.clamp(pct,0,100)
    local p=pct/100
    wpFill.Size=UDim2.new(p,0,1,0); wpKnob.Position=UDim2.new(p,0,0.5,0)
    wpSlValLbl.Text=tostring(math.floor(pct)).."%"
    -- 0% do slider = totalmente visível (ImageTransparency 0), 100% = invisível (1)
    Wallpaper.ImageTransparency = 1 - p
    PudimCfg.wallpaperOpacity = math.floor(pct)
end
local wpSb=Instance.new("TextButton",wpTrack); wpSb.BackgroundTransparency=1
wpSb.Size=UDim2.new(1,24,1,24); wpSb.Position=UDim2.new(0,-12,0,-12); wpSb.Text=""; wpSb.ZIndex=12
wpSb.MouseButton1Down:Connect(function()
    wpDrag=true
    local ap=wpTrack.AbsolutePosition; local as=wpTrack.AbsoluteSize
    setWpSlider(((UserInputService:GetMouseLocation().X-ap.X)/as.X)*100)
end)
UserInputService.InputChanged:Connect(function(i)
    if not wpDrag then return end
    if i.UserInputType~=Enum.UserInputType.MouseMovement then return end
    local ap=wpTrack.AbsolutePosition; local as=wpTrack.AbsoluteSize
    setWpSlider(((i.Position.X-ap.X)/as.X)*100)
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 and wpDrag then
        wpDrag=false
        pudimSaveConfig()
    end
end)
-- Sincroniza com o que já foi salvo/carregado (se tiver)
setWpSlider(PudimCfg.wallpaperOpacity or 45)
if PudimCfg.wallpaperId and PudimCfg.wallpaperId ~= "" then
    wpBox.Text = tostring(PudimCfg.wallpaperId)
end
end -- Papel de Parede
...
_estBtnRow(_ey, "🎨  Tema", "▾  Tema", Color3.fromRGB(255,160,80), function(btn)
    _openDropdown(btn, TEMAS, function(item)
        ...
        PudimCfg.themeR, PudimCfg.themeG, PudimCfg.themeB = math.floor(item.hue.R*255), math.floor(item.hue.G*255), math.floor(item.hue.B*255)
        pudimSaveConfig()
    end, 3)
end)
_ey=_ey+44
...
-- ═════════════════════════════════════════════════════
_accDivLine(_estCF, _ey, EST_COR); _ey=_ey+9
_estSlider(_ey, "🔡  Letra", 1, 10, PudimCfg.textSizeValue or 5, Color3.fromRGB(100,200,255), function(v)
    _applyTextSize((v-5)*2)   -- 5→0, 1→-8, 10→+10
    PudimCfg.textSizeValue = v
    pudimSaveConfig()
end)
_ey=_ey+44
...
-- ═════════════════════════════════════════════════════
_accDivLine(_estCF, _ey, EST_COR); _ey=_ey+9
_accToggle(_estCF, _ey, "✨  Bordas Neon", PudimCfg.borderNeon or false, Color3.fromRGB(200,150,255), function(s)
    _borderNeon=s
    _applyBorderNeon(s)
    if s then Notify.success("✨ Bordas Neon","Brilho neon ativado nos accordions!")
    else Notify.info("✨ Bordas Neon","Desativado.") end
    PudimCfg.borderNeon = s
    pudimSaveConfig()
end)
_ey=_ey+44
...
-- ═════════════════════════════════════════════════════
_accDivLine(_estCF, _ey, EST_COR); _ey=_ey+9
_accToggle(_estCF, _ey, "⬜  Arredondar Tudo", PudimCfg.rounded or false, Color3.fromRGB(150,220,255), function(s)
    _rounded=s
    _applyRounded(s)
...
_estBtnRow(_ey, "🖊️  Cor de Escrita", "▾  Cor", Color3.fromRGB(255,200,80), function(btn)
    _openDropdown(btn, TEXT_COLORS, function(item)
        ...
        if item.col then
            PudimCfg.textColorSet = true
            PudimCfg.textColorR, PudimCfg.textColorG, PudimCfg.textColorB = math.floor(item.col.R*255), math.floor(item.col.G*255), math.floor(item.col.B*255)
        else
            PudimCfg.textColorSet = false
        end
        pudimSaveConfig()
    end, 4)
end)
...
-- ═════════════════════════════════════════════════════
_accDivLine(_estCF, _ey, EST_COR); _ey=_ey+9
_accToggle(_estCF, _ey, "🌈  Bordas RGB", PudimCfg.rgbActive or false, Color3.fromRGB(255,100,200), function(s)
    _rgbActive=s
    if s then
        _startRGB()
    ...
    PudimCfg.rgbActive = s
    pudimSaveConfig()
end)
_ey=_ey+44

-- ═════════════════════════════════════════════════════
-- Restaura tudo que foi salvo (aplica funcionalmente E visualmente
-- já com a UI toda montada, por isso fica aqui no final)
-- ═════════════════════════════════════════════════════
pcall(function()
    _applyTheme(Color3.fromRGB(PudimCfg.themeR or 105, PudimCfg.themeG or 78, PudimCfg.themeB or 158))
    if PudimCfg.textColorSet then
        _applyTextColor(Color3.fromRGB(PudimCfg.textColorR or 255, PudimCfg.textColorG or 255, PudimCfg.textColorB or 255))
    end
    _applyTextSize(((PudimCfg.textSizeValue or 5)-5)*2)
    if PudimCfg.borderNeon then _borderNeon=true; _applyBorderNeon(true) end
    if PudimCfg.rounded then _rounded=true; _applyRounded(true) end
    if PudimCfg.rgbActive then _rgbActive=true; _startRGB() end
end)
...
local function _setPosActive(activePos)
    ...
    PudimCfg.notifPosition = activePos
    pudimSaveConfig()
end
...
UserInputService.InputEnded:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 and durDrag then
        durDrag=false
        PudimCfg.notifDuration = NOTIF_CFG.DEFAULT_DURATION
        pudimSaveConfig()
    end
end)
_ny = _ny+62
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            if not (nm:find("chest",1,true) or nm:find("bau",1,true) or
                    nm:find("baú",1,true)) then continue end
            if nm:find("corpse",1,true) or nm:find("npc",1,true)
                or nm:find("chestplate",1,true) then continue end
            local part = nil
            pcall(function()
                local maxSz = 0
                for _, bp in ipairs(obj:GetDescendants()) do
...
local function getMobsInRange(origin, radius)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            ...
            -- Humanoid é OPCIONAL: alguns mobs desse jogo usam sistema de vida
            -- próprio (sem Humanoid padrão). Só filtra por Health se existir.
            local hum = obj:FindFirstChildWhichIsA("Humanoid")
...
-- ── Pega a "raiz" do mob (HRP, ou PrimaryPart, ou qualquer BasePart) ───
        ...
        KA_hookMt.__namecall = newcclosure(function(self, ...)
            ...
            if ok and method == "InvokeServer" and self and self.Name == "ToolDamageObject" then
                ...
                if type(tok) == "string" then
                    ...
                    local wasNil = (KA_sessionId == nil)
                    if sid then KA_sessionId = sid end
                    if cnt and cnt > KA_hitCounter then KA_hitCounter = cnt end
                    if wasNil and KA_sessionId then
                        task.spawn(function()
                            pcall(function() Notify.success("✅ Kill/Tree Aura", "ID de ataque aprendido! Agora o modo automático já funciona.", 4) end)
                        end)
                    end
                end
            end
...
local function attackMob(mob)
    ...
    -- Sem ID de sessão capturado ainda? Precisa de UM golpe manual seu real
    -- (o hook aprende sozinho assim que você acertar algo com o dedo).
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.warn("⚔️ Kill Aura", "Ataque um mob MANUALMENTE 1 vez (toque na tela) pra o Kill Aura aprender o ID de ataque. Depois disso ele vira automático.", 6)
            end)
        end
        return
    end
...
local function startKillAura()
    ...
    kaAutoLoop = RunService.Heartbeat:Connect(function()
        ...
        for _, m in ipairs(mobs) do
            task.spawn(function()
                local ok, err = pcall(function() attackMob(m.model) end)
                if not ok then warn("[PudimHub DEBUG] Erro no Kill Aura: "..tostring(err)) end
            end)
        end
    end)
...
local function chopTree(treeModel, treePart)
    ...
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.warn("🪓 Tree Aura", "Corte uma árvore MANUALMENTE 1 vez (toque na tela) pra o Tree Aura aprender o ID de ataque. Depois disso ele vira automático.", 6)
            end)
        end
        return
    end
    ...
    taLoop = RunService.Heartbeat:Connect(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                ...
                lastChop = now
                task.spawn(function()
                    local ok, err = pcall(function() chopTree(obj, root) end)
                    if not ok then warn("[PudimHub DEBUG] Erro no Tree Aura: "..tostring(err)) end
                end)
            end)
        end
    end)
...
_accDivLine(_kacCF,_y,KA_COR); _y=_y+9
_y=_y+_nfs(_kacCF,_y,"Alcance",1,200,kaRange,KA_COR,function(v) kaRange=v end)
...
_y=_y+44
_accDivLine(_tacCF,_y,TA_COR); _y=_y+9
_y=_y+_nfs(_tacCF,_y,"Alcance",5,200,taRange,TA_COR,function(v) taRange=v end)
...
local AFC_ALL_ITEMS = {
    ...
    {key="sapling",label="Sapling",icon="🌱",cat="wood"},{key="wood",label="Wood",icon="🪵",cat="wood"},
    {key="plank",label="Plank",icon="🪵",cat="wood"},{key="chair",label="Chair",icon="🪑",cat="chair"},
    ...
    {key="bear corpse",label="Bear Corpse",icon="🐻",cat="corpse"},{key="polar bear corpse",label="Polar Bear Corpse",icon="🐻‍❄️",cat="corpse"},
    {key="cultist corpse",label="Cultist Corpse",icon="🧟",cat="corpse"},{key="crossbow cultist corpse",label="Crossbow Cultist Corpse",icon="🧟",cat="corpse"},
    {key="juggernaut cultist corpse",label="Juggernaut Corpse",icon="🧟",cat="corpse"},
    {key="deer corpse",label="Deer Corpse",icon="🦌",cat="corpse"},{key="bunny corpse",label="Bunny Corpse",icon="🐰",cat="corpse"},
    {key="frog corpse",label="Frog Corpse",icon="🐸",cat="corpse"},{key="turkey corpse",label="Turkey Corpse",icon="🦃",cat="corpse"},
    {key="boar corpse",label="Boar Corpse",icon="🐗",cat="corpse"},{key="horse corpse",label="Horse Corpse",icon="🐴",cat="corpse"},
    {key="scorpion corpse",label="Scorpion Corpse",icon="🦂",cat="corpse"},{key="meteor crab corpse",label="Meteor Crab Corpse",icon="🦀",cat="corpse"},
    {key="mammoth corpse",label="Mammoth Corpse",icon="🦣",cat="corpse"},{key="hellephant corpse",label="Hellephant Corpse",icon="🐘",cat="corpse"},
    {key="alien corpse",label="Alien Corpse",icon="👽",cat="corpse"},
}
local afcSel={}
...
_y=_y+34+8
local _afBtn,_afBtnS=_accToggleSwitch(_afCF,_y,"🔥",AFC_COR)
...
local _afhpBtn,_afhpBtnS=_accToggleSwitch(_afhpCF,_y,"❤️",AFHP_COR)
...
local _ackBtn,_ackBtnS=_accToggleSwitch(_ackCF,_y,"🍖",ACK_COR); _y=_y+40
...
local AMG_ALL_ITEMS={
    ...
    {key="log",label="Log",icon="🪵",scrap=1,cat="wood"},{key="super log",label="Super Log",icon="🪵",scrap=2,cat="wood"},
    {key="wood",label="Wood",icon="🪵",scrap=1,cat="wood"},{key="plank",label="Plank",icon="🪵",scrap=1,cat="wood"},
    {key="ufo battery",label="UFO Battery",icon="🔋",scrap=2,cat="metal"},{key="alien battery",label="Alien Battery",icon="🔋",scrap=2,cat="metal"},
    {key="alien egg",label="Alien Egg",icon="🥚",scrap=2,cat="metal"},{key="alien chest key",label="Alien Chest Key",icon="🔑",scrap=1,cat="metal"},
    {key="dynamite",label="Dynamite",icon="🧨",scrap=2,cat="metal"},{key="ammo box",label="Ammo Box",icon="📦",scrap=3,cat="metal"},
    {key="corrupted gear",label="Corrupted Gear",icon="⚙️",scrap=3,cat="metal"},{key="corrupted throwing axe",label="Corrupted Throwing Axe",icon="🪓",scrap=2,cat="metal"},
    {key="forest gem",label="Forest Gem",icon="💚",scrap=2,cat="gem"},{key="mossy coin",label="Mossy Coin",icon="🪙",scrap=1,cat="gem"},
    {key="alien tech",label="Alien Tech",icon="👽",scrap=3,cat="metal"},{key="feather",label="Feather",icon="🪶",scrap=1,cat="wood"},
    {key="lunchbox",label="Lunchbox",icon="🍱",scrap=1,cat="metal"},{key="yo-yo",label="Yo-Yo",icon="🪀",scrap=1,cat="metal"},
    {key="crayon",label="Crayon",icon="🖍️",scrap=1,cat="metal"},{key="plush",label="Plush",icon="🧸",scrap=1,cat="metal"},
}
...
Instance.new("UICorner",_amgDdBtn).CornerRadius = UDim.new(0,12)
...
_amgPop.ZIndex = 460; _amgPop.Visible = false; _amgPop.Size = UDim2.new(0,180,0,0)
_amgPop.ClipsDescendants = true
Instance.new("UICorner",_amgPop).CornerRadius = UDim.new(0,14)
...
_amgSearch.BorderSizePixel = 0; _amgSearch.Size = UDim2.new(1,0,0,32); _amgSearch.ZIndex = 461
Instance.new("UICorner",_amgSearch).CornerRadius = UDim.new(0,14)
...
for idx, item in ipairs(AMG_ALL_ITEMS) do
    ...
    iRow.Size = UDim2.new(1,0,0,32); iRow.LayoutOrder = idx; iRow.ZIndex = 462
    ...
    iPill.BackgroundColor3 = AMG_COR
    iPill.BackgroundTransparency = amgSel[item.key] and 0 or 1
    iPill.BorderSizePixel = 0
    iPill.AnchorPoint = Vector2.new(1,0.5); iPill.Position = UDim2.new(1,-8,0.5,0)
    iPill.Size = UDim2.new(0,16,0,16); iPill.ZIndex = 463
    Instance.new("UICorner",iPill).CornerRadius = UDim.new(1,0)
    local iPillRing = Instance.new("UIStroke",iPill)
    iPillRing.Color = AMG_COR; iPillRing.Thickness = 1.3
    iPillRing.Transparency = amgSel[item.key] and 1 or 0.15
    iPillRing.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local iPillCheck = Instance.new("TextLabel",iPill)
    iPillCheck.BackgroundTransparency = 1
    iPillCheck.Size = UDim2.new(1,0,1,0)
    iPillCheck.Font = Enum.Font.GothamBlack
    iPillCheck.Text = "✓"
    iPillCheck.TextColor3 = Color3.fromRGB(255,255,255)
    iPillCheck.TextSize = 10
    iPillCheck.TextTransparency = amgSel[item.key] and 0 or 1
    iPillCheck.ZIndex = 464
    -- Botão invisível para clicar
    ...
    iBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(iPillRing,TweenInfo.new(0.15),{
            Transparency = sel and 1 or 0.15,
        }):Play()
        TweenService:Create(iPillCheck,TweenInfo.new(0.15),{
            TextTransparency = sel and 0 or 1,
        }):Play()
        -- Fundo da linha
...
local function amgOpenPop()
    ...
    TweenService:Create(_amgPop,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Size=UDim2.new(0,180,0,AMG_POP_H),BackgroundTransparency=0}):Play()
...
local function amgClosePop()
    ...
    TweenService:Create(_amgPop,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.In),
        {Size=UDim2.new(0,180,0,0),BackgroundTransparency=0.3}):Play()
...
-- Pill toggle estilo iOS (com bolinha branca deslizante)
...
local _togKnob = Instance.new("Frame",_togPill); _togKnob.BorderSizePixel=0
_togKnob.BackgroundColor3=Color3.fromRGB(255,255,255)
_togKnob.AnchorPoint=Vector2.new(0.5,0.5); _togKnob.ZIndex=9
_togKnob.Size=UDim2.new(0,16,0,16)
_togKnob.Position=UDim2.new(0,12,0.5,0)
Instance.new("UICorner",_togKnob).CornerRadius=UDim.new(1,0)
local _togKnobS=Instance.new("UIStroke",_togKnob); _togKnobS.Color=Color3.fromRGB(0,0,0)
_togKnobS.Thickness=0.8; _togKnobS.Transparency=0.7; _togKnobS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
...
local function amgSetToggle(on)
    ...
    TweenService:Create(_togKnob,TweenInfo.new(0.2,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
...
local _abfBtn,_abfBtnS=_accToggleSwitch(_abfCF,_y,"🧪",ABF_COR); _y=_y+40
...
local _fbcCard,_fbcCF,_fbcSetDrop = makeAccordionCard(Pages["Farm"], fNextLO, {
    ...
    color=FBC_COR, contentH=36+9+14+30+8+40+8+14,
})
fbcCard=_fbcCard
...
-- ── Popup dropdown (ABRE PRA CIMA, mais compacto) ──────────────
...
_fbcPop.Size=UDim2.new(0,200,0,0); _fbcPop.ClipsDescendants=true
...
_fbcPopPad.PaddingLeft=UDim.new(0,4); _fbcPopPad.PaddingRight=UDim.new(0,4)

local ITEM_H=40; local TOTAL_H=#FBC_MODES*ITEM_H+16
local _fbcOpen=false; local _fbcItemRefs={}
...
for i,mode in ipairs(FBC_MODES) do
    ...
    if i < #FBC_MODES then
        ...
        sp.Size=UDim2.new(1,0,0,3); sp.LayoutOrder=0; sp.ZIndex=422
    end
    local lbl=Instance.new("TextLabel",item); lbl.BackgroundTransparency=1
    lbl.Position=UDim2.new(0,10,0,0); lbl.Size=UDim2.new(0.7,0,1,0)
    lbl.Font=Enum.Font.GothamBold; lbl.Text=mode.icon.." "..mode.label
    lbl.TextColor3=isSel and Color3.fromRGB(255,248,255) or Color3.fromRGB(185,165,215)
    lbl.TextSize=12; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=423
    local desc=Instance.new("TextLabel",item); desc.BackgroundTransparency=1
    desc.Position=UDim2.new(0,8,0,20); desc.Size=UDim2.new(1,-12,0,16)
    desc.Font=Enum.Font.Gotham; desc.Text=mode.desc
    desc.TextColor3=Color3.fromRGB(140,120,165); desc.TextSize=8
    desc.TextXAlignment=Enum.TextXAlignment.Left; desc.TextTruncate=Enum.TextTruncate.AtEnd; desc.ZIndex=423
    ...
    chk.Size=UDim2.new(0,18,0,18); chk.Font=Enum.Font.GothamBold
    chk.Text=isSel and "✓" or ""; chk.TextColor3=mode.color; chk.TextSize=14; chk.ZIndex=424
    local btn=Instance.new("TextButton",item); btn.BackgroundTransparency=1; btn.BorderSizePixel=0
    btn.Size=UDim2.new(1,0,1,0); btn.Text=""; btn.ZIndex=425; btn.AutoButtonColor=false
    btn.MouseEnter:Connect(function()
        TweenService:Create(item,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(44,26,68)}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(item,TweenInfo.new(0.08),{BackgroundColor3=isSel and Color3.fromRGB(52,32,80) or Color3.fromRGB(28,16,48)}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        fbcMode=mode.key; _fbcClosePop(); _fbcUpdateBtn()
        Notify.info("Destino",mode.icon.." "..mode.desc)
    end)
    _fbcItemRefs[i]=function(refresh)
        if refresh then
            item.BackgroundColor3=fbcMode==mode.key and Color3.fromRGB(52,32,80) or Color3.fromRGB(28,16,48)
            item.BackgroundTransparency=fbcMode==mode.key and 0.1 or 0.5
        end
    end
end
...
local function _fbcOpenPop()
    ...
    -- Abre para CIMA (acima do botão)
    _fbcPop.Position=UDim2.new(0,ap.X+as.X-200,0,ap.Y-TOTAL_H-6)
    _fbcPop.Size=UDim2.new(0,200,0,0); _fbcPop.BackgroundTransparency=0.4; _fbcPop.Visible=true
    TweenService:Create(_fbcPop,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Size=UDim2.new(0,200,0,TOTAL_H),BackgroundTransparency=0}):Play()
...
local function _fbcClosePop()
    ...
    _fbcOpen=false
    TweenService:Create(_fbcPop,TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.In),
        {Size=UDim2.new(0,200,0,0),BackgroundTransparency=0.3}):Play()
    task.delay(0.13,function() _fbcPop.Visible=false end)
    TweenService:Create(_fbcArrow,TweenInfo.new(0.18),{Rotation=0}):Play()
    TweenService:Create(_fbcModeBtn,TweenInfo.new(0.1),{BackgroundTransparency=0.2}):Play()
    _vdOpen=nil
end
...
-- ── iOS Style Toggle (no lugar do botão antigo) ────────────────
local _fbcBtn,_fbcBtnS=_accToggleSwitch(_fbcCF,_y,"🎁",FBC_COR); _y=_y+40
local _fbcStatus=_accStatusLbl(_fbcCF,_y); fbcStatus=_fbcStatus

-- ── Remote para abrir baús ──────────────────────────────────────
local _fbcRemote = nil
pcall(function()
    _fbcRemote = game:GetService("ReplicatedStorage")
        :WaitForChild("RemoteEvents", 5)
        :WaitForChild("RequestOpenItemChest", 5)
end)

-- ── Lógica Farm Baús melhorada ────────────────────────────────
        ...
        for _,obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Model") then
                ...
                if nm:find("chest",1,true) or nm:find("bau",1,true) or nm:find("baú",1,true) then
                    ...
                    if pp and pp.Position.Y>-200 then
                        table.insert(chests,{model=obj,part=pp,pos=pp.Position})
                    end
                end
            end
        end
...
local function autoFarmBaus()
    ...
    -- Salva posição original do jogador para modo "Jogador"
    local originPos = nil
    local ch = Player.Character
    if ch then
        local hrp = ch:FindFirstChild("HumanoidRootPart")
        if hrp then originPos = hrp.Position end
    end
    if not originPos then Notify.warn("Farm Baús","Personagem não encontrado!",3); fbcRunning=false; return end
    
    local chests=findAllChests()
    if #chests==0 then Notify.warn("Farm Baús","Nenhum baú encontrado!",4); fbcRunning=false; return end
    
    -- Determina posição de destino (fogueira ou jogador)
    local fogPos=nil
    if fbcMode=="fogueira" or fbcMode=="pertofog" then fogPos=getCampfirePos() end
    local destPos = originPos
    if fbcMode=="fogueira" and fogPos then destPos = fogPos
    elseif fbcMode=="pertofog" and fogPos then destPos = fogPos end
    
    local collected=0; local skipped=0
    ...
    for i,chest in ipairs(chests) do
        ...
        local ok = pcall(function()
            ...
            -- 1. Teleporta para o baú
            hrp2.CFrame=CFrame.new(chest.pos+Vector3.new(0,4,0))
            task.wait(0.3)
            
            -- 2. Abre o baú via RemoteEvent (se disponível)
            if _fbcRemote and chest.model and chest.model.Parent then
                pcall(function() _fbcRemote:FireServer(chest.model) end)
            end
            
            -- 3. Varre itens próximos em múltiplas tentativas
            local nearItems={}; local seen={}
            local scanAttempts=3
            for attempt=1,scanAttempts do
                ...
                pcall(function()
                    for _,obj in ipairs(workspace:GetDescendants()) do
                        ...
                        -- Pula o próprio baú
                        if obj==chest.model or obj:IsDescendantOf(chest.model) then return end
                        -- Pula o jogador
                        ...
                        if not p2 then return end
                        local key=tostring(p2)
                        if seen[key] then return end
                        local dist=(p2.Position-chest.pos).Magnitude
                        if dist<14 then
                            seen[key]=true
                            -- Verifica se tem um nome válido (item do jogo)
                            local nm=obj.Name:lower()
                            if not (nm:find("chest",1,true) or nm:find("bau",1,true) or nm:find("baú",1,true) or nm:find("humanoid",1,true) or nm:find("rootpart",1,true)) then
                                table.insert(nearItems,{part=p2,obj=obj})
                            end
                        end
                    end
                end)
                if #nearItems>0 then break end -- já achou itens, não precisa mais escanear
                if attempt<scanAttempts then task.wait(0.5) end
            end
            
            -- 4. Move os itens para o destino
            if #nearItems>0 then
                local lote=#nearItems
                for si,e in ipairs(nearItems) do
                    if not fbcRunning then break end
                    if e.part and e.part.Parent and e.obj and e.obj.Parent then
                        local ang=(si/lote)*math.pi*2
                        local r=1.5+math.floor((si-1)/8)*1.5
                        if fbcMode=="pertofog" then r=r+6 end -- um pouco mais afastado da fogueira
                        local target=Vector3.new(
                            destPos.X+math.cos(ang)*r,
                            destPos.Y+1,
                            destPos.Z+math.sin(ang)*r
                        )
                        pcall(function()
                            if e.obj:IsA("Model") then
                                e.obj:SetPrimaryPartCFrame(CFrame.new(target))
                            else
                                e.part.CFrame=CFrame.new(target)
                            end
                        end)
                        collected=collected+1; task.wait(0.05)
                    end
                end
            else
                skipped=skipped+1
        ...
        pcall(function()
            _fbcStatus.Text=string.format("🎁 Baú %d/%d — %d itens",i,#chests,collected)
        end)
        task.wait(0.2)
    end
    
    local msg=string.format("✅ %d itens de %d baús!",collected,#chests)
    if skipped>0 then msg=msg..string.format(" (%d baús vazios)",skipped) end
    Notify.success("Farm Baús",msg,5)
    fbcRunning=false
...
local function findBiomePos(biome)
    -- Floresta = área inicial, mais confiável usar a posição real da fogueira
    -- do que tentar achar por palavra-chave (evita colisão com outros biomas)
    if biome.useCampfire then
        local fp = getCampfirePos()
        if fp then return fp end
        -- se não achar a fogueira por algum motivo, cai pro método normal
    end
