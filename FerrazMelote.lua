...
local TR_BASE = {
    ...
    -- ── Status Tab ────────────────────────────────────
    stLive="● LIVE",
    stFps="FPS",                  stFpsExc=T("stFpsExc"),    stFpsBom=T("stFpsBom"),     stFpsBaixo=T("stFpsBaixo"),
    stPing="Ping",                stPingBoa=T("stPingBoa"), stPingMod=T("stPingMod"),stPingRuim=T("stPingRuim"),
    ...
    -- ── Farm Tab — Kill Aura ──────────────────────────
    secCombate="⚔️  COMBATE",
    killAuraTitle="⚔️  Kill Aura",
    killAuraDesc="Equipe qualquer arma (espada, machado, faca...) — o hitbox vai até cada mob automaticamente. Dano real da arma, sem animação, sem limite de distância.",
    killAuraWarn="⚠️ Kill Aura usa ToolDamageObject real. Precisa de ferramenta no Inventory. Funciona com espada, machado, faca, qualquer arma melee.",
    killAuraRadius="⚔️  Raio do Kill Aura",
    killAuraOn="Ativada — equipe uma arma melee. Hitbox vai até os mobs! ⚔️",
    killAuraOff="Desativada — cliques voltam ao normal.",
    killAuraConnected="⚔️  conectado — clique para acertar TODOS no range!",
    killAuraEquip="Equipe uma arma — Kill Aura conecta sozinho! ⚔️",
    killAuraEquipWarn="Equipe uma arma para o Kill Aura funcionar! ⚔️",
    -- ── Farm Tab — Aura Congelar ──────────────────────
    freezeTitle="❄️  Aura Congelar",
    freezeDesc="Congela todos os mobs no raio — imóveis, presos no chão.",
    freezeOn="Aura Congelar",     freezeOnMsg=" studs de raio.",
    freezeOff="Aura Congelar",    freezeOffMsg="Desativada — mobs descongelados.",
    -- ── Player Tab ────────────────────────────────────
    ...
    ancChestTitle="📦  ANC Chest",ancChestDesc="Baús abrem instantaneamente.",
    ancChestOn="ANC Chest",       ancChestOnMsg="Baús agora abrem instantaneamente.",
    ancChestOff="ANC Chest",      ancChestOffMsg="Velocidade de baú normalizada.",
    ...
    -- ── Avançado Funções — Kill Aura AUTO ─────────────
    kaAutoSecTitle="⚔️ KILL AURA — ToolDamageObject Real",
    kaAutoTitle="⚔️ Kill Aura AUTO",
    kaAutoDesc="Dano automático via RemoteFunction real do jogo. Arma do Inventory, sem precisar equipar.",
    kaAutoKillAllTitle="🎯 Kill ALL no Range",
    kaAutoKillAllDesc="Ataca TODOS os inimigos no raio. Desativado = só o mais próximo.",
    kaAutoRangeLabel="📏 Kill Aura Range",
    kaAutoRateLabel="⚡ Fire Rate (delay)",
    kaAutoOn="Kill Aura",         kaAutoOnMsg="Ativado | Range: ",kaAutoOnSuffix="st",
    kaAutoOff="Kill Aura",        kaAutoOffPrefix="Desativado. Abates: ",
    kaAutoKillAllOn="Kill Aura",  kaAutoKillAllOnMsg="Modo: todos no range ativado.",
    kaAutoKillAllOff="Kill Aura", kaAutoKillAllOffMsg="Modo: só o mais próximo.",
    -- ── Aimbot ────────────────────────────────────────
...
local TR_LANGS = {
    ["EN-US"] = {
        ...
        -- Kill Aura
        killAuraTitle="⚔️  Kill Aura",
        killAuraDesc="Equip any weapon (sword, axe, knife...) — hitbox goes to each mob automatically. Real weapon damage, no animation, no distance limit.",
        killAuraWarn="⚠️ Kill Aura uses real ToolDamageObject. Needs tool in Inventory. Works with sword, axe, knife, any melee weapon.",
        killAuraRadius="⚔️  Kill Aura Radius",
        killAuraOn="Activated — equip a melee weapon. Hitbox goes to mobs! ⚔️",
        killAuraOff="Deactivated — clicks back to normal.",
        killAuraConnected="⚔️  connected — click to hit ALL in range!",
        killAuraEquip="Equip a weapon — Kill Aura connects automatically! ⚔️",
        killAuraEquipWarn="Equip a melee weapon for Kill Aura to work! ⚔️",
        -- Freeze
        freezeTitle="❄️  Freeze Aura",
        freezeDesc="Freezes all mobs in radius — immobile, stuck to the ground.",
        freezeOnMsg=" studs radius.",
        ...
        ancChestTitle="📦  ANC Chest", ancChestDesc="Chests open instantly.",
        ancChestOnMsg="Chests now open instantly.",
        ancChestOffMsg="Chest speed normalized.",
        ...
        -- Kill Aura AUTO
        kaAutoSecTitle="⚔️ KILL AURA — Real ToolDamageObject",
        kaAutoTitle="⚔️ Kill Aura AUTO",
        kaAutoDesc="Automatic damage via real RemoteFunction. Weapon from Inventory, no need to equip.",
        kaAutoKillAllTitle="🎯 Kill ALL in Range",
        kaAutoKillAllDesc="Attacks ALL enemies in radius. Off = closest only.",
        kaAutoRangeLabel="📏 Kill Aura Range",
        kaAutoRateLabel="⚡ Fire Rate (delay)",
        kaAutoOn="Kill Aura",          kaAutoOff="Kill Aura",
        kaAutoOffPrefix="Disabled. Kills: ",
        kaAutoKillAllOnMsg="Mode: attack all in range enabled.",
        kaAutoKillAllOffMsg="Mode: closest only.",
        -- Aimbot
    ...
    ["ES-ES"] = {
        ...
        killAuraTitle="⚔️  Kill Aura",
        killAuraDesc="Equipa cualquier arma — el hitbox va a cada mob automáticamente.",
    ...
    ["ZH-CN"] = {
        ...
        killAuraTitle="⚔️  杀戮光环",  freezeTitle="❄️  冻结光环",
...
-- Suporta MÚLTIPLOS labels por chave (array por chave)
...
-- T() — retorna tradução de uma chave no idioma atual
-- Uso: T("killAuraOn") em qualquer lugar do script
...
local function applyLanguage(lang)
    currentLang = lang
    local trans = TRANSLATIONS[lang.code] or TRANSLATIONS["PT-BR"]
    -- Atualiza todos os labels rastreados (agora suporta múltiplos por chave)
    for key, lblList in pairs(langTrackedLabels) do
        local val = trans[key] or TRANSLATIONS["PT-BR"][key]
        if val then
            for _, lbl in ipairs(lblList) do
                pcall(function()
                    if lbl and lbl.Parent then lbl.Text = val end
                end)
            end
        end
    end
    -- Atualiza label "Idioma:" com dois-pontos
    if infoLangKeyLbl then
        local word = trans.langCurrent or TRANSLATIONS["PT-BR"].langCurrent or "Idioma"
        infoLangKeyLbl.Text = word .. ":"
    end
end
...
local GroupConfig = {
    ...
    {label="COMBATE", trKey="groupCombate", keys={"Farm","Esp","Bring","AvancadoFarm"}},
...
local function setFly(state)
    ...
    if state then
        ...
        Notify.info(T("flyOn"), T("flyOnMsg"))
    else
...
local function setBauANC(state)
    ...
    if state then
        ...
        Notify.info(T("ancChestOn"), T("ancChestOnMsg"))
    else
        if bauANCConn then bauANCConn:Disconnect(); bauANCConn=nil end
        Notify.info(T("ancChestOff"), T("ancChestOffMsg"))
    end
end
...
fzBtn.MouseButton1Click:Connect(function()
    ...
    if freezeEnabled then
        startFreezeAura()
        Notify.send({type="custom",icon="❄️",accent=FREEZE_COR,title=T("freezeOn"),msg=tostring(freezeRadius=="inf" and "∞" or math.floor(freezeRadius))..T("freezeOnMsg"),duration=3})
    else
        stopFreezeAura()
...
-- ══════════════════════════════════════════════════════
--  ⚔️ KILL AURA REAL v4 — Farm Tab
--
--  RemoteFunction REAL descoberto via Remote Spy:
--  ReplicatedStorage.RemoteEvents.ToolDamageObject:InvokeServer(
--      mob,        -- [1] Model do mob em workspace.Characters
--      weapon,     -- [2] Item da LocalPlayer.Inventory (não do Character!)
--      attackId,   -- [3] string ID do ataque (lida da animação tocando)
--      hitCFrame   -- [4] CFrame da posição do hit (HRP do mob)
--  )
--
--  Todos os mobs no range recebem InvokeServer individual.
--  Delay natural = lido do AnimationTrack da arma em tempo real.
-- ══════════════════════════════════════════════════════
local killAuraEnabled     = false
local killAuraRadius      = 30
local killAuraConn        = nil   -- conexão no Activated da tool
local killAuraToolWatcher = nil   -- detecta troca de ferramenta
local KILL_COR            = Color3.fromRGB(255, 80, 80)
local kaNoToolWarned      = false
local currentKaToolName   = ""

-- RemoteFunction de dano real do jogo
local KA_RF = nil
local function loadKillAuraRemote()
    pcall(function()
        KA_RF = game:GetService("ReplicatedStorage")
            .RemoteEvents.ToolDamageObject
    end)
end
task.delay(2, loadKillAuraRemote)

-- ─── Pega arma do Inventory (não do Character!) ────────
-- O servidor valida o [2] como item do Inventory, não da mão
local function getWeaponFromInventory(toolName)
    local inv = Player:FindFirstChild("Inventory")
    if not inv then return nil end
    -- Tenta pelo nome exato da tool equipada
    local item = inv:FindFirstChild(toolName)
    if item then return item end
    -- Fallback: qualquer item do Inventory
    for _, v in ipairs(inv:GetChildren()) do
        if v:IsA("Tool") or v:IsA("Instance") then return v end
    end
    return nil
end

-- ─── Lê o Attack ID real da animação tocando ───────────
-- O servidor usa esse ID para validar que o ataque é legítimo.
-- Formato real capturado: "1_8853122779" (índice_animId)
-- Lemos da AnimationTrack ativa para ser 100% natural.
local function getAttackId()
    local result = nil
    pcall(function()
        local ch = Player.Character; if not ch then return end
        local hum = ch:FindFirstChildWhichIsA("Humanoid"); if not hum then return end
        ...
        for _, track in ipairs(tracks) do
            ...
            -- Ignora animações de movimento — pega a de ataque
            local isMovement = nm:find("idle") or nm:find("walk") or nm:find("run")
                or nm:find("jump") or nm:find("fall") or nm:find("swim") or nm:find("climb")
            if not isMovement and track.Length > 0.05 then
                -- Constrói o ID no formato real do jogo: "1_animationId"
                local animId = tostring(track.Animation and track.Animation.AnimationId or "")
                -- Remove prefixo rbxassetid:// se existir
    ...
    -- Fallback: usa o ID capturado no Remote Spy (Old Axe)
    -- Para outras armas o servidor pode aceitar este mesmo padrão
    return result or "1_8853122779"
end

-- ─── Lê delay natural da animação de ataque ────────────
-- Não usamos delay fixo — lemos o Length real da animação
local function getNaturalAttackDelay()
    local result = 0.4  -- fallback seguro
    pcall(function()
        local ch = Player.Character; if not ch then return end
        local hum = ch:FindFirstChildWhichIsA("Humanoid"); if not hum then return end
        local animator = hum:FindFirstChildWhichIsA("Animator"); if not animator then return end
...
-- ─── Hit em todos os mobs usando o RemoteFunction real ─
local function doKillAuraHit(toolName)
    if not KA_RF then loadKillAuraRemote() end
    if not KA_RF then return end

    -- Pega arma do Inventory (argumento [2] real do jogo)
    local weapon = getWeaponFromInventory(toolName)
    if not weapon then return end

    -- Lê attack ID da animação atual
    local attackId = getAttackId()

    -- Busca todos os mobs no range
    local ch = Player.Character; if not ch then return end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    local mobs = getMobsInRange(hrp.Position, killAuraRadius)

    for _, entry in ipairs(mobs) do
        task.spawn(function()
            pcall(function()
                if not entry.model or not entry.model.Parent then return end
                if not entry.hum or entry.hum.Health <= 0 then return end
                local mobHrp = entry.model:FindFirstChild("HumanoidRootPart")
                if not mobHrp then return end

                -- Invoca o RemoteFunction real com os 4 argumentos corretos:
                -- [1] mob model  [2] weapon do Inventory  [3] attackId  [4] CFrame do hit
                KA_RF:InvokeServer(
                    entry.model,    -- mob em workspace.Characters
                    weapon,         -- item do LocalPlayer.Inventory
                    attackId,       -- ID do ataque lido da animação
                    mobHrp.CFrame   -- posição exata do hit
                )
            end)
        end)
    end
...
-- ─── Conecta no Activated da ferramenta equipada ───────
-- Quando o jogador clica → Activated dispara → invocamos
-- o remote para TODOS os mobs no range nesse mesmo instante
local function connectKillAuraToTool(tool)
    ...
    currentKaToolName = tool.Name
    kaNoToolWarned    = false

    killAuraConn = tool.Activated:Connect(function()
        if not killAuraEnabled then return end
        task.spawn(function()
            doKillAuraHit(tool.Name)
        end)
    end)

    Notify.info(T("killAuraTitle"), "⚔️ "..tool.Name..T("killAuraConnected"), 4)
end

-- ─── Start / Stop ───────────────────────────────────────
local function startKillAura()
    if killAuraToolWatcher then
        pcall(function() killAuraToolWatcher:Disconnect() end)
        killAuraToolWatcher = nil
    end
    kaNoToolWarned    = false
    currentKaToolName = ""

    -- Conecta imediatamente se já tiver arma
    local tool = getEquippedTool()
    if tool then
        connectKillAuraToTool(tool)
    else
        Notify.warn(T("killAuraTitle"), T("killAuraEquip"), 4)
    end

    -- Monitora troca de ferramenta em tempo real
    killAuraToolWatcher = RunService.Heartbeat:Connect(function()
        if not killAuraEnabled then return end
        local currentTool = getEquippedTool()
        local currentName = currentTool and currentTool.Name or ""
        if currentName == currentKaToolName then return end  -- nada mudou
        currentKaToolName = currentName
        if currentTool then
            connectKillAuraToTool(currentTool)
        else
            if killAuraConn then
                pcall(function() killAuraConn:Disconnect() end)
                killAuraConn = nil
            end
            if not kaNoToolWarned then
                kaNoToolWarned = true
                Notify.warn(T("killAuraTitle"), T("killAuraEquipWarn"), 4)
            end
        end
    end)
end
...
-- Pega a ferramenta melee equipada no character
...
trackLabel(kaTitleLbl, "killAuraTitle")
...
kaDescLbl.Text="Equipe qualquer arma (espada, machado, faca...) — o hitbox vai até cada mob automaticamente. Dano real da arma, sem animação, sem limite de distância."; kaDescLbl.TextColor3=Color3.fromRGB(90,100,120)
trackLabel(kaDescLbl, "killAuraDesc")
...
kaBtn.MouseButton1Click:Connect(function()
    ...
    if killAuraEnabled then
        startKillAura()
        -- Notificação principal vem de dentro do startKillAura/connectKillAuraToTool
    else
        stopKillAura()
        Notify.info(T("killAuraTitle"), T("killAuraOff"))
    end
end)
...
warnLbl.Font=Enum.Font.GothamSemibold; warnLbl.Text="⚠️ Kill Aura usa Hitbox Teleport: o Handle da arma vai até o mob e Tool:Activate() dispara o dano real. Precisa de ferramenta equipada. Funciona com espada, machado, faca, qualquer arma melee."
trackLabel(warnLbl, "killAuraWarn")
...
do -- [[ KILL AURA + ADVANCED ]]

-- ══════════════════════════════════════════════════════
-- KILL AURA v3 — RemoteEvent Real: CarnivalCompleteShootingGallery
-- Pesquisado e confirmado em scripts públicos do jogo (Fev 2026)
-- Dispara dano server-side via RemoteEvent legítimo do jogo
-- ══════════════════════════════════════════════════════

-- Nomes reais dos inimigos confirmados no 99 Nights in the Forest
local ENEMY_NAMES = {
    -- Animais selvagens
    ["wolf"]=true,           ["alpha wolf"]=true,     ["alphawolf"]=true,
    ["bear"]=true,           ["polar bear"]=true,     ["polarbear"]=true,
    ["arctic fox"]=true,     ["arcticfox"]=true,      ["deer"]=true,
    ["frog"]=true,           ["blue frog"]=true,      ["purple frog"]=true,
    ["green frog"]=true,     ["bluefrog"]=true,       ["purplefrog"]=true,
    ["greenfrog"]=true,      ["scorpion"]=true,       ["hellephant"]=true,
    ["meteor crab"]=true,    ["meteorcrab"]=true,     ["mammoth"]=true,
    ["bunny"]=true,          ["horse"]=true,          ["kiwi"]=true,
    ["turkey"]=true,         ["alien"]=true,          ["elite alien"]=true,
    ["elitealien"]=true,     ["cultist"]=true,        ["ram"]=true,
    ["cultist ram"]=true,    ["owl"]=true,            ["bat"]=true,
    ["wolf alpha"]=true,     ["creature"]=true,       ["monster"]=true,
    ["wendigo"]=true,        ["entity"]=true,         ["enemy"]=true,
    -- Variações comuns de nome NPC no jogo
    ["npc"]=true,            ["mob"]=true,            ["boss"]=true,
}

-- RemoteEvent real descoberto via engenharia reversa de scripts públicos
-- Estrutura: ReplicatedStorage.RemoteEvents.CarnivalCompleteShootingGallery
-- Uso: Event:FireServer(basePart) — causa dano no servidor
local KA_Remote = nil
local KA_RemoteAlt = nil  -- fallback alternativo

local function loadRemotes()
    pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        -- Evento principal — confirmado em múltiplos scripts públicos
        if RS:FindFirstChild("RemoteEvents") then
            KA_Remote = RS.RemoteEvents:FindFirstChild("CarnivalCompleteShootingGallery")
        end
        -- Evento alternativo de dano (descoberto em outros scripts)
        if RS:FindFirstChild("Events") then
            KA_RemoteAlt = RS.Events:FindFirstChild("DamageEnemy")
                or RS.Events:FindFirstChild("AttackEntity")
                or RS.Events:FindFirstChild("HitEnemy")
        end
        -- Fallback: scan completo do ReplicatedStorage por eventos de dano
        if not KA_Remote then
            for _, child in ipairs(RS:GetDescendants()) do
                if child:IsA("RemoteEvent") then
                    local nm = child.Name:lower()
                    if nm:find("shoot") or nm:find("shooting") or nm:find("carnival") then
                        KA_Remote = child; break
                    end
                end
            end
        end
...
task.delay(2, loadRemotes)  -- espera o jogo carregar

-- Configurações do Kill Aura
local kaEnabled     = false
local kaAuto        = false       -- Kill Aura automático (sem necessidade de arma)
local kaRange       = 50          -- Raio em studs
local kaRate        = 0.08        -- Segundos entre disparos (mais baixo = mais rápido)
local kaKillAll     = true        -- Atacar todos no range ou só o mais próximo
local kaRunning     = false
local kaKillCount   = 0

-- Cache de personagens dos players para não atacar jogadores
local function getPlayerChars()
    local t = {}
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl.Character then t[pl.Character] = true end
    end
    return t
end

-- Busca inimigos no range — APENAS os da lista ENEMY_NAMES (Opção A: lista branca)
-- NPCs amigos, crianças, mercadores e outros fora da lista são 100% ignorados
local function findEnemiesInRange()
    ...
    -- Escaneia todas as áreas relevantes do workspace
    local areas = {workspace}
    pcall(function()
        if workspace:FindFirstChild("Characters") then table.insert(areas, workspace.Characters) end
        if workspace:FindFirstChild("Map")        then table.insert(areas, workspace.Map)        end
        if workspace:FindFirstChild("Entities")   then table.insert(areas, workspace.Entities)   end
        if workspace:FindFirstChild("Mobs")       then table.insert(areas, workspace.Mobs)       end
        if workspace:FindFirstChild("Enemies")    then table.insert(areas, workspace.Enemies)    end
    end)
    ...
    for _, area in ipairs(areas) do
        pcall(function()
            for _, obj in ipairs(area:GetDescendants()) do
                if scanned[obj] then continue end; scanned[obj] = true
                pcall(function()
                    if not obj:IsA("Model") then return end

                    -- 🛡️ LISTA BRANCA: ignora qualquer NPC que NÃO está em ENEMY_NAMES
                    -- Isso protege crianças, mercadores, NPCs de quest, aldeões, etc.
                    if not ENEMY_NAMES[obj.Name:lower()] then return end

                    -- 🛡️ Nunca atacar jogadores reais
                    ...
                    if dist > kaRange then return end

                    table.insert(found, {model=obj, root=rootPart, hum=hum, dist=dist})
                end)
            end
        end)
    end
...
-- Função que dispara o RemoteEvent de dano em um alvo
local function fireKillEvent(targetPart)
    if not targetPart or not targetPart.Parent then return false end
    local fired = false
    -- Método 1: RemoteEvent principal (CarnivalCompleteShootingGallery)
    if KA_Remote then
        pcall(function()
            KA_Remote:FireServer(targetPart)
            fired = true
        end)
    end
    -- Método 2: RemoteEvent alternativo
    if KA_RemoteAlt then
        pcall(function()
            KA_RemoteAlt:FireServer(targetPart)
            fired = true
        end)
    end
    -- Método 3: Fallback — procura qualquer RemoteEvent de dano em tempo real
    if not fired then
        pcall(function()
            local RS = game:GetService("ReplicatedStorage")
            for _, child in ipairs(RS:GetDescendants()) do
                if child:IsA("RemoteEvent") then
                    local nm = child.Name:lower()
                    if nm:find("shoot") or nm:find("damage") or nm:find("attack") or nm:find("hit") then
                        child:FireServer(targetPart)
                        fired = true
                        break
                    end
                end
...
local function runKillAura()
    if kaRunning then return end; kaRunning = true
    task.spawn(function()
        while kaAuto do
            task.wait(kaRate)
            if not kaAuto then break end
            pcall(function()
                local enemies = findEnemiesInRange()
                if #enemies == 0 then return end
                if kaKillAll then
                    -- Ataca todos os inimigos no range
                    for _, e in ipairs(enemies) do
                        if not kaAuto then break end
                        if e.root and e.root.Parent then
                            local ok = fireKillEvent(e.root)
                            if ok then
                                kaKillCount += 1
                                -- Também tenta via todas as BaseParts do modelo para garantir hit
                                pcall(function()
                                    for _, part in ipairs(e.model:GetDescendants()) do
                        ...
                        task.wait(kaRate * 0.5)
                    end
                else
                    -- Ataca só o mais próximo
                    local e = enemies[1]
                    if e and e.root and e.root.Parent then
                        fireKillEvent(e.root)
                        kaKillCount += 1
                    end
                end
            end)
...
-- Aimbot Guided (mantido do original — guia projéteis)
...
local function makeTeleportPanel(cfg)
    ...
    selBtn.Font = Enum.Font.GothamBold; selBtn.Text = T("tpPanelSelect")
    ...
    local function openDrop()
        dropOpen = true; selBtn.Text = T("tpPanelClose")
    ...
    local function closeDrop()
        dropOpen = false; selBtn.Text = T("tpPanelSelect")
    ...
    for idx, item in ipairs(ITEMS) do
        ...
        row.MouseButton1Click:Connect(function()
            ...
            tpBtn.Text = T("tpPanelBtn"); tpBtn.BackgroundTransparency = 0.1
    ...
    tpBtn.MouseButton1Click:Connect(function()
        ...
        tpBtn.Text = T("tpPanelSearching"); tpBtn.BackgroundTransparency = 0.4
        ...
        task.spawn(function()
            ...
            if pos then
                ...
                if ok then
                    tpBtn.Text = T("tpPanelArrived")
                    tpBtn.BackgroundTransparency = 0.05
                    statusDot.BackgroundColor3 = Color3.fromRGB(87,242,135)
                    Notify.success(cfg.notifTag, T("tpPanelSuccess")..selectedItem.name..T("tpPanelSuccessEnd"), 4)
                    ...
                    tpBtn.Text = "🚀  Teleportar"; tpBtn.BackgroundTransparency = 0.1
                    progressBar.Size = UDim2.new(0,0,0,BAR_H)
                    errLbl.Text = T("tpPanelError")
                    errRow.Visible = true; refreshPnl(true)
                    Notify.error(cfg.notifTag, T("tpPanelFail"))
                ...
                errLbl.Text = "🔍 "..selectedItem.name..T("tpPanelNotFound")
                errRow.Visible = true; refreshPnl(true)
                Notify.warn(cfg.notifTag, selectedItem.name..T("tpPanelNotFound"), 5)
...
makeAvSec("⚔️ KILL AURA — RemoteEvent Real", Color3.fromRGB(255,60,60))

-- Toggle: Kill Aura Automático (usa RemoteEvent do jogo)
makeAvToggle("⚔️ Kill Aura AUTO", "Dano automático via RemoteEvent real do jogo. Não precisa de arma equipada.", Color3.fromRGB(255,60,60), function(s)
    kaAuto = s
    if s then
        -- Recarrega remotes caso não tenha carregado ainda
        if not KA_Remote then loadRemotes() end
        runKillAura()
        local remoteStatus = KA_Remote and "✓ Remote OK" or "⚠ Buscando remote..."
        Notify.warn(T("kaAutoOn"), T("kaAutoOnMsg")..tostring(kaRange)..T("kaAutoOnSuffix").." ("..remoteStatus..")")
    else
        Notify.info(T("kaAutoOff"), T("kaAutoOffPrefix")..tostring(kaKillCount))
    end
end)

-- Toggle: Kill All (todos no range) vs só o mais próximo
makeAvToggle("🎯 Kill ALL no Range", "Ataca TODOS os inimigos no raio. Desativado = só o mais próximo.", Color3.fromRGB(255,120,40), function(s)
    kaKillAll = s
    Notify.info(T("kaAutoKillAllOn"), s and T("kaAutoKillAllOnMsg") or T("kaAutoKillAllOffMsg"))
end)

-- Slider de range
do
    local sliderRow = Instance.new("Frame", Pages["AvancadoFuncoes"])
    sliderRow.BackgroundColor3 = Color3.fromRGB(28,30,38)
    sliderRow.BorderSizePixel = 0
    sliderRow.Size = UDim2.new(1,0,0,70)
    sliderRow.LayoutOrder = avNextLO()
    sliderRow.ZIndex = 5
    Instance.new("UICorner", sliderRow).CornerRadius = UDim.new(0,9)

    local cor = Color3.fromRGB(255,80,120)

    local sLbl = Instance.new("TextLabel", sliderRow)
    sLbl.BackgroundTransparency=1; sLbl.Position=UDim2.new(0,12,0,8)
    sLbl.Size=UDim2.new(1,-80,0,16); sLbl.Font=Enum.Font.GothamBold
    sLbl.Text="📏 Kill Aura Range"; sLbl.TextColor3=Color3.fromRGB(220,225,240)
    trackLabel(sLbl, "kaAutoRangeLabel")
    sLbl.TextSize=11; sLbl.TextXAlignment=Enum.TextXAlignment.Left; sLbl.ZIndex=6

    local sVal = Instance.new("TextLabel", sliderRow)
    sVal.BackgroundTransparency=1; sVal.Position=UDim2.new(1,-68,0,8)
    sVal.Size=UDim2.new(0,58,0,16); sVal.Font=Enum.Font.GothamBlack
    sVal.Text=tostring(kaRange).." st"; sVal.TextColor3=cor
    sVal.TextSize=11; sVal.TextXAlignment=Enum.TextXAlignment.Right; sVal.ZIndex=6

    -- Track
    local trackBg = Instance.new("Frame", sliderRow)
    trackBg.BackgroundColor3=Color3.fromRGB(40,44,56); trackBg.BorderSizePixel=0
    trackBg.Position=UDim2.new(0,12,0,38); trackBg.Size=UDim2.new(1,-24,0,8)
    trackBg.ZIndex=6; Instance.new("UICorner",trackBg).CornerRadius=UDim.new(1,0)

    local trackFill = Instance.new("Frame", trackBg)
    trackFill.BackgroundColor3=cor; trackFill.BorderSizePixel=0
    trackFill.Size=UDim2.new(kaRange/200, 0, 1, 0); trackFill.ZIndex=7
    Instance.new("UICorner",trackFill).CornerRadius=UDim.new(1,0)

    local knob = Instance.new("Frame", trackBg)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255); knob.BorderSizePixel=0
    knob.AnchorPoint=Vector2.new(0.5,0.5); knob.Position=UDim2.new(kaRange/200,0,0.5,0)
    knob.Size=UDim2.new(0,16,0,16); knob.ZIndex=8
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local kStroke=Instance.new("UIStroke",knob); kStroke.Color=cor; kStroke.Thickness=2

    local sBtn=Instance.new("TextButton",trackBg); sBtn.BackgroundTransparency=1
    sBtn.Size=UDim2.new(1,0,1,0); sBtn.Text=""; sBtn.ZIndex=9
    ...
    game:GetService("UserInputService").InputChanged:Connect(function(i)
        if not dragging then return end
        local absPos=trackBg.AbsolutePosition; local absSize=trackBg.AbsoluteSize
        local rel = math.clamp((i.Position.X - absPos.X)/absSize.X, 0, 1)
        kaRange = math.floor(rel * 200)  -- 0–200 studs
        if kaRange < 5 then kaRange=5 end
        trackFill.Size=UDim2.new(rel,0,1,0)
        knob.Position=UDim2.new(rel,0,0.5,0)
        sVal.Text=tostring(kaRange).." st"
    end)
end

-- Slider de fire rate
do
    local rateRow = Instance.new("Frame", Pages["AvancadoFuncoes"])
    rateRow.BackgroundColor3 = Color3.fromRGB(28,30,38)
    rateRow.BorderSizePixel = 0
    rateRow.Size = UDim2.new(1,0,0,70)
    rateRow.LayoutOrder = avNextLO()
    rateRow.ZIndex = 5
    Instance.new("UICorner", rateRow).CornerRadius = UDim.new(0,9)

    local cor = Color3.fromRGB(255,140,60)

    local rLbl = Instance.new("TextLabel", rateRow)
    rLbl.BackgroundTransparency=1; rLbl.Position=UDim2.new(0,12,0,8)
    rLbl.Size=UDim2.new(1,-80,0,16); rLbl.Font=Enum.Font.GothamBold
    rLbl.Text="⚡ Fire Rate (delay)"; rLbl.TextColor3=Color3.fromRGB(220,225,240)
    trackLabel(rLbl, "kaAutoRateLabel")
    rLbl.TextSize=11; rLbl.TextXAlignment=Enum.TextXAlignment.Left; rLbl.ZIndex=6

    local rVal = Instance.new("TextLabel", rateRow)
    rVal.BackgroundTransparency=1; rVal.Position=UDim2.new(1,-68,0,8)
    rVal.Size=UDim2.new(0,58,0,16); rVal.Font=Enum.Font.GothamBlack
    rVal.Text=tostring(kaRate).."s"; rVal.TextColor3=cor
    rVal.TextSize=11; rVal.TextXAlignment=Enum.TextXAlignment.Right; rVal.ZIndex=6

    local rTrack = Instance.new("Frame", rateRow)
    rTrack.BackgroundColor3=Color3.fromRGB(40,44,56); rTrack.BorderSizePixel=0
    rTrack.Position=UDim2.new(0,12,0,38); rTrack.Size=UDim2.new(1,-24,0,8)
    rTrack.ZIndex=6; Instance.new("UICorner",rTrack).CornerRadius=UDim.new(1,0)

    local rFill = Instance.new("Frame", rTrack)
    rFill.BackgroundColor3=cor; rFill.BorderSizePixel=0
    rFill.Size=UDim2.new(kaRate/0.5, 0, 1, 0); rFill.ZIndex=7
    Instance.new("UICorner",rFill).CornerRadius=UDim.new(1,0)

    local rKnob = Instance.new("Frame", rTrack)
    rKnob.BackgroundColor3=Color3.fromRGB(255,255,255); rKnob.BorderSizePixel=0
    rKnob.AnchorPoint=Vector2.new(0.5,0.5); rKnob.Position=UDim2.new(kaRate/0.5,0,0.5,0)
    rKnob.Size=UDim2.new(0,16,0,16); rKnob.ZIndex=8
    Instance.new("UICorner",rKnob).CornerRadius=UDim.new(1,0)
    local rkS=Instance.new("UIStroke",rKnob); rkS.Color=cor; rkS.Thickness=2

    local rBtn=Instance.new("TextButton",rTrack); rBtn.BackgroundTransparency=1
    rBtn.Size=UDim2.new(1,0,1,0); rBtn.Text=""; rBtn.ZIndex=9

    local rDrag=false
    rBtn.MouseButton1Down:Connect(function() rDrag=true end)
    game:GetService("UserInputService").InputEnded:Connect(function(i)
    ...
    game:GetService("UserInputService").InputChanged:Connect(function(i)
        ...
        local absPos=rTrack.AbsolutePosition; local absSize=rTrack.AbsoluteSize
        local rel = math.clamp((i.Position.X - absPos.X)/absSize.X, 0, 1)
        kaRate = math.max(0.03, rel * 0.5)  -- 0.03s (rápido) a 0.5s (devagar)
        rFill.Size=UDim2.new(rel,0,1,0)
        rKnob.Position=UDim2.new(rel,0,0.5,0)
        rVal.Text=string.format("%.2fs", kaRate)
    end)
end
