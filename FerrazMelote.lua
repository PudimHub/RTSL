...
-- ══════════════════════════════════════════════════════
-- TOP BAR — Brawl Stars 2026 Cartoon Style
-- ══════════════════════════════════════════════════════
...
TopBarGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,  Color3.fromRGB(255, 245, 80)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 200, 0)),
    ColorSequenceKeypoint.new(1,  Color3.fromRGB(210, 135, 0)),
})
TopBarGrad.Rotation = 90
-- Borda cartoon preta grossa
local TopBarStroke = Instance.new("UIStroke", TopBar)
TopBarStroke.Color     = Color3.fromRGB(14, 6, 0)
...
TopShine.Size             = UDim2.new(0,90,0,10)
TopShine.Position         = UDim2.new(0,10,0,5)
TopShine.BackgroundColor3 = Color3.fromRGB(255,255,255)
TopShine.BackgroundTransparency = 0.50
TopShine.BorderSizePixel  = 0
TopShine.Rotation         = -3
TopShine.ZIndex           = 4
Instance.new("UICorner",TopShine).CornerRadius = UDim.new(1,0)
-- Shine menor
local TopShine2 = Instance.new("Frame", TopBar)
TopShine2.Size             = UDim2.new(0,45,0,6)
TopShine2.Position         = UDim2.new(0,14,0,18)
TopShine2.BackgroundColor3 = Color3.fromRGB(255,255,255)
TopShine2.BackgroundTransparency = 0.70
TopShine2.BorderSizePixel  = 0
TopShine2.Rotation         = -3
TopShine2.ZIndex           = 4
Instance.new("UICorner",TopShine2).CornerRadius = UDim.new(1,0)

-- ── Título estilo Brawl Stars — banner diagonal cartoon ─────────────
local TitleBox = Instance.new("Frame", TopBar)
TitleBox.BackgroundTransparency = 1
TitleBox.Position = UDim2.new(0, 8, 0, 0)
TitleBox.Size     = UDim2.new(0, 260, 1, 0)
TitleBox.ZIndex   = 4

-- Ícone pudim flutuando (Brawl Stars style - grande, animado)
...
TitleIcon.ZIndex   = 6
-- Ícone bobbing animado
TweenService:Create(TitleIcon, TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
    {Position=UDim2.new(0,0,0.5,-18)}):Play()

-- Texto principal "PudimHub" — Brawl Stars: branco grande com stroke preta grossa
local TitleLabel = Instance.new("TextLabel", TitleBox)
TitleLabel.BackgroundTransparency = 1
TitleLabel.Position       = UDim2.new(0, 32, 0, -1)
TitleLabel.Size           = UDim2.new(1, -32, 0, 32)
TitleLabel.Font           = Enum.Font.GothamBlack
TitleLabel.Text           = "PudimHub"
...
TLStroke.Color     = Color3.fromRGB(14, 6, 0)
TLStroke.Thickness = 3.5

-- Badge "v5" — pequeno banner inclinado estilo Brawl Stars
local V5Badge = Instance.new("Frame", TitleBox)
V5Badge.BackgroundColor3 = Color3.fromRGB(255,60,80)
V5Badge.BorderSizePixel  = 0
V5Badge.Rotation         = -5
V5Badge.Position         = UDim2.new(0,32,0,27)
V5Badge.Size             = UDim2.new(0,28,0,14)
V5Badge.ZIndex           = 7
Instance.new("UICorner", V5Badge).CornerRadius = UDim.new(0,4)
local V5S = Instance.new("UIStroke", V5Badge)
V5S.Color = Color3.fromRGB(14,6,0); V5S.Thickness = 2
local V5G = Instance.new("UIGradient", V5Badge)
V5G.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(255,100,110)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(200,30,50)),
})
V5G.Rotation = 90
local V5Lbl = Instance.new("TextLabel", V5Badge)
V5Lbl.BackgroundTransparency = 1
V5Lbl.Size = UDim2.new(1,0,1,0)
V5Lbl.Font = Enum.Font.GothamBlack
V5Lbl.Text = "v5"
V5Lbl.TextColor3 = Color3.fromRGB(255,255,255)
V5Lbl.TextSize = 9
V5Lbl.ZIndex = 8
local V5LS = Instance.new("UIStroke", V5Lbl)
V5LS.Color = Color3.fromRGB(14,6,0); V5LS.Thickness = 1.5
-- Badge "v5" pulsante
TweenService:Create(V5Badge, TweenInfo.new(0.7,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
    {Size=UDim2.new(0,29,0,15)}):Play()

-- Estrelinhas animadas ao redor do título (Brawl Stars style)
local _titleStars = {
    {x=24, y=2,  sz=8,  rot=15},
    {x=150,y=-2, sz=7,  rot=-10},
    {x=230,y=4,  sz=6,  rot=20},
    {x=72, y=38, sz=7,  rot=-15},
    {x=190,y=36, sz=6,  rot=5},
}
for i, sd in ipairs(_titleStars) do
    local st = Instance.new("TextLabel", TitleBox)
    st.BackgroundTransparency = 1
    st.Text      = (i % 2 == 0) and "★" or "✦"
    st.TextSize  = sd.sz
    st.TextColor3= Color3.fromRGB(255,255,200)
    st.TextTransparency = 0.2
    st.Font      = Enum.Font.GothamBlack
    st.Position  = UDim2.new(0, sd.x, 0, sd.y)
    st.Size      = UDim2.new(0, sd.sz+2, 0, sd.sz+2)
    st.Rotation  = sd.rot
    st.ZIndex    = 7
    local UIStk = Instance.new("UIStroke", st)
    UIStk.Color = Color3.fromRGB(14,6,0); UIStk.Thickness=1
    -- Animação de piscar e rotacionar aleatório
    TweenService:Create(st, TweenInfo.new(0.6+i*0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
        {TextTransparency=0.75}):Play()
    TweenService:Create(st, TweenInfo.new(1.4+i*0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
        {Rotation=sd.rot+25}):Play()
end

-- ── Botões da TopBar — Brawl Stars cartoon round buttons ─────────────
local TopBtns = {}
local _btnDefs = {
    { name="Theme",    sym="⚡", col=Color3.fromRGB(120,60,220) },
    { name="Minimize", sym="—", col=Color3.fromRGB(60,160,255)  },
    { name="Maximize", sym="□", col=Color3.fromRGB(60,200,100)  },
    { name="Close",    sym="✕", col=Color3.fromRGB(255,60,80)   },
}
local _bx = -8
for _, d in ipairs(_btnDefs) do
    -- Container do botão
    local bFrame = Instance.new("Frame", TopBar)
    bFrame.Name            = d.name.."Frame"
    bFrame.BackgroundColor3= d.col
    bFrame.BorderSizePixel = 0
    bFrame.AnchorPoint     = Vector2.new(1, 0.5)
    bFrame.Position        = UDim2.new(1, _bx, 0.5, 0)
    bFrame.Size            = UDim2.new(0, 26, 0, 26)
    bFrame.ZIndex          = 6
    Instance.new("UICorner", bFrame).CornerRadius = UDim.new(1, 0)
    -- Borda cartoon preta
    local bStroke = Instance.new("UIStroke", bFrame)
    bStroke.Color     = Color3.fromRGB(14, 6, 0)
    bStroke.Thickness = 2.5
    bStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    -- Gradiente no botão
    local bGrad = Instance.new("UIGradient", bFrame)
    bGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(
            math.min(d.col.R*255+60,255),
            math.min(d.col.G*255+60,255),
            math.min(d.col.B*255+60,255))),
        ColorSequenceKeypoint.new(1, d.col),
    })
    bGrad.Rotation = 110
    -- Shine no botão
    local bShine = Instance.new("Frame", bFrame)
    bShine.BackgroundColor3 = Color3.fromRGB(255,255,255)
    bShine.BackgroundTransparency = 0.55
    bShine.BorderSizePixel = 0
    bShine.Position = UDim2.new(0,4,0,3)
    bShine.Size     = UDim2.new(0,10,0,5)
    bShine.ZIndex   = 7
    Instance.new("UICorner", bShine).CornerRadius = UDim.new(1,0)
    -- Símbolo do botão
    ...
    bLbl.TextSize = 12
    bLbl.ZIndex   = 7
    local bLblS = Instance.new("UIStroke", bLbl)
    bLblS.Color     = Color3.fromRGB(14,6,0)
    bLblS.Thickness = 1.8
    ...
    bBtn.Text  = ""
    bBtn.ZIndex= 8
    -- Hover: bounce + lighten
    bBtn.MouseEnter:Connect(function()
        TweenService:Create(bFrame, TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Size=UDim2.new(0,30,0,30)}):Play()
    end)
    bBtn.MouseLeave:Connect(function()
        TweenService:Create(bFrame, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Size=UDim2.new(0,26,0,26)}):Play()
    end)
    bBtn.MouseButton1Down:Connect(function()
        TweenService:Create(bFrame, TweenInfo.new(0.07), {Size=UDim2.new(0,22,0,22)}):Play()
    end)
    bBtn.MouseButton1Up:Connect(function()
        TweenService:Create(bFrame, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Size=UDim2.new(0,26,0,26)}):Play()
    end)
    TopBtns[d.name] = bBtn
    _bx = _bx - 32
end
-- Animação de entrada dos botões (aparecem de cima com bounce)
for i, d in ipairs(_btnDefs) do
    local fr = TopBar:FindFirstChild(d.name.."Frame")
    if fr then
        local origPos = fr.Position
        fr.Position = UDim2.new(origPos.X.Scale, origPos.X.Offset, origPos.Y.Scale, origPos.Y.Offset - 30)
        fr.BackgroundTransparency = 1
        task.delay(0.05 * i, function()
            TweenService:Create(fr, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                {Position=origPos, BackgroundTransparency=0}):Play()
        end)
    end
end
...
local function makeGroupLabel(text, trKey, groupTabs)
    ...
    -- Estrela branca (brilho) — posicionada corretamente dentro do badge
    local spark=Instance.new("TextLabel",badge)
    spark.BackgroundTransparency=1; spark.Text="✦"
    spark.Position=UDim2.new(0,2,0,1); spark.Size=UDim2.new(0,12,0,12)
    ...
    TweenService:Create(spark,TweenInfo.new(1.1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Rotation=25}):Play()
...
local function makeTab(cfg,groupTabs)
    ...
    TweenService:Create(spark,TweenInfo.new(1.0,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Rotation=20}):Play()
