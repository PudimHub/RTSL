...
-- Nomes possíveis das crianças no jogo (português + inglês + genéricos)
local CHILD_NAMES = {
    -- Nomes em português (99 Dias na Floresta)
    "crianca","criança","criança perdida","crianca perdida",
    "menino","menina","garoto","garota","infante",
    -- Nomes em inglês conhecidos
    "lost child","lostchild","missing child","missingchild",
    "captured child","capturedchild",
    "lost child 2","lost child 3","lost child 4",
    "lostchild2","lostchild3","lostchild4",
    -- Chapéu/tema (wiki)
    "dino kid","dinokid","dino",
    "kraken kid","krakenkid","kraken",
    "squid kid","squidkid","squid",
    "koala kid","koalakid","koala",
    -- Genéricos / variações de NPC
    "child","kid","npc_child","npc child","missedkid",
}
...
local function scanBuildings(isRefresh)
    ...
    task.spawn(function()
        ...
        -- Cria lookup rápido de nomes conhecidos
        local knownLookup = {}
        for _, sn in ipairs(KNOWN_STRUCTURES) do knownLookup[sn:lower()] = sn end

        pcall(function()
            -- Varre TUDO no workspace para não perder nada
            ...
            for _, obj in ipairs(descs) do
                ...
                if obj:IsA("Model") then
                    ...
                    -- Decide se é uma estrutura: nome exato OU contém keyword
                    local isBuilding = knownLookup[oNameLow] ~= nil
                    if not isBuilding then
                        for _, kw in ipairs(BUILDING_KEYWORDS) do
                            if oNameLow:find(kw, 1, true) then
                                isBuilding = true
                                break
                            end
                        end
                    ...
                    -- Precisa ter ao menos uma BasePart razoavelmente grande (evita items falso-positivos)
                    local part = nil
                    local bigEnough = false
                    for _, bp in ipairs(obj:GetDescendants()) do
                        if bp:IsA("BasePart") then
                            if not part then part = bp end
                            local sz = bp.Size
                            if sz.X > 3 or sz.Y > 3 or sz.Z > 3 then
                                bigEnough = true; part = bp; break
                            end
                        end
                    end
                    if not part or not bigEnough then continue end

                    -- Tem Humanoid = NPC, não construção
                    if obj:FindFirstChildWhichIsA("Humanoid") then continue end

                    local pos  = part.Position
                    -- Ignora objetos enterrados abaixo do mapa
                    if pos.Y < -500 then continue end

                    -- Deduplicação por POSIÇÃO (não por nome) → múltiplas casas do mesmo tipo aparecem
                    local pk = posKey(pos)
                    if buildingSeenKeys[pk] then continue end
                    buildingSeenKeys[pk] = true

                    makeBuildingBtn(oNameOrig, pos, isRefresh)
                    found += 1
                    buildEmptyLbl.Visible = false

                    -- Yield a cada 20 construções para não travar
...
-- ══════════════════════════════════════════════════════════════
-- PAINEL DE BAÚS — Tp por baú individual com raridade (Level)
-- ══════════════════════════════════════════════════════════════
local TP_COR_CHEST = Color3.fromRGB(255, 200, 60)

makeTpSec("🎁  PAINEL DE BAÚS", TP_COR_CHEST)

-- Raridade por Level (extraído do atributo "Level" ou do nome)
local RARITY_TABLE = {
    { max=1,  label="Level 1",  tag="LVL 1",  cor=Color3.fromRGB(140,160,180), icon="📦" },
    { max=2,  label="Level 2",  tag="LVL 2",  cor=Color3.fromRGB(100,210,120), icon="📦" },
    { max=3,  label="Level 3",  tag="LVL 3",  cor=Color3.fromRGB(80,160,255),  icon="💎" },
    { max=4,  label="Level 4",  tag="LVL 4",  cor=Color3.fromRGB(180,80,255),  icon="💎" },
    { max=5,  label="Level 5",  tag="LVL 5",  cor=Color3.fromRGB(255,140,30),  icon="👑" },
    { max=999,label="MAX",      tag="MAX",    cor=Color3.fromRGB(255,60,60),   icon="🔥" },
}
...
local function getRarity(chestModel)
    ...
    pcall(function()
        local v = chestModel:GetAttribute("Level")
              ...
              or chestModel:GetAttribute("Rarity")
              or chestModel:GetAttribute("ChestLevel")
        if type(v) == "number" then level = v end
    end)
    -- Tenta extrair do nome também (ex: "Item Chest 3", "Chest_Level2")
    if level == 0 then
        pcall(function()
            local nm = chestModel.Name
            local n = nm:match("%d+")
            if n then level = tonumber(n) end
        end)
    end
    for _, r in ipairs(RARITY_TABLE) do
        if level <= r.max then return r end
    end
    return RARITY_TABLE[#RARITY_TABLE]
end
...
chestBtnRefL.TextColor3 = Color3.fromRGB(255,220,150); chestBtnRefL.TextSize = 16; chestBtnRefL.ZIndex = 9
...
chestBtnClrL.Font = Enum.Font.GothamBold; chestBtnClrL.Text = "✕"
chestBtnClrL.TextColor3 = Color3.fromRGB(255,180,180); chestBtnClrL.TextSize = 14; chestBtnClrL.ZIndex = 9
...
-- ── Cria botão de baú ───────────────────────────────────────────
local function makeChestBtn(name, pos, rarity, isNew)
    ...
    nameLbl.Position = UDim2.new(0,32,0,5); nameLbl.Size = UDim2.new(0.52,0,0,16)
    nameLbl.Font = Enum.Font.GothamSemibold; nameLbl.Text = name
    nameLbl.TextColor3 = Color3.fromRGB(235,220,180); nameLbl.TextSize = 11
    ...
    rarLbl.Position = UDim2.new(0,32,0,23); rarLbl.Size = UDim2.new(0.52,0,0,14)
    rarLbl.Font = Enum.Font.GothamBold; rarLbl.TextColor3 = COR
    rarLbl.TextSize = 9; rarLbl.TextXAlignment = Enum.TextXAlignment.Left; rarLbl.ZIndex = 8
    local distTxt = ""
    pcall(function()
        local ch = Player.Character; local hrp = ch and ch:FindFirstChild("HumanoidRootPart")
        if hrp then distTxt = " • "..math.floor((pos-hrp.Position).Magnitude).."m" end
    end)
    rarLbl.Text = rarity.label .. distTxt

    -- Etiqueta NOVO / ANTIGO
    local tagBadge = Instance.new("Frame",row)
    tagBadge.BorderSizePixel = 0; tagBadge.AnchorPoint = Vector2.new(1,0)
    tagBadge.Position = UDim2.new(1,-42,0,5); tagBadge.Size = UDim2.new(0,42,0,14); tagBadge.ZIndex = 9
    Instance.new("UICorner",tagBadge).CornerRadius = UDim.new(0,4)
    local tagLbl = Instance.new("TextLabel",tagBadge)
    tagLbl.BackgroundTransparency = 1; tagLbl.Size = UDim2.new(1,0,1,0)
    tagLbl.Font = Enum.Font.GothamBlack; tagLbl.TextSize = 8; tagLbl.ZIndex = 10
    ...
    local function doTp()
        ...
        safeTp(pos, 5)
        entry.visited = true; tpLastChest = entry
        -- Flash no botão
        TweenService:Create(tpb,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(255,255,255),BackgroundTransparency=0}):Play()
        ...
        Notify.send({type="custom",icon=rarity.icon,accent=COR,
            title="Baú",msg=name.." ("..rarity.label..")",duration=2.5})
    end
    tpb.MouseButton1Click:Connect(doTp)
...
local function scanChests(isRefresh)
    ...
    chestScanRun = true
    chestBtnRefL.Text = "⏳"; task.delay(0.5, function() chestBtnRefL.Text = "↻" end)
    ...
    task.spawn(function()
        ...
        pcall(function()
            ...
            local batch = 0
            for _, obj in ipairs(descs) do
                batch += 1; if batch % 150 == 0 then task.wait() end
                if obj:IsA("Model") then
                    local nm = obj.Name:lower()
                    -- Filtra apenas baús — nome contém "chest" ou "bau"/"baú"
                    if not (nm:find("chest",1,true) or nm:find("bau",1,true) or nm:find("baú",1,true)) then continue end
                    -- Ignora modelos que são construções/terreno (muito grandes)
                    local part = nil
                    for _, bp in ipairs(obj:GetDescendants()) do
                        if bp:IsA("BasePart") then part = bp; break end
                    end
                    if not part then continue end
                    local sz = part.Size
                    if sz.X > 12 or sz.Y > 12 or sz.Z > 12 then continue end
                    -- Ignora abaixo do mapa
                    ...
                    makeChestBtn(obj.Name, part.Position, rarity, isRefresh)
                    found += 1
                    chestEmptyLbl.Visible = false
                    if found % 15 == 0 then task.wait() end
                end
            end
        end)

        -- Ordena por raridade (mais raro no topo)
        table.sort(chestButtons, function(a, b)
            return a.rarity.max > b.rarity.max
        end)
