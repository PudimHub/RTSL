...
-- ══════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════
-- POPUP DE CONFIRMAÇÃO DE IDIOMA — dentro da MainFrame
-- ══════════════════════════════════════════════════════
-- Overlay escurecido que cobre só a MainFrame
local LangPopupOverlay = Instance.new("TextButton", MainFrame)
LangPopupOverlay.Text = ""; LangPopupOverlay.AutoButtonColor = false
LangPopupOverlay.Name = "LangPopupOverlay"
LangPopupOverlay.BackgroundColor3 = Color3.fromRGB(0,0,0)
LangPopupOverlay.BackgroundTransparency = 0.45
LangPopupOverlay.BorderSizePixel = 0
LangPopupOverlay.Size = UDim2.new(1,0,1,0)
LangPopupOverlay.ZIndex = 80
LangPopupOverlay.Visible = false
Instance.new("UICorner", LangPopupOverlay).CornerRadius = UDim.new(0,12)

-- Card do popup centralizado dentro da MainFrame
local LangPopup = Instance.new("Frame", LangPopupOverlay)
LangPopup.Name = "LangPopup"
LangPopup.BackgroundColor3 = Color3.fromRGB(22,24,34)
LangPopup.BorderSizePixel = 0
LangPopup.AnchorPoint = Vector2.new(0.5,0.5)
LangPopup.Position = UDim2.new(0.5,0,0.5,0)
LangPopup.Size = UDim2.new(0,290,0,0)
LangPopup.ZIndex = 82
LangPopup.ClipsDescendants = true
Instance.new("UICorner",LangPopup).CornerRadius = UDim.new(0,16)
local lpStroke = Instance.new("UIStroke",LangPopup)
lpStroke.Color = Color3.fromRGB(88,101,242); lpStroke.Thickness = 2

-- Barra de cor no topo
local lpTopBar = Instance.new("Frame",LangPopup)
lpTopBar.BackgroundColor3 = Color3.fromRGB(88,101,242)
lpTopBar.BorderSizePixel = 0
lpTopBar.Size = UDim2.new(1,0,0,3); lpTopBar.ZIndex = 83
Instance.new("UICorner",lpTopBar).CornerRadius = UDim.new(0,16)

-- Ícone círculo
local lpIconBg = Instance.new("Frame",LangPopup)
lpIconBg.BackgroundColor3 = Color3.fromRGB(88,101,242)
lpIconBg.BackgroundTransparency = 0.7; lpIconBg.BorderSizePixel = 0
lpIconBg.AnchorPoint = Vector2.new(0.5,0)
lpIconBg.Position = UDim2.new(0.5,0,0,20); lpIconBg.Size = UDim2.new(0,50,0,50); lpIconBg.ZIndex = 83
...
lpIconLbl.Font=Enum.Font.GothamBold; lpIconLbl.Text="🌐"; lpIconLbl.TextSize=26; lpIconLbl.ZIndex=84

-- Título
local lpTitle = Instance.new("TextLabel",LangPopup)
lpTitle.BackgroundTransparency=1
lpTitle.Position = UDim2.new(0,16,0,82); lpTitle.Size = UDim2.new(1,-32,0,20)
lpTitle.Font = Enum.Font.GothamBlack; lpTitle.Text = "Deseja mudar o idioma?"
lpTitle.TextColor3 = Color3.fromRGB(235,238,255)
lpTitle.TextSize = 13; lpTitle.TextXAlignment = Enum.TextXAlignment.Center
lpTitle.TextWrapped = true; lpTitle.ZIndex = 83

-- Idioma selecionado (flag + nome)
local lpLangLbl = Instance.new("TextLabel",LangPopup)
lpLangLbl.BackgroundTransparency=1
lpLangLbl.Position = UDim2.new(0,16,0,106); lpLangLbl.Size = UDim2.new(1,-32,0,22)
lpLangLbl.Font = Enum.Font.GothamBold; lpLangLbl.Text = ""
lpLangLbl.TextColor3 = Color3.fromRGB(88,101,242)
lpLangLbl.TextSize = 14; lpLangLbl.TextXAlignment = Enum.TextXAlignment.Center; lpLangLbl.ZIndex = 83

-- Separador
local lpSep = Instance.new("Frame",LangPopup)
lpSep.BackgroundColor3 = Color3.fromRGB(50,55,75); lpSep.BorderSizePixel=0
lpSep.Position = UDim2.new(0,16,0,136); lpSep.Size = UDim2.new(1,-32,0,1); lpSep.ZIndex=83

-- Botões Não / Sim
local lpBtnsRow = Instance.new("Frame",LangPopup)
lpBtnsRow.BackgroundTransparency=1; lpBtnsRow.BorderSizePixel=0
lpBtnsRow.Position = UDim2.new(0,16,0,146); lpBtnsRow.Size = UDim2.new(1,-32,0,40); lpBtnsRow.ZIndex=83
local lpBtnsList = Instance.new("UIListLayout",lpBtnsRow)
lpBtnsList.FillDirection=Enum.FillDirection.Horizontal
lpBtnsList.Padding=UDim.new(0,12)
lpBtnsList.HorizontalAlignment=Enum.HorizontalAlignment.Center
lpBtnsList.VerticalAlignment=Enum.VerticalAlignment.Center

local lpNoBtn = Instance.new("TextButton",lpBtnsRow)
lpNoBtn.BackgroundColor3=Color3.fromRGB(45,48,65); lpNoBtn.BorderSizePixel=0
lpNoBtn.Size=UDim2.new(0,118,0,38); lpNoBtn.Font=Enum.Font.GothamBold
lpNoBtn.Text="Não"; lpNoBtn.TextColor3=Color3.fromRGB(185,190,215); lpNoBtn.TextSize=12; lpNoBtn.ZIndex=84
Instance.new("UICorner",lpNoBtn).CornerRadius=UDim.new(0,10)
Instance.new("UIStroke",lpNoBtn).Color=Color3.fromRGB(70,78,105)
...
lpYesBtn.Size=UDim2.new(0,118,0,38); lpYesBtn.Font=Enum.Font.GothamBold
lpYesBtn.Text="Sim"; lpYesBtn.TextColor3=Color3.fromRGB(255,255,255); lpYesBtn.TextSize=12; lpYesBtn.ZIndex=84
Instance.new("UICorner",lpYesBtn).CornerRadius=UDim.new(0,10)

local lpPendingLang = nil
local langInfoLabel = nil
local langFooterLabel = nil
...
local function showLangPopup(lang)
    lpPendingLang = lang
    local T = TRANSLATIONS[lang.code] or TRANSLATIONS["PT-BR"]
    lpTitle.Text    = T.popupTitle or "Deseja mudar o idioma?"
    lpNoBtn.Text    = T.popupNo   or "Não"
    lpYesBtn.Text   = T.popupYes  or "Sim"
    lpLangLbl.Text  = lang.flag .. "  " .. lang.name
    LangPopupOverlay.Visible = true
    LangPopup.Size = UDim2.new(0,290,0,0)
    TweenService:Create(LangPopupOverlay,TweenInfo.new(0.15),{BackgroundTransparency=0.45}):Play()
    TweenService:Create(LangPopup,TweenInfo.new(0.32,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,290,0,POPUP_H)}):Play()
end

local function hideLangPopup()
    TweenService:Create(LangPopupOverlay,TweenInfo.new(0.15),{BackgroundTransparency=1}):Play()
    TweenService:Create(LangPopup,TweenInfo.new(0.2,Enum.EasingStyle.Quad),{Size=UDim2.new(0,290,0,0)}):Play()
...
LangPopupOverlay.MouseButton1Click:Connect(function() hideLangPopup() end)
...
lpYesBtn.MouseButton1Click:Connect(function()
    ...
    -- Animação de confirmação no botão
    TweenService:Create(lpYesBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(60,200,130)}):Play()
    task.delay(0.15, function()
        applyLanguage(lpPendingLang)
        if langInfoLabel   then langInfoLabel.Text   = lpPendingLang.flag.."  "..lpPendingLang.short end
        if langFooterLabel then langFooterLabel.Text = lpPendingLang.flag.." "..lpPendingLang.short  end
        hideLangPopup()
        local T = TRANSLATIONS[lpPendingLang.code] or TRANSLATIONS["PT-BR"]
        task.delay(0.35, function()
            Notify.success(
                T.langSystem or "Language",
                (T.notifLangChanged or "Language changed to ")..lpPendingLang.flag.." "..lpPendingLang.name,
                4
            )
        end)
        TweenService:Create(lpYesBtn,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(88,101,242)}):Play()
    end)
end)

lpNoBtn.MouseEnter:Connect(function() TweenService:Create(lpNoBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(60,65,90)}):Play() end)
lpNoBtn.MouseLeave:Connect(function() TweenService:Create(lpNoBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(45,48,65)}):Play() end)
lpYesBtn.MouseEnter:Connect(function() TweenService:Create(lpYesBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(105,118,255)}):Play() end)
