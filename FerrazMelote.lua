...
-- ══════════════════════════════════════════════════════════════════════
-- PUDIM HUB — SPLASH PROFISSIONAL
-- Vidro rachado + Typewriter + Partículas
-- ══════════════════════════════════════════════════════════════════════
task.spawn(function()
    ...
    -- ── Paleta PudimHub ───────────────────────────────────────────────
    local C_BG       = Color3.fromRGB(8,  4,  18)   -- fundo profundo
    local C_PURPLE   = Color3.fromRGB(148,112,220)   -- roxo principal
    local C_VIOLET   = Color3.fromRGB(190,148,255)   -- violeta claro
    local C_GOLD     = Color3.fromRGB(255,200,60)    -- dourado acento
    local C_WHITE    = Color3.fromRGB(255,255,255)
    local C_CRACK    = Color3.fromRGB(180,148,255)   -- cor das rachaduras

    -- ── ScreenGui principal ───────────────────────────────────────────
    local SG = Instance.new("ScreenGui")
    SG.Name = "PudimHubSplash"; SG.ResetOnSpawn = false
    ...
    BG.Size = UDim2.fromScale(1,1); BG.BackgroundColor3 = C_BG
    BG.BackgroundTransparency = 0; BG.BorderSizePixel = 0; BG.ZIndex = 1

    -- Gradiente radial simulado no fundo
    local bgGrad = Instance.new("UIGradient", BG)
    bgGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(22,10,48)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(12,6,28)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(4,2,12)),
    })
    bgGrad.Rotation = 135

    -- ── RACHADURAS DE VIDRO ───────────────────────────────────────────
    -- Cada rachadura é uma série de frames finos em ângulos variados
    -- partindo de pontos de impacto no fundo
    local function makeCrack(parent, x, y, angle, length, thickness, alpha)
        local f = Instance.new("Frame", parent)
        f.BackgroundColor3 = C_CRACK
        f.BackgroundTransparency = alpha or 0.3
        f.BorderSizePixel = 0
        f.AnchorPoint = Vector2.new(0, 0.5)
        f.Position = UDim2.new(0, x, 0, y)
        f.Size = UDim2.new(0, length, 0, thickness or 1)
        f.Rotation = angle
        f.ZIndex = 2
        return f
    end

    local function makeCrackGroup(parent, cx, cy, numCracks)
        -- Gera um grupo de rachaduras irradiando de cx,cy
        for i = 1, numCracks do
            local baseAngle = (i / numCracks) * 360 + math.random(-15, 15)
            local len = math.random(60, 220)
            local thick = math.random(1, 2)
            local alpha = math.random(40, 75) / 100
            makeCrack(parent, cx, cy, baseAngle, len, thick, alpha)

            -- Sub-rachaduras ramificadas
            if math.random() > 0.4 then
                local subLen = math.floor(len * 0.45)
                local subAngle = baseAngle + math.random(-35, 35)
                local offX = math.floor(cx + math.cos(math.rad(baseAngle)) * len * 0.55)
                local offY = math.floor(cy + math.sin(math.rad(baseAngle)) * len * 0.55)
                makeCrack(parent, offX, offY, subAngle, subLen, 1, alpha + 0.15)
            end

            -- Micro-rachadura extra
            if math.random() > 0.6 then
                local mLen = math.random(20, 60)
                local mAngle = baseAngle + math.random(-50, 50)
                local mX = math.floor(cx + math.cos(math.rad(baseAngle)) * len * 0.3)
                local mY = math.floor(cy + math.sin(math.rad(baseAngle)) * len * 0.3)
                makeCrack(parent, mX, mY, mAngle, mLen, 1, alpha + 0.2)
            end
        end

        -- Ponto de impacto central (círculo pequeno)
        local dot = Instance.new("Frame", parent)
        dot.BackgroundColor3 = C_VIOLET
        ...
        -- Halo do impacto
        local halo = Instance.new("Frame", parent)
        halo.BackgroundColor3 = C_PURPLE
        halo.BackgroundTransparency = 0.7
        halo.BorderSizePixel = 0
        halo.AnchorPoint = Vector2.new(0.5, 0.5)
        halo.Position = UDim2.new(0, cx, 0, cy)
        halo.Size = UDim2.new(0, 28, 0, 28)
        halo.ZIndex = 2
        Instance.new("UICorner", halo).CornerRadius = UDim.new(1, 0)
        Instance.new("UIStroke", halo).Color = C_PURPLE
    end

    -- Container das rachaduras (começa invisível)
    local crackLayer = Instance.new("Frame", BG)
    crackLayer.Size = UDim2.fromScale(1,1)
    crackLayer.BackgroundTransparency = 1
    crackLayer.BorderSizePixel = 0
    crackLayer.ZIndex = 2
    crackLayer.Visible = false

    -- Gera 4 grupos de rachaduras em posições estratégicas
    local vw, vh = VP.X, VP.Y
    makeCrackGroup(crackLayer, math.floor(vw*0.18), math.floor(vh*0.22), 7)
    makeCrackGroup(crackLayer, math.floor(vw*0.82), math.floor(vh*0.18), 6)
    makeCrackGroup(crackLayer, math.floor(vw*0.12), math.floor(vh*0.78), 5)
    makeCrackGroup(crackLayer, math.floor(vw*0.88), math.floor(vh*0.82), 6)
    makeCrackGroup(crackLayer, math.floor(vw*0.50), math.floor(vh*0.08), 5)

    -- Overlay de "fragmentos de vidro" — triângulos simulados com frames rotacionados
    for i = 1, 18 do
        local fx = math.random(0, vw)
        local fy = math.random(0, vh)
        local fw = math.random(30, 90)
        local fh = math.random(2, 5)
        local frag = Instance.new("Frame", crackLayer)
        frag.BackgroundColor3 = C_VIOLET
        frag.BackgroundTransparency = math.random(70,92)/100
        frag.BorderSizePixel = 0
        frag.Position = UDim2.new(0, fx, 0, fy)
        frag.Size = UDim2.new(0, fw, 0, fh)
        frag.Rotation = math.random(0, 180)
        frag.ZIndex = 2
    end
    ...
    local panelS = Instance.new("UIStroke", panel)
    panelS.Color = C_PURPLE; panelS.Thickness = 2; panelS.Transparency = 0.15
    panelS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Gradiente do painel
    local panelG = Instance.new("UIGradient", panel)
    panelG.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(30, 14, 62)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(18, 8, 40)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(10, 4, 24)),
    })
    panelG.Rotation = 135

    -- Linha decorativa superior
    local topLine = Instance.new("Frame", panel)
    topLine.BackgroundColor3 = C_PURPLE; topLine.BackgroundTransparency = 0
    topLine.BorderSizePixel = 0
    topLine.Position = UDim2.new(0.1, 0, 0, 0); topLine.Size = UDim2.new(0.8, 0, 0, 2)
    topLine.ZIndex = 11
    Instance.new("UIGradient", topLine).Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(0,0,0)),
        ColorSequenceKeypoint.new(0.5, C_VIOLET),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0,0,0)),
    })

    -- Shine no painel (reflexo)
    local panelShine = Instance.new("Frame", panel)
    panelShine.BackgroundColor3 = C_WHITE
    panelShine.BackgroundTransparency = 0.88
    panelShine.BorderSizePixel = 0
    panelShine.Position = UDim2.new(0.05, 0, 0, 6)
    panelShine.Size = UDim2.new(0.4, 0, 0, 3)
    panelShine.ZIndex = 12
    Instance.new("UICorner", panelShine).CornerRadius = UDim.new(1, 0)

    -- ── ÍCONE / LOGO ──────────────────────────────────────────────────
    local iconBg = Instance.new("Frame", panel)
    iconBg.AnchorPoint = Vector2.new(0.5, 0)
    iconBg.Position = UDim2.new(0.5, 0, 0, 22)
    iconBg.Size = UDim2.new(0, 58, 0, 58)
    iconBg.BackgroundColor3 = C_PURPLE
    iconBg.BackgroundTransparency = 0.6
    iconBg.BorderSizePixel = 0
    iconBg.ZIndex = 11
    Instance.new("UICorner", iconBg).CornerRadius = UDim.new(0, 16)
    local iconS = Instance.new("UIStroke", iconBg)
    iconS.Color = C_VIOLET; iconS.Thickness = 1.5; iconS.Transparency = 0.2
    iconS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    local iconLbl = Instance.new("TextLabel", iconBg)
    iconLbl.BackgroundTransparency = 1; iconLbl.Size = UDim2.fromScale(1,1)
    iconLbl.Text = "🍮"; iconLbl.TextSize = 30; iconLbl.ZIndex = 12
    ...
    titleLbl.Position = UDim2.new(0.5, 0, 0, 88)
    titleLbl.Size = UDim2.new(0.9, 0, 0, 36)
    ...
    -- Gradiente no título
    local titleG = Instance.new("UIGradient", titleLbl)
    titleG.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, C_VIOLET),
    ...
    subLbl.Position = UDim2.new(0.5, 0, 0, 128)
    subLbl.Size = UDim2.new(0.85, 0, 0, 20)
    subLbl.Font = Enum.Font.GothamBold
    subLbl.Text = ""
    subLbl.TextColor3 = C_PURPLE
    subLbl.TextSize = 12
    subLbl.TextXAlignment = Enum.TextXAlignment.Center
    subLbl.ZIndex = 12

    -- Versão / créditos
    local verLbl = Instance.new("TextLabel", panel)
    verLbl.BackgroundTransparency = 1
    verLbl.AnchorPoint = Vector2.new(0.5, 0)
    verLbl.Position = UDim2.new(0.5, 0, 0, 152)
    verLbl.Size = UDim2.new(0.85, 0, 0, 16)
    verLbl.Font = Enum.Font.Gotham
    verLbl.Text = ""
    verLbl.TextColor3 = Color3.fromRGB(120, 100, 160)
    verLbl.TextSize = 10
    ...
    barBg.Position = UDim2.new(0.5, 0, 0, 178)
    barBg.Size = UDim2.new(0.78, 0, 0, 4)
    barBg.BackgroundColor3 = Color3.fromRGB(30, 18, 60)
    barBg.BackgroundTransparency = 0; barBg.BorderSizePixel = 0; barBg.ZIndex = 11
    Instance.new("UICorner", barBg).CornerRadius = UDim.new(1, 0)

    local barFill = Instance.new("Frame", barBg)
    barFill.BackgroundColor3 = C_PURPLE; barFill.BackgroundTransparency = 0
    barFill.BorderSizePixel = 0; barFill.Size = UDim2.new(0, 0, 1, 0); barFill.ZIndex = 12
    Instance.new("UICorner", barFill).CornerRadius = UDim.new(1, 0)
    local barG = Instance.new("UIGradient", barFill)
    barG.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, C_PURPLE),
        ColorSequenceKeypoint.new(0.7, C_VIOLET),
        ColorSequenceKeypoint.new(1, C_GOLD),
    })
    barG.Rotation = 0
    ...
    statusLbl.Position = UDim2.new(0.5, 0, 0, 188)
    statusLbl.Size = UDim2.new(0.85, 0, 0, 14)
    statusLbl.Font = Enum.Font.GothamBold
    statusLbl.Text = ""
    statusLbl.TextColor3 = Color3.fromRGB(140, 110, 190)
    ...
    local function spawnParticle()
        ...
        p.BackgroundColor3 = math.random() > 0.5 and C_PURPLE or C_VIOLET
        ...
        -- Tweens de fade
        TS:Create(p, TweenInfo.new(math.random(8,18)/10, Enum.EasingStyle.Sine), {
    ...
    -- ── CURSOR PISCANTE (typewriter) ──────────────────────────────────
    local cursorVisible = true
    local cursorConn
    local function startCursor(lbl)
        if cursorConn then cursorConn:Disconnect() end
        cursorVisible = true
        local base = lbl.Text:gsub("│$",""):gsub("▌$","")
        cursorConn = RS.Heartbeat:Connect(function()
            local t = tick() % 0.9
            if t < 0.45 then
    ...
    local function stopCursor(lbl)
        if cursorConn then cursorConn:Disconnect(); cursorConn=nil end
        lbl.Text = lbl.Text:gsub("[│▌]$","")
    end

    -- ── FUNÇÃO TYPEWRITER ─────────────────────────────────────────────
    local function typewrite(lbl, text, speed, noSound)
        lbl.Text = ""
        startCursor(lbl)
        for i = 1, #text do
            lbl.Text = text:sub(1,i)
            task.wait(speed or 0.045)
        end
        stopCursor(lbl)
        lbl.Text = text
    end
    ...
    -- Fase 1: fade in do fundo
    panel.BackgroundTransparency = 1
    panelS.Transparency = 1
    iconBg.BackgroundTransparency = 1
    iconS.Transparency = 1
    ...
    -- Fundo aparece
    TS:Create(BG, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {BackgroundTransparency=0}):Play()
    task.wait(0.35)

    -- Rachaduras aparecem com flash
    crackLayer.Visible = true
    local crackFlash = Instance.new("Frame", SG)
    crackFlash.Size = UDim2.fromScale(1,1)
    crackFlash.BackgroundColor3 = C_VIOLET
    crackFlash.BackgroundTransparency = 0.5
    crackFlash.BorderSizePixel = 0; crackFlash.ZIndex = 8
    TS:Create(crackFlash, TweenInfo.new(0.35, Enum.EasingStyle.Quad), {BackgroundTransparency=1}):Play()
    task.delay(0.36, function() pcall(function() crackFlash:Destroy() end) end)

    -- Fade in crack layer
    for _, ch in ipairs(crackLayer:GetDescendants()) do
        if ch:IsA("Frame") then
            local orig = ch.BackgroundTransparency
            ch.BackgroundTransparency = 1
            TS:Create(ch, TweenInfo.new(0.5, Enum.EasingStyle.Quad), {BackgroundTransparency=orig}):Play()
        end
    end
    task.wait(0.5)

    -- Painel aparece com bounce
    TS:Create(panel, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {BackgroundTransparency=0.08}):Play()
    TS:Create(panelS, TweenInfo.new(0.3), {Transparency=0.15}):Play()
    panel.Size = UDim2.new(0, 340, 0, 180)
    TS:Create(panel, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size=UDim2.new(0,380,0,220)}):Play()
    task.wait(0.25)

    -- Ícone aparece girando
    TS:Create(iconBg, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {BackgroundTransparency=0.6}):Play()
    TS:Create(iconS, TweenInfo.new(0.3), {Transparency=0.2}):Play()
    TS:Create(iconLbl, TweenInfo.new(0.3), {TextTransparency=0}):Play()
    TS:Create(topLine, TweenInfo.new(0.4), {BackgroundTransparency=0}):Play()
    task.wait(0.4)

    -- Partículas começam
    local particleConn = task.spawn(function()
        while SG.Parent do
            spawnParticle()
            task.wait(0.18)
        end
    ...
    -- Typewriter: título principal
    barBg.BackgroundTransparency = 0
    typewrite(titleLbl, "PUDIM HUB", 0.07)
    task.wait(0.1)

    -- Barra avança para 30%
    TS:Create(barFill, TweenInfo.new(0.4), {Size=UDim2.new(0.3,0,1,0)}):Play()
    statusLbl.Text = "Inicializando módulos..."
    task.wait(0.5)

    -- Subtítulo
    typewrite(subLbl, "99 Nights in the Forest", 0.055)
    task.wait(0.1)

    -- Barra avança para 65%
    TS:Create(barFill, TweenInfo.new(0.4), {Size=UDim2.new(0.65,0,1,0)}):Play()
    statusLbl.Text = "Carregando interface..."
    task.wait(0.45)

    -- Versão typewriter
    typewrite(verLbl, "v6.0  ·  by FerrazMelote  ·  discord.gg/pudim", 0.03)
    task.wait(0.1)

    -- Barra completa
    TS:Create(barFill, TweenInfo.new(0.55, Enum.EasingStyle.Quad), {Size=UDim2.new(1,0,1,0)}):Play()
    statusLbl.Text = "Pronto!"
    task.wait(0.6)

    -- Pulsar a borda antes de fechar
    for _ = 1, 2 do
        TS:Create(panelS, TweenInfo.new(0.12), {Transparency=0, Thickness=3}):Play()
        task.wait(0.13)
        TS:Create(panelS, TweenInfo.new(0.12), {Transparency=0.15, Thickness=2}):Play()
        task.wait(0.13)
    end
    task.wait(0.15)

    -- Fade out com flash roxo
    local exitFlash = Instance.new("Frame", SG)
    exitFlash.Size = UDim2.fromScale(1,1)
    exitFlash.BackgroundColor3 = C_PURPLE
    exitFlash.BackgroundTransparency = 1
    exitFlash.BorderSizePixel = 0; exitFlash.ZIndex = 20
    TS:Create(exitFlash, TweenInfo.new(0.22, Enum.EasingStyle.Quad), {BackgroundTransparency=0.15}):Play()
    task.wait(0.22)
    TS:Create(exitFlash, TweenInfo.new(0.35, Enum.EasingStyle.Quad), {BackgroundTransparency=1}):Play()

    -- Slide out do painel
    TS:Create(panel, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
        {Position=UDim2.fromScale(0.5,-0.2), BackgroundTransparency=1}):Play()
    TS:Create(panelS, TweenInfo.new(0.25), {Transparency=1}):Play()

    -- Fade out do fundo + rachaduras
    TS:Create(BG, TweenInfo.new(0.45, Enum.EasingStyle.Quad), {BackgroundTransparency=1}):Play()
    for _, ch in ipairs(crackLayer:GetDescendants()) do
        if ch:IsA("Frame") then
            TS:Create(ch, TweenInfo.new(0.4), {BackgroundTransparency=1}):Play()
        end
    end
    task.wait(0.5)

    -- Limpa e lança hub
    pcall(function() task.cancel(particleConn) end)
...
local notifPosition = "TR"   -- TR | BR | TL | BL
...
local NOTIF_CFG = {
    ...
    GAP              = 8,
    SOUND_ENABLED    = true,
    GROUP_WINDOW     = 3,    -- segundos para agrupar notificações iguais
}
...
local function nReflow()
    local totalOff = 0
    local isBottom = (notifPosition == "BR" or notifPosition == "BL")
...
local function nCreateCard(cfg, tipo)
    ...
    iconBg.Position=UDim2.new(0,10,0.5,-14); iconBg.Size=UDim2.new(0,28,0,28); iconBg.ZIndex=521
    ...
    titleLbl.BackgroundTransparency=1
    titleLbl.Position=UDim2.new(0,46,0,titleY); titleLbl.Size=UDim2.new(1,-80,0,18)
    titleLbl.Font=Enum.Font.GothamBlack; titleLbl.Text=title
    titleLbl.TextColor3=Color3.fromRGB(245,245,250); titleLbl.TextSize=12
    ...
    hitbox.MouseEnter:Connect(function()
        ...
        TweenService:Create(cardStroke,TweenInfo.new(0.12),{Transparency=0.15,Thickness=1.8}):Play()
    ...
    hitbox.MouseLeave:Connect(function()
        ...
        TweenService:Create(cardStroke,TweenInfo.new(0.12),{Transparency=0.6,Thickness=1.4}):Play()
...
function Notify.setPosition(pos)
    if pos=="TR" or pos=="BR" or pos=="TL" or pos=="BL" then
        notifPosition=pos
        nReflow()
    end
end
...
-- ══════════════════════════════════════════════════════════════════
-- CARD 4 — MAPA DE REFERÊNCIA
-- ══════════════════════════════════════════════════════════════════
local mapCard, mapStroke = mkCard(212, S_GREEN)
mkHdr(mapCard, "🗺️", "MAPA DE REFERÊNCIA", S_GREEN, 10)

local MAP_POINTS = {
    {icon="🔥", label="Fogueira",         coords="(0, 19, 0)",       cor=S_ORANGE},
    {icon="⚙️", label="Máquina (Grind)",  coords="(21, 16, -5)",     cor=S_CYAN},
    {icon="🧪", label="Biofuel",          coords="(~workspace)",     cor=S_GREEN},
    {icon="💎", label="Diamond Chest",    coords="Stronghold",       cor=Color3.fromRGB(100,200,255)},
    {icon="🏰", label="Stronghold",       coords="(300, 20, 400)",   cor=Color3.fromRGB(220,180,100)},
    {icon="🌀", label="Safe Zone",        coords="(0, 110, 0)",      cor=S_VIOLET},
    {icon="🕳️", label="Cavernas",        coords="(300, -50, 400)",  cor=Color3.fromRGB(180,140,100)},
    {icon="🌲", label="Floresta Inicial", coords="(0, 8, 0)",        cor=S_GREEN},
}

local mapY = 48
for i, pt in ipairs(MAP_POINTS) do
    local row = Instance.new("Frame",mapCard)
    row.BackgroundColor3 = Color3.fromRGB(20,11,40); row.BackgroundTransparency = 0.3
    row.BorderSizePixel = 0
    row.Position = UDim2.new(0,10,0,mapY); row.Size = UDim2.new(1,-20,0,18); row.ZIndex = 6
    Instance.new("UICorner",row).CornerRadius = UDim.new(0,6)
    -- Dot colorido
    local dot = Instance.new("Frame",row); dot.BackgroundColor3 = pt.cor
    dot.BackgroundTransparency = 0.2; dot.BorderSizePixel = 0
    dot.Position = UDim2.new(0,6,0.5,-4); dot.Size = UDim2.new(0,8,0,8); dot.ZIndex = 7
    Instance.new("UICorner",dot).CornerRadius = UDim.new(1,0)
    -- Ícone
    local ico = Instance.new("TextLabel",row); ico.BackgroundTransparency = 1
    ico.Position = UDim2.new(0,18,0,0); ico.Size = UDim2.new(0,18,1,0)
    ico.Text = pt.icon; ico.TextSize = 11; ico.ZIndex = 7
    -- Label
    local lbl = Instance.new("TextLabel",row); lbl.BackgroundTransparency = 1
    lbl.Position = UDim2.new(0,38,0,0); lbl.Size = UDim2.new(0.48,0,1,0)
    lbl.Font = Enum.Font.GothamBold; lbl.Text = pt.label
    lbl.TextColor3 = Color3.fromRGB(215,205,240); lbl.TextSize = 9
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 7
    -- Coords
    local crd = Instance.new("TextLabel",row); crd.BackgroundTransparency = 1
    crd.AnchorPoint = Vector2.new(1,0.5); crd.Position = UDim2.new(1,-8,0.5,0)
    crd.Size = UDim2.new(0.46,0,1,0)
    crd.Font = Enum.Font.Gotham; crd.Text = pt.coords
    crd.TextColor3 = pt.cor; crd.TextSize = 8
    crd.TextXAlignment = Enum.TextXAlignment.Right; crd.ZIndex = 7
    mapY = mapY + 20
end

-- ══════════════════════════════════════════════════════════════════
-- CARD 5 — DICAS ROTATIVAS
-- ══════════════════════════════════════════════════════════════════
local tipCard, tipStroke = mkCard(88, S_ORANGE)
mkHdr(tipCard, "💡", "DICA DA PARTIDA", S_ORANGE, 10)

local TIPS = {
    "🔥 Mantenha sempre combustível na fogueira — ela protege do frio da noite.",
    "💎 O Diamond Chest aparece no Stronghold toda noite. Use o Bring para pegar!",
    "⚙️ Leve sucata para a trituradora para conseguir Scrap Metal valioso.",
    "🌲 Árvores pequenas regeneram mais rápido que as grandes.",
    "👶 As crianças ficam presas no mapa — resgate para ganhar recompensas.",
    "🧪 O Biofuel Processor converte comida orgânica em combustível eficiente.",
    "🏃 Use o Fly + Noclip para explorar rapidamente o mapa à noite.",
    "🎁 Baús abrem mais rápido com o ACS ativado — nunca perca loot!",
    "❄️ A Freeze Aura + Kill Aura é a combinação mais eficiente para farm.",
    "🗺️ O Safe Zone fica acima do mapa — ótimo lugar para ficar offline.",
}
local tipIdx = 1
local tipLbl = Instance.new("TextLabel",tipCard); tipLbl.BackgroundTransparency = 1
tipLbl.Position = UDim2.new(0,10,0,48); tipLbl.Size = UDim2.new(1,-20,0,32)
tipLbl.Font = Enum.Font.GothamBold; tipLbl.Text = TIPS[1]
tipLbl.TextColor3 = Color3.fromRGB(240,220,180); tipLbl.TextSize = 9
tipLbl.TextXAlignment = Enum.TextXAlignment.Left
tipLbl.TextWrapped = true; tipLbl.ZIndex = 6

-- Indicadores de ponto
local tipDots = {}
local dotRow = Instance.new("Frame",tipCard); dotRow.BackgroundTransparency = 1
dotRow.AnchorPoint = Vector2.new(0.5,0); dotRow.Position = UDim2.new(0.5,0,0,72)
dotRow.Size = UDim2.new(0,#TIPS*12,0,8); dotRow.ZIndex = 7
local dotLayout = Instance.new("UIListLayout",dotRow)
dotLayout.FillDirection = Enum.FillDirection.Horizontal; dotLayout.Padding = UDim.new(0,4)
for i = 1, #TIPS do
    local d = Instance.new("Frame",dotRow); d.BackgroundColor3 = i==1 and S_ORANGE or Color3.fromRGB(80,60,40)
    d.BackgroundTransparency = i==1 and 0.1 or 0.6; d.BorderSizePixel = 0
    d.Size = UDim2.new(0,6,0,6); d.ZIndex = 8
    Instance.new("UICorner",d).CornerRadius = UDim.new(1,0)
    tipDots[i] = d
end

local function nextTip()
    TweenService:Create(tipLbl,TweenInfo.new(0.2),{TextTransparency=1}):Play()
    TweenService:Create(tipDots[tipIdx],TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(80,60,40),BackgroundTransparency=0.6}):Play()
    task.wait(0.22)
    tipIdx = tipIdx % #TIPS + 1
    tipLbl.Text = TIPS[tipIdx]
    TweenService:Create(tipLbl,TweenInfo.new(0.3),{TextTransparency=0}):Play()
    TweenService:Create(tipDots[tipIdx],TweenInfo.new(0.2),{BackgroundColor3=S_ORANGE,BackgroundTransparency=0.1}):Play()
end
task.spawn(function() while true do task.wait(6); pcall(nextTip) end end)
...
local function makeMultiDropdown(parent, yPos, cfg)
    ...
    local SEL      = cfg.sel
    local BTN_H    = 30
    local ITEM_H   = 44  -- itens maiores
    local POP_W    = 320  -- popup mais largo
    local MAX_VIS  = cfg.maxVisible or 6
    local POP_H    = math.min(MAX_VIS, #ITEMS) * ITEM_H + 72
    local isOpen   = false
    ...
    local function buildLabel()
        local sel, n = {}, 0
        for _, item in ipairs(ITEMS) do
            if SEL[item.key] then table.insert(sel, item.label); n = n + 1 end
        end
        if n == 0 then return "Selecionar itens  ▾" end
        if n == #ITEMS then return "Todos ("..n..")  ▾" end
        if n <= 2 then return table.concat(sel, ", ").."  ▾" end
        return sel[1]..", "..sel[2].." +".. (n-2).."  ▾"
    end

    -- ── Botão trigger — Glassmorphism ────────────────────────
    -- Fundo ultra-transparente + borda brilhante + blur layer
    local trigBtn = Instance.new("TextButton", parent)
    trigBtn.BackgroundColor3       = Color3.fromRGB(255, 255, 255)
    trigBtn.BackgroundTransparency = 0.88  -- quase transparente (vidro)
    trigBtn.BorderSizePixel        = 0
    trigBtn.Position               = UDim2.new(0, 10, 0, yPos)
    trigBtn.Size                   = UDim2.new(1, -20, 0, BTN_H)
    trigBtn.Font                   = Enum.Font.GothamBold
    trigBtn.Text                   = buildLabel()
    trigBtn.TextColor3             = Color3.fromRGB(235, 228, 255)
    trigBtn.TextSize               = 10
    trigBtn.ZIndex                 = 8
    trigBtn.AutoButtonColor        = false
    trigBtn.TextTruncate           = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", trigBtn).CornerRadius = UDim.new(0, 9)

    -- Borda colorida brilhante (glow effect)
    local trigS = Instance.new("UIStroke", trigBtn)
    trigS.Color = COR; trigS.Thickness = 1.4; trigS.Transparency = 0.2
    trigS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Camada de blur simulado: tint colorido bem suave por baixo
    local trigTint = Instance.new("Frame", trigBtn)
    trigTint.BackgroundColor3       = COR
    trigTint.BackgroundTransparency = 0.82
    trigTint.BorderSizePixel        = 0
    trigTint.Size                   = UDim2.new(1, 0, 1, 0)
    trigTint.ZIndex                 = 7
    Instance.new("UICorner", trigTint).CornerRadius = UDim.new(0, 9)

    -- Linha de brilho no topo (reflexo de vidro)
    local trigGlass = Instance.new("Frame", trigBtn)
    trigGlass.BackgroundColor3       = Color3.fromRGB(255, 255, 255)
    trigGlass.BackgroundTransparency = 0.72
    trigGlass.BorderSizePixel        = 0
    trigGlass.Position               = UDim2.new(0, 6, 0, 2)
    trigGlass.Size                   = UDim2.new(0.55, 0, 0, 1)
    trigGlass.ZIndex                 = 9
    Instance.new("UICorner", trigGlass).CornerRadius = UDim.new(1, 0)

    -- Ponto de brilho canto superior esquerdo
    local trigGlow = Instance.new("Frame", trigBtn)
    trigGlow.BackgroundColor3       = COR
    trigGlow.BackgroundTransparency = 0.75
    trigGlow.BorderSizePixel        = 0
    trigGlow.Position               = UDim2.new(0, 0, 0, 0)
    trigGlow.Size                   = UDim2.new(0, 18, 1, 0)
    trigGlow.ZIndex                 = 7
    Instance.new("UICorner", trigGlow).CornerRadius = UDim.new(0, 9)

    -- Seta ▾
    local trigArrow = Instance.new("TextLabel", trigBtn)
    trigArrow.BackgroundTransparency = 1
    trigArrow.AnchorPoint            = Vector2.new(1, 0.5)
    trigArrow.Position               = UDim2.new(1, -8, 0.5, 0)
    trigArrow.Size                   = UDim2.new(0, 14, 0, 14)
    trigArrow.Font                   = Enum.Font.GothamBlack
    trigArrow.Text                   = "▾"
    trigArrow.TextColor3             = COR
    trigArrow.TextSize               = 10
    trigArrow.ZIndex                 = 9

    -- ── Popup no ScreenGui ────────────────────────────────────
    local pop = Instance.new("Frame", ScreenGui)
    pop.BackgroundColor3       = Color3.fromRGB(18, 10, 32)
    ...
    popS.Color = COR; popS.Thickness = 1.5; popS.Transparency = 0.3
    popS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Brilho topo
    local popShine = Instance.new("Frame", pop)
    popShine.BackgroundColor3       = Color3.fromRGB(255,255,255)
    popShine.BackgroundTransparency = 0.88
    popShine.BorderSizePixel        = 0
    popShine.Size                   = UDim2.new(0.5, 0, 0, 1)
    popShine.Position               = UDim2.new(0.25, 0, 0, 0)
    popShine.ZIndex                 = 431

    -- Header do popup
    local popHdr = Instance.new("Frame", pop)
    popHdr.BackgroundColor3       = Color3.fromRGB(28, 16, 50)
    popHdr.BackgroundTransparency = 0
    popHdr.BorderSizePixel        = 0
    popHdr.Size                   = UDim2.new(1, 0, 0, 46)
    popHdr.ZIndex                 = 431
    Instance.new("UICorner", popHdr).CornerRadius = UDim.new(0, 12)
    local popHdrFix = Instance.new("Frame", popHdr)
    popHdrFix.BackgroundColor3 = Color3.fromRGB(28,16,50)
    popHdrFix.BorderSizePixel  = 0
    popHdrFix.Position         = UDim2.new(0,0,0.5,0)
    popHdrFix.Size             = UDim2.new(1,0,0.5,0)
    popHdrFix.ZIndex           = 431

    local popHdrLbl = Instance.new("TextLabel", popHdr)
    popHdrLbl.BackgroundTransparency = 1
    popHdrLbl.Position  = UDim2.new(0, 14, 0, 0)
    popHdrLbl.Size      = UDim2.new(0.6, 0, 1, 0)
    popHdrLbl.Font      = Enum.Font.GothamBlack
    popHdrLbl.Text      = cfg.title or "Selecionar"
    popHdrLbl.TextColor3 = COR
    popHdrLbl.TextSize  = 14
    popHdrLbl.TextXAlignment = Enum.TextXAlignment.Left
    popHdrLbl.ZIndex    = 432

    -- Botões Todos / Nenhum no header
    local hdrAll = Instance.new("TextButton", popHdr)
    hdrAll.BackgroundColor3       = COR
    hdrAll.BackgroundTransparency = 0.5
    hdrAll.BorderSizePixel        = 0
    hdrAll.AnchorPoint            = Vector2.new(1, 0.5)
    hdrAll.Position               = UDim2.new(1, -10, 0.5, 0)
    hdrAll.Size                   = UDim2.new(0, 72, 0, 26)
    hdrAll.Font                   = Enum.Font.GothamBold
    hdrAll.Text                   = "✅ Todos"
    hdrAll.TextColor3             = Color3.fromRGB(230, 248, 230)
    hdrAll.TextSize               = 11
    hdrAll.ZIndex                 = 432
    Instance.new("UICorner", hdrAll).CornerRadius = UDim.new(0, 8)
    hdrAll.MouseEnter:Connect(function()
        TweenService:Create(hdrAll,TweenInfo.new(0.1),{BackgroundTransparency=0.3}):Play()
    end)
    hdrAll.MouseLeave:Connect(function()
        TweenService:Create(hdrAll,TweenInfo.new(0.1),{BackgroundTransparency=0.6}):Play()
    end)

    local hdrNone = Instance.new("TextButton", popHdr)
    hdrNone.BackgroundColor3       = Color3.fromRGB(160, 40, 40)
    hdrNone.BackgroundTransparency = 0.4
    hdrNone.BorderSizePixel        = 0
    hdrNone.AnchorPoint            = Vector2.new(1, 0.5)
    hdrNone.Position               = UDim2.new(1, -88, 0.5, 0)
    hdrNone.Size                   = UDim2.new(0, 70, 0, 26)
    hdrNone.Font                   = Enum.Font.GothamBold
    hdrNone.Text                   = "✕ Limpar"
    hdrNone.TextColor3             = Color3.fromRGB(255, 210, 210)
    hdrNone.TextSize               = 11
    hdrNone.ZIndex                 = 432
    Instance.new("UICorner", hdrNone).CornerRadius = UDim.new(0, 8)
    hdrNone.MouseEnter:Connect(function()
        TweenService:Create(hdrNone,TweenInfo.new(0.1),{BackgroundTransparency=0.2}):Play()
    end)
    hdrNone.MouseLeave:Connect(function()
        TweenService:Create(hdrNone,TweenInfo.new(0.1),{BackgroundTransparency=0.5}):Play()
    end)
    ...
    -- ── Itens com separadores de categoria ────────────────────
    local itemRefs = {}
    local lo = 0

    for _, catKey in ipairs(catOrder) do
        local catItems = cats[catKey]

        -- Separador de categoria
        local catHdr = Instance.new("Frame", scroll)
        catHdr.BackgroundTransparency = 1
        catHdr.BorderSizePixel        = 0
        catHdr.Size                   = UDim2.new(1, 0, 0, 18)
        lo = lo + 1; catHdr.LayoutOrder   = lo
        catHdr.ZIndex                 = 432
        local catLine = Instance.new("Frame", catHdr)
        catLine.BackgroundColor3       = COR
        catLine.BackgroundTransparency = 0.75
        catLine.BorderSizePixel        = 0
        catLine.Size                   = UDim2.new(1, 0, 0, 1)
        catLine.Position               = UDim2.new(0, 0, 0.5, 0)
        catLine.ZIndex                 = 432
        local catLbl = Instance.new("TextLabel", catHdr)
        catLbl.BackgroundColor3       = Color3.fromRGB(18, 10, 32)
        catLbl.BorderSizePixel        = 0
        catLbl.AnchorPoint            = Vector2.new(0, 0.5)
        catLbl.Position               = UDim2.new(0, 4, 0.5, 0)
        catLbl.Size                   = UDim2.new(0, 0, 1, 0)
        catLbl.AutomaticSize          = Enum.AutomaticSize.X
        catLbl.Font                   = Enum.Font.GothamBold
        catLbl.Text                   = "  "..(CAT_LABELS[catKey] or catKey).."  "
        catLbl.TextColor3             = Color3.fromRGB(160, 148, 200)
        catLbl.TextSize               = 8
        catLbl.ZIndex                 = 433
        ...
        for _, item in ipairs(catItems) do
            ...
            row.BackgroundColor3       = isSel and Color3.fromRGB(40,24,68) or Color3.fromRGB(26,14,44)
            row.BackgroundTransparency = isSel and 0.1 or 0.55
            row.BorderSizePixel        = 0
            ...
            rowS.Color = COR; rowS.Thickness = 1.1
            rowS.Transparency = isSel and 0.25 or 0.82
            rowS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

            -- Checkbox
            local chkBg = Instance.new("Frame", row)
            chkBg.BackgroundColor3       = isSel and COR or Color3.fromRGB(32, 20, 54)
            chkBg.BackgroundTransparency = isSel and 0.15 or 0
            chkBg.BorderSizePixel        = 0
            chkBg.Position               = UDim2.new(0, 8, 0.5, -9)
            chkBg.Size                   = UDim2.new(0, 18, 0, 18)
            chkBg.ZIndex                 = 433
            Instance.new("UICorner", chkBg).CornerRadius = UDim.new(0, 5)
            local chkS = Instance.new("UIStroke", chkBg)
            chkS.Color = COR; chkS.Thickness = 1.3; chkS.Transparency = isSel and 0 or 0.45
            local chkMark = Instance.new("TextLabel", chkBg)
            chkMark.BackgroundTransparency = 1
            chkMark.Size                   = UDim2.new(1, 0, 1, 0)
            chkMark.Font                   = Enum.Font.GothamBlack
            chkMark.Text                   = isSel and "✓" or ""
            ...
            icoLbl.Position               = UDim2.new(0, 30, 0, 0)
            icoLbl.Size                   = UDim2.new(0, 26, 1, 0)
            icoLbl.Text                   = item.icon or "▪"
            icoLbl.TextSize               = 18
            ...
            nameLbl.TextColor3             = isSel and Color3.fromRGB(255, 252, 255) or Color3.fromRGB(185, 175, 215)
            nameLbl.TextSize               = 13
            nameLbl.TextXAlignment         = Enum.TextXAlignment.Left
            nameLbl.TextYAlignment         = Enum.TextYAlignment.Center
            nameLbl.ZIndex                 = 433

            -- Função refresh correta (sem bugs)
            local function refreshRow(hover)
                local on = SEL[item.key]
                local bgColor, bgTrans, sTrans, chkColor, chkAlpha, nameColor
                if on then
                    bgColor=Color3.fromRGB(40,24,68); bgTrans=0.1; sTrans=0.25
                    chkColor=COR; chkAlpha=0.15; nameColor=Color3.fromRGB(255,252,255)
                elseif hover then
                    bgColor=Color3.fromRGB(36,20,60); bgTrans=0.3; sTrans=0.6
                    chkColor=COR; chkAlpha=0.7; nameColor=Color3.fromRGB(215,205,245)
                else
                    bgColor=Color3.fromRGB(26,14,44); bgTrans=0.55; sTrans=0.82
                    chkColor=Color3.fromRGB(32,20,54); chkAlpha=0; nameColor=Color3.fromRGB(185,175,215)
                end
                TweenService:Create(row,TweenInfo.new(0.13),{BackgroundColor3=bgColor,BackgroundTransparency=bgTrans}):Play()
                TweenService:Create(rowS,TweenInfo.new(0.13),{Transparency=sTrans}):Play()
                TweenService:Create(chkBg,TweenInfo.new(0.13),{BackgroundColor3=chkColor,BackgroundTransparency=chkAlpha}):Play()
                TweenService:Create(chkS,TweenInfo.new(0.13),{Transparency=on and 0 or 0.45}):Play()
            ...
            rowBtn.MouseLeave:Connect(function() refreshRow(false) end)
            rowBtn.MouseButton1Click:Connect(function()
                SEL[item.key] = not SEL[item.key]
                -- Animação de check: bounce
                if SEL[item.key] then
                    TweenService:Create(chkBg,TweenInfo.new(0.12,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,20,0,20),Position=UDim2.new(0,7,0.5,-10)}):Play()
                    task.delay(0.15,function()
                        TweenService:Create(chkBg,TweenInfo.new(0.1),{Size=UDim2.new(0,18,0,18),Position=UDim2.new(0,8,0.5,-9)}):Play()
                    end)
                end
                refreshRow(false)
                trigBtn.Text = buildLabel()
    ...
    local function closePop()
        ...
        TweenService:Create(pop,TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.In),
            {Size=UDim2.new(0,POP_W,0,0),BackgroundTransparency=0.4}):Play()
        task.delay(0.2,function() pop.Visible=false; pop.BackgroundTransparency=0 end)
        _vdOpen = nil
        TweenService:Create(trigArrow,TweenInfo.new(0.2),{Rotation=0}):Play()
        TweenService:Create(trigBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.88,BackgroundColor3=Color3.fromRGB(255,255,255)}):Play()
        TweenService:Create(trigS,TweenInfo.new(0.15),{Transparency=0.2,Thickness=1.4}):Play()
    end
    local function openPop()
        ...
        local vp = workspace.CurrentCamera.ViewportSize
        local cx  = math.floor(vp.X/2 - POP_W/2)
        local cy  = math.floor(vp.Y/2 - POP_H/2)
        -- Garante que não sai da tela
        cx = math.max(8, math.min(cx, vp.X - POP_W - 8))
        cy = math.max(8, math.min(cy, vp.Y - POP_H - 8))
        pop.Position = UDim2.new(0,cx,0,cy)
        pop.Size     = UDim2.new(0,POP_W,0,0)
        pop.BackgroundTransparency = 0.4; pop.Visible = true
        TweenService:Create(pop,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
            {Size=UDim2.new(0,POP_W,0,POP_H),BackgroundTransparency=0}):Play()
        isOpen = true; _vdOpen = pop
        TweenService:Create(trigArrow,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Rotation=180}):Play()
        TweenService:Create(trigBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.78,BackgroundColor3=COR}):Play()
        TweenService:Create(trigS,TweenInfo.new(0.15),{Transparency=0,Thickness=1.8}):Play()
    end
...
local function _accActivBtn(parent, y, icon, color)
    ...
    btn.BorderSizePixel=0; btn.Position=UDim2.new(0,10,0,y); btn.Size=UDim2.new(1,-20,0,32)
    btn.Font=Enum.Font.GothamBlack; btn.Text=(icon or "▶").."  ATIVAR"
    btn.TextColor3=Color3.fromRGB(230,225,255); btn.TextSize=11; btn.ZIndex=8; btn.AutoButtonColor=false
...
local function _accToggle(parent, y, label, initOn, COR, onToggle)
    local H=44
    local tl=Instance.new("TextLabel",parent); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,12,0,y); tl.Size=UDim2.new(0.65,0,0,H)
    ...
    pill.AnchorPoint=Vector2.new(1,0); pill.Position=UDim2.new(1,-12,0,y+(H-28)/2)
    pill.Size=UDim2.new(0,52,0,28); pill.ZIndex=8
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    pill.BackgroundColor3=initOn and COR or Color3.fromRGB(65,72,90)
    local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
    knob.Size=UDim2.new(0,22,0,22); knob.ZIndex=9
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255)
    knob.Position=initOn and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
    local ck=Instance.new("TextLabel",knob); ck.BackgroundTransparency=1
    ck.Size=UDim2.new(1,0,1,0); ck.Font=Enum.Font.GothamBlack
    ck.Text=initOn and "✓" or "✕"
    ck.TextColor3=initOn and COR or Color3.fromRGB(65,72,90); ck.TextSize=11; ck.ZIndex=10
    local en=initOn
    ...
    hbtn.MouseButton1Click:Connect(function()
        en=not en
        TweenService:Create(pill,TweenInfo.new(0.18),{BackgroundColor3=en and COR or Color3.fromRGB(65,72,90)}):Play()
        TweenService:Create(knob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=en and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
        }):Play()
        ck.Text=en and "✓" or "✕"; ck.TextColor3=en and COR or Color3.fromRGB(65,72,90)
...
local ESP_CATS = {
    ...
    {key="Animais", trLabel="espAnimaisLabel",trDesc="espAnimaisDesc",label="🐾 Animals", cor=Color3.fromRGB(130,220,100), tipo="entity", alcance=700,
     ...
     nomes={"Bunny","Horse","Kiwi","Turkey","Kiwi Bird","Wolf","Alpha Wolf","AlphaWolf","Bear","Polar Bear","PolarBear","Arctic Fox","ArcticFox","Frog","Blue Frog","Purple Frog","Green Frog","BlueFrog","PurpleFrog","GreenFrog","Scorpion","Mammoth","Hellephant","Meteor Crab","MeteorCrab"}},
    {key="Monstros", trLabel="espMonstrosLabel",trDesc="espMonstrosDesc",label="💀 Monsters", cor=Color3.fromRGB(255,50,50), tipo="entity", alcance=math.huge,
     nomes={"The Deer","TheDeer","The Owl","TheOwl","The Ram","TheRam","Shadow","Nightmare","Phantom"}},
    {key="Cultistas", trLabel="espCultistasLabel",trDesc="espCultistasDesc",label="⚔️ Cultists", cor=Color3.fromRGB(200,80,255), tipo="entity", alcance=600,
     nomes={"Cultist","Cultist Beast","CultistBeast","Cultist Juggernaut","CultistJuggernaut","Cultist King","CultistKing","Mega Cultist","MegaCultist","Cultist Archer","CultistArcher","Cultist Mage","CultistMage"}},
    ...
    {key="EspCarcacas",  trLabel="espCarcacasLabel",  trDesc="espCarcacasDesc",  label="🦴 Corpses",    cor=Color3.fromRGB(180,100,50),  tipo="item", alcance=350, nomes={"Wolf Corpse","WolfCorpse","Bear Corpse","BearCorpse","Polar Bear Corpse","PolarBearCorpse","Mammoth Corpse","MammothCorpse","Hellephant Corpse","HellephantCorpse","Frog Corpse","FrogCorpse","Deer Corpse","DeerCorpse","Bunny Corpse","BunnyCorpse","Cultist Corpse","CultistCorpse"}},
    {key="EspSucata",    trLabel="espSucataLabel",    trDesc="espSucataDesc",    label="🔩 Scrap",      cor=Color3.fromRGB(160,170,180), tipo="item", alcance=300, nomes={"Bolt","Sheet Metal","SheetMetal","Tyre","Broken Fan","BrokenFan","Old Radio","OldRadio","Broken Radio","BrokenRadio","Metal Chair","MetalChair","Broken Microwave","BrokenMicrowave","Old Car Engine","OldCarEngine","Washing Machine","WashingMachine","UFO Junk","UFOJunk","UFO Component","UFOComponent","UFO Scrap","UFOScrap","Alien Junk","AlienJunk","Meteor Shard","MeteorShard","Gold Shard","GoldShard"}},
    {key="EspMateriais", trLabel="espMateriaisLabel", trDesc="espMateriaisDesc", label="💎 Materials",  cor=Color3.fromRGB(180,120,255), tipo="item", alcance=300, nomes={"Cultist Gem","CultistGem","Cultist Experiment","CultistExperiment","Cultist Prototype","CultistPrototype","Raw Obsidiron Ore","RawObsidironOre","Obsidiron Ingot","ObsidironIngot","Scalding Obsidiron Ingot","ScaldingObsidironIngot","Forest Gem","ForestGem","Moss Coin","MossCoin"}},
    {key="EspComidas",   trLabel="espComidasLabel",   trDesc="espComidasDesc",   label="🍖 Food",       cor=Color3.fromRGB(255,180,60),  tipo="item", alcance=250, nomes={"Carrot","Corn","Berry","Apple","Mushroom","Truffle","Steak","Ribs","Turkey Leg","Cooked Steak","Cooked Ribs","Morsel","Cooked Morsel"}},
    {key="EspPeixes",    trLabel="espPeixesLabel",    trDesc="espPeixesDesc",    label="🐟 Fish",        cor=Color3.fromRGB(80,200,220),  tipo="item", alcance=350, nomes={"Mackerel","Salmon","Clownfish","Shark","Eel","Lava Eel","Swordfish","Char","Lionfish"}},
    {key="EspSementes",  trLabel="espSementesLabel",  trDesc="espSementesDesc",  label="🌱 Seeds",      cor=Color3.fromRGB(100,220,100), tipo="item", alcance=200, nomes={"Pepper Seed","Berry Seed","Flower Seed","Firefly Seed","Dripleaf Seed"}},
    {key="EspFerr",      trLabel="espFerrLabel",      trDesc="espFerrDesc",      label="🪓 Tools",      cor=Color3.fromRGB(200,180,100), tipo="item", alcance=400, nomes={"Axe","Bag","Rod","Flute","Armor","Pickaxe","Shovel","Hammer","Fishing Rod"}},
    {key="EspArmas",     trLabel="espArmasLabel",     trDesc="espArmasDesc",     label="⚔️ Weapons",    cor=Color3.fromRGB(255,100,100), tipo="item", alcance=500, nomes={"Spear","Crossbow","Ice Sword","IceSword","Revolver","Rifle","Bow","Arrow","Sword","Knife","Dagger"}},
    {key="EspAmmo",      trLabel="espAmmoLabel",      trDesc="espAmmoDesc",      label="🔫 Ammo",       cor=Color3.fromRGB(255,160,60),  tipo="item", alcance=400, nomes={"Bullet","Arrow","Bolt Ammo","BoltAmmo","Shell","Magazine"}},
    ...
    {key="EspChaves",    trLabel="espChavesLabel",    trDesc="espChavesDesc",    label="🗝️ Keys",        cor=Color3.fromRGB(255,220,80),  tipo="item", alcance=500, nomes={"Key","Master Key","MasterKey","Keycard","Access Key","AccessKey"}},
    {key="EspBigorna",   trLabel="espBigornaLabel",   trDesc="espBigornaDesc",   label="⚒️ Anvil",       cor=Color3.fromRGB(160,160,180), tipo="item", alcance=600, nomes={"Anvil","Forge","Workbench"}},
    {key="EspBlueprint", trLabel="espBlueprintLabel", trDesc="espBlueprintDesc", label="📋 Blueprint",   cor=Color3.fromRGB(100,180,255), tipo="item", alcance=400, nomes={"Blueprint","Schematic","Recipe"}},
    {key="EspPelts",     trLabel="espPeltsLabel",     trDesc="espPeltsDesc",     label="🦺 Pelts",       cor=Color3.fromRGB(200,150,100), tipo="item", alcance=300, nomes={"Wolf Pelt","WolfPelt","Bear Pelt","BearPelt","Fur","Hide","Leather"}},
    {key="EspPocoes",    trLabel="espPocoesLabel",    trDesc="espPocoesDesc",    label="🧪 Potions",     cor=Color3.fromRGB(200,100,255), tipo="item", alcance=350, nomes={"Potion","Energy Drink","EnergyDrink","Speed Potion","SpeedPotion","Strength Potion","StrengthPotion"}},
}
...
local function findCat(nameL, tipo)
    for _,c in ipairs(ESP_CATS) do
        if espAtivo[c.key] and c.tipo==tipo and matchEspCat(nameL,c.key) then
            return c
        end
    end
    return nil
...
local function buildCache()
    ...
    task.spawn(function()
        ...
        local doEnt  = anyEspActive("entity") or anyEspActive("player")
            ...
            if obj and obj.Parent then
                if doEnt and obj:IsA("Model") and not pchars[obj] then
                    ...
                    if not seenEnt[id] then
                        ...
                        if hum and hum.Health>0 then
                            ...
                            if hrp then
                                local cat=findCat(obj.Name:lower(),"entity")
                                if cat then
                                    seenEnt[id]=true
                ...
                elseif doItem and obj:IsA("Model") and not pchars[obj] then
                    if not obj:FindFirstChildWhichIsA("Humanoid") then
                        local cat=findCat(obj.Name:lower(),"item")
                        if cat then
                ...
                elseif doItem and obj:IsA("BasePart") then
                    ...
                    if not isNPC and not pchars[par] then
                        ...
                        local cat=findCat(nmCheck,"item")
                        if cat then
...
local function newProLabel()
    ...
    dp.Size=UDim2.new(0,0,0,14); dp.AutomaticSize=Enum.AutomaticSize.X; dp.ZIndex=13
    ...
    dl.BackgroundTransparency=1; dl.Size=UDim2.new(0,0,1,0)
    dl.AutomaticSize=Enum.AutomaticSize.X; dl.Font=Enum.Font.GothamBold
...
local function newNotifLabel()
    ...
    badge.Position=UDim2.new(1,-3,0.5,0); badge.Size=UDim2.new(0,0,0,16)
    badge.AutomaticSize=Enum.AutomaticSize.X; badge.ZIndex=13
    ...
    bl.AutomaticSize=Enum.AutomaticSize.X; bl.Font=Enum.Font.GothamBold
...
local espTopPanel = Instance.new("Frame",Pages["Esp"])
espTopPanel.BackgroundColor3=Color3.fromRGB(26,16,48); espTopPanel.BackgroundTransparency=0
espTopPanel.BorderSizePixel=0; espTopPanel.Size=UDim2.new(1,0,0,186)
espTopPanel.LayoutOrder=espLO(); espTopPanel.ZIndex=5
Instance.new("UICorner",espTopPanel).CornerRadius=UDim.new(0,14)
local espTopS=Instance.new("UIStroke",espTopPanel)
espTopS.Color=ESP_ACCENT; espTopS.Thickness=1.5; espTopS.Transparency=0.5
espTopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
-- Gradiente
local espTopG=Instance.new("UIGradient",espTopPanel)
espTopG.Color=ColorSequence.new({
    ColorSequenceKeypoint.new(0,Color3.fromRGB(36,22,68)),
    ColorSequenceKeypoint.new(1,Color3.fromRGB(18,10,36)),
}); espTopG.Rotation=135
-- Shine
local espTopShine=Instance.new("Frame",espTopPanel)
espTopShine.BackgroundColor3=Color3.fromRGB(255,255,255); espTopShine.BackgroundTransparency=0.88
espTopShine.BorderSizePixel=0; espTopShine.Size=UDim2.new(0.5,0,0,1)
espTopShine.Position=UDim2.new(0.25,0,0,0); espTopShine.ZIndex=6

-- Header row
local espHeaderRow=Instance.new("Frame",espTopPanel)
espHeaderRow.BackgroundTransparency=1; espHeaderRow.BorderSizePixel=0
espHeaderRow.Position=UDim2.new(0,12,0,10); espHeaderRow.Size=UDim2.new(1,-24,0,22); espHeaderRow.ZIndex=6

local espHeaderIcon=Instance.new("TextLabel",espHeaderRow); espHeaderIcon.BackgroundTransparency=1
espHeaderIcon.Size=UDim2.new(0,22,1,0); espHeaderIcon.Font=Enum.Font.GothamBold
espHeaderIcon.Text="🔍"; espHeaderIcon.TextSize=16; espHeaderIcon.ZIndex=7

local espHeaderTitle=Instance.new("TextLabel",espHeaderRow); espHeaderTitle.BackgroundTransparency=1
espHeaderTitle.Position=UDim2.new(0,26,0,0); espHeaderTitle.Size=UDim2.new(0.5,0,1,0)
espHeaderTitle.Font=Enum.Font.GothamBlack; espHeaderTitle.Text="ESP Control"
espHeaderTitle.TextColor3=Color3.fromRGB(220,205,255); espHeaderTitle.TextSize=13
espHeaderTitle.TextXAlignment=Enum.TextXAlignment.Left; espHeaderTitle.ZIndex=7

-- Contador de ESPs
local espCountBadge=Instance.new("Frame",espHeaderRow); espCountBadge.BackgroundColor3=ESP_ACCENT
espCountBadge.BackgroundTransparency=0.45; espCountBadge.BorderSizePixel=0
espCountBadge.AnchorPoint=Vector2.new(1,0.5); espCountBadge.Position=UDim2.new(1,0,0.5,0)
espCountBadge.Size=UDim2.new(0,0,0,18); espCountBadge.AutomaticSize=Enum.AutomaticSize.X; espCountBadge.ZIndex=7
Instance.new("UICorner",espCountBadge).CornerRadius=UDim.new(0,6)
local ecbPad=Instance.new("UIPadding",espCountBadge)
ecbPad.PaddingLeft=UDim.new(0,8); ecbPad.PaddingRight=UDim.new(0,8)
local espCountLbl=Instance.new("TextLabel",espCountBadge); espCountLbl.BackgroundTransparency=1
espCountLbl.Size=UDim2.new(0,0,1,0); espCountLbl.AutomaticSize=Enum.AutomaticSize.X
espCountLbl.Font=Enum.Font.GothamBold; espCountLbl.Text=#ESP_CATS.." ESPs"
espCountLbl.TextColor3=Color3.fromRGB(230,220,255); espCountLbl.TextSize=9; espCountLbl.ZIndex=8

-- Atualiza contador de ativos dinamicamente
local function updateEspCount()
    local n=0; for _,c in ipairs(ESP_CATS) do if espAtivo[c.key] then n=n+1 end end
    espCountLbl.Text = n.."/"..#ESP_CATS.." ativos"
end

-- ── HELPER: cria dropdown trigger button ──────────────────────────
local function makeEspDropBtn(parent,y,labelTxt,accentCor)
    local btn=Instance.new("TextButton",parent)
    btn.BackgroundColor3=Color3.fromRGB(20,12,40); btn.BackgroundTransparency=0.2
    btn.BorderSizePixel=0; btn.Position=UDim2.new(0,12,0,y); btn.Size=UDim2.new(1,-24,0,28)
    btn.Font=Enum.Font.GothamBold; btn.ZIndex=7; btn.AutoButtonColor=false
    btn.TextTruncate=Enum.TextTruncate.AtEnd
    Instance.new("UICorner",btn).CornerRadius=UDim.new(0,9)
    local s=Instance.new("UIStroke",btn); s.Thickness=1.2; s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    s.Color=accentCor; s.Transparency=0.5

    -- Label left
    local lbl=Instance.new("TextLabel",btn); lbl.BackgroundTransparency=1
    lbl.Position=UDim2.new(0,10,0,0); lbl.Size=UDim2.new(0.55,0,1,0)
    lbl.Font=Enum.Font.GothamBold; lbl.Text=labelTxt
    lbl.TextColor3=Color3.fromRGB(180,165,215); lbl.TextSize=9
    lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=8

    -- Value right
    local val=Instance.new("TextLabel",btn); val.Name="VAL"; val.BackgroundTransparency=1
    val.AnchorPoint=Vector2.new(1,0.5); val.Position=UDim2.new(1,-22,0.5,0)
    val.Size=UDim2.new(0.45,0,1,0); val.Font=Enum.Font.GothamBold
    val.TextColor3=accentCor; val.TextSize=10
    val.TextXAlignment=Enum.TextXAlignment.Right; val.ZIndex=8

    -- Arrow
    local arr=Instance.new("TextLabel",btn); arr.Name="ARR"; arr.BackgroundTransparency=1
    arr.AnchorPoint=Vector2.new(1,0.5); arr.Position=UDim2.new(1,-5,0.5,0)
    arr.Size=UDim2.new(0,14,0,14); arr.Font=Enum.Font.GothamBlack
    arr.Text="▾"; arr.TextColor3=accentCor; arr.TextSize=10; arr.ZIndex=8

    -- Shine
    local shine=Instance.new("Frame",btn)
    shine.BackgroundColor3=Color3.fromRGB(255,255,255); shine.BackgroundTransparency=0.88
    shine.BorderSizePixel=0; shine.Position=UDim2.new(0,6,0,3)
    shine.Size=UDim2.new(0,40,0,4); shine.ZIndex=8
    Instance.new("UICorner",shine).CornerRadius=UDim.new(1,0)

    return btn, val, arr, s
end

-- ── DROPDOWN 1: ESTILO DE ESP ─────────────────────────────────────
local STYLE_OPTS = {
    {key="profissional", label="Profissional", icon="✨", desc="Elegante com seta direcional e pill de dist.", cor=Color3.fromRGB(180,120,255)},
    {key="simples",      label="Simples",      icon="📦", desc="Hitbox + nome + distância, limpo.",           cor=Color3.fromRGB(100,200,255)},
    {key="notificacao",  label="Notificação",  icon="🔔", desc="Mini card compacto com badge de distância.", cor=Color3.fromRGB(255,180,60)},
}
local styleBtn,styleVal,styleArr,styleBtnS = makeEspDropBtn(espTopPanel,40,"⚡ Estilo",Color3.fromRGB(180,120,255))
styleVal.Text="✨ Profissional"

-- Popup Estilo
local stylePop=Instance.new("Frame",ScreenGui); stylePop.BackgroundColor3=Color3.fromRGB(18,10,34)
stylePop.BackgroundTransparency=0; stylePop.BorderSizePixel=0
stylePop.ZIndex=420; stylePop.Visible=false; stylePop.Size=UDim2.new(0,250,0,0)
stylePop.ClipsDescendants=true
Instance.new("UICorner",stylePop).CornerRadius=UDim.new(0,12)
local stylePopS=Instance.new("UIStroke",stylePop); stylePopS.Color=Color3.fromRGB(180,120,255)
stylePopS.Thickness=1.5; stylePopS.Transparency=0.3; stylePopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
local stylePopLayout=Instance.new("UIListLayout",stylePop); stylePopLayout.SortOrder=Enum.SortOrder.LayoutOrder
local stylePopPad=Instance.new("UIPadding",stylePop)
stylePopPad.PaddingTop=UDim.new(0,6); stylePopPad.PaddingBottom=UDim.new(0,6)
stylePopPad.PaddingLeft=UDim.new(0,6); stylePopPad.PaddingRight=UDim.new(0,6)
local styleOpen=false
local STYLE_H=#STYLE_OPTS*56+18

local styleItemRefs={}
for i,opt in ipairs(STYLE_OPTS) do
    local isSel=(espStyle==opt.key)
    local item=Instance.new("Frame",stylePop)
    item.BackgroundColor3=isSel and Color3.fromRGB(40,22,68) or Color3.fromRGB(22,12,40)
    item.BackgroundTransparency=isSel and 0.1 or 0.5; item.BorderSizePixel=0
    item.Size=UDim2.new(1,0,0,50); item.LayoutOrder=i; item.ZIndex=422
    Instance.new("UICorner",item).CornerRadius=UDim.new(0,9)
    local itemS=Instance.new("UIStroke",item); itemS.Color=opt.cor
    itemS.Thickness=1.2; itemS.Transparency=isSel and 0.2 or 0.8; itemS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    -- Ícone
    local icoF=Instance.new("Frame",item); icoF.BackgroundColor3=opt.cor
    icoF.BackgroundTransparency=isSel and 0.45 or 0.75; icoF.BorderSizePixel=0
    icoF.Position=UDim2.new(0,8,0.5,-16); icoF.Size=UDim2.new(0,32,0,32); icoF.ZIndex=423
    Instance.new("UICorner",icoF).CornerRadius=UDim.new(0,8)
    local icoL=Instance.new("TextLabel",icoF); icoL.BackgroundTransparency=1
    icoL.Size=UDim2.new(1,0,1,0); icoL.Font=Enum.Font.GothamBold
    icoL.Text=opt.icon; icoL.TextSize=17; icoL.ZIndex=424
    -- Nome
    local nl=Instance.new("TextLabel",item); nl.BackgroundTransparency=1
    nl.Position=UDim2.new(0,48,0,7); nl.Size=UDim2.new(1,-68,0,16)
    nl.Font=Enum.Font.GothamBlack; nl.Text=opt.label
    nl.TextColor3=isSel and Color3.new(1,1,1) or Color3.fromRGB(195,180,230)
    nl.TextSize=12; nl.TextXAlignment=Enum.TextXAlignment.Left; nl.ZIndex=423
    -- Desc
    local dl=Instance.new("TextLabel",item); dl.BackgroundTransparency=1
    dl.Position=UDim2.new(0,48,0,25); dl.Size=UDim2.new(1,-68,0,18)
    dl.Font=Enum.Font.Gotham; dl.Text=opt.desc
    dl.TextColor3=isSel and Color3.fromRGB(180,160,215) or Color3.fromRGB(130,115,165)
    dl.TextSize=8; dl.TextWrapped=true; dl.TextXAlignment=Enum.TextXAlignment.Left; dl.ZIndex=423
    -- Check
    local chk=Instance.new("TextLabel",item); chk.BackgroundTransparency=1
    chk.AnchorPoint=Vector2.new(1,0.5); chk.Position=UDim2.new(1,-8,0.5,0)
    chk.Size=UDim2.new(0,18,0,18); chk.Font=Enum.Font.GothamBlack
    chk.Text=isSel and "✓" or ""; chk.TextColor3=opt.cor; chk.TextSize=12; chk.ZIndex=424

    local function refreshStyleItem(hover)
        local sel=(espStyle==opt.key)
        local bgC = sel and Color3.fromRGB(40,22,68) or (hover and Color3.fromRGB(32,18,56) or Color3.fromRGB(22,12,40))
        local bgT = sel and 0.1 or (hover and 0.3 or 0.5)
        TweenService:Create(item,TweenInfo.new(0.13),{BackgroundColor3=bgC,BackgroundTransparency=bgT}):Play()
        TweenService:Create(itemS,TweenInfo.new(0.13),{Transparency=sel and 0.2 or 0.75}):Play()
        TweenService:Create(icoF,TweenInfo.new(0.13),{BackgroundTransparency=sel and 0.45 or 0.75}):Play()
        nl.TextColor3=sel and Color3.new(1,1,1) or Color3.fromRGB(195,180,230)
        chk.Text=sel and "✓" or ""
    end
    styleItemRefs[opt.key]=refreshStyleItem

    if i<#STYLE_OPTS then
        local sp=Instance.new("Frame",stylePop); sp.BackgroundTransparency=1
        sp.BorderSizePixel=0; sp.Size=UDim2.new(1,0,0,3); sp.LayoutOrder=i*10+5; sp.ZIndex=421
    end

    local ibtn=Instance.new("TextButton",item); ibtn.BackgroundTransparency=1
    ibtn.BorderSizePixel=0; ibtn.Size=UDim2.new(1,0,1,0); ibtn.Text=""; ibtn.ZIndex=425; ibtn.AutoButtonColor=false
    ibtn.MouseEnter:Connect(function() if espStyle~=opt.key then refreshStyleItem(true) end end)
    ibtn.MouseLeave:Connect(function() refreshStyleItem(false) end)
    ibtn.MouseButton1Click:Connect(function()
        espStyle=opt.key
        for _,fn in pairs(styleItemRefs) do pcall(fn,false) end
        styleVal.Text=opt.icon.." "..opt.label; styleVal.TextColor3=opt.cor
        styleBtnS.Color=opt.cor; styleArr.TextColor3=opt.cor
        -- close
        styleOpen=false
        TweenService:Create(stylePop,TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,250,0,0),BackgroundTransparency=0.4}):Play()
        task.delay(0.2,function() stylePop.Visible=false; stylePop.BackgroundTransparency=0 end)
        _vdOpen=nil
        TweenService:Create(styleArr,TweenInfo.new(0.2),{Rotation=0}):Play()
        TweenService:Create(styleBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.2}):Play()
        Notify.send({type="info",icon=opt.icon,accent=opt.cor,title="Estilo ESP",msg=opt.label,duration=2})
    end)
end

local function openStylePop()
    if _vdOpen and _vdOpen~=stylePop then
        TweenService:Create(_vdOpen,TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,_vdOpen.AbsoluteSize.X,0,0)}):Play()
        task.delay(0.13,function() _vdOpen.Visible=false end)
    end
    for _,fn in pairs(styleItemRefs) do pcall(fn,false) end
    local ap=styleBtn.AbsolutePosition; local as=styleBtn.AbsoluteSize
    stylePop.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+6)
    stylePop.Size=UDim2.new(0,250,0,0); stylePop.BackgroundTransparency=0.4; stylePop.Visible=true
    TweenService:Create(stylePop,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,250,0,STYLE_H),BackgroundTransparency=0}):Play()
    styleOpen=true; _vdOpen=stylePop
    TweenService:Create(styleArr,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Rotation=180}):Play()
    TweenService:Create(styleBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.05}):Play()
end
local function closeStylePop()
    styleOpen=false
    TweenService:Create(stylePop,TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,250,0,0),BackgroundTransparency=0.4}):Play()
    task.delay(0.2,function() stylePop.Visible=false; stylePop.BackgroundTransparency=0 end)
    _vdOpen=nil
...
styleBtn.MouseButton1Click:Connect(function() if styleOpen then closeStylePop() else openStylePop() end end)
UserInputService.InputBegan:Connect(function(inp)
    if not styleOpen then return end
    if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
    local mp=UserInputService:GetMouseLocation()
    local function inside(f) local a,s=f.AbsolutePosition,f.AbsoluteSize; return mp.X>=a.X and mp.X<=a.X+s.X and mp.Y>=a.Y and mp.Y<=a.Y+s.Y end
    if not inside(stylePop) and not inside(styleBtn) then closeStylePop() end
end)

-- ── DROPDOWN 2: COR DO ESP ────────────────────────────────────────
local COR_ACCENT = Color3.fromRGB(100,200,255)
local corBtn,corVal,corArr,corBtnS = makeEspDropBtn(espTopPanel,76,"🎨 Cor do ESP",COR_ACCENT)
corVal.Text="🟣 Roxo"

-- Popup Cor (color wheel + intensity + presets)
local corPop=Instance.new("Frame",ScreenGui); corPop.BackgroundColor3=Color3.fromRGB(14,8,28)
corPop.BackgroundTransparency=0; corPop.BorderSizePixel=0
corPop.ZIndex=420; corPop.Visible=false; corPop.Size=UDim2.new(0,280,0,0)
corPop.ClipsDescendants=true
Instance.new("UICorner",corPop).CornerRadius=UDim.new(0,14)
local corPopS=Instance.new("UIStroke",corPop); corPopS.Color=COR_ACCENT
corPopS.Thickness=1.5; corPopS.Transparency=0.3; corPopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
local corOpen=false

-- Header
local corPopHdr=Instance.new("Frame",corPop); corPopHdr.BackgroundColor3=Color3.fromRGB(22,12,44)
corPopHdr.BackgroundTransparency=0; corPopHdr.BorderSizePixel=0
corPopHdr.Size=UDim2.new(1,0,0,36); corPopHdr.ZIndex=421
Instance.new("UICorner",corPopHdr).CornerRadius=UDim.new(0,14)
local corPopHdrFix=Instance.new("Frame",corPopHdr); corPopHdrFix.BackgroundColor3=Color3.fromRGB(22,12,44)
corPopHdrFix.BorderSizePixel=0; corPopHdrFix.Position=UDim2.new(0,0,0.5,0); corPopHdrFix.Size=UDim2.new(1,0,0.5,0); corPopHdrFix.ZIndex=421
local corPopTitle=Instance.new("TextLabel",corPopHdr); corPopTitle.BackgroundTransparency=1
corPopTitle.Position=UDim2.new(0,14,0,0); corPopTitle.Size=UDim2.new(1,-28,1,0)
corPopTitle.Font=Enum.Font.GothamBlack; corPopTitle.Text="🎨  Cor do ESP"
corPopTitle.TextColor3=COR_ACCENT; corPopTitle.TextSize=12
corPopTitle.TextXAlignment=Enum.TextXAlignment.Left; corPopTitle.ZIndex=422

-- COLOR WHEEL (simplificado com gradiente radial HSV)
local cwContainer=Instance.new("Frame",corPop); cwContainer.BackgroundTransparency=1
cwContainer.BorderSizePixel=0; cwContainer.Position=UDim2.new(0.5,-70,0,44)
cwContainer.Size=UDim2.new(0,140,0,140); cwContainer.ZIndex=421

-- Fundo preto circular
local cwBg=Instance.new("Frame",cwContainer); cwBg.BackgroundColor3=Color3.fromRGB(0,0,0)
cwBg.BorderSizePixel=0; cwBg.Size=UDim2.new(1,0,1,0); cwBg.ZIndex=422
Instance.new("UICorner",cwBg).CornerRadius=UDim.new(1,0)

-- Arco de cor com segmentos (12 fatias coloridas)
local HUE_COLORS={
    Color3.fromHSV(0,1,1),Color3.fromHSV(0.083,1,1),Color3.fromHSV(0.167,1,1),
    Color3.fromHSV(0.25,1,1),Color3.fromHSV(0.333,1,1),Color3.fromHSV(0.417,1,1),
    Color3.fromHSV(0.5,1,1),Color3.fromHSV(0.583,1,1),Color3.fromHSV(0.667,1,1),
    Color3.fromHSV(0.75,1,1),Color3.fromHSV(0.833,1,1),Color3.fromHSV(0.917,1,1),
}
for j,hc in ipairs(HUE_COLORS) do
    local seg=Instance.new("Frame",cwContainer)
    seg.BackgroundColor3=hc; seg.BackgroundTransparency=0; seg.BorderSizePixel=0
    local angle=(j-1)/12*math.pi*2
    local cx=70+math.cos(angle)*52-10; local cy=70+math.sin(angle)*52-10
    seg.Position=UDim2.new(0,cx,0,cy); seg.Size=UDim2.new(0,20,0,20); seg.ZIndex=423
    Instance.new("UICorner",seg).CornerRadius=UDim.new(1,0)
    -- Gradiente branco->cor
    local segBtn=Instance.new("TextButton",seg); segBtn.BackgroundTransparency=1
    segBtn.Size=UDim2.new(1,0,1,0); segBtn.Text=""; segBtn.ZIndex=424; segBtn.AutoButtonColor=false
    segBtn.MouseButton1Click:Connect(function()
        espGlobalCor=hc
        -- Update preview and button
        local r,g,b=hc.R*255,hc.G*255,hc.B*255
        local cnames={
            [0]="Vermelho",[30]="Laranja",[60]="Amarelo",
            [90]="Verde-Amarelo",[120]="Verde",[150]="Verde-Ciano",
            [180]="Ciano",[210]="Azul-Ciano",[240]="Azul",
            [270]="Roxo",[300]="Magenta",[330]="Rosa",
        }
        local hue=math.floor((j-1)/12*360/30)*30
        local name=cnames[hue] or "Personalizada"
        corVal.Text="⬤ "..name; corVal.TextColor3=hc; corBtnS.Color=hc; corArr.TextColor3=hc
        TweenService:Create(corPopS,TweenInfo.new(0.2),{Color=hc}):Play()
        lastCache=0
    end)
end

-- Centro branco (para cores claras/brancas)
local cwCenter=Instance.new("TextButton",cwContainer); cwCenter.BackgroundColor3=Color3.new(1,1,1)
cwCenter.BackgroundTransparency=0.1; cwCenter.BorderSizePixel=0
cwCenter.AnchorPoint=Vector2.new(0.5,0.5); cwCenter.Position=UDim2.new(0.5,0,0.5,0)
cwCenter.Size=UDim2.new(0,32,0,32); cwCenter.Text="⬜"; cwCenter.Font=Enum.Font.GothamBold
cwCenter.TextSize=12; cwCenter.ZIndex=424; cwCenter.AutoButtonColor=false
Instance.new("UICorner",cwCenter).CornerRadius=UDim.new(1,0)
cwCenter.MouseButton1Click:Connect(function()
    espGlobalCor=Color3.new(1,1,1)
    corVal.Text="⬜ Branco"; corVal.TextColor3=Color3.fromRGB(220,220,220)
    corBtnS.Color=Color3.fromRGB(220,220,220); lastCache=0
end)

-- Intensidade
local intensRow=Instance.new("Frame",corPop); intensRow.BackgroundTransparency=1
intensRow.BorderSizePixel=0; intensRow.Position=UDim2.new(0,10,0,192)
intensRow.Size=UDim2.new(1,-20,0,18); intensRow.ZIndex=421
local intensLbl=Instance.new("TextLabel",intensRow); intensLbl.BackgroundTransparency=1
intensLbl.Size=UDim2.new(0.4,0,1,0); intensLbl.Font=Enum.Font.GothamBold
intensLbl.Text="💡 Intensidade:"; intensLbl.TextColor3=Color3.fromRGB(170,155,205)
intensLbl.TextSize=9; intensLbl.TextXAlignment=Enum.TextXAlignment.Left; intensLbl.ZIndex=422

local INTENSITY_OPTS={"mega_fraca","fraca","normal","forte","mega_forte","neon","transparente"}
local INTENSITY_LABELS={"Mega Fraca","Fraca","Normal","Forte","Mega Forte","Neon","Trans."}
local intensScroll=Instance.new("ScrollingFrame",corPop); intensScroll.BackgroundTransparency=1
intensScroll.BorderSizePixel=0; intensScroll.Position=UDim2.new(0,8,0,214)
intensScroll.Size=UDim2.new(1,-16,0,30); intensScroll.ScrollBarThickness=0
intensScroll.CanvasSize=UDim2.new(0,0,0,0); intensScroll.AutomaticCanvasSize=Enum.AutomaticSize.X
intensScroll.ZIndex=422; intensScroll.ScrollingDirection=Enum.ScrollingDirection.X
local intensLayout=Instance.new("UIListLayout",intensScroll)
intensLayout.FillDirection=Enum.FillDirection.Horizontal; intensLayout.Padding=UDim.new(0,4)
intensLayout.VerticalAlignment=Enum.VerticalAlignment.Center

local intensBtns={}
for j,ikey in ipairs(INTENSITY_OPTS) do
    local ib=Instance.new("TextButton",intensScroll)
    ib.BackgroundColor3=ikey==espIntensity and COR_ACCENT or Color3.fromRGB(30,18,52)
    ib.BackgroundTransparency=ikey==espIntensity and 0.3 or 0.5; ib.BorderSizePixel=0
    ib.Size=UDim2.new(0,0,0,24); ib.AutomaticSize=Enum.AutomaticSize.X; ib.ZIndex=423
    ib.Font=Enum.Font.GothamBold; ib.Text=INTENSITY_LABELS[j]; ib.TextSize=8
    ib.TextColor3=ikey==espIntensity and Color3.new(1,1,1) or Color3.fromRGB(170,155,205)
    ib.AutoButtonColor=false
    Instance.new("UICorner",ib).CornerRadius=UDim.new(0,6)
    local ibPad=Instance.new("UIPadding",ib)
    ibPad.PaddingLeft=UDim.new(0,8); ibPad.PaddingRight=UDim.new(0,8)
    intensBtns[ikey]=ib
    ib.MouseButton1Click:Connect(function()
        espIntensity=ikey
        for k,b in pairs(intensBtns) do
            local sel=(k==ikey)
            TweenService:Create(b,TweenInfo.new(0.1),{BackgroundColor3=sel and COR_ACCENT or Color3.fromRGB(30,18,52),BackgroundTransparency=sel and 0.3 or 0.5}):Play()
            b.TextColor3=sel and Color3.new(1,1,1) or Color3.fromRGB(170,155,205)
        end
        lastCache=0
    end)
end

-- Altura total do popup cor
local COR_POP_H=254
corPop.ClipsDescendants=true

local function openCorPop()
    if _vdOpen and _vdOpen~=corPop then
        TweenService:Create(_vdOpen,TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,_vdOpen.AbsoluteSize.X,0,0)}):Play()
        task.delay(0.13,function() _vdOpen.Visible=false end)
    end
    local ap=corBtn.AbsolutePosition; local as=corBtn.AbsoluteSize
    corPop.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+6)
    corPop.Size=UDim2.new(0,280,0,0); corPop.BackgroundTransparency=0.4; corPop.Visible=true
    TweenService:Create(corPop,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,280,0,COR_POP_H),BackgroundTransparency=0}):Play()
    corOpen=true; _vdOpen=corPop
    TweenService:Create(corArr,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Rotation=180}):Play()
    TweenService:Create(corBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.05}):Play()
end
local function closeCorPop()
    corOpen=false
    TweenService:Create(corPop,TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,280,0,0),BackgroundTransparency=0.4}):Play()
    task.delay(0.2,function() corPop.Visible=false; corPop.BackgroundTransparency=0 end)
    _vdOpen=nil
    TweenService:Create(corArr,TweenInfo.new(0.2),{Rotation=0}):Play()
    TweenService:Create(corBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.2}):Play()
end
corBtn.MouseButton1Click:Connect(function() if corOpen then closeCorPop() else openCorPop() end end)
UserInputService.InputBegan:Connect(function(inp)
    if not corOpen then return end
    if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
    local mp=UserInputService:GetMouseLocation()
    local function inside(f) local a,s=f.AbsolutePosition,f.AbsoluteSize; return mp.X>=a.X and mp.X<=a.X+s.X and mp.Y>=a.Y and mp.Y<=a.Y+s.Y end
    if not inside(corPop) and not inside(corBtn) then closeCorPop() end
end)

-- ── DROPDOWN 3: ESP CORES QUAL? ───────────────────────────────────
local CAT_ACCENT = Color3.fromRGB(255,185,60)
local catBtn,catVal,catArr,catBtnS = makeEspDropBtn(espTopPanel,112,"🗂️ Cores por ESP",CAT_ACCENT)
catVal.Text="Selecionar ESP"

local catPop=Instance.new("Frame",ScreenGui); catPop.BackgroundColor3=Color3.fromRGB(14,8,28)
catPop.BackgroundTransparency=0; catPop.BorderSizePixel=0
catPop.ZIndex=420; catPop.Visible=false; catPop.Size=UDim2.new(0,320,0,0)
catPop.ClipsDescendants=true
Instance.new("UICorner",catPop).CornerRadius=UDim.new(0,14)
local catPopS=Instance.new("UIStroke",catPop); catPopS.Color=CAT_ACCENT
catPopS.Thickness=1.5; catPopS.Transparency=0.3; catPopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
local catOpen=false

-- Header
local catPopHdr=Instance.new("Frame",catPop); catPopHdr.BackgroundColor3=Color3.fromRGB(22,14,40)
catPopHdr.BackgroundTransparency=0; catPopHdr.BorderSizePixel=0
catPopHdr.Size=UDim2.new(1,0,0,36); catPopHdr.ZIndex=421
Instance.new("UICorner",catPopHdr).CornerRadius=UDim.new(0,14)
local catPopHdrFix=Instance.new("Frame",catPopHdr); catPopHdrFix.BackgroundColor3=Color3.fromRGB(22,14,40)
catPopHdrFix.BorderSizePixel=0; catPopHdrFix.Position=UDim2.new(0,0,0.5,0); catPopHdrFix.Size=UDim2.new(1,0,0.5,0); catPopHdrFix.ZIndex=421
local catPopTitle=Instance.new("TextLabel",catPopHdr); catPopTitle.BackgroundTransparency=1
catPopTitle.Position=UDim2.new(0,14,0,0); catPopTitle.Size=UDim2.new(0.55,1,1,0)
catPopTitle.Font=Enum.Font.GothamBlack; catPopTitle.Text="🗂️  Cor por ESP"
catPopTitle.TextColor3=CAT_ACCENT; catPopTitle.TextSize=11
catPopTitle.TextXAlignment=Enum.TextXAlignment.Left; catPopTitle.ZIndex=422
local catResetAll=Instance.new("TextButton",catPopHdr); catResetAll.BackgroundColor3=Color3.fromRGB(140,40,40)
catResetAll.BackgroundTransparency=0.4; catResetAll.BorderSizePixel=0
catResetAll.AnchorPoint=Vector2.new(1,0.5); catResetAll.Position=UDim2.new(1,-10,0.5,0)
catResetAll.Size=UDim2.new(0,72,0,22); catResetAll.Font=Enum.Font.GothamBold; catResetAll.Text="↺ Reset"
catResetAll.TextColor3=Color3.fromRGB(255,200,200); catResetAll.TextSize=9; catResetAll.ZIndex=422
Instance.new("UICorner",catResetAll).CornerRadius=UDim.new(0,7)
catResetAll.MouseButton1Click:Connect(function()
    espCatColors={}
    Notify.info("Cores ESP","↺ Todas as cores resetadas para global.",2)
end)

-- Layout: sidebar esquerda (lista de ESPs) + painel direita (seletor de cor)
local catSidebar=Instance.new("ScrollingFrame",catPop); catSidebar.BackgroundTransparency=1
catSidebar.BorderSizePixel=0; catSidebar.Position=UDim2.new(0,0,0,38)
catSidebar.Size=UDim2.new(0,130,1,-38); catSidebar.ScrollBarThickness=2
catSidebar.ScrollBarImageColor3=CAT_ACCENT; catSidebar.CanvasSize=UDim2.new(0,0,0,0)
catSidebar.AutomaticCanvasSize=Enum.AutomaticSize.Y; catSidebar.ZIndex=421
local catSideLayout=Instance.new("UIListLayout",catSidebar)
catSideLayout.SortOrder=Enum.SortOrder.LayoutOrder; catSideLayout.Padding=UDim.new(0,2)
local catSidePad=Instance.new("UIPadding",catSidebar)
catSidePad.PaddingTop=UDim.new(0,4); catSidePad.PaddingLeft=UDim.new(0,4); catSidePad.PaddingRight=UDim.new(0,2)

-- Divisor vertical
local catDivider=Instance.new("Frame",catPop); catDivider.BackgroundColor3=CAT_ACCENT
catDivider.BackgroundTransparency=0.7; catDivider.BorderSizePixel=0
catDivider.Position=UDim2.new(0,132,0,38); catDivider.Size=UDim2.new(0,1,1,-40); catDivider.ZIndex=421

-- Painel direita: color wheel mini
local catColorPanel=Instance.new("Frame",catPop); catColorPanel.BackgroundTransparency=1
catColorPanel.BorderSizePixel=0; catColorPanel.Position=UDim2.new(0,136,0,38)
catColorPanel.Size=UDim2.new(1,-140,1,-38); catColorPanel.ZIndex=421

local catColorTitle=Instance.new("TextLabel",catColorPanel); catColorTitle.BackgroundTransparency=1
catColorTitle.Position=UDim2.new(0,4,0,4); catColorTitle.Size=UDim2.new(1,-8,0,16)
catColorTitle.Font=Enum.Font.GothamBold; catColorTitle.Text="Selecione um ESP →"
catColorTitle.TextColor3=Color3.fromRGB(150,135,185); catColorTitle.TextSize=9
catColorTitle.TextXAlignment=Enum.TextXAlignment.Center; catColorTitle.ZIndex=422

local selectedCatKey = nil

-- Mini color wheel para seleção por categoria
local cwMini=Instance.new("Frame",catColorPanel); cwMini.BackgroundTransparency=1
cwMini.BorderSizePixel=0; cwMini.Position=UDim2.new(0.5,-55,0,24)
cwMini.Size=UDim2.new(0,110,0,110); cwMini.ZIndex=422
local cwMiniBg=Instance.new("Frame",cwMini); cwMiniBg.BackgroundColor3=Color3.fromRGB(0,0,0)
cwMiniBg.BackgroundTransparency=0.3; cwMiniBg.BorderSizePixel=0; cwMiniBg.Size=UDim2.new(1,0,1,0); cwMiniBg.ZIndex=422
Instance.new("UICorner",cwMiniBg).CornerRadius=UDim.new(1,0)

for j,hc in ipairs(HUE_COLORS) do
    local seg=Instance.new("Frame",cwMini)
    seg.BackgroundColor3=hc; seg.BackgroundTransparency=0; seg.BorderSizePixel=0
    local angle=(j-1)/12*math.pi*2
    local cx=55+math.cos(angle)*40-8; local cy=55+math.sin(angle)*40-8
    seg.Position=UDim2.new(0,cx,0,cy); seg.Size=UDim2.new(0,16,0,16); seg.ZIndex=423
    Instance.new("UICorner",seg).CornerRadius=UDim.new(1,0)
    local segBtn=Instance.new("TextButton",seg); segBtn.BackgroundTransparency=1
    segBtn.Size=UDim2.new(1,0,1,0); segBtn.Text=""; segBtn.ZIndex=424; segBtn.AutoButtonColor=false
    segBtn.MouseButton1Click:Connect(function()
        if not selectedCatKey then
            Notify.warn("Cores ESP","Selecione um ESP na lista primeiro!",2); return
        end
        espCatColors[selectedCatKey]=hc
        lastCache=0
        local cnames={[0]="Vermelho",[30]="Laranja",[60]="Amarelo",[90]="Verde-Am.",[120]="Verde",[150]="V-Ciano",[180]="Ciano",[210]="Az-Ciano",[240]="Azul",[270]="Roxo",[300]="Magenta",[330]="Rosa"}
        local hue=math.floor((j-1)/12*360/30)*30
        catColorTitle.Text="✓ "..( cnames[hue] or "Custom")
        catColorTitle.TextColor3=hc
        Notify.send({type="custom",icon="🎨",accent=hc,title="Cor ESP",msg=(selectedCatKey or "?")..": "..(cnames[hue] or "Custom"),duration=2})
    end)
end

-- Reset cor desta categoria
local catResetThis=Instance.new("TextButton",catColorPanel); catResetThis.BackgroundColor3=Color3.fromRGB(80,40,40)
catResetThis.BackgroundTransparency=0.4; catResetThis.BorderSizePixel=0
catResetThis.Position=UDim2.new(0,4,1,-28); catResetThis.Size=UDim2.new(1,-8,0,22)
catResetThis.Font=Enum.Font.GothamBold; catResetThis.Text="↺ Reset este"
catResetThis.TextColor3=Color3.fromRGB(255,180,180); catResetThis.TextSize=9; catResetThis.ZIndex=422
Instance.new("UICorner",catResetThis).CornerRadius=UDim.new(0,7)
catResetThis.MouseButton1Click:Connect(function()
    if selectedCatKey then
        espCatColors[selectedCatKey]=nil; lastCache=0
        catColorTitle.Text="↺ Reset — usa global"; catColorTitle.TextColor3=Color3.fromRGB(150,135,185)
    end
end)

-- Sidebar items
local catSideItemRefs={}
for lo,cat in ipairs(ESP_CATS) do
    local si=Instance.new("TextButton",catSidebar)
    si.BackgroundColor3=Color3.fromRGB(24,14,44); si.BackgroundTransparency=0.45
    si.BorderSizePixel=0; si.Size=UDim2.new(1,0,0,26); si.LayoutOrder=lo; si.ZIndex=422
    si.Font=Enum.Font.GothamBold; si.Text=cat.label:sub(1,2).." "..cat.label:sub(4)
    si.TextColor3=Color3.fromRGB(175,160,210); si.TextSize=9
    si.TextTruncate=Enum.TextTruncate.AtEnd; si.AutoButtonColor=false
    Instance.new("UICorner",si).CornerRadius=UDim.new(0,6)
    -- Dot de cor atual
    local dot=Instance.new("Frame",si); dot.Name="DOT"
    dot.BackgroundColor3=espCatColors[cat.key] or cat.cor
    dot.BorderSizePixel=0; dot.AnchorPoint=Vector2.new(1,0.5); dot.Position=UDim2.new(1,-4,0.5,0)
    dot.Size=UDim2.new(0,6,0,6); dot.ZIndex=423
    Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
    catSideItemRefs[cat.key]=si
    si.MouseButton1Click:Connect(function()
        selectedCatKey=cat.key
        catColorTitle.Text="Escolha cor: "..cat.label:sub(4)
        catColorTitle.TextColor3=espCatColors[cat.key] or cat.cor
        -- Highlight selected
        for k,s in pairs(catSideItemRefs) do
            local isSel=(k==cat.key)
            TweenService:Create(s,TweenInfo.new(0.1),{BackgroundColor3=isSel and Color3.fromRGB(52,28,84) or Color3.fromRGB(24,14,44),BackgroundTransparency=isSel and 0.1 or 0.45}):Play()
            s.TextColor3=isSel and Color3.new(1,1,1) or Color3.fromRGB(175,160,210)
        end
    end)
end

-- Update dot colors whenever espCatColors changes
task.spawn(function()
    while true do
        task.wait(0.5)
        for _,cat in ipairs(ESP_CATS) do
            local si=catSideItemRefs[cat.key]
            if si then
                local dot=si:FindFirstChild("DOT")
                if dot then dot.BackgroundColor3=espCatColors[cat.key] or espGlobalCor end
            end
        end
    end
end)
...
local CAT_POP_H=320
local function openCatPop()
    if _vdOpen and _vdOpen~=catPop then
        TweenService:Create(_vdOpen,TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,_vdOpen.AbsoluteSize.X,0,0)}):Play()
        task.delay(0.13,function() _vdOpen.Visible=false end)
    end
    local ap=catBtn.AbsolutePosition; local as=catBtn.AbsoluteSize
    catPop.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+6)
    catPop.Size=UDim2.new(0,320,0,0); catPop.BackgroundTransparency=0.4; catPop.Visible=true
    TweenService:Create(catPop,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,320,0,CAT_POP_H),BackgroundTransparency=0}):Play()
    catOpen=true; _vdOpen=catPop
    TweenService:Create(catArr,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Rotation=180}):Play()
    TweenService:Create(catBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.05}):Play()
end
local function closeCatPop()
    catOpen=false
    TweenService:Create(catPop,TweenInfo.new(0.18,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,320,0,0),BackgroundTransparency=0.4}):Play()
    task.delay(0.2,function() catPop.Visible=false; catPop.BackgroundTransparency=0 end)
    _vdOpen=nil
    TweenService:Create(catArr,TweenInfo.new(0.2),{Rotation=0}):Play()
    TweenService:Create(catBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.2}):Play()
end
catBtn.MouseButton1Click:Connect(function() if catOpen then closeCatPop() else openCatPop() end end)
UserInputService.InputBegan:Connect(function(inp)
    if not catOpen then return end
    if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
    local mp=UserInputService:GetMouseLocation()
    local function inside(f) local a,s=f.AbsolutePosition,f.AbsoluteSize; return mp.X>=a.X and mp.X<=a.X+s.X and mp.Y>=a.Y and mp.Y<=a.Y+s.Y end
    if not inside(catPop) and not inside(catBtn) then closeCatPop() end
end)
...
for _,grp in ipairs(ESP_GROUPS) do
    ...
    if #validCats>0 then
        ...
        for i,cat in ipairs(validCats) do
            ...
            Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
            pill.BackgroundColor3=Color3.fromRGB(65,72,90)
            local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
            knob.Size=UDim2.new(0,18,0,18); knob.ZIndex=10
            Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
            knob.BackgroundColor3=Color3.fromRGB(255,255,255); knob.Position=UDim2.new(0,2,0.5,-9)
            local ck=Instance.new("TextLabel",knob); ck.BackgroundTransparency=1
            ck.Size=UDim2.new(1,0,1,0); ck.Font=Enum.Font.GothamBlack
            ck.Text="✕"; ck.TextColor3=Color3.fromRGB(65,72,90); ck.TextSize=9; ck.ZIndex=11
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                TweenService:Create(pill,TweenInfo.new(0.18),{BackgroundColor3=state and displayCor or Color3.fromRGB(65,72,90)}):Play()
                TweenService:Create(knob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
                    Position=state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9),
                    BackgroundColor3=state and Color3.fromRGB(255,255,255) or Color3.fromRGB(130,90,30),
                }):Play()
                ck.Text=state and "✓" or "✕"; ck.TextColor3=state and displayCor or Color3.fromRGB(65,72,90)
...
do
    ...
    infoPnl.Size = UDim2.new(1,0,0,112); infoPnl.LayoutOrder = 0; infoPnl.ZIndex = 5
    ...
    -- ══════════════════════════════════════════════════════
    -- HELPER: Cria dropdown profissional para o painel Bring
    -- Itens arredondados, animações suaves, sem bug visual
    -- ══════════════════════════════════════════════════════
    local function makeBringDropdown(cfg)
        -- cfg: { opts, getVal, setVal, btn, popup, openState,
        --        itemH, width, onClose }
        local opts    = cfg.opts
        local ITEM_H  = cfg.itemH or 40
        local WIDTH   = cfg.width or 180
        local PAD     = 6
        local TOTAL_H = #opts * ITEM_H + PAD*2
        local isOpen  = false

        -- ── Popup container ──────────────────────────────────
        local pop = Instance.new("Frame", ScreenGui)
        pop.BackgroundColor3      = Color3.fromRGB(28, 18, 48)
        pop.BackgroundTransparency = 0
        pop.BorderSizePixel        = 0
        pop.ZIndex                 = 420
        pop.Visible                = false
        ...
        popStroke.Color          = Color3.fromRGB(120, 86, 188)
        ...
        popShine.BackgroundTransparency = 0.88
        popShine.BorderSizePixel        = 0
        popShine.Size                   = UDim2.new(0.6, 0, 0, 1)
        popShine.Position               = UDim2.new(0.2, 0, 0, 0)
        popShine.ZIndex                 = 421

        local popLayout = Instance.new("UIListLayout", pop)
        popLayout.SortOrder = Enum.SortOrder.LayoutOrder
        popLayout.Padding   = UDim.new(0, 0)

        local popPad = Instance.new("UIPadding", pop)
        popPad.PaddingTop    = UDim.new(0, PAD)
        popPad.PaddingBottom = UDim.new(0, PAD)
        ...
        for i, opt in ipairs(opts) do
            local isSel = (cfg.getVal() == opt.key)

            local itemFrame = Instance.new("Frame", pop)
            itemFrame.BackgroundColor3       = isSel and Color3.fromRGB(60, 38, 96) or Color3.fromRGB(36, 22, 60)
            itemFrame.BackgroundTransparency = isSel and 0.15 or 0.55
            itemFrame.BorderSizePixel        = 0
            itemFrame.Size                   = UDim2.new(1, 0, 0, ITEM_H - 2)
            itemFrame.LayoutOrder            = i
            itemFrame.ZIndex                 = 422
            Instance.new("UICorner", itemFrame).CornerRadius = UDim.new(0, 8)

            -- Borda colorida no item selecionado
            local itemStroke = Instance.new("UIStroke", itemFrame)
            itemStroke.Color          = opt.cor
            itemStroke.Thickness      = 1.2
            itemStroke.Transparency   = isSel and 0.2 or 0.85
            itemStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

            -- Dot colorido à esquerda
            local dot = Instance.new("Frame", itemFrame)
            dot.BackgroundColor3      = opt.cor
            dot.BackgroundTransparency = isSel and 0 or 0.6
            dot.BorderSizePixel        = 0
            ...
            -- Ícone
            local icoLbl = Instance.new("TextLabel", itemFrame)
            icoLbl.BackgroundTransparency = 1
            icoLbl.Position               = UDim2.new(0, 20, 0, 0)
            icoLbl.Size                   = UDim2.new(0, 22, 1, 0)
            icoLbl.Font                   = Enum.Font.GothamBold
            icoLbl.Text                   = opt.icon or ""
            icoLbl.TextSize               = 14
            icoLbl.ZIndex                 = 423
            ...
            nameLbl.BackgroundTransparency = 1
            nameLbl.Position               = UDim2.new(0, opt.icon and 44 or 22, 0, 0)
            nameLbl.Size                   = UDim2.new(1, -(opt.icon and 50 or 28), 1, 0)
            nameLbl.Font                   = Enum.Font.GothamBold
            nameLbl.Text                   = opt.label
            nameLbl.TextColor3             = isSel and Color3.fromRGB(255, 250, 255) or Color3.fromRGB(185, 172, 215)
            nameLbl.TextSize               = 11
            nameLbl.TextXAlignment         = Enum.TextXAlignment.Left
            nameLbl.ZIndex                 = 423

            -- Check mark ✓ direita
            local checkLbl = Instance.new("TextLabel", itemFrame)
            checkLbl.BackgroundTransparency = 1
            checkLbl.AnchorPoint           = Vector2.new(1, 0.5)
            checkLbl.Position              = UDim2.new(1, -8, 0.5, 0)
            checkLbl.Size                  = UDim2.new(0, 16, 0, 16)
            checkLbl.Font                  = Enum.Font.GothamBlack
            checkLbl.Text                  = isSel and "✓" or ""
            checkLbl.TextColor3            = opt.cor
            checkLbl.TextSize              = 11
            checkLbl.ZIndex                = 423

            -- Spacer entre itens
            local spacer = Instance.new("Frame", pop)
            spacer.BackgroundTransparency = 1
            spacer.BorderSizePixel        = 0
            spacer.Size                   = UDim2.new(1, 0, 0, 2)
            spacer.LayoutOrder            = i * 10 + 5
            spacer.ZIndex                 = 421

            -- Função refresh (sem bug: sempre arredondado, sem branco)
            local function refreshItem(hover)
                local sel = (cfg.getVal() == opt.key)
                local bgColor, bgTrans, strokeTrans, dotTrans, textColor
                if sel then
                    bgColor    = Color3.fromRGB(60, 38, 96)
                    bgTrans    = 0.05
                    strokeTrans = 0.15
                    dotTrans   = 0
                    textColor  = Color3.fromRGB(255, 250, 255)
                elseif hover then
                    bgColor    = Color3.fromRGB(50, 30, 80)
                    bgTrans    = 0.25
                    strokeTrans = 0.6
                    dotTrans   = 0.3
                    textColor  = Color3.fromRGB(220, 210, 240)
                else
                    bgColor    = Color3.fromRGB(36, 22, 60)
                    bgTrans    = 0.55
                    strokeTrans = 0.85
                    dotTrans   = 0.6
                    textColor  = Color3.fromRGB(185, 172, 215)
                end
                TweenService:Create(itemFrame, TweenInfo.new(0.14), {
                    BackgroundColor3 = bgColor, BackgroundTransparency = bgTrans
                }):Play()
                TweenService:Create(itemStroke, TweenInfo.new(0.14), {Transparency = strokeTrans}):Play()
                TweenService:Create(dot, TweenInfo.new(0.14), {BackgroundTransparency = dotTrans}):Play()
                nameLbl.TextColor3 = textColor
                checkLbl.Text = sel and "✓" or ""
            end
            itemRefs[opt.key] = refreshItem
            ...
            itemBtn.MouseEnter:Connect(function()
                if cfg.getVal() ~= opt.key then refreshItem(true) end
            end)
            itemBtn.MouseLeave:Connect(function() refreshItem(false) end)
            itemBtn.MouseButton1Click:Connect(function()
                local prev = cfg.getVal()
                cfg.setVal(opt.key)
                ...
                isOpen = false
                TweenService:Create(pop, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                    {Size = UDim2.new(0, WIDTH, 0, 0), BackgroundTransparency = 0.3}):Play()
                task.delay(0.2, function() pop.Visible = false; pop.BackgroundTransparency = 0 end)
                _vdOpen = nil
                -- Anima botão trigger
                TweenService:Create(cfg.btn, TweenInfo.new(0.15),
                    {BackgroundTransparency = 0.65}):Play()
                if cfg.onSelect then cfg.onSelect(opt) end
            end)
        end
        ...
        local function closeDrop()
            ...
            task.delay(0.2, function() pop.Visible = false; pop.BackgroundTransparency = 0 end)
            _vdOpen = nil
            TweenService:Create(cfg.btn, TweenInfo.new(0.15), {BackgroundTransparency = 0.65}):Play()
        end
        ...
        local function openDrop()
            ...
            -- Refresh visuais antes de abrir
            for _, fn in pairs(itemRefs) do pcall(fn, false) end
            local ap = cfg.btn.AbsolutePosition
            local as = cfg.btn.AbsoluteSize
            local posX = ap.X + as.X - WIDTH
            local posY = ap.Y + as.Y + 6
            pop.Position = UDim2.new(0, posX, 0, posY)
            pop.Size     = UDim2.new(0, WIDTH, 0, 0)
            pop.BackgroundTransparency = 0.4
            pop.Visible  = true
            TweenService:Create(pop, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                {Size = UDim2.new(0, WIDTH, 0, TOTAL_H), BackgroundTransparency = 0}):Play()
            isOpen = true; _vdOpen = pop
            TweenService:Create(cfg.btn, TweenInfo.new(0.15), {BackgroundTransparency = 0.35}):Play()
        end

        cfg.btn.MouseButton1Click:Connect(function()
        ...
        UserInputService.InputBegan:Connect(function(inp)
            ...
            local function inside(f)
                ...
                return mp.X>=a.X and mp.X<=a.X+s.X and mp.Y>=a.Y and mp.Y<=a.Y+s.Y
            end
            if not inside(pop) and not inside(cfg.btn) then closeDrop() end
        end)

        return pop, closeDrop
    end

    -- ══════════════════════════════════════════════════════
    -- LINHA ESTILO DE BRING
    -- ══════════════════════════════════════════════════════
    local STYLE_OPTS = {
        {key="padrao",    label="Padrão",    icon="🔵", cor=Color3.fromRGB(100,160,255)},
        {key="espalhado", label="Espalhado", icon="🟠", cor=Color3.fromRGB(255,150,60)},
        {key="juntos",    label="Juntos",    icon="🟢", cor=Color3.fromRGB(80,220,120)},
        {key="circulo",   label="Círculo",   icon="🔴", cor=Color3.fromRGB(255,90,90)},
    }
    local bStyleRow = Instance.new("Frame", infoPnl)
    bStyleRow.BackgroundTransparency = 1; bStyleRow.BorderSizePixel = 0
    bStyleRow.Size = UDim2.new(1,0,0,26); bStyleRow.Position = UDim2.new(0,0,1,-54); bStyleRow.ZIndex = 7
    local bStyleLbl = Instance.new("TextLabel", bStyleRow); bStyleLbl.BackgroundTransparency = 1
    bStyleLbl.Position = UDim2.new(0,10,0,0); bStyleLbl.Size = UDim2.new(0.42,0,1,0)
    bStyleLbl.Font = Enum.Font.GothamBold; bStyleLbl.Text = "⚡ Estilo"
    bStyleLbl.TextColor3 = Color3.fromRGB(200,185,235); bStyleLbl.TextSize = 10
    bStyleLbl.TextXAlignment = Enum.TextXAlignment.Left; bStyleLbl.ZIndex = 8

    -- Botão trigger Estilo
    local bStyleBtn = Instance.new("TextButton", bStyleRow)
    bStyleBtn.BackgroundColor3      = Color3.fromRGB(52, 32, 88)
    bStyleBtn.BackgroundTransparency = 0.65
    bStyleBtn.BorderSizePixel        = 0
    bStyleBtn.AnchorPoint            = Vector2.new(1, 0.5)
    bStyleBtn.Position               = UDim2.new(1,-8,0.5,0)
    bStyleBtn.Size                   = UDim2.new(0,118,0,22)
    bStyleBtn.Font                   = Enum.Font.GothamBold
    bStyleBtn.TextSize               = 10; bStyleBtn.ZIndex = 8
    bStyleBtn.AutoButtonColor        = false
    bStyleBtn.TextTruncate           = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", bStyleBtn).CornerRadius = UDim.new(0, 8)
    local bStyleBtnS = Instance.new("UIStroke", bStyleBtn)
    bStyleBtnS.Thickness = 1.2; bStyleBtnS.Transparency = 0.45
    bStyleBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    local function updateStyleBtn()
        for _, s in ipairs(STYLE_OPTS) do
            if s.key == bringStyle then
                bStyleBtn.Text      = s.icon .. "  " .. s.label .. "  ▾"
                bStyleBtn.TextColor3 = s.cor
                bStyleBtnS.Color    = s.cor
                return
            end
        end
        bStyleBtn.Text       = "🔵  Padrão  ▾"
        bStyleBtn.TextColor3 = Color3.fromRGB(100,160,255)
        bStyleBtnS.Color     = Color3.fromRGB(100,160,255)
    end
    updateStyleBtn()

    makeBringDropdown({
        opts    = STYLE_OPTS,
        getVal  = function() return bringStyle end,
        setVal  = function(k) bringStyle = k; updateStyleBtn() end,
        btn     = bStyleBtn,
        itemH   = 38,
        width   = 180,
        onSelect = function(opt)
            Notify.send({type="custom",icon=opt.icon,accent=opt.cor,
                title="Estilo de Bring",msg=opt.label,duration=2})
        end,
    })

    -- ══════════════════════════════════════════════════════
    -- LINHA SELECT DESTINO (MODO)
    -- ══════════════════════════════════════════════════════
    local bModeRow = Instance.new("Frame", infoPnl)
    bModeRow.BackgroundTransparency = 1; bModeRow.BorderSizePixel = 0
    bModeRow.Size = UDim2.new(1,0,0,26); bModeRow.Position = UDim2.new(0,0,1,-26); bModeRow.ZIndex = 7
    local bModeSelLbl = Instance.new("TextLabel", bModeRow); bModeSelLbl.BackgroundTransparency = 1
    bModeSelLbl.Position = UDim2.new(0,10,0,0); bModeSelLbl.Size = UDim2.new(0.42,0,1,0)
    bModeSelLbl.Font = Enum.Font.GothamBold; bModeSelLbl.Text = "🎯 Destino"
    bModeSelLbl.TextColor3 = Color3.fromRGB(200,185,235); bModeSelLbl.TextSize = 10
    bModeSelLbl.TextXAlignment = Enum.TextXAlignment.Left; bModeSelLbl.ZIndex = 8

    -- Reposiciona e reformata o modeBtn existente
    modeBtn.Parent           = bModeRow
    modeBtn.AnchorPoint      = Vector2.new(1,0.5)
    modeBtn.Position         = UDim2.new(1,-8,0.5,0)
    modeBtn.Size             = UDim2.new(0,118,0,22)
    modeBtn.BackgroundColor3 = Color3.fromRGB(52,32,88)
    modeBtn.BackgroundTransparency = 0.65
    modeBtn.Font             = Enum.Font.GothamBold
    modeBtn.TextSize         = 10
    modeBtn.ZIndex           = 8
    modeBtn.AutoButtonColor  = false
    modeBtn.TextTruncate     = Enum.TextTruncate.AtEnd
    -- Remove gradiente e shine do modeBtn antigo
    pcall(function() modeBtnG:Destroy() end)
    pcall(function() modeBtnShine:Destroy() end)
    -- Remove stroke antigo e adiciona novo
    for _, c in ipairs(modeBtn:GetChildren()) do
        if c:IsA("UICorner") or c:IsA("UIStroke") then c:Destroy() end
    end
    Instance.new("UICorner", modeBtn).CornerRadius = UDim.new(0, 8)
    local modeBtnSNew = Instance.new("UIStroke", modeBtn)
    modeBtnSNew.Thickness = 1.2; modeBtnSNew.Transparency = 0.45
    modeBtnSNew.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Override updateModeBtn para usar novo visual
    local function updateModeBtnNew()
        local mc = MODE_COLORS[bringDestMode] or Color3.fromRGB(148,112,220)
        for _, o in ipairs(MODE_OPTS) do
            if o.key == bringDestMode then
                modeBtn.Text       = o.icon.."  "..o.label.."  ▾"
                modeBtn.TextColor3 = mc
                modeBtnSNew.Color  = mc
                return
            end
        end
        modeBtn.Text       = "🔒  Modo  ▾"
        modeBtn.TextColor3 = Color3.fromRGB(180,130,60)
        modeBtnSNew.Color  = Color3.fromRGB(180,130,60)
    end
    updateModeBtnNew()

    -- Override a função updateModeBtn original
    updateModeBtn = updateModeBtnNew

    -- MODE_OPTS com ícones e cores para o helper
    local MODE_OPTS_DROP = {}
    for _, o in ipairs(MODE_OPTS) do
        table.insert(MODE_OPTS_DROP, {
            key   = o.key,
            label = o.label,
            icon  = o.icon,
            cor   = MODE_COLORS[o.key] or Color3.fromRGB(148,112,220),
        })
    end

    makeBringDropdown({
        opts    = MODE_OPTS_DROP,
        getVal  = function() return bringDestMode end,
        setVal  = function(k)
        ...
        onSelect = function(opt)
            Notify.send({type="info",icon=opt.icon,accent=opt.cor,
                title="Modo Bring",msg=opt.label,duration=2.5})
        end,
    })
                    ...
                    if lk then
                        for _,obj in ipairs(workspace:GetDescendants()) do
                            pcall(function()
                                ...
                                if lk[nm] then total = total + 1 end
                            end)
                        end
                    end
                end
...
for _,grp in ipairs(BRING_GROUPS) do
    ...
    for i, bcat in ipairs(validCats) do
...
attsToggleBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(attsKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Position=attsOn and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
    }):Play()
...
local function _nt(c,y,lbl,initOn,onToggle)
    ...
    pill.Position=UDim2.new(1,-12,0,y+(H-28)/2)
    pill.Size=UDim2.new(0,52,0,28); pill.ZIndex=8
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    pill.BackgroundColor3=initOn and _ON or _OF
    local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
    knob.Size=UDim2.new(0,22,0,22); knob.ZIndex=9
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255)
    ...
    ck.Size=UDim2.new(1,0,1,0); ck.Font=Enum.Font.GothamBlack
    ck.Text=initOn and "✓" or "✕"
    ck.TextColor3=initOn and _ON or _OF
    ck.TextSize=11; ck.ZIndex=10
    local en=initOn
    ...
    btn.MouseButton1Click:Connect(function()
        en=not en
        TweenService:Create(pill,TweenInfo.new(0.18),{BackgroundColor3=en and _ON or _OF}):Play()
        TweenService:Create(knob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=en and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
        }):Play()
        ck.Text=en and "✓" or "✕"
        ck.TextColor3=en and _ON or _OF
        if onToggle then onToggle(en) end
    end)
    return H
end
...
do
    ...
    -- Seção
-- ── Auto Fogueira Anoitecer — Accordion ─────────────────────
do
...
_accToggle(_nightCF,_ny,"🌙  Auto Fogueira Anoitecer",false,NIGHT_COR2,function(s)
    nightAutoEnabled=s
    if s then Notify.success("🌙 Auto Fogueira","Ativado — teleportará ao anoitecer!")
    else Notify.info("🌙 Auto Fogueira","Desativado.") end
end)
end -- Auto Fogueira Anoitecer
end -- do (escopo Auto Fogueira Anoitecer)
...
_eatBtn.MouseButton1Click:Connect(function()
    if eatRunning then
        stopAutoEat()
        _eatBtn.Text="🍖  ATIVAR"
        TweenService:Create(_eatBtn,TweenInfo.new(0.15),{BackgroundColor3=ATE_COR_P}):Play()
...
local function bpScanBuildings(isRefresh)
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            local nm = obj.Name:lower()
            -- Filtra itens que não são construções (traps, spikes, baús, NPCs)
            if not (nm:find("trap",1,true) or nm:find("spike",1,true)
            or nm:find("chest",1,true) or nm:find("bau",1,true) or nm:find("baú",1,true)) then
            -- É construção — pega parte representativa
            ...
            end -- fim do filtro de construção
        end
        -- Atualiza subtítulo
...
local function cpScanChests(isRefresh)
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            local total_bd = 0
            if biomeLabel == "Floresta" then
                for _, sub in pairs(bd.rarSubs) do total_bd = total_bd + #sub.entries end
            else
                total_bd = #bd.entries
            end
        ...
        for k, bd in pairs(biomeData) do
            totalBiomes = totalBiomes + 1
            if k == "Floresta" then
                for _, sub in pairs(bd.rarSubs) do totalChests = totalChests + #sub.entries end
            else
                totalChests = totalChests + #bd.entries
            end
        end
...
local function _nft(c2,y,lbl,initOn,onToggle)
    ...
    pill.Size=UDim2.new(0,52,0,28); pill.ZIndex=8
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    pill.BackgroundColor3=initOn and _ON_F or _OF_F
    local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
    knob.Size=UDim2.new(0,22,0,22); knob.ZIndex=9
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255)
    ...
    ck.Size=UDim2.new(1,0,1,0); ck.Font=Enum.Font.GothamBlack
    ck.Text=initOn and "✓" or "✕"
    ck.TextColor3=initOn and _ON_F or _OF_F
    ck.TextSize=11; ck.ZIndex=10
    local en=initOn
    ...
    btn.MouseButton1Click:Connect(function()
        en=not en
        TweenService:Create(pill,TweenInfo.new(0.18),{BackgroundColor3=en and _ON_F or _OF_F}):Play()
        TweenService:Create(knob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=en and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
        }):Play()
        ck.Text=en and "✓" or "✕"
        ck.TextColor3=en and _ON_F or _OF_F
        if onToggle then onToggle(en) end
    end)
    return H
end
...
local kaHitboxConn = nil
local KA_HITBOX_SIZE = 300

-- ── Expande hitbox do mob ──────────────────────────────────────
local kaExpandedMobs = {}  -- [mob] = originalSize
...
local function expandMobHitbox(mob)
    if isPlayerChar(mob) then return end
    local root = getMobRoot(mob)
    if not root then return end
    if kaExpandedMobs[mob] then return end  -- já expandido
    pcall(function()
        kaExpandedMobs[mob] = root.Size  -- guarda original
        root.Size = Vector3.new(KA_HITBOX_SIZE, KA_HITBOX_SIZE, KA_HITBOX_SIZE)
        root.Transparency = 1
...
local function restoreMobHitbox(mob)
    ...
    kaExpandedMobs[mob] = nil
end

local function restoreAllHitboxes()
    for mob,_ in pairs(kaExpandedMobs) do
        restoreMobHitbox(mob)
    end
    kaExpandedMobs = {}
end

-- Atualiza hitboxes baseado no alcance atual
local function updateHitboxesByRange()
    local ch = Player.Character; if not ch then return end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    local hrpPos = hrp.Position

    -- Expande mobs dentro do range, restaura os que saíram
    local inRange = {}
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            if not obj:IsA("Model") then return end
            ...
            local hum = obj:FindFirstChildWhichIsA("Humanoid")
            if not hum or hum.Health <= 0 then return end
            local root = getMobRoot(obj); if not root then return end
    ...
    -- Restaura mobs que saíram do alcance
    for mob,_ in pairs(kaExpandedMobs) do
        if not inRange[mob] then
            restoreMobHitbox(mob)
        end
    end
end
...
local function startKillAura()
    ...
    if kaHitboxConn then pcall(function() kaHitboxConn:Disconnect() end) end
    kaExpandedMobs = {}

    -- Expande hitboxes de novos mobs que aparecem
    kaHitboxConn = workspace.DescendantAdded:Connect(function(obj)
        if not kaEnabled then return end
        if obj:IsA("Model") and not isPlayerChar(obj) and obj:FindFirstChildWhichIsA("Humanoid") then
            task.wait(0.1)
            -- só expande se estiver no alcance
            local ch = Player.Character
            local hrp = ch and ch:FindFirstChild("HumanoidRootPart")
            if hrp then
                local root = getMobRoot(obj)
                if root and (root.Position - hrp.Position).Magnitude <= kaRange then
                    expandMobHitbox(obj)
                end
            end
        end
    ...
    -- Loop de auto-attack com mouse1press + atualização de hitboxes por range
    local lastAttack = 0
    local lastHitboxUpdate = 0
    kaAutoLoop = RunService.Heartbeat:Connect(function()
        if not kaEnabled then return end
        local now = tick()
        -- Atualiza hitboxes por range a cada 0.5s
        if now - lastHitboxUpdate >= 0.5 then
            lastHitboxUpdate = now
            pcall(updateHitboxesByRange)
        end
        -- Auto-attack
        if now - lastAttack < 0.15 then return end
        local ch = Player.Character; if not ch then return end
        local hum = ch:FindFirstChildWhichIsA("Humanoid")
        if not hum or hum.Health <= 0 then return end
...
local function stopKillAura()
    ...
    if kaHitboxConn then pcall(function() kaHitboxConn:Disconnect() end); kaHitboxConn = nil end
    restoreAllHitboxes()
end
...
local function autoFeedCampfire()
    ...
    afcRunning=false
    pcall(function() if afcRef.btn then afcRef.btn.Text="🔥  ATIVAR"; TweenService:Create(afcRef.btn,TweenInfo.new(0.15),{BackgroundColor3=AFC_COR}):Play() end end)
end
...
_afBtn.MouseButton1Click:Connect(function()
    ...
    if afcRunning then
        afcRunning=false; _afBtn.Text="🔥  ATIVAR"; TweenService:Create(_afBtn,TweenInfo.new(0.15),{BackgroundColor3=AFC_COR}):Play()
        ...
        _afBtn.Text="⏹  PARAR"; TweenService:Create(_afBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(200,50,50)}):Play()
        task.spawn(function() autoFeedCampfire()
            if not afcRunning then pcall(function() _afBtn.Text="🔥  ATIVAR"; TweenService:Create(_afBtn,TweenInfo.new(0.15),{BackgroundColor3=AFC_COR}):Play() end) end
        end)
    end
end)
end
...
_afhpBtn.MouseButton1Click:Connect(function()
    if afhpRunning then
        afhpRunning=false; _afhpBtn.Text="❤️  ATIVAR"; TweenService:Create(_afhpBtn,TweenInfo.new(0.15),{BackgroundColor3=AFHP_COR}):Play()
        ...
        _afhpBtn.Text="⏹  PARAR"; TweenService:Create(_afhpBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(200,50,50)}):Play()
        task.spawn(function() autoFeedHP()
            if not afhpRunning then pcall(function() _afhpBtn.Text="❤️  ATIVAR"; TweenService:Create(_afhpBtn,TweenInfo.new(0.15),{BackgroundColor3=AFHP_COR}):Play() end) end
        end)
    end
end)
end
...
local function autoCookFood()
    ...
    ackRunning=false
    pcall(function() if ackRef.btn then ackRef.btn.Text="🍖  ATIVAR"; TweenService:Create(ackRef.btn,TweenInfo.new(0.15),{BackgroundColor3=ACK_COR}):Play() end end)
end
...
local function autoMachineGrind()
    ...
    amgRunning=false
    pcall(function() if amgRef.btn then amgRef.btn.Text="⚙️  ATIVAR"; TweenService:Create(amgRef.btn,TweenInfo.new(0.15),{BackgroundColor3=AMG_COR}):Play() end end)
end


-- ── AUTO MACHINE GRIND — Accordion + Inline Dropdown ─────────
...
local _amgCard,_amgCF,_amgDrop = makeAccordionCard(Pages["Farm"], fNextLO, {
    ...
    color=AMG_COR, contentH=36+9+34+8+22+8+32+14,
})
amgCard=_amgCard
local _y=36+8; _accDivLine(_amgCF,_y,AMG_COR); _y=_y+9
local _amgSelBtn,_,_,_amgSelAll,_amgClearAll=makeInlineDropdown(_amgCF,_y,{
    title="Itens para Triturar",items=AMG_ALL_ITEMS,sel=amgSel,color=AMG_COR,maxVisible=6,setDropH=_amgDrop,
})
amgRef.selBtn=_amgSelBtn; amgRef.selBtnS=_amgSelBtn:FindFirstChildOfClass("UIStroke")
_y=_y+34+8
-- Botões Todos/Nenhum
local _qRow=Instance.new("Frame",_amgCF); _qRow.BackgroundTransparency=1; _qRow.BorderSizePixel=0
_qRow.Position=UDim2.new(0,10,0,_y); _qRow.Size=UDim2.new(1,-20,0,22); _qRow.ZIndex=8
local _qAll=Instance.new("TextButton",_qRow); _qAll.BackgroundColor3=Color3.fromRGB(30,100,180); _qAll.BorderSizePixel=0
_qAll.Size=UDim2.new(0.48,0,1,0); _qAll.Font=Enum.Font.GothamBold; _qAll.Text="✅ Todos"; _qAll.TextColor3=Color3.fromRGB(220,235,255); _qAll.TextSize=9; _qAll.ZIndex=9
Instance.new("UICorner",_qAll).CornerRadius=UDim.new(0,6)
local _qNone=Instance.new("TextButton",_qRow); _qNone.BackgroundColor3=Color3.fromRGB(100,30,30); _qNone.BorderSizePixel=0
_qNone.Position=UDim2.new(0.52,0,0,0); _qNone.Size=UDim2.new(0.48,0,1,0)
_qNone.Font=Enum.Font.GothamBold; _qNone.Text="✕ Nenhum"; _qNone.TextColor3=Color3.fromRGB(255,200,200); _qNone.TextSize=9; _qNone.ZIndex=9
Instance.new("UICorner",_qNone).CornerRadius=UDim.new(0,6)
_qAll.MouseButton1Click:Connect(_amgSelAll); _qNone.MouseButton1Click:Connect(_amgClearAll)
_y=_y+28+8
local _amgBtn,_amgBtnS=_accActivBtn(_amgCF,_y,"⚙️",AMG_COR); _y=_y+40
amgRef.btn=_amgBtn; amgRef.btnG=_amgBtnS; amgRef.status=_accStatusLbl(_amgCF,_y)
amgRef.stroke=_amgCard:FindFirstChildOfClass("UIStroke")
_amgBtn.MouseButton1Click:Connect(function()
    if amgRunning then
        amgRunning=false; _amgBtn.Text="⚙️  ATIVAR"; TweenService:Create(_amgBtn,TweenInfo.new(0.15),{BackgroundColor3=AMG_COR}):Play()
        amgRef.status.Text="⏹ Parado"; Notify.error("Auto Machine Grind","⏹ Desativado")
        ...
        if not amgHasSelection() then
            ...
            TweenService:Create(_amgSelBtn:FindFirstChildOfClass("UIStroke"),TweenInfo.new(0.1),{Color=Color3.fromRGB(255,80,80),Transparency=0}):Play()
            task.delay(0.8,function() TweenService:Create(_amgSelBtn:FindFirstChildOfClass("UIStroke"),TweenInfo.new(0.2),{Color=AMG_COR,Transparency=0.55}):Play() end)
            return
        end
        _amgBtn.Text="⏹  PARAR"; TweenService:Create(_amgBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(200,50,50)}):Play()
        task.spawn(function() autoMachineGrind()
            if not amgRunning then pcall(function() _amgBtn.Text="⚙️  ATIVAR"; TweenService:Create(_amgBtn,TweenInfo.new(0.15),{BackgroundColor3=AMG_COR}):Play() end) end
        end)
    end
end)
end
...
local function autoBiofuel()
    ...
    pcall(function() if abfRef.btn then abfRef.btn.Text="🧪  ATIVAR"; TweenService:Create(abfRef.btn,TweenInfo.new(0.15),{BackgroundColor3=ABF_COR}):Play() end end)
end
...
local function autoFarmBaus()
    ...
    fbcRunning=false
    pcall(function() _fbcBtn.Text="🎁  ATIVAR"; TweenService:Create(_fbcBtn,TweenInfo.new(0.15),{BackgroundColor3=FBC_COR}):Play() end)
    pcall(function() _fbcStatus.Text="" end)
end
_fbcBtn.MouseButton1Click:Connect(function()
    if fbcRunning then
        fbcRunning=false; _fbcBtn.Text="🎁  ATIVAR"; TweenService:Create(_fbcBtn,TweenInfo.new(0.15),{BackgroundColor3=FBC_COR}):Play()
        _fbcStatus.Text="⏹ Parado"; Notify.error("Farm Baús","⏹ Desativado")
        task.delay(1.5,function() pcall(function() _fbcStatus.Text="" end) end)
    else
        _fbcBtn.Text="⏹  PARAR"; TweenService:Create(_fbcBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(200,50,50)}):Play()
        task.spawn(function() autoFarmBaus(); if not fbcRunning then pcall(function() _fbcBtn.Text="🎁  ATIVAR"; TweenService:Create(_fbcBtn,TweenInfo.new(0.15),{BackgroundColor3=FBC_COR}):Play() end) end end)
    end
end)
