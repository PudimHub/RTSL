...
-- ══════════════════════════════════════════════════════════════
--  NEXUS FOREST — Nova Interface Completa v6
--  Layout: Intro → Splash → NavRail + SubNav + Content + Footer
-- ══════════════════════════════════════════════════════════════

-- Dimensões da janela principal
local WIN_W, WIN_H = 600, 440
local RAIL_W       = 54   -- largura da barra lateral de ícones
local TOP_H        = 42   -- altura da TopBar
local FOOT_H       = 30   -- altura do rodapé
local CONTENT_H    = WIN_H - TOP_H - FOOT_H  -- 368
local SUBNAV_H     = 34   -- altura das sub-abas

-- Constantes de cor (Floresta Sombria)
local C_ACCENT      = Color3.fromRGB(52, 211, 153)
local C_TEXT_OFF    = Color3.fromRGB(80,  120,  98)
local C_TEXT_ON     = Color3.fromRGB(210, 255, 235)
local C_BG_HOV      = Color3.fromRGB(30,  44,   37)
local C_BG_ACT      = Color3.fromRGB(20,  45,   33)
local C_ICON_IDLE   = Color3.fromRGB(60,  100,  80)
local C_ICON_ACTIVE = Color3.fromRGB(52,  211, 153)

-- ══════════════════════════════════════════════════════════════
-- JANELA PRINCIPAL (criada primeiro, oculta até a intro terminar)
-- ══════════════════════════════════════════════════════════════
local MainFrame = Instance.new("Frame", ScreenGui)
MainFrame.Name             = "MainFrame"
MainFrame.BackgroundColor3 = Color3.fromRGB(11, 16, 13)
...
TopBar.Name             = "TopBar"
TopBar.BackgroundColor3 = Color3.fromRGB(8, 13, 10)
...
-- Fix: arredondar só o topo (cobrir cantos inferiores)
local topFix = Instance.new("Frame", TopBar)
topFix.BackgroundColor3 = Color3.fromRGB(8, 13, 10)
topFix.Size             = UDim2.new(1, 0, 0, 20)
topFix.Position         = UDim2.new(0, 0, 1, -20)
topFix.BorderSizePixel  = 0
topFix.ZIndex           = 7

-- Logo: ícone + nome + versão
...
LogoText.Position          = UDim2.new(0, 38, 0, 0)
LogoText.Size              = UDim2.new(0, 95, 1, 0)
LogoText.Font              = Enum.Font.GothamBlack
LogoText.Text              = "PudimHub"
LogoText.TextColor3        = Color3.fromRGB(52, 211, 153)
LogoText.TextSize          = 14
LogoText.TextXAlignment    = Enum.TextXAlignment.Left
LogoText.ZIndex            = 10
local LogoStroke = Instance.new("UIStroke", LogoText)
LogoStroke.Color        = Color3.fromRGB(20, 100, 60)
LogoStroke.Thickness    = 1.2
LogoStroke.Transparency = 0.5

local LogoSub = Instance.new("TextLabel", TopBar)
LogoSub.BackgroundTransparency = 1
LogoSub.Position        = UDim2.new(0, 137, 0, 0)
LogoSub.Size            = UDim2.new(0, 80, 1, 0)
LogoSub.Font            = Enum.Font.Gotham
LogoSub.Text            = "v5 · Floresta"
LogoSub.TextColor3      = Color3.fromRGB(55, 90, 70)
LogoSub.TextSize        = 9
LogoSub.TextXAlignment  = Enum.TextXAlignment.Left
LogoSub.ZIndex          = 10

-- Separador vertical entre logo e botões
local LogoDivider = Instance.new("Frame", TopBar)
LogoDivider.BackgroundColor3  = Color3.fromRGB(30, 55, 42)
LogoDivider.BorderSizePixel   = 0
LogoDivider.Position          = UDim2.new(0, 224, 0.5, -12)
LogoDivider.Size              = UDim2.new(0, 1, 0, 24)
LogoDivider.ZIndex            = 9

-- Botões da TopBar (direita)
local TopBtns = {}
do
    local BTN_DEFS = {
        {name="Theme",   icon="⚡", hoverColor=Color3.fromRGB(251,191,36)},
        {name="Minimize",icon="─",  hoverColor=Color3.fromRGB(52,211,153)},
        {name="Maximize",icon="⬜", hoverColor=Color3.fromRGB(52,211,153)},
        {name="Close",   icon="✕",  hoverColor=Color3.fromRGB(255,80,80)},
    }
    local bx = WIN_W - 8
    for i = #BTN_DEFS, 1, -1 do
        local d = BTN_DEFS[i]
        bx = bx - 30
        local bBg = Instance.new("Frame", TopBar)
        bBg.BackgroundColor3      = Color3.fromRGB(25, 42, 32)
        bBg.BackgroundTransparency = 1
        bBg.BorderSizePixel       = 0
        bBg.Position              = UDim2.new(0, bx, 0.5, -13)
        bBg.Size                  = UDim2.new(0, 26, 0, 26)
        bBg.ZIndex                = 9
        Instance.new("UICorner", bBg).CornerRadius = UDim.new(0, 8)

        local btn = Instance.new("TextButton", TopBar)
        btn.BackgroundTransparency = 1
        btn.Position               = UDim2.new(0, bx, 0.5, -13)
        btn.Size                   = UDim2.new(0, 26, 0, 26)
        btn.Font                   = Enum.Font.GothamBold
        btn.Text                   = d.icon
        btn.TextSize               = 12
        btn.TextColor3             = Color3.fromRGB(65, 100, 80)
        btn.ZIndex                 = 10
        btn.MouseEnter:Connect(function()
            TweenService:Create(bBg,  TweenInfo.new(0.15), {BackgroundTransparency=0.6}):Play()
            TweenService:Create(btn,  TweenInfo.new(0.15), {TextColor3=d.hoverColor}):Play()
        end)
        btn.MouseLeave:Connect(function()
            TweenService:Create(bBg,  TweenInfo.new(0.15), {BackgroundTransparency=1}):Play()
            TweenService:Create(btn,  TweenInfo.new(0.15), {TextColor3=Color3.fromRGB(65,100,80)}):Play()
        end)
        TopBtns[d.name] = btn
    end
end

-- Separator linha sob TopBar (só na área de conteúdo)
local TopSep = Instance.new("Frame", MainFrame)
TopSep.BackgroundColor3 = Color3.fromRGB(20, 36, 27)
TopSep.BorderSizePixel  = 0
TopSep.Position         = UDim2.new(0, RAIL_W, 0, TOP_H)
TopSep.Size             = UDim2.new(0, WIN_W - RAIL_W, 0, 1)
TopSep.ZIndex           = 5

-- ── DRAGGING (arrastar pelo TopBar) ──────────────────────────
do
...
-- ══════════════════════════════════════════════════════════════
-- NAV RAIL — barra lateral de ícones (grupos)
-- ══════════════════════════════════════════════════════════════
local NavRail = Instance.new("Frame", MainFrame)
NavRail.Name             = "NavRail"
NavRail.BackgroundColor3 = Color3.fromRGB(8, 13, 10)
NavRail.BorderSizePixel  = 0
NavRail.Visible          = false
NavRail.ZIndex           = 6
NavRail.Position         = UDim2.new(0, 0, 0, TOP_H)
NavRail.Size             = UDim2.new(0, RAIL_W, 0, CONTENT_H)

-- Fix cantos inferiores esquerdos
local NRFix = Instance.new("Frame", NavRail)
NRFix.BackgroundColor3 = Color3.fromRGB(8, 13, 10)
NRFix.Size             = UDim2.new(1, 0, 0, 20)
NRFix.Position         = UDim2.new(0, 0, 1, -20)
NRFix.BorderSizePixel  = 0
NRFix.ZIndex           = 5

-- Separador direito do rail
local RailSep = Instance.new("Frame", MainFrame)
RailSep.BackgroundColor3 = Color3.fromRGB(20, 36, 27)
RailSep.BorderSizePixel  = 0
RailSep.Position         = UDim2.new(0, RAIL_W, 0, TOP_H)
RailSep.Size             = UDim2.new(0, 1, 0, CONTENT_H)
RailSep.ZIndex           = 5

-- alias de compatibilidade
local SideBar = NavRail

-- ══════════════════════════════════════════════════════════════
-- PAINEL DE CONTEÚDO (à direita do NavRail)
-- ══════════════════════════════════════════════════════════════
...
ContentPanel.Position         = UDim2.new(0, RAIL_W + 1, 0, TOP_H + 1)
ContentPanel.Size             = UDim2.new(0, WIN_W - RAIL_W - 1, 0, CONTENT_H - 1)

-- Sub-navegação horizontal (abas da página atual)
local SubNav = Instance.new("Frame", ContentPanel)
SubNav.Name             = "SubNav"
SubNav.BackgroundColor3 = Color3.fromRGB(9, 13, 11)
SubNav.BorderSizePixel  = 0
SubNav.ZIndex           = 8
SubNav.Size             = UDim2.new(1, 0, 0, SUBNAV_H)
SubNav.Position         = UDim2.new(0, 0, 0, 0)
local snLayout = Instance.new("UIListLayout", SubNav)
snLayout.FillDirection       = Enum.FillDirection.Horizontal
snLayout.Padding             = UDim.new(0, 4)
snLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left
snLayout.VerticalAlignment   = Enum.VerticalAlignment.Center
snLayout.SortOrder           = Enum.SortOrder.LayoutOrder
local snPad = Instance.new("UIPadding", SubNav)
snPad.PaddingLeft = UDim.new(0, 8)

local SubNavSep = Instance.new("Frame", ContentPanel)
SubNavSep.BackgroundColor3 = Color3.fromRGB(20, 36, 27)
SubNavSep.BorderSizePixel  = 0
SubNavSep.ZIndex           = 6
SubNavSep.Size             = UDim2.new(1, 0, 0, 1)
SubNavSep.Position         = UDim2.new(0, 0, 0, SUBNAV_H)

-- Área de conteúdo principal (abaixo do SubNav)
local ContentArea = Instance.new("Frame", ContentPanel)
ContentArea.Name             = "ContentArea"
ContentArea.BackgroundColor3 = Color3.fromRGB(11, 16, 13)
ContentArea.BorderSizePixel  = 0
ContentArea.ZIndex           = 5
ContentArea.Position         = UDim2.new(0, 0, 0, SUBNAV_H + 1)
ContentArea.Size             = UDim2.new(1, 0, 0, CONTENT_H - SUBNAV_H - 1)

-- alias de compatibilidade
local Divider = SubNavSep
...
Footer.Name             = "Footer"
Footer.BackgroundColor3 = Color3.fromRGB(8, 12, 10)
...
local footFix = Instance.new("Frame", Footer)
footFix.BackgroundColor3 = Color3.fromRGB(8, 12, 10)
footFix.Size             = UDim2.new(1, 0, 0, 20)
footFix.Position         = UDim2.new(0, 0, 0, 0)
footFix.BorderSizePixel  = 0
footFix.ZIndex           = 5

local footSepLine = Instance.new("Frame", MainFrame)
footSepLine.BackgroundColor3 = Color3.fromRGB(20, 36, 27)
footSepLine.BorderSizePixel  = 0
footSepLine.Position         = UDim2.new(0, 0, 1, -FOOT_H - 1)
footSepLine.Size             = UDim2.new(1, 0, 0, 1)
footSepLine.ZIndex           = 5

-- Avatar iniciais
local AvatarBg = Instance.new("Frame", Footer)
AvatarBg.BackgroundColor3 = C_ACCENT
AvatarBg.Size             = UDim2.new(0, 20, 0, 20)
AvatarBg.AnchorPoint      = Vector2.new(0, 0.5)
AvatarBg.Position         = UDim2.new(0, 10, 0.5, 0)
AvatarBg.BorderSizePixel  = 0
AvatarBg.ZIndex           = 7
Instance.new("UICorner", AvatarBg).CornerRadius = UDim.new(1, 0)
local AvatarLbl = Instance.new("TextLabel", AvatarBg)
AvatarLbl.BackgroundTransparency = 1
AvatarLbl.Size      = UDim2.new(1, 0, 1, 0)
AvatarLbl.Font      = Enum.Font.GothamBold
AvatarLbl.TextSize  = 8
AvatarLbl.Text      = string.upper(string.sub(Player.DisplayName, 1, 2))
AvatarLbl.TextColor3 = Color3.fromRGB(5, 12, 8)
AvatarLbl.ZIndex    = 8
...
OnlineDot.Position         = UDim2.new(0, 33, 0.5, 0)
...
FName.Position         = UDim2.new(0, 45, 0, 0)
FName.Size             = UDim2.new(0, 130, 1, 0)
FName.Font             = Enum.Font.GothamSemibold
FName.Text             = Player.DisplayName
FName.TextColor3       = Color3.fromRGB(190, 235, 215)
FName.TextSize         = 10
...
FLangLbl.TextColor3      = Color3.fromRGB(52, 211, 153)
FLangLbl.TextSize        = 8
FLangLbl.ZIndex          = 8
langFooterLabel = FLangLbl

-- ══════════════════════════════════════════════════════════════
-- PÁGINAS (ScrollingFrames em ContentArea)
-- ══════════════════════════════════════════════════════════════
local Pages = {}
local TabConfig = {
    ...
    {key="Teleportar",     label="Teleport",      trKey="tabTeleportar"},
    {key="Esp",            label="ESP",           trKey="tabEsp"},
    {key="Bring",          label="Bring",         trKey="tabBring"},
    {key="AvancadoFarm",   label="Av.Farm",       trKey="tabAvFarm"},
    {key="Player",         label="Player",        trKey="tabPlayer"},
    {key="Configuracoes",  label="Config",        trKey="tabConfig"},
    {key="AvancadoFuncoes",label="Av.Funções",    trKey="tabAvFunc"},
}
local GroupConfig = {
    {label="GERAL",   trKey="groupGeral",   keys={"Info","Status"},                                    icon="🏠"},
    {label="COMBATE", trKey="groupCombate", keys={"Farm","Teleportar","Esp","Bring","AvancadoFarm"},    icon="⚔"},
    {label="EXTRA",   trKey="groupExtra",   keys={"Player","Configuracoes","AvancadoFuncoes"},          icon="✦"},
}
local tabLabelMap = {}
for _, t in ipairs(TabConfig) do tabLabelMap[t.key] = t end
...
local allTabs      = {}   -- compatibilidade
local subTabBtns   = {}   -- key → {pill, lbl}
local groupBtns    = {}   -- gi  → {bg, icon, lbl, bar}

-- Constrói os botões de sub-aba para o grupo selecionado
local function showGroupSubNav(groupKeys)
    for _, child in ipairs(SubNav:GetChildren()) do
    ...
    for _, key in ipairs(groupKeys) do
        lo += 1
        local tc = tabLabelMap[key]
        if tc then
            local pill = Instance.new("Frame", SubNav)
            pill.BackgroundColor3      = Color3.fromRGB(18, 32, 24)
            ...
            local plbl = Instance.new("TextLabel", pill)
            plbl.BackgroundTransparency = 1
            plbl.Font           = Enum.Font.GothamSemibold
            plbl.Text           = tc.label
            plbl.TextColor3     = C_TEXT_OFF
            plbl.TextSize       = 10
            plbl.ZIndex         = 10
            if tc.trKey then trackLabel(plbl, tc.trKey) end

            -- Mede o tamanho do texto
            local ts = game:GetService("TextService"):GetTextSize(
                tc.label, 10, Enum.Font.GothamSemibold, Vector2.new(999, 999))
            local pw = ts.X + 22
            pill.Size  = UDim2.new(0, pw, 0, SUBNAV_H - 8)
            plbl.Position = UDim2.new(0, 11, 0, 0)
            plbl.Size     = UDim2.new(0, ts.X, 1, 0)
            ...
            pbtn.Text   = ""
            pbtn.ZIndex = 11
            pbtn.MouseButton1Click:Connect(function() selectTab(key) end)

            subTabBtns[key] = {pill = pill, lbl = plbl}
        end
    end
end

-- Seleciona uma aba: exibe a página e destaca a sub-aba
selectTab = function(key)
    if currentTab == key then return end
    currentTab = key
    for _, p in pairs(Pages) do p.Visible = false end
    if Pages[key] then Pages[key].Visible = true end
    for k, entry in pairs(subTabBtns) do
        local isThis = (k == key)
        TweenService:Create(entry.pill, TweenInfo.new(0.18), {
            BackgroundTransparency = isThis and 0.6 or 1,
            BackgroundColor3       = isThis and C_BG_ACT or Color3.fromRGB(18, 32, 24),
        }):Play()
        TweenService:Create(entry.lbl, TweenInfo.new(0.18), {
            TextColor3 = isThis and C_TEXT_ON or C_TEXT_OFF
        }):Play()
    end
end

-- ── BOTÕES DE GRUPO NO NAVRAIL ────────────────────────────────
do
    local railList = Instance.new("UIListLayout", NavRail)
    ...
    local railPad = Instance.new("UIPadding", NavRail)
    railPad.PaddingTop    = UDim.new(0, 10)
    railPad.PaddingBottom = UDim.new(0, 10)

    for gi, g in ipairs(GroupConfig) do
        local gFrame = Instance.new("Frame", NavRail)
        gFrame.BackgroundTransparency = 1
        gFrame.BorderSizePixel        = 0
        gFrame.ZIndex                 = 7
        gFrame.LayoutOrder            = gi * 10
        gFrame.Size                   = UDim2.new(0, 42, 0, 56)

        -- Ícone de fundo (pill arredondado)
        local gBg = Instance.new("Frame", gFrame)
        gBg.BackgroundColor3      = C_BG_ACT
        gBg.BackgroundTransparency = 1
        gBg.BorderSizePixel       = 0
        gBg.ZIndex                = 7
        gBg.AnchorPoint           = Vector2.new(0.5, 0)
        gBg.Position              = UDim2.new(0.5, 0, 0, 0)
        gBg.Size                  = UDim2.new(0, 40, 0, 40)
        Instance.new("UICorner", gBg).CornerRadius = UDim.new(0, 14)

        -- Emoji ícone
        local gIcon = Instance.new("TextLabel", gBg)
        gIcon.BackgroundTransparency = 1
        gIcon.Size        = UDim2.new(1, 0, 0, 28)
        gIcon.Position    = UDim2.new(0, 0, 0, 5)
        gIcon.Font        = Enum.Font.GothamBlack
        gIcon.Text        = g.icon
        gIcon.TextSize    = 20
        gIcon.TextColor3  = C_ICON_IDLE
        gIcon.ZIndex      = 8

        -- Rótulo de texto do grupo
        local gLabel = Instance.new("TextLabel", gFrame)
        gLabel.BackgroundTransparency = 1
        gLabel.Size           = UDim2.new(1, 0, 0, 14)
        gLabel.Position       = UDim2.new(0, 0, 0, 42)
        gLabel.Font           = Enum.Font.GothamBold
        gLabel.Text           = g.label
        gLabel.TextColor3     = Color3.fromRGB(45, 72, 58)
        gLabel.TextSize       = 6
        gLabel.ZIndex         = 8
        if g.trKey then trackLabel(gLabel, g.trKey) end

        -- Barra de indicador ativo (lado direito)
        local gBar = Instance.new("Frame", gBg)
        gBar.BackgroundColor3 = C_ACCENT
        gBar.BorderSizePixel  = 0
        gBar.ZIndex           = 9
        gBar.Visible          = false
        gBar.Position         = UDim2.new(1, -3, 0.15, 0)
        gBar.Size             = UDim2.new(0, 3, 0.7, 0)
        Instance.new("UICorner", gBar).CornerRadius = UDim.new(0, 2)

        -- Botão de clique
        local gbtn = Instance.new("TextButton", gFrame)
        gbtn.BackgroundTransparency = 1
        gbtn.Size   = UDim2.new(1, 0, 1, 0)
        gbtn.Text   = ""
        gbtn.ZIndex = 10
        gbtn.MouseEnter:Connect(function()
            if currentGroup ~= gi then
                TweenService:Create(gBg, TweenInfo.new(0.15), {BackgroundTransparency=0.75}):Play()
            end
        end)
        gbtn.MouseLeave:Connect(function()
            if currentGroup ~= gi then
                TweenService:Create(gBg, TweenInfo.new(0.15), {BackgroundTransparency=1}):Play()
            end
        end)
        gbtn.MouseButton1Click:Connect(function()
            if currentGroup == gi then return end
            -- Desativar grupo anterior
            if currentGroup and groupBtns[currentGroup] then
                local old = groupBtns[currentGroup]
                TweenService:Create(old.bg,   TweenInfo.new(0.2), {BackgroundTransparency=1}):Play()
                TweenService:Create(old.icon, TweenInfo.new(0.2), {TextColor3=C_ICON_IDLE}):Play()
                TweenService:Create(old.lbl,  TweenInfo.new(0.2), {TextColor3=Color3.fromRGB(45,72,58)}):Play()
                old.bar.Visible = false
            end
            currentGroup = gi
            TweenService:Create(gBg,   TweenInfo.new(0.2), {BackgroundTransparency=0.6}):Play()
            TweenService:Create(gIcon, TweenInfo.new(0.2), {TextColor3=C_ICON_ACTIVE}):Play()
            TweenService:Create(gLabel,TweenInfo.new(0.2), {TextColor3=C_ACCENT}):Play()
            gBar.Visible = true
            showGroupSubNav(g.keys)
            if g.keys[1] then selectTab(g.keys[1]) end
        end)
        groupBtns[gi] = {bg=gBg, icon=gIcon, lbl=gLabel, bar=gBar, keys=g.keys}
    end
end

-- ══════════════════════════════════════════════════════════════
-- INTRO ANIMATION — "PudimHub"
-- ══════════════════════════════════════════════════════════════
do
    -- Fundo escuro total
    local IntroScreen = Instance.new("Frame", ScreenGui)
    IntroScreen.Name             = "IntroScreen"
    IntroScreen.Size             = UDim2.new(1, 0, 1, 0)
    IntroScreen.BackgroundColor3 = Color3.fromRGB(4, 7, 5)
    IntroScreen.BorderSizePixel  = 0
    IntroScreen.ZIndex           = 900

    -- Partículas decorativas de fundo
    for i = 1, 16 do
        local dot = Instance.new("Frame", IntroScreen)
        ...
        dot.BackgroundTransparency = 0.6 + math.random() * 0.38
        dot.BorderSizePixel   = 0
        dot.ZIndex            = 901
        Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
        -- Pulsar as partículas
        local function pulseDot()
            local t = TweenInfo.new(1.2 + math.random() * 0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true)
            TweenService:Create(dot, t, {BackgroundTransparency=0.85}):Play()
        end
        task.delay(math.random() * 0.5, pulseDot)
    end

    -- Container das letras (centralizado horizontalmente)
    local letterRow = Instance.new("Frame", IntroScreen)
    letterRow.BackgroundTransparency = 1
    letterRow.BorderSizePixel        = 0
    letterRow.AnchorPoint            = Vector2.new(0.5, 0.5)
    letterRow.Position               = UDim2.new(0.5, 0, 0.43, 0)
    letterRow.Size                   = UDim2.new(0, 352, 0, 72)
    letterRow.ZIndex                 = 902

    -- 8 letras: P U D I M H U B
    local LETTERS = {"P","U","D","I","M","H","U","B"}
    local letterEntries = {}
    for i, ch in ipairs(LETTERS) do
        local lf = Instance.new("TextLabel", letterRow)
        lf.BackgroundTransparency = 1
        lf.Size         = UDim2.new(0, 40, 0, 72)
        lf.Position     = UDim2.new(0, (i - 1) * 44, 0, 0)
        lf.Font         = Enum.Font.GothamBlack
        lf.Text         = ch
        lf.TextSize     = 54
        lf.TextColor3   = Color3.fromRGB(52, 211, 153)
        lf.TextTransparency = 1
        lf.ZIndex       = 903
        local ls = Instance.new("UIStroke", lf)
        ls.Color        = Color3.fromRGB(20, 160, 90)
        ls.Thickness    = 1.8
        ls.Transparency = 1
        table.insert(letterEntries, {lbl = lf, stroke = ls})
    end

    -- Subtítulo abaixo das letras
    local subLbl = Instance.new("TextLabel", IntroScreen)
    subLbl.BackgroundTransparency = 1
    subLbl.Size           = UDim2.new(0, 520, 0, 26)
    subLbl.AnchorPoint    = Vector2.new(0.5, 0)
    subLbl.Position       = UDim2.new(0.5, 0, 0.595, 0)
    subLbl.Font           = Enum.Font.GothamSemibold
    subLbl.Text           = "—  99 DIAS NA FLORESTA  —"
    subLbl.TextColor3     = Color3.fromRGB(251, 191, 36)
    subLbl.TextSize       = 13
    subLbl.TextTransparency = 1
    subLbl.ZIndex         = 902

    -- Barra de progresso
    local barBg = Instance.new("Frame", IntroScreen)
    barBg.BackgroundColor3    = Color3.fromRGB(18, 32, 24)
    barBg.BackgroundTransparency = 0.5
    barBg.BorderSizePixel     = 0
    barBg.Size                = UDim2.new(0, 320, 0, 3)
    ...
    -- ── SPLASH CARD ──────────────────────────────────────────────
    local SplashCard = Instance.new("Frame", ScreenGui)
    SplashCard.Name             = "SplashCard"
    SplashCard.Visible          = false
    SplashCard.ZIndex           = 950
    SplashCard.Size             = UDim2.new(0, 440, 0, 230)
    SplashCard.AnchorPoint      = Vector2.new(0.5, 0.5)
    SplashCard.Position         = UDim2.new(0.5, 0, 0.5, 30)
    SplashCard.BackgroundColor3 = Color3.fromRGB(10, 16, 13)
    SplashCard.BackgroundTransparency = 1
    SplashCard.BorderSizePixel  = 0
    Instance.new("UICorner", SplashCard).CornerRadius = UDim.new(0, 22)
    local scStroke = Instance.new("UIStroke", SplashCard)
    scStroke.Color       = Color3.fromRGB(52, 211, 153)
    scStroke.Thickness   = 1.8
    scStroke.Transparency = 0.35

    -- Linha decorativa no topo do card
    local scTopLine = Instance.new("Frame", SplashCard)
    scTopLine.BackgroundColor3 = Color3.fromRGB(52, 211, 153)
    scTopLine.BorderSizePixel  = 0
    scTopLine.Size             = UDim2.new(0.5, 0, 0, 2)
    scTopLine.AnchorPoint      = Vector2.new(0.5, 0)
    scTopLine.Position         = UDim2.new(0.5, 0, 0, 0)
    scTopLine.ZIndex           = 951
    Instance.new("UICorner", scTopLine).CornerRadius = UDim.new(1, 0)

    local scIcon = Instance.new("TextLabel", SplashCard)
    scIcon.BackgroundTransparency = 1
    scIcon.Size     = UDim2.new(1, 0, 0, 52)
    scIcon.Position = UDim2.new(0, 0, 0, 16)
    scIcon.Font     = Enum.Font.GothamBlack
    scIcon.Text     = "🌿"
    scIcon.TextSize = 38
    scIcon.TextTransparency = 1
    scIcon.ZIndex   = 951

    local scName = Instance.new("TextLabel", SplashCard)
    scName.BackgroundTransparency = 1
    scName.Size         = UDim2.new(1, 0, 0, 40)
    scName.Position     = UDim2.new(0, 0, 0, 66)
    scName.Font         = Enum.Font.GothamBlack
    scName.Text         = "PudimHub"
    scName.TextColor3   = Color3.fromRGB(52, 211, 153)
    scName.TextSize     = 32
    scName.TextTransparency = 1
    scName.ZIndex       = 951

    local scGame = Instance.new("TextLabel", SplashCard)
    scGame.BackgroundTransparency = 1
    scGame.Size       = UDim2.new(1, 0, 0, 22)
    scGame.Position   = UDim2.new(0, 0, 0, 108)
    scGame.Font       = Enum.Font.GothamSemibold
    scGame.Text       = "99 Dias na Floresta  ·  2026"
    scGame.TextColor3 = Color3.fromRGB(251, 191, 36)
    scGame.TextSize   = 14
    scGame.TextTransparency = 1
    scGame.ZIndex     = 951

    -- Data e hora de atualização
    local _n = os.date("*t")
    local _days = {"Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado"}
    local _ds = string.format(
        "Atualizado em %02d/%02d/%04d  às  %02d:%02d  —  %s",
        _n.day, _n.month, _n.year, _n.hour, _n.min, _days[_n.wday])
    local scDate = Instance.new("TextLabel", SplashCard)
    scDate.BackgroundTransparency = 1
    scDate.Size       = UDim2.new(1, -40, 0, 18)
    scDate.Position   = UDim2.new(0, 20, 0, 140)
    scDate.Font       = Enum.Font.Gotham
    scDate.Text       = _ds
    scDate.TextColor3 = Color3.fromRGB(80, 135, 105)
    scDate.TextSize   = 11
    scDate.TextTransparency = 1
    scDate.ZIndex     = 951

    local scVerBg = Instance.new("Frame", SplashCard)
    scVerBg.BackgroundColor3    = Color3.fromRGB(18, 36, 27)
    scVerBg.BackgroundTransparency = 1
    scVerBg.BorderSizePixel     = 0
    scVerBg.Size                = UDim2.new(0, 96, 0, 24)
    scVerBg.AnchorPoint         = Vector2.new(0.5, 0)
    scVerBg.Position            = UDim2.new(0.5, 0, 0, 170)
    scVerBg.ZIndex              = 951
    Instance.new("UICorner", scVerBg).CornerRadius = UDim.new(0, 10)
    local scVerLbl = Instance.new("TextLabel", scVerBg)
    scVerLbl.BackgroundTransparency = 1
    scVerLbl.Size       = UDim2.new(1, 0, 1, 0)
    scVerLbl.Font       = Enum.Font.GothamBold
    scVerLbl.Text       = "v5  ·  2026"
    scVerLbl.TextColor3 = Color3.fromRGB(52, 211, 153)
    scVerLbl.TextSize   = 11
    scVerLbl.TextTransparency = 1
    scVerLbl.ZIndex     = 952

    -- ── SEQUÊNCIA DE ANIMAÇÃO ────────────────────────────────────
    task.spawn(function()
        -- Fase 1: Letras aparecem uma a uma (0.0 – ~1.0s)
        for i, entry in ipairs(letterEntries) do
            task.wait(0.085)
            TweenService:Create(entry.lbl, TweenInfo.new(0.30, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                TextTransparency = 0
            }):Play()
            TweenService:Create(entry.stroke, TweenInfo.new(0.22), {Transparency = 0.45}):Play()
        end
        task.wait(0.28)

        -- Fase 2: Subtítulo aparece (~1.1s)
        TweenService:Create(subLbl, TweenInfo.new(0.45, Enum.EasingStyle.Quad), {TextTransparency=0}):Play()
        task.wait(0.25)

        -- Fase 3: Barra de progresso (~1.3s – 2.1s)
        TweenService:Create(barFill, TweenInfo.new(0.88, Enum.EasingStyle.Quad), {Size=UDim2.new(1,0,1,0)}):Play()
        task.wait(0.95)

        -- Fade out intro (~2.3s total)
        TweenService:Create(IntroScreen, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {BackgroundTransparency=1}):Play()
        for _, e in ipairs(letterEntries) do
            TweenService:Create(e.lbl,    TweenInfo.new(0.22), {TextTransparency=1}):Play()
            TweenService:Create(e.stroke, TweenInfo.new(0.22), {Transparency=1}):Play()
        end
        TweenService:Create(subLbl,   TweenInfo.new(0.22), {TextTransparency=1}):Play()
        TweenService:Create(barBg,    TweenInfo.new(0.22), {BackgroundTransparency=1}):Play()
        TweenService:Create(barFill,  TweenInfo.new(0.22), {BackgroundTransparency=1}):Play()
        task.wait(0.32)
        IntroScreen.Visible = false

        -- Fase 4: Splash card sobe e aparece (~2.6s)
        SplashCard.Visible = true
        SplashCard.BackgroundTransparency = 1
        SplashCard.Position = UDim2.new(0.5, 0, 0.5, 40)
        TweenService:Create(SplashCard, TweenInfo.new(0.40, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            BackgroundTransparency = 0,
            Position               = UDim2.new(0.5, 0, 0.5, 0),
        }):Play()
        task.wait(0.18)
        for _, item in ipairs({scIcon, scName, scGame, scDate}) do
            TweenService:Create(item, TweenInfo.new(0.32, Enum.EasingStyle.Quad), {TextTransparency=0}):Play()
            task.wait(0.07)
        end
        TweenService:Create(scVerBg,  TweenInfo.new(0.28), {BackgroundTransparency=0}):Play()
        TweenService:Create(scVerLbl, TweenInfo.new(0.28), {TextTransparency=0}):Play()

        task.wait(2.1)

        -- Fase 5: Splash some, janela principal aparece
        TweenService:Create(SplashCard, TweenInfo.new(0.30, Enum.EasingStyle.Quad), {
            BackgroundTransparency = 1,
            Position               = UDim2.new(0.5, 0, 0.5, -28),
        }):Play()
        for _, item in ipairs({scIcon, scName, scGame, scDate, scVerLbl}) do
            TweenService:Create(item, TweenInfo.new(0.22), {TextTransparency=1}):Play()
        end
        TweenService:Create(scVerBg, TweenInfo.new(0.22), {BackgroundTransparency=1}):Play()
        task.wait(0.33)
        SplashCard.Visible = false

        -- Fase 6: MainFrame expande para cima (~5.0s)
        ...
        -- Fase 7: Ativa o primeiro grupo automaticamente
        task.wait(0.08)
        local g1 = groupBtns[1]
        if g1 then
            currentGroup = 1
            g1.bg.BackgroundTransparency = 0.6
            g1.icon.TextColor3           = C_ICON_ACTIVE
            g1.lbl.TextColor3            = C_ACCENT
            g1.bar.Visible               = true
            showGroupSubNav(GroupConfig[1].keys)
            selectTab("Info")
        end
    end)
end  -- fim do bloco de intro
...
BoostPopup.BackgroundColor3=Color3.fromRGB(11, 17, 14); BoostPopup.Size=UDim2.new(0,190,0,0)
...
local function makePopupToggle(text,callback)
    local row=Instance.new("Frame",BoostPopup); row.BackgroundColor3=Color3.fromRGB(20, 32, 26)
    ...
    lbl.Text=text; lbl.TextColor3=Color3.fromRGB(170, 230, 200); lbl.TextSize=10
    ...
    local knob=Instance.new("Frame",pill); knob.BackgroundColor3=Color3.fromRGB(80, 120, 98); knob.BorderSizePixel=0
...
FloatBtn.BackgroundColor3=Color3.fromRGB(8, 13, 10); FloatBtn.BorderSizePixel=0; FloatBtn.Visible=false; FloatBtn.ZIndex=100; FloatBtn.Active=true
...
PDText.Text="🌿"; PDText.TextColor3=Color3.fromRGB(180, 250, 220); PDText.TextSize=20; PDText.TextTransparency=0.05; PDText.ZIndex=105
...
FloatClick.MouseButton1Click:Connect(function()
    ...
    task.delay(0.22,function()
        ...
        task.delay(0.2,function() NavRail.Visible=true; ContentPanel.Visible=true; Footer.Visible=true end)
    end)
end)
...
TopBtns["Minimize"].MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        Footer.Visible=false
        NavRail.Visible=false; ContentPanel.Visible=false
        ...
        task.delay(0.2,function()
            NavRail.Visible=true; ContentPanel.Visible=true; Footer.Visible=true
        end)
    end
end)
...
TopBtns["Maximize"].MouseButton1Click:Connect(function()
    ...
    if isMaximized then
        ...
        TweenService:Create(MainFrame,TweenInfo.new(0.3,Enum.EasingStyle.Quad),{Size=UDim2.new(0,820,0,560),Position=UDim2.new(0.5,-410,0.5,-280)}):Play()
    else
