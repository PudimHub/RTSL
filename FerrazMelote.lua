...
local function pibLancarDoSky(encontrados, campPos)
    ...
    for i, entry in ipairs(encontrados) do
        pcall(function()
            ...
            part.Velocity    = Vector3.new((math.random()-0.5)*4, 0, (math.random()-0.5)*4)
            part.RotVelocity = Vector3.new((math.random()-0.5)*2, (math.random()-0.5)*2, (math.random()-0.5)*2)
...
-- ──────────────────────────────────────────────
local _campfirePosCache = nil   -- posição cacheada
...
local function autoTriturador()
    ...
    for i, entry in ipairs(encontrados) do
        pcall(function()
            ...
            part.CanCollide  = true
            part.CFrame      = CFrame.new(skyPos)
            part.Velocity    = Vector3.zero   -- sem impulso lateral → cai reto
