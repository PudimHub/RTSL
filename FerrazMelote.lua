...
local function buildCache()
    ...
    task.spawn(function()
        ...
        for _,obj in ipairs(allObjs) do
            ...
            if not obj or not obj.Parent then goto _c1 end
...
local ESP_GROUPS = {
    ...
    {key="espGroupEquipment", cor=Color3.fromRGB(255,200,55), icon="⚔️", title="Equipamentos",keys={"EspFerr","EspArmas","EspAmmo","EspCura","EspChaves","EspBigorna","EspBlueprint"}},
}

for _,grp in ipairs(ESP_GROUPS) do
    -- Conta quantas cats válidas tem neste grupo
    local validCats = {}
    for _,k in ipairs(grp.keys) do if espCatMap[k] then table.insert(validCats, espCatMap[k]) end end
    if #validCats > 0 then

    local itemH = 40
    local cH = 36 + 9 + #validCats * itemH + 8
    ...
    for i, cat in ipairs(validCats) do
        -- Row compacta: ícone + nome + toggle
        local row = Instance.new("Frame",_egCF)
        ...
        -- Divisor entre itens
        if i < #validCats then
            local div=Instance.new("Frame",_egCF); div.BackgroundColor3=grp.cor
            div.BackgroundTransparency=0.85; div.BorderSizePixel=0
            div.Position=UDim2.new(0,44,0,_ey+itemH-3); div.Size=UDim2.new(1,-50,0,1); div.ZIndex=7
        end
        ...
        btn.MouseButton1Click:Connect(function()
            state=not state; espAtivo[cat.key]=state; lastCache=0
            TweenService:Create(pill,TweenInfo.new(0.18),{BackgroundColor3=state and cat.cor or Color3.fromRGB(65,72,90)}):Play()
            TweenService:Create(knob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
                Position=state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9),
                BackgroundColor3=state and Color3.fromRGB(255,255,255) or Color3.fromRGB(130,90,30),
            }):Play()
            ck.Text=state and "✓" or "✕"; ck.TextColor3=state and cat.cor or Color3.fromRGB(65,72,90)
...
local function bpScanBuildings(isRefresh)
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            if not obj:IsA("Model") then goto _c2 end
            local nm = obj.Name:lower()
            ...
            or nm:find("snare",1,true) or nm:find("item",1,true) then goto _c14 end
            -- Filtra NPCs/mobs
            if obj:FindFirstChildWhichIsA("Humanoid") then goto _c3 end
            -- É construção?
            if not bpIsBuilding(nm) then goto _c4 end
            ...
            if not part or not bigEnough then goto _c5 end
            if part.Position.Y < -500 then goto _c6 end
            local pk = bpPosKey(part.Position)
            if buildSeenKeys[pk] then goto _c7 end
...
local function cpScanChests(isRefresh)
    ...
    task.spawn(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            batch += 1
            if batch % 500 == 0 then task.wait() end
            if not obj:IsA("Model") then goto _c8 end
            local nm = obj.Name:lower()
            if not (nm:find("chest",1,true) or nm:find("bau",1,true) or nm:find("baú",1,true)) then goto _c9 end
            ...
            if not part then goto _c10 end
            if part.Size.X>14 or part.Size.Y>14 or part.Size.Z>14 then goto _c11 end
            if part.Position.Y < -400 then goto _c12 end
            local pk = cpPosKey(part.Position)
            if chestSeenKeys2[pk] then goto _c13 end
