...
-- ══════════════════════════════════════════════════════════════════════
-- PUDIM HUB — SPLASH SCREEN v1 (Estilo Brawl Stars / Cartoon)
-- ══════════════════════════════════════════════════════════════════════
local function _PudimSplash()
    ...
    OV.BackgroundColor3    = Color3.fromRGB(8, 6, 18)
    OV.BackgroundTransparency = 0
    OV.BorderSizePixel     = 0
    OV.ZIndex              = 1

    -- Gradiente radial no overlay
    ...
    -- Partículas decorativas (estrelinhas de fundo)
    for i = 1, 18 do
        local dot = Instance.new("TextLabel", SG)
        dot.BackgroundTransparency = 1
        dot.Text      = math.random(1,3)==1 and "★" or "✦"
        dot.TextSize  = math.random(8,22)
        dot.TextColor3= Color3.fromRGB(255, math.random(170,230), 0)
        dot.TextTransparency = math.random(3,8)/10
        dot.Font      = Enum.Font.GothamBlack
        dot.Position  = UDim2.new(math.random(0,95)/100, 0, math.random(0,95)/100, 0)
        dot.Size      = UDim2.new(0,30,0,30)
        dot.ZIndex    = 2
        -- pulsar lento
        local _pT = TweenInfo.new(math.random(8,18)/10, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true)
        TS:Create(dot, _pT, {TextTransparency = math.random(0,3)/10}):Play()
    end

    -- ── CONTAINER PRINCIPAL do Banner ────────────────────────────────
    local BC = Instance.new("Frame", SG)
    BC.Size              = UDim2.new(0, 540, 0, 175)
    BC.Position          = UDim2.new(0.5, -270, -0.6, 0)  -- começa fora de tela (cima)
    BC.BackgroundTransparency = 1
    BC.BorderSizePixel   = 0
    BC.ZIndex            = 5

    -- Sombra do banner (offset + rotação)
    local SH = Instance.new("Frame", BC)
    SH.Size              = UDim2.new(1, 16, 1, 16)
    SH.Position          = UDim2.new(0, 10, 0, 10)
    SH.BackgroundColor3  = Color3.fromRGB(0,0,0)
    SH.BackgroundTransparency = 0.25
    SH.BorderSizePixel   = 0
    SH.Rotation          = -4
    SH.ZIndex            = 4
    Instance.new("UICorner", SH).CornerRadius = UDim.new(0,18)

    -- Banner principal (amarelo Brawl Stars)
    local BN = Instance.new("Frame", BC)
    BN.Size              = UDim2.fromScale(1,1)
    BN.Position          = UDim2.new(0,0,0,0)
    BN.BackgroundColor3  = Color3.fromRGB(252, 198, 0)
    BN.BorderSizePixel   = 0
    BN.Rotation          = -4
    BN.ZIndex            = 5
    Instance.new("UICorner", BN).CornerRadius = UDim.new(0,18)

    -- Contorno preto grosso (estilo cartoon)
    local BNS = Instance.new("UIStroke", BN)
    BNS.Color            = Color3.fromRGB(25, 15, 0)
    BNS.Thickness        = 6
    ...
    BNG.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0,  Color3.fromRGB(255, 235, 70)),
        ColorSequenceKeypoint.new(0.45,Color3.fromRGB(252, 198, 0)),
        ColorSequenceKeypoint.new(1,  Color3.fromRGB(190, 130, 0)),
    })
    BNG.Rotation = 100

    -- Brilho no canto superior esquerdo (efeito cartoon shine)
    local SHINE = Instance.new("Frame", BN)
    SHINE.Size           = UDim2.new(0, 80, 0, 30)
    ...
    -- PUDIM — texto grande
    local TL1 = Instance.new("TextLabel", BN)
    TL1.Size             = UDim2.new(1,-20,0,90)
    TL1.Position         = UDim2.new(0,10,0,4)
    TL1.BackgroundTransparency = 1
    TL1.Text             = "PUDIM"
    TL1.TextColor3       = Color3.fromRGB(255,255,255)
    TL1.Font             = Enum.Font.GothamBlack
    TL1.TextSize         = 78
    TL1.TextXAlignment   = Enum.TextXAlignment.Center
    TL1.ZIndex           = 7
    local TL1S = Instance.new("UIStroke", TL1)
    TL1S.Color           = Color3.fromRGB(20,10,0)
    TL1S.Thickness       = 5

    -- HUB — texto menor e escuro
    local TL2 = Instance.new("TextLabel", BN)
    TL2.Size             = UDim2.new(1,-20,0,55)
    TL2.Position         = UDim2.new(0,10,0,88)
    TL2.BackgroundTransparency = 1
    TL2.Text             = "HUB"
    TL2.TextColor3       = Color3.fromRGB(35, 18, 0)
    TL2.Font             = Enum.Font.GothamBlack
    TL2.TextSize         = 44
    TL2.TextXAlignment   = Enum.TextXAlignment.Center
    TL2.ZIndex           = 7
    local TL2S = Instance.new("UIStroke", TL2)
    TL2S.Color           = Color3.fromRGB(0,0,0)
    TL2S.Thickness       = 2

    -- Estrelas decorativas no banner
    local _stars = {
        {0.03,0.06, 26,  12}, {0.87,0.04, 22, -10},
        {0.91,0.72, 18,  18}, {0.02,0.75, 20, -14},
    }
    for _, s in ipairs(_stars) do
        local st = Instance.new("TextLabel", BN)
        st.Size            = UDim2.new(0,s[3],0,s[3])
        st.Position        = UDim2.new(s[1],0,s[2],0)
        st.BackgroundTransparency = 1
        st.Text            = "★"
        st.TextSize        = s[3]
        st.TextColor3      = Color3.fromRGB(255,255,190)
        st.Font            = Enum.Font.GothamBlack
        st.Rotation        = s[4]
        st.ZIndex          = 8
        local stS = Instance.new("UIStroke", st)
        stS.Color          = Color3.fromRGB(120,70,0)
        stS.Thickness      = 2
        -- Piscar suave
        TS:Create(st, TweenInfo.new(0.7,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
            {TextTransparency=0.5}):Play()
    end

    -- Ícone de pudim acima do banner
    local ICO = Instance.new("TextLabel", SG)
    ICO.Size             = UDim2.new(0,80,0,80)
    ICO.Position         = UDim2.new(0.5,-40, 0.5,-195)
    ...
    ICO.ZIndex           = 6
    -- Bobbing lento
    TS:Create(ICO, TweenInfo.new(1.2,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
        {Position=UDim2.new(0.5,-40,0.5,-205)}):Play()

    -- Subtítulo abaixo
    local SUB = Instance.new("TextLabel", SG)
    SUB.Size             = UDim2.new(0,540,0,30)
    SUB.Position         = UDim2.new(0.5,-270,0.5, 90)
    SUB.BackgroundTransparency = 1
    SUB.Text             = "99 Dias na Floresta  •  v5"
    SUB.TextColor3       = Color3.fromRGB(210,230,255)
    ...
    -- Dots de loading
    local DOT = Instance.new("TextLabel", SG)
    DOT.Size             = UDim2.new(0,540,0,24)
    DOT.Position         = UDim2.new(0.5,-270,0.5,125)
    DOT.BackgroundTransparency = 1
    DOT.Text             = "● ● ●"
    ...
    -- ── ANIMAÇÃO DE ENTRADA ─────────────────────────────────────────
    TS:Create(BC, TweenInfo.new(0.65,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Position=UDim2.new(0.5,-270,0.5,-100)}):Play()

    task.delay(0.35, function()
        TS:Create(ICO,  TweenInfo.new(0.4,Enum.EasingStyle.Quad), {TextTransparency=0}):Play()
        TS:Create(SUB,  TweenInfo.new(0.4,Enum.EasingStyle.Quad), {TextTransparency=0}):Play()
    end)
    task.delay(0.55, function()
        TS:Create(DOT,  TweenInfo.new(0.4,Enum.EasingStyle.Quad), {TextTransparency=0}):Play()
    end)
    ...
    -- ── SAÍDA DO SPLASH (após 2.6s) ────────────────────────────────
    task.delay(2.6, function()
        _dotAlive = false

        -- Banner sai voando para baixo
        TS:Create(BC,  TweenInfo.new(0.45,Enum.EasingStyle.Back,Enum.EasingDirection.In),
            {Position=UDim2.new(0.5,-270,1.6,0)}):Play()
        TS:Create(OV,  TweenInfo.new(0.5,Enum.EasingStyle.Quad), {BackgroundTransparency=1}):Play()
        TS:Create(SUB, TweenInfo.new(0.25,Enum.EasingStyle.Quad),{TextTransparency=1}):Play()
        TS:Create(DOT, TweenInfo.new(0.25,Enum.EasingStyle.Quad),{TextTransparency=1}):Play()
        TS:Create(ICO, TweenInfo.new(0.25,Enum.EasingStyle.Quad),{TextTransparency=1}):Play()

        -- ── JANELA DE INFO ──────────────────────────────────────────
        task.delay(0.5, function()
            if not (SG and SG.Parent) then return end
            OV:Destroy(); BC:Destroy(); SUB:Destroy(); DOT:Destroy(); ICO:Destroy()

            -- Fundo semi-transparente
            local IBKG = Instance.new("Frame", SG)
            IBKG.Size              = UDim2.fromScale(1,1)
            IBKG.BackgroundColor3  = Color3.fromRGB(0,0,0)
            IBKG.BackgroundTransparency = 0.45
            IBKG.BorderSizePixel   = 0
            IBKG.ZIndex            = 1
            TS:Create(IBKG, TweenInfo.new(0.4,Enum.EasingStyle.Quad),
                {BackgroundTransparency=0.45}):Play()

            -- Card principal
            local CARD = Instance.new("Frame", SG)
            CARD.Size              = UDim2.new(0,450,0,285)
            CARD.Position          = UDim2.new(0.5,-225,1.6,0) -- começa abaixo
            CARD.BackgroundColor3  = Color3.fromRGB(18,12,32)
            CARD.BorderSizePixel   = 0
            CARD.ZIndex            = 3
            Instance.new("UICorner", CARD).CornerRadius = UDim.new(0,22)

            -- Stroke amarelo no card
            local CARDS = Instance.new("UIStroke", CARD)
            CARDS.Color            = Color3.fromRGB(255,198,0)
            CARDS.Thickness        = 4
            ...
            CARDG.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(38,25,65)),
            ...
            TOPB.Size              = UDim2.new(1,0,0,9)
            TOPB.BackgroundColor3  = Color3.fromRGB(255,198,0)
            TOPB.BorderSizePixel   = 0
            TOPB.ZIndex            = 4
            Instance.new("UICorner", TOPB).CornerRadius = UDim.new(0,4)

            -- Gradiente na barra topo
            ...
            CICO.Size              = UDim2.new(0,66,0,66)
            CICO.Position          = UDim2.new(0.5,-33,0,16)
            ...
            TS:Create(CICO, TweenInfo.new(1.0,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
                {Position=UDim2.new(0.5,-33,0,10)}):Play()

            -- Título: "PudimHub"
            local CTIT = Instance.new("TextLabel", CARD)
            CTIT.Size              = UDim2.new(1,-20,0,40)
            CTIT.Position          = UDim2.new(0,10,0,80)
            CTIT.BackgroundTransparency = 1
            CTIT.Text              = "PudimHub"
            CTIT.TextColor3        = Color3.fromRGB(255,198,0)
            CTIT.Font              = Enum.Font.GothamBlack
            CTIT.TextSize          = 32
            CTIT.TextXAlignment    = Enum.TextXAlignment.Center
            CTIT.ZIndex            = 5
            local CTITS = Instance.new("UIStroke", CTIT)
            CTITS.Color            = Color3.fromRGB(80,45,0)
            CTITS.Thickness        = 2.5

            -- Divisória
            local DIV = Instance.new("Frame", CARD)
            DIV.Size               = UDim2.new(0.78,0,0,2)
            DIV.Position           = UDim2.new(0.11,0,0,124)
            DIV.BackgroundColor3   = Color3.fromRGB(255,198,0)
            ...
            CGAME.Size             = UDim2.new(1,-20,0,30)
            CGAME.Position         = UDim2.new(0,10,0,130)
            CGAME.BackgroundTransparency = 1
            CGAME.Text             = "99 Dias na Floresta"
            CGAME.TextColor3       = Color3.fromRGB(170,220,255)
            ...
            CUPD.Size              = UDim2.new(1,-20,0,22)
            CUPD.Position          = UDim2.new(0,10,0,164)
            CUPD.BackgroundTransparency = 1
            CUPD.Text              = "🔄  Última Atualização"
            CUPD.TextColor3        = Color3.fromRGB(140,140,175)
            ...
            CDATE.Size             = UDim2.new(1,-20,0,28)
            CDATE.Position         = UDim2.new(0,10,0,186)
            CDATE.BackgroundTransparency = 1
            CDATE.Text             = _dateStr
            CDATE.TextColor3       = Color3.fromRGB(255,220,60)
            CDATE.Font             = Enum.Font.GothamBold
            CDATE.TextSize         = 15
            CDATE.TextXAlignment   = Enum.TextXAlignment.Center
            CDATE.ZIndex           = 5
            -- Glow pulsante na data
            TS:Create(CDATE, TweenInfo.new(1.1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
                {TextColor3=Color3.fromRGB(255,240,120)}):Play()

            -- Botão JOGAR (cartoon)
            local CBTN = Instance.new("TextButton", CARD)
            CBTN.Size              = UDim2.new(0,170,0,44)
            CBTN.Position          = UDim2.new(0.5,-85,0,226)
            CBTN.BackgroundColor3  = Color3.fromRGB(255,198,0)
            CBTN.BorderSizePixel   = 0
            CBTN.Text              = "✓   JOGAR"
            CBTN.TextColor3        = Color3.fromRGB(25,12,0)
            CBTN.Font              = Enum.Font.GothamBlack
            CBTN.TextSize          = 19
            CBTN.ZIndex            = 6
            Instance.new("UICorner", CBTN).CornerRadius = UDim.new(0,11)
            local CBTNS = Instance.new("UIStroke", CBTN)
            CBTNS.Color            = Color3.fromRGB(20,10,0)
            CBTNS.Thickness        = 3.5
            -- Gradiente no botão
            ...
            CBTNGG.Color = ColorSequence.new({
                ...
                ColorSequenceKeypoint.new(1,Color3.fromRGB(210,145,0)),
            })
            CBTNGG.Rotation = 90

            -- Hover no botão
            CBTN.MouseEnter:Connect(function()
                TS:Create(CBTN, TweenInfo.new(0.12),{
                    Size     = UDim2.new(0,178,0,48),
                    Position = UDim2.new(0.5,-89,0,222),
                }):Play()
            end)
            CBTN.MouseLeave:Connect(function()
                TS:Create(CBTN, TweenInfo.new(0.12),{
                    Size     = UDim2.new(0,170,0,44),
                    Position = UDim2.new(0.5,-85,0,226),
                }):Play()
            end)

            -- Fechar ao clicar
            local function _closeInfo()
                if not (CARD and CARD.Parent) then return end
                TS:Create(CARD, TweenInfo.new(0.42,Enum.EasingStyle.Back,Enum.EasingDirection.In),
                    {Position=UDim2.new(0.5,-225,1.6,0)}):Play()
            ...
            CBTN.MouseButton1Click:Connect(_closeInfo)
            -- Auto-fechar após 7 segundos
            task.delay(7, function()
                pcall(_closeInfo)
            end)

            -- Bounce de entrada do card
            TS:Create(CARD, TweenInfo.new(0.6,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
                {Position=UDim2.new(0.5,-225,0.5,-142)}):Play()
        end)
    end)
end
task.spawn(_PudimSplash)
-- ══════════════════════════════════════════════════════════════════════
...
local function stopAutoExplore(tpBack)
    ...
    pcall(function()
        if aePill   then TweenService:Create(aePill,  TweenInfo.new(0.22), {BackgroundColor3=Color3.fromRGB(45,50,62)}):Play() end
...
aeStroke = Instance.new("UIStroke",aeCard); aeStroke.Color = Color3.fromRGB(42,46,58); aeStroke.Thickness = 1
...
-- Toggle pill
aePill = Instance.new("Frame",aeCard); aePill.BackgroundColor3=Color3.fromRGB(45,50,62)
...
aeKnob = Instance.new("Frame",aePill); aeKnob.BackgroundColor3=Color3.fromRGB(160,170,185)
