...
for idx,opt in ipairs(STYLE_OPTS) do
    ...
    item.LayoutOrder = idx; item.ZIndex = 451; item.AutomaticSize = Enum.AutomaticSize.None
...
svWhite.BorderSizePixel = 0; svWhite.Size = UDim2.fromScale(1,1); svWhite.ZIndex = 502
...
svBlack.BorderSizePixel = 0; svBlack.Size = UDim2.fromScale(1,1); svBlack.ZIndex = 503
...
svHit.Size = UDim2.fromScale(1,1); svHit.Text = ""; svHit.ZIndex = 506
...
hueHit.Size = UDim2.fromScale(1,1); hueHit.Text = ""; hueHit.ZIndex = 503
...
local function refreshCP()
    ...
    -- Cursor hue
    local hb = hueBar.AbsolutePosition; local hs = hueBar.AbsoluteSize
    hueCursor.Position = UDim2.new(0, hb.X + cpHue*hs.X, 0, hb.Y + hs.Y/2)
end
...
cpHexBox.FocusLost:Connect(function()
    local hex = cpHexBox.Text:gsub("[^%x]",""):upper():sub(1,6)
    if #hex==6 then
        local r=tonumber(hex:sub(1,2),16)/255
        local g=tonumber(hex:sub(3,4),16)/255
        local b=tonumber(hex:sub(5,6),16)/255
        local h,s,v = Color3.fromRGB(r*255,g*255,b*255):ToHSV()
