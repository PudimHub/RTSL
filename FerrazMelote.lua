...
local function atSoltar(part, obj, pos)
    pcall(function()
        ...
        -- 6. Impulso direto para baixo — sem variação horizontal
        --    (variação horizontal causa itens voando ao colidir entre si)
        pcall(function()
            part.CanCollide = false   -- evita explosão ao nascer junto de outro item
            part.AssemblyLinearVelocity = Vector3.new(0, -22, 0)
            -- Reativa colisão após estar caindo e separado dos outros
...
-- ── MAIN: Alternado 2 Logs → aguarda → 2 Metais → aguarda → repete ──
--
-- Por que não jogar tudo de uma vez: itens colidem entre si ao nascer
-- no mesmo ponto e voam para longe. Solução: 2 por vez, espera consumir, próximos 2.
--
local function autoTriturador()
    ...
    local ALTURA    = 9     -- studs acima do triturador
    local LOTE      = 2     -- 2 itens por vez (evita colisão)
    local TIMEOUT   = 7     -- segundos aguardando consumo
    local totalEnv  = 0
    local ciclo     = 0

    -- 2 posições fixas: ligeiramente separadas para não colidir entre si
    local function dropPos(slot)
        local angulo = (slot / LOTE) * math.pi * 2
        return Vector3.new(
            shredPos.X + math.cos(angulo) * 0.8,
            shredPos.Y + ALTURA,
            shredPos.Z + math.sin(angulo) * 0.8
        )
    end

    -- Aguarda todos os itens de um lote serem consumidos (sumirem do workspace)
    local function aguardarConsumo(entries)
        local t0 = tick()
        while tick() - t0 < TIMEOUT and atRunning do
            task.wait(0.35)
            local algumVivo = false
            for _, e in ipairs(entries) do
                if e.part and e.part.Parent and e.obj and e.obj.Parent then
                    algumVivo = true; break
                end
            end
            if not algumVivo then return true end  -- todos consumidos
        end
        return false  -- timeout
    end

    Notify.send({type="info", icon="⚙️", accent=AT_COR,
        title="Auto Triturador", msg="Iniciando — 2 Logs, aguarda, 2 Metais, aguarda...", duration=3})

    -- Fase alterna: "log" ou "metal"
    local fase = "log"   -- começa com logs

    while atRunning do
        local logs, metals = atColetarItens(shredder)
        local temLog = #logs > 0
        local temMet = #metals > 0

        if not temLog and not temMet then break end

        -- Escolhe a lista de acordo com a fase atual
        local lista
        if fase == "log" then
            lista = temLog and logs or metals
            fase  = "metal"
        else
            lista = temMet and metals or logs
            fase  = "log"
        end

        -- Se a lista escolhida estiver vazia, usa a outra
        if #lista == 0 then
            lista = (#logs > 0) and logs or metals
        end
        ...
        for s = 1, math.min(LOTE, #lista) do
            ...
            if entry and entry.part and entry.part.Parent then
                atSoltar(entry.part, entry.obj, dropPos(s - 1))
                table.insert(loteEntries, entry)
                totalEnv += 1
                task.wait(0.15)  -- pequeno delay entre os 2 para não colidirem
            end
        end

        -- Status na UI
        pcall(function()
            local tipoAtual = (fase == "metal") and "🪵 Logs" or "⚙️ Metais"
            atStatus.Text = string.format(
                "%s  •  🪵 %d restantes  ⚙️ %d restantes  [lote %d]",
                tipoAtual, #logs, #metals, ciclo)
        end)

        -- Aguarda consumo antes do próximo lote
        aguardarConsumo(loteEntries)
    end
    ...
    if totalEnv > 0 then
        Notify.send({type="success", icon="✅", accent=Color3.fromRGB(87,242,135),
            ...
            msg=string.format("⚙️ %d item(s) triturados em %d lotes!", totalEnv, ciclo),
            duration=5})
    else
    ...
    -- Restaura botão para "ATIVAR"
    pcall(function()
        atBtn.Text = "⚙️  ATIVAR"
        local atBtnG2 = atBtn:FindFirstChildWhichIsA("UIGradient")
        if atBtnG2 then
            atBtnG2.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(255,200,60)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(180,110,0))
            })
        end
        TweenService:Create(atStroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
    end)
end
...
-- Botão toggle ATIVAR/PARAR (Brawl Stars style)
local atBtn = Instance.new("TextButton",atCard)
atBtn.BackgroundColor3 = AT_COR; atBtn.BackgroundTransparency = 0
atBtn.BorderSizePixel = 0; atBtn.AutoButtonColor = false
atBtn.Position = UDim2.new(1,-148,0.5,-18); atBtn.Size = UDim2.new(0,140,0,36)
atBtn.Font = Enum.Font.GothamBlack; atBtn.Text = "⚙️  ATIVAR"
atBtn.TextColor3 = Color3.fromRGB(255,255,255); atBtn.TextSize = 11; atBtn.ZIndex = 8
Instance.new("UICorner",atBtn).CornerRadius = UDim.new(0,10)
local atBtnStroke = Instance.new("UIStroke",atBtn)
atBtnStroke.Color = Color3.fromRGB(8,4,0); atBtnStroke.Thickness = 2.5; atBtnStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local atBtnG = Instance.new("UIGradient",atBtn)
atBtnG.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,200,60)),ColorSequenceKeypoint.new(1,Color3.fromRGB(180,110,0))}); atBtnG.Rotation = 90
local atBtnShine = Instance.new("Frame",atBtn); atBtnShine.BackgroundColor3 = Color3.fromRGB(255,255,255)
atBtnShine.BackgroundTransparency = 0.65; atBtnShine.BorderSizePixel = 0
atBtnShine.Position = UDim2.new(0,6,0,4); atBtnShine.Size = UDim2.new(0,55,0,8); atBtnShine.ZIndex = 9
Instance.new("UICorner",atBtnShine).CornerRadius = UDim.new(1,0)
local atBtnLblS = Instance.new("UIStroke",atBtn); atBtnLblS.Color = Color3.fromRGB(8,4,0); atBtnLblS.Thickness = 1.5

atBtn.MouseEnter:Connect(function()
    if not atRunning then
        TweenService:Create(atBtn,TweenInfo.new(0.1),{Size=UDim2.new(0,144,0,38),Position=UDim2.new(1,-150,0.5,-19)}):Play()
    end
end)
atBtn.MouseLeave:Connect(function()
    if not atRunning then
        TweenService:Create(atBtn,TweenInfo.new(0.1),{Size=UDim2.new(0,140,0,36),Position=UDim2.new(1,-148,0.5,-18)}):Play()
    end
end)
atBtn.MouseButton1Click:Connect(function()
    if atRunning then
        ...
        atRunning = false
        atBtn.Text = "⚙️  ATIVAR"
        atBtnG.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,200,60)),ColorSequenceKeypoint.new(1,Color3.fromRGB(180,110,0))})
        atStatus.Text = "⏹ Parado"
        TweenService:Create(atStroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0),Thickness=3}):Play()
        Notify.error("⚙️ Auto Triturador", "⏹ Desativado manualmente")
        task.delay(1.5, function() pcall(function() if atStatus then atStatus.Text="" end end) end)
    else
        -- ATIVAR
        atBtn.Text = "⏹  PARAR"
        atBtnG.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(220,60,60)),ColorSequenceKeypoint.new(1,Color3.fromRGB(140,20,20))})
        atStatus.Text = "🔍 Localizando triturador..."
        TweenService:Create(atStroke,TweenInfo.new(0.2),{Color=AT_COR}):Play()
        -- Pulso no card
        task.spawn(function()
            while atRunning do
                TweenService:Create(atStroke,TweenInfo.new(0.8,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Thickness=4.5}):Play()
                task.wait(0.8); if not atRunning then break end
                TweenService:Create(atStroke,TweenInfo.new(0.8,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Thickness=3}):Play()
                task.wait(0.8)
            end
            TweenService:Create(atStroke,TweenInfo.new(0.3),{Thickness=3,Color=Color3.fromRGB(8,4,0)}):Play()
        end)
        task.spawn(function()
            autoTriturador()
            -- Restaura botão quando terminar naturalmente
            pcall(function()
                atBtn.Text = "⚙️  ATIVAR"
                atBtnG.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,200,60)),ColorSequenceKeypoint.new(1,Color3.fromRGB(180,110,0))})
                atStatus.Text = ""
...
-- ── Lista de COMBUSTÍVEL da Fogueira ─────────────────────────
-- REGRAS:
--   AF_FUEL_SUB  = match por SUBSTRING (palavras que nunca aparecem em coletáveis)
--   AF_FUEL_EXACT = match EXATO apenas (palavras curtas/genéricas)
--   AF_BLACKLIST  = NUNCA é combustível, mesmo que o nome combine
local AF_FUEL = {}
local AF_FUEL_EXACT = {}
local AF_FUEL_SUB   = {}
do
    -- Substring seguro — essas palavras nunca aparecem em coletáveis/pickups
    local subList = {
        "log","firewood","kindling","tinder","charcoal",
    ...
    -- Exact match — palavras curtas que podem colidir com outros itens
    local exactList = {
        "wood","coal","bone","wax","stick","branch","twig",
        "turf","amber","feather","skull","antler",
    }
    for _, n in ipairs(subList)   do AF_FUEL_SUB[n]   = true; AF_FUEL[n] = true end
    for _, n in ipairs(exactList) do AF_FUEL_EXACT[n] = true; AF_FUEL[n] = true end
end

-- Palavras que NUNCA são combustível (coletáveis, moedas, efeitos visuais)
local AF_BLACKLIST_SET = {}
do
    local bl = {
        "gem","gemstone","crystal","ore","ingot","rock","stone","coin","soul",
        "spirit","orb","pickup","collectible","currency","badge",
        "token","point","score","xp","exp","reward","loot","drop",
        "spark","sparkle","glow","particle","effect","decor",
        "decoration","prop","deco","fake","dummy","visual",
        "hitbox","trigger","zone","sensor","ui","gui","hud",
        "icon","marker","indicator","arrow","dot","sphere",
        "light","lamp","lantern","torch","candle",
    }
    for _, n in ipairs(bl) do AF_BLACKLIST_SET[n] = true end
end

-- ═══════════════════════════════════════════════════════════════
-- afAcharFogueira: busca POSIÇÃO da fogueira (para largar itens)
-- afGetNivel:      lê o NÍVEL real da fogueira
--
-- ESTRATÉGIA DE DETECÇÃO DE NÍVEL:
--   O jogo 99 Nights in the Forest armazena o nível da fogueira
--   provavelmente como atributo "Level" ou "FireLevel" no model MainFire.
--   Se não achar, compara FogEnd ANTES vs DEPOIS para detectar upgrade.
--   FogEnd padrão Roblox = 100000+ → NÃO usar como indicador de nível
--   (o bug antigo era: FogEnd >= 400 → nível 6 → parava imediatamente)
-- ═══════════════════════════════════════════════════════════════
local AF_DEBUG_PRINTED = false  -- imprime estrutura uma vez no output

local function afAcharFogueira()
    local best, bestScore = nil, -1
    pcall(function()
        -- 1. Caminho específico do 99 Nights
        local camp = workspace:FindFirstChild("Campground")
        if camp then
            local mf = camp:FindFirstChild("MainFire")
            if mf then best = mf; bestScore = 999
                -- DEBUG: imprime estrutura do MainFire uma vez
                if not AF_DEBUG_PRINTED then
                    AF_DEBUG_PRINTED = true
                    task.spawn(function()
                        print("[AF DEBUG] ══════ workspace.Campground.MainFire ══════")
                        print("[AF DEBUG] Atributos:")
                        pcall(function()
                            for k, v in pairs(mf:GetAttributes()) do
                                print("[AF DEBUG]   Attr: "..tostring(k).." = "..tostring(v))
                            end
                        end)
                        print("[AF DEBUG] Filhos:")
                        pcall(function()
                            for _, c in ipairs(mf:GetChildren()) do
                                local val = ""
                                if c:IsA("NumberValue") or c:IsA("IntValue") or c:IsA("StringValue") or c:IsA("BoolValue") then
                                    val = " = "..tostring(c.Value)
                                end
                                print("[AF DEBUG]   "..c.ClassName..": "..c.Name..val)
                            end
                        end)
                        print("[AF DEBUG] FogEnd atual: "..tostring(pcall(function() return game.Lighting.FogEnd end)))
                        print("[AF DEBUG] ════════════════════════════════════════")
                    end)
                end
                return
            end
            -- Tenta outros filhos de Campground
            for _, obj in ipairs(camp:GetChildren()) do
                local nm = obj.Name:lower()
                if nm:find("fire",1,true) or nm:find("camp",1,true) then
                    best = obj; bestScore = 500; return
                end
            end
        end
        -- 2. Scan geral com pontuação
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                if not (obj:IsA("Model") or obj:IsA("BasePart")) then return end
                local nm = obj.Name:lower()
                ...
                pcall(function()
                    local v = obj:GetAttribute("Level") or obj:GetAttribute("CampfireLevel")
                           ...
                           or obj:GetAttribute("FireTier") or obj:GetAttribute("CurrentLevel")
                    if v ~= nil then score = score + 100 end
                end)
                pcall(function()
                    for _, c in ipairs(obj:GetChildren()) do
                        local cn = c.Name:lower()
                        if (c:IsA("NumberValue") or c:IsA("IntValue")) and
                           (cn == "level" or cn == "fuellevel" or cn == "firelevel"
                            or cn == "tier" or cn == "currentlevel" or cn == "campfirelevel") then
                ...
                elseif nm:find("campfire",1,true) or nm:find("mainfire",1,true)
                    or nm:find("central",1,true) then score = score + 10
                end
...
-- FogEnd usado para DETECTAR MUDANÇA DE NÍVEL (delta), não como valor absoluto
-- O FogEnd padrão do Roblox é 100000+ → não pode ser mapeado diretamente para nível
local AF_FOGEND_BASELINE = nil  -- capturado ao iniciar Auto Fogueira
local AF_FOGEND_DELTA_TABLE = {
    -- Quanta studs de aumento no FogEnd corresponde a cada nível acima do baseline
    -- Calibrado para 99 Nights in the Forest (cada nível ≈ +60-80 studs de visão)
    -- Se FogEnd subiu 60+ studs → subiu 1 nível, etc.
    [1] = 60,
    [2] = 130,
    [3] = 200,
    [4] = 270,
...
local function afGetNivel(fogModel)
    local nivel = nil
    -- ── FONTE 1: Atributo direto no model ─────────────────────────
    pcall(function()
        nivel = fogModel:GetAttribute("Level")
             or fogModel:GetAttribute("CampfireLevel")
             or fogModel:GetAttribute("FireLevel")
             or fogModel:GetAttribute("Tier")
             or fogModel:GetAttribute("CurrentLevel")
             or fogModel:GetAttribute("FuelLevel")
             or fogModel:GetAttribute("FuelCount")
             or fogModel:GetAttribute("FireTier")
        if nivel and type(nivel) == "number" and nivel >= 1 and nivel <= 10 then
            return
        else
            nivel = nil
        end
    end)
    -- ── FONTE 2: Children NumberValue/IntValue ─────────────────────
    if nivel == nil then
        pcall(function()
            local names = {"Level","CampfireLevel","FireLevel","Tier",
                           "CurrentLevel","FuelLevel","FuelCount","FireTier"}
            for _, nm in ipairs(names) do
                local c = fogModel:FindFirstChild(nm)
                if c and (c:IsA("NumberValue") or c:IsA("IntValue")) and c.Value >= 1 then
                    nivel = math.floor(c.Value)
                    break
                end
            end
        end)
    end
    -- ── FONTE 3: FogEnd DELTA (variação desde o início) ────────────
    -- Só usa FogEnd se ele estiver num intervalo realista de jogo (50–2000 studs)
    -- FogEnd padrão Roblox = 100000+ → IGNORADO para evitar falso nível 6
    if nivel == nil and AF_FOGEND_BASELINE then
        pcall(function()
            local fog = game:GetService("Lighting").FogEnd
            -- Só considera FogEnd válido se entre 50 e 2000 studs
            if fog and fog >= 50 and fog <= 2000 then
                local delta = fog - AF_FOGEND_BASELINE
                local lvl = 1
                for lvlAbove, threshold in pairs(AF_FOGEND_DELTA_TABLE) do
                    if delta >= threshold then lvl = math.max(lvl, lvlAbove + 1) end
                end
                nivel = math.min(lvl, 6)
            end
        end)
    end
    -- ── FALLBACK: retorna nil (desconhecido) ───────────────────────
    -- NUNCA retorna 6 por padrão! Se nil, o loop continua enviando combustível.
    return nivel
end
    ...
    pcall(function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                ...
                if obj:IsA("BasePart") then
                    local pm = obj.Parent
                    -- Se a part está dentro de um Model → o Model branch vai cuidar disso
                    -- Evita coletar partes internas de itens (só queremos o Model inteiro)
                    if pm and pm:IsA("Model") then return end
                    -- Também ignora parts dentro de caracteres/humanoids
                    if pm and pm:FindFirstChildWhichIsA("Humanoid") then return end
                    targetPart = obj; checkName = obj.Name:lower()
                elseif obj:IsA("Model") and not obj:FindFirstChildWhichIsA("Humanoid") then
                    if seen[obj] then return end
                    local p2 = obj:FindFirstChildWhichIsA("BasePart")
                ...
                -- ── Blacklist: palavras que nunca são combustível ──────
                for blName in pairs(AF_BLACKLIST_SET) do
                    if checkName == blName or checkName:find(blName, 1, true) then return end
                end

                -- ── Match de combustível ──────────────────────────────
                local matched = false
                -- 1. Exact match (nome completo)
                if AF_FUEL_EXACT[checkName] then
                    matched = true
                end
                -- 2. Substring match apenas para palavras seguras
                if not matched then
                    for fuelName in pairs(AF_FUEL_SUB) do
                        if checkName == fuelName or checkName:find(fuelName, 1, true) then
                            matched = true; break
                        end
                    end
                end
                -- 3. Exclui itens muito pequenos em forma de esfera (coletáveis/pickups)
                --    Esferas com diâmetro < 1.8 studs são quase certamente coletáveis
                if matched and targetPart.Shape == Enum.PartType.Ball then
                    local sz = targetPart.Size
                    if sz.X < 1.8 and sz.Y < 1.8 and sz.Z < 1.8 then return end
                end
...
local function afSoltar(part, obj, pos)
    pcall(function()
        ...
        part.Anchored = false
        -- CanCollide = false inicialmente para não colidir com outros itens
        -- (evita o bug de explodir ao nascer junto de outro item)
        ...
        -- Impulso APENAS para baixo, sem variação horizontal
        -- (variação horizontal causa itens voando para longe)
        part.AssemblyLinearVelocity = Vector3.new(0, -25, 0)
        -- Reativa colisão após 0.4s (quando já está caindo, longe dos outros)
        task.delay(0.4, function()
            pcall(function()
                if part and part.Parent then
                    part.CanCollide = true
...
local function autoFogueira()
    ...
    -- Captura FogEnd inicial como baseline para detecção de nível por delta
    AF_FOGEND_BASELINE = nil
    pcall(function()
        local fog = game:GetService("Lighting").FogEnd
        -- Só captura se FogEnd está num intervalo realista de jogo
        if fog and fog >= 50 and fog <= 2000 then
            AF_FOGEND_BASELINE = fog
        end
    end)

    -- Detecta nível ATUAL antes de começar
    local nivelInicial = fogModel and afGetNivel(fogModel)
    local nivelStr = nivelInicial and ("Nível atual: "..tostring(nivelInicial).." / "..tostring(maxNivel)) or "Nível: desconhecido (sem atributo)"

    Notify.send({type="warn", icon="🔥", accent=AF_COR,
        title="Auto Fogueira", msg="Iniciando — "..nivelStr, duration=3})
    ...
    while afRunning do
        if fogModel then
            ...
            if nivel ~= nil then
                -- Nível detectado com sucesso
                pcall(function()
                    afStatus.Text = string.format("🔥 Nível %d / %d", nivel, maxNivel)
                end)
                -- Só para se nível REAL (via atributo/IntValue) for >= máximo
                -- NUNCA para baseado em FogEnd absoluto (bug antigo)
                ...
                -- Nível desconhecido (sem atributo na fogueira)
                -- Continua enviando combustível normalmente
                pcall(function()
                    afStatus.Text = string.format("🔥 Enviando... (%d itens) [sem attr nível]", totalEnv)
                end)
            end
        end
        ...
        if fogModel then
            local nivel = afGetNivel(fogModel)
            -- nil = nível desconhecido, não para
            if nivel ~= nil and nivel >= maxNivel then
                pcall(function() afStatus.Text = "✅ Nível máximo atingido!" end)
                Notify.send({type="achievement", icon="🔥", accent=Color3.fromRGB(255,210,60),
                    title="Auto Fogueira — Concluído!",
