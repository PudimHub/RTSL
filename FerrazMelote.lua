...
makeSliderBar(Pages["Player"], T("plSpeedTitle"), T("plSpeedDesc"), Color3.fromRGB(255,180,30), 16, 275, 30, function(v) playerSpeed=v; applySpeed(v) end)
makeSliderBar(Pages["Player"], T("plJumpTitle"), T("plJumpDesc"), Color3.fromRGB(100,220,255), 50, 1285, 80, function(v) playerJump=v; applyJump(v) end)
...
-- ══════════════════════════════════════════════════════
-- GOD MOD — invisibilidade REAL para os mobs
-- ══════════════════════════════════════════════════════
...
local function setGodMod(state)
    godModEnabled = state
    if state then
        local function applyGodMod()
            pcall(function()
                local ch = Player.Character; if not ch then return end
                -- 1) Torna partes do personagem invisíveis localmente
                for _, part in ipairs(ch:GetDescendants()) do
                    if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                        part.LocalTransparencyModifier = 1
                    end
                end
                -- 2) Remove o NameTag / distância de detecção do Humanoid
        ...
        -- Loop para mobs próximos: zera aggro/TargetPoint deles continuamente
        godModConn = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                -- Mantém invisibilidade mesmo com respawn ou mudança de roupa
                for _, part in ipairs(ch:GetDescendants()) do
                    if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                ...
                -- Limpa aggro de mobs próximos (força TargetPoint para longe)
                local charFolder = workspace:FindFirstChild("Characters")
                local source = charFolder or workspace
                for _, mob in ipairs(source:GetChildren()) do
                    pcall(function()
                        ...
                        if dist > 50 then return end -- só mobs próximos

                        -- Zera target — alguns jogos armazenam em atributos ou valores
                        pcall(function() mob:SetAttribute("TargetPlayer", nil) end)
                        pcall(function() mob:SetAttribute("Target", nil) end)
                        pcall(function() mob:SetAttribute("AggroTarget", nil) end)
                        -- Muda TargetPoint do Humanoid para longe do jogador
                        local mHum = mob:FindFirstChildWhichIsA("Humanoid")
                        if mHum then
                            pcall(function() mHum:MoveTo(mHrp.Position) end) -- faz mob ficar parado
                        end
                        -- Remove scripts de aggro client-side
                        for _, child in ipairs(mob:GetDescendants()) do
                            if child:IsA("LocalScript") and child.Name:lower():find("aggro") then
        ...
        Notify.send({type="custom", icon="👻", accent=Color3.fromRGB(140,255,140),
            title="God Mod", msg="Você está invisível para os mobs!", duration=5})
    else
        ...
        pcall(function()
            ...
            local hum = ch:FindFirstChildWhichIsA("Humanoid")
            if hum then
                pcall(function() hum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.Viewer end)
                pcall(function() hum.HealthDisplayDistance = 20 end)
                pcall(function() hum.NameDisplayDistance = 100 end)
            end
        end)
        Notify.info("👻 God Mod", "Visibilidade restaurada.")
    end
end
...
-- ──────────────────────────────────────────────
-- Função: encontrar centro da fogueira (MainFire)
-- ──────────────────────────────────────────────
local function getCampfirePos()
    ...
    pcall(function()
        ...
        if not pos then
            -- fallback: procura qualquer objeto chamado MainFire
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj.Name == "MainFire" and obj:IsA("Model") then
...
local CHILD_NAMES = {
    -- Nomes no workspace confirmados (outros scripts usam esses)
    "lost child","lostchild",
    "lost child 2","lost child 3","lost child 4",
    "lostchild2","lostchild3","lostchild4",
    -- Apelidos visuais por chapéu
    "dino kid","dinokid",
    "kraken kid","krakenkid",
    "squid kid","squidkid",
    "koala kid","koalakid",
    -- Genéricos fallback
    "missing child","missingchild","capturedchild","captured child",
}

local tpChildActive = false
local tpChildCard, tpChildStroke, tpChildBtn, tpChildBtnL

local function isChildCaptured(model)
    -- Criança capturada = tem Humanoid vivo E não foi salva
    local hum = model:FindFirstChildWhichIsA("Humanoid")
    if not hum or hum.Health <= 0 then return false end
    -- Verifica atributos de "salvo"
    local rescued = false
    pcall(function()
        rescued = model:GetAttribute("Rescued") == true
            or model:GetAttribute("Saved") == true
            or model:GetAttribute("IsSaved") == true
            or model:GetAttribute("Free") == true
    end)
    if rescued then return false end
    -- Verifica se o modelo tem partes de "grade/cage" como descendente (sinal de capturada)
    -- Ou simplesmente verifica que está na pasta Characters com um nome de criança
    return true
end
...
local function findNearestCapturedChild()
    ...
    pcall(function()
        ...
        if chars then table.insert(sources, chars) end
    ...
    for _, src in ipairs(sources) do
        for _, obj in ipairs(src:GetDescendants()) do
            if obj:IsA("Model") and not isPlayerChar(obj) then
                local objNameLow = obj.Name:lower()
                local isChild = false
                for _, cn in ipairs(CHILD_NAMES) do
                    if objNameLow:find(cn, 1, true) then isChild = true; break end
                end
                if isChild and isChildCaptured(obj) then
...
tpChildBtn.MouseButton1Click:Connect(function()
    ...
    if target then
        ...
        Notify.send({type="custom",icon="👶",accent=TP_COR_CHILD,
            title="Tp Criança",msg="Teleportado para a criança capturada mais próxima!",duration=3})
    else
        Notify.warn("Tp Criança","⚠️ Nenhuma criança capturada encontrada nas proximidades!")
    end
end)
...
local KNOWN_STRUCTURES = {
    "Small Shed","SmallShed","Wooden Shed","WoodenShed","Armory",
    "Water Tower","WaterTower","Watchtower","WatchTower",
    "Medium Cabin","MediumCabin","Large Cabin","LargeCabin",
    "Fishing Cabin","FishingCabin",
    "Abandoned Animal Shelter","AbandonedAnimalShelter",
    "Bank","Barn","Basketball Court","BasketballCourt",
    "Clinic","Mineshaft","Mine Shaft","Food Warehouse","FoodWarehouse",
    "Playground","Restroom",
    "Abandoned Camp","AbandonedCamp","Abandoned Pillars","AbandonedPillars",
    "Farm","Plane Wreckage","PlaneWreckage","Plane Crash","PlaneCrash",
    ...
    "Lava Pool","LavaPool","Scorpion Pit","ScorpionPit",
    "Ammo Furnace","AmmoFurnace","Volcanic Church","VolcanicChurch",
    "Lava Fall","LavaFall","Lava-Isolated Chest",
    "Fresh Pond","FreshPond","Algal Pond","AlgalPond","Bone Pond","BonePond",
    "Cold Pond","ColdPond","Frog Pond","FrogPond",
    "Ranger Camp","RangerCamp","Hunter Camp","HunterCamp",
}

local buildingButtons = {}       -- { name, pos, btn, lastVisited }
local buildingSeenNames = {}     -- set de nomes já adicionados
...
btnRefreshL.Text = "↻"; btnRefreshL.TextColor3 = Color3.fromRGB(200,220,255); btnRefreshL.TextSize = 16; btnRefreshL.ZIndex = 9
...
btnClearL.Text = "✕"; btnClearL.TextColor3 = Color3.fromRGB(255,180,180); btnClearL.TextSize = 14; btnClearL.ZIndex = 9
...
local function makeBuildingBtn(name, pos, isNew)
    ...
    local function doTp()
        -- Remove etiqueta NOVA se existir
        if entry.isNew then
            entry.isNew = false
            tagBadge.Visible = false
        end
        -- Marca o anterior como ANTIGA
        ...
        -- TP 55 studs acima da construção
        safeTp(pos, 55)
        entry.visited = true
...
local function scanBuildings(isRefresh)
    ...
    scanRunning = true
    btnRefreshL.Text = "⏳"; task.delay(0.5, function() btnRefreshL.Text = "↻" end)

    task.spawn(function()
        local found = 0
        local prevNames = {}
        if isRefresh then
            -- Guarda quais já existiam
            for name, _ in pairs(buildingSeenNames) do prevNames[name] = true end
        end

        pcall(function()
            local toSearch = {workspace}
            pcall(function()
                local map = workspace:FindFirstChild("Map")
                if map then table.insert(toSearch, map) end
                local items = workspace:FindFirstChild("Items")
                if items then table.insert(toSearch, items) end
            end)

            for _, src in ipairs(toSearch) do
                for _, obj in ipairs(src:GetDescendants()) do
                    if obj:IsA("Model") then
                        local oName = obj.Name
                        local oNameLow = oName:lower()
                        -- Verifica se é uma estrutura conhecida e não está ainda no painel
                        for _, sn in ipairs(KNOWN_STRUCTURES) do
                            if oNameLow == sn:lower() and not buildingSeenNames[oName] then
                                -- Tem uma BasePart?
                                local part = obj:FindFirstChildWhichIsA("BasePart")
                                if part then
                                    buildingSeenNames[oName] = true
                                    local isNewEntry = isRefresh and not prevNames[oName]
                                    makeBuildingBtn(oName, part.Position, isNewEntry)
                                    found += 1
        ...
        elseif found > 0 then
            local msg = isRefresh and (tostring(found).." nova(s) construção(ões) adicionadas!") or (tostring(found).." construções encontradas!")
        ...
        buildPanelCard.Size = UDim2.new(1,0,0, math.clamp(48 + contentH, 100, 420))
        buildScroll.Size = UDim2.new(1,-12,0, math.clamp(contentH, 40, 364))
...
-- Coleta todos os mobs vivos no range
...
local function onWeaponActivated(tool)
    ...
    for _, mhrp in ipairs(mobs) do
        pcall(function()
            ...
            -- Posição 1.5 studs na frente do mob, olhando para ele
            ...
            -- Fica atrás do mob olhando para ele (assim o hitbox do swing acerta)
            local attackCF = CFrame.new(mobPos - dir * 1.5, mobPos)
            hrp.CFrame = attackCF

            -- Espera 1 RunService step para o servidor ver a posição nova
            local stepped = false
            ...
            local t = tick()
            while not stepped and (tick()-t) < 0.1 do task.wait() end

            -- Dispara o evento Activated da tool manualmente
            -- Isso reutiliza o handler REAL do jogo (não um remote fake)
            pcall(function() tool:Activate() end)

            -- Aguarda mais 1 frame para o hitbox registrar
            local stepped2 = false
            local conn2; conn2 = RunService.Stepped:Connect(function()
                stepped2 = true; conn2:Disconnect()
            end)
            local t2 = tick()
            while not stepped2 and (tick()-t2) < 0.1 do task.wait() end
        end)
    end
