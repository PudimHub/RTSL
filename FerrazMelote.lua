...
local function nCreateCard(cfg, tipo)
    ...
    local BASE_H=58; local extraH=(msg~="" and 14 or 0)+(action and 30 or 0); local TOTAL_H=BASE_H+extraH
    local isLeft=NOTIF_CFG.CORNER=="TL" or NOTIF_CFG.CORNER=="BL"
    local isBottom=NOTIF_CFG.CORNER=="BR" or NOTIF_CFG.CORNER=="BL"
    ...
    iconBg.Position=UDim2.new(0,24,0,10); iconBg.Size=UDim2.new(0,26,0,26); iconBg.ZIndex=522
    ...
    iconLbl.TextColor3=accent; iconLbl.TextSize=12; iconLbl.ZIndex=523
    -- Title
    local titleLbl=Instance.new("TextLabel",card); titleLbl.BackgroundTransparency=1
    titleLbl.Position=UDim2.new(0,46,0,8); titleLbl.Size=UDim2.new(1,-68,0,15)
    titleLbl.Font=Enum.Font.GothamBlack; titleLbl.Text=title
    titleLbl.TextColor3=Color3.fromRGB(238,242,255); titleLbl.TextSize=11
    ...
    typeBadge.Position=UDim2.new(0,46,0,25); typeBadge.Size=UDim2.new(0,0,0,11)
    ...
    if msg~="" then
        ...
        msgLbl.Position=UDim2.new(0,46,0,38); msgLbl.Size=UDim2.new(1,-56,0,13)
        msgLbl.Font=Enum.Font.Gotham; msgLbl.Text=msg
        msgLbl.TextColor3=Color3.fromRGB(150,160,185); msgLbl.TextSize=9
    ...
    closeBtn.Position=UDim2.new(1,-22,0,4); closeBtn.Size=UDim2.new(0,18,0,18)
    closeBtn.Font=Enum.Font.GothamBold; closeBtn.Text="×"; closeBtn.TextColor3=Color3.fromRGB(90,100,120)
    closeBtn.TextSize=16; closeBtn.ZIndex=524
    ...
    -- Slide in — nova notificação entra no topo, reflow empurra as outras para baixo
    local targetXOff = isLeft and NOTIF_CFG.PADDING or -NOTIF_CFG.PADDING
    local topYOff    = isBottom and -(NOTIF_CFG.PADDING + TOTAL_H) or NOTIF_CFG.PADDING
    nReflow()  -- reposiciona todos (os antigos descem, novo fica no topo)
    card.Position = UDim2.new(isLeft and 0 or 1, startXOff, isBottom and 1 or 0, topYOff)
    task.wait()
    TweenService:Create(card, TweenInfo.new(0.38, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(isLeft and 0 or 1, targetXOff, isBottom and 1 or 0, topYOff)
    }):Play()
    task.spawn(function()
        task.wait(0.1)
        TweenService:Create(iconBg,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(0,30,0,30),Position=UDim2.new(0,22,0,8)}):Play()
        task.wait(0.2)
        TweenService:Create(iconBg,TweenInfo.new(0.2,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,26,0,26),Position=UDim2.new(0,24,0,10)}):Play()
    end)
    -- Timer
    task.spawn(function()
...
histToggleBtn.BackgroundTransparency=1; histToggleBtn.Size=UDim2.new(1,0,1,0); histToggleBtn.Text=""; histToggleBtn.ZIndex=9
...
-- Botão header abre/fecha
local histHeaderBtn=Instance.new("TextButton",histHeader)
histHeaderBtn.BackgroundTransparency=1; histHeaderBtn.Size=UDim2.new(1,0,1,0); histHeaderBtn.Text=""; histHeaderBtn.ZIndex=7
