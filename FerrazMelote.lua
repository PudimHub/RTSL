...
local function nReflow()
    ...
    for _, entry in ipairs(nActive) do
        if entry and entry.frame and entry.frame.Parent then
            ...
            -- Move strokeWrap junto com o card
            if entry.strokeWrap and entry.strokeWrap.Parent then
                TweenService:Create(entry.strokeWrap, TweenInfo.new(0.26, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    Position = UDim2.new(1, -NOTIF_CFG.PADDING, 0, NOTIF_CFG.PADDING + totalOff)
                }):Play()
            end
...
local function nRemoveEntry(entry, instant)
    ...
    -- Anima o strokeWrap também junto
    if entry.strokeWrap and entry.strokeWrap.Parent then
        TweenService:Create(entry.strokeWrap, TweenInfo.new(dur, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Position = UDim2.new(1, NOTIF_CFG.WIDTH + 40, 0, frame.Position.Y.Offset)
        }):Play()
    end
    task.delay(dur + 0.05, function()
        pcall(function() frame:Destroy() end)
        pcall(function() if entry.strokeWrap then entry.strokeWrap:Destroy() end end)
        nReflow()
    end)
end
...
local function nCreateCard(cfg, tipo)
    ...
    card.Size=UDim2.new(0,NOTIF_CFG.WIDTH,0,TOTAL_H); card.ZIndex=520; card.ClipsDescendants=false
    local cardCorner=Instance.new("UICorner",card); cardCorner.CornerRadius=UDim.new(0,22)
    -- Stroke externo em wrapper separado para não ser cortado pelo ClipsDescendants
    local strokeWrap=Instance.new("Frame",NotifRoot)
    strokeWrap.Name="PudimNotifStroke_"..tostring(tick()); strokeWrap.BackgroundTransparency=1
    strokeWrap.BorderSizePixel=0; strokeWrap.AnchorPoint=Vector2.new(1,0)
    strokeWrap.Position=UDim2.new(1, startX, 0, topY)
    strokeWrap.Size=UDim2.new(0,NOTIF_CFG.WIDTH,0,TOTAL_H); strokeWrap.ZIndex=519
    Instance.new("UICorner",strokeWrap).CornerRadius=UDim.new(0,22)
    local cardStroke=Instance.new("UIStroke",strokeWrap); cardStroke.Color=accent; cardStroke.Thickness=1.5; cardStroke.Transparency=0.45
    -- Sidebar
    local sideBar=Instance.new("Frame",card); sideBar.BackgroundColor3=accent; sideBar.BorderSizePixel=0
    sideBar.Size=UDim2.new(0,4,1,0); sideBar.ZIndex=521; Instance.new("UICorner",sideBar).CornerRadius=UDim.new(0,22)
    -- Glow com UICorner para seguir o arredondamento do card
    ...
    -- Progress bar — inset para não vazar nos cantos arredondados
    local progressBg=Instance.new("Frame",card); progressBg.BackgroundColor3=Color3.fromRGB(25,28,38)
    progressBg.BorderSizePixel=0; progressBg.Position=UDim2.new(0,6,1,-5); progressBg.Size=UDim2.new(1,-12,0,3); progressBg.ZIndex=522
    ...
    local entry={frame=card,strokeWrap=strokeWrap,cfg=cfg,tipo=tipo,startTick=tick(),duration=dur,paused=false,pauseAcc=0,pauseFrom=0,_removed=false,progress=progressFill}
    ...
    hitbox.MouseEnter:Connect(function()
        ...
        TweenService:Create(cardStroke,TweenInfo.new(0.15),{Transparency=0.0,Thickness=2}):Play()
    end)
    hitbox.MouseLeave:Connect(function()
        if entry._removed then return end; entry.paused=false; entry.pauseAcc=entry.pauseAcc+(tick()-entry.pauseFrom)
        TweenService:Create(cardStroke,TweenInfo.new(0.15),{Transparency=0.45,Thickness=1.5}):Play()
    end)
    ...
    TweenService:Create(strokeWrap, TweenInfo.new(0.38, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, targetX, 0, topY)
    }):Play()
    task.spawn(function()
        task.wait(0.1)
...
local ESP_CATS = {
    ...
    {key="Kids", trLabel="espKidsLabel",trDesc="espKidsDesc",label="👶 Lost Children", cor=Color3.fromRGB(100,220,255), tipo="entity", alcance=math.huge, desc="Dino, Kraken, Squid, Koala Kid",
...
local entityCache={}; local itemCache={}; local cacheBuilding=false; local lastCache=0; local CACHE_INTER=2  -- reduzido para detecção mais rápida
local function isAlive(model)
    local hum=model:FindFirstChildWhichIsA("Humanoid"); if not hum then return false end
    if hum.Health<=0 then return false end
    -- MaxHealth pode ser 0 antes de inicializar, não filtrar
    ...
    if pos.Y < -500 then return false end  -- muito abaixo do mapa
    return true
end
...
local function buildCache()
    ...
    task.spawn(function()
        ...
        -- Scan workspace.Characters pasta diretamente (mais confiável que GetDescendants para mobs)
        local extraEnts = {}
        pcall(function()
        ...
        for _,obj in ipairs(allObjs) do
            ...
            elseif doItem and obj:IsA("Model") and not obj:FindFirstChildWhichIsA("Humanoid") then
                -- Detecta itens que são Models (ex: itens dropados como Model)
                if not pchars[obj] then
                    ...
                    if part then
                        for _,c in ipairs(ESP_CATS) do
                            if espAtivo[c.key] and c.tipo=="item" then
                                local lk=espLookup[c.key]
                                if lk and lk[nl] then table.insert(newItem,{key=c.key,cor=c.cor,nome=obj.Name,alcance=c.alcance,obj=part}); break end
                            end
                        end
                    end
                end
...
local BRING_CATS = {
    ...
    {key="BCombust",  trLabel="bCombustLabel", trDesc="bCombustDesc", label="🔥 Bring Combustível", cor=Color3.fromRGB(255,120,30),  desc="Coal, Biofuel, Fuel Canister, Oil Barrel…",
     nomes={"Coal","Biofuel","Oil Barrel","OilBarrel","Fuel Canister","FuelCanister","Purple Fur Tuft","PurpleFurTuft","Chair"}},
    ...
    {key="BPeixes",   trLabel="bPeixesLabel",trDesc="bPeixesDesc",label="🐟 Bring Peixes",      cor=Color3.fromRGB(80,180,255),  desc="Mackerel, Salmon, Clownfish, Shark, Lava Eel…",
     nomes={"Mackerel","Cooked Mackerel","CookedMackerel","Salmon","Cooked Salmon","CookedSalmon","Clownfish","Cooked Clownfish","CookedClownfish","Jellyfish","Char","Cooked Char","CookedChar","Eel","Cooked Eel","CookedEel","Swordfish","Cooked Swordfish","CookedSwordfish","Shark","Cooked Shark","CookedShark","Lava Eel","LavaEel","Cooked Lava Eel","CookedLavaEel","Lionfish","Cooked Lionfish","CookedLionfish"}},
    {key="BSementes", trLabel="bSementesLabel",trDesc="bSementesDesc",label="🌱 Bring Sementes",    cor=Color3.fromRGB(135,245,115), desc="Chili, Berry, Flower, Dripleaf, Moonflower…",
     nomes={"Chili Seeds","ChiliSeeds",
             "Flower Seeds","FlowerSeeds",
             "Berry Seeds","BerrySeeds",
             "Apple Seeds","AppleSeeds",
    ...
    {key="BArmas",    trLabel="bArmasLabel",trDesc="bArmasDesc",label="⚔️ Bring Armas",       cor=Color3.fromRGB(255,70,70),   desc="Spear, Ice Sword, Crossbow, Revolver, Rifle…",
     nomes={
       "Spear","Morningstar","Katana",
       "Laser Sword","LaserSword","Ice Sword","IceSword",
       "Trident","Poison Spear","PoisonSpear",
    ...
    {key="BPelts",    trLabel="bPeltsLabel",trDesc="bPeltsDesc",label="🦺 Bring Pelts",       cor=Color3.fromRGB(210,170,120), desc="Bunny Foot, Wolf Pelt, Bear Pelt, Arctic Fox…",
    ...
    {key="BPocoes",   trLabel="bPocoesLabel",trDesc="bPocoesDesc",label="🧪 Bring Poções",      cor=Color3.fromRGB(195,100,255), desc="Dripleaf, Moonflower Bulb, Stareweed, Mandrake",
     nomes={"Dripleaf","Moonflower Bulb","MoonflowerBulb",
             "Stareweed Petal","StareweedPetal",
             "Cave Vine Flower","CaveVineFlower","CaveVine Flower","Cavevine Flower",
             "Mandrake","Mandrake Root","MandrakeRoot",
...
-- ══════════════════════════════════════════════════════
local bringHistory = {} -- [key] = { {obj=BasePart, originalCFrame=CFrame, modelParts={{bp,cf},...}} }
    ...
    for _, e in ipairs(hist) do
        pcall(function()
            if e.obj and e.obj.Parent then
                e.obj.CFrame = e.originalCFrame
                e.obj.Velocity = Vector3.zero
                if e.modelParts then
                    for _, mp in ipairs(e.modelParts) do
                        pcall(function() if mp.bp and mp.bp.Parent then mp.bp.CFrame = mp.originalCF; mp.bp.Velocity = Vector3.zero end end)
                    end
                end
                restored += 1
            end
...
local function executarBring(key)
    ...
    for _,obj in ipairs(descs) do
        ...
        pcall(function()
            ...
            -- Salva posição original antes de mover
            local originalCF = targetPart.CFrame
            -- Coloca 3 studs ao lado do jogador com pequena variação
            local rightVec = cf.RightVector
            local spread = rightVec * 3 + Vector3.new(math.random(-1,1)*0.4, 0.5 + math.random()*1.5, math.random(-1,1)*0.4)
            local target=cf.Position+spread
            for _,s in ipairs(obj:GetDescendants()) do if s:IsA("Script") or s:IsA("LocalScript") then pcall(function() s.Disabled=true end) end end
            targetPart.CFrame=CFrame.new(target); targetPart.Velocity=Vector3.zero
            pcall(function() targetPart.CanCollide=true end)
            if obj:IsA("Model") then
                for _,bp in ipairs(obj:GetDescendants()) do
                    if bp:IsA("BasePart") and bp ~= targetPart then
                        pcall(function() bp.CFrame = CFrame.new(target + Vector3.new(math.random(-1,1)*0.1,0,math.random(-1,1)*0.1)) end)
                    end
                end
            end
            count+=1
            ...
            if ok then
                ...
                for i,obj in ipairs(descs) do
                    ...
                    pcall(function()
                        ...
                        local rightVec = cf.RightVector
                        local spread = rightVec * 3 + Vector3.new(math.random(-1,1)*0.5, 0.5 + math.random()*1.5, math.random(-1,1)*0.5)
                        local target=cf.Position+spread
                        ...
                        count+=1
                        local entry = {obj=obj, originalCFrame=originalCF, pos=target}
...
local function applyJump(v)
    pcall(function()
        ...
        hum.JumpPower = jumpEnabled and v or 50
    end)
end
...
-- ══════════════════════════════════════════════════════
--  FLY ANTI-CHEAT BYPASS — 99 Dias na Floresta 2026
--  Técnica: PlatformStand + CFrame increment + BodyForce
--  gravity cancel + humanoid state override
-- ══════════════════════════════════════════════════════
local function setFly(state)
    flyEnabled = state
    if state then
        local ch=Player.Character; if not ch then flyEnabled=false; return end
        local hrp=ch:FindFirstChild("HumanoidRootPart"); if not hrp then flyEnabled=false; return end
        local hum=ch:FindFirstChildWhichIsA("Humanoid"); if not hum then flyEnabled=false; return end

        -- Limpa instâncias antigas
        pcall(function() if flyBodyVel and flyBodyVel.Parent then flyBodyVel:Destroy() end end)
        pcall(function() if flyBodyGyro and flyBodyGyro.Parent then flyBodyGyro:Destroy() end end)
        if flyConn then flyConn:Disconnect(); flyConn=nil end

        -- BYPASS PASSO 1: PlatformStand desativa física do servidor sobre o char
        hum.PlatformStand = true
        -- Garante que o humanoid não "caia" detectavelmente
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.Physics) end)

        -- BYPASS PASSO 2: BodyForce cancela a gravidade sem alterar velocity detectavelmente
        local gravCancel = Instance.new("BodyForce", hrp)
        gravCancel.Name = "FlyGravCancel"
        gravCancel.Force = Vector3.new(0, workspace.Gravity * (hrp.AssemblyMass or 20), 0)
        flyBodyVel = gravCancel

        -- BYPASS PASSO 3: BodyGyro suave apenas no eixo Y (menos suspeito)
        ...
        -- BYPASS PASSO 4: Movimento via CFrame incremental no RenderStepped (client-side puro)
        -- O servidor não detecta pequenos CFrame deltas frame a frame como teleporte
        flyConn = RunService.RenderStepped:Connect(function(dt)
            ...
            local hum2 = c2:FindFirstChildWhichIsA("Humanoid"); if not hum2 then return end

            -- Mantém PlatformStand ativo a cada frame (alguns anti-cheats revertem)
            if not hum2.PlatformStand then
                hum2.PlatformStand = true
                pcall(function() hum2:ChangeState(Enum.HumanoidStateType.Physics) end)
            end

            -- Atualiza cancel de gravidade com massa atual
            pcall(function()
                if gravCancel and gravCancel.Parent then
                    gravCancel.Force = Vector3.new(0, workspace.Gravity * (h2.AssemblyMass or 20), 0)
                end
            end)
            ...
            if UIS:IsKeyDown(Enum.KeyCode.W) then
                dir = dir + Vector3.new(cam.CFrame.LookVector.X, 0, cam.CFrame.LookVector.Z)
            end
            if UIS:IsKeyDown(Enum.KeyCode.S) then
                dir = dir - Vector3.new(cam.CFrame.LookVector.X, 0, cam.CFrame.LookVector.Z)
            end
            if UIS:IsKeyDown(Enum.KeyCode.A) then
                dir = dir - Vector3.new(cam.CFrame.RightVector.X, 0, cam.CFrame.RightVector.Z)
            end
            ...
            if dir.Magnitude > 0 then
                -- Movimento incremental suave: pequeno delta * flySpeed por frame
                local moveVec = dir.Unit * flySpeed * dt
                h2.CFrame = h2.CFrame + moveVec
                -- Mantém velocidade zero para não acumular momentum detectável
                pcall(function() h2.AssemblyLinearVelocity = Vector3.zero end)
                -- Rotação do gyro acompanha câmera horizontalmente
                if bg and bg.Parent then
                    local lookFlat = Vector3.new(cam.CFrame.LookVector.X, 0, cam.CFrame.LookVector.Z)
                    if lookFlat.Magnitude > 0.01 then
                        bg.CFrame = CFrame.new(h2.Position, h2.Position + lookFlat)
                    end
                end
            else
                -- Parado: zera velocidade para não deslizar
                pcall(function() h2.AssemblyLinearVelocity = Vector3.zero end)
            end
        end)
        ...
        -- Desativa fly
        if flyConn then flyConn:Disconnect(); flyConn=nil end
        pcall(function() if flyBodyVel and flyBodyVel.Parent then flyBodyVel:Destroy(); flyBodyVel=nil end end)
        pcall(function() if flyBodyGyro and flyBodyGyro.Parent then flyBodyGyro:Destroy(); flyBodyGyro=nil end end)
...
-- Toggle Velocidade
makePlToggle(T("plSpeedTitle").." ON/OFF", "Ativa/desativa boost de velocidade", Color3.fromRGB(255,180,30), function(s)
    speedEnabled = s
    applySpeed(playerSpeed)
    if s then Notify.success("Velocidade", "Boost de velocidade ativado!") else Notify.info("Velocidade", "Velocidade restaurada ao padrão.") end
end)
makeSliderBar(Pages["Player"], T("plSpeedTitle"), T("plSpeedDesc"), Color3.fromRGB(255,180,30), 16, 275, 30, function(v) playerSpeed=v; if speedEnabled then applySpeed(v) end end)
-- Toggle Pulo
makePlToggle(T("plJumpTitle").." ON/OFF", "Ativa/desativa boost de pulo", Color3.fromRGB(100,220,255), function(s)
    jumpEnabled = s
    applyJump(playerJump)
    if s then Notify.success("Pulo", "Boost de pulo ativado!") else Notify.info("Pulo", "Pulo restaurado ao padrão.") end
end)
makeSliderBar(Pages["Player"], T("plJumpTitle"), T("plJumpDesc"), Color3.fromRGB(100,220,255), 50, 1285, 80, function(v) playerJump=v; if jumpEnabled then applyJump(v) end end)
...
makeSliderBar(Pages["Player"], T("plFlySpeedTitle"), T("plFlySpeedDesc"), Color3.fromRGB(120,200,255), 16, 345, 40, function(v) flySpeed=v end)
...
local function loadKaRemote()
    pcall(function()
        KA_REMOTE = game:GetService("ReplicatedStorage").RemoteEvents.ToolDamageObject
    end)
end
task.delay(1, loadKaRemote)

local function getKaWeapon()
    -- Pega a arma EQUIPADA no personagem
    local ch = Player.Character
    local equippedTool = ch and ch:FindFirstChildWhichIsA("Tool")
    if not equippedTool then return nil end
    -- Busca o item equivalente no Inventory pelo nome (como o jogo espera)
    local inv = Player:FindFirstChild("Inventory")
    if inv then
        local invItem = inv:FindFirstChild(equippedTool.Name)
        if invItem then return invItem end
    end
    -- Fallback: retorna a própria tool equipada
    return equippedTool
end

local function generateAttackId()
    return "1_" .. tostring(math.floor(math.random(1000000000, 9999999999)))
end

-- Dispara dano para todos os mobs no range a cada clique
local function onWeaponClick()
    if not kaEnabled then return end
    if not KA_REMOTE then loadKaRemote(); return end
    local weapon = getKaWeapon()
    if not weapon then
        Notify.warn("Kill Aura", "⚠️ Nenhuma arma no Inventory!")
        return
    end
    local ch = Player.Character
    ...
    local charFolder = workspace:FindFirstChild("Characters")
    local source = charFolder or workspace
    for _, obj in ipairs(source:GetChildren()) do
        pcall(function()
            ...
            if d > kaRange then return end
            KA_REMOTE:InvokeServer(obj, weapon, generateAttackId(), mobHrp.CFrame)
        end)
    end
end
...
local function connectToEquippedTool(tool)
    ...
    kaToolConn = tool.Activated:Connect(onWeaponClick)
    Notify.info("Kill Aura", "⚔️ Conectado: "..tool.Name.." — clique para acertar todos!")
end
...
kaCard.BorderSizePixel = 0; kaCard.Size = UDim2.new(1,0,0,68)
...
kaTitleLbl.Position = UDim2.new(0,14,0,6); kaTitleLbl.Size = UDim2.new(1,-80,0,18)
...
kaDescLbl.Position = UDim2.new(0,14,0,26); kaDescLbl.Size = UDim2.new(1,-80,0,36)
...
kaBtnClick.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(kaKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Position=kaEnabled and UDim2.new(1,-24,0.5,-11) or UDim2.new(0,2,0.5,-11),
...
local kaRangeCard = Instance.new("Frame", Pages["Farm"])
kaRangeCard.BackgroundColor3 = Color3.fromRGB(22,18,18)
kaRangeCard.BorderSizePixel = 0; kaRangeCard.Size = UDim2.new(1,0,0,54)
kaRangeCard.LayoutOrder = fNextLO(); kaRangeCard.ZIndex = 5
Instance.new("UICorner",kaRangeCard).CornerRadius = UDim.new(0,9)
Instance.new("UIStroke",kaRangeCard).Color = Color3.fromRGB(80,30,30)

local kaRLabel = Instance.new("TextLabel",kaRangeCard); kaRLabel.BackgroundTransparency = 1
kaRLabel.Position = UDim2.new(0,14,0,6); kaRLabel.Size = UDim2.new(0.5,0,0,18)
kaRLabel.Font = Enum.Font.GothamBold; TL(kaRLabel, "kaRangeLabel")
kaRLabel.TextColor3 = KA_COR; kaRLabel.TextSize = 12
kaRLabel.TextXAlignment = Enum.TextXAlignment.Left; kaRLabel.ZIndex = 7

local kaRValLbl = Instance.new("TextLabel",kaRangeCard); kaRValLbl.BackgroundTransparency = 1
kaRValLbl.Position = UDim2.new(0,14,0,28); kaRValLbl.Size = UDim2.new(0.4,0,0,18)
kaRValLbl.Font = Enum.Font.GothamBlack; kaRValLbl.Text = tostring(kaRange).." st"
kaRValLbl.TextColor3 = Color3.fromRGB(255,255,255); kaRValLbl.TextSize = 14
kaRValLbl.TextXAlignment = Enum.TextXAlignment.Left; kaRValLbl.ZIndex = 7

local kaMinus = Instance.new("TextButton",kaRangeCard); kaMinus.BackgroundColor3 = Color3.fromRGB(60,20,20)
kaMinus.BorderSizePixel = 0; kaMinus.Position = UDim2.new(1,-110,0.5,-16); kaMinus.Size = UDim2.new(0,32,0,32)
kaMinus.Text = "-"; kaMinus.TextColor3 = KA_COR; kaMinus.Font = Enum.Font.GothamBlack; kaMinus.TextSize = 18; kaMinus.ZIndex = 8
Instance.new("UICorner",kaMinus).CornerRadius = UDim.new(0,8)
kaMinus.MouseButton1Click:Connect(function()
    kaRange = math.max(2, kaRange - 5)
    kaRValLbl.Text = tostring(kaRange).." st"
end)

local kaPlus = Instance.new("TextButton",kaRangeCard); kaPlus.BackgroundColor3 = Color3.fromRGB(80,20,20)
kaPlus.BorderSizePixel = 0; kaPlus.Position = UDim2.new(1,-70,0.5,-16); kaPlus.Size = UDim2.new(0,32,0,32)
kaPlus.Text = "+"; kaPlus.TextColor3 = KA_COR; kaPlus.Font = Enum.Font.GothamBlack; kaPlus.TextSize = 18; kaPlus.ZIndex = 8
Instance.new("UICorner",kaPlus).CornerRadius = UDim.new(0,8)
kaPlus.MouseButton1Click:Connect(function()
    kaRange = math.min(125, kaRange + 5)
    kaRValLbl.Text = tostring(kaRange).." st"
end)

local kaReset = Instance.new("TextButton",kaRangeCard); kaReset.BackgroundColor3 = Color3.fromRGB(70,25,25)
kaReset.BorderSizePixel = 0; kaReset.Position = UDim2.new(1,-34,0.5,-10); kaReset.Size = UDim2.new(0,28,0,20)
kaReset.Text = "↺"; kaReset.TextColor3 = Color3.fromRGB(255,160,160); kaReset.Font = Enum.Font.GothamBold; kaReset.TextSize = 13; kaReset.ZIndex = 8
Instance.new("UICorner",kaReset).CornerRadius = UDim.new(0,6)
kaReset.MouseButton1Click:Connect(function()
    kaRange = 30
    kaRValLbl.Text = "30 st"
end)
