...
local bringDestMode = nil  -- nil = sem modo selecionado (trancado), "jogador"/"fogueira"/"ceu"
    ...
    task.spawn(function()
        while true do
            ...
            pcall(function()
                ...
                starLbl.ZIndex = 531
                -- corre para a direita — fica visível durante toda a viagem
                local dur2 = math.random(10,18)/10
                local driftY = startY + math.random(-8,8)
                -- entra visível, sai fade no final
                starLbl.TextTransparency = 0.85  -- começa quase invisível
                TweenService:Create(starLbl, TweenInfo.new(dur2*0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    {TextTransparency = 0.05}):Play()  -- fade in rápido
                TweenService:Create(starFrame, TweenInfo.new(dur2, Enum.EasingStyle.Sine), {
                    Position = UDim2.new(0, NOTIF_CFG.WIDTH+14, 0, driftY)
                }):Play()
                -- fade out no final (nos últimos 30%)
                task.delay(dur2*0.7, function()
                    if starLbl and starLbl.Parent then
                        TweenService:Create(starLbl, TweenInfo.new(dur2*0.3, Enum.EasingStyle.Quad), {TextTransparency=1}):Play()
                    end
                end)
                game:GetService("Debris"):AddItem(starFrame, dur2+0.1)
            end)
        end
    end)
...
-- ── TopBar Buttons — circular, coloridos, custom ─────────────────
local TopBtns = {}
local _topBtnCfg = {
    -- {name, bg normal, bg hover, symbol, symbolSize}
    {name="Theme",    bgN=Color3.fromRGB(130,60,200),  bgH=Color3.fromRGB(160,80,240),  sym="✦", sz=13},
    {name="Minimize", bgN=Color3.fromRGB(80,80,80),    bgH=Color3.fromRGB(110,110,110), sym="─", sz=11},
...
for i = #_topBtnCfg, 1, -1 do
    ...
    sym.Font = Enum.Font.GothamBlack; sym.Text = d.sym
...
local function makePlKACard(COR, titleTxt, descTxt, minV, maxV, defV, maxStr, getDescFn, initEnabled, onToggle, onSlider)
    ...
    cardG.Color=ColorSequence.new({
        ColorSequenceKeypoint.new(0,Color3.fromRGB(
            math.clamp(r//7+18,18,36), math.clamp(g//8+10,10,24), math.clamp(b//8+4,4,16)
        )),
...
cfgPopup.Name = "BringStylePopup"; cfgPopup.BackgroundColor3 = Color3.fromRGB(20,12,4)
cfgPopup.BorderSizePixel=0; cfgPopup.ZIndex=310; cfgPopup.Visible=false; cfgPopup.Size=UDim2.new(0,210,0,0)
Instance.new("UICorner",cfgPopup).CornerRadius=UDim.new(0,14)
local cfgPopupStroke=Instance.new("UIStroke",cfgPopup)
cfgPopupStroke.Color=CFG_COR; cfgPopupStroke.Thickness=2.5; cfgPopupStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
local cfgPopupG=Instance.new("UIGradient",cfgPopup)
cfgPopupG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(32,18,8)),ColorSequenceKeypoint.new(1,Color3.fromRGB(14,8,2))}); cfgPopupG.Rotation=135
-- Cabeçalho
local cfgPopHdr=Instance.new("Frame",cfgPopup); cfgPopHdr.BackgroundColor3=CFG_COR
cfgPopHdr.BackgroundTransparency=0.85; cfgPopHdr.BorderSizePixel=0; cfgPopHdr.Size=UDim2.new(1,0,0,32); cfgPopHdr.ZIndex=311
Instance.new("UICorner",cfgPopHdr).CornerRadius=UDim.new(0,12)
local cfgPopHdrFix=Instance.new("Frame",cfgPopHdr); cfgPopHdrFix.BackgroundColor3=CFG_COR
cfgPopHdrFix.BackgroundTransparency=0.85; cfgPopHdrFix.BorderSizePixel=0
cfgPopHdrFix.Position=UDim2.new(0,0,0.5,0); cfgPopHdrFix.Size=UDim2.new(1,0,0.5,0); cfgPopHdrFix.ZIndex=311
local cfgPopTitle=Instance.new("TextLabel",cfgPopHdr); cfgPopTitle.BackgroundTransparency=1
cfgPopTitle.Position=UDim2.new(0,12,0,0); cfgPopTitle.Size=UDim2.new(1,-16,1,0)
cfgPopTitle.Font=Enum.Font.GothamBlack; cfgPopTitle.Text="📐  Estilo de Bring"
cfgPopTitle.TextColor3=CFG_COR; cfgPopTitle.TextSize=11
cfgPopTitle.TextXAlignment=Enum.TextXAlignment.Left; cfgPopTitle.ZIndex=312
-- Lista
local cfgPopList=Instance.new("Frame",cfgPopup); cfgPopList.BackgroundTransparency=1
cfgPopList.BorderSizePixel=0; cfgPopList.Position=UDim2.new(0,8,0,38)
cfgPopList.Size=UDim2.new(1,-16,0,0); cfgPopList.ZIndex=311; cfgPopList.AutomaticSize=Enum.AutomaticSize.Y
local cfgPopLayout=Instance.new("UIListLayout",cfgPopList); cfgPopLayout.Padding=UDim.new(0,5); cfgPopLayout.SortOrder=Enum.SortOrder.LayoutOrder
...
local function openPopup()
    ...
    local popH = 32 + #STYLE_OPTIONS*55 + 8
    cfgPopup.Position=UDim2.new(0,ap.X+as.X-210,0,ap.Y-popH-6)
    cfgPopup.Size=UDim2.new(0,210,0,0); cfgPopup.BackgroundTransparency=1; cfgPopup.Visible=true
    TweenService:Create(cfgPopup,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Size=UDim2.new(0,210,0,popH),BackgroundTransparency=0
    }):Play()
    cfgPopOpen=true
end
...
for i, opt in ipairs(STYLE_OPTIONS) do
    local ob=Instance.new("TextButton",cfgPopList); ob.BackgroundColor3=Color3.fromRGB(30,18,6)
    ob.BackgroundTransparency=0; ob.BorderSizePixel=0
    ob.Size=UDim2.new(1,0,0,50); ob.LayoutOrder=i; ob.ZIndex=312; ob.AutoButtonColor=false
    Instance.new("UICorner",ob).CornerRadius=UDim.new(0,10)
    local obS=Instance.new("UIStroke",ob); obS.Color=opt.cor; obS.Thickness=1.5; obS.Transparency=0.6
    local obG=Instance.new("UIGradient",ob); obG.Rotation=135
    obG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(42,24,8)),ColorSequenceKeypoint.new(1,Color3.fromRGB(20,10,3))})
    local obBar=Instance.new("Frame",ob); obBar.BackgroundColor3=opt.cor; obBar.BorderSizePixel=0
    ...
    ob.MouseButton1Click:Connect(function()
        ...
        closePopup()
        -- Refresh highlights of all buttons
        for _, entry in pairs(cfgOptBtns) do
            local sel2=(bringStyle==opt.key and entry==cfgOptBtns[opt.key])
            TweenService:Create(entry.stroke,TweenInfo.new(0.15),{Transparency=sel2 and 0 or 0.6}):Play()
        end
        Notify.send({type="custom",icon=opt.icon,accent=opt.cor,title="Estilo de Bring",msg=opt.label.." — "..opt.desc,duration=3})
    end)
end
-- Espaço inferior
