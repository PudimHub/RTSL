...
local godTakeDmgHook = nil  -- hook de TakeDamage
local godNamecallHook = nil -- hook de __namecall
...
local function setGodMod(state)
    ...
    if state then
        ...
        local function applyGodModToChar(ch)
            ...
            -- ── Método 1: Hook direto em TakeDamage (melhor método) ──
            -- Substitui TakeDamage por função vazia — mob chama TakeDamage
            -- mas nada acontece
            pcall(function()
                if hookfunction then
                    -- Remove hook anterior se existir
            ...
            -- ── Método 2: Hook em __namecall (captura TakeDamage pelo metatable) ──
            pcall(function()
                if hookmetamethod and not godNamecallHook then
                    local orig = hookmetamethod(game, "__namecall", function(self, ...)
                        if not godModEnabled then return orig(self, ...) end
                        local method = getnamecallmethod()
                        -- Bloqueia qualquer chamada de dano no Humanoid do player
                        if method == "TakeDamage" then
                            local playerChar = Player.Character
                            if playerChar then
                                local playerHum = playerChar:FindFirstChildWhichIsA("Humanoid")
                                if self == playerHum then
                    ...
                    godNamecallHook = orig
                end
            end)

            -- ── Método 3: MaxHealth gigante + restauração no HealthChanged ──
            -- Backup caso os hooks não estejam disponíveis no executor
            pcall(function()
                hum.MaxHealth = math.huge
                hum.Health    = math.huge
            end)

            -- HealthChanged: restaura imediatamente se cair
            local hcConn
            ...
            -- ── Método 4: Heartbeat — restaura a cada frame ──
            -- Cobre casos onde o servidor força Health = 0 antes de matar
            ...
            godModConn = RunService.Heartbeat:Connect(function()
                ...
                pcall(function()
                    ...
                    -- Se Health caiu, restaura instantaneamente
                    if h2.Health < h2.MaxHealth and h2.Health > 0 then
                        h2.Health = h2.MaxHealth
                    end
                    -- Se morreu (Health == 0), tenta ressuscitar
                    if h2.Health <= 0 then
                        pcall(function()
        ...
        Notify.send({type="custom", icon="♾️", accent=Color3.fromRGB(140,255,140),
            title="God Mod ATIVO",
            msg="TakeDamage bloqueado + HP infinito ativo!",
            duration=5})
    else
        -- Desativa tudo
        if godModConn then godModConn:Disconnect(); godModConn = nil end
        ...
        -- Remove hook de namecall
        pcall(function()
            if hookmetamethod and godNamecallHook then
                hookmetamethod(game, "__namecall", godNamecallHook)
                godNamecallHook = nil
            end
        end)

        -- Restaura TakeDamage original no Humanoid
        pcall(function()
            if hookfunction and godTakeDmgHook then
                ...
                if ch then
                    local hum = ch:FindFirstChildWhichIsA("Humanoid")
                    if hum then
                        hookfunction(hum.TakeDamage, godTakeDmgHook)
                    end
                end
                godTakeDmgHook = nil
            end
...
-- Helper: botão de TP compacto (grid 2 colunas)
-- ── Grid container para botões de bioma ─────────────────────────
-- Os botões de bioma ficam em grade 2 colunas, compactos (42px)
-- Em vez de cards largos de 58px cada, ficam 2 por linha = 50% menos altura

local _tpGridContainer = nil  -- container atual do grid
local _tpGridLeft = true      -- próximo botão vai para esquerda ou direita?

local function makeTpGridSection()
    -- Cria um novo container de grade para os próximos botões
    _tpGridContainer = Instance.new("Frame", Pages["Teleportar"])
    _tpGridContainer.BackgroundTransparency = 1
    _tpGridContainer.BorderSizePixel = 0
    _tpGridContainer.Size = UDim2.new(1, 0, 0, 0)
    _tpGridContainer.AutomaticSize = Enum.AutomaticSize.Y
    _tpGridContainer.LayoutOrder = tpNextLO()
    _tpGridContainer.ZIndex = 5
    local gl = Instance.new("UIGridLayout", _tpGridContainer)
    gl.CellSize = UDim2.new(0.5, -3, 0, 54)
    gl.CellPadding = UDim2.new(0, 6, 0, 6)
    gl.SortOrder = Enum.SortOrder.LayoutOrder
    gl.FillDirection = Enum.FillDirection.Horizontal
    _tpGridLeft = true
    return _tpGridContainer
end

local function makeTpBtn(icon, titleTxt, descTxt, cor, onClick)
    -- Se não tem grid container, cria um
    if not _tpGridContainer then makeTpGridSection() end

    local lo = #_tpGridContainer:GetChildren()
    local card = Instance.new("TextButton", _tpGridContainer)
    ...
    stroke.Color = Color3.fromRGB(42, 46, 60)
    stroke.Thickness = 1.2

    -- Glow de fundo (accent sutil)
    local glow = Instance.new("Frame", card)
    glow.BackgroundColor3 = cor; glow.BackgroundTransparency = 0.92
    glow.BorderSizePixel = 0; glow.Size = UDim2.new(1,0,1,0); glow.ZIndex = 5
    Instance.new("UICorner", glow).CornerRadius = UDim.new(0, 10)

    -- Barra de cor no topo
    local topBar = Instance.new("Frame", card)
    topBar.BackgroundColor3 = cor; topBar.BorderSizePixel = 0
    topBar.Size = UDim2.new(1, 0, 0, 3); topBar.ZIndex = 6
    ...
    iconLbl.Text = icon; iconLbl.TextSize = 20; iconLbl.ZIndex = 7

    -- Nome do TP
    local nameLbl = Instance.new("TextLabel", card)
    nameLbl.BackgroundTransparency = 1
    nameLbl.Position = UDim2.new(0, 10, 0, 33)
    nameLbl.Size = UDim2.new(1, -14, 0, 14)
    nameLbl.Font = Enum.Font.GothamBold
    nameLbl.Text = titleTxt
    ...
    card.MouseEnter:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.12), {BackgroundColor3=Color3.fromRGB(28,32,46)}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.12), {Color=cor, Transparency=0.2}):Play()
        TweenService:Create(glow, TweenInfo.new(0.12), {BackgroundTransparency=0.82}):Play()
    end)
    card.MouseLeave:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.12), {BackgroundColor3=Color3.fromRGB(22,24,34)}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.12), {Color=Color3.fromRGB(42,46,60), Transparency=0}):Play()
        TweenService:Create(glow, TweenInfo.new(0.12), {BackgroundTransparency=0.92}):Play()
    end)
    card.MouseButton1Click:Connect(function()
        -- Flash rápido de click
        TweenService:Create(topBar, TweenInfo.new(0.08), {BackgroundTransparency=0.3}):Play()
...
-- Fecha o grid atual e inicia um novo após uma seção
local function makeTpSec(titleTxt, cor)
    _tpGridContainer = nil  -- próximo makeTpBtn cria novo grid
