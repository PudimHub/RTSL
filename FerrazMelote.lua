...
local ESP_CATS = {
    ...
    {key="Animais", label="🐾 Animals", cor=Color3.fromRGB(130,220,100), tipo="entity", alcance=700,
...
local bringGroupOrder={
    ...
    {"BRING — Specials", Color3.fromRGB(255,230,80), {"BChaves","BBlueprint"}},
}
...
kaBtnClick.MouseButton1Click:Connect(function()
    ...
    if kaEnabled then
        ...
        Notify.send({type="custom",icon="⚔️",accent=KA_COR,
            title="Kill Aura",
            msg="Ativo! Equipe uma arma e clique para acertar todos no range ("..kaRange.." st).",
            duration=5})
    else
        stopKillAura()
        Notify.info("Kill Aura", "Desativado.")
    end
end)
