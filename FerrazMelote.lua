...
local function makeMultiDropdown(parent, yPos, cfg)
    ...
    -- ── Filtro de busca ───────────────────────────────────────
    -- Cada item registra sua row para visibilidade (chave=item.key)
    local _itemVisRows = {}  -- key -> {row, catHdrFrame, catKey}
    -- Preenchido abaixo na loop de itens via patch retroativo
...
_eatBtn.MouseButton1Click:Connect(function()
    if eatRunning then
        ...
        if _eatBtn._setState then _eatBtn._setState(false) end
...
local function autoFeedCampfire()
    ...
    afcRunning=false
    pcall(function() if afcRef.btn then if afcRef.btn._setState then afcRef.btn._setState(false) end end end)
end
...
_afBtn.MouseButton1Click:Connect(function()
    ...
    if afcRunning then
        afcRunning=false; if _afBtn._setState then _afBtn._setState(false) end
        ...
        if _afBtn._setState then _afBtn._setState(true) end
        task.spawn(function() autoFeedCampfire()
            if not afcRunning then pcall(function() if _afBtn._setState then _afBtn._setState(false) end end) end
        end)
    end
end)
end
...
_afhpBtn.MouseButton1Click:Connect(function()
    if afhpRunning then
        afhpRunning=false; if _afhpBtn._setState then _afhpBtn._setState(false) end
        ...
        if _afhpBtn._setState then _afhpBtn._setState(true) end
        task.spawn(function() autoFeedHP()
            if not afhpRunning then pcall(function() if _afhpBtn._setState then _afhpBtn._setState(false) end end) end
        end)
    end
end)
end
...
local function autoCookFood()
    ...
    ackRunning=false
    pcall(function() if ackRef.btn then if ackRef.btn._setState then ackRef.btn._setState(false) end end end)
end
...
local function autoMachineGrind()
    ...
    amgRunning=false
    pcall(function() if amgRef.btn then if amgRef.btn._setState then amgRef.btn._setState(false) end end end)
end
...
local function autoBiofuel()
    ...
    pcall(function() if abfRef.btn then if abfRef.btn._setState then abfRef.btn._setState(false) end end end)
end
...
local function autoFarmBaus()
    ...
    fbcRunning=false
    pcall(function() if _fbcBtn._setState then _fbcBtn._setState(false) end end)
    pcall(function() _fbcStatus.Text="" end)
end
_fbcBtn.MouseButton1Click:Connect(function()
    if fbcRunning then
        fbcRunning=false; if _fbcBtn._setState then _fbcBtn._setState(false) end
        _fbcStatus.Text="⏹ Parado"; Notify.error("Farm Baús","⏹ Desativado")
        task.delay(1.5,function() pcall(function() _fbcStatus.Text="" end) end)
    else
        if _fbcBtn._setState then _fbcBtn._setState(true) end
        task.spawn(function() autoFarmBaus(); if not fbcRunning then pcall(function() if _fbcBtn._setState then _fbcBtn._setState(false) end end) end end)
    end
end)
