...
local function nAddHistory(cfg, tipo)
    ...
    hBadge. hBadge.AnchorPoint=Vector2.new(1,0); hBadge.ZIndex=7
    ...
    hBadgeLbl.Size=UDim2.new(1,0,1,0); hBadgeLbl.
...
-- ══════════════════════════════════════════════════════
;do -- [[ BOOST SYSTEM ]]
local _ok_BOOST_SYSTEM, _err_BOOST_SYSTEM = pcall(function()
...
if not _ok_BOOST_SYSTEM then warn("\n❌❌ ERRO NA ABA [BOOST SYSTEM]:\n" .. tostring(_err_BOOST_SYSTEM)) end
end -- [[ BOOST SYSTEM ]]

-- ABA INFO — includes notification toggle
-- ══════════════════════════════════════════════════════
;do -- [[ INFO TAB ]]
local _ok_INFO_TAB, _err_INFO_TAB = pcall(function()
...
dadosText. dadosText.Font=Enum.Font.Gotham
...
if not _ok_INFO_TAB then warn("\n❌❌ ERRO NA ABA [INFO TAB]:\n" .. tostring(_err_INFO_TAB)) end
end -- [[ INFO TAB ]]
...
-- ══════════════════════════════════════════════════════
;do -- [[ STATUS TAB ]]
local _ok_STATUS_TAB, _err_STATUS_TAB = pcall(function()
-- ══════════════════════════════════════════════════════════════════
...
if not _ok_STATUS_TAB then warn("\n❌❌ ERRO NA ABA [STATUS TAB]:\n" .. tostring(_err_STATUS_TAB)) end
end -- [[ STATUS TAB ]]


-- ══════════════════════════════════════════════════════════════════
;do -- [[ ATUALIZACAO TAB ]]
local _ok_ATUALIZACAO_TAB, _err_ATUALIZACAO_TAB = pcall(function()
-- ══════════════════════════════════════════════════════════════════
    ...
    for gi, grp in ipairs(entry.grupos) do
        ...
        for _, item in ipairs(grp.items) do
            ...
            if not isOld and item.tipo ~= "old" then
                ...
                tagL.Size = UDim2.new(0,0,1,0); tagL.
...
if not _ok_ATUALIZACAO_TAB then warn("\n❌❌ ERRO NA ABA [ATUALIZACAO TAB]:\n" .. tostring(_err_ATUALIZACAO_TAB)) end
end -- [[ ATUALIZACAO TAB ]]
...
-- ══
;do -- [[ ESP + BRING ]]
local _ok_ESP__BRING, _err_ESP__BRING = pcall(function()
-- ESP ICONS (14x14)
-- ══════════════════════════════════════════════════════
...
for idx,opt in ipairs(STYLE_OPTS) do
    ...
    item.LayoutOrder = idx; item.ZIndex = 451; item.
...
do
    ...
    local _bcCard, _bcCF, _bcDrop = makeAccordionCard(Pages["Bring"], bringLO, {
        ...
        color   = BRING_CFG_COR,
        contentH = 36 + 9 + 34 + 8 + 34 + 8 + 34 + 8 + 14,
    })
    -- Guarda referência ao label do título para atualizar depois
    -- Busca o label do título dentro do card (é o TextLabel com Font GothamBlack)
    ...
    local bStyleLbl = Instance.new("TextLabel", _bcCF); bStyleLbl.BackgroundTransparency = 1
    bStyleLbl.Position = UDim2.new(0,10,0,_bcy+8); bStyleLbl.Size = UDim2.new(0,55,0,18)
    ...
    local bStyleBtn = Instance.new("TextButton", _bcCF)
    bStyleBtn.BackgroundColor3       = Color3.fromRGB(255,255,255)
    bStyleBtn.BackgroundTransparency = 0.88
    bStyleBtn.BorderSizePixel        = 0
    bStyleBtn.AnchorPoint            = Vector2.new(1,0)
    bStyleBtn.Position               = UDim2.new(1,-10,0,_bcy)
    bStyleBtn.Size                   = UDim2.new(0,130,0,34)
    ...
    Instance.new("UICorner", bStyleBtn).CornerRadius = UDim.new(0, 9)
    local bStyleBtnS = Instance.new("UIStroke", bStyleBtn)
    bStyleBtnS.Thickness = 1.3; bStyleBtnS.Transparency = 0.25
    bStyleBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    -- Tint colorido (glassmorphism)
    local bStyleTint = Instance.new("Frame", bStyleBtn)
    bStyleTint.BackgroundColor3 = BRING_CFG_COR; bStyleTint.BackgroundTransparency = 0.82
    bStyleTint.BorderSizePixel = 0; bStyleTint.Size = UDim2.new(1,0,1,0); bStyleTint.ZIndex = 7
    Instance.new("UICorner",bStyleTint).CornerRadius = UDim.new(0,9)
    -- Reflexo vidro
    local bStyleGlass = Instance.new("Frame",bStyleBtn)
    bStyleGlass.BackgroundColor3 = Color3.fromRGB(255,255,255); bStyleGlass.BackgroundTransparency = 0.72
    bStyleGlass.BorderSizePixel = 0; bStyleGlass.Position = UDim2.new(0,6,0,2)
    bStyleGlass.Size = UDim2.new(0.5,0,0,1); bStyleGlass.ZIndex = 9
    Instance.new("UICorner",bStyleGlass).CornerRadius = UDim.new(1,0)
    -- Seta
    local bStyleArr = Instance.new("TextLabel",bStyleBtn); bStyleArr.BackgroundTransparency = 1
    bStyleArr.AnchorPoint = Vector2.new(1,0.5); bStyleArr.Position = UDim2.new(1,-6,0.5,0)
    bStyleArr.Size = UDim2.new(0,12,0,14); bStyleArr.Font = Enum.Font.GothamBlack
    bStyleArr.Text = "▾"; bStyleArr.TextSize = 9; bStyleArr.ZIndex = 9
    ...
    local function updateStyleBtn()
        for _, s in ipairs(STYLE_OPTS) do
            if s.key == bringStyle then
                ...
                bStyleTint.BackgroundColor3 = s.cor
                bStyleArr.TextColor3 = s.cor
                return
            end
        end
    ...
    local MODE_COLORS = {
        jogador  = Color3.fromRGB(100,200,255),
        fogueira = Color3.fromRGB(255,120,40),
        ceu      = Color3.fromRGB(140,200,255),
    }

    local bModeLbl = Instance.new("TextLabel", _bcCF); bModeLbl.BackgroundTransparency = 1
    bModeLbl.Position = UDim2.new(0,10,0,_bcy+8); bModeLbl.Size = UDim2.new(0,55,0,18)
    bModeLbl.Font = Enum.Font.GothamBold; bModeLbl.Text = "🎯 Destino"
    bModeLbl.TextColor3 = Color3.fromRGB(200,185,235); bModeLbl.TextSize = 10
    bModeLbl.TextXAlignment = Enum.TextXAlignment.Left; bModeLbl.ZIndex = 8

    local modeBtn = Instance.new("TextButton", _bcCF)
    modeBtn.BackgroundColor3       = Color3.fromRGB(255,255,255)
    modeBtn.BackgroundTransparency = 0.88
    modeBtn.BorderSizePixel        = 0
    modeBtn.AnchorPoint            = Vector2.new(1,0)
    modeBtn.Position               = UDim2.new(1,-10,0,_bcy)
    modeBtn.Size                   = UDim2.new(0,130,0,34)
    modeBtn.Font                   = Enum.Font.GothamBold
    modeBtn.TextSize               = 10; modeBtn.ZIndex = 8
    modeBtn.AutoButtonColor        = false
    modeBtn.TextTruncate           = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", modeBtn).CornerRadius = UDim.new(0,9)
    local modeBtnS = Instance.new("UIStroke", modeBtn)
    modeBtnS.Thickness = 1.3; modeBtnS.Transparency = 0.25
    modeBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local modeTint = Instance.new("Frame",modeBtn)
    modeTint.BackgroundColor3 = BRING_CFG_COR; modeTint.BackgroundTransparency = 0.82
    modeTint.BorderSizePixel = 0; modeTint.Size = UDim2.new(1,0,1,0); modeTint.ZIndex = 7
    Instance.new("UICorner",modeTint).CornerRadius = UDim.new(0,9)
    local modeGlass = Instance.new("Frame",modeBtn)
    modeGlass.BackgroundColor3 = Color3.fromRGB(255,255,255); modeGlass.BackgroundTransparency = 0.72
    modeGlass.BorderSizePixel = 0; modeGlass.Position = UDim2.new(0,6,0,2)
    modeGlass.Size = UDim2.new(0.5,0,0,1); modeGlass.ZIndex = 9
    Instance.new("UICorner",modeGlass).CornerRadius = UDim.new(1,0)
    local modeArr = Instance.new("TextLabel",modeBtn); modeArr.BackgroundTransparency = 1
    modeArr.AnchorPoint = Vector2.new(1,0.5); modeArr.Position = UDim2.new(1,-6,0.5,0)
    modeArr.Size = UDim2.new(0,12,0,14); modeArr.Font = Enum.Font.GothamBlack
    modeArr.Text = "▾"; modeArr.TextSize = 9; modeArr.ZIndex = 9

    local function updateModeBtn()
        local mc = bringDestMode and (MODE_COLORS[bringDestMode] or BRING_CFG_COR) or Color3.fromRGB(180,130,60)
        local icon,lbl = "🔒","Modo"
        for _, o in ipairs(MODE_OPTS) do
            if o.key==bringDestMode then icon=o.icon; lbl=o.label; break end
        end
        modeBtn.Text = icon.."  "..lbl
        modeBtn.TextColor3 = mc; modeBtnS.Color = mc
        modeTint.BackgroundColor3 = mc; modeArr.TextColor3 = mc
    end
    if bringDestMode then updateModeBtn()
    else modeBtn.Text="🔒  Modo"; modeBtn.TextColor3=Color3.fromRGB(180,130,60) end
    ...
    -- ── Linha: Velocidade ────────────────────────────────────────
    local SPEED_OPTS = {
        {key="normal", label="Normal",      icon="🐢", cor=Color3.fromRGB(100,200,255),  desc="Delay padrão (atual)"},
        {key="rapido", label="Rápido",      icon="🐇", cor=Color3.fromRGB(100,240,120),  desc="Delay reduzido"},
        {key="mega",   label="Mega rápido", icon="⚡", cor=Color3.fromRGB(255,200,60),   desc="Instantâneo — sem delay"},
    }

    local bSpeedLbl = Instance.new("TextLabel", _bcCF); bSpeedLbl.BackgroundTransparency = 1
    bSpeedLbl.Position = UDim2.new(0,10,0,_bcy+8); bSpeedLbl.Size = UDim2.new(0,65,0,18)
    bSpeedLbl.Font = Enum.Font.GothamBold; bSpeedLbl.Text = "⚡ Veloc."
    bSpeedLbl.TextColor3 = Color3.fromRGB(200,185,235); bSpeedLbl.TextSize = 10
    bSpeedLbl.TextXAlignment = Enum.TextXAlignment.Left; bSpeedLbl.ZIndex = 8

    local bSpeedBtn = Instance.new("TextButton", _bcCF)
    bSpeedBtn.BackgroundColor3       = Color3.fromRGB(255,255,255)
    bSpeedBtn.BackgroundTransparency = 0.88
    bSpeedBtn.BorderSizePixel        = 0
    bSpeedBtn.AnchorPoint            = Vector2.new(1,0)
    bSpeedBtn.Position               = UDim2.new(1,-10,0,_bcy)
    bSpeedBtn.Size                   = UDim2.new(0,130,0,34)
    bSpeedBtn.Font                   = Enum.Font.GothamBold
    bSpeedBtn.TextSize               = 10; bSpeedBtn.ZIndex = 8
    bSpeedBtn.AutoButtonColor        = false
    bSpeedBtn.TextTruncate           = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", bSpeedBtn).CornerRadius = UDim.new(0,9)
    local bSpeedBtnS = Instance.new("UIStroke", bSpeedBtn)
    bSpeedBtnS.Thickness = 1.3; bSpeedBtnS.Transparency = 0.25
    bSpeedBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local bSpeedTint = Instance.new("Frame",bSpeedBtn)
    bSpeedTint.BackgroundColor3 = BRING_CFG_COR; bSpeedTint.BackgroundTransparency = 0.82
    bSpeedTint.BorderSizePixel = 0; bSpeedTint.Size = UDim2.new(1,0,1,0); bSpeedTint.ZIndex = 7
    Instance.new("UICorner",bSpeedTint).CornerRadius = UDim.new(0,9)
    local bSpeedGlass = Instance.new("Frame",bSpeedBtn)
    bSpeedGlass.BackgroundColor3 = Color3.fromRGB(255,255,255); bSpeedGlass.BackgroundTransparency = 0.72
    bSpeedGlass.BorderSizePixel = 0; bSpeedGlass.Position = UDim2.new(0,6,0,2)
    bSpeedGlass.Size = UDim2.new(0.5,0,0,1); bSpeedGlass.ZIndex = 9
    Instance.new("UICorner",bSpeedGlass).CornerRadius = UDim.new(1,0)
    local bSpeedArr = Instance.new("TextLabel",bSpeedBtn); bSpeedArr.BackgroundTransparency = 1
    bSpeedArr.AnchorPoint = Vector2.new(1,0.5); bSpeedArr.Position = UDim2.new(1,-6,0.5,0)
    bSpeedArr.Size = UDim2.new(0,12,0,14); bSpeedArr.Font = Enum.Font.GothamBlack
    bSpeedArr.Text = "▾"; bSpeedArr.TextSize = 9; bSpeedArr.ZIndex = 9

    local function updateSpeedBtn()
        for _, s in ipairs(SPEED_OPTS) do
            if s.key == bringSpeed then
                bSpeedBtn.Text = s.icon.."  "..s.label
                bSpeedBtn.TextColor3 = s.cor; bSpeedBtnS.Color = s.cor
                bSpeedTint.BackgroundColor3 = s.cor; bSpeedArr.TextColor3 = s.cor
                -- Atualiza título do accordion
                if _bcTitleLbl then
                    _bcTitleLbl.Text = "Bring Configuração ("..s.label..")"
                else
                    pcall(function()
                        local lbl = _bcCard:FindFirstChild("TextLabel",true)
                        if lbl then lbl.Text = "Bring Configuração ("..s.label..")" end
                    end)
                end
                return
            end
    ...
    updateSpeedBtn()  -- inicia com Normal
    _bcy = _bcy + 34 + 8

    makeBringDropdown({
        opts    = SPEED_OPTS,
        getVal  = function() return bringSpeed end,
        setVal  = function(k)
            bringSpeed = k
            updateSpeedBtn()
        end,
        ...
        onSelect = function(opt)
            Notify.send({type="custom",icon=opt.icon,accent=opt.cor,
                title="Velocidade do Bring",msg=opt.desc,duration=2.5})
        end,
    })

    -- Status label no footer do accordion
    local countLb = _accStatusLbl(_bcCF, _bcy + 34 + 8)

    -- Dropdowns (usando makeBringDropdown)
    makeBringDropdown({
        opts    = STYLE_OPTS,
    ...
    makeBringDropdown({
        opts    = (function()
            local t={}
            for _,o in ipairs(MODE_OPTS) do
                table.insert(t,{key=o.key,label=o.label,icon=o.icon,cor=MODE_COLORS[o.key] or BRING_CFG_COR})
            end
            return t
        end)(),
    ...
    -- Atualiza contagem de itens em background
    task.spawn(function()
        while true do
            task.wait(5)
            pcall(function()
...
if not _ok_ESP__BRING then warn("\n❌❌ ERRO NA ABA [ESP + BRING]:\n" .. tostring(_err_ESP__BRING)) end
end -- [[ ESP + BRING ]]
...
-- ══════════════════════════════════════════════════════
;do -- [[ PLAYER TAB ]]
local _ok_PLAYER_TAB, _err_PLAYER_TAB = pcall(function()
...
if not _ok_PLAYER_TAB then warn("\n❌❌ ERRO NA ABA [PLAYER TAB]:\n" .. tostring(_err_PLAYER_TAB)) end
end -- [[ PLAYER TAB ]]
...
-- ══════════════════════════════════════════════════════════════
;do -- [[ TELEPORTAR TAB ]]
local _ok_TELEPORTAR_TAB, _err_TELEPORTAR_TAB = pcall(function()
...
if not _ok_TELEPORTAR_TAB then warn("\n❌❌ ERRO NA ABA [TELEPORTAR TAB]:\n" .. tostring(_err_TELEPORTAR_TAB)) end
end -- [[ TELEPORTAR TAB ]]
...
;do -- [[ FARM PART 1 ]]
local _ok_FARM_PART_1, _err_FARM_PART_1 = pcall(function()
...
if not _ok_FARM_PART_1 then warn("\n❌❌ ERRO NA ABA [FARM PART 1]:\n" .. tostring(_err_FARM_PART_1)) end
end -- [[ FARM PART 1 ]]
;do -- [[ FARM PART 2 ]]
local _ok_FARM_PART_2, _err_FARM_PART_2 = pcall(function()
...
if not _ok_FARM_PART_2 then warn("\n❌❌ ERRO NA ABA [FARM PART 2]:\n" .. tostring(_err_FARM_PART_2)) end
end -- [[ FARM PART 2 ]]

;do -- [[ AIMBOT + ADVANCED ]]
local _ok_AIMBOT__ADVANCED, _err_AIMBOT__ADVANCED = pcall(function()
...
if not _ok_AIMBOT__ADVANCED then warn("\n❌❌ ERRO NA ABA [AIMBOT + ADVANCED]:\n" .. tostring(_err_AIMBOT__ADVANCED)) end
end -- [[ AIMBOT + ADVANCED ]]
