...
local function attackMob(mob)
    ...
    -- (o Activate() simulado não dispara o ataque de verdade nesse jogo —
    -- o hook aprende sozinho assim que você acertar algo com o dedo/mouse).
    -- Não mexemos no Parent da tool aqui pra evitar o "flicker" no inventário.
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.warn("⚔️ Kill Aura", "Ataque um mob MANUALMENTE 1 vez pra o Kill Aura aprender o ID de ataque. Depois disso ele vira automático.", 6)
            end)
        end
        return
    end
...
local function chopTree(treeModel, treePart)
    ...
    -- Sem ID de sessão capturado ainda? Precisa de UM corte manual seu real
    -- (o Activate() simulado não dispara o ataque de verdade nesse jogo —
    -- o hook aprende sozinho assim que você cortar algo com o dedo/mouse).
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.warn("🪓 Tree Aura", "Corte uma árvore MANUALMENTE 1 vez pra o Tree Aura aprender o ID de ataque. Depois disso ele vira automático.", 6)
            end)
        end
        return
    end
