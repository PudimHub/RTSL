...
-- ══════════════════════════════════════════════════════════════
--  PUDIMHUB — Interface v6  ◈ ECLIPSE EDITION
--  Layout: TopBar(44) | Sidebar(182) | ContentArea | StatusBar(26)
--  Features: Barra Busca • Session Timer • Live Stats • Keybind Toggle
--            Animações Spring • Gradient Border Glow • Intro Upgrade
-- ══════════════════════════════════════════════════════════════
...
local SIDE_W  = 182
local BOT_H   = 26
local CNT_H   = WIN_H - TOP_H - BOT_H   -- 410
local TAB_H   = 42  -- compat legado

-- ── PALETA ECLIPSE ────────────────────────────────────────────
local C_BASE     = Color3.fromRGB(  7,   7,  13)
local C_SURFACE  = Color3.fromRGB( 12,  11,  20)
local C_ELEV     = Color3.fromRGB( 19,  17,  32)
local C_CARD     = Color3.fromRGB( 26,  23,  44)
local C_ACCENT   = Color3.fromRGB(120,  80, 255)   -- violet principal
local C_ACCENT2  = Color3.fromRGB( 80, 200, 255)   -- cyan secundário
local C_AMBER    = Color3.fromRGB(255, 175,  40)   -- gold / aviso
local C_RED      = Color3.fromRGB(255,  70,  90)   -- erro / fechar
local C_GREEN    = Color3.fromRGB( 50, 220, 130)   -- sucesso
local C_HI       = Color3.fromRGB(235, 228, 255)   -- texto primário
local C_MID      = Color3.fromRGB(135, 120, 172)   -- texto secundário
local C_LOW      = Color3.fromRGB( 54,  48,  84)   -- texto discreto
local C_STROKE   = Color3.fromRGB( 38,  33,  64)   -- bordas

-- aliases de compatibilidade para código legado
local C_TEXT_OFF   = C_MID
local C_TEXT_ON    = C_HI
local C_BG_HOV     = Color3.fromRGB(28, 25, 46)
local C_BG_ACT     = Color3.fromRGB(22, 18, 40)
local C_ICON_IDLE  = C_LOW
local C_ICON_ACTIVE= C_ACCENT

-- Estado de visibilidade p/ animação do border glow
local _mainVisible  = false
local _gradAngle    = 0
...
-- Gradient animado na borda (efeito "aurora")
local _borderGrad = Instance.new("UIGradient", MainStroke)
_borderGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.00, C_ACCENT),
    ColorSequenceKeypoint.new(0.42, C_ACCENT2),
    ColorSequenceKeypoint.new(0.78, C_AMBER),
    ColorSequenceKeypoint.new(1.00, C_ACCENT),
})
_borderGrad.Rotation = 0

-- Glow interno sutil
local _innerGlow = Instance.new("Frame", MainFrame)
_innerGlow.BackgroundColor3 = C_ACCENT
_innerGlow.BackgroundTransparency = 0.93
_innerGlow.BorderSizePixel = 0
_innerGlow.Size = UDim2.new(1, 0, 1, 0)
_innerGlow.ZIndex = 2
Instance.new("UICorner", _innerGlow).CornerRadius = UDim.new(0, 14)

-- Animação contínua: girar gradient + pulsar borda
RunService.RenderStepped:Connect(function(dt)
    if not _mainVisible then return end
    _gradAngle = (_gradAngle + dt * 55) % 360
    _borderGrad.Rotation = _gradAngle
    local pulse = 0.42 + 0.22 * math.sin(tick() * 1.8)
...
-- Gradient na topbar
local _topGrad = Instance.new("UIGradient", TopBar)
_topGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.0, Color3.fromRGB(16, 13, 28)),
    ColorSequenceKeypoint.new(1.0, Color3.fromRGB(9,  9, 16)),
})
_topGrad.Rotation = 90
...
-- Linha accent colorida sob a topbar (apenas na área de conteúdo)
local _topAccentLine = Instance.new("Frame", MainFrame)
_topAccentLine.BackgroundColor3 = C_ACCENT
_topAccentLine.BackgroundTransparency = 0.55
_topAccentLine.BorderSizePixel = 0
_topAccentLine.Position = UDim2.new(0, SIDE_W + 1, 0, TOP_H - 1)
_topAccentLine.Size     = UDim2.new(1, -(SIDE_W + 1), 0, 2)
_topAccentLine.ZIndex   = 7
Instance.new("UICorner", _topAccentLine).CornerRadius = UDim.new(1, 0)

-- Ícone do logo (animado)
local _logoIcon = Instance.new("TextLabel", TopBar)
_logoIcon.BackgroundTransparency = 1
_logoIcon.Position  = UDim2.new(0, 13, 0.5, -9)
_logoIcon.Size      = UDim2.new(0, 18, 0, 18)
_logoIcon.Font      = Enum.Font.GothamBlack
_logoIcon.Text      = "◈"
_logoIcon.TextColor3 = C_ACCENT
_logoIcon.TextSize  = 14
_logoIcon.ZIndex    = 9

-- Pulso do ícone
task.spawn(function()
    while true do
        TweenService:Create(_logoIcon, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {TextTransparency=0.4}):Play()
        task.wait(1.4)
        TweenService:Create(_logoIcon, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {TextTransparency=0}):Play()
        task.wait(1.4)
    end
end)
...
VerBg.BackgroundTransparency = 0.82
VerBg.BorderSizePixel        = 0
VerBg.AnchorPoint            = Vector2.new(0, 0.5)
VerBg.Position               = UDim2.new(0, 168, 0.5, 0)
VerBg.Size                   = UDim2.new(0, 0, 0, 18)
VerBg.AutomaticSize          = Enum.AutomaticSize.X
VerBg.ZIndex                 = 9
Instance.new("UICorner", VerBg).CornerRadius = UDim.new(0, 5)
local _vbStroke = Instance.new("UIStroke", VerBg); _vbStroke.Color = C_ACCENT; _vbStroke.Thickness = 1; _vbStroke.Transparency = 0.5
local _vbP = Instance.new("UIPadding", VerBg)
_vbP.PaddingLeft = UDim.new(0, 7); _vbP.PaddingRight = UDim.new(0, 7)
...
VerLbl.Text         = "v6  ECLIPSE"
VerLbl.TextColor3   = C_ACCENT
VerLbl.TextSize     = 9
VerLbl.ZIndex       = 10

-- ── SESSION TIMER (centro da TopBar) ─────────────────────────
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            SessionLbl.Text = string.format("⏱  %02d:%02d:%02d", h, m, s)
        end)
    end
end)
...
do
    local _defs = {
        {name="Theme",   ch="⊙", tooltip="Quick Actions", hov=C_AMBER},
        {name="Minimize",ch="—",  tooltip="Minimizar",    hov=C_ACCENT2},
        {name="Maximize",ch="⊡", tooltip="Maximizar",    hov=C_ACCENT},
        {name="Close",   ch="✕", tooltip="Fechar",       hov=C_RED},
    }
    local _bx = WIN_W - 8
    for _i = #_defs, 1, -1 do
        local _d = _defs[_i]; _bx = _bx - 30
        local _bg = Instance.new("Frame", TopBar)
        _bg.BackgroundColor3       = C_CARD
        _bg.BackgroundTransparency = 1
        _bg.BorderSizePixel        = 0
        _bg.Position               = UDim2.new(0, _bx, 0.5, -12)
        _bg.Size                   = UDim2.new(0, 24, 0, 24)
        _bg.ZIndex                 = 9
        Instance.new("UICorner", _bg).CornerRadius = UDim.new(0, 8)
        local _btn = Instance.new("TextButton", TopBar)
        _btn.BackgroundTransparency = 1
        _btn.BorderSizePixel        = 0
        _btn.Position               = UDim2.new(0, _bx, 0.5, -12)
        _btn.Size                   = UDim2.new(0, 24, 0, 24)
        ...
        _btn.MouseEnter:Connect(function()
            TweenService:Create(_bg,  TweenInfo.new(0.12), {BackgroundTransparency=0.35}):Play()
            TweenService:Create(_btn, TweenInfo.new(0.12), {TextColor3=_d.hov}):Play()
        end)
        _btn.MouseLeave:Connect(function()
            TweenService:Create(_bg,  TweenInfo.new(0.12), {BackgroundTransparency=1}):Play()
            TweenService:Create(_btn, TweenInfo.new(0.12), {TextColor3=C_LOW}):Play()
        end)
...
-- ── SIDEBAR PANEL ─────────────────────────────────────────────
-- Nomeado TabBar para compatibilidade com código legado
local TabBar = Instance.new("Frame", MainFrame)
TabBar.Name             = "TabBar"
TabBar.BackgroundColor3 = C_SURFACE
TabBar.BorderSizePixel  = 0
TabBar.Position         = UDim2.new(0, 0, 0, TOP_H + 1)
TabBar.Size             = UDim2.new(0, SIDE_W, 0, WIN_H - TOP_H - 1)
TabBar.ZIndex           = 5
TabBar.Visible          = false
Instance.new("UICorner", TabBar).CornerRadius = UDim.new(0, 14)

-- Fix canto superior direito da sidebar
local _sbCornerFix = Instance.new("Frame", TabBar)
_sbCornerFix.BackgroundColor3 = C_SURFACE
_sbCornerFix.BorderSizePixel  = 0
_sbCornerFix.Position         = UDim2.new(1, -14, 0, 0)
_sbCornerFix.Size             = UDim2.new(0, 14, 0, 60)
_sbCornerFix.ZIndex           = 4

-- Gradient vertical na sidebar
local _sbGrad = Instance.new("UIGradient", TabBar)
_sbGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.0, Color3.fromRGB(14, 12, 24)),
    ColorSequenceKeypoint.new(1.0, Color3.fromRGB(8,  8, 16)),
})
_sbGrad.Rotation = 90

-- Borda direita da sidebar (acento)
local _sbRightBorder = Instance.new("Frame", TabBar)
_sbRightBorder.BackgroundColor3 = C_ACCENT
_sbRightBorder.BackgroundTransparency = 0.7
_sbRightBorder.BorderSizePixel = 0
_sbRightBorder.Position = UDim2.new(1, -1, 0, 30)
_sbRightBorder.Size     = UDim2.new(0, 1, 1, -60)
_sbRightBorder.ZIndex   = 7
Instance.new("UICorner", _sbRightBorder).CornerRadius = UDim.new(1, 0)

-- Separador vertical TopBar-Sidebar
local TabSep = Instance.new("Frame", MainFrame)
TabSep.BackgroundColor3 = C_STROKE
TabSep.BorderSizePixel  = 0
TabSep.Position         = UDim2.new(0, SIDE_W, 0, TOP_H + 1)
TabSep.Size             = UDim2.new(0, 1, 1, -(TOP_H + 1))
TabSep.ZIndex           = 6
...
-- ── BARRA DE BUSCA ───────────────────────────────────────────
local _searchOuter = Instance.new("Frame", TabBar)
_searchOuter.BackgroundColor3 = C_ELEV
_searchOuter.BorderSizePixel  = 0
_searchOuter.Position = UDim2.new(0, 10, 0, 10)
_searchOuter.Size     = UDim2.new(1, -20, 0, 30)
_searchOuter.ZIndex   = 7
Instance.new("UICorner", _searchOuter).CornerRadius = UDim.new(0, 9)

local _searchStroke = Instance.new("UIStroke", _searchOuter)
_searchStroke.Color = C_STROKE; _searchStroke.Thickness = 1.2

local _searchIco = Instance.new("TextLabel", _searchOuter)
_searchIco.BackgroundTransparency = 1
_searchIco.Position = UDim2.new(0, 8, 0, 0)
_searchIco.Size     = UDim2.new(0, 18, 1, 0)
_searchIco.Font     = Enum.Font.GothamBold
_searchIco.Text     = "⊘"
_searchIco.TextColor3 = C_LOW
_searchIco.TextSize = 11
_searchIco.ZIndex   = 8

local SearchBox = Instance.new("TextBox", _searchOuter)
SearchBox.BackgroundTransparency = 1
SearchBox.BorderSizePixel = 0
SearchBox.Position = UDim2.new(0, 28, 0, 0)
SearchBox.Size     = UDim2.new(1, -36, 1, 0)
SearchBox.Font     = Enum.Font.Gotham
SearchBox.PlaceholderText = "Buscar aba..."
SearchBox.Text     = ""
SearchBox.PlaceholderColor3 = C_LOW
SearchBox.TextColor3 = C_HI
SearchBox.TextSize = 10
SearchBox.ClearTextOnFocus = true
SearchBox.ZIndex   = 9

SearchBox.Focused:Connect(function()
    TweenService:Create(_searchStroke, TweenInfo.new(0.15), {Color=C_ACCENT, Thickness=1.5}):Play()
    TweenService:Create(_searchIco,   TweenInfo.new(0.15), {TextColor3=C_ACCENT}):Play()
end)
SearchBox.FocusLost:Connect(function()
    TweenService:Create(_searchStroke, TweenInfo.new(0.15), {Color=C_STROKE, Thickness=1.2}):Play()
    TweenService:Create(_searchIco,   TweenInfo.new(0.15), {TextColor3=C_LOW}):Play()
    if SearchBox.Text == "" then _searchIco.Text = "⊘" end
end)

-- ── TAB SCROLL (lista vertical de abas) ─────────────────────
local TabScroll = Instance.new("ScrollingFrame", TabBar)
TabScroll.BackgroundTransparency = 1
TabScroll.BorderSizePixel        = 0
TabScroll.Position               = UDim2.new(0, 0, 0, 48)
TabScroll.Size                   = UDim2.new(1, 0, 1, -108)
TabScroll.ScrollBarThickness     = 2
TabScroll.ScrollBarImageColor3   = C_ACCENT
TabScroll.AutomaticCanvasSize    = Enum.AutomaticSize.Y
TabScroll.CanvasSize             = UDim2.new(0, 0, 0, 0)
TabScroll.ScrollingDirection     = Enum.ScrollingDirection.Y
TabScroll.ZIndex                 = 6

local _tbl = Instance.new("UIListLayout", TabScroll)
_tbl.FillDirection       = Enum.FillDirection.Vertical
_tbl.HorizontalAlignment = Enum.HorizontalAlignment.Center
_tbl.VerticalAlignment   = Enum.VerticalAlignment.Top
_tbl.Padding             = UDim.new(0, 2)
_tbl.SortOrder           = Enum.SortOrder.LayoutOrder

local _tbP = Instance.new("UIPadding", TabScroll)
_tbP.PaddingLeft   = UDim.new(0, 8)
_tbP.PaddingRight  = UDim.new(0, 8)
_tbP.PaddingTop    = UDim.new(0, 4)
_tbP.PaddingBottom = UDim.new(0, 4)

-- ── SIDEBAR FOOTER ───────────────────────────────────────────
local _sideFooter = Instance.new("Frame", TabBar)
_sideFooter.BackgroundTransparency = 1
_sideFooter.BorderSizePixel = 0
_sideFooter.Position = UDim2.new(0, 0, 1, -58)
_sideFooter.Size     = UDim2.new(1, 0, 0, 58)
_sideFooter.ZIndex   = 6

local _sfSep = Instance.new("Frame", _sideFooter)
_sfSep.BackgroundColor3 = C_STROKE; _sfSep.BorderSizePixel = 0
_sfSep.Position = UDim2.new(0, 10, 0, 0); _sfSep.Size = UDim2.new(1, -20, 0, 1)
_sfSep.ZIndex   = 7

-- Gradient decorativo no footer
local _sfGradFrame = Instance.new("Frame", _sideFooter)
_sfGradFrame.BackgroundColor3 = C_ACCENT
_sfGradFrame.BackgroundTransparency = 0.95
_sfGradFrame.BorderSizePixel = 0
_sfGradFrame.Position = UDim2.new(0, 0, 0, 1)
_sfGradFrame.Size     = UDim2.new(1, 0, 1, -1)
_sfGradFrame.ZIndex   = 5

local _sfGrad = Instance.new("UIGradient", _sfGradFrame)
...
-- Keybind info
local _keybindLbl = Instance.new("TextLabel", _sideFooter)
_keybindLbl.BackgroundTransparency = 1
_keybindLbl.Position = UDim2.new(0, 10, 0, 8)
_keybindLbl.Size     = UDim2.new(1, -20, 0, 14)
_keybindLbl.Font     = Enum.Font.Gotham
_keybindLbl.Text     = "[ RShift ]  Toggle Hub"
_keybindLbl.TextColor3 = C_LOW
_keybindLbl.TextSize = 8
_keybindLbl.TextXAlignment = Enum.TextXAlignment.Left
_keybindLbl.ZIndex   = 7

-- Nome do player
local _playerFooter = Instance.new("TextLabel", _sideFooter)
_playerFooter.BackgroundTransparency = 1
_playerFooter.Position = UDim2.new(0, 10, 0, 26)
_playerFooter.Size     = UDim2.new(1, -20, 0, 14)
_playerFooter.Font     = Enum.Font.GothamBold
_playerFooter.Text     = "◈  " .. (Player.DisplayName or Player.Name)
_playerFooter.TextColor3 = C_ACCENT
_playerFooter.TextSize = 9
_playerFooter.TextXAlignment = Enum.TextXAlignment.Left
_playerFooter.ZIndex   = 7

-- Branding
local _brandLbl = Instance.new("TextLabel", _sideFooter)
_brandLbl.BackgroundTransparency = 1
_brandLbl.Position = UDim2.new(0, 10, 0, 42)
_brandLbl.Size     = UDim2.new(1, -20, 0, 12)
_brandLbl.Font     = Enum.Font.Gotham
_brandLbl.Text     = "PUDIMHUB  v6  ◈  ECLIPSE"
_brandLbl.TextColor3 = C_LOW
_brandLbl.TextSize = 8
_brandLbl.TextXAlignment = Enum.TextXAlignment.Left
_brandLbl.ZIndex   = 7
...
ContentArea.Position            = UDim2.new(0, SIDE_W + 1, 0, TOP_H + 1)
ContentArea.Size                = UDim2.new(1, -(SIDE_W + 1), 0, CNT_H)
ContentArea.ZIndex              = 4
ContentArea.Visible             = false

-- ── STATUS BAR (live FPS / Ping / Memória) ───────────────────
local StatusBar = Instance.new("Frame", MainFrame)
StatusBar.Name             = "StatusBar"
StatusBar.BackgroundColor3 = C_SURFACE
StatusBar.BorderSizePixel  = 0
StatusBar.Position         = UDim2.new(0, SIDE_W + 1, 0, WIN_H - BOT_H)
StatusBar.Size             = UDim2.new(1, -(SIDE_W + 1), 0, BOT_H)
StatusBar.ZIndex           = 5
StatusBar.Visible          = false
Instance.new("UICorner", StatusBar).CornerRadius = UDim.new(0, 10)

-- Fix canto superior do StatusBar
local _stBarFix = Instance.new("Frame", StatusBar)
_stBarFix.BackgroundColor3 = C_SURFACE; _stBarFix.BorderSizePixel = 0
_stBarFix.Position = UDim2.new(0, 0, 0, 0); _stBarFix.Size = UDim2.new(1, 0, 0, 8)
_stBarFix.ZIndex   = 4

-- Separador topo do StatusBar
local _stSep = Instance.new("Frame", MainFrame)
_stSep.BackgroundColor3 = C_STROKE; _stSep.BorderSizePixel = 0
_stSep.Position = UDim2.new(0, SIDE_W + 1, 0, WIN_H - BOT_H)
_stSep.Size     = UDim2.new(1, -(SIDE_W + 1), 0, 1)
_stSep.ZIndex   = 5

-- StatusBar: lista horizontal de stats
local _stList = Instance.new("UIListLayout", StatusBar)
_stList.FillDirection       = Enum.FillDirection.Horizontal
_stList.VerticalAlignment   = Enum.VerticalAlignment.Center
_stList.HorizontalAlignment = Enum.HorizontalAlignment.Left
_stList.Padding             = UDim.new(0, 0)

local _stPad = Instance.new("UIPadding", StatusBar)
_stPad.PaddingLeft = UDim.new(0, 12)

-- Função para criar um item de status
local function makeStatItem(icon, initText, iconColor)
    local f = Instance.new("Frame", StatusBar)
    f.BackgroundTransparency = 1; f.BorderSizePixel = 0
    f.Size = UDim2.new(0, 0, 1, 0); f.AutomaticSize = Enum.AutomaticSize.X; f.ZIndex = 6
    local _fl = Instance.new("UIListLayout", f)
    _fl.FillDirection = Enum.FillDirection.Horizontal
    _fl.VerticalAlignment = Enum.VerticalAlignment.Center
    _fl.Padding = UDim.new(0, 3)
    local _fp = Instance.new("UIPadding", f)
    _fp.PaddingLeft = UDim.new(0, 8); _fp.PaddingRight = UDim.new(0, 14)
    local iL = Instance.new("TextLabel", f)
    iL.BackgroundTransparency = 1; iL.Size = UDim2.new(0, 10, 1, 0)
    iL.Font = Enum.Font.GothamBold; iL.Text = icon
    iL.TextColor3 = iconColor or C_ACCENT; iL.TextSize = 9; iL.ZIndex = 7
    local vL = Instance.new("TextLabel", f)
    vL.BackgroundTransparency = 1; vL.Size = UDim2.new(0, 0, 1, 0); vL.AutomaticSize = Enum.AutomaticSize.X
    vL.Font = Enum.Font.GothamSemibold; vL.Text = initText
    vL.TextColor3 = C_MID; vL.TextSize = 9; vL.ZIndex = 7
    -- Separador
    local sep = Instance.new("TextLabel", f)
    sep.BackgroundTransparency = 1; sep.Size = UDim2.new(0, 6, 1, 0)
    sep.Font = Enum.Font.Gotham; sep.Text = "·"
    sep.TextColor3 = C_LOW; sep.TextSize = 9; sep.ZIndex = 7
    return vL, iL
end

local FpsLbl, FpsIco   = makeStatItem("⚡", "FPS --",     C_GREEN)
local PingLbl, PingIco = makeStatItem("◎", "Ping --",    C_ACCENT2)
local MemLbl,  _       = makeStatItem("▣", "Mem --",     C_AMBER)
local PlayLbl, _       = makeStatItem("◐", "Players --", C_MID)
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
            ...
            table.insert(_fpsBuf, rawFps)
            if #_fpsBuf > 5 then table.remove(_fpsBuf, 1) end
            local avg = 0
            for _, v in ipairs(_fpsBuf) do avg = avg + v end
            avg = math.floor(avg / #_fpsBuf)
            FpsLbl.Text = "FPS " .. avg
            local fpsCol = avg >= 55 and C_GREEN or (avg >= 30 and C_AMBER or C_RED)
            FpsLbl.TextColor3 = fpsCol; FpsIco.TextColor3 = fpsCol
            -- Ping
            local ping = math.floor(Players.LocalPlayer:GetNetworkPing() * 1000)
            PingLbl.Text = "Ping " .. ping .. "ms"
            local pingCol = ping < 80 and C_GREEN or (ping < 180 and C_AMBER or C_RED)
            PingLbl.TextColor3 = pingCol; PingIco.TextColor3 = pingCol
            -- Memória
            local mem = math.floor(StatsService:GetTotalMemoryUsageMb())
            MemLbl.Text = "Mem " .. mem .. "mb"
            -- Players
            local pc = #Players:GetPlayers()
            PlayLbl.Text = "Players " .. pc
        end)
    end
end)

-- ── RODAPÉ (stub de compatibilidade) ─────────────────────────
local Footer = Instance.new("Frame", MainFrame)
Footer.Name                  = "Footer"
Footer.BackgroundTransparency = 1
Footer.BorderSizePixel       = 0
Footer.Position              = UDim2.new(0, 0, 1, 0)
Footer.Size                  = UDim2.new(0, 0, 0, 0)
Footer.Visible               = true
Footer.ZIndex                = 2
local FLangFrame = Instance.new("Frame", Footer)
FLangFrame.BackgroundTransparency = 1
FLangFrame.BorderSizePixel        = 0
FLangFrame.Size                   = UDim2.new(0, 0, 0, 0)
FLangFrame.ZIndex                 = 2
local FLangLbl = Instance.new("TextLabel", FLangFrame)
FLangLbl.BackgroundTransparency = 1
FLangLbl.Size      = UDim2.new(0, 0, 0, 0)
FLangLbl.Text      = currentLang.flag .. " " .. currentLang.short
FLangLbl.TextColor3 = C_ACCENT
FLangLbl.TextSize  = 8
FLangLbl.ZIndex    = 2
...
local function selectTab(key)
    ...
    for _, e in ipairs(allTabs) do
        local on = (e.key == key)
        TweenService:Create(e.pill, TweenInfo.new(0.22, Enum.EasingStyle.Quint), {
            BackgroundTransparency = on and 0.05 or 1,
            BackgroundColor3       = on and C_BG_ACT or C_CARD,
        }):Play()
        TweenService:Create(e.iconLbl, TweenInfo.new(0.18), {TextColor3 = on and C_ACCENT or C_LOW}):Play()
        TweenService:Create(e.textLbl, TweenInfo.new(0.18), {TextColor3 = on and C_HI    or C_MID}):Play()
        if e.bar then
            TweenService:Create(e.bar, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                BackgroundTransparency = on and 0 or 1,
                Size = on and UDim2.new(0, 3, 0.65, 0) or UDim2.new(0, 3, 0.3, 0)
            }):Play()
        end
    end
end

-- Constrói items da sidebar agrupados
do
    local _lo = 0
    local _tabLookup = {}
    for _, t in ipairs(TabConfig) do _tabLookup[t.key] = t end

    for _gi, grp in ipairs(GroupConfig) do
        _lo = _lo + 1

        -- Header do grupo
        local _grpContainer = Instance.new("Frame", TabScroll)
        _grpContainer.BackgroundTransparency = 1
        _grpContainer.BorderSizePixel = 0
        _grpContainer.Size    = UDim2.new(1, 0, 0, 24)
        _grpContainer.LayoutOrder = _lo * 1000
        _grpContainer.ZIndex  = 6

        local _grpLbl = Instance.new("TextLabel", _grpContainer)
        _grpLbl.BackgroundTransparency = 1
        _grpLbl.Position = UDim2.new(0, 4, 0, 4)
        _grpLbl.Size     = UDim2.new(1, -8, 0, 14)
        _grpLbl.Font     = Enum.Font.GothamBlack
        _grpLbl.Text     = grp.label
        _grpLbl.TextColor3 = C_ACCENT
        _grpLbl.TextTransparency = 0.25
        _grpLbl.TextSize = 8
        _grpLbl.TextXAlignment = Enum.TextXAlignment.Left
        _grpLbl.ZIndex   = 7
        if grp.trKey then trackLabel(_grpLbl, grp.trKey) end

        local _grpLine = Instance.new("Frame", _grpContainer)
        _grpLine.BackgroundColor3 = C_ACCENT
        _grpLine.BackgroundTransparency = 0.82
        _grpLine.BorderSizePixel = 0
        _grpLine.Position = UDim2.new(0, 4, 1, -2)
        _grpLine.Size     = UDim2.new(1, -8, 0, 1)
        _grpLine.ZIndex   = 7

        -- Tab items neste grupo
        for _ti, tabKey in ipairs(grp.keys) do
            local t = _tabLookup[tabKey]
            if not t then continue end
            _lo = _lo + 1

            local pill = Instance.new("Frame", TabScroll)
            pill.BackgroundColor3      = C_BG_ACT
            pill.BackgroundTransparency = 1
            pill.BorderSizePixel       = 0
            pill.LayoutOrder           = _lo * 1000 + _ti
            pill.ZIndex                = 6
            ...
            -- Fundo do ícone (aparece quando ativo)
            local _iconBg = Instance.new("Frame", pill)
            _iconBg.BackgroundColor3 = C_ACCENT
            _iconBg.BackgroundTransparency = 1
            _iconBg.BorderSizePixel = 0
            _iconBg.Position = UDim2.new(0, 8, 0.5, -10)
            _iconBg.Size     = UDim2.new(0, 20, 0, 20)
            _iconBg.ZIndex   = 7
            Instance.new("UICorner", _iconBg).CornerRadius = UDim.new(1, 0)
            ...
            textLbl.Size       = UDim2.new(1, -42, 1, 0)
            textLbl.Font       = Enum.Font.GothamSemibold
            textLbl.Text       = t.label
            textLbl.TextColor3 = C_MID
            textLbl.TextSize   = 10
            ...
            pbtn.MouseEnter:Connect(function()
                if currentTab ~= t.key then
                    TweenService:Create(pill,    TweenInfo.new(0.12), {BackgroundTransparency=0.88}):Play()
                    TweenService:Create(iconLbl, TweenInfo.new(0.12), {TextColor3=C_MID}):Play()
                end
            end)
            pbtn.MouseLeave:Connect(function()
                if currentTab ~= t.key then
                    TweenService:Create(pill,    TweenInfo.new(0.12), {BackgroundTransparency=1}):Play()
                    TweenService:Create(iconLbl, TweenInfo.new(0.12), {TextColor3=C_LOW}):Play()
                end
            end)

            table.insert(allTabs, {key=t.key, pill=pill, iconLbl=iconLbl, textLbl=textLbl, bar=bar, _iconBg=_iconBg})
        end

        -- Espaçador entre grupos
        local _spacer = Instance.new("Frame", TabScroll)
        _spacer.BackgroundTransparency = 1; _spacer.BorderSizePixel = 0
        _spacer.Size = UDim2.new(1, 0, 0, 5)
        _spacer.LayoutOrder = (_lo + 1) * 1000; _spacer.ZIndex = 6
    end
end

-- ── BUSCA: filtrar tabs ao vivo ────────────────────────────────
SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    _searchIco.Text = "✕"
    for _, e in ipairs(allTabs) do
        local lbl = e.textLbl.Text:lower()
        local key = e.key:lower()
        e.pill.Visible = (lbl:find(q, 1, true) ~= nil or key:find(q, 1, true) ~= nil)
    end
end)

-- Clicar no ícone X da busca limpa
_searchOuter.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 and _searchIco.Text == "✕" then
        SearchBox.Text = ""
    end
end)
...
local function openHub()
    ...
    MainFrame.Position = UDim2.new(0.5, -WIN_W/2, 0.5, -(WIN_H/2) - 18)
...
local function closeHub()
    ...
    TweenService:Create(MainFrame, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        ...
        Position = UDim2.new(0.5, -WIN_W/2, 0.5, -(WIN_H/2) - 18)
    }):Play()
...
-- ══════════════════════════════════════════════════════════════
--  INTRO ECLIPSE EDITION
--  Scanlines → Logo emerge → Badge → Barra de loading → Reveal
-- ══════════════════════════════════════════════════════════════
do
    -- Overlay escuro total
    local OV = Instance.new("Frame", ScreenGui)
    OV.Name             = "EclipseIntro"
    OV.Size             = UDim2.new(1, 0, 1, 0)
    OV.BackgroundColor3 = Color3.fromRGB(4, 3, 8)
    OV.BorderSizePixel  = 0
    OV.ZIndex           = 900

    -- Partículas flutuantes de fundo
    for _p = 1, 40 do
        local _dot = Instance.new("Frame", OV)
        local _sz  = math.random(2, 6)
        _dot.Size              = UDim2.new(0, _sz, 0, _sz)
        _dot.Position          = UDim2.new(math.random()*0.96, 0, math.random()*0.96, 0)
        local _isViolet = math.random() > 0.45
        _dot.BackgroundColor3  = _isViolet and Color3.fromRGB(120,80,255) or Color3.fromRGB(80,200,255)
        _dot.BackgroundTransparency = 0.55 + math.random()*0.38
        _dot.BorderSizePixel   = 0
        _dot.ZIndex            = 901
        Instance.new("UICorner", _dot).CornerRadius = UDim.new(1, 0)
        task.spawn(function()
            local baseY = _dot.Position.Y.Scale
            local amp   = 0.006 + math.random()*0.018
            local spd   = 0.35  + math.random()*0.7
            local phase = math.random() * math.pi * 2
            while _dot.Parent do
                task.wait(0.05)
                pcall(function()
                    _dot.Position = UDim2.new(
                        _dot.Position.X.Scale, 0,
                        baseY + math.sin(tick()*spd + phase)*amp, 0
                    )
                end)
            end
        end)
    ...
    -- Scanlines (3) — efeito de varredura
    local _lines = {}
    local _lineColors = {
    ...
    for _i, _ly in ipairs({0.26, 0.50, 0.74}) do
        local _ln = Instance.new("Frame", OV)
        _ln.BackgroundColor3       = _lineColors[_i]
        _ln.BackgroundTransparency = 0.72
        _ln.BorderSizePixel        = 0
        _ln.Size                   = UDim2.new(0, 0, 0, 1)
        _ln.Position               = UDim2.new(0, 0, _ly, 0)
        _ln.ZIndex                 = 902
        table.insert(_lines, _ln)
    end

    -- Container central do conteúdo do intro
    local _C = Instance.new("Frame", OV)
    _C.BackgroundTransparency = 1; _C.BorderSizePixel = 0
    _C.AnchorPoint = Vector2.new(0.5, 0.5)
    _C.Position    = UDim2.new(0.5, 0, 0.5, -20)
    _C.Size        = UDim2.new(0, 540, 0, 200)
    _C.ZIndex      = 903

    -- Barra vertical de acento (cresce para cima)
    local _acBar = Instance.new("Frame", _C)
    _acBar.BackgroundColor3 = Color3.fromRGB(120, 80, 255)
    _acBar.BorderSizePixel  = 0
    _acBar.Position         = UDim2.new(0, 0, 0, 0)
    _acBar.Size             = UDim2.new(0, 3, 0, 0)
    _acBar.ZIndex           = 904
    Instance.new("UICorner", _acBar).CornerRadius = UDim.new(1, 0)

    -- Gradient na barra de acento
    local _acGrad = Instance.new("UIGradient", _acBar)
    _acGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(120,80,255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(80,200,255)),
    })
    _acGrad.Rotation = 90

    -- Título PUDIMHUB (glow + texto real em camadas)
    local _titleGlow = Instance.new("TextLabel", _C)
    _titleGlow.BackgroundTransparency = 1
    _titleGlow.Position = UDim2.new(0, 22, 0, 0)
    _titleGlow.Size     = UDim2.new(1, -22, 0, 72)
    _titleGlow.Font     = Enum.Font.GothamBlack
    _titleGlow.Text     = "PUDIMHUB"
    _titleGlow.TextColor3 = Color3.fromRGB(120,80,255)
    _titleGlow.TextSize = 56
    _titleGlow.TextTransparency = 1
    _titleGlow.TextXAlignment = Enum.TextXAlignment.Left
    _titleGlow.ZIndex   = 903

    local _title = Instance.new("TextLabel", _C)
    _title.BackgroundTransparency = 1
    _title.Position = UDim2.new(0, 22, 0, 0)
    _title.Size     = UDim2.new(1, -22, 0, 72)
    _title.Font     = Enum.Font.GothamBlack
    _title.Text     = "PUDIMHUB"
    _title.TextColor3 = Color3.fromRGB(235, 228, 255)
    _title.TextSize = 56
    _title.TextTransparency = 1
    _title.TextXAlignment = Enum.TextXAlignment.Left
    _title.ZIndex   = 904

    -- Badge ECLIPSE EDITION
    local _edBg = Instance.new("Frame", _C)
    _edBg.BackgroundColor3 = Color3.fromRGB(120,80,255)
    _edBg.BackgroundTransparency = 1
    _edBg.BorderSizePixel = 0
    _edBg.Position = UDim2.new(0, 22, 0, 76)
    _edBg.Size     = UDim2.new(0, 0, 0, 22)
    _edBg.AutomaticSize = Enum.AutomaticSize.X
    _edBg.ZIndex   = 905
    Instance.new("UICorner", _edBg).CornerRadius = UDim.new(0, 6)
    local _edStroke = Instance.new("UIStroke", _edBg)
    _edStroke.Color = Color3.fromRGB(120,80,255); _edStroke.Thickness = 1.2; _edStroke.Transparency = 0.5
    local _edPad = Instance.new("UIPadding", _edBg)
    _edPad.PaddingLeft = UDim.new(0, 8); _edPad.PaddingRight = UDim.new(0, 8)
    local _edLbl = Instance.new("TextLabel", _edBg)
    _edLbl.BackgroundTransparency = 1
    _edLbl.Size         = UDim2.new(0, 0, 1, 0); _edLbl.AutomaticSize = Enum.AutomaticSize.X
    _edLbl.Font         = Enum.Font.GothamBold
    _edLbl.Text         = "◈  ECLIPSE EDITION  ·  v6"
    _edLbl.TextColor3   = Color3.fromRGB(180, 150, 255)
    _edLbl.TextSize     = 9; _edLbl.ZIndex = 906

    -- Subtítulo do jogo
    local _sub = Instance.new("TextLabel", _C)
    _sub.BackgroundTransparency = 1
    _sub.Position = UDim2.new(0, 22, 0, 104)
    _sub.Size     = UDim2.new(1, -22, 0, 18)
    _sub.Font     = Enum.Font.GothamSemibold
    _sub.Text     = "99  NIGHTS  IN  THE  FOREST   ·   2026"
    _sub.TextColor3 = Color3.fromRGB(255, 175, 40)
    _sub.TextSize = 12
    _sub.TextTransparency = 1
    _sub.TextXAlignment = Enum.TextXAlignment.Left
    _sub.ZIndex   = 904

    -- Divisor horizontal
    local _div = Instance.new("Frame", _C)
    _div.BackgroundColor3 = Color3.fromRGB(38,33,64)
    _div.BackgroundTransparency = 0.15; _div.BorderSizePixel = 0
    _div.Position = UDim2.new(0, 22, 0, 130); _div.Size = UDim2.new(0, 0, 0, 1)
    _div.ZIndex   = 904

    -- Data de atualização
    local _n  = os.date("*t")
    local _dt = string.format("Atualizado em  %02d/%02d/%04d  —  %02d:%02d", _n.day, _n.month, _n.year, _n.hour, _n.min)
    local _dateLbl = Instance.new("TextLabel", _C)
    _dateLbl.BackgroundTransparency = 1
    _dateLbl.Position = UDim2.new(0, 22, 0, 140); _dateLbl.Size = UDim2.new(1, -22, 0, 16)
    _dateLbl.Font = Enum.Font.Gotham; _dateLbl.Text = _dt
    _dateLbl.TextColor3 = Color3.fromRGB(135,120,172); _dateLbl.TextSize = 10
    _dateLbl.TextTransparency = 1; _dateLbl.TextXAlignment = Enum.TextXAlignment.Left
    _dateLbl.ZIndex = 904

    -- ── Loading bar ──────────────────────────────────────────
    local _loadCont = Instance.new("Frame", OV)
    _loadCont.BackgroundTransparency = 1; _loadCont.BorderSizePixel = 0
    _loadCont.AnchorPoint = Vector2.new(0.5, 0)
    _loadCont.Position    = UDim2.new(0.5, 0, 0.5, 100)
    _loadCont.Size        = UDim2.new(0, 540, 0, 20)
    _loadCont.ZIndex      = 903

    local _loadBg = Instance.new("Frame", _loadCont)
    _loadBg.BackgroundColor3 = Color3.fromRGB(19,17,32); _loadBg.BorderSizePixel = 0
    _loadBg.Position = UDim2.new(0,0,0,0); _loadBg.Size = UDim2.new(1,0,0,4)
    _loadBg.ZIndex = 904
    Instance.new("UICorner", _loadBg).CornerRadius = UDim.new(1,0)

    local _loadFill = Instance.new("Frame", _loadBg)
    _loadFill.BackgroundColor3 = Color3.fromRGB(120,80,255); _loadFill.BorderSizePixel = 0
    _loadFill.Size = UDim2.new(0,0,1,0); _loadFill.ZIndex = 905
    Instance.new("UICorner", _loadFill).CornerRadius = UDim.new(1,0)
    local _lfGrad = Instance.new("UIGradient", _loadFill)
    _lfGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(120,80,255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(80,200,255)),
    })

    local _loadLbl = Instance.new("TextLabel", _loadCont)
    _loadLbl.BackgroundTransparency = 1
    _loadLbl.Position = UDim2.new(0,0,0,8); _loadLbl.Size = UDim2.new(1,0,0,12)
    _loadLbl.Font = Enum.Font.Gotham; _loadLbl.Text = "Inicializando..."
    _loadLbl.TextColor3 = Color3.fromRGB(54,48,84); _loadLbl.TextSize = 9
    _loadLbl.ZIndex = 904

    local _loadPct = Instance.new("TextLabel", _loadCont)
    _loadPct.BackgroundTransparency = 1
    _loadPct.AnchorPoint = Vector2.new(1,0)
    _loadPct.Position = UDim2.new(1,0,0,8); _loadPct.Size = UDim2.new(0,40,0,12)
    _loadPct.Font = Enum.Font.GothamBold; _loadPct.Text = "0%"
    _loadPct.TextColor3 = Color3.fromRGB(120,80,255); _loadPct.TextSize = 9
    _loadPct.ZIndex = 904

    -- ── Sequência animada de intro ────────────────────────────
    task.spawn(function()
        -- 1) Scanlines expandem da esquerda
        for _i2, _ln in ipairs(_lines) do
            task.delay((_i2-1)*0.07, function()
                TweenService:Create(_ln, TweenInfo.new(0.42, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    {Size=UDim2.new(1,0,0,1)}):Play()
            end)
        end
        task.wait(0.52)

        -- 2) Barra de acento cresce
        TweenService:Create(_acBar, TweenInfo.new(0.26, Enum.EasingStyle.Quint),
            {Size=UDim2.new(0,3,0,72)}):Play()
        task.wait(0.13)

        -- 3) Glow do título surge (translúcido)
        TweenService:Create(_titleGlow, TweenInfo.new(0.18), {TextTransparency=0.82}):Play()
        -- 4) Título principal emerge com drift lateral
        _title.Position = UDim2.new(0, 30, 0, 0)
        TweenService:Create(_title, TweenInfo.new(0.40, Enum.EasingStyle.Quint),
            {TextTransparency=0, Position=UDim2.new(0,22,0,0)}):Play()
        task.wait(0.28)

        -- 5) Badge ECLIPSE
        TweenService:Create(_edBg, TweenInfo.new(0.22), {BackgroundTransparency=0.82}):Play()
        task.wait(0.18)

        -- 6) Subtítulo
        TweenService:Create(_sub, TweenInfo.new(0.28), {TextTransparency=0}):Play()
        task.wait(0.20)

        -- 7) Divisor cresce
        TweenService:Create(_div, TweenInfo.new(0.35, Enum.EasingStyle.Quad),
            {Size=UDim2.new(0,500,0,1)}):Play()
        task.wait(0.18)

        -- 8) Data
        TweenService:Create(_dateLbl, TweenInfo.new(0.22), {TextTransparency=0}):Play()

        -- 9) Barra de loading
        local _steps = {
            {pct=0.18, text="Carregando módulos...",     pctStr="18%"},
            {pct=0.38, text="Inicializando ESP...",       pctStr="38%"},
            {pct=0.58, text="Configurando Bring...",      pctStr="58%"},
            {pct=0.75, text="Processando tradução...",    pctStr="75%"},
            {pct=0.90, text="Aplicando tema ECLIPSE...",  pctStr="90%"},
            {pct=1.00, text="Sistema pronto!",            pctStr="100%"},
        }
        for _, step in ipairs(_steps) do
            task.wait(0.26)
            TweenService:Create(_loadFill, TweenInfo.new(0.28, Enum.EasingStyle.Quint),
                {Size=UDim2.new(step.pct,0,1,0)}):Play()
            _loadLbl.Text = step.text
            _loadPct.Text = step.pctStr
        end
        task.wait(0.55)

        -- 10) Overlay sai (sobe como cortina)
        TweenService:Create(OV, TweenInfo.new(0.50, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
            {Position=UDim2.new(0,0,-1,0)}):Play()
        task.wait(0.52)
        OV.Visible = false

        -- 11) MainFrame aparece com spring
        _mainVisible             = true
        MainFrame.Visible        = true
        MainFrame.BackgroundTransparency = 1
...
BoostPopup.BackgroundColor3=Color3.fromRGB(10, 9, 18); BoostPopup.Size=UDim2.new(0,190,0,0)
...
local function makePopupToggle(text,callback)
    local row=Instance.new("Frame",BoostPopup); row.BackgroundColor3=Color3.fromRGB(18, 16, 32)
    ...
    lbl.Text=text; lbl.TextColor3=Color3.fromRGB(210, 200, 255); lbl.TextSize=10
    ...
    local knob=Instance.new("Frame",pill); knob.BackgroundColor3=Color3.fromRGB(80, 60, 150); knob.BorderSizePixel=0
...
rejBtn.BackgroundColor3=Color3.fromRGB(180,40,50); rejBtn.BorderSizePixel=0
...
FloatBtn.BackgroundColor3=Color3.fromRGB(8, 7, 14); FloatBtn.BorderSizePixel=0; FloatBtn.Visible=false; FloatBtn.ZIndex=100; FloatBtn.Active=true
...
PDText.Text="◈"; PDText.TextColor3=Color3.fromRGB(200, 185, 255); PDText.TextSize=20; PDText.TextTransparency=0.05; PDText.ZIndex=105
...
FloatClick.MouseButton1Click:Connect(function()
    ...
    task.delay(0.22,function()
        ...
        task.delay(0.2,function() TabBar.Visible=true; ContentArea.Visible=true; StatusBar.Visible=true end)
    end)
end)
...
TopBtns["Minimize"].MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        TabBar.Visible=false; ContentArea.Visible=false; StatusBar.Visible=false
        ...
        task.delay(0.2,function()
            TabBar.Visible=true; ContentArea.Visible=true; StatusBar.Visible=true
        end)
    end
end)
...
TopBtns["Maximize"].MouseButton1Click:Connect(function()
    ...
    if isMaximized then
        ...
        TweenService:Create(MainFrame,TweenInfo.new(0.3,Enum.EasingStyle.Quad),{Size=UDim2.new(0,900,0,600),Position=UDim2.new(0.5,-450,0.5,-300)}):Play()
    else
...
TopBtns["Close"].MouseButton1Click:Connect(function()
    _hubOpen = false; _mainVisible = false
    TweenService:Create(MainFrame,TweenInfo.new(0.22,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{
        BackgroundTransparency=1,
        Size=UDim2.new(0,WIN_W,0,0)
    }):Play()
    task.delay(0.24,function()
        MainFrame.Visible=false; MainFrame.Size=UDim2.new(0,WIN_W,0,WIN_H)
        TabBar.Visible=false; ContentArea.Visible=false; StatusBar.Visible=false
        showFloatBtn()
    end)
end)
