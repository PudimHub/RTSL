...
local TOP_H = 34
local TAB_H = 42
local CNT_H = WIN_H - TOP_H - TAB_H   -- 332

-- ── Paleta  (muted, orgânica — sem néon) ───────────────────────
local C_BASE     = Color3.fromRGB( 10,  12,  10)
local C_SURFACE  = Color3.fromRGB( 15,  19,  14)
local C_ELEV     = Color3.fromRGB( 21,  26,  20)
local C_CARD     = Color3.fromRGB( 27,  33,  26)
local C_ACCENT   = Color3.fromRGB( 88, 162, 108)   -- sage muted
local C_AMBER    = Color3.fromRGB(188, 150,  84)   -- âmbar quente
local C_HI       = Color3.fromRGB(208, 220, 204)   -- texto primário
local C_MID      = Color3.fromRGB(118, 145, 112)   -- texto secundário
local C_LOW      = Color3.fromRGB( 58,  76,  56)   -- texto discreto
local C_STROKE   = Color3.fromRGB( 28,  38,  26)   -- bordas

-- aliases para código legado
local C_TEXT_OFF  = C_MID
local C_TEXT_ON   = C_HI
local C_BG_HOV    = Color3.fromRGB(26, 36, 24)
local C_BG_ACT    = Color3.fromRGB(18, 34, 24)
...
_tpFix.Position = UDim2.new(0, 0, 1, -10)
_tpFix.Size     = UDim2.new(1, 0, 0, 10)
_tpFix.ZIndex   = 7
...
-- Fio de acento vertical (esquerdo, estilo editorial)
local TopAccent = Instance.new("Frame", TopBar)
TopAccent.BackgroundColor3 = C_ACCENT
TopAccent.BorderSizePixel  = 0
TopAccent.Position         = UDim2.new(0, 12, 0.5, -9)
TopAccent.Size             = UDim2.new(0, 2, 0, 18)
TopAccent.ZIndex           = 9
...
-- Badge versão (ao lado do logo)
local VerBg = Instance.new("Frame", TopBar)
VerBg.BackgroundColor3       = C_ELEV
VerBg.BackgroundTransparency = 0.35
VerBg.BorderSizePixel        = 0
VerBg.AnchorPoint            = Vector2.new(0, 0.5)
VerBg.Position               = UDim2.new(0, 146, 0.5, 0)
VerBg.Size                   = UDim2.new(0, 0, 0, 17)
...
do
    local _defs = {
        ...
        {name="Minimize",ch="—",  hov=C_ACCENT},
        {name="Maximize",ch="⊡",  hov=C_ACCENT},
        {name="Close",   ch="✕",  hov=Color3.fromRGB(215,70,70)},
    }
    local _bx = WIN_W - 8
    for _i = #_defs, 1, -1 do
        local _d = _defs[_i]; _bx = _bx - 28
        ...
        _bg.ZIndex                = 9
        Instance.new("UICorner", _bg).CornerRadius = UDim.new(0, 7)
        ...
        _btn.MouseEnter:Connect(function()
            TweenService:Create(_bg,  TweenInfo.new(0.13), {BackgroundTransparency=0.55}):Play()
            TweenService:Create(_btn, TweenInfo.new(0.13), {TextColor3=_d.hov}):Play()
        end)
        _btn.MouseLeave:Connect(function()
            TweenService:Create(_bg,  TweenInfo.new(0.13), {BackgroundTransparency=1}):Play()
            TweenService:Create(_btn, TweenInfo.new(0.13), {TextColor3=C_LOW}):Play()
        end)
        TopBtns[_d.name] = _btn
    end
end

-- Dragging pelo TopBar
do
...
-- ── TAB BAR — barra horizontal de abas no fundo ───────────────
...
local _tbFix = Instance.new("Frame", TabBar)
_tbFix.BackgroundColor3 = C_SURFACE
_tbFix.BorderSizePixel  = 0
_tbFix.Position         = UDim2.new(0, 0, 0, 0)
_tbFix.Size             = UDim2.new(1, 0, 0, 10)
_tbFix.ZIndex           = 7

-- Separador TabBar → ContentArea
local TabSep = Instance.new("Frame", MainFrame)
TabSep.BackgroundColor3 = C_STROKE
TabSep.BorderSizePixel  = 0
TabSep.Position         = UDim2.new(0, 0, 1, -TAB_H - 1)
TabSep.Size             = UDim2.new(1, 0, 0, 1)
TabSep.ZIndex           = 6

-- ScrollingFrame dentro da TabBar (suporte a 10 abas)
...
_tbl.HorizontalAlignment = Enum.HorizontalAlignment.Left
_tbl.VerticalAlignment   = Enum.VerticalAlignment.Center
_tbl.Padding             = UDim.new(0, 3)
...
-- ── RODAPÉ (stub para compatibilidade) ────────────────────────
...
local allTabs    = {}   -- {key, pill, iconLbl, textLbl, bar}
...
local function selectTab(key)
    ...
    for _, e in ipairs(allTabs) do
        ...
        TweenService:Create(e.iconLbl, TweenInfo.new(0.16), {TextColor3 = on and C_ACCENT or C_LOW}):Play()
        TweenService:Create(e.textLbl, TweenInfo.new(0.16), {TextColor3 = on and C_HI    or C_LOW}):Play()
        if e.bar then e.bar.Visible = on end
    end
end

-- Constrói pills de abas na TabBar
do
    local _TS = game:GetService("TextService")
    local _lo = 0
    for _, t in ipairs(TabConfig) do
        _lo += 1
        local _ts = _TS:GetTextSize(t.label, 8, Enum.Font.GothamSemibold, Vector2.new(999, 999))
        local _pw = math.max(_ts.X + 22, 50)
        ...
        pill.ZIndex                = 9
        pill.Size                  = UDim2.new(0, _pw, 0, TAB_H - 10)
...
-- ══════════════════════════════════════════════════════════════
--  INTRO "EMERGENCE"
--  3 scanlines → acento cresce → PUDIMHUB emerge da esquerda →
--  subtítulo → divisor → data/hora → overlay desliza para cima →
--  MainFrame aparece
-- ══════════════════════════════════════════════════════════════
do
    -- Overlay preto total
    local OV = Instance.new("Frame", ScreenGui)
    OV.Name             = "EmergenceOverlay"
    OV.Size             = UDim2.new(1, 0, 1, 0)
    OV.BackgroundColor3 = Color3.fromRGB(5, 7, 5)
    OV.BorderSizePixel  = 0
    OV.ZIndex           = 900

    -- Partículas sutis de fundo (sem piscar - apenas estáticas e quase invisíveis)
    for _p = 1, 20 do
        local _d = Instance.new("Frame", OV)
        local _s = math.random(2, 4)
        _d.Size              = UDim2.new(0, _s, 0, _s)
        _d.Position          = UDim2.new(math.random()*0.96, 0, math.random()*0.96, 0)
        _d.BackgroundColor3  = C_ACCENT
        _d.BackgroundTransparency = 0.72 + math.random()*0.24
        _d.BorderSizePixel   = 0
        _d.ZIndex            = 901
        Instance.new("UICorner", _d).CornerRadius = UDim.new(1, 0)
    end

    -- 3 scanlines horizontais finas
    local _lineYs = {0.28, 0.50, 0.72}
    local _lines  = {}
    for _, _ly in ipairs(_lineYs) do
        local _ln = Instance.new("Frame", OV)
        _ln.BackgroundColor3      = C_ACCENT
        _ln.BackgroundTransparency = 0.80
    ...
    _sub.Position         = UDim2.new(0, 22, 0, 72)
    _sub.Size             = UDim2.new(1, -22, 0, 20)
    _sub.Font             = Enum.Font.GothamSemibold
    _sub.Text             = "99  DIAS  NA  FLORESTA   ·   2026"
    _sub.TextColor3       = C_AMBER
    _sub.TextSize         = 13
    ...
    _div.BackgroundColor3      = C_STROKE
    _div.BackgroundTransparency = 0.3
    _div.BorderSizePixel       = 0
    _div.Position              = UDim2.new(0, 22, 0, 102)
    _div.Size                  = UDim2.new(0, 0, 0, 1)
    _div.ZIndex                = 904

    -- Data/hora de atualização
    local _n   = os.date("*t")
    local _dys = {"Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado"}
    local _dt  = string.format(
        "Atualizado em %02d/%02d/%04d   às   %02d:%02d   —   %s",
        _n.day, _n.month, _n.year, _n.hour, _n.min, _dys[_n.wday])
    local _dateLbl = Instance.new("TextLabel", _C)
    _dateLbl.BackgroundTransparency = 1
    _dateLbl.Position        = UDim2.new(0, 22, 0, 114)
    _dateLbl.Size            = UDim2.new(1, -22, 0, 18)
    _dateLbl.Font            = Enum.Font.Gotham
    _dateLbl.Text            = _dt
    _dateLbl.TextColor3      = C_MID
    _dateLbl.TextSize        = 10
    _dateLbl.TextTransparency = 1
    _dateLbl.TextXAlignment  = Enum.TextXAlignment.Left
    _dateLbl.ZIndex          = 904

    -- Badge versão
    local _vbg = Instance.new("Frame", _C)
    _vbg.BackgroundColor3       = C_ELEV
    _vbg.BackgroundTransparency = 1
    _vbg.BorderSizePixel        = 0
    _vbg.Position               = UDim2.new(0, 22, 0, 142)
    _vbg.Size                   = UDim2.new(0, 0, 0, 22)
    _vbg.AutomaticSize          = Enum.AutomaticSize.X
    _vbg.ZIndex                 = 904
    Instance.new("UICorner", _vbg).CornerRadius = UDim.new(0, 7)
    local _vbgP = Instance.new("UIPadding", _vbg)
    _vbgP.PaddingLeft = UDim.new(0, 8); _vbgP.PaddingRight = UDim.new(0, 8)
    local _verLbl = Instance.new("TextLabel", _vbg)
    ...
    _verLbl.Text         = "v5   ·   FLORESTA SOMBRIA"
    _verLbl.TextColor3   = C_ACCENT
    _verLbl.TextSize     = 9
    _verLbl.ZIndex       = 905

    -- ── Sequência de animação ──────────────────────────────────
    task.spawn(function()
        -- Passo 1 (0.0–0.65s): 3 scanlines expandem da esquerda
        for _i2, _ln in ipairs(_lines) do
            task.delay((_i2 - 1) * 0.09, function()
                TweenService:Create(_ln,
        ...
        -- Passo 2 (0.70s): fio vertical de acento cresce
        TweenService:Create(_acBar,
            TweenInfo.new(0.28, Enum.EasingStyle.Quad),
            {Size = UDim2.new(0, 2, 0, 62)}):Play()
        task.wait(0.14)

        -- Passo 3 (0.84s): PUDIMHUB emerge (drift + fade)
        _title.Position = UDim2.new(0, 28, 0, 0)
        TweenService:Create(_title,
            TweenInfo.new(0.40, Enum.EasingStyle.Quad),
            {TextTransparency = 0, Position = UDim2.new(0, 20, 0, 0)}):Play()
        task.wait(0.30)

        -- Passo 4 (1.14s): subtítulo
        TweenService:Create(_sub,
            TweenInfo.new(0.34, Enum.EasingStyle.Quad),
            {TextTransparency = 0}):Play()
        task.wait(0.28)

        -- Passo 5 (1.42s): divisor cresce
        TweenService:Create(_div,
            TweenInfo.new(0.38, Enum.EasingStyle.Quad),
            {Size = UDim2.new(0, 460, 0, 1)}):Play()
        task.wait(0.22)

        -- Passo 6 (1.64s): data e badge
        TweenService:Create(_dateLbl,
            TweenInfo.new(0.28, Enum.EasingStyle.Quad),
            {TextTransparency = 0}):Play()
        task.wait(0.16)
        TweenService:Create(_vbg,
            TweenInfo.new(0.26),
            {BackgroundTransparency = 0.28}):Play()

        -- Aguarda leitura (~1.5s de pausa)
        task.wait(1.55)

        -- Passo 7: Overlay desliza PARA CIMA (cortina levantando)
        TweenService:Create(OV,
            TweenInfo.new(0.52, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
            {Position = UDim2.new(0, 0, -1, 0)}):Play()
        task.wait(0.54)
        OV.Visible = false

        -- Passo 8: MainFrame fade-in suave
        MainFrame.Visible                = true
        MainFrame.BackgroundTransparency = 1
        TweenService:Create(MainFrame,
            TweenInfo.new(0.30, Enum.EasingStyle.Quad),
...
TopBtns["Maximize"].MouseButton1Click:Connect(function()
    ...
    if isMaximized then
        ...
        TweenService:Create(MainFrame,TweenInfo.new(0.3,Enum.EasingStyle.Quad),{Size=UDim2.new(0,800,0,560),Position=UDim2.new(0.5,-400,0.5,-280)}):Play()
    else
