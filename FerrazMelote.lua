...
local function makeMultiDropdown(parent, yPos, cfg)
    ...
    local POP_H    = math.min(MAX_VIS, #ITEMS) * ITEM_H + 56 -- header + padding
    local isOpen   = false
    ...
    trigBtn.BackgroundColor3      = Color3.fromRGB(22, 14, 42)
    trigBtn.BackgroundTransparency = 0.25
    ...
    trigS.Color = COR; trigS.Thickness = 1.2; trigS.Transparency = 0.45
    trigS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Shine
    local trigShine = Instance.new("Frame", trigBtn)
    trigShine.BackgroundColor3       = Color3.fromRGB(255,255,255)
    trigShine.BackgroundTransparency = 0.88
    trigShine.BorderSizePixel        = 0
    trigShine.Position               = UDim2.new(0,6,0,3)
    trigShine.Size                   = UDim2.new(0,44,0,5)
    trigShine.ZIndex                 = 9
    Instance.new("UICorner", trigShine).CornerRadius = UDim.new(1, 0)
    ...
    popHdr.Size                   = UDim2.new(1, 0, 0, 36)
    popHdr.ZIndex                 = 431
    ...
    popHdrLbl.Position  = UDim2.new(0, 12, 0, 0)
    ...
    hdrNone.BackgroundTransparency = 0.5
    hdrNone.BorderSizePixel        = 0
    hdrNone.AnchorPoint            = Vector2.new(1, 0.5)
    hdrNone.Position               = UDim2.new(1, -76, 0.5, 0)
    hdrNone.Size                   = UDim2.new(0, 58, 0, 22)
    hdrNone.Font                   = Enum.Font.GothamBold
    hdrNone.Text                   = "✕ Limpar"
    hdrNone.TextColor3             = Color3.fromRGB(255, 210, 210)
    hdrNone.TextSize               = 9
    hdrNone.ZIndex                 = 432
    Instance.new("UICorner", hdrNone).CornerRadius = UDim.new(0, 7)
    ...
    for _, catKey in ipairs(catOrder) do
        ...
        for _, item in ipairs(catItems) do
            ...
            icoLbl.Position               = UDim2.new(0, 34, 0, 0)
    ...
    local function closePop()
        ...
        TweenService:Create(trigBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.25}):Play()
        TweenService:Create(trigS,TweenInfo.new(0.15),{Transparency=0.45}):Play()
    end
    local function openPop()
        ...
        pop.Position = UDim2.new(0,ap.X,0,ap.Y+as.Y+6)
        pop.Size     = UDim2.new(0,POP_W,0,0)
        ...
        TweenService:Create(trigBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.05}):Play()
        TweenService:Create(trigS,TweenInfo.new(0.15),{Transparency=0.1}):Play()
    end
...
local function eatOneFood()
    ...
    pcall(function()
        for _,obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                if not obj or not obj.Parent then return end
                local nm = obj.Name:lower()
                if EAT_NAMES[nm] then table.insert(found,{obj=obj}) end
            end)
        end
    end)
    ...
    local e=found[1]
    pcall(function()
        if remConsume:IsA("RemoteFunction") then remConsume:InvokeServer(e.obj)
        else remConsume:FireServer(e.obj) end
    end)
    return true
end
...
_accDivLine(_kacCF,_y,KA_COR); _y=_y+9
_y=_y+_nfs(_kacCF,_y,"Alcance",5,125,kaRange,KA_COR,function(v) kaRange=v end)
...
local function autoFeedCampfire()
    ...
    while afcRunning do
        local fogPos=getCampfirePos(); if not fogPos then task.wait(3); break end
        local lista=collectItemsByNames(ns)
        if #lista==0 then pcall(function() if afcRef.status then afcRef.status.Text="⚠️ Sem combustível" end end); task.wait(5)
        else
            ciclo = ciclo + 1; pcall(function() if afcRef.status then afcRef.status.Text=string.format("🔥 Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then dropNearCampfire(e.part,e.obj,fogPos,s-1,LOTE); table.insert(loteE,e); total = total + 1; task.wait(0.1) end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local function autoFeedHP()
    ...
    while afhpRunning do
        local fogPos=getCampfirePos(); if not fogPos then task.wait(3); break end
        local hum=pcall(function() return Players.LocalPlayer.Character.Humanoid end) and Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        local hp=hum and (hum.Health/hum.MaxHealth*100) or 100
        if hp>afhpThreshold then task.wait(2)
        else
            local ns=afcBuildNameSet(); local lista=collectItemsByNames(ns)
            if #lista==0 then pcall(function() if afhpRef.status then afhpRef.status.Text="⚠️ Sem combustível" end end); task.wait(5)
            else
                pcall(function() if afhpRef.status then afhpRef.status.Text=string.format("❤️ HP=%d%% — %d itens",math.floor(hp),#lista) end end)
                local loteE={}; local running={true}
                for s=1,math.min(LOTE,#lista) do local e=lista[s]
                    if e and e.part and e.part.Parent then dropNearCampfire(e.part,e.obj,fogPos,s-1,LOTE); table.insert(loteE,e); total = total + 1; task.wait(0.1) end
                end; waitConsumed(loteE,TIMEOUT,running)
...
local function autoCookFood()
    ...
    while ackRunning do
        ...
        if #lista==0 then pcall(function() if ackRef.status then ackRef.status.Text="⚠️ Sem itens crus" end end); task.wait(5)
        else
            ciclo = ciclo + 1; pcall(function() if ackRef.status then ackRef.status.Text=string.format("🍖 Lote %d — %d disponíveis",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then dropNearCampfire(e.part,e.obj,fogPos,s-1,LOTE); table.insert(loteE,e); total = total + 1; task.wait(0.15) end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local function autoMachineGrind()
    ...
    while amgRunning do
        local lista=collectItemsByNames(ns)
        if #lista==0 then pcall(function() if amgRef.status then amgRef.status.Text="⚠️ Sem itens" end end); task.wait(5)
        else
            ciclo = ciclo + 1; pcall(function() if amgRef.status then amgRef.status.Text=string.format("⚙️ Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then
                    ...
                    dropAtPos(e.part,e.obj,dropPos); table.insert(loteE,e); total = total + 1; task.wait(0.15)
                end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local function autoBiofuel()
    ...
    while abfRunning do
        ...
        if #lista==0 then pcall(function() if abfRef.status then abfRef.status.Text="⚠️ Sem itens" end end); task.wait(5)
        else
            ciclo = ciclo + 1; pcall(function() if abfRef.status then abfRef.status.Text=string.format("🧪 Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then
                    local ang=(s/LOTE)*math.pi*2
                    local dropPos=Vector3.new(abfPos.X+math.cos(ang)*0.6,abfPos.Y,abfPos.Z+math.sin(ang)*0.6)
                    dropAtPos(e.part,e.obj,dropPos); table.insert(loteE,e); total = total + 1; task.wait(0.1)
                end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
