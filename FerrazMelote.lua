...
do
    ...
    local function makePremiumDrop(triggerY, opts, getVal, setVal, onSel,
        ...
        topDiv.BackgroundColor3 = BRING_CFG_COR
        topDiv.BackgroundTransparency = 0.55
        topDiv.BorderSizePixel = 0
        topDiv.AnchorPoint = Vector2.new(0.5, 0)
        ...
        -- Atualiza trigger com opção selecionada
        local function updateTrigger(opt)
            local cor = opt.cor or BRING_CFG_COR
            mainRef.Text   = opt.label or opt.key
            mainRef.TextColor3 = cor
            descRef.Text   = opt.desc or ""
            iconRef.Text   = opt.icon or "●"
            tagLblRef.Text = opt.tag or opt.label or ""
            tagLblRef.TextColor3 = cor
            tagBadgeRef.BackgroundColor3 = cor
            arrRef.TextColor3 = cor
            stripRef.BackgroundColor3 = cor
            glowRef.BackgroundColor3 = cor
        ...
        for i, opt in ipairs(opts) do
            ...
            if i < #opts then
                local div = Instance.new("Frame", itemF)
                div.BackgroundColor3 = Color3.fromRGB(30, 18, 55)
                div.BackgroundTransparency = 0.4; div.BorderSizePixel = 0
                div.Position = UDim2.new(0.04, 0, 1, -1)
            ...
            local ibG2 = Instance.new("UIGradient", iBox)
            ibG2.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(255,255,255)),
            ...
            chkF.BackgroundColor3 = (opt.key == getVal()) and cor or Color3.fromRGB(20, 12, 40)
            chkF.BackgroundTransparency = (opt.key == getVal()) and 0.15 or 0.4
            chkF.BorderSizePixel = 0; chkF.ZIndex = 34
            Instance.new("UICorner", chkF).CornerRadius = UDim.new(1, 0)
            local chkStroke = Instance.new("UIStroke", chkF)
            chkStroke.Color = cor; chkStroke.Thickness = 1.5; chkStroke.Transparency = 0.4
            ...
            chkLbl.Text = (opt.key == getVal()) and "●" or ""
            chkLbl.TextColor3 = Color3.fromRGB(255,255,255); chkLbl.ZIndex = 35

            -- Highlight inicial se selecionado
            if opt.key == getVal() then
            ...
            table.insert(itemBtns, {btn=iBtnReal, chk=chkLbl, chkF=chkF, bar=bar, frame=itemF, opt=opt, main=iMain})

            iBtnReal.MouseEnter:Connect(function()
                if opt.key ~= getVal() then
                    TweenService:Create(itemF,  TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(24,14,48), BackgroundTransparency=0.05}):Play()
                    TweenService:Create(bar,    TweenInfo.new(0.1), {BackgroundTransparency=0.2}):Play()
                    TweenService:Create(iMain,  TweenInfo.new(0.1), {TextColor3=Color3.fromRGB(190,148,255)}):Play()
                end
            end)
            iBtnReal.MouseLeave:Connect(function()
                if opt.key ~= getVal() then
                    TweenService:Create(itemF,  TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(20,12,40), BackgroundTransparency=0.15}):Play()
                    TweenService:Create(bar,    TweenInfo.new(0.1), {BackgroundTransparency=0.5}):Play()
                    TweenService:Create(iMain,  TweenInfo.new(0.1), {TextColor3=cor}):Play()
                end
            end)

            iBtnReal.MouseButton1Click:Connect(function()
                setVal(opt.key)
                updateTrigger(opt)
                for _, ib in ipairs(itemBtns) do
                    local sel = (ib.opt.key == opt.key)
                    ...
                    TweenService:Create(ib.main,  TweenInfo.new(0.15), {TextColor3=sel and (ib.opt.cor or BRING_CFG_COR) or Color3.fromRGB(ib.opt.cor and ib.opt.cor.R*255 or 148, ib.opt.cor and ib.opt.cor.G*255 or 112, ib.opt.cor and ib.opt.cor.B*255 or 220)}):Play()
                end
                if onSel then onSel(opt) end
                closeCurrentDrop()
            end)
