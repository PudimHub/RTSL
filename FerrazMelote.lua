...
local function nPlaySound(id)
    ...
    pcall(function()
        ...
        snd.RollOffMaxDistance=0; snd:Play()
...
-- ══════════════════════════════════════════════════════════════
-- ABA CONFIGURAÇÕES — VISUAL CUSTOMIZER v2 + SAVE/LOAD
-- 10 opções visuais + 12 Temas Prontos + Persistência automática
-- ══════════════════════════════════════════════════════════════
do
local VC_COR = Color3.fromRGB(200,160,255)
local HS = game:GetService("HttpService")
local CFG_FILE = "PudimHub_Config.json"
local function rgb(r,g,b) return Color3.fromRGB(r,g,b) end
local function ckey(c)
    return math.floor(c.R*255+.5)..","..math.floor(c.G*255+.5)..","..math.floor(c.B*255+.5)
end
local function c3arr(c) return {math.floor(c.R*255+.5),math.floor(c.G*255+.5),math.floor(c.B*255+.5)} end
local function arr3(t) return rgb(t[1],t[2],t[3]) end

-- ── Padrões do hub ────────────────────────────────────────────
local DEFAULT_THEME = {
    accent={148,112,220}, bg={68,44,108},   sidebar={56,36,92},
    topbar={58,38,96},    row={88,62,132},   rowhov={100,74,148},
    tabact={96,66,148},   text={255,248,255},muted={175,155,210},
    stroke={108,82,158},  divider={100,76,148},
}
local DEFAULT_SAVE = {
    theme=DEFAULT_THEME, transparency=0, cornerRadius=12,
    fontSize=11, fontName="GothamBold", glow=false,
}

-- ── Carrega config salva (ou usa padrão) ──────────────────────
local SAVE = {}
for k,v in pairs(DEFAULT_SAVE) do
    if type(v) == "table" then
        SAVE[k] = {}
        for kk,vv in pairs(v) do SAVE[k][kk]=vv end
    else
        SAVE[k] = v
    end
end
        ...
        if ok and type(data)=="table" then
            if type(data.theme)=="table" then
                for k,v in pairs(data.theme) do
                    if type(v)=="table" and #v==3 then SAVE.theme[k]=v end
                end
            end
            if type(data.transparency)=="number" then SAVE.transparency=data.transparency end
            if type(data.cornerRadius)=="number"  then SAVE.cornerRadius=data.cornerRadius end
            if type(data.fontSize)=="number"      then SAVE.fontSize=data.fontSize end
            if type(data.fontName)=="string"      then SAVE.fontName=data.fontName end
            if type(data.glow)=="boolean"         then SAVE.glow=data.glow end
        end
    end
end)
...
local THEME = {}
for k,v in pairs(SAVE.theme) do THEME[k] = arr3(v) end

-- ── Salva config no disco automaticamente ────────────────────
local function saveConfig()
    pcall(function()
        if not writefile then return end
        local themeData = {}
        for k,c in pairs(THEME) do themeData[k] = c3arr(c) end
        SAVE.theme = themeData
        writefile(CFG_FILE, HS:JSONEncode(SAVE))
    end)
end

-- ── Aplica tema: troca cores em todos os descendants ─────────
local function applyTheme(newT, noNotif, noSave)
    local swap = {}
    for role, oldC in pairs(THEME) do
        local newC = newT[role]
        if newC and ckey(oldC) ~= ckey(newC) then
            swap[ckey(oldC)] = newC
        end
    end
    pcall(function()
        for _, obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj.BackgroundTransparency ~= nil and obj.BackgroundTransparency < 0.99 then
                    local k = ckey(obj.BackgroundColor3)
                    if swap[k] then obj.BackgroundColor3 = swap[k] end
                end
                if obj.TextColor3 ~= nil then
                    local k = ckey(obj.TextColor3)
                    if swap[k] then obj.TextColor3 = swap[k] end
                end
                if obj:IsA("UIStroke") then
                    local k = ckey(obj.Color)
                    if swap[k] then obj.Color = swap[k] end
                end
                if obj:IsA("ScrollingFrame") then
                    local k = ckey(obj.ScrollBarImageColor3)
                    if swap[k] then obj.ScrollBarImageColor3 = swap[k] end
                end
            end)
        end
    end)
    ...
    if not noNotif then Notify.success("🎨 Visual", "Tema aplicado!") end
end

local function applyRole(role, newC, noNotif)
    local t = {}; for k,v in pairs(THEME) do t[k] = v end
    t[role] = newC; applyTheme(t, true)
    if not noNotif then Notify.success("🎨 Visual", "Cor atualizada!") end
end

-- ── Aplica fonte no hub inteiro ───────────────────────────────
local FONT_MAP = {
    Gotham=Enum.Font.Gotham, GothamBold=Enum.Font.GothamBold,
    GothamBlack=Enum.Font.GothamBlack, Arcade=Enum.Font.Arcade,
}
local function applyFont(fontName, noNotif)
    local fv = FONT_MAP[fontName] or Enum.Font.GothamBold
    pcall(function()
        for _,obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                    if obj.Font==Enum.Font.GothamBold or obj.Font==Enum.Font.Gotham
                    or obj.Font==Enum.Font.GothamBlack or obj.Font==Enum.Font.Arcade then
                        obj.Font = fv
                    end
                end
            end)
    ...
    SAVE.fontName = fontName; saveConfig()
    if not noNotif then Notify.success("✍️ Fonte","Fonte → "..fontName) end
end

-- ── 12 Temas Prontos ─────────────────────────────────────────
local TEMAS = {
    {e="🟣",n="Dark Purple", accent=rgb(148,112,220),bg=rgb(68,44,108), sidebar=rgb(56,36,92), topbar=rgb(58,38,96), row=rgb(88,62,132),  rowhov=rgb(100,74,148),tabact=rgb(96,66,148), text=rgb(255,248,255),muted=rgb(175,155,210),stroke=rgb(108,82,158), divider=rgb(100,76,148)},
    {e="🔵",n="Deep Blue",   accent=rgb(88,140,255), bg=rgb(16,28,72),  sidebar=rgb(12,22,60), topbar=rgb(14,24,64), row=rgb(24,40,88),   rowhov=rgb(30,50,110), tabact=rgb(30,55,120),text=rgb(240,245,255),muted=rgb(140,165,220),stroke=rgb(48,80,160),  divider=rgb(36,64,140)},
    {e="🌊",n="Ocean Teal",  accent=rgb(64,200,180), bg=rgb(12,44,52),  sidebar=rgb(8,34,42),  topbar=rgb(10,38,46), row=rgb(18,56,64),   rowhov=rgb(24,90,100), tabact=rgb(20,100,110),text=rgb(220,248,248),muted=rgb(120,185,185),stroke=rgb(40,130,130), divider=rgb(30,110,120)},
    {e="🟢",n="Forest Green",accent=rgb(80,210,110), bg=rgb(12,44,20),  sidebar=rgb(8,34,16),  topbar=rgb(10,38,18), row=rgb(18,56,26),   rowhov=rgb(24,90,36),  tabact=rgb(20,100,40), text=rgb(220,248,220),muted=rgb(120,185,130),stroke=rgb(40,130,60),  divider=rgb(30,110,50)},
    {e="🔴",n="Crimson Red", accent=rgb(220,70,90),  bg=rgb(60,14,22),  sidebar=rgb(48,10,18), topbar=rgb(52,12,20), row=rgb(80,22,32),   rowhov=rgb(90,20,34),  tabact=rgb(100,24,40), text=rgb(255,235,235),muted=rgb(200,145,155),stroke=rgb(140,40,58),  divider=rgb(120,30,46)},
    {e="🟠",n="Sunset",      accent=rgb(255,140,40), bg=rgb(60,28,8),   sidebar=rgb(48,22,6),  topbar=rgb(52,24,8),  row=rgb(78,38,12),   rowhov=rgb(110,52,14), tabact=rgb(120,60,18), text=rgb(255,242,220),muted=rgb(210,170,120),stroke=rgb(160,90,30),  divider=rgb(140,75,22)},
    {e="💛",n="Golden",      accent=rgb(255,200,50), bg=rgb(54,42,8),   sidebar=rgb(44,34,6),  topbar=rgb(48,36,8),  row=rgb(70,54,12),   rowhov=rgb(108,85,14), tabact=rgb(120,96,18), text=rgb(255,248,210),muted=rgb(210,190,130),stroke=rgb(160,130,30), divider=rgb(140,112,22)},
    {e="🩷",n="Hot Pink",    accent=rgb(255,80,165), bg=rgb(62,12,48),  sidebar=rgb(50,8,38),  topbar=rgb(54,10,42), row=rgb(80,18,62),   rowhov=rgb(108,18,82), tabact=rgb(120,22,92), text=rgb(255,230,245),muted=rgb(210,145,190),stroke=rgb(160,40,120), divider=rgb(140,30,105)},
    {e="🌸",n="Sakura",      accent=rgb(255,160,200),bg=rgb(56,22,40),  sidebar=rgb(46,16,32), topbar=rgb(50,18,36), row=rgb(74,28,52),   rowhov=rgb(108,60,95), tabact=rgb(120,68,105),text=rgb(255,238,248),muted=rgb(210,165,195),stroke=rgb(160,100,138),divider=rgb(140,85,122)},
    {e="🖤",n="Midnight",    accent=rgb(180,180,210),bg=rgb(10,10,18),  sidebar=rgb(6,6,12),   topbar=rgb(8,8,14),   row=rgb(18,18,28),   rowhov=rgb(24,24,42),  tabact=rgb(28,28,48),  text=rgb(230,230,240),muted=rgb(130,130,160),stroke=rgb(50,50,80),   divider=rgb(40,40,68)},
    {e="🤍",n="Arctic",      accent=rgb(100,180,255),bg=rgb(14,24,50),  sidebar=rgb(10,18,42), topbar=rgb(12,20,44), row=rgb(20,34,66),   rowhov=rgb(24,62,138), tabact=rgb(28,70,150), text=rgb(225,238,255),muted=rgb(140,175,220),stroke=rgb(50,100,180), divider=rgb(40,85,165)},
    {e="🤎",n="Coffee",      accent=rgb(200,140,80), bg=rgb(44,26,10),  sidebar=rgb(36,20,8),  topbar=rgb(38,22,8),  row=rgb(60,36,14),   rowhov=rgb(90,58,22),  tabact=rgb(100,65,26), text=rgb(250,235,215),muted=rgb(195,165,130),stroke=rgb(130,90,45),  divider=rgb(115,78,35)},
}

local PAL_ACCENT = {rgb(148,112,220),rgb(88,140,255),rgb(64,200,180),rgb(80,210,110),rgb(220,70,90),rgb(255,140,40),rgb(255,200,50),rgb(255,80,165)}
local PAL_BG     = {rgb(68,44,108),rgb(16,28,72),rgb(12,44,52),rgb(12,44,20),rgb(60,14,22),rgb(60,28,8),rgb(10,10,18),rgb(44,26,10)}
local PAL_TEXT   = {rgb(255,248,255),rgb(240,245,255),rgb(220,248,248),rgb(255,235,235),rgb(255,242,220),rgb(230,230,240),rgb(255,238,248),rgb(250,235,215)}
local PAL_CARD   = {rgb(88,62,132),rgb(24,40,88),rgb(18,56,64),rgb(18,56,26),rgb(80,22,32),rgb(78,38,12),rgb(18,18,28),rgb(60,36,14)}

cfgMkSec("🎨  VISUAL CUSTOMIZER", VC_COR)

-- ── Helper: swatch row ────────────────────────────────────────
local function mkSwatchRow(cf, yPos, label, palette, onPick)
    local lbl = Instance.new("TextLabel", cf)
    lbl.BackgroundTransparency = 1
    lbl.Position = UDim2.new(0,12,0,yPos); lbl.Size = UDim2.new(1,-24,0,16)
    lbl.Font = Enum.Font.GothamBold; lbl.Text = label
    lbl.TextColor3 = rgb(200,188,230); lbl.TextSize = 9
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 7
    for i, col in ipairs(palette) do
        local sw = Instance.new("TextButton", cf)
        sw.BackgroundColor3 = col; sw.BorderSizePixel = 0
        sw.Position = UDim2.new(0, 10 + (i-1)*35, 0, yPos+20)
        sw.Size = UDim2.new(0,28,0,28); sw.Text = ""; sw.ZIndex = 7
        Instance.new("UICorner",sw).CornerRadius = UDim.new(0,7)
        local swS = Instance.new("UIStroke",sw)
        swS.Color = rgb(255,255,255); swS.Thickness = 1.5; swS.Transparency = 0.75
        swS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        sw.MouseButton1Click:Connect(function()
            TweenService:Create(swS,TweenInfo.new(0.12),{Transparency=0.1}):Play()
            task.delay(0.35,function() pcall(function() TweenService:Create(swS,TweenInfo.new(0.3),{Transparency=0.75}):Play() end) end)
            onPick(col)
        end)
        sw.MouseEnter:Connect(function() TweenService:Create(sw,TweenInfo.new(0.1),{Size=UDim2.new(0,31,0,31)}):Play() end)
        sw.MouseLeave:Connect(function() TweenService:Create(sw,TweenInfo.new(0.1),{Size=UDim2.new(0,28,0,28)}):Play() end)
    end
end

-- ── Helper: slider dentro de contentFrame ────────────────────
local function mkCfSlider(cf, yPos, label, minV, maxV, defV, cor, fmt, onChange)
    local pct0 = (defV-minV)/(maxV-minV)
    local row = Instance.new("Frame",cf)
    row.BackgroundColor3 = rgb(72,50,108); row.BorderSizePixel = 0
    row.Position = UDim2.new(0,10,0,yPos); row.Size = UDim2.new(1,-20,0,60); row.ZIndex = 7
    Instance.new("UICorner",row).CornerRadius = UDim.new(0,12)
    local tl = Instance.new("TextLabel",row); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,14,0,0); tl.Size=UDim2.new(0.50,0,1,0)
    tl.Font=Enum.Font.GothamBold; tl.Text=label
    tl.TextColor3=rgb(215,205,235); tl.TextSize=11
    tl.TextXAlignment=Enum.TextXAlignment.Left; tl.TextYAlignment=Enum.TextYAlignment.Center; tl.ZIndex=8
    local curV = defV
    local vl = Instance.new("TextLabel",row); vl.BackgroundTransparency=1
    vl.Position=UDim2.new(0.52,0,0.5,-10); vl.Size=UDim2.new(0,38,0,20)
    vl.Font=Enum.Font.GothamBold; vl.Text=string.format(fmt or "%d",defV)
    vl.TextColor3=rgb(215,205,235); vl.TextSize=12; vl.TextXAlignment=Enum.TextXAlignment.Left; vl.ZIndex=9
    local track = Instance.new("Frame",row); track.BackgroundColor3=rgb(90,68,124)
    track.BorderSizePixel=0; track.Position=UDim2.new(0.52,44,0.5,-2)
    track.Size=UDim2.new(0.45,-54,0,4); track.ZIndex=9
    Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
    local fill = Instance.new("Frame",track); fill.BackgroundColor3=cor
    fill.BorderSizePixel=0; fill.Size=UDim2.new(pct0,0,1,0); fill.ZIndex=10
    Instance.new("UICorner",fill).CornerRadius=UDim.new(1,0)
    local knob = Instance.new("Frame",track); knob.BackgroundColor3=rgb(50,32,80)
    knob.BorderSizePixel=0; knob.AnchorPoint=Vector2.new(0.5,0.5)
    knob.Position=UDim2.new(pct0,0,0.5,0); knob.Size=UDim2.new(0,18,0,18); knob.ZIndex=11
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local dragging = false
    local function setV(pct)
        pct=math.clamp(pct,0,1)
        curV=math.floor(minV+(maxV-minV)*pct+0.5)
        fill.Size=UDim2.new(pct,0,1,0); knob.Position=UDim2.new(pct,0,0.5,0)
        vl.Text=string.format(fmt or "%d",curV); pcall(onChange,curV)
    end
...
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🎨",title="Cor de Destaque",color=VC_COR,
    summary="Muda a cor dos títulos, bordas e ícones do hub.",
    contentH=36+9+56+14,
})
local _y=36+8; _accDivLine(_cf,_y,VC_COR); _y=_y+9
mkSwatchRow(_cf,_y,"Escolha a cor de destaque:",PAL_ACCENT,function(c) applyRole("accent",c) end)
end
...
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🖋️",title="Cor dos Textos",color=rgb(200,200,255),
    summary="Altera a cor das palavras nos cards e labels.",
    contentH=36+9+56+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(200,200,255)); _y=_y+9
mkSwatchRow(_cf,_y,"Escolha a cor dos textos:",PAL_TEXT,function(c) applyRole("text",c) end)
end
...
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🌑",title="Cor do Fundo",color=rgb(160,180,255),
    summary="Muda o background principal do painel.",
    contentH=36+9+56+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(160,180,255)); _y=_y+9
mkSwatchRow(_cf,_y,"Escolha a cor do fundo:",PAL_BG,function(c)
    applyRole("bg",c)
    task.delay(0.05,function() applyRole("sidebar",c:Lerp(Color3.fromRGB(0,0,0),0.15)) end)
    task.delay(0.10,function() applyRole("topbar",c:Lerp(Color3.fromRGB(0,0,0),0.12)) end)
end)
end

-- ════════════════════════════════════════════════════════
-- 3B. IMAGEM DE FUNDO (Asset ID)
-- ════════════════════════════════════════════════════════
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🖼️",title="Imagem de Fundo",color=rgb(190,148,255),
    summary="Cole um Asset ID e veja antes de aplicar.",
    contentH=36+9+52+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(190,148,255)); _y=_y+9

-- ── Row com input + botão Visualizar ─────────────────────────────
local inputRow = Instance.new("Frame",_cf)
inputRow.BackgroundTransparency = 1; inputRow.BorderSizePixel = 0
inputRow.Size = UDim2.new(1,-16,0,36); inputRow.Position = UDim2.new(0,8,0,_y)
inputRow.ZIndex = 6

local inputBg = Instance.new("Frame",inputRow)
inputBg.BackgroundColor3 = Color3.fromRGB(28,18,52); inputBg.BorderSizePixel = 0
inputBg.Size = UDim2.new(1,-100,1,0); inputBg.Position = UDim2.new(0,0,0,0); inputBg.ZIndex = 7
Instance.new("UICorner",inputBg).CornerRadius = UDim.new(0,8)
Instance.new("UIStroke",inputBg).Color = Color3.fromRGB(90,60,140)

local inputBox = Instance.new("TextBox",inputBg)
inputBox.BackgroundTransparency = 1; inputBox.BorderSizePixel = 0
inputBox.Size = UDim2.new(1,-10,1,0); inputBox.Position = UDim2.new(0,8,0,0)
inputBox.Font = Enum.Font.Gotham; inputBox.TextSize = 11
inputBox.TextColor3 = Color3.fromRGB(210,200,240)
inputBox.PlaceholderText = "Cole o Asset ID aqui..."
inputBox.PlaceholderColor3 = Color3.fromRGB(100,80,140)
inputBox.Text = ""; inputBox.ClearTextOnFocus = false
inputBox.TextXAlignment = Enum.TextXAlignment.Left; inputBox.ZIndex = 8

local previewBtn = Instance.new("TextButton",inputRow)
previewBtn.BackgroundColor3 = Color3.fromRGB(148,112,220); previewBtn.BorderSizePixel = 0
previewBtn.Size = UDim2.new(0,90,1,0); previewBtn.Position = UDim2.new(1,-90,0,0)
previewBtn.Font = Enum.Font.GothamBold; previewBtn.Text = "👁 Visualizar"
previewBtn.TextColor3 = Color3.fromRGB(255,255,255); previewBtn.TextSize = 11; previewBtn.ZIndex = 7
Instance.new("UICorner",previewBtn).CornerRadius = UDim.new(0,8)

-- ── Popup de Pré-visualização ─────────────────────────────────────
previewBtn.MouseButton1Click:Connect(function()
    local rawId = inputBox.Text:match("%d+") or ""
    if rawId == "" then
        inputBox.PlaceholderText = "⚠ Digite um ID válido!"
        return
    end
    ...
    overlay.ZIndex = 200; overlay.Name = "BgPreviewOverlay"

    -- Painel central
    local popup = Instance.new("Frame", overlay)
    popup.Size = UDim2.new(0,420,0,480); popup.BackgroundColor3 = Color3.fromRGB(12,7,28)
    popup.BorderSizePixel = 0; popup.ZIndex = 201
    popup.AnchorPoint = Vector2.new(0.5,0.5)
    popup.Position = UDim2.new(0.5,0,0.5,0)
    Instance.new("UICorner",popup).CornerRadius = UDim.new(0,16)
    local popStroke = Instance.new("UIStroke",popup)
    popStroke.Color = Color3.fromRGB(148,112,220); popStroke.Thickness = 1.5

    -- Título
    local popTitle = Instance.new("TextLabel",popup)
    popTitle.BackgroundTransparency = 1; popTitle.BorderSizePixel = 0
    popTitle.Size = UDim2.new(1,-20,0,32); popTitle.Position = UDim2.new(0,10,0,10)
    popTitle.Font = Enum.Font.GothamBlack; popTitle.Text = "🖼  Pré-visualização"
    popTitle.TextColor3 = Color3.fromRGB(190,148,255); popTitle.TextSize = 14
    popTitle.TextXAlignment = Enum.TextXAlignment.Left; popTitle.ZIndex = 202

    -- ID exibido
    local idLabel = Instance.new("TextLabel",popup)
    idLabel.BackgroundColor3 = Color3.fromRGB(24,14,52); idLabel.BorderSizePixel = 0
    idLabel.Size = UDim2.new(1,-20,0,26); idLabel.Position = UDim2.new(0,10,0,46)
    idLabel.Font = Enum.Font.Code; idLabel.Text = "ID: "..rawId
    idLabel.TextColor3 = Color3.fromRGB(148,220,255); idLabel.TextSize = 12; idLabel.ZIndex = 202
    Instance.new("UICorner",idLabel).CornerRadius = UDim.new(0,6)

    -- Moldura da imagem
    local imgFrame = Instance.new("Frame",popup)
    imgFrame.BackgroundColor3 = Color3.fromRGB(22,12,44); imgFrame.BorderSizePixel = 0
    imgFrame.Size = UDim2.new(1,-20,0,310); imgFrame.Position = UDim2.new(0,10,0,80)
    imgFrame.ZIndex = 202
    Instance.new("UICorner",imgFrame).CornerRadius = UDim.new(0,10)
    Instance.new("UIStroke",imgFrame).Color = Color3.fromRGB(80,50,120)

    -- Imagem grande
    local previewImg = Instance.new("ImageLabel",imgFrame)
    previewImg.BackgroundTransparency = 1; previewImg.BorderSizePixel = 0
    previewImg.Size = UDim2.new(1,-8,1,-8); previewImg.Position = UDim2.new(0,4,0,4)
    previewImg.Image = assetUrl; previewImg.ScaleType = Enum.ScaleType.Fit; previewImg.ZIndex = 203

    -- Loading label (desaparece quando imagem carregar)
    local loadLbl = Instance.new("TextLabel",imgFrame)
    loadLbl.BackgroundTransparency = 1; loadLbl.BorderSizePixel = 0
    loadLbl.Size = UDim2.new(1,0,1,0); loadLbl.Position = UDim2.new(0,0,0,0)
    loadLbl.Font = Enum.Font.Gotham; loadLbl.Text = "⏳ Carregando..."
    loadLbl.TextColor3 = Color3.fromRGB(150,130,190); loadLbl.TextSize = 13; loadLbl.ZIndex = 204
    task.spawn(function()
        local cs = game:GetService("ContentProvider")
        pcall(function() cs:PreloadAsync({previewImg}) end)
        loadLbl.Text = ""
    end)

    -- Botão APLICAR
    local applyBtn = Instance.new("TextButton",popup)
    applyBtn.BackgroundColor3 = Color3.fromRGB(120,80,220); applyBtn.BorderSizePixel = 0
    applyBtn.Size = UDim2.new(0,180,0,38); applyBtn.Position = UDim2.new(0,10,0,406)
    applyBtn.Font = Enum.Font.GothamBold; applyBtn.Text = "✅  Aplicar Fundo"
    applyBtn.TextColor3 = Color3.fromRGB(255,255,255); applyBtn.TextSize = 13; applyBtn.ZIndex = 202
    Instance.new("UICorner",applyBtn).CornerRadius = UDim.new(0,10)

    -- Botão FECHAR
    local closeBtn = Instance.new("TextButton",popup)
    closeBtn.BackgroundColor3 = Color3.fromRGB(60,30,90); closeBtn.BorderSizePixel = 0
    closeBtn.Size = UDim2.new(0,180,0,38); closeBtn.Position = UDim2.new(1,-190,0,406)
    closeBtn.Font = Enum.Font.GothamBold; closeBtn.Text = "✖  Fechar"
    closeBtn.TextColor3 = Color3.fromRGB(200,180,230); closeBtn.TextSize = 13; closeBtn.ZIndex = 202
    Instance.new("UICorner",closeBtn).CornerRadius = UDim.new(0,10)

    -- Animação de entrada
    popup.Size = UDim2.new(0,0,0,0)
    TweenService:Create(popup, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
        {Size=UDim2.new(0,420,0,480)}):Play()
    ...
    applyBtn.MouseButton1Click:Connect(function()
        ...
        local old = MainFrame:FindFirstChild("_BgImage")
        if old then old:Destroy() end

        -- Cria ImageLabel atrás de tudo no MainFrame
        ...
        bgImg.Image = assetUrl; bgImg.ScaleType = Enum.ScaleType.Crop
        bgImg.ImageTransparency = 0.15; bgImg.ZIndex = 1
...
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🟣",title="Cor dos Cards",color=rgb(180,140,240),
    summary="Muda a cor dos accordions e linhas do hub.",
    contentH=36+9+56+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(180,140,240)); _y=_y+9
mkSwatchRow(_cf,_y,"Escolha a cor dos cards:",PAL_CARD,function(c)
    applyRole("row",c)
    task.delay(0.05,function() applyRole("rowhov",c:Lerp(Color3.fromRGB(255,255,255),0.10)) end)
    task.delay(0.10,function() applyRole("tabact",c:Lerp(Color3.fromRGB(255,255,255),0.08)) end)
end)
end

-- ════════════════════════════════════════════════════════
-- 5. TRANSPARÊNCIA DO HUB
-- ════════════════════════════════════════════════════════
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🔲",title="Transparência do Hub",color=rgb(180,200,255),
    summary="Controla a opacidade do painel principal.",
    contentH=36+9+68+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(180,200,255)); _y=_y+9
mkCfSlider(_cf,_y,"Transparência",0,60,SAVE.transparency,rgb(180,200,255),"%d%%",function(v)
    SAVE.transparency = v
    pcall(function() MainFrame.BackgroundTransparency = v/100 end)
    saveConfig()
end)
end

-- ════════════════════════════════════════════════════════
-- 6. ARREDONDAMENTO DOS CARDS
-- ════════════════════════════════════════════════════════
do
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="📐",title="Arredondamento",color=rgb(160,220,200),
    summary="Ajusta o arredondamento dos cantos dos cards.",
    contentH=36+9+68+14,
})
local _y=36+8; _accDivLine(_cf,_y,rgb(160,220,200)); _y=_y+9
mkCfSlider(_cf,_y,"Raio dos Cantos",0,24,SAVE.cornerRadius,rgb(160,220,200),"%dpx",function(v)
    SAVE.cornerRadius = v
    local cr = UDim.new(0,v)
    pcall(function()
        for _,obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj:IsA("UICorner") then
                    local cur = obj.CornerRadius.Offset
                    if cur >= 7 and cur <= 16 then obj.CornerRadius = cr end
                end
            end)
        end
    end)
...
do
local FONTS = {
    {n="Gotham",v=Enum.Font.Gotham},{n="GothamBold",v=Enum.Font.GothamBold},
    {n="GothamBlack",v=Enum.Font.GothamBlack},{n="Arcade",v=Enum.Font.Arcade},
}
local FONTE_COR = rgb(255,210,120)
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="✍️",title="Fonte do Hub",color=FONTE_COR,
    summary="Troca a fonte de todos os textos do hub.",
    contentH=36+9+100+14,
})
local _y=36+8; _accDivLine(_cf,_y,FONTE_COR); _y=_y+9
for i, fd in ipairs(FONTS) do
    local col = (i-1) % 2
    local rr  = math.floor((i-1) / 2)
    local isActive = (fd.n == SAVE.fontName)
    local btn = Instance.new("TextButton",_cf)
    btn.BackgroundColor3 = isActive and rgb(80,55,120) or rgb(60,40,90)
    btn.BorderSizePixel = 0
    btn.Position = UDim2.new(col*0.5, col==0 and 10 or 6, 0, _y + rr*50)
    btn.Size = UDim2.new(0.5,-16,0,42); btn.Text = ""; btn.ZIndex = 7
    Instance.new("UICorner",btn).CornerRadius = UDim.new(0,10)
    local bS = Instance.new("UIStroke",btn)
    bS.Color = FONTE_COR; bS.Thickness = isActive and 2.0 or 1.2
    bS.Transparency = isActive and 0.1 or 0.6
    bS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local btl = Instance.new("TextLabel",btn)
    btl.BackgroundTransparency=1; btl.Size=UDim2.new(1,0,1,0)
    btl.Font=fd.v; btl.Text=fd.n
    btl.TextColor3=rgb(220,210,245); btl.TextSize=10; btl.ZIndex=8
    btn.MouseEnter:Connect(function() TweenService:Create(bS,TweenInfo.new(0.12),{Transparency=0.15}):Play() end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(bS,TweenInfo.new(0.15),{Transparency=SAVE.fontName==fd.n and 0.1 or 0.6}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=rgb(80,55,120)}):Play()
        applyFont(fd.n)
    end)
end
end
...
-- ════════════════════════════════════════════════════════
do
local SZ_COR = rgb(255,240,140)
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🔡",title="Tamanho da Fonte",color=SZ_COR,
    summary="Altera o tamanho base dos textos do hub.",
    contentH=36+9+68+14,
})
local _y=36+8; _accDivLine(_cf,_y,SZ_COR); _y=_y+9
local baseSizes = {}
mkCfSlider(_cf,_y,"Tamanho",8,16,SAVE.fontSize,SZ_COR,"%dpx",function(v)
    SAVE.fontSize = v
    pcall(function()
        for _,obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                    local id = tostring(obj)
                    if not baseSizes[id] then baseSizes[id] = obj.TextSize end
                    local base = baseSizes[id]
                    if base >= 9 and base <= 13 then obj.TextSize = v end
                end
            end)
        end
    end)
...
-- ════════════════════════════════════════════════════════
do
local GLOW_COR = rgb(200,160,255)
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="✨",title="Glow nas Bordas",color=GLOW_COR,
    summary="Ativa brilho nas bordas dos cards do hub.",
    contentH=36+9+44+14,
})
local _y=36+8; _accDivLine(_cf,_y,GLOW_COR); _y=_y+9
local glowStrokes = {}
local function setGlow(en)
    SAVE.glow = en; saveConfig()
    if en then
        pcall(function()
            for _,obj in ipairs(MainFrame:GetDescendants()) do
                pcall(function()
                    if obj:IsA("UIStroke") and obj.ApplyStrokeMode==Enum.ApplyStrokeMode.Border then
                        if not glowStrokes[obj] then glowStrokes[obj]={t=obj.Thickness,tr=obj.Transparency} end
                        TweenService:Create(obj,TweenInfo.new(0.3),{Thickness=2.5,Transparency=0.05}):Play()
                    end
                end)
            end
        end)
        Notify.success("✨ Glow","Glow nas bordas ativado!")
    else
        pcall(function()
            for obj,orig in pairs(glowStrokes) do
                pcall(function() TweenService:Create(obj,TweenInfo.new(0.3),{Thickness=orig.t,Transparency=orig.tr}):Play() end)
            end
        end)
        glowStrokes = {}
        Notify.info("✨ Glow","Glow nas bordas desativado.")
    end
end
_accToggle(_cf,_y,"✨  Glow nas Bordas",SAVE.glow,GLOW_COR,setGlow)
end

-- ════════════════════════════════════════════════════════
-- 10. TEMAS PRONTOS — 12 temas
-- ════════════════════════════════════════════════════════
do
local THEME_COR = rgb(255,200,120)
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🎭",title="Temas Prontos",color=THEME_COR,
    summary="12 temas completos — clique para aplicar tudo de uma vez.",
    contentH=36+9+238+14,
})
local _y=36+8; _accDivLine(_cf,_y,THEME_COR); _y=_y+9
for i, tm in ipairs(TEMAS) do
    local col  = (i-1) % 2
    local rrow = math.floor((i-1) / 2)
    local btn  = Instance.new("TextButton",_cf)
    btn.BackgroundColor3 = tm.bg; btn.BorderSizePixel = 0
    btn.Position = UDim2.new(col*0.5, col==0 and 10 or 6, 0, _y + rrow*42)
    btn.Size = UDim2.new(0.5,-16,0,36); btn.Text = ""; btn.ZIndex = 7
    Instance.new("UICorner",btn).CornerRadius = UDim.new(0,10)
    local bS = Instance.new("UIStroke",btn)
    bS.Color = tm.accent; bS.Thickness = 1.5; bS.Transparency = 0.35
    bS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local dot = Instance.new("Frame",btn)
    dot.BackgroundColor3 = tm.accent; dot.BorderSizePixel = 0
    dot.AnchorPoint = Vector2.new(0,0.5)
    dot.Position = UDim2.new(0,8,0.5,0); dot.Size = UDim2.new(0,10,0,10); dot.ZIndex = 8
    Instance.new("UICorner",dot).CornerRadius = UDim.new(1,0)
    local ntl = Instance.new("TextLabel",btn)
    ntl.BackgroundTransparency = 1
    ntl.Position = UDim2.new(0,24,0,0); ntl.Size = UDim2.new(1,-28,1,0)
    ntl.Font = Enum.Font.GothamBold; ntl.Text = tm.e.." "..tm.n
    ntl.TextColor3 = tm.text; ntl.TextSize = 9
    ntl.TextXAlignment = Enum.TextXAlignment.Left
    ntl.TextYAlignment = Enum.TextYAlignment.Center; ntl.ZIndex = 8
    btn.MouseEnter:Connect(function() TweenService:Create(bS,TweenInfo.new(0.12),{Transparency=0.0}):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(bS,TweenInfo.new(0.15),{Transparency=0.35}):Play() end)
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(bS,TweenInfo.new(0.08),{Thickness=3.0}):Play()
        task.delay(0.25,function() pcall(function() TweenService:Create(bS,TweenInfo.new(0.2),{Thickness=1.5}):Play() end) end)
        applyTheme({
            accent=tm.accent, bg=tm.bg,    sidebar=tm.sidebar,
            topbar=tm.topbar, row=tm.row,  rowhov=tm.rowhov,
            tabact=tm.tabact, text=tm.text, muted=tm.muted,
            stroke=tm.stroke, divider=tm.divider,
        })
        Notify.success("🎭 Tema",tm.e.." "..tm.n.." aplicado!")
    end)
end
end -- Temas Prontos
...
-- ════════════════════════════════════════════════════════
do
local RST_COR = rgb(255,100,100)
local _,_cf=makeAccordionCard(Pages["Configuracoes"],cfg2NextLO,{
    icon="🔄",title="Resetar Visual",color=RST_COR,
    summary="Volta todas as cores e configs para o padrão original.",
    contentH=36+9+48+14,
})
local _y=36+8; _accDivLine(_cf,_y,RST_COR); _y=_y+9
local rstBtn = Instance.new("TextButton",_cf)
rstBtn.BackgroundColor3 = rgb(80,20,30); rstBtn.BorderSizePixel = 0
rstBtn.Position = UDim2.new(0,10,0,_y); rstBtn.Size = UDim2.new(1,-20,0,40)
rstBtn.Text = "🔄  Resetar para o Padrão"; rstBtn.Font = Enum.Font.GothamBold
rstBtn.TextColor3 = rgb(255,180,180); rstBtn.TextSize = 11; rstBtn.ZIndex = 7
Instance.new("UICorner",rstBtn).CornerRadius = UDim.new(0,10)
local rstS = Instance.new("UIStroke",rstBtn)
rstS.Color = RST_COR; rstS.Thickness = 1.5; rstS.Transparency = 0.4
rstS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
rstBtn.MouseEnter:Connect(function() TweenService:Create(rstS,TweenInfo.new(0.12),{Transparency=0.0}):Play() end)
rstBtn.MouseLeave:Connect(function() TweenService:Create(rstS,TweenInfo.new(0.15),{Transparency=0.4}):Play() end)
rstBtn.MouseButton1Click:Connect(function()
    TweenService:Create(rstBtn,TweenInfo.new(0.1),{BackgroundColor3=rgb(120,30,40)}):Play()
    task.delay(0.2,function() pcall(function() TweenService:Create(rstBtn,TweenInfo.new(0.2),{BackgroundColor3=rgb(80,20,30)}):Play() end) end)
    -- Aplica tema padrão
    local defT = {}
    for k,v in pairs(DEFAULT_THEME) do defT[k] = arr3(v) end
    applyTheme(defT, true, true)
    -- Reseta transparência
    pcall(function() MainFrame.BackgroundTransparency = 0 end)
    -- Reseta fonte
    applyFont("GothamBold", true)
    -- Reseta tamanho da fonte — não temos base fácil, só apaga o save
    -- Reseta arredondamento
    local cr = UDim.new(0,12)
    pcall(function()
        for _,obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj:IsA("UICorner") then
                    local cur = obj.CornerRadius.Offset
                    if cur >= 4 and cur <= 20 then obj.CornerRadius = cr end
                end
            end)
        end
    end)
    -- Deleta o arquivo de config
    pcall(function() if delfile then delfile(CFG_FILE) end end)
    -- Reseta SAVE
    SAVE.transparency = 0; SAVE.cornerRadius = 12
    SAVE.fontSize = 11; SAVE.fontName = "GothamBold"; SAVE.glow = false
    Notify.success("🔄 Resetado","Visual voltou ao padrão original!")
end)
end -- Resetar

-- ════════════════════════════════════════════════════════
-- APLICAR CONFIG SALVA NA INICIALIZAÇÃO
-- ════════════════════════════════════════════════════════
task.defer(function()
    -- Tema
    local savedT = {}
    local needsApply = false
    for k,v in pairs(SAVE.theme) do
        savedT[k] = arr3(v)
        local defArr = DEFAULT_THEME[k]
        if defArr and (v[1]~=defArr[1] or v[2]~=defArr[2] or v[3]~=defArr[3]) then
            needsApply = true
        end
    end
    if needsApply then applyTheme(savedT, true, true) end
    -- Transparência
    if SAVE.transparency > 0 then
        pcall(function() MainFrame.BackgroundTransparency = SAVE.transparency/100 end)
    end
    -- Arredondamento
    if SAVE.cornerRadius ~= 12 then
        local cr = UDim.new(0, SAVE.cornerRadius)
        pcall(function()
            for _,obj in ipairs(MainFrame:GetDescendants()) do
                pcall(function()
                    if obj:IsA("UICorner") then
    ...
    -- Fonte
    if SAVE.fontName ~= "GothamBold" then applyFont(SAVE.fontName, true) end
    -- Glow
    if SAVE.glow then
        pcall(function()
            for _,obj in ipairs(MainFrame:GetDescendants()) do
                pcall(function()
                    if obj:IsA("UIStroke") and obj.ApplyStrokeMode==Enum.ApplyStrokeMode.Border then
                        TweenService:Create(obj,TweenInfo.new(0.4),{Thickness=2.5,Transparency=0.05}):Play()
                    end
                end)
            end
        end)
