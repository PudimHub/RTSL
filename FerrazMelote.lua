...
local godTakeDmgHook = nil   -- hookfunction backup de TakeDamage
local godDiedConns   = {}    -- conexões Died por personagem

-- ──────────────────────────────────────────────────────
-- GOD MODE — 5 camadas empilhadas (do mais eficaz ao backup)
--
-- COMO O 99 NIGHTS APLICA DANO:
--   O servidor chama hum:TakeDamage(x) ou seta hum.Health diretamente.
--   O cliente não pode impedir isso diretamente, mas pode:
--   1) Hookar TakeDamage para não fazer nada
--   2) Setar BreakJointsOnDeath = false (personagem não quebra ao morrer)
--   3) Usar HealthChanged para restaurar HP imediatamente
--   4) Usar Died para cancelar o estado de morte via ChangeState
--   5) Heartbeat como segurança final a cada frame
-- ──────────────────────────────────────────────────────
local function setGodMod(state)
    ...
    if state then
        ...
        local function applyGodModToChar(ch)
            ...
            -- ── CAMADA 1: BreakJointsOnDeath = false ─────────────────────
            -- Impede que o Roblox destrua as joints (ragdoll) quando HP = 0
            -- Isso evita o personagem cair e a câmera travar na tela de morte
            pcall(function() hum.BreakJointsOnDeath = false end)
            pcall(function() hum.RequiresNeck = false end)  -- não morre por perder a cabeça

            -- ── CAMADA 2: Hook em TakeDamage ─────────────────────────────
            -- O mob chama hum:TakeDamage(x) → intercepta e não faz nada
            pcall(function()
                if hookfunction then
                    -- Desfaz hook anterior se existir
                    if godTakeDmgHook then
                        pcall(function() hookfunction(hum.TakeDamage, godTakeDmgHook) end)
                        godTakeDmgHook = nil
                    end
                    godTakeDmgHook = hookfunction(hum.TakeDamage, function() end)
                end
            end)

            -- ── CAMADA 3: HP alto mas razoável (não math.huge) ───────────
            -- 9999 é alto o suficiente — não quebra câmera nem UI
            pcall(function()
                hum.MaxHealth = 9999
                hum.Health    = 9999
            end)

            -- ── CAMADA 4: HealthChanged — restaura no instante que cair ──
            -- Cobre danos que passam pelo TakeDamage hookado
            local hcConn
            ...
            -- ── CAMADA 5: Died → cancela estado de morte ─────────────────
            -- Quando o servidor força Health = 0 e o Humanoid entra em Died,
            -- usamos ChangeState(None) para sair do estado morto e restauramos HP
            local diedConn
            diedConn = hum.Died:Connect(function()
                if not godModEnabled then diedConn:Disconnect(); return end
                task.defer(function()  -- task.defer executa no próximo ciclo, após o estado ser setado
                    pcall(function()
                        hum:ChangeState(Enum.HumanoidStateType.None)
                        hum.MaxHealth = 9999
                        hum.Health    = 9999
                        -- Reposiciona se estiver fora do mapa
        ...
        -- ── CAMADA 5B: Heartbeat — restaura HP a cada frame ──────────────
        -- Backup final: se qualquer camada falhar, isso pega o que restou
        ...
        -- Reaplica ao renascer (ex: morreu antes de ativar)
        ...
        Notify.send({type="custom", icon="♾️", accent=Color3.fromRGB(140,255,140),
            title="God Mod ATIVO",
            msg="5 camadas: TakeDamage bloqueado, morte cancelada, HP restaurado!",
            duration=4})
    else
        -- ── DESLIGAR: remove tudo ─────────────────────────────────────────
        if godModConn     then godModConn:Disconnect();     godModConn     = nil end
        if godModCharConn then godModCharConn:Disconnect(); godModCharConn = nil end

        -- Desconecta conexões Died
        for _, c in ipairs(godDiedConns) do pcall(function() c:Disconnect() end) end
        godDiedConns = {}

        -- Restaura TakeDamage original
        pcall(function()
            if hookfunction and godTakeDmgHook then
                local ch = Player.Character
                if ch then
                    local hum = ch:FindFirstChildWhichIsA("Humanoid")
                    if hum then hookfunction(hum.TakeDamage, godTakeDmgHook) end
                end
                godTakeDmgHook = nil
            end
        end)
        ...
        Notify.info("♾️ God Mod", "Desativado — HP e morte normais restaurados.")
    end
end
...
-- ══════════════════════════════════════════════════════════════
-- KILL AURA v3 — Sistema correto para 99 Nights in the Forest
-- ══════════════════════════════════════════════════════════════
--
-- COMO O 99 NIGHTS REGISTRA DANO COM TOOL:
--   O servidor valida colisão FÍSICA entre a Part da tool e o mob.
--   Teleportar o JOGADOR não adianta — o servidor não leva em conta
--   onde o jogador está, só onde a tool tocou.
--
-- SISTEMA CORRETO (em ordem de prioridade):
--   MÉTODO 1 — Bring Mob + Activate (mais confiável):
--     Traz o mob até 1.5 studs do jogador → ativa a tool →
--     restaura posição do mob. O jogador não se move, câmera fica ok.
--
--   MÉTODO 2 — Hitbox Expansion:
--     Expande o Size da Part da tool para cobrir todo o range →
--     ativa a tool → restaura o size.
--     O servidor registra o toque fisicamente.
--
--   MÉTODO 3 — FireTouchInterest (executores avançados):
--     Simula colisão entre a Part da tool e cada BasePart do mob.
--     Não requer TP nem Size hack.
--
--   MODO AUTO: loop que ativa a tool automaticamente a cada 0.35s
--              sem precisar clicar. Aplica todos os métodos juntos.
-- ══════════════════════════════════════════════════════════════
local kaEnabled   = false
local kaRange     = 30
local kaAutoLoop  = nil   -- Heartbeat do modo auto
local kaCharConn  = nil   -- CharacterAdded
local kaBusy      = false

-- ── Utilitários ────────────────────────────────────────────────
-- Pega a Part principal da tool (onde a hitbox fica)
local function getToolPart(tool)
    if not tool then return nil end
    -- Tenta Handle primeiro (padrão Roblox), depois qualquer BasePart
    return tool:FindFirstChild("Handle")
        or tool:FindFirstChildWhichIsA("BasePart")
end

-- Coleta todos os modelos de mob vivos no range
local function getMobModelsInRange(hrp, range)
    local mobs = {}
    local seen  = {}
    ...
    -- Verifica pastas conhecidas + workspace inteiro
    for _, src in ipairs({
...
-- ── MÉTODO 1: Bring Mob → Player ───────────────────────────────
-- Move o mob até o jogador (não o jogador até o mob).
-- O jogador fica parado, câmera não treme.
local function applyBringMob(tool, mobs, hrp)
    local toolPart = getToolPart(tool)
    local origCFs  = {}
    -- Salva CFrames originais
    for _, mob in ipairs(mobs) do
        pcall(function()
            local root = mob:FindFirstChild("HumanoidRootPart")
                      or mob:FindFirstChild("Torso")
                      or mob:FindFirstChildWhichIsA("BasePart")
            if root then origCFs[mob] = root.CFrame end
        end)
    end
    -- Move mob até 1.5 studs à frente do jogador
    for _, mob in ipairs(mobs) do
        pcall(function()
            local root = mob:FindFirstChild("HumanoidRootPart")
                      or mob:FindFirstChild("Torso")
            ...
            if mob:FindFirstChild("HumanoidRootPart") then
                mob:SetPrimaryPartCFrame(targetCF)
            else
                root.CFrame = targetCF
            end
        end)
    end
    -- Aguarda 2 frames para servidor processar
    task.wait()
    task.wait()
    -- Ativa a tool
    pcall(function() tool:Activate() end)
    -- Aguarda 2 frames para hitbox registrar
    task.wait()
    task.wait()
    -- Restaura posições
    for _, mob in ipairs(mobs) do
        pcall(function()
            if not origCFs[mob] then return end
            local root = mob:FindFirstChild("HumanoidRootPart")
                      or mob:FindFirstChild("Torso")
                      or mob:FindFirstChildWhichIsA("BasePart")
            if not root then return end
            if mob:FindFirstChild("HumanoidRootPart") then
                mob:SetPrimaryPartCFrame(origCFs[mob])
            else
                root.CFrame = origCFs[mob]
            end
        end)
    end
end
...
-- ── MÉTODO 2: Hitbox Expansion ─────────────────────────────────
-- Expande o Size da Part da tool para cobrir o range inteiro.
-- O servidor vê a colisão física da Part grande tocando os mobs.
local function applyHitboxExpansion(tool, origSize)
    local toolPart = getToolPart(tool)
    if not toolPart then return end
    pcall(function() toolPart.Size = Vector3.new(kaRange*2, kaRange*2, kaRange*2) end)
    task.wait()  -- 1 frame para o servidor ver o novo tamanho
    pcall(function() tool:Activate() end)
    task.wait()  -- 1 frame para registrar
    pcall(function() toolPart.Size = origSize end)
end

-- ── MÉTODO 3: FireTouchInterest ─────────────────────────────────
-- Simula toque entre a Part da tool e cada BasePart do mob.
-- Funciona em executores que suportam a função (Synapse, etc.).
local hasFTI = (typeof(firetouchinterest) == "function") or
               (rawget(getfenv(), "firetouchinterest") ~= nil)

local function applyFireTouchInterest(tool, mobs)
    if not hasFTI then return end
    local toolPart = getToolPart(tool)
    if not toolPart then return end
    for _, mob in ipairs(mobs) do
        pcall(function()
            for _, part in ipairs(mob:GetDescendants()) do
                if part:IsA("BasePart") then
                    firetouchinterest(toolPart, part, 0)  -- touch begin
                end
            end
        end)
    end
    ...
    for _, mob in ipairs(mobs) do
        pcall(function()
            for _, part in ipairs(mob:GetDescendants()) do
                if part:IsA("BasePart") then
                    firetouchinterest(toolPart, part, 1)  -- touch end
                end
            end
        end)
    end
...
-- ── Núcleo: executa todos os métodos ───────────────────────────
local kaOrigToolSize = nil  -- salva size original da tool

local function kaDoAttack(tool)
    if not kaEnabled or kaBusy then return end
    local ch = Player.Character; if not ch then return end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    if not tool or not tool.Parent then return end

    local mobs = getMobModelsInRange(hrp, kaRange)
    if #mobs == 0 then
        -- Sem mobs no range: ativa a tool normalmente (para cortar árvores etc.)
        pcall(function() tool:Activate() end)
        return
    end
    ...
    -- Salva size original da tool (só uma vez por ferramenta)
    local toolPart = getToolPart(tool)
    if toolPart and not kaOrigToolSize then
        kaOrigToolSize = toolPart.Size
    end
    local origSize = kaOrigToolSize or Vector3.new(1,1,1)

    -- Método 3 (FireTouchInterest) — mais limpo, não mexe em nada visual
    if hasFTI then
        pcall(applyFireTouchInterest, tool, mobs)
    else
        -- Método 2 (Hitbox Expansion) — rápido e eficaz
        pcall(applyHitboxExpansion, tool, origSize)
        -- Método 1 (Bring Mob) — fallback adicional para mobs que escapam
        pcall(applyBringMob, tool, mobs, hrp)
    end

    kaBusy = false
end

-- ── Loop automático (modo AUTO) ────────────────────────────────
-- Ataca automaticamente a cada 0.35s sem precisar clicar.
-- Encontra a tool equipada pelo próprio loop.
local function startKillAura()
    if kaAutoLoop then pcall(function() kaAutoLoop:Disconnect() end) end
    kaOrigToolSize = nil

    local attackInterval = 0.35  -- segundos entre ataques
    local lastAttack     = 0

    kaAutoLoop = RunService.Heartbeat:Connect(function()
        if not kaEnabled then return end
        local now = tick()
        if now - lastAttack < attackInterval then return end
        lastAttack = now
        ...
        pcall(function()
            ...
            -- Não executa se jogador estiver morto
            local hum = ch:FindFirstChildWhichIsA("Humanoid")
            if not hum or hum.Health <= 0 then return end
            kaDoAttack(tool)
        end)
...
local function stopKillAura()
    ...
    kaBusy = false
    kaOrigToolSize = nil
    -- Restaura size da tool se ficou expandida
    pcall(function()
        local ch = Player.Character; if not ch then return end
        local tool = ch:FindFirstChildWhichIsA("Tool")
        if not tool then return end
        local toolPart = getToolPart(tool)
        if toolPart and kaOrigToolSize then
            toolPart.Size = kaOrigToolSize
        end
    end)
end
...
kaBtnClick.MouseButton1Click:Connect(function()
    ...
    if kaEnabled then
        startKillAura()
        local ftiStr = hasFTI and " [FireTouchInterest]" or " [HitboxExpand+BringMob]"
        Notify.send({type="custom",icon="⚔️",accent=KA_COR,
            title="Kill Aura AUTO",
            msg="Atacando mobs automaticamente ("..kaRange.." studs)"..ftiStr,
            duration=5})
    else
        stopKillAura()
