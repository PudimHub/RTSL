...
local function executarBring(key)
    ...
    for i, entry in ipairs(eligiveis) do
        ...
        pcall(function()
            ...
            -- Raio maior (4-9 studs) + camadas para itens não sobrepostos
            local layer  = math.floor((i-1) / 12)  -- 12 itens por camada
            local slot   = (i-1) % 12
...
-- ── ATAQUE PRINCIPAL: Teleporte + Swing Real ───────────────────
-- Teleporta o jogador para ao lado de cada mob e ativa a tool.
-- O servidor vê o jogador no lugar certo → dano aceito.
local function kaAttackTeleport(hrp, tool, mobs)
    ...
    for _, entry in ipairs(mobs) do
        pcall(function()
            ...
            -- Teleporta para 1.5 studs na frente do mob, orientado para ele
            local dir = (mobRoot.Position - hrp.Position)
            local dirXZ = Vector3.new(dir.X, 0, dir.Z)
            if dirXZ.Magnitude < 0.1 then dirXZ = Vector3.new(1,0,0) end
            local standPos = mobRoot.Position + dirXZ.Unit * (-1.5)
            standPos = Vector3.new(standPos.X, mobRoot.Position.Y, standPos.Z)

            hrp.CFrame = CFrame.new(standPos, mobRoot.Position)
            task.wait()  -- aguarda 1 frame para replicar posição

            -- Ativa a tool (swing real ao lado do mob)
            pcall(function() tool:Activate() end)
            pcall(function()
                local ls = tool:FindFirstChildWhichIsA("LocalScript")
                    or tool:FindFirstChildWhichIsA("Script")
                if ls then
                    local act = ls:FindFirstChild("Activate")
                    if act and act:IsA("BindableEvent") then act:Fire() end
                end
            end)

            -- Camada extra: firetouchinterest se disponível
            if hasFTI and handle then
                -- Move handle PARA CIMA do mob antes do FTI
            ...
            -- Camada extra: tenta fireremote com args comuns do 99 Nights
            if dmgRemote then
                pcall(function()
                    dmgRemote:FireServer(entry.model, entry.hum)
                end)
                pcall(function()
                    dmgRemote:FireServer(mobRoot)
                end)
            end

            task.wait(0.08)  -- aguarda hitbox da animação registrar
        end)
    end
...
local function startKillAura()
    ...
    kaAutoLoop = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            kaRunning = true
            task.spawn(function()
                kaAttackTeleport(hrp, tool, mobs)
                kaRunning = false
            end)
        end)
    end)
...
local function atSoltar(part, obj, pos)
    -- Remove tudo que pode prender o item
    for _, v in ipairs(obj:GetDescendants()) do
        pcall(function()
            if v:IsA("BodyPosition") or v:IsA("BodyForce") or v:IsA("BodyVelocity")
    ...
    -- BodyVelocity para forçar queda no servidor (sem network ownership)
    pcall(function()
...
local function autoTriturador()
    ...
    local ALTURA   = 15   -- 15 studs acima do triturador (pedido pelo usuário)
    ...
    local function dropPos(slotIdx, totalSlots)
        local angle = (slotIdx / totalSlots) * math.pi * 2
        local r = 1.5 + (slotIdx % 3) * 0.8  -- 1.5 / 2.3 / 3.1 studs de dist
        return Vector3.new(
...
local function startAutoExplore()
    ...
    aeBodyGyro.D = 50; aeBodyGyro.CFrame = hrp.CFrame
    ...
    aeConn = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            if dist > 5 then
                -- Voa em direção ao ponto mais próximo da névoa
                ...
                -- Chegou! Marca como visitado e escolhe o próximo mais próximo
                aeBodyVel.Velocity = Vector3.zero
                visitados   = visitados + 1
                targetAtual = nil   -- vai escolher o próximo no próximo frame
            end
