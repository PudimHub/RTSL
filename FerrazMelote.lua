...
local bringDestMode = "jogador"  -- "jogador", "fogueira" ou "ceu"
        ...
        pcall(function()
            ...
            elseif obj:IsA("BasePart") then
                ...
                for pc in pairs(pchars) do
                    if par == pc or (par and par.IsAncestorOf and par == pc) then isPlayerChar=true; break end
                end
...
do
    ...
    modeBtn.Font=Enum.Font.GothamBlack; modeBtn.Text="Modo ?"
    ...
    -- Popup com 3 botões (aparece ACIMA do botão)
    local popup=Instance.new("Frame",infoPnl); popup.BackgroundColor3=Color3.fromRGB(18,10,3)
    popup.BorderSizePixel=0; popup.ZIndex=20; popup.Visible=false
    popup.Size=UDim2.new(0,98,0,118); popup.Position=UDim2.new(1,-101,0,-124)
    Instance.new("UICorner",popup).CornerRadius=UDim.new(0,10)
    local popupS=Instance.new("UIStroke",popup); popupS.Color=Color3.fromRGB(252,198,0); popupS.Thickness=2; popupS.Transparency=0.3
    -- Ponteirinha no fundo do popup
    local arrow=Instance.new("Frame",popup); arrow.BackgroundColor3=Color3.fromRGB(252,198,0)
    arrow.BackgroundTransparency=0.3; arrow.BorderSizePixel=0
    arrow.Position=UDim2.new(0.5,-6,1,-1); arrow.Size=UDim2.new(0,12,0,8); arrow.Rotation=45; arrow.ZIndex=19

    local popupOpen = false
    local function closePopup() popup.Visible=false; popupOpen=false end

    for idx, opt in ipairs(MODE_OPTS) do
        local ob=Instance.new("TextButton",popup); ob.BackgroundTransparency=1
        ob.BorderSizePixel=0; ob.Size=UDim2.new(1,-8,0,34); ob.Position=UDim2.new(0,4,0,(idx-1)*38+4)
        ob.Font=Enum.Font.GothamBlack; ob.Text=opt.icon.."  "..opt.label; ob.TextSize=10
        ob.TextColor3=MODE_COLORS[opt.key] or Color3.fromRGB(255,235,190); ob.TextXAlignment=Enum.TextXAlignment.Left; ob.ZIndex=21
        ob.AutoButtonColor=false
        Instance.new("UICorner",ob).CornerRadius=UDim.new(0,8)
        local obS=Instance.new("UIStroke",ob); obS.Color=MODE_COLORS[opt.key]; obS.Thickness=1.5; obS.Transparency=0.7
        local descLb=Instance.new("TextLabel",ob); descLb.BackgroundTransparency=1
        descLb.Position=UDim2.new(0,28,0,18); descLb.Size=UDim2.new(1,-32,0,12)
        descLb.Font=Enum.Font.Gotham; descLb.Text=opt.desc; descLb.TextColor3=Color3.fromRGB(140,110,60); descLb.TextSize=7; descLb.TextXAlignment=Enum.TextXAlignment.Left; descLb.ZIndex=22
        ob.MouseEnter:Connect(function()
            TweenService:Create(ob,TweenInfo.new(0.1),{BackgroundTransparency=0.8}):Play()
            ob.BackgroundColor3=MODE_COLORS[opt.key]
        end)
        ob.MouseLeave:Connect(function() TweenService:Create(ob,TweenInfo.new(0.1),{BackgroundTransparency=1}):Play() end)
    ...
    UserInputService.InputBegan:Connect(function(inp)
        if popupOpen and inp.UserInputType==Enum.UserInputType.MouseButton1 then
            task.wait()
...
local BUILD_GROUPS_DEF = {
    -- Casas (house, hut, bungalow, shack, cottage) — SEM cabin
    { key="house",    name="Casas",              icon="🏡", color=Color3.fromRGB(245,175,80),  kw={"house","hut","bungalow","shack","cottage","home","chalet"} },
    -- Cabanas (cabin, lodge) — separado de Casas
    { key="cabin",    name="Cabanas",             icon="🏠", color=Color3.fromRGB(210,140,60),  kw={"cabin","lodge","ranger cabin","hunting cabin","fishing cabin","hunter cabin","warm clothing cabin"} },
    -- Abrigos & Galpões (shed, storage, silo, barn) — separado de Cabanas
    { key="shed",     name="Abrigos & Galpões",  icon="🪵", color=Color3.fromRGB(190,130,60),  kw={"shed","shelter","storage","silo","barn"} },
    -- Torres (watchtower, radio, lookout) — separado de Cultistas
    { key="tower",    name="Torres & Postos",    icon="🗼", color=Color3.fromRGB(100,180,255), kw={"watchtower","watch tower","lookout tower","guard tower","bell tower","radio tower","lighthouse","water tower"} },
    -- Minas & Cavernas
    { key="mine",     name="Minas & Cavernas",   icon="⛏️", color=Color3.fromRGB(140,110,80),  kw={"mine","mineshaft","mine shaft","cave","cavern","tunnel","bunker","underground"} },
    -- Cultistas (stronghold, cultist base, cultist king)
    { key="cultist",  name="Cultistas",          icon="⚔️", color=Color3.fromRGB(180,60,255),  kw={"cultist stronghold","cultist base","cultist king","cultist camp","stronghold","fortress","fort"} },
    -- Torre Cultista separada (cultist tower = tower com cultist)
    { key="cultisttower", name="Torres Cultistas", icon="🗼", color=Color3.fromRGB(200,80,255), kw={"cultist tower"} },
    -- Templos da Selva (jungle temple, mother temple) — novo bioma
    { key="jungle",   name="Selva & Templos",    icon="🌿", color=Color3.fromRGB(60,200,80),   kw={"jungle temple","jungle chest","mother temple","jungle","selva","temple of"} },
    -- Ruínas & Templos antigos (ancient ruins, old fort, abandoned pillars)
    { key="ruin",     name="Ruínas & Templos",   icon="🏛️", color=Color3.fromRGB(200,160,80),  kw={"ruin","ancient ruin","ancient temple","abandoned pillar","old fort","castle","keep","temple"} },
    -- UFO & Aliens
    { key="ufo",      name="UFO & Aliens",       icon="🛸", color=Color3.fromRGB(60,255,180),  kw={"ufo","alien","mothership","nave","ovni","broken ufo"} },
    -- Fazendas & Celeiros
    { key="farm",     name="Fazendas & Celeiros",icon="🌾", color=Color3.fromRGB(140,210,80),  kw={"farm","greenhouse","stable","windmill","orchard","garden"} },
    -- Construções Civis (banco, igreja, escola, museu, mercado, etc.)
    { key="civil",    name="Construções Civis",  icon="🏢", color=Color3.fromRGB(100,160,255), kw={"bank","clinic","hospital","school","church","court","playground","bakery","diner","market","shop","restroom","depot","warehouse","station","gas station","museum","library","post office"} },
    -- Acampamentos (camp, outpost, tent, checkpoint)
    { key="camp",     name="Acampamentos",       icon="⛺", color=Color3.fromRGB(255,200,60),  kw={"hunter camp","military camp","ranger camp","abandoned camp","outpost","checkpoint","tent","camp"} },
    -- Abandonados & Destroços (crash, wreckage, abandoned)
    { key="abandon",  name="Abandonados",        icon="🏚️", color=Color3.fromRGB(160,130,100), kw={"abandoned","wreckage","plane crash","plane wreckage","crash site","broken","derelict"} },
    -- Ponds & Fontes
    { key="water",    name="Ponds & Fontes",     icon="🐟", color=Color3.fromRGB(80,200,255),  kw={"pond","pool","spring","hot spring","lake","river","frog pond","algal pond","bone pond","cold pond","fresh pond"} },
    -- Área Vulcânica (volcano, lava, ammo furnace, scorpion pit)
    { key="volcanic", name="Área Vulcânica",     icon="🌋", color=Color3.fromRGB(255,100,30),  kw={"volcano","volcanic","lava fall","lava pool","lava-isolated","scorpion pit","ammo furnace","volcanic church","hellephant","cultist king base"} },
    -- Especiais (meteor, bridge, armory, dock)
    { key="special",  name="Especiais",          icon="✨", color=Color3.fromRGB(255,230,80),  kw={"meteor crater","meteor","bridge","dam","dock","harbor","armory","ammo"} },
}

-- Classifica um nome de construção no grupo mais adequado
-- Estratégia: testa keywords mais longas primeiro para evitar matches errados
-- Ex: "cultist tower" deve ir para cultisttower, não para tower ou cultist
local function classifyBuilding(nameLow)
    -- Primeira passagem: testa TODOS os grupos por keywords exatas (palavra completa)
    -- Segunda passagem: testa substring, priorizando keyword mais longa
    local bestKey = nil
    local bestLen = 0
    for _, g in ipairs(BUILD_GROUPS_DEF) do
        for _, kw in ipairs(g.kw) do
            if nameLow:find(kw, 1, true) then
                if #kw > bestLen then
                    bestLen = #kw
                    bestKey = g.key
                end
    ...
    return bestKey  -- nil se não encontrou → grupo dinâmico
end
...
local KNOWN_STRUCTURES = {
    ...
    -- Selva (Jungle Biome — Março 2026)
    "Jungle Temple","JungleTemple","Mother Temple","MotherTemple",
    "Jungle Chest","JungleChest","Tar Pit","TarPit",
    "Jungle Cultist Base","JungleCultistBase",
    -- Fazendas e natureza
...
local function makeFarmSec(title, cor)
    ...
    sec.BackgroundColor3 = Color3.fromRGB(26,15,6); sec.BackgroundTransparency = 0
    sec.BorderSizePixel  = 0; sec.Size = UDim2.new(1,0,0,26); sec.LayoutOrder = fNextLO(); sec.ZIndex = 4
    Instance.new("UICorner", sec).CornerRadius = UDim.new(0,9)
    local g = Instance.new("UIGradient",sec)
    g.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(40,22,8)),ColorSequenceKeypoint.new(1,Color3.fromRGB(18,10,3))}); g.Rotation = 135
    local s = Instance.new("UIStroke",sec); s.Color = Color3.fromRGB(8,4,0); s.Thickness = 2.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local bar = Instance.new("Frame",sec); bar.BackgroundColor3 = cor; bar.BorderSizePixel = 0
    bar.Size = UDim2.new(0,5,0.75,0); bar.Position = UDim2.new(0,0,0.12,0); bar.ZIndex = 5
    Instance.new("UICorner",bar).CornerRadius = UDim.new(0,4)
    ...
    lbl.TextColor3 = Color3.fromRGB(255,255,255); lbl.TextSize = 10
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 5
    Instance.new("UIStroke",lbl).Color = Color3.fromRGB(8,4,0)
end

-- ── Utilitário: construtor de card de farm genérico ───────────────
...
local function makeFarmCard(h, cor, icon, title, descTxt)
    ...
    card.Size = UDim2.new(1,0,0,h); card.LayoutOrder = fNextLO(); card.ZIndex = 5
    Instance.new("UICorner",card).CornerRadius = UDim.new(0,12)
    ...
    local tb = Instance.new("Frame",card); tb.BackgroundColor3 = cor; tb.BorderSizePixel = 0; tb.Size = UDim2.new(1,0,0,4); tb.ZIndex = 6; Instance.new("UICorner",tb).CornerRadius = UDim.new(0,12)
    local lb = Instance.new("Frame",card); lb.BackgroundColor3 = cor; lb.BorderSizePixel = 0; lb.Size = UDim2.new(0,5,0.76,0); lb.Position = UDim2.new(0,0,0.12,0); lb.ZIndex = 6; Instance.new("UICorner",lb).CornerRadius = UDim.new(0,4)

    local ib = Instance.new("Frame",card); ib.BackgroundColor3 = cor; ib.BackgroundTransparency = 0.3; ib.BorderSizePixel = 0
    ...
    local il = Instance.new("TextLabel",ib); il.BackgroundTransparency = 1; il.Size = UDim2.new(1,0,1,0); il.Text = icon; il.TextSize = 18; il.Font = Enum.Font.GothamBold; il.ZIndex = 8
    ...
    tl.Font = Enum.Font.GothamBlack; tl.Text = title; tl.TextColor3 = Color3.fromRGB(255,255,255); tl.TextSize = 12; tl.TextXAlignment = Enum.TextXAlignment.Left; tl.ZIndex = 7; Instance.new("UIStroke",tl).Color = Color3.fromRGB(8,4,0)

    local dl = Instance.new("TextLabel",card); dl.BackgroundTransparency = 1; dl.Position = UDim2.new(0,54,0,27); dl.Size = UDim2.new(1,-216,0,26)
    dl.Font = Enum.Font.Gotham; dl.Text = descTxt; dl.TextColor3 = Color3.fromRGB(180,160,110); dl.TextSize = 9; dl.TextXAlignment = Enum.TextXAlignment.Left; dl.TextWrapped = true; dl.ZIndex = 7

    local sl = Instance.new("TextLabel",card); sl.BackgroundTransparency = 1; sl.Position = UDim2.new(0,54,0,h-18); sl.Size = UDim2.new(1,-216,0,16)
    sl.Font = Enum.Font.GothamBold; sl.Text = ""; sl.TextColor3 = cor; sl.TextSize = 9; sl.TextXAlignment = Enum.TextXAlignment.Left; sl.ZIndex = 7

    local aBtn = Instance.new("TextButton",card); aBtn.BackgroundColor3 = cor; aBtn.BackgroundTransparency = 0; aBtn.BorderSizePixel = 0
    aBtn.Position = UDim2.new(1,-148,0,9); aBtn.Size = UDim2.new(0,140,0,34)
    aBtn.Font = Enum.Font.GothamBlack; aBtn.Text = icon.."  ATIVAR"; aBtn.TextColor3 = Color3.fromRGB(255,255,255); aBtn.TextSize = 11; aBtn.ZIndex = 8; aBtn.AutoButtonColor = false
    Instance.new("UICorner",aBtn).CornerRadius = UDim.new(0,10)
    local aBtnS = Instance.new("UIStroke",aBtn); aBtnS.Color = Color3.fromRGB(8,4,0); aBtnS.Thickness = 2.5; aBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local aBtnG = Instance.new("UIGradient",aBtn); aBtnG.Rotation = 90
    local shine = Instance.new("Frame",aBtn); shine.BackgroundColor3 = Color3.fromRGB(255,255,255); shine.BackgroundTransparency = 0.65; shine.BorderSizePixel = 0
...
do
    ...
    btn.MouseButton1Click:Connect(function()
        if af2Running then
            af2Running=false; btn.Text="🔥  ATIVAR"; btnGrad(btnG,Color3.fromRGB(255,220,60),Color3.fromRGB(200,140,0))
            status.Text="⏹ Parado"; TweenService:Create(cardS,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
            Notify.error("🔥 Auto Fogueira (sem Log)","⏹ Desativado")
            task.delay(1.5,function() pcall(function() status.Text="" end) end)
        else
            btn.Text="⏹  PARAR"; btnGrad(btnG,Color3.fromRGB(220,60,60),Color3.fromRGB(140,20,20))
            TweenService:Create(cardS,TweenInfo.new(0.2),{Color=AF2_COR}):Play()
            task.spawn(function()
                autoFogueiraSemLog()
                if not af2Running then btn.Text="🔥  ATIVAR"; btnGrad(btnG,Color3.fromRGB(255,220,60),Color3.fromRGB(200,140,0)) end
            end)
        end
    end)
end
...
do
    ...
    btn.MouseButton1Click:Connect(function()
        if acRunning then
            acRunning=false; btn.Text="🍖  ATIVAR"; btnGrad(btnG,Color3.fromRGB(80,220,80),Color3.fromRGB(30,140,30))
            status.Text="⏹ Parado"; TweenService:Create(cardS,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
            Notify.error("🍖 Auto Cozinhar","⏹ Desativado")
            task.delay(1.5,function() pcall(function() status.Text="" end) end)
        else
            btn.Text="⏹  PARAR"; btnGrad(btnG,Color3.fromRGB(220,60,60),Color3.fromRGB(140,20,20))
            TweenService:Create(cardS,TweenInfo.new(0.2),{Color=AC_COR}):Play()
            task.spawn(function()
                autoCozinhar()
                if not acRunning then btn.Text="🍖  ATIVAR"; btnGrad(btnG,Color3.fromRGB(80,220,80),Color3.fromRGB(30,140,30)) end
            end)
        end
    end)
end
...
do
    ...
    -- ── POPUP de % ─────────────────────────────────────────────────
    local popup=Instance.new("Frame",ScreenGui); popup.BackgroundColor3=Color3.fromRGB(22,12,18); popup.BorderSizePixel=0
    popup.Size=UDim2.new(0,164,0,162); popup.Visible=false; popup.ZIndex=300
    Instance.new("UICorner",popup).CornerRadius=UDim.new(0,12)
    local popupS=Instance.new("UIStroke",popup); popupS.Color=AHP_COR; popupS.Thickness=2; popupS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local popupG=Instance.new("UIGradient",popup); popupG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(44,18,30)),ColorSequenceKeypoint.new(1,Color3.fromRGB(18,6,12))}); popupG.Rotation=135

    local pTitle=Instance.new("TextLabel",popup); pTitle.BackgroundTransparency=1; pTitle.Position=UDim2.new(0,10,0,7); pTitle.Size=UDim2.new(1,-20,0,16)
    pTitle.Font=Enum.Font.GothamBlack; pTitle.Text="🔥 Selecionar %"; pTitle.TextColor3=AHP_COR; pTitle.TextSize=11; pTitle.TextXAlignment=Enum.TextXAlignment.Left; pTitle.ZIndex=301

    local pGrid=Instance.new("Frame",popup); pGrid.BackgroundTransparency=1; pGrid.Position=UDim2.new(0,8,0,28); pGrid.Size=UDim2.new(1,-16,1,-34); pGrid.ZIndex=301
    local pLayout=Instance.new("UIGridLayout",pGrid); pLayout.CellSize=UDim2.new(0,70,0,26); pLayout.CellPaddingSize=UDim2.new(0,4,0,4); pLayout.SortOrder=Enum.SortOrder.LayoutOrder
    ...
    local function pctRefreshBtns()
        for v, pb in pairs(pctBtns) do
            ...
            pb.BackgroundColor3 = sel and AHP_COR or Color3.fromRGB(44,20,30)
            pb.BackgroundTransparency = sel and 0.25 or 0
            pb.TextColor3 = Color3.fromRGB(255,255,255)
            local pbs = pb:FindFirstChildWhichIsA("UIStroke"); if pbs then pbs.Transparency = sel and 0 or 0.55 end
        end
    end
    for _, v in ipairs(pctVals) do
        local pb=Instance.new("TextButton",pGrid); pb.BackgroundColor3=Color3.fromRGB(44,20,30); pb.BorderSizePixel=0
        pb.Font=Enum.Font.GothamBlack; pb.Text=v.."%"; pb.TextColor3=Color3.fromRGB(255,255,255); pb.TextSize=12; pb.ZIndex=302; pb.AutoButtonColor=false
        Instance.new("UICorner",pb).CornerRadius=UDim.new(0,7)
        local pbs=Instance.new("UIStroke",pb); pbs.Color=AHP_COR; pbs.Thickness=1.2; pbs.Transparency=0.55; pbs.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
        pctBtns[v] = pb
        pb.MouseButton1Click:Connect(function()
            ahpThreshold=v; pctBtn.Text=v.."% ▾"; popup.Visible=false; ahpPopupOpen=false; pctRefreshBtns()
        end)
    end
    pctRefreshBtns()

    local function openPctPopup()
        local ap = pctBtn.AbsolutePosition
        popup.Position = UDim2.new(0, ap.X-40, 0, ap.Y-170)
        popup.Visible=true; ahpPopupOpen=true
    end

    pctBtn.MouseButton1Click:Connect(function()
        if ahpPopupOpen then popup.Visible=false; ahpPopupOpen=false
    ...
    MainFrame.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 and ahpPopupOpen then
            ...
            if mx<pA.X or mx>pA.X+pSz.X or my<pA.Y or my>pA.Y+pSz.Y then
                popup.Visible=false; ahpPopupOpen=false
            end
        end
    end)
    ...
    btn.MouseButton1Click:Connect(function()
        if ahpRunning then
            ahpRunning=false; btn.Text="🔥  ATIVAR"; btnGrad(btnG,Color3.fromRGB(255,100,140),Color3.fromRGB(200,40,80))
            status.Text="⏹ Parado"; TweenService:Create(cardS,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
            Notify.error("🔥 Fogueira HP","⏹ Desativado"); task.delay(1.5,function() pcall(function() status.Text="" end) end)
        else
            btn.Text="⏹  PARAR"; btnGrad(btnG,Color3.fromRGB(220,60,60),Color3.fromRGB(140,20,20))
            TweenService:Create(cardS,TweenInfo.new(0.2),{Color=AHP_COR}):Play(); task.spawn(autoFogueiraHP)
        end
    end)
end
...
do
    ...
    -- ── POPUP seletor ─────────────────────────────────────────────
    local gdPopup=Instance.new("Frame",ScreenGui); gdPopup.BackgroundColor3=Color3.fromRGB(22,14,6); gdPopup.BorderSizePixel=0
    gdPopup.Size=UDim2.new(0,296,0,286); gdPopup.Visible=false; gdPopup.ZIndex=300
    Instance.new("UICorner",gdPopup).CornerRadius=UDim.new(0,12)
    local gdPopupS=Instance.new("UIStroke",gdPopup); gdPopupS.Color=GD_COR; gdPopupS.Thickness=2; gdPopupS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local gdG=Instance.new("UIGradient",gdPopup); gdG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(40,24,8)),ColorSequenceKeypoint.new(1,Color3.fromRGB(18,10,3))}); gdG.Rotation=135

    -- Header
    local gdH=Instance.new("Frame",gdPopup); gdH.BackgroundColor3=GD_COR; gdH.BackgroundTransparency=0.3; gdH.BorderSizePixel=0
    gdH.Size=UDim2.new(1,0,0,32); gdH.ZIndex=301; Instance.new("UICorner",gdH).CornerRadius=UDim.new(0,12)
    local gdHL=Instance.new("TextLabel",gdH); gdHL.BackgroundTransparency=1; gdHL.Position=UDim2.new(0,10,0,0); gdHL.Size=UDim2.new(0.65,0,1,0)
    gdHL.Font=Enum.Font.GothamBlack; gdHL.Text="⚙️ Itens para Triturar"; gdHL.TextColor3=Color3.fromRGB(255,255,255); gdHL.TextSize=11; gdHL.TextXAlignment=Enum.TextXAlignment.Left; gdHL.ZIndex=302

    -- Botão TODOS
    local gdTodos=Instance.new("TextButton",gdH); gdTodos.BackgroundColor3=Color3.fromRGB(40,24,8); gdTodos.BorderSizePixel=0
    gdTodos.AnchorPoint=Vector2.new(1,0.5); gdTodos.Position=UDim2.new(1,-6,0.5,0); gdTodos.Size=UDim2.new(0,62,0,22)
    gdTodos.Font=Enum.Font.GothamBlack; gdTodos.Text="Todos"; gdTodos.TextColor3=GD_COR; gdTodos.TextSize=10; gdTodos.ZIndex=303; gdTodos.AutoButtonColor=false
    Instance.new("UICorner",gdTodos).CornerRadius=UDim.new(0,6)
    local gdTodosS=Instance.new("UIStroke",gdTodos); gdTodosS.Color=GD_COR; gdTodosS.Thickness=1.5; gdTodosS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

    -- Scroll com grid
    local gdScroll=Instance.new("ScrollingFrame",gdPopup); gdScroll.BackgroundTransparency=1; gdScroll.BorderSizePixel=0
    gdScroll.Position=UDim2.new(0,6,0,38); gdScroll.Size=UDim2.new(1,-12,1,-44)
    gdScroll.ScrollBarThickness=3; gdScroll.ScrollBarImageColor3=GD_COR
    gdScroll.CanvasSize=UDim2.new(0,0,0,0); gdScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; gdScroll.ZIndex=301
    local gdGrid=Instance.new("UIGridLayout",gdScroll); gdGrid.CellSize=UDim2.new(0.5,-4,0,28); gdGrid.CellPaddingSize=UDim2.new(0,4,0,4); gdGrid.SortOrder=Enum.SortOrder.LayoutOrder
    Instance.new("UIPadding",gdScroll).PaddingTop=UDim.new(0,4)
    ...
    local function gdRefreshLabel()
        ...
        gdTodos.BackgroundColor3 = allOn and GD_COR or Color3.fromRGB(40,24,8)
        gdTodos.BackgroundTransparency = allOn and 0.25 or 0
        gdTodosS.Transparency = allOn and 0 or 0.4
    end

    for i, item in ipairs(GRIND_ITEMS) do
        local ib2=Instance.new("TextButton",gdScroll); ib2.BackgroundColor3=Color3.fromRGB(30,18,6); ib2.BorderSizePixel=0
        ib2.Font=Enum.Font.GothamBold; ib2.Text=item.name; ib2.TextColor3=Color3.fromRGB(200,180,140); ib2.TextSize=10; ib2.LayoutOrder=i; ib2.ZIndex=302; ib2.AutoButtonColor=false
        Instance.new("UICorner",ib2).CornerRadius=UDim.new(0,7)
        local ib2S=Instance.new("UIStroke",ib2); ib2S.Color=GD_COR; ib2S.Thickness=1.2; ib2S.Transparency=0.7; ib2S.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
        ...
        ib2.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(ib2,TweenInfo.new(0.12),{BackgroundColor3=on and GD_COR or Color3.fromRGB(30,18,6), BackgroundTransparency=on and 0.25 or 0}):Play()
            ib2S.Transparency=on and 0 or 0.7
            ib2.TextColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(200,180,140)
            gdRefreshLabel()
        end)
    end
    ...
    gdTodos.MouseButton1Click:Connect(function()
        ...
        for _, item in ipairs(GRIND_ITEMS) do
            ...
            if d then
                TweenService:Create(d.btn,TweenInfo.new(0.12),{BackgroundColor3=allActive and GD_COR or Color3.fromRGB(30,18,6), BackgroundTransparency=allActive and 0.25 or 0}):Play()
                d.stroke.Transparency=allActive and 0 or 0.7
                d.btn.TextColor3=allActive and Color3.fromRGB(255,255,255) or Color3.fromRGB(200,180,140)
            end
        end
        gdRefreshLabel()
    end)
    ...
    local function openGdPopup()
        ...
        gdPopup.Position=UDim2.new(0,ap.X,0,ap.Y-294)
        gdPopup.Visible=true; grindPopupOpen=true
    end

    selBtn.MouseButton1Click:Connect(function()
        if grindPopupOpen then gdPopup.Visible=false; grindPopupOpen=false
        else openGdPopup() end
    end)
    ...
    MainFrame.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 and grindPopupOpen then
            ...
            if mx<pA.X or mx>pA.X+pSz.X or my<pA.Y or my>pA.Y+pSz.Y then
                gdPopup.Visible=false; grindPopupOpen=false
            end
        end
    end)
...
-- Popup flutuante (acima do botão)
local fbcPopup = Instance.new("Frame",fbcCard); fbcPopup.BackgroundColor3=Color3.fromRGB(18,10,3)
fbcPopup.BorderSizePixel=0; fbcPopup.ZIndex=22; fbcPopup.Visible=false
fbcPopup.Size=UDim2.new(0,106,0,128); fbcPopup.Position=UDim2.new(1,-107,0,-136)
Instance.new("UICorner",fbcPopup).CornerRadius=UDim.new(0,10)
local fbcPopupS=Instance.new("UIStroke",fbcPopup); fbcPopupS.Color=FBC_COR; fbcPopupS.Thickness=2; fbcPopupS.Transparency=0.3
-- Ponteirinha
local fbcArrow=Instance.new("Frame",fbcPopup); fbcArrow.BackgroundColor3=FBC_COR
fbcArrow.BackgroundTransparency=0.3; fbcArrow.BorderSizePixel=0
fbcArrow.Position=UDim2.new(0.5,-6,1,-2); fbcArrow.Size=UDim2.new(0,12,0,8); fbcArrow.Rotation=45; fbcArrow.ZIndex=21

local fbcPopupOpen = false
local function closeFbcPopup() fbcPopup.Visible=false; fbcPopupOpen=false end

for idx, opt in ipairs(FBC_MODES) do
    local ob=Instance.new("TextButton",fbcPopup); ob.BackgroundTransparency=1
    ob.BorderSizePixel=0; ob.Size=UDim2.new(1,-8,0,36); ob.Position=UDim2.new(0,4,0,(idx-1)*40+4)
    ob.Font=Enum.Font.GothamBlack; ob.Text=opt.icon.."  "..opt.label; ob.TextSize=10
    ob.TextColor3=opt.color; ob.TextXAlignment=Enum.TextXAlignment.Left; ob.ZIndex=23; ob.AutoButtonColor=false
    Instance.new("UICorner",ob).CornerRadius=UDim.new(0,8)
    local obS=Instance.new("UIStroke",ob); obS.Color=opt.color; obS.Thickness=1.5; obS.Transparency=0.7
    local descLb=Instance.new("TextLabel",ob); descLb.BackgroundTransparency=1
    descLb.Position=UDim2.new(0,26,0,19); descLb.Size=UDim2.new(1,-30,0,12)
    descLb.Font=Enum.Font.Gotham; descLb.Text=opt.desc; descLb.TextColor3=Color3.fromRGB(140,110,60); descLb.TextSize=7; descLb.TextXAlignment=Enum.TextXAlignment.Left; descLb.ZIndex=24
    ob.MouseEnter:Connect(function()
        ob.BackgroundColor3=opt.color
        TweenService:Create(ob,TweenInfo.new(0.1),{BackgroundTransparency=0.8}):Play()
    end)
    ob.MouseLeave:Connect(function() TweenService:Create(ob,TweenInfo.new(0.1),{BackgroundTransparency=1}):Play() end)
    ob.MouseButton1Click:Connect(function()
        ...
        updateFbcModeBtn()
        closeFbcPopup()
        Notify.send({type="info",icon=opt.icon,accent=opt.color,title="Farm Baús — Modo",msg=opt.label..": "..opt.desc,duration=2.5})
    end)
end

fbcModeBtn.MouseButton1Click:Connect(function()
    fbcPopupOpen = not fbcPopupOpen
    fbcPopup.Visible = fbcPopupOpen
end)
UserInputService.InputBegan:Connect(function(inp)
    if fbcPopupOpen and inp.UserInputType==Enum.UserInputType.MouseButton1 then
        task.wait(); if fbcPopupOpen then closeFbcPopup() end
    end
end)
