...
for _, t in ipairs(TabConfig) do
    ...
    pp.PaddingRight = UDim.new(0,12); pp.PaddingBottom = UDim.new(0,12)
...
local function makeSlider(page, lo_fn, lbl_txt, minV, maxV, defV, cor, fmt, onChange)
    local row=Instance.new("Frame", page)
    row.BackgroundColor3=VD_ROW; row.BackgroundTransparency=0.65; row.BorderSizePixel=0
    row.Size=UDim2.new(1,0,0,52); row.LayoutOrder=lo_fn(); row.ZIndex=5
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
    -- Título
    local tl=Instance.new("TextLabel",row); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,14,0,8); tl.Size=UDim2.new(1,-90,0,17); tl.Font=Enum.Font.GothamBold
    tl.Text=lbl_txt; tl.TextColor3=VD_TEXT; tl.TextSize=12
    tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=7
    -- Valor (lado direito, acima do slider)
    local valLbl=Instance.new("TextLabel",row); valLbl.BackgroundTransparency=1
    valLbl.AnchorPoint=Vector2.new(1,0)
    valLbl.Position=UDim2.new(1,-14,0,8); valLbl.Size=UDim2.new(0,60,0,17)
    valLbl.Font=Enum.Font.GothamBold; valLbl.TextColor3=VD_MUTED; valLbl.TextSize=11
    valLbl.TextXAlignment=Enum.TextXAlignment.Right; valLbl.ZIndex=7
    -- Track
    local track=Instance.new("Frame",row); track.BackgroundColor3=Color3.fromRGB(55,38,80)
    track.BorderSizePixel=0; track.Position=UDim2.new(0,14,0,34); track.Size=UDim2.new(1,-28,0,4)
    ...
    thumb.Position=UDim2.new(0,0,0.5,0); thumb.Size=UDim2.new(0,14,0,14); thumb.ZIndex=8
    ...
    local function onInput(x)
        local rel = track.AbsolutePosition.X; local w = track.AbsoluteSize.X
    ...
    uis.InputChanged:Connect(function(inp)
        if dragging and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then onInput(inp.Position.X) end
    end)
...
kaSliderRow.BackgroundColor3=VD_ROW; kaSliderRow.BackgroundTransparency=0.65; kaSliderRow.BorderSizePixel=0
kaSliderRow.Size=UDim2.new(1,0,0,52); kaSliderRow.LayoutOrder=fNextLO(); kaSliderRow.ZIndex=5
Instance.new("UICorner",kaSliderRow).CornerRadius=UDim.new(0,8)
...
local kaMiniTrack=Instance.new("Frame",kaSliderRow); kaMiniTrack.BackgroundColor3=Color3.fromRGB(55,38,80)
kaMiniTrack.BorderSizePixel=0; kaMiniTrack.Position=UDim2.new(0,14,0,34); kaMiniTrack.Size=UDim2.new(1,-28,0,4)
...
kaMiniDot.Position=UDim2.new(pct0ka,0,0.5,0); kaMiniDot.Size=UDim2.new(0,14,0,14); kaMiniDot.ZIndex=9
...
local function autoFeedAlways()
    ...
    pcall(function()
        ...
        if afcRef.stroke then TweenService:Create(afcRef.stroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play() end
    end)
end
...
afcCardG.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(30,14,4)),ColorSequenceKeypoint.new(1,Color3.fromRGB(10,5,2))})
afcCardG.Rotation = 135
local afcCardS = Instance.new("UIStroke",afcCard)
afcCardS.Color=Color3.fromRGB(8,4,0); afcCardS.Thickness=3; afcCardS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
...
Instance.new("UIStroke",afcIB).Color=Color3.fromRGB(8,4,0)
...
Instance.new("UIStroke",afcTit).Color=Color3.fromRGB(8,4,0)
-- Desc
...
Instance.new("UIStroke",afcBtn).Color=Color3.fromRGB(8,4,0)
...
-- POPUP SELETOR
-- ══════════════════════════════════════════════════════════════
local afcPopup=Instance.new("Frame",MainFrame)
afcPopup.Name="AfcPopup"; afcPopup.BackgroundColor3=Color3.fromRGB(28,14,4)
afcPopup.BorderSizePixel=0; afcPopup.Size=UDim2.new(0,230,0,0)
afcPopup.Position=UDim2.new(0,0,0,0); afcPopup.Visible=false; afcPopup.ZIndex=200; afcPopup.ClipsDescendants=true
Instance.new("UICorner",afcPopup).CornerRadius=UDim.new(0,12)
local afcPopS=Instance.new("UIStroke",afcPopup); afcPopS.Color=AFC_COR; afcPopS.Thickness=2.5; afcPopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

-- Header popup
local afcPopHdr=Instance.new("Frame",afcPopup)
afcPopHdr.BackgroundColor3=AFC_COR; afcPopHdr.BorderSizePixel=0
afcPopHdr.Size=UDim2.new(1,0,0,32); afcPopHdr.ZIndex=201
Instance.new("UICorner",afcPopHdr).CornerRadius=UDim.new(0,10)
local afcPopHdrFix=Instance.new("Frame",afcPopHdr)
afcPopHdrFix.BackgroundColor3=AFC_COR; afcPopHdrFix.BorderSizePixel=0
afcPopHdrFix.Position=UDim2.new(0,0,0.5,0); afcPopHdrFix.Size=UDim2.new(1,0,0.5,0); afcPopHdrFix.ZIndex=201
...
afcPopTitle.TextColor3=Color3.fromRGB(30,12,2); afcPopTitle.TextSize=11; afcPopTitle.ZIndex=202
...
afcScroll.ScrollBarThickness=3; afcScroll.ScrollBarImageColor3=AFC_COR
afcScroll.CanvasSize=UDim2.new(0,0,0,0); afcScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; afcScroll.ZIndex=201
...
afcSP.PaddingLeft=UDim.new(0,6); afcSP.PaddingRight=UDim.new(0,6); afcSP.PaddingTop=UDim.new(0,4)
...
for _, cat in ipairs(afcCatOrder) do
    ...
    hdrF.BackgroundColor3=Color3.fromRGB(20,10,3); hdrF.BorderSizePixel=0
    hdrF.Size=UDim2.new(1,0,0,20); hdrF.LayoutOrder=loAfc*100; hdrF.ZIndex=202
    ...
    hdrL.TextColor3=ci.cor; hdrL.TextSize=10; hdrL.TextXAlignment=Enum.TextXAlignment.Left; hdrL.ZIndex=203
    -- Itens da categoria
    for i, item in ipairs(afcCatItems[cat]) do
        loAfc += 1
        -- Row
        local row=Instance.new("Frame",afcScroll)
        row.BackgroundColor3=Color3.fromRGB(22,11,4); row.BorderSizePixel=0
        row.Size=UDim2.new(1,0,0,28); row.LayoutOrder=loAfc*100+i; row.ZIndex=202
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,7)
        local rowS=Instance.new("UIStroke",row); rowS.Color=Color3.fromRGB(8,4,0); rowS.Thickness=1.5; rowS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
        -- Ícone
        local ico=Instance.new("TextLabel",row); ico.BackgroundTransparency=1
        ico.Position=UDim2.new(0,4,0,0); ico.Size=UDim2.new(0,24,1,0)
        ico.Text=item.icon; ico.TextSize=14; ico.Font=Enum.Font.GothamBold; ico.ZIndex=203
        -- Label
        local lbl=Instance.new("TextLabel",row); lbl.BackgroundTransparency=1
        lbl.Position=UDim2.new(0,28,0,0); lbl.Size=UDim2.new(1,-60,1,0)
        lbl.Font=Enum.Font.GothamBold; lbl.Text=item.label
        lbl.TextColor3=Color3.fromRGB(200,180,140); lbl.TextSize=10
        lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=203
        -- Fuel indicator
        ...
        fi.TextSize=8; fi.ZIndex=203
        -- Checkbox
        local cb=Instance.new("Frame",row); cb.BorderSizePixel=0
        cb.Position=UDim2.new(1,-22,0.5,-9); cb.Size=UDim2.new(0,18,0,18)
        cb.BackgroundColor3=Color3.fromRGB(40,20,6); cb.ZIndex=203
        Instance.new("UICorner",cb).CornerRadius=UDim.new(0,5)
        local cbS=Instance.new("UIStroke",cb); cbS.Color=Color3.fromRGB(80,50,15); cbS.Thickness=1.5; cbS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
        ...
        cbMark.Font=Enum.Font.GothamBlack; cbMark.ZIndex=204
        -- Botão invisível
        local btn=Instance.new("TextButton",row); btn.BackgroundTransparency=1; btn.BorderSizePixel=0
        btn.Size=UDim2.new(1,0,1,0); btn.Text=""; btn.ZIndex=205; btn.AutoButtonColor=false
        -- Hover
        btn.MouseEnter:Connect(function() TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(34,17,6)}):Play() end)
        btn.MouseLeave:Connect(function() TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,11,4)}):Play() end)
        -- Refresh visual do checkbox
        local function refreshCB()
            local on = afcSel[item.key]
            TweenService:Create(cb,TweenInfo.new(0.12),{BackgroundColor3=on and AFC_COR or Color3.fromRGB(40,20,6)}):Play()
            TweenService:Create(cbS,TweenInfo.new(0.12),{Color=on and Color3.fromRGB(8,4,0) or Color3.fromRGB(80,50,15)}):Play()
            cbMark.Text = on and "✓" or ""
            cbMark.TextColor3 = Color3.fromRGB(20,10,2)
            lbl.TextColor3 = on and Color3.fromRGB(255,240,200) or Color3.fromRGB(200,180,140)
            rowS.Color = on and AFC_COR or Color3.fromRGB(8,4,0)
            rowS.Transparency = on and 0.3 or 0
        end
        btn.MouseButton1Click:Connect(function()
            afcSel[item.key] = not afcSel[item.key]
            refreshCB()
            pcall(function() afcSelBtn.Text = afcSelLabel() end)
            pcall(function()
...
local function afcOpenPopup()
    if afcPopupOpen then afcClosePopup(); return end
    afcPopupOpen = true
    local ap = afcSelBtn.AbsolutePosition
    local ms = MainFrame.AbsolutePosition
    local rx = ap.X - ms.X
    local ry = ap.Y - ms.Y - AFC_POP_H - 6
    if ry < 0 then ry = ap.Y - ms.Y + afcSelBtn.AbsoluteSize.Y + 4 end
    ...
    TweenService:Create(afcSelBtnS,TweenInfo.new(0.15),{Transparency=0}):Play()
end

afcSelBtn.MouseButton1Click:Connect(afcOpenPopup)
...
afcBtn.MouseButton1Click:Connect(function()
    ...
    if afcRunning then
        afcRunning = false
        TweenService:Create(afcCardS,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
...
local function autoFeedHP()
    ...
    while afhpRunning do
        ...
        if pct == nil or pct < afhpThreshold then
            local lista = collectItemsByNames(AFHP_NAMES)
            if #lista > 0 then
                local loteE={}; local running={true}
                for s=1,math.min(LOTE,#lista) do
                    local e=lista[s]
    ...
    pcall(function()
        ...
        if afhpRef.stroke then TweenService:Create(afhpRef.stroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play() end
    end)
end
...
-- ── Card ─────────────────────────────────────────────────────
do
    local card,cardS,_,_,_,status,btn,btnG = makeFarmCard(100,AFHP_COR,"🔥","Auto Feed Campfire (HP Based)",string.format("Reabastece quando nível < %d%%. Log, Coal, Biofuel...",afhpThreshold))
    ...
    -- ── TextBox de threshold ──────────────────────────────────
    -- Container (row abaixo da desc)
    local tbRow=Instance.new("Frame",card); tbRow.BackgroundTransparency=1; tbRow.BorderSizePixel=0
    tbRow.Position=UDim2.new(0,54,0,54); tbRow.Size=UDim2.new(0,148,0,28); tbRow.ZIndex=8
    ...
    tbLbl.Text="Limite:"; tbLbl.TextColor3=Color3.fromRGB(200,180,140)
    ...
    tbBox.BackgroundColor3=Color3.fromRGB(20,10,30)
    tbBox.Position=UDim2.new(0,52,0.5,-13); tbBox.Size=UDim2.new(0,44,0,26); tbBox.ZIndex=9
    ...
    tb.Size=UDim2.new(1,-4,1,0); tb.Position=UDim2.new(0,2,0,0)
    tb.Font=Enum.Font.GothamBlack; tb.Text=tostring(afhpThreshold)
    tb.TextColor3=Color3.fromRGB(255,255,255); tb.TextSize=14
    tb.PlaceholderText="1-99"; tb.PlaceholderColor3=Color3.fromRGB(120,90,100)
    tb.ClearTextOnFocus=true; tb.ZIndex=10

    -- Label "%" depois do TextBox
    local tbPct=Instance.new("TextLabel",tbRow); tbPct.BackgroundTransparency=1
    tbPct.Position=UDim2.new(0,100,0,0); tbPct.Size=UDim2.new(0,20,1,0)
    tbPct.Font=Enum.Font.GothamBlack; tbPct.Text="%"
    tbPct.TextColor3=AFHP_COR; tbPct.TextSize=13; tbPct.ZIndex=9

    -- Função de aplicar valor
    local function applyThreshold(val)
        val = math.clamp(math.floor(tonumber(val) or afhpThreshold), 1, 99)
        afhpThreshold = val
        ...
        -- Pulso no stroke da caixa
        TweenService:Create(tbBoxS,TweenInfo.new(0.1),{Transparency=0,Color=Color3.fromRGB(255,200,80)}):Play()
        task.delay(0.3,function() TweenService:Create(tbBoxS,TweenInfo.new(0.2),{Transparency=0.4,Color=AFHP_COR}):Play() end)
    end

    -- Filtra: só dígitos, máx 2 chars
    tb:GetPropertyChangedSignal("Text"):Connect(function()
        local clean = tb.Text:gsub("%D","")  -- remove não-dígitos
        if #clean > 2 then clean = clean:sub(1,2) end
        if tb.Text ~= clean then tb.Text = clean end
    end)

    -- Foco: acende borda
    tb.Focused:Connect(function()
        TweenService:Create(tbBoxS,TweenInfo.new(0.15),{Transparency=0,Color=Color3.fromRGB(255,200,80)}):Play()
        TweenService:Create(tbBox,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(30,15,40)}):Play()
    end)

    -- Perde foco: aplica valor
    tb.FocusLost:Connect(function(enterPressed)
        TweenService:Create(tbBoxS,TweenInfo.new(0.15),{Transparency=0.4,Color=AFHP_COR}):Play()
        TweenService:Create(tbBox,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(20,10,30)}):Play()
        if tb.Text == "" then tb.Text = tostring(afhpThreshold); return end
        local v = tonumber(tb.Text)
        if v then
            applyThreshold(v)
            if enterPressed then
                Notify.send({type="info",icon="🔥",accent=AFHP_COR,title="Auto Feed HP",msg=string.format("Limite configurado: %d%%",afhpThreshold),duration=2})
            end
        else
...
local function autoCookFood()
    ...
    pcall(function()
        ...
        if ackRef.stroke then TweenService:Create(ackRef.stroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play() end
    end)
end
...
local function autoMachineGrind()
    ...
    pcall(function()
        ...
        if amgRef.stroke then TweenService:Create(amgRef.stroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play() end
    end)
end
...
amgCard.BackgroundColor3 = Color3.fromRGB(26,15,6); amgCard.BorderSizePixel = 0
...
amgCardS.Color = Color3.fromRGB(8,4,0); amgCardS.Thickness = 3; amgCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
local amgIBStr=Instance.new("UIStroke",amgIB); amgIBStr.Color=Color3.fromRGB(8,4,0); amgIBStr.Thickness=2.5; amgIBStr.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
...
Instance.new("UIStroke",amgTit).Color=Color3.fromRGB(8,4,0)
-- Desc
...
amgDesc.TextColor3=Color3.fromRGB(180,160,110); amgDesc.TextSize=9; amgDesc.TextXAlignment=Enum.TextXAlignment.Left; amgDesc.TextWrapped=true; amgDesc.ZIndex=7
-- Status
...
Instance.new("UIStroke",amgBtn).Color=Color3.fromRGB(8,4,0)
...
-- ══════════════════════════════════════════════════════════════
local amgPopup = Instance.new("Frame", MainFrame)
amgPopup.Name = "AmgPopup"
amgPopup.BackgroundColor3 = Color3.fromRGB(30,18,8)
amgPopup.BorderSizePixel = 0
amgPopup.Size = UDim2.new(0,230,0,0)   -- altura animada
amgPopup.Position = UDim2.new(0,0,0,0) -- posicionado ao abrir
amgPopup.Visible = false
amgPopup.ZIndex = 200
amgPopup.ClipsDescendants = true
Instance.new("UICorner",amgPopup).CornerRadius=UDim.new(0,12)
local amgPopS=Instance.new("UIStroke",amgPopup); amgPopS.Color=AMG_COR; amgPopS.Thickness=2.5; amgPopS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

-- Header do popup
local amgPopHdr=Instance.new("Frame",amgPopup)
amgPopHdr.BackgroundColor3=AMG_COR; amgPopHdr.BorderSizePixel=0
amgPopHdr.Size=UDim2.new(1,0,0,32); amgPopHdr.ZIndex=201
Instance.new("UICorner",amgPopHdr).CornerRadius=UDim.new(0,10)
local amgPopHdrFix=Instance.new("Frame",amgPopHdr)
amgPopHdrFix.BackgroundColor3=AMG_COR; amgPopHdrFix.BorderSizePixel=0
amgPopHdrFix.Position=UDim2.new(0,0,0.5,0); amgPopHdrFix.Size=UDim2.new(1,0,0.5,0); amgPopHdrFix.ZIndex=201
...
amgPopTitle.TextColor3=Color3.fromRGB(20,10,0); amgPopTitle.TextSize=11; amgPopTitle.ZIndex=202
...
amgScroll.ScrollBarThickness=3; amgScroll.ScrollBarImageColor3=AMG_COR
amgScroll.CanvasSize=UDim2.new(0,0,0,0); amgScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; amgScroll.ZIndex=201
...
amgSP.PaddingLeft=UDim.new(0,6); amgSP.PaddingRight=UDim.new(0,6); amgSP.PaddingTop=UDim.new(0,4)
...
local function amgMakeItemRow(item, lo)
    ...
    row.BackgroundColor3=Color3.fromRGB(24,14,5); row.BorderSizePixel=0
    row.Size=UDim2.new(1,0,0,28); row.LayoutOrder=lo; row.ZIndex=202
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,7)
    local rowS=Instance.new("UIStroke",row); rowS.Color=Color3.fromRGB(8,4,0); rowS.Thickness=1.5; rowS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    -- Ícone
    local ico=Instance.new("TextLabel",row); ico.BackgroundTransparency=1
    ...
    lbl.Position=UDim2.new(0,28,0,0); lbl.Size=UDim2.new(1,-60,1,0)
    lbl.Font=Enum.Font.GothamBold; lbl.Text=item.label
    lbl.TextColor3=Color3.fromRGB(220,220,220); lbl.TextSize=10
    lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=203
    -- Scrap indicator
    ...
    scr.TextSize=8; scr.ZIndex=203
    -- Checkbox
    local cb=Instance.new("Frame",row); cb.BorderSizePixel=0
    cb.Position=UDim2.new(1,-22,0.5,-9); cb.Size=UDim2.new(0,18,0,18)
    cb.BackgroundColor3=Color3.fromRGB(40,24,8); cb.ZIndex=203
    Instance.new("UICorner",cb).CornerRadius=UDim.new(0,5)
    local cbS=Instance.new("UIStroke",cb); cbS.Color=Color3.fromRGB(80,60,20); cbS.Thickness=1.5; cbS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local cbMark=Instance.new("TextLabel",cb); cbMark.BackgroundTransparency=1
    cbMark.Size=UDim2.new(1,0,1,0); cbMark.Text=""; cbMark.TextSize=12
    cbMark.Font=Enum.Font.GothamBlack; cbMark.ZIndex=204
    -- Botão invisível
    ...
    btn.Size=UDim2.new(1,0,1,0); btn.Text=""; btn.ZIndex=205; btn.AutoButtonColor=false
    -- Hover
    btn.MouseEnter:Connect(function() TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(36,22,8)}):Play() end)
    ...
    local function refreshCB()
        ...
        TweenService:Create(cb,TweenInfo.new(0.12),{BackgroundColor3=on and AMG_COR or Color3.fromRGB(40,24,8)}):Play()
        TweenService:Create(cbS,TweenInfo.new(0.12),{Color=on and Color3.fromRGB(8,4,0) or Color3.fromRGB(80,60,20)}):Play()
        cbMark.Text = on and "✓" or ""
        cbMark.TextColor3 = Color3.fromRGB(20,10,0)
        lbl.TextColor3 = on and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,180,180)
        rowS.Color = on and AMG_COR or Color3.fromRGB(8,4,0)
        rowS.Transparency = on and 0.3 or 0
    end
    btn.MouseButton1Click:Connect(function()
...
for _, cat in ipairs(catOrder) do
    ...
    hdrLbl.Size=UDim2.new(1,0,0,20); hdrLbl.LayoutOrder=loCount*100; hdrLbl.ZIndex=202
...
amgBtn.MouseButton1Click:Connect(function()
    ...
    if amgRunning then
        amgRunning=false
        TweenService:Create(amgCardS,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play()
...
local function autoBiofuel()
    ...
    pcall(function()
        ...
        if abfRef.stroke then TweenService:Create(abfRef.stroke,TweenInfo.new(0.2),{Color=Color3.fromRGB(8,4,0)}):Play() end
    end)
end
...
-- ── Popup Farm Baús Modo — Voidware style ─────────────────────
local fbcPopup=Instance.new("Frame",ScreenGui)
fbcPopup.BackgroundColor3=VD_SIDEBAR; fbcPopup.BorderSizePixel=0
fbcPopup.ZIndex=310; fbcPopup.Visible=false; fbcPopup.Size=UDim2.new(0,180,0,0)
Instance.new("UICorner",fbcPopup).CornerRadius=UDim.new(0,12)
local fbcPopupS=Instance.new("UIStroke",fbcPopup)
fbcPopupS.Color=VD_STROKE; fbcPopupS.Thickness=1.5; fbcPopupS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
-- Lista
local fbcPopList=Instance.new("Frame",fbcPopup); fbcPopList.BackgroundTransparency=1
fbcPopList.BorderSizePixel=0; fbcPopList.Position=UDim2.new(0,8,0,8)
fbcPopList.Size=UDim2.new(1,-16,0,0); fbcPopList.ZIndex=311; fbcPopList.AutomaticSize=Enum.AutomaticSize.Y
local fbcPopLayout=Instance.new("UIListLayout",fbcPopList); fbcPopLayout.Padding=UDim.new(0,4); fbcPopLayout.SortOrder=Enum.SortOrder.LayoutOrder

local fbcPopupOpen=false
local function closeFbcPopup()
    fbcPopupOpen=false
    TweenService:Create(fbcPopup,TweenInfo.new(0.13,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,180,0,0),BackgroundTransparency=1}):Play()
    task.delay(0.15,function() fbcPopup.Visible=false; fbcPopup.BackgroundTransparency=0 end)
end

for idx, opt in ipairs(FBC_MODES) do
    local ob=Instance.new("TextButton",fbcPopList); ob.BackgroundColor3=VD_ROW
    ob.BackgroundTransparency=0.5; ob.BorderSizePixel=0
    ob.Size=UDim2.new(1,0,0,38); ob.LayoutOrder=idx; ob.ZIndex=312; ob.AutoButtonColor=false
    Instance.new("UICorner",ob).CornerRadius=UDim.new(0,8)
    local obS=Instance.new("UIStroke",ob); obS.Color=VD_STROKE; obS.Thickness=1; obS.Transparency=0.7
    local obIco=Instance.new("TextLabel",ob); obIco.BackgroundTransparency=1
    obIco.Position=UDim2.new(0,8,0,0); obIco.Size=UDim2.new(0,24,1,0)
    obIco.Font=Enum.Font.GothamBold; obIco.Text=opt.icon; obIco.TextSize=16; obIco.ZIndex=313
    local obLbl=Instance.new("TextLabel",ob); obLbl.BackgroundTransparency=1
    obLbl.Position=UDim2.new(0,34,0,0); obLbl.Size=UDim2.new(1,-38,1,0)
    obLbl.Font=Enum.Font.GothamBold; obLbl.Text=opt.label
    obLbl.TextColor3=VD_TEXT; obLbl.TextSize=11
    obLbl.TextXAlignment=Enum.TextXAlignment.Left; obLbl.ZIndex=313
    local function fbcRefreshSel()
        local sel=(fbcMode==opt.key)
        TweenService:Create(obS,TweenInfo.new(0.15),{Transparency=sel and 0.3 or 0.7, Color=sel and opt.color or VD_STROKE}):Play()
        TweenService:Create(ob,TweenInfo.new(0.15),{BackgroundTransparency=sel and 0.3 or 0.5}):Play()
        obLbl.TextColor3 = sel and opt.color or VD_TEXT
    end
    fbcRefreshSel()
    ob.MouseEnter:Connect(function()
        TweenService:Create(ob,TweenInfo.new(0.1),{BackgroundTransparency=0.35}):Play()
    end)
...
fbcModeBtn.MouseButton1Click:Connect(function()
    ...
    local popH=8+#FBC_MODES*42+12
    fbcPopup.Position=UDim2.new(0,ap.X+as.X-180,0,ap.Y-popH-6)
    fbcPopup.Size=UDim2.new(0,180,0,0); fbcPopup.BackgroundTransparency=1; fbcPopup.Visible=true
    TweenService:Create(fbcPopup,TweenInfo.new(0.18,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Size=UDim2.new(0,180,0,popH), BackgroundTransparency=0
    }):Play()
    fbcPopupOpen=true
end)
MainFrame.InputBegan:Connect(function(inp)
