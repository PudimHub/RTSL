...
-- TELEPORTAR TAB
-- ══════════════════════════════════════════════════════════════
local TP_COR_CHEST = Color3.fromRGB(255, 200, 60) -- movido para fora (usado em P1 e P2)
local _dbgOk_11847, _dbgErr_11847 = pcall(function() -- [[ TELEPORTAR PART 1 ]]
...
-- ══════════════════════════════════════════════════════════════
-- (TP_COR_CHEST já declarada antes do bloco TELEPORTAR)
...
end); if not _dbgOk_11847 then warn('[PudimHub DEBUG] Erro na secao TELEPORTAR P1: '..tostring(_dbgErr_11847)) end
local _dbgOk_11847b, _dbgErr_11847b = pcall(function() -- [[ TELEPORTAR PART 2 ]]
...
end); if not _dbgOk_11847b then warn('[PudimHub DEBUG] Erro na secao TELEPORTAR P2: '..tostring(_dbgErr_11847b)) end
...
-- ── Pega a MELHOR ferramenta disponível (maior dano, sem precisar equipar) ─
-- ── Lista de prioridade por dano (do MAIOR pro MENOR) — SÓ CORPO A CORPO ──
-- Baseado em pesquisa de tier lists do jogo (Sportskeeda, Bloxspot, GameRant,
-- wiki oficial). Armas de longa distância (Rifle, Crossbow, Revolver, Raygun,
-- Blowpipe, arremesso como Kunai/Shuriken/Snowball) foram removidas de propósito.
-- ⚠️ Cultist King Mace tem obtenção incerta — a wiki oficial diz que só existe
-- como item de Admin, mas alguns guias dizem que dropa do boss Cultist King.
-- Deixei ela no topo caso você consiga o item; ajuste a ordem se notar dano
-- diferente do esperado na prática.
local DAMAGE_PRIORITY = {
    "Cultist King Mace", "CultistKingMace",
    "Obsidiron Hammer", "ObsidironHammer",
    "Infernal Sword", "InfernalSword",
    "Morningstar",
    "Poison Spear", "PoisonSpear",
    "Strong Axe", "StrongAxe",
    "Ice Axe", "IceAxe",
    "Ice Sword", "IceSword",
    "Chainsaw",
    "Katana",
...
-- ── Pega a MELHOR ferramenta disponível (maior dano primeiro),
--    sem precisar estar equipada — Inventory > Character > Backpack ──
local function getBestTool()
    local ch  = Player.Character
    local inv = Player:FindFirstChild("Inventory")
    local bp  = Player.Backpack

    for _, name in ipairs(DAMAGE_PRIORITY) do
        if inv then
            local t = inv:FindFirstChild(name)
            if t then return t, (ch and t.Parent == ch) end
        end
        if ch then
    ...
    -- Nenhum item conhecido na lista? Pega qualquer Tool disponível (fallback)
    if ch then
...
-- Mantido por compatibilidade (usa a mesma lógica de prioridade)
local function getWeapon() return getBestTool() end

-- ── Pega o MACHADO atual (item único — o jogo substitui o antigo pelo
--    novo quando você faz upgrade, então só precisa achar "o" machado
--    que existe agora, seja qual for o nome/tier dele) ──────────────
local function isAxeTool(t)
    if not t or not t:IsA("Tool") then return false end
    local nm = t.Name:lower()
    return nm:find("axe") ~= nil or nm:find("chainsaw") ~= nil
end
...
local function getAxeTool()
    ...
    if ch then
        local t = ch:FindFirstChildWhichIsA("Tool")
        if isAxeTool(t) then return t, true end
    end
    if inv then
        for _, c in ipairs(inv:GetChildren()) do
            if isAxeTool(c) then return c, false end
        end
    end
    if bp then
...
local function attackMob(mob)
    ...
    local tool, isEquipped = getBestTool()
    if not tool then
        if not KA_askedNoWeapon then
            KA_askedNoWeapon = true
            pcall(function()
                Notify.warn("⚔️ Kill Aura", "Nenhuma arma encontrada no inventário.", 4)
            end)
        end
        return
    end
    KA_askedNoWeapon = false

    installKaSessionHook()

    -- Sem ID de sessão capturado ainda? Equipa por um instante SÓ pra
    -- disparar o Activate() nativo do jogo (o hook aprende o ID sozinho).
    -- Depois disso, os ataques seguem sem precisar equipar de verdade.
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.info("⚔️ Kill Aura", "Aprendendo ID de ataque automaticamente...", 3)
            end)
        end
        local origParent = tool.Parent
        if not isEquipped then pcall(function() tool.Parent = ch end) end
        pcall(function() if tool.Activate then tool:Activate() end end)
        task.wait(0.15)
        pcall(function() if tool.Deactivate then tool:Deactivate() end end)
        if not isEquipped then
            task.wait(0.05)
            pcall(function() tool.Parent = origParent end)
        end
        if not KA_sessionId then
            return
        else
            pcall(function() Notify.success("✅ Kill Aura", "ID aprendido! Ativo sem precisar equipar.", 3) end)
        end
    end
...
-- Usa getAxeTool() — detecta o machado atual por nome (item único: o jogo
-- substitui o antigo pelo novo quando você faz upgrade)
local function chopTree(treeModel, treePart)
    local ch = Player.Character; if not ch then return end
    local myHRP = ch:FindFirstChild("HumanoidRootPart"); if not myHRP then return end

    local axe, isEquipped = getAxeTool()
    ...
    if not KA_sessionId then
        if not KA_askedManualHit then
            KA_askedManualHit = true
            pcall(function()
                Notify.info("🪓 Tree Aura", "Aprendendo ID de ataque automaticamente...", 3)
            end)
        end
        local origParent = axe.Parent
        if not isEquipped then pcall(function() axe.Parent = ch end) end
        pcall(function() if axe.Activate then axe:Activate() end end)
        task.wait(0.15)
        pcall(function() if axe.Deactivate then axe:Deactivate() end end)
        if not isEquipped then
            task.wait(0.05)
            pcall(function() axe.Parent = origParent end)
        end
        if not KA_sessionId then
            return
        else
            pcall(function() Notify.success("✅ Tree Aura", "ID aprendido! Ativo sem precisar equipar.", 3) end)
        end
    end
...
_accToggle(_tacCF, _y, "🪓 Tree Aura (sem equipar)", false, TA_COR, function(s)
    taEnabled=s
    if s then
        -- Verifica se tem algum machado disponível
