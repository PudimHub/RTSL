...
local function executarBring(key)
    ...
    for _,obj in ipairs(descs) do
        ...
        pcall(function()
            ...
            -- Salva posição VERDADEIRA original (antes do 1º bring)
            ...
            -- Espalha em círculo único ao redor do jogador
            local angle = (count * 47 + math.random(0,15)) * (math.pi / 180) -- ângulo único por item
            local radius = 1.8 + math.random() * 2.5 -- raio variado 1.8-4.3 studs
            local offsetX = math.cos(angle) * radius
            local offsetZ = math.sin(angle) * radius
            local target = cf.Position + Vector3.new(offsetX, 0.3 + math.random()*0.8, offsetZ)
            ...
            if obj:IsA("Model") then
                for _,bp in ipairs(obj:GetDescendants()) do
                    if bp:IsA("BasePart") and bp ~= targetPart then
                        pcall(function() bp.CFrame = CFrame.new(target + Vector3.new(math.random(-1,1)*0.15, 0, math.random(-1,1)*0.15)) end)
                    end
                end
            end
            count+=1
    ...
    if #trazidos>0 then
        task.spawn(function()
            for _=1,6 do
                task.wait(2) -- reduzido para 2 segundos
            ...
            if ok then
                ...
                for i,obj in ipairs(descs) do
                    ...
                    pcall(function()
                        ...
                        -- Salva posição verdadeira original
                        saveTrueOrigin(obj, originalCF)
                        -- Espalha em círculo único
                        local angle = (count * 47 + math.random(0,15)) * (math.pi / 180)
                        local radius = 1.8 + math.random() * 2.5
                        local target = cf.Position + Vector3.new(math.cos(angle)*radius, 0.3 + math.random()*0.8, math.sin(angle)*radius)
...
local function onWeaponActivated(tool)
    ...
    for _, mhrp in ipairs(mobs) do
        pcall(function()
            ...
            local attackCF = CFrame.new(mobPos - dir * 2, mobPos)
            hrp.CFrame = attackCF

            -- Espera 3 frames para o servidor receber a posição nova
            for _=1,3 do
                local stepped = false
                ...
                local t = tick()
                while not stepped and (tick()-t) < 0.05 do task.wait() end
            end

            -- Dispara o Activated da tool — usa o handler REAL do jogo
            pcall(function() tool:Activate() end)

            -- Espera mais 2 frames para o hitbox registrar no servidor
            for _=1,2 do
                ...
                local t2 = tick()
                while not stepped2 and (tick()-t2) < 0.05 do task.wait() end
            end
        end)
    end
