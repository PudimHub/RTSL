...
local function setGodMod(state)
    ...
    if state then
        ...
        local function applyGodModToChar(ch)
            ...
            -- Método 1: Hook direto em TakeDamage
            -- (substitui a função por vazia — mob chama mas nada acontece)
            pcall(function()
            ...
            -- Método 2: MaxHealth alto (mas NÃO math.huge — causa bug de câmera)
            -- 999999 é alto o suficiente para nunca morrer mas não quebra o Roblox
            pcall(function()
                hum.MaxHealth = 999999
                hum.Health    = 999999
            end)

            -- HealthChanged: restaura instantaneamente se cair
            local hcConn
            ...
            -- Método 3: Heartbeat de segurança (backup se hooks não funcionarem)
        ...
        Notify.send({type="custom", icon="♾️", accent=Color3.fromRGB(140,255,140),
            title="God Mod ATIVO",
            msg="HP máximo + TakeDamage bloqueado!",
            duration=4})
    else
        if godModConn then godModConn:Disconnect(); godModConn = nil end
