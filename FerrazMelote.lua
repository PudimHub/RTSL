...
local function setGodMod(state)
    ...
    if state then
        ...
        -- Backup: restaura HP a cada frame caso os metamethods não sejam suportados
        godModConn = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                if h2.Health < h2.MaxHealth * 0.5 then
...
local function autoTriturador()
    ...
    local total   = #encontrados
    local ALTURA  = 8      -- studs acima do triturador
    local RAIO    = 1.2    -- raio pequeno: cai direto dentro do triturador
    ...
    for i, entry in ipairs(encontrados) do
        pcall(function()
            ...
            -- Posição em espiral apertada acima do triturador
            local angle  = (i / math.max(total, 1)) * math.pi * 2
            ...
            -- Remove qualquer força/constraint que prende o item no ar
            ...
            -- Solta completamente livre
            pcall(function() part.Anchored = false end)
            part.CanCollide = true
            part.CFrame     = CFrame.new(skyPos)

            -- AssemblyLinearVelocity é a API moderna do Roblox (Velocity está depreciado)
            pcall(function()
                ...
                for _, bp in ipairs(obj:GetDescendants()) do
                    if bp:IsA("BasePart") and bp ~= part then
                        pcall(function()
                            ...
                            bp.CanCollide = true
                            bp.CFrame     = CFrame.new(bp.Position + offset)
                            pcall(function() bp.AssemblyLinearVelocity = Vector3.new(0,-2,0) end)
                        end)
                    end
                end
            end
        ...
        -- Stagger leve para não congelar
...
local function aeComputeMaxRadius()
    -- FONTE 1: Lighting.FogEnd (fonte primária — atualizada pelo servidor)
    local fogEnd = 0
    pcall(function()
        fogEnd = game:GetService("Lighting").FogEnd or 0
    end)

    -- FogEnd acima de 3000 = névoa desligada (nível 6 ou máximo)
    -- FogEnd muito baixo = início do jogo ou nevoa padrão (ignorar)
    if fogEnd > 50 and fogEnd <= 3000 then
        local radius = math.floor(fogEnd * AE_FOG_FACTOR)
        -- Garante um mínimo sensato
        radius = math.max(radius, AE_LEVEL_RADIUS[1])
        return radius, fogEnd, "FogEnd"
    end

    -- FogEnd desligado (>3000): usa nível máximo
    if fogEnd > 3000 then
        return AE_LEVEL_RADIUS[6], fogEnd, "FogEnd·Max"
    end

    -- FONTE 2: Atributo/Value do Model da fogueira
    local lvl = aeReadCampfireLevel()
    if lvl >= 1 then
        local radius = AE_LEVEL_RADIUS[math.clamp(lvl, 1, 6)]
        return radius, fogEnd, "CampLvl"..lvl
    end
...
local function startAutoExplore()
    ...
    -- Detecta raio real antes de começar
    local maxRadius, campLvl = aeRefreshLevel()

    local campPos  = aeGetCampfire()
    aeHeight       = campPos.Y + AE_HEIGHT_OFF
    aeCurAngle     = 0
    aeCurRadius    = AE_MIN_RADIUS
    aeStartTime    = tick()
    local lastNotifyRadius = 0
    local lastFogCheck     = tick()

    -- Física própria
    aeCleanPhysics()
    aeBodyVel = Instance.new("BodyVelocity", hrp)
    aeBodyVel.MaxForce = Vector3.new(1e6,1e6,1e6); aeBodyVel.Velocity = Vector3.zero; aeBodyVel.Name = "AE_BodyVel"
    aeBodyGyro = Instance.new("BodyGyro", hrp)
    aeBodyGyro.MaxTorque = Vector3.new(1e6,1e6,1e6); aeBodyGyro.D = 50; aeBodyGyro.CFrame = hrp.CFrame; aeBodyGyro.Name = "AE_BodyGyro"
    ...
    pcall(function()
        if aeLabelState then
            ...
            TweenService:Create(aeLabelState, TweenInfo.new(0.2), {BackgroundColor3=Color3.fromRGB(20,40,60)}):Play()
        end
    end)

    Notify.send({type="custom", icon="🗺️", accent=AE_COR,
        title="Auto Explorar INICIADO",
        msg=string.format("🔥 Fogueira Nível %d  •  Raio alvo: ~%d studs (FogEnd dinâmico)", campLvl, maxRadius),
        duration=5})

    -- Monitor em tempo real do FogEnd (se a fogueira subir de nível durante a exploração)
        ...
        pcall(function()
            ...
            if newRadius > aeDynMaxRadius + 10 then
                ...
                Notify.send({type="custom", icon="🔥", accent=Color3.fromRGB(255,200,80),
                    ...
                    msg=string.format("Raio expandido: %d → %d studs — Continuando exploração!", oldRadius, newRadius),
                    duration=5})
            end
        end)
    end)
    ...
    aeConn = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            -- Re-checa FogEnd a cada 5 segundos como segurança adicional
            if tick() - lastFogCheck > 5 then
                lastFogCheck = tick()
                aeRefreshLevel()
            end

            local curMax = aeDynMaxRadius   -- raio alvo atualizado em tempo real

            -- Posição alvo na espiral
            local tx = campPos.X + math.cos(aeCurAngle) * aeCurRadius
            local tz = campPos.Z + math.sin(aeCurAngle) * aeCurRadius
            local targetPos = Vector3.new(tx, aeHeight, tz)
            ...
            if dist > 1.5 then
                ...
                -- Avança ângulo (velocidade angular proporcional ao raio)
                local angStep = math.clamp(AE_SPEED / (math.max(aeCurRadius,1) * 2 * math.pi) * (math.pi*2), 0.04, 0.35)
                aeCurAngle  = aeCurAngle + angStep
                aeCurRadius = aeCurRadius + AE_STEP

                -- Notificação de progresso a cada ~25% do raio restante
                local notifyStep = math.max(60, (curMax - AE_MIN_RADIUS) * 0.25)
                if (aeCurRadius - lastNotifyRadius) >= notifyStep then
                    lastNotifyRadius = aeCurRadius
                    local elapsed = math.floor(tick() - aeStartTime)
                    local pct = math.clamp((aeCurRadius - AE_MIN_RADIUS)/(curMax - AE_MIN_RADIUS), 0, 1)
                    Notify.send({type="custom", icon="🗺️", accent=AE_COR,
                        title="Auto Explorar",
                        msg=string.format("%.0f%%  •  Raio atual: %.0f / %d studs  •  %s",
                            pct*100, aeCurRadius, curMax, aeFormatTime(elapsed)),
                        duration=3.5})
                end
            end

            -- Orientação do personagem
            local vel = aeBodyVel.Velocity
            if vel.Magnitude > 1 then
                aeBodyGyro.CFrame = CFrame.new(hrp2.Position, hrp2.Position + Vector3.new(vel.X,0,vel.Z))
            end
            ...
            pcall(function()
                if aeTimerLabel then
                    ...
                    aeTimerLabel.Text = string.format("%d:%02d  %.0f%%  •  %.0f / %d studs",
                        m, s, pct*100, aeCurRadius, aeDynMaxRadius)
                end
            end)

            -- CONCLUÍDO
            if aeCurRadius >= aeDynMaxRadius then
                ...
                Notify.send({type="custom", icon="✅", accent=Color3.fromRGB(87,242,135),
                    ...
                    msg=string.format("Mapa do Nível %d explorado em %s! (%d studs cobertos) Teleportando...",
                        aeCampLevel, aeFormatTime(totalTime), math.floor(aeDynMaxRadius)),
                    duration=8})
            end
        end)
    end)
