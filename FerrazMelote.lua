...
task.spawn(function()
    ...
    exitFlash.Size = UDim2.new(1,0,1,0)
    ...
    TS:Create(panel, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
        {Position=UDim2.new(0.5,0,-0.2,0), BackgroundTransparency=1}):Play()
...
-- ── iOS Apple Dark Mode palette ──────────────────────────────
local VD_BG      = Color3.fromRGB(28,  28,  30)   -- iOS systemBackground
local VD_SIDEBAR = Color3.fromRGB(22,  22,  24)   -- iOS groupedBackground
local VD_TOPBAR  = Color3.fromRGB(22,  22,  24)   -- iOS navigationBar
local VD_ROW     = Color3.fromRGB(44,  44,  46)   -- iOS secondarySystemBackground
local VD_ROW_HOV = Color3.fromRGB(54,  54,  56)   -- iOS tertiarySystemFill
local VD_TAB_ACT = Color3.fromRGB(10, 132, 255)   -- iOS systemBlue
local VD_TEXT    = Color3.fromRGB(255, 255, 255)  -- iOS label
local VD_MUTED   = Color3.fromRGB(152, 152, 157)  -- iOS secondaryLabel
local VD_SECTION = Color3.fromRGB(152, 152, 157)  -- iOS sectionHeader
local VD_STROKE  = Color3.fromRGB(56,  56,  58)   -- iOS separator
local _vdOpen = nil
local VD_DIVIDER = Color3.fromRGB(56,  56,  58)   -- iOS opaqueSeparator
-- iOS system accent colors
local IOS_BLUE   = Color3.fromRGB(10,  132, 255)
local IOS_GREEN  = Color3.fromRGB(48,  209,  88)
local IOS_RED    = Color3.fromRGB(255,  69,  58)
local IOS_ORANGE = Color3.fromRGB(255, 159,  10)
local IOS_PURPLE = Color3.fromRGB(191,  90, 242)
local IOS_FILL2  = Color3.fromRGB(58,  58,  60)
local IOS_FILL3  = Color3.fromRGB(44,  44,  46)
...
MainStroke.Color = Color3.fromRGB(72, 72, 74); MainStroke.Thickness = 0.5
MainStroke.Transparency = 0.1; MainStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
MainShadow.Size              = UDim2.new(0,546,0,396)
MainShadow.ZIndex            = 1
...
topFix.ZIndex = 3
-- Linha separadora iOS — 0.5px, muito sutil
...
TopDivider.Size = UDim2.new(1,0,0,0)
TopDivider.ZIndex = 4

-- Ícone + título — iOS navigation bar style
...
TitleSub.Position       = UDim2.new(0, 28, 0, 32)
...
-- Botões de controle — estilo macOS/iOS (traffic lights)
local TopBtns = {}
local _topBtnCfg = {
    {name="Theme",    bgN=Color3.fromRGB(191,90,242),  bgH=Color3.fromRGB(210,110,255), sym="🎨", sz=13},
    {name="Minimize", bgN=Color3.fromRGB(255,196,0),   bgH=Color3.fromRGB(255,214,60),  sym="▬",  sz=10},
    {name="Maximize", bgN=Color3.fromRGB(48,209,88),   bgH=Color3.fromRGB(70,235,110),  sym="⛶",  sz=11},
    {name="Close",    bgN=Color3.fromRGB(255,69,58),   bgH=Color3.fromRGB(255,95,85),   sym="✕",  sz=12},
}
local _topBtnSounds = {
    Theme    = 6012002983,
    Minimize = 6031221736,
...
for i = #_topBtnCfg, 1, -1 do
    ...
    sym.TextTransparency = 0.35; sym.ZIndex = 6
...
SearchFrame.Size = UDim2.new(0, 175, 0, 0)
SearchFrame.Visible = false
SearchFrame.ZIndex = 3
-- Fix cantos superiores
...
SFLine.BackgroundColor3 = VD_DIVIDER; SFLine.BackgroundTransparency = 0.3
SFLine.BorderSizePixel = 0
SFLine.Position = UDim2.new(0,0,1,-1); SFLine.Size = UDim2.new(1,0,0,1); SFLine.ZIndex = 4

-- iOS pill search — cinza escuro arredondado
local SearchBg = Instance.new("Frame", SearchFrame)
SearchBg.BackgroundColor3 = IOS_FILL2
SearchBg.BorderSizePixel = 0
SearchBg.Position = UDim2.new(0,8,0.5,-13)
...
SearchBgS.Color = VD_STROKE; SearchBgS.Thickness = 0.5; SearchBgS.Transparency = 0.5

local SearchIcon = Instance.new("TextLabel", SearchBg)
SearchIcon.BackgroundTransparency = 1
SearchIcon.Position = UDim2.new(0,6,0,0); SearchIcon.Size = UDim2.new(0,18,1,0)
SearchIcon.Font = Enum.Font.GothamBold; SearchIcon.Text = "🔍"
SearchIcon.TextSize = 11; SearchIcon.ZIndex = 5; SearchIcon.TextColor3 = VD_MUTED

local SearchBox = Instance.new("TextBox", SearchBg)
SearchBox.BackgroundTransparency = 1
SearchBox.Position = UDim2.new(0,24,0,0); SearchBox.Size = UDim2.new(1,-30,1,0)
SearchBox.Font = Enum.Font.Gotham
SearchBox.PlaceholderText = "Buscar..."
SearchBox.PlaceholderColor3 = VD_MUTED
SearchBox.Text = ""; SearchBox.TextColor3 = VD_TEXT
SearchBox.TextSize = 11; SearchBox.ZIndex = 5
...
SideBar.Name = "SideBar"; SideBar.BackgroundColor3 = Color3.fromRGB(18,18,20)
SideBar.Position = UDim2.new(0,0,0,56); SideBar.Size = UDim2.new(0,64,1,-122)
...
SideList.Padding = UDim.new(0,1); SideList.SortOrder = Enum.SortOrder.LayoutOrder
SideList.HorizontalAlignment = Enum.HorizontalAlignment.Center
local SidePad = Instance.new("UIPadding", SideBar)
SidePad.PaddingTop = UDim.new(0,6); SidePad.PaddingLeft = UDim.new(0,4)
SidePad.PaddingRight = UDim.new(0,4); SidePad.PaddingBottom = UDim.new(0,6)

-- Linha separadora vertical sutil entre sidebar e content
local Divider = Instance.new("Frame", MainFrame)
Divider.BackgroundColor3 = Color3.fromRGB(56,56,58); Divider.BackgroundTransparency = 0
Divider.BorderSizePixel = 0
Divider.Position = UDim2.new(0,64,0,56); Divider.Size = UDim2.new(0,1,1,-56); Divider.ZIndex = 3

-- ══════════════════════════════════════════════════════
--  CONTENT AREA — Voidware
-- ══════════════════════════════════════════════════════
local ContentArea = Instance.new("Frame", MainFrame)
ContentArea.Name = "ContentArea"; ContentArea.BackgroundColor3 = Color3.fromRGB(28,28,30)
ContentArea.Position = UDim2.new(0,65,0,56); ContentArea.Size = UDim2.new(1,-65,1,-56)
...
Footer.Position = UDim2.new(0,0,1,-38); Footer.Size = UDim2.new(0,64,0,0); Footer.Visible = false; Footer.ZIndex = 4
...
local C_ACCENT   = Color3.fromRGB(10, 132, 255)   -- iOS systemBlue
...
local function selectTab(key)
    ...
    for _,e in ipairs(allTabs) do
        ...
        -- iOS pill: só aparece quando ativo
        if e.actBg then
            TweenService:Create(e.actBg,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{
                BackgroundTransparency=isThis and 0.82 or 1
            }):Play()
        end
        if e.hovBg then
            TweenService:Create(e.hovBg,TweenInfo.new(0.15),{BackgroundTransparency=1}):Play()
        end
        -- Emoji icon: azul iOS quando ativo, cinza quando inativo + bounce size
        if e.icon then
            TweenService:Create(e.icon,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{
                TextColor3=isThis and Color3.fromRGB(10,132,255) or Color3.fromRGB(152,152,157),
                TextSize=isThis and 20 or 18
            }):Play()
        end
        -- Dot indicator azul embaixo do ícone
        if e.dot then
            TweenService:Create(e.dot,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{
                BackgroundTransparency=isThis and 0 or 1
            }):Play()
        end
        setIconColor(e.iconParts, isThis and Color3.fromRGB(10,132,255) or Color3.fromRGB(152,152,157))
        if e.label then
            e.label.TextColor3 = isThis and Color3.fromRGB(10,132,255) or Color3.fromRGB(152,152,157)
        end
...
local layoutOrder=0
-- ── makeGroupLabel — iOS icon-rail: thin separator line ──────
local function makeGroupLabel(text, trKey, groupTabs)
    layoutOrder = layoutOrder + 1
    -- Small spacer + 1px separator line between groups
    local spacer=Instance.new("Frame",SideBar)
    spacer.BackgroundTransparency=1; spacer.BorderSizePixel=0
    spacer.Size=UDim2.new(1,0,0,8); spacer.LayoutOrder=layoutOrder*100
    layoutOrder = layoutOrder + 1
    local sep=Instance.new("Frame",SideBar)
    sep.BackgroundColor3=Color3.fromRGB(56,56,58); sep.BackgroundTransparency=0.3
    sep.BorderSizePixel=0; sep.Size=UDim2.new(0,28,0,1)
    sep.LayoutOrder=layoutOrder*100; sep.ZIndex=4
end

-- ── makeTab — iOS icon-rail (64px wide, icon centered + dot) ──
local _IOS_TAB_ICON_MAP = {
    Info="ℹ️", Status="📊", Farm="🌾", Teleportar="📍",
    Esp="👁", Bring="📦", AvancadoFarm="⚡", Player="👤",
    Configuracoes="⚙️", AvancadoFuncoes="🔧", Atualizacao="🔄",
}
local function makeTab(cfg,groupTabs)
    ...
    bg.Size=UDim2.new(1,0,0,44); bg.LayoutOrder=order; bg.ZIndex=4

    -- iOS: pill de fundo ativo (rounded rect azul)
    local actBg=Instance.new("Frame",bg)
    actBg.BackgroundColor3=Color3.fromRGB(10,132,255); actBg.BackgroundTransparency=1
    actBg.BorderSizePixel=0
    actBg.AnchorPoint=Vector2.new(0.5,0.5)
    actBg.Position=UDim2.new(0.5,0,0.5,0)
    actBg.Size=UDim2.new(0,40,0,36); actBg.ZIndex=4
    Instance.new("UICorner",actBg).CornerRadius=UDim.new(0,10)

    -- Hover ghost
    local hovBg=Instance.new("Frame",bg)
    hovBg.BackgroundColor3=Color3.fromRGB(255,255,255); hovBg.BackgroundTransparency=1
    hovBg.BorderSizePixel=0
    hovBg.AnchorPoint=Vector2.new(0.5,0.5)
    hovBg.Position=UDim2.new(0.5,0,0.5,0)
    hovBg.Size=UDim2.new(0,40,0,36); hovBg.ZIndex=4
    Instance.new("UICorner",hovBg).CornerRadius=UDim.new(0,10)

    -- Emoji icon — centered, no pixel art for clean iOS look
    local icon=Instance.new("TextLabel",bg); icon.BackgroundTransparency=1
    icon.AnchorPoint=Vector2.new(0.5,0.5)
    icon.Position=UDim2.new(0.5,0,0.5,-2); icon.Size=UDim2.new(0,28,0,28)
    icon.Font=Enum.Font.GothamBold
    icon.Text=_IOS_TAB_ICON_MAP[cfg.key] or "●"
    icon.TextSize=18; icon.TextColor3=Color3.fromRGB(152,152,157)
    icon.ZIndex=6
    local iconParts={}

    -- Active dot indicator below icon
    local dot=Instance.new("Frame",bg)
    dot.BackgroundColor3=Color3.fromRGB(10,132,255); dot.BackgroundTransparency=1
    dot.BorderSizePixel=0
    dot.AnchorPoint=Vector2.new(0.5,1)
    dot.Position=UDim2.new(0.5,0,1,-3); dot.Size=UDim2.new(0,4,0,4); dot.ZIndex=7
    Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)

    -- Invisible label kept for trackLabel compat
    local label=Instance.new("TextLabel",bg); label.BackgroundTransparency=1
    label.Size=UDim2.new(0,0,0,0); label.Text=cfg.label
    label.TextColor3=Color3.fromRGB(152,152,157); label.ZIndex=4
    ...
    btn.MouseEnter:Connect(function()
        if currentTab~=cfg.key then
            TweenService:Create(hovBg,TweenInfo.new(0.12),{BackgroundTransparency=0.9}):Play()
        end
    end)
    btn.MouseLeave:Connect(function()
    ...
    btn.MouseButton1Down:Connect(function()
        -- Haptic scale: icon shrinks slightly
        TweenService:Create(icon,TweenInfo.new(0.08),{TextSize=15}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(icon,TweenInfo.new(0.18,Enum.EasingStyle.Quint),{TextSize=18}):Play()
        selectTab(cfg.key)
    end)

    local entry={
        key=cfg.key, bg=bg,
        actBg=actBg, hovBg=hovBg, dot=dot,
        idleBadge=nil, idleS=nil, actBadge=nil,
...
local function makeAccordionCard(parent, loFn, cfg)
    ...
    local COR       = cfg.color or Color3.fromRGB(10,132,255)
    local isOpen    = false
    local dropExtra = 0

    -- Card container — fundo sólido iOS secondarySystemBackground
    local card = Instance.new("Frame", parent)
    ...
    -- Stroke ultra-fino iOS separator
    local stroke = Instance.new("UIStroke",card)
    stroke.Color           = Color3.fromRGB(60,60,62)
    stroke.Thickness       = 0.5
    stroke.Transparency    = 0.0
    stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

    -- Ícone app-style (rounded square sólido, igual iOS app icons)
    local iconBg = Instance.new("Frame",card)
    iconBg.BackgroundColor3   = COR
    iconBg.BackgroundTransparency = 0
    iconBg.BorderSizePixel    = 0
    iconBg.Position           = UDim2.new(0,12,0,(HEADER_H-30)/2)
    ...
    iconLbl.TextSize=16; iconLbl.ZIndex=7; iconLbl.TextColor3=Color3.fromRGB(255,255,255)

    -- Título — SF Pro Display weight semibold (GothamBold)
    local titleLbl = Instance.new("TextLabel",card)
    titleLbl.BackgroundTransparency=1
    titleLbl.Position  = UDim2.new(0,52,0,0)
    titleLbl.Size      = UDim2.new(1,-80,0,HEADER_H)
    titleLbl.Font      = Enum.Font.GothamBold
    titleLbl.Text      = cfg.title or "Função"
    ...
    -- Chevron iOS (›) — cinza, roda para baixo quando aberto
    local arrowLbl = Instance.new("TextLabel",card)
    arrowLbl.BackgroundTransparency=1
    arrowLbl.AnchorPoint = Vector2.new(1,0.5)
    arrowLbl.Position    = UDim2.new(1,-12,0,HEADER_H/2)
    arrowLbl.Size        = UDim2.new(0,18,0,22)
    arrowLbl.Font        = Enum.Font.GothamBold
    arrowLbl.Text        = "›"
    arrowLbl.TextColor3  = Color3.fromRGB(152,152,157)
    arrowLbl.TextSize    = 22
    arrowLbl.ZIndex      = 7

    -- Linha separadora interna (iOS separator, começa após o ícone)
    local divLine = Instance.new("Frame",card)
    divLine.BackgroundColor3       = Color3.fromRGB(56,56,58)
    divLine.BackgroundTransparency = 0
    divLine.BorderSizePixel        = 0
    divLine.Position               = UDim2.new(0,52,0,HEADER_H-1)
    divLine.Size                   = UDim2.new(1,-52,0,1)
    divLine.ZIndex                 = 6
    divLine.Visible                = false

    -- Summary label — iOS secondary label style
    ...
    if cfg.summary then
        local sumBg = Instance.new("Frame",contentFrame)
        sumBg.BackgroundColor3=Color3.fromRGB(38,38,40); sumBg.BackgroundTransparency=0
        sumBg.BorderSizePixel=0; sumBg.Position=UDim2.new(0,0,0,0)
        sumBg.Size=UDim2.new(1,0,0,34); sumBg.ZIndex=6
        local sumLbl = Instance.new("TextLabel",sumBg)
        sumLbl.BackgroundTransparency=1
        sumLbl.Position=UDim2.new(0,52,0,0); sumLbl.Size=UDim2.new(1,-60,1,0)
        sumLbl.Font=Enum.Font.Gotham; sumLbl.Text=cfg.summary
        sumLbl.TextColor3=Color3.fromRGB(152,152,157); sumLbl.TextSize=10
        sumLbl.TextWrapped=true; sumLbl.TextXAlignment=Enum.TextXAlignment.Left
        sumLbl.TextYAlignment=Enum.TextYAlignment.Center; sumLbl.ZIndex=7
    end

    -- ── Hover feedback iOS (toque leve no fundo) ──────────────
    local function _press()
        TweenService:Create(card,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(56,56,58)}):Play()
    end
    local function _release()
        TweenService:Create(card,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{BackgroundColor3=Color3.fromRGB(44,44,46)}):Play()
    end

    local function closeCard(instant)
        isOpen=false; divLine.Visible=false
        -- Chevron volta para ›
        TweenService:Create(arrowLbl,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{Rotation=0,TextColor3=Color3.fromRGB(152,152,157)}):Play()
        TweenService:Create(stroke,TweenInfo.new(0.2),{Transparency=0.0}):Play()
        if instant then card.Size=UDim2.new(1,0,0,HEADER_H)
        else TweenService:Create(card,TweenInfo.new(0.28,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Size=UDim2.new(1,0,0,HEADER_H)}):Play() end
    ...
    local function openCard()
        ...
        -- Chevron vira ˅ (90°) — iOS behavior
        TweenService:Create(arrowLbl,TweenInfo.new(0.3,Enum.EasingStyle.Quint),{Rotation=90,TextColor3=COR}):Play()
        TweenService:Create(stroke,TweenInfo.new(0.2),{Transparency=0.0}):Play()
        TweenService:Create(card,TweenInfo.new(0.38,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,TOTAL_H+dropExtra)}):Play()
    end

    local function setDropExtraH(extra)
        dropExtra=extra or 0
        if isOpen then TweenService:Create(card,TweenInfo.new(0.28,Enum.EasingStyle.Quint),{Size=UDim2.new(1,0,0,TOTAL_H+dropExtra)}):Play() end
    end
    ...
    hBtn.MouseButton1Down:Connect(_press)
    hBtn.MouseButton1Up:Connect(_release)
    hBtn.MouseLeave:Connect(_release)
...
do
    -- ═══════════════════════════════════════════════════════════════
    -- ACCORDION: Bring Configuração
    -- ═══════════════════════════════════════════════════════════════
    local BRING_CFG_COR = Color3.fromRGB(148,112,220)
    local _bcTitleLbl = nil  -- referência ao TextLabel do título (para atualizar)
    local _bcCard, _bcCF, _bcDrop = makeAccordionCard(Pages["Bring"], bringLO, {
        ...
        color   = BRING_CFG_COR,
        contentH = 36 + 9 + 44 + 6 + 44 + 6 + 44 + 6 + 16,
    })
    -- Guarda referência ao label do título para atualizar depois
    ...
    -- ══════════════════════════════════════════════════════
    -- DROPDOWN INLINE — helper local para os 3 controles
    -- ══════════════════════════════════════════════════════
    local ROW_H  = 44
    local GAP    = 6
    local ITEM_H = 36
    ...
    -- Cria linha: label esquerda + botão direita
    local function makeCfgRow(y, labelTxt, initText, initColor)
        local tl = Instance.new("TextLabel", _bcCF)
        tl.BackgroundTransparency = 1
        tl.Position = UDim2.new(0,12,0,y)
        tl.Size = UDim2.new(0,110,0,ROW_H)
        tl.Font = Enum.Font.GothamBold
        tl.Text = labelTxt
        tl.TextColor3 = Color3.fromRGB(200,185,235)
        tl.TextSize = 11
        tl.TextXAlignment = Enum.TextXAlignment.Left
        tl.TextYAlignment = Enum.TextYAlignment.Center
        tl.ZIndex = 8

        local btn = Instance.new("TextButton", _bcCF)
        btn.BackgroundColor3 = initColor or BRING_CFG_COR
        btn.BackgroundTransparency = 0.82
        btn.BorderSizePixel = 0
        btn.AnchorPoint = Vector2.new(1,0.5)
        btn.Position = UDim2.new(1,-12,0,y+ROW_H/2)
        btn.Size = UDim2.new(0,148,0,34)
        btn.Font = Enum.Font.GothamBold
        btn.Text = initText or "Selecionar"
        btn.TextColor3 = initColor or Color3.fromRGB(220,210,255)
        btn.TextSize = 10
        btn.ZIndex = 8
        btn.AutoButtonColor = false
        ...
        bs.Thickness = 1.4; bs.Transparency = 0.3
        bs.Color = initColor or BRING_CFG_COR
        bs.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        local tint = Instance.new("Frame", btn)
        tint.BackgroundColor3 = initColor or BRING_CFG_COR
        tint.BackgroundTransparency = 0.82
        tint.BorderSizePixel = 0; tint.Size = UDim2.new(1,0,1,0); tint.ZIndex = 7
        Instance.new("UICorner",tint).CornerRadius = UDim.new(0,9)
        local arr = Instance.new("TextLabel", btn)
        arr.BackgroundTransparency = 1
        arr.AnchorPoint = Vector2.new(1,0.5); arr.Position = UDim2.new(1,-8,0.5,0)
        arr.Size = UDim2.new(0,14,0,14); arr.Font = Enum.Font.GothamBlack
        arr.Text = "▾"; arr.TextSize = 10; arr.ZIndex = 9
        arr.TextColor3 = initColor or Color3.fromRGB(220,210,255)

        return btn, bs, tint, arr
    end

    -- Cria popup dropdown ligado ao botão
    local function makeDrop(rowY, opts, getVal, setVal, onSel, btnRef, bsRef, tintRef, arrRef)
        local POP_H = #opts * ITEM_H + 8
        local pop = Instance.new("Frame", _bcCF)
        pop.BackgroundColor3 = Color3.fromRGB(18,10,36)
        pop.BackgroundTransparency = 0.05
        pop.BorderSizePixel = 0
        pop.AnchorPoint = Vector2.new(1,0)
        pop.Position = UDim2.new(1,-12,0,rowY+ROW_H+4)
        pop.Size = UDim2.new(0,148,0,POP_H)
        pop.ZIndex = 25
        pop.Visible = false
        pop.ClipsDescendants = true
        Instance.new("UICorner",pop).CornerRadius = UDim.new(0,10)
        local popS = Instance.new("UIStroke",pop)
        popS.Color = BRING_CFG_COR; popS.Thickness = 1.4; popS.Transparency = 0.35

        -- Atualiza visual do botão para a opção
        local function updateBtn(opt)
            local cor = opt.cor or BRING_CFG_COR
            btnRef.Text = (opt.icon or "").."  "..(opt.label or opt.key)
            btnRef.TextColor3 = cor
            bsRef.Color = cor; tintRef.BackgroundColor3 = cor; arrRef.TextColor3 = cor
        end
        ...
        for i, opt in ipairs(opts) do
            ...
            item.BackgroundColor3 = cor
            item.BackgroundTransparency = 0.88
            item.BorderSizePixel = 0
            item.Position = UDim2.new(0,4,0,4+(i-1)*ITEM_H)
            item.Size = UDim2.new(1,-8,0,ITEM_H-2)
            item.Font = Enum.Font.GothamBold
            item.Text = (opt.icon or "").."  "..(opt.label or opt.key)
            item.TextColor3 = cor
            item.TextSize = 10
            item.ZIndex = 26
            ...
            chk.Size = UDim2.new(0,14,0,14); chk.Font = Enum.Font.GothamBlack
            chk.Text = "✓"; chk.TextSize = 10; chk.TextColor3 = cor; chk.ZIndex = 27
            chk.Visible = (opt.key == getVal())
            table.insert(itemButtons, {btn=item, chk=chk, key=opt.key})

            item.MouseEnter:Connect(function()
                TweenService:Create(item,TweenInfo.new(0.1),{BackgroundTransparency=0.6}):Play()
            end)
            ...
            item.MouseButton1Click:Connect(function()
                ...
                updateBtn(opt)
                -- Atualiza checkmarks
                for _,ib in ipairs(itemButtons) do ib.chk.Visible=(ib.key==opt.key) end
                if onSel then onSel(opt) end
                closeCurrentDrop()
            end)
        end

        -- Inicializa visual do botão com valor atual
        for _,o in ipairs(opts) do
            if o.key == getVal() then updateBtn(o); break end
        end

        -- Clique no botão: abre/fecha dropdown
        btnRef.MouseButton1Click:Connect(function()
            if _openDropFrame and _openDropFrame ~= pop then
                closeCurrentDrop()
            end
            local opening = not pop.Visible
            pop.Visible = opening
            TweenService:Create(arrRef,TweenInfo.new(0.2),{Rotation=opening and 180 or 0}):Play()
            if opening then
                _openDropFrame = pop; _openDropArr = arrRef
                _bcDrop(POP_H + 8)
            else
    ...
    local sBtn,sBs,sTint,sArr = makeCfgRow(_bcy, "⚡  Estilo", "🔵  Padrão", Color3.fromRGB(100,160,255))
    makeDrop(_bcy, STYLE_OPTS,
        function() return bringStyle end,
        function(k) bringStyle = k end,
        function(opt) Notify.send({type="custom",icon=opt.icon,accent=opt.cor,title="Estilo Bring",msg=opt.label,duration=2}) end,
        sBtn, sBs, sTint, sArr)
    _bcy = _bcy + ROW_H + GAP

    -- ── Row 2: Velocidade ─────────────────────────────────────────
    local SPEED_OPTS = {
        {key="normal", label="Normal",      icon="🐢", cor=Color3.fromRGB(100,200,255), desc="Delay padrão"},
        {key="rapido", label="Rápido",      icon="🐇", cor=Color3.fromRGB(100,240,120), desc="Delay reduzido"},
        {key="mega",   label="Mega",        icon="⚡", cor=Color3.fromRGB(255,200,60),  desc="Sem delay"},
    }
    local vBtn,vBs,vTint,vArr = makeCfgRow(_bcy, "⏩  Velocidade", "🐢  Normal", Color3.fromRGB(100,200,255))
    makeDrop(_bcy, SPEED_OPTS,
        ...
        function(opt) Notify.send({type="custom",icon=opt.icon,accent=opt.cor,title="Velocidade Bring",msg=opt.desc,duration=2.5}) end,
        vBtn, vBs, vTint, vArr)
    _bcy = _bcy + ROW_H + GAP

    -- ── Row 3: Destino ────────────────────────────────────────────
    local MODE_OPTS = {
        {key="jogador",  label="Jogador",  icon="🧍", cor=Color3.fromRGB(100,200,255), desc="Itens vêm até você"},
        {key="fogueira", label="Fogueira", icon="🔥", cor=Color3.fromRGB(255,120,40),  desc="Itens na fogueira"},
        {key="ceu",      label="Céu",      icon="🌤️", cor=Color3.fromRGB(140,200,255), desc="Itens caem do alto"},
    }
    local function fireBringUnlock()
        for _, fn in ipairs(_bringUnlockCallbacks) do pcall(fn) end
    end
    local initDestText  = bringDestMode and ("🧍  Jogador") or "🔒  Selecionar"
    local initDestColor = bringDestMode and Color3.fromRGB(100,200,255) or Color3.fromRGB(180,130,60)
    local dBtn,dBs,dTint,dArr = makeCfgRow(_bcy, "🎯  Destino", initDestText, initDestColor)
    makeDrop(_bcy, MODE_OPTS,
        ...
        function(opt) Notify.send({type="info",icon=opt.icon,accent=opt.cor,title="Modo Bring",msg=opt.desc,duration=2.5}) end,
        dBtn, dBs, dTint, dArr)
    _bcy = _bcy + ROW_H + GAP
...
local _rgbConn       = nil
-- Imagem de fundo
local _bgImageId     = nil
local _bgImageLabel  = nil
local _bgOverlay     = nil
-- RGB TUDO
local _rgbTudoActive      = false
local _rgbTudoConn        = nil
local _rgbTudoBgEntries   = {}
local _rgbTudoTextEntries = {}
local _rgbTudoBgCollected   = false
local _rgbTudoTextCollected = false
...
local _ESTILO_H = 36+9 + 44+9 + 44+9 + 44+9 + 44+9 + 44+9 + 44+9 + 58+9 + 44+9 + 44 + 14
local _estCard, _estCF = makeAccordionCard(Pages["Configuracoes"], cfg2NextLO, {
    icon="🎨", title="Estilo de Interface", color=EST_COR,
    summary="Tema, letra, bordas neon/RGB, arredondamento, cor, imagem de fundo e RGB Total.",
    contentH = _ESTILO_H,
})
local _ey = 36+8
...
-- Função para aplicar/remover imagem de fundo
local function _applyBgImage(id)
    pcall(function() if _bgImageLabel then _bgImageLabel:Destroy() end end); _bgImageLabel=nil
    pcall(function() if _bgOverlay    then _bgOverlay:Destroy()    end end); _bgOverlay=nil
    if not id or id=="" then _bgImageId=nil; return end
    _bgImageId = id
    local img = Instance.new("ImageLabel", MainFrame)
    img.Name="HubBgImage"; img.Size=UDim2.new(1,0,1,0); img.Position=UDim2.new(0,0,0,0)
    img.BackgroundTransparency=1; img.Image="rbxassetid://"..tostring(id)
    img.ScaleType=Enum.ScaleType.Crop; img.ImageTransparency=1; img.ZIndex=1; img.BorderSizePixel=0
    _bgImageLabel = img
    local ov = Instance.new("Frame", MainFrame)
    ov.Name="HubBgOverlay"; ov.Size=UDim2.new(1,0,1,0); ov.Position=UDim2.new(0,0,0,0)
    ov.BackgroundColor3=Color3.fromRGB(0,0,0); ov.BackgroundTransparency=0.48
    ov.BorderSizePixel=0; ov.ZIndex=2
    _bgOverlay = ov
    TweenService:Create(img,TweenInfo.new(0.65,Enum.EasingStyle.Quad),{ImageTransparency=0.06}):Play()
end

-- Row da imagem (height 52)
local imgRow = Instance.new("Frame", _estCF)
imgRow.BackgroundColor3=Color3.fromRGB(52,32,82); imgRow.BorderSizePixel=0
imgRow.Position=UDim2.new(0,10,0,_ey); imgRow.Size=UDim2.new(1,-20,0,52); imgRow.ZIndex=7
Instance.new("UICorner",imgRow).CornerRadius=UDim.new(0,10)
local imgRowS=Instance.new("UIStroke",imgRow)
imgRowS.Color=Color3.fromRGB(255,160,80); imgRowS.Thickness=1.2; imgRowS.Transparency=0.6
imgRowS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
-- Barra lateral laranja
local imgBar=Instance.new("Frame",imgRow)
imgBar.BackgroundColor3=Color3.fromRGB(255,160,80); imgBar.BorderSizePixel=0
imgBar.Size=UDim2.new(0,3,0,32); imgBar.Position=UDim2.new(0,0,0.5,-16); imgBar.ZIndex=8
Instance.new("UICorner",imgBar).CornerRadius=UDim.new(0,2)
-- Label topo
local imgLbl=Instance.new("TextLabel",imgRow); imgLbl.BackgroundTransparency=1
imgLbl.Position=UDim2.new(0,10,0,5); imgLbl.Size=UDim2.new(1,-14,0,14)
imgLbl.Font=Enum.Font.GothamBold; imgLbl.Text="🖼️  ID Imagem de Fundo"
imgLbl.TextColor3=Color3.fromRGB(255,200,120); imgLbl.TextSize=10
imgLbl.TextXAlignment=Enum.TextXAlignment.Left; imgLbl.ZIndex=8
-- TextBox para o ID
local imgBox=Instance.new("TextBox",imgRow)
imgBox.BackgroundColor3=Color3.fromRGB(22,12,42); imgBox.BorderSizePixel=0
imgBox.Position=UDim2.new(0,10,0,24); imgBox.Size=UDim2.new(1,-96,0,22)
imgBox.Font=Enum.Font.Gotham; imgBox.Text=""; imgBox.PlaceholderText="Ex: 7072576363"
imgBox.PlaceholderColor3=Color3.fromRGB(110,80,150)
imgBox.TextColor3=Color3.fromRGB(220,210,245); imgBox.TextSize=10
imgBox.ClearTextOnFocus=false; imgBox.ZIndex=9
Instance.new("UICorner",imgBox).CornerRadius=UDim.new(0,6)
local imgBoxS=Instance.new("UIStroke",imgBox)
imgBoxS.Color=Color3.fromRGB(255,160,80); imgBoxS.Thickness=1; imgBoxS.Transparency=0.55
imgBoxS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
imgBox.Focused:Connect(function() TweenService:Create(imgBoxS,TweenInfo.new(0.15),{Transparency=0,Thickness=1.5}):Play() end)
imgBox.FocusLost:Connect(function()  TweenService:Create(imgBoxS,TweenInfo.new(0.2),{Transparency=0.55,Thickness=1}):Play() end)
-- Botão Aplicar
local imgApply=Instance.new("TextButton",imgRow)
imgApply.BackgroundColor3=Color3.fromRGB(200,110,20); imgApply.BackgroundTransparency=0.35; imgApply.BorderSizePixel=0
imgApply.AnchorPoint=Vector2.new(1,1); imgApply.Position=UDim2.new(1,-8,1,-6)
imgApply.Size=UDim2.new(0,76,0,22); imgApply.Font=Enum.Font.GothamBold
imgApply.Text="✔ Aplicar"; imgApply.TextColor3=Color3.fromRGB(255,255,255); imgApply.TextSize=9; imgApply.ZIndex=10
Instance.new("UICorner",imgApply).CornerRadius=UDim.new(0,7)
local imgApplyS=Instance.new("UIStroke",imgApply)
imgApplyS.Color=Color3.fromRGB(255,160,80); imgApplyS.Thickness=1.2; imgApplyS.Transparency=0.2
imgApplyS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
imgApply.MouseEnter:Connect(function() TweenService:Create(imgApply,TweenInfo.new(0.1),{BackgroundTransparency=0.1}):Play() end)
imgApply.MouseLeave:Connect(function() TweenService:Create(imgApply,TweenInfo.new(0.12),{BackgroundTransparency=0.35}):Play() end)
imgApply.MouseButton1Click:Connect(function()
    local id=imgBox.Text:match("%d+")
    if id then
        _applyBgImage(id)
        TweenService:Create(imgApply,TweenInfo.new(0.08),{Size=UDim2.new(0,70,0,20)}):Play()
        task.delay(0.15,function() pcall(function() TweenService:Create(imgApply,TweenInfo.new(0.15,Enum.EasingStyle.Back),{Size=UDim2.new(0,76,0,22)}):Play() end) end)
        Notify.success("🖼️ Fundo","Imagem aplicada! ID: "..id)
    else
        imgBox.Text=""
        TweenService:Create(imgRow,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(80,20,20)}):Play()
        task.delay(0.25,function() pcall(function() TweenService:Create(imgRow,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(52,32,82)}):Play() end) end)
        Notify.error("🖼️ Fundo","ID inválido. Use apenas os números do asset!")
    end
end)
-- Botão Remover (topo direito)
local imgRemove=Instance.new("TextButton",imgRow)
imgRemove.BackgroundColor3=Color3.fromRGB(180,35,35); imgRemove.BackgroundTransparency=0.55; imgRemove.BorderSizePixel=0
imgRemove.AnchorPoint=Vector2.new(1,0); imgRemove.Position=UDim2.new(1,-8,0,5)
imgRemove.Size=UDim2.new(0,76,0,17); imgRemove.Font=Enum.Font.Gotham
imgRemove.Text="✕ Remover"; imgRemove.TextColor3=Color3.fromRGB(255,140,140); imgRemove.TextSize=8; imgRemove.ZIndex=10
Instance.new("UICorner",imgRemove).CornerRadius=UDim.new(0,5)
imgRemove.MouseEnter:Connect(function() TweenService:Create(imgRemove,TweenInfo.new(0.1),{BackgroundTransparency=0.2}):Play() end)
imgRemove.MouseLeave:Connect(function() TweenService:Create(imgRemove,TweenInfo.new(0.12),{BackgroundTransparency=0.55}):Play() end)
imgRemove.MouseButton1Click:Connect(function()
    _applyBgImage(nil); imgBox.Text=""
    Notify.info("🖼️ Fundo","Imagem de fundo removida.")
end)
_ey=_ey+58
...
-- Coleta objetos para animar (lazy, só na primeira ativação)
local function _rgbCollectBg()
    if _rgbTudoBgCollected then return end; _rgbTudoBgCollected=true
    for _,obj in ipairs(MainFrame:GetDescendants()) do
        pcall(function()
            if _isInTopBar(obj) then return end
            if (obj:IsA("Frame") or obj:IsA("ScrollingFrame")) and obj.BackgroundTransparency<0.95 then
                table.insert(_rgbTudoBgEntries,{obj=obj,orig=obj.BackgroundColor3})
            end
        end)
    end
    table.insert(_rgbTudoBgEntries,{obj=MainFrame,orig=MainFrame.BackgroundColor3})
end
local function _rgbCollectText()
    if _rgbTudoTextCollected then return end; _rgbTudoTextCollected=true
    for _,obj in ipairs(ScreenGui:GetDescendants()) do
        pcall(function()
            if _isInTopBar(obj) then return end
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                table.insert(_rgbTudoTextEntries,{obj=obj,orig=obj.TextColor3})
            end
        end)
    end
end
local function _startRgbTudo()
    _rgbCollectBg(); _rgbCollectText()
    if _rgbTudoConn then _rgbTudoConn:Disconnect(); _rgbTudoConn=nil end
    _rgbTudoConn=RunService.Heartbeat:Connect(function()
        local t=tick()
        local hFast=(t*0.55)%1
        local hSlow=(t*0.20)%1
        local colText=Color3.fromHSV(hFast,0.92,1)
        local colStroke=Color3.fromHSV(hFast,1,1)
        local colBg=Color3.fromHSV(hSlow,0.65,0.18)
        -- Bordas (UIStroke)
        pcall(function() MainStroke.Color=colStroke end)
        for _,obj in ipairs(MainFrame:GetDescendants()) do
            pcall(function()
                if obj:IsA("UIStroke") and not _isInTopBar(obj) then obj.Color=colStroke end
            end)
        end
        -- Textos
        for _,e in ipairs(_rgbTudoTextEntries) do
            pcall(function() if e.obj and e.obj.Parent then e.obj.TextColor3=colText end end)
        end
        -- Título especial (ciclo mais rápido e saturado)
        pcall(function() TitleLabel.TextColor3=Color3.fromHSV((t*0.7)%1,1,1) end)
        -- Fundo dos frames (só se NÃO houver imagem de fundo)
        if not _bgImageId then
            for _,e in ipairs(_rgbTudoBgEntries) do
                pcall(function()
                    if e.obj and e.obj.Parent then
                        e.obj.BackgroundColor3=Color3.fromHSV((hSlow+0.03)%1,0.60,0.17)
                    end
                end)
            end
        end
...
local function _stopRgbTudo()
    if _rgbTudoConn then _rgbTudoConn:Disconnect(); _rgbTudoConn=nil end
    for _,e in ipairs(_rgbTudoTextEntries) do
        pcall(function() if e.obj and e.obj.Parent then e.obj.TextColor3=_textColorOver or e.orig end end)
    end
    for _,e in ipairs(_rgbTudoBgEntries) do
        pcall(function() if e.obj and e.obj.Parent then e.obj.BackgroundColor3=e.orig end end)
    end
    pcall(function() TitleLabel.TextColor3=VD_TEXT end)
    _applyTheme(_themeAccent)
    _rgbTudoBgCollected=false; _rgbTudoTextCollected=false
    _rgbTudoBgEntries={}; _rgbTudoTextEntries={}
end

_accToggle(_estCF, _ey, "🌈  RGB TUDO", false, Color3.fromHSV(0,1,1), function(s)
    _rgbTudoActive=s
    if s then
        _startRgbTudo()
        Notify.warn("🌈 RGB TUDO","Modo arco-íris total ativado em tudo!")
    else
        _stopRgbTudo()
        Notify.info("🌈 RGB TUDO","Desativado — interface restaurada.")
    end
end)
_ey=_ey+44
...
-- ── Paleta iOS (Apple HIG Dark Mode) ─────────────────
local IOS_BG      = Color3.fromRGB(28,  28,  30)
local IOS_CARD    = Color3.fromRGB(44,  44,  46)
local IOS_DEEP    = Color3.fromRGB(18,  18,  20)
local IOS_TOPBAR  = Color3.fromRGB(22,  22,  24)
local IOS_ACCENT  = Color3.fromRGB(10, 132, 255)
local IOS_TEXT    = Color3.fromRGB(242, 242, 247)
local IOS_MUTED   = Color3.fromRGB(142, 142, 147)
local IOS_STROKE  = Color3.fromRGB(58,  58,  60)
local IOS_ROW     = Color3.fromRGB(44,  44,  46)
local IOS_ROW_HOV = Color3.fromRGB(56,  56,  58)
local IOS_SEP     = Color3.fromRGB(38,  38,  40)

-- ── Helpers de mapeamento ────────────────────────────
local function _ck(c)
    return math.floor(c.R*255+0.5)..","..math.floor(c.G*255+0.5)..","..math.floor(c.B*255+0.5)
end
local _iosBgMap, _iosTextMap = {}, {}
local function _iosRegBg(orig, new)  _iosBgMap[_ck(orig)]   = new end
local function _iosRegTxt(orig, new) _iosTextMap[_ck(orig)] = new end

-- VD_ → iOS
_iosRegBg(VD_BG,      IOS_BG);     _iosRegBg(VD_SIDEBAR, IOS_BG)
_iosRegBg(VD_TOPBAR,  IOS_TOPBAR); _iosRegBg(VD_ROW,     IOS_ROW)
_iosRegBg(VD_ROW_HOV, IOS_ROW_HOV);_iosRegBg(VD_TAB_ACT, IOS_CARD)
_iosRegBg(VD_STROKE,  IOS_STROKE); _iosRegBg(VD_DIVIDER, IOS_SEP)
-- Hardcoded purples usados nos accordions / sliders / cards
_iosRegBg(Color3.fromRGB(16,20,38),  IOS_DEEP); _iosRegBg(Color3.fromRGB(14,8,30),   IOS_DEEP)
_iosRegBg(Color3.fromRGB(22,12,42),  IOS_DEEP); _iosRegBg(Color3.fromRGB(30,14,62),  IOS_DEEP)
_iosRegBg(Color3.fromRGB(18,8,40),   IOS_DEEP); _iosRegBg(Color3.fromRGB(44,28,72),  IOS_ROW)
_iosRegBg(Color3.fromRGB(52,32,82),  IOS_ROW);  _iosRegBg(Color3.fromRGB(72,50,108), IOS_ROW)
_iosRegBg(Color3.fromRGB(88,62,120), IOS_CARD); _iosRegBg(Color3.fromRGB(90,68,124), IOS_CARD)
_iosRegBg(Color3.fromRGB(60,38,72),  IOS_ROW);  _iosRegBg(Color3.fromRGB(58,38,88),  IOS_ROW)
_iosRegBg(Color3.fromRGB(40,24,66),  IOS_SEP);  _iosRegBg(Color3.fromRGB(50,32,80),  IOS_ROW)
_iosRegBg(Color3.fromRGB(38,22,66),  IOS_DEEP); _iosRegBg(Color3.fromRGB(54,34,88),  IOS_CARD)
_iosRegBg(Color3.fromRGB(28,16,50),  IOS_DEEP); _iosRegBg(Color3.fromRGB(26,28,38),  IOS_CARD)
_iosRegBg(Color3.fromRGB(100,80,120),IOS_ROW);  _iosRegBg(Color3.fromRGB(44,28,72),  IOS_ROW)
_iosRegBg(Color3.fromRGB(30,18,60),  IOS_DEEP); _iosRegBg(Color3.fromRGB(12,6,28),   IOS_DEEP)
_iosRegBg(Color3.fromRGB(8,4,18),    IOS_DEEP); _iosRegBg(Color3.fromRGB(10,4,24),   IOS_DEEP)
-- Textos
_iosRegTxt(VD_TEXT,    IOS_TEXT);  _iosRegTxt(VD_MUTED,   IOS_MUTED)
_iosRegTxt(VD_SECTION, IOS_TEXT)
_iosRegTxt(Color3.fromRGB(238,232,255), IOS_TEXT)
_iosRegTxt(Color3.fromRGB(220,210,245), IOS_TEXT)
_iosRegTxt(Color3.fromRGB(215,205,235), IOS_TEXT)
_iosRegTxt(Color3.fromRGB(222,214,248), IOS_TEXT)
_iosRegTxt(Color3.fromRGB(200,188,228), IOS_MUTED)
_iosRegTxt(Color3.fromRGB(175,155,210), IOS_MUTED)
_iosRegTxt(Color3.fromRGB(150,110,190), IOS_MUTED)
_iosRegTxt(Color3.fromRGB(120,100,160), IOS_MUTED)
_iosRegTxt(Color3.fromRGB(148,112,220), IOS_ACCENT)
_iosRegTxt(Color3.fromRGB(120,86,188),  IOS_ACCENT)
_iosRegTxt(Color3.fromRGB(190,148,255), IOS_TEXT)

-- ── Estado e save/revert ──────────────────────────────
local _iosActive = false
local _iosSaved  = {}

local function _iosSave(obj, prop)
    table.insert(_iosSaved, {obj=obj, prop=prop, val=obj[prop]})
end
local function _iosTween(obj, prop, newVal, dur)
    _iosSave(obj, prop)
    TweenService:Create(obj, TweenInfo.new(dur or 0.4, Enum.EasingStyle.Quint), {[prop]=newVal}):Play()
end

-- ── Função principal: aplicar iOS ────────────────────
local function _applyIOS()
    _iosSaved = {}
    local D = TweenInfo.new(0.45, Enum.EasingStyle.Quint)
    local DT = TweenInfo.new(0.3,  Enum.EasingStyle.Quad)

    -- Containers principais
    _iosTween(MainFrame, "BackgroundColor3", IOS_BG,     0.5)
    _iosSave(MainStroke, "Color");     _iosSave(MainStroke, "Thickness"); _iosSave(MainStroke, "Transparency")
    TweenService:Create(MainStroke, TweenInfo.new(0.3), {Color=IOS_STROKE, Thickness=0.5, Transparency=0.4}):Play()
    local mfUC = MainFrame:FindFirstChildOfClass("UICorner")
    if mfUC then _iosSave(mfUC,"CornerRadius"); TweenService:Create(mfUC,D,{CornerRadius=UDim.new(0,22)}):Play() end

    _iosTween(TopBar,  "BackgroundColor3", IOS_TOPBAR, 0.4)
    _iosTween(topFix,  "BackgroundColor3", IOS_TOPBAR, 0.4)
    _iosTween(TopDivider, "BackgroundColor3", IOS_SEP, 0.3)
    _iosTween(TitleLabel, "TextColor3", IOS_TEXT,  0.3)
    _iosTween(TitleSub,   "TextColor3", IOS_MUTED, 0.3)

    _iosTween(SearchFrame, "BackgroundColor3", IOS_TOPBAR, 0.4)
    _iosTween(sfFix,       "BackgroundColor3", IOS_TOPBAR, 0.4)
    _iosTween(SFLine,      "BackgroundColor3", IOS_SEP,    0.3)
    _iosTween(SearchBg,    "BackgroundColor3", IOS_CARD,   0.35)
    _iosSave(SearchBgS,"Color"); SearchBgS.Color=IOS_STROKE
    _iosSave(SearchBgS,"Transparency"); SearchBgS.Transparency=0.5
    local sbUC = SearchBg:FindFirstChildOfClass("UICorner")
    if sbUC then _iosSave(sbUC,"CornerRadius"); TweenService:Create(sbUC,D,{CornerRadius=UDim.new(0,10)}):Play() end
    _iosTween(SearchBox,  "TextColor3", IOS_TEXT,  0.3)
    _iosTween(SearchIcon, "TextColor3", IOS_MUTED, 0.3)

    _iosTween(SideBar,     "BackgroundColor3", Color3.fromRGB(22,22,24), 0.4)
    _iosTween(Divider,     "BackgroundColor3", IOS_SEP,    0.3)
    _iosTween(ContentArea, "BackgroundColor3", IOS_BG,     0.4)
    _iosTween(Footer,      "BackgroundColor3", Color3.fromRGB(22,22,24), 0.4)
    _iosTween(FooterTopLine,"BackgroundColor3",IOS_SEP,    0.3)
    _iosTween(AvatarBg,    "BackgroundColor3", IOS_ACCENT, 0.3)
    _iosTween(FName, "TextColor3", IOS_TEXT,  0.3)
    _iosTween(FTag,  "TextColor3", IOS_MUTED, 0.3)
    _iosTween(FLangFrame, "BackgroundColor3", IOS_CARD, 0.3)

    -- Tab actBg → iOS blue highlight
    for _, e in ipairs(allTabs) do
        pcall(function()
            if e.actBg then
                _iosSave(e.actBg, "BackgroundColor3")
                e.actBg.BackgroundColor3 = IOS_ACCENT
            end
        end)
    end

    -- ── Todos os descendentes de MainFrame ───────────
    task.spawn(function()
        local n = 0
        for _, obj in ipairs(MainFrame:GetDescendants()) do
            if _isInTopBar(obj) then continue end
            n = n + 1; if n % 80 == 0 then task.wait() end
            pcall(function()
                -- Frames / ScrollingFrames
                if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                    if obj.BackgroundTransparency < 0.97 then
                        local mapped = _iosBgMap[_ck(obj.BackgroundColor3)]
                        if mapped then
                            _iosSave(obj,"BackgroundColor3")
                            TweenService:Create(obj,D,{BackgroundColor3=mapped}):Play()
                        end
                    end
                    -- CornerRadius → iOS equivalente
                    local uc = obj:FindFirstChildOfClass("UICorner")
                    if uc then
                        local rv = uc.CornerRadius.Offset
                        local nr = rv
                        if     rv>0  and rv<=4  then nr=8
                        elseif rv<=8            then nr=12
                        elseif rv<=12           then nr=14
                        elseif rv<=18           then nr=20
                        elseif rv>18            then nr=24 end
                        if nr~=rv then
                            _iosSave(uc,"CornerRadius")
                            TweenService:Create(uc,D,{CornerRadius=UDim.new(0,nr)}):Play()
                        end
                    end
                end
                -- UIStroke: roxa → sutil cinza
                if obj:IsA("UIStroke") then
                    local c = obj.Color
                    local r,g,b = math.floor(c.R*255),math.floor(c.G*255),math.floor(c.B*255)
                    if b>r*1.1 and b>50 then
                        _iosSave(obj,"Color"); _iosSave(obj,"Transparency")
                        obj.Color = IOS_STROKE
                        TweenService:Create(obj,TweenInfo.new(0.3),{Transparency=0.4}):Play()
                    end
                end
                -- Textos
                if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                    if obj==TitleLabel then return end
                    local mapped = _iosTextMap[_ck(obj.TextColor3)]
                    if mapped then
                        _iosSave(obj,"TextColor3")
                        TweenService:Create(obj,DT,{TextColor3=mapped}):Play()
                    end
                end
            end)
        end
...
local function _revertIOS()
    local DRev = TweenInfo.new(0.38, Enum.EasingStyle.Quint)
    local n=0
    for i=#_iosSaved,1,-1 do
        local s=_iosSaved[i]; n=n+1
        pcall(function()
            if s.obj and s.obj.Parent then
                if typeof(s.val)=="Color3" then
                    TweenService:Create(s.obj,DRev,{[s.prop]=s.val}):Play()
                elseif typeof(s.val)=="UDim" then
                    TweenService:Create(s.obj,DRev,{[s.prop]=s.val}):Play()
                else
                    s.obj[s.prop]=s.val
                end
...
_accToggle(_estCF, _ey, "🍎  Modo iOS", false, IOS_ACCENT, function(s)
    _iosActive=s
    if s then
        _applyIOS()
        Notify.success("🍎 Modo iOS","Interface reestilizada no padrão Apple!",5)
    else
        _revertIOS()
        Notify.info("🍎 Modo iOS","Interface restaurada ao padrão original.",3)
    end
end)
_ey=_ey+44
...
makeToggle = function(page, lo_fn, lbl_txt, desc_txt, cor, onToggle)
    -- iOS Settings cell
    local IOS_SW_OFF = Color3.fromRGB(120,120,128)
    local IOS_SW_ON  = Color3.fromRGB(48,209,88)
    local row = Instance.new("Frame", page)
    row.BackgroundColor3    = Color3.fromRGB(44,44,46)
    row.BackgroundTransparency = 0
    row.BorderSizePixel     = 0
    row.Size                = UDim2.new(1,0,0,54)
    row.LayoutOrder         = lo_fn()
    row.ZIndex              = 5
    Instance.new("UICorner",row).CornerRadius = UDim.new(0,12)
    local sep = Instance.new("Frame",row)
    sep.BackgroundColor3=Color3.fromRGB(56,56,58); sep.BackgroundTransparency=0
    sep.BorderSizePixel=0; sep.Position=UDim2.new(0,16,1,-1)
    sep.Size=UDim2.new(1,-16,0,1); sep.ZIndex=6
    local tl = Instance.new("TextLabel",row)
    tl.BackgroundTransparency=1; tl.Position=UDim2.new(0,16,0,9)
    tl.Size=UDim2.new(1,-82,0,18); tl.Font=Enum.Font.GothamBold
    tl.Text=lbl_txt; tl.TextColor3=Color3.fromRGB(255,255,255)
    tl.TextSize=13; tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=7
    local td = Instance.new("TextLabel",row)
    td.BackgroundTransparency=1; td.Position=UDim2.new(0,16,0,30)
    td.Size=UDim2.new(1,-82,0,16); td.Font=Enum.Font.Gotham
    td.Text=desc_txt; td.TextColor3=Color3.fromRGB(152,152,157)
    td.TextSize=10; td.TextXAlignment=Enum.TextXAlignment.Left
    td.TextWrapped=true; td.ZIndex=7
    local pill = Instance.new("Frame",row)
    pill.BackgroundColor3=IOS_SW_OFF; pill.BorderSizePixel=0
    pill.AnchorPoint=Vector2.new(1,0.5); pill.Position=UDim2.new(1,-12,0.5,0)
    pill.Size=UDim2.new(0,51,0,31); pill.ZIndex=9
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    local pillInner=Instance.new("Frame",pill)
    pillInner.BackgroundColor3=Color3.fromRGB(0,0,0); pillInner.BackgroundTransparency=0.88
    pillInner.BorderSizePixel=0; pillInner.Size=UDim2.new(1,0,1,0); pillInner.ZIndex=9
    Instance.new("UICorner",pillInner).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",pill)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255); knob.BorderSizePixel=0
    knob.AnchorPoint=Vector2.new(0.5,0.5); knob.Position=UDim2.new(0,17,0.5,0)
    knob.Size=UDim2.new(0,27,0,27); knob.ZIndex=11
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local knobS=Instance.new("UIStroke",knob)
    knobS.Color=Color3.fromRGB(0,0,0); knobS.Thickness=1; knobS.Transparency=0.8
    knobS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local state=false
    local btn=Instance.new("TextButton",row); btn.BackgroundTransparency=1; btn.BorderSizePixel=0
    btn.AutoButtonColor=false; btn.Size=UDim2.new(1,0,1,0); btn.Text=""; btn.ZIndex=12
    btn.MouseEnter:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(52,52,54)}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.15,Enum.EasingStyle.Quint),{BackgroundColor3=Color3.fromRGB(44,44,46)}):Play()
    end)
    btn.MouseButton1Down:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.07),{BackgroundColor3=Color3.fromRGB(60,60,62)}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        state=not state
        TweenService:Create(row,TweenInfo.new(0.15,Enum.EasingStyle.Quint),{BackgroundColor3=Color3.fromRGB(44,44,46)}):Play()
        TweenService:Create(pill,TweenInfo.new(0.22,Enum.EasingStyle.Quint),{BackgroundColor3=state and IOS_SW_ON or IOS_SW_OFF}):Play()
        TweenService:Create(knob,TweenInfo.new(0.1),{Size=UDim2.new(0,32,0,27)}):Play()
        task.delay(0.1,function()
            pcall(function()
                TweenService:Create(knob,TweenInfo.new(0.18,Enum.EasingStyle.Quint),{
                    Position=state and UDim2.new(1,-17,0.5,0) or UDim2.new(0,17,0.5,0),
        ...
        task.spawn(function()
            pcall(function()
                local snd=Instance.new("Sound")
                snd.SoundId="rbxassetid://"..(state and "6031221736" or "2544086171")
                snd.Volume=0.3; snd.RollOffMaxDistance=0; snd.Parent=SoundService
                if not snd.IsLoaded then snd.Loaded:Wait() end
                snd:Play(); game:GetService("Debris"):AddItem(snd,3)
            end)
        end)
        if state then Notify.success(lbl_txt,"✓ Ativado")
        else Notify.send({type="error",icon="✕",accent=Color3.fromRGB(255,69,58),title=lbl_txt,msg="✗ Desativado"}) end
        onToggle(state)
    end)
    return function() return state end
end

-- Slider genérico — iOS: track azul, knob branco com sombra
local function makeSlider(page, lo_fn, lbl_txt, minV, maxV, defV, cor, fmt, onChange)
    local IOS_TRACK_BG  = Color3.fromRGB(58,58,60)   -- systemFill
    local IOS_TRACK_FG  = Color3.fromRGB(10,132,255)  -- systemBlue
    local row=Instance.new("Frame",page)
    row.BackgroundColor3=Color3.fromRGB(44,44,46); row.BackgroundTransparency=0
    row.BorderSizePixel=0
    row.Size=UDim2.new(1,0,0,54); row.LayoutOrder=lo_fn(); row.ZIndex=5
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,14)
    -- Separador inferior
    local slSep=Instance.new("Frame",row)
    slSep.BackgroundColor3=Color3.fromRGB(56,56,58); slSep.BackgroundTransparency=0
    slSep.BorderSizePixel=0; slSep.Position=UDim2.new(0,16,1,-1)
    slSep.Size=UDim2.new(1,-16,0,1); slSep.ZIndex=6
    -- Label topo esquerda
    local tl=Instance.new("TextLabel",row); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,16,0,8); tl.Size=UDim2.new(0.55,0,0,18)
    ...
    -- Valor — direita, cinza iOS secondary label
    local valLbl=Instance.new("TextLabel",row); valLbl.BackgroundTransparency=1
    valLbl.AnchorPoint=Vector2.new(1,0)
    valLbl.Position=UDim2.new(1,-16,0,8); valLbl.Size=UDim2.new(0,50,0,18)
    valLbl.Font=Enum.Font.GothamBold; valLbl.TextSize=13
    valLbl.TextColor3=Color3.fromRGB(152,152,157)
    valLbl.TextXAlignment=Enum.TextXAlignment.Right; valLbl.ZIndex=7

    -- Track iOS: 4px alto, cinza escuro
    local trackBg=Instance.new("Frame",row)
    trackBg.BackgroundColor3=IOS_TRACK_BG; trackBg.BorderSizePixel=0
    trackBg.Position=UDim2.new(0,16,1,-18); trackBg.Size=UDim2.new(1,-32,0,4); trackBg.ZIndex=7
    Instance.new("UICorner",trackBg).CornerRadius=UDim.new(1,0)

    -- Fill azul iOS
    local fill=Instance.new("Frame",trackBg)
    fill.BackgroundColor3=IOS_TRACK_FG; fill.BorderSizePixel=0
    fill.Size=UDim2.new(t0,0,1,0); fill.ZIndex=8
    Instance.new("UICorner",fill).CornerRadius=UDim.new(1,0)

    -- Knob branco iOS (22px com sombra)
    local knob=Instance.new("Frame",trackBg)
    knob.BackgroundColor3=Color3.fromRGB(255,255,255); knob.BorderSizePixel=0
    knob.AnchorPoint=Vector2.new(0.5,0.5)
    knob.Position=UDim2.new(t0,0,0.5,0); knob.Size=UDim2.new(0,22,0,22); knob.ZIndex=9
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    -- Sombra iOS knob
    local knobS=Instance.new("UIStroke",knob); knobS.Color=Color3.fromRGB(0,0,0)
    knobS.Thickness=1.2; knobS.Transparency=0.75; knobS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
...
-- ── Pega a melhor arma disponível (equipada > backpack) ────────
local function getWeapon()
    local ch = Player.Character
    -- 1. Já tem ferramenta equipada? Usa ela
    if ch then
        local t = ch:FindFirstChildWhichIsA("Tool")
        if t then return t, false end
    end
    -- 2. Procura no Backpack por nome
    local bp = Player.Backpack
    for _, name in ipairs(KA_WEAPON_NAMES) do
        local t = bp:FindFirstChild(name)
        if t then return t, true end -- true = veio do backpack, precisa equipar
    end
    -- 3. Qualquer Tool no backpack
    local t = bp:FindFirstChildWhichIsA("Tool")
    if t then return t, true end
    return nil, false
end

-- ── Ataca um mob via firetouchinterest ou mouse1press ──────────
local function attackMob(mob)
    ...
    -- Equipa do backpack se necessário
    ...
    -- Tenta firetouchinterest em todas as partes do mob
    local hit = false
    pcall(function()
    ...
    -- Fallback: mouse1press se firetouchinterest não existir
    if not hit then
        pcall(function()
            mouse1press()
            task.wait(0.05)
...
local function getAxe()
    local ch = Player.Character
    -- 1. Machado já equipado
    if ch then
        local t = ch:FindFirstChildWhichIsA("Tool")
        if t and t.Name:lower():find("axe") or (t and t.Name:lower() == "chainsaw") then
            return t, false
        end
    end
    -- 2. Machado no backpack
    local bp = Player.Backpack
...
local function chopTree(treePart)
    local ch = Player.Character; if not ch then return end
    local myHRP = ch:FindFirstChild("HumanoidRootPart"); if not myHRP then return end
    ...
    -- Equipa o machado do backpack
    if fromBackpack then
        pcall(function() axe.Parent = ch end)
        task.wait(0.06)
    end

    -- firetouchinterest no tronco da árvore
    pcall(function()
        if firetouchinterest then
            firetouchinterest(myHRP, treePart, 0)
            task.wait(0.06)
            firetouchinterest(myHRP, treePart, 1)
        else
            -- fallback: mouse1press
            mouse1press(); task.wait(0.06); mouse1release()
        end
    end)

    -- Devolve pro backpack
    ...
    taLoop = RunService.Heartbeat:Connect(function()
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                ...
                lastChop = now
                task.spawn(function()
                    pcall(function() chopTree(root) end)
                end)
            end)
        end
    end)
