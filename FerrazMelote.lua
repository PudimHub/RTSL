...
local function makePopupToggle(text,callback)
    ...
    local pill=Instance.new("Frame",row); pill.BackgroundColor3=Color3.fromRGB(55,22,88); pill.BorderSizePixel=0
    ...
    -- ✓ checkmark igual à foto
    local espCk=Instance.new("TextLabel",knob); espCk.BackgroundTransparency=1
    espCk.Size=UDim2.new(1,0,1,0); espCk.Text="✓"
    espCk.TextColor3=cat.cor; espCk.TextSize=12; espCk.Font=Enum.Font.GothamBold
    espCk.Visible=false; espCk.ZIndex=11
    local state=false
    ...
    btn.MouseButton1Click:Connect(function()
        state=not state
        TweenService:Create(pill,TweenInfo.new(0.2),{BackgroundColor3=state and Color3.fromRGB(87,242,135) or Color3.fromRGB(55,22,88)}):Play()
...
infoStatus.BackgroundColor3=Color3.fromRGB(55,22,88); infoStatus.BorderSizePixel=0
...
ntBtn.MouseButton1Click:Connect(function()
    ...
    local offColor  = Color3.fromRGB(55,22,88)
    TweenService:Create(ntPill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{
        BackgroundColor3 = notifEnabled and onColor or Color3.fromRGB(55,22,88)
    }):Play()
...
dadosHeader.BackgroundColor3=Color3.fromRGB(55,22,88); dadosHeader.BorderSizePixel=0
...
for idx, lang in ipairs(LANGUAGES) do
    ...
    lBtn.BackgroundColor3=isSelected and Color3.fromRGB(148,112,220) or Color3.fromRGB(55,22,88)
    ...
    lBtn.MouseLeave:Connect(function()
        if lang.code~=currentLang.code then
            TweenService:Create(lBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(55,22,88),BackgroundTransparency=0.15}):Play()
        end
    end)
...
srvCard.BackgroundColor3=Color3.fromRGB(55,22,88); srvCard.BorderSizePixel=0
...
-- TextBox de ID
local srvBoxBg=Instance.new("Frame",srvCard); srvBoxBg.BackgroundColor3=Color3.fromRGB(55,22,88)
...
titleCard.BackgroundColor3=Color3.fromRGB(55,22,88)
...
local function makeEspRow(cat)
    ...
    local iconContainer=Instance.new("Frame",row); iconContainer.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    btn.MouseLeave:Connect(function() TweenService:Create(row,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end)
    btn.MouseButton1Click:Connect(function()
        state=not state; espAtivo[cat.key]=state; lastCache=0
        TweenService:Create(pill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{BackgroundColor3=state and cat.cor or Color3.fromRGB(55,22,88)}):Play()
        TweenService:Create(knob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9),
            BackgroundColor3=state and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220),
        }):Play()
        plCk.Visible=state
        espCk.Visible=state
...
local function makeBringRow(bcat)
    local row=Instance.new("Frame",Pages["Bring"]); row.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    local function applyLockState()
        ...
        if locked then
            btnBring.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    btnLimpar.MouseButton1Click:Connect(function()
        ...
        if restored > 0 then
            ...
            task.delay(0.8, function() TweenService:Create(btnLimpar,TweenInfo.new(0.3),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end)
            task.delay(2.5,function() TweenService:Create(feedbackLbl,TweenInfo.new(0.4),{TextTransparency=1}):Play(); task.wait(0.5); feedbackLbl.Text=""; feedbackLbl.TextTransparency=0; feedbackLbl.TextColor3=bcat.cor end)
        else
...
do
    ...
    -- Barra de progresso
    local baProgBg=Instance.new("Frame",baCard); baProgBg.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    -- Botão LIMPAR ALL
    local baLimparBtn=Instance.new("TextButton",baCard); baLimparBtn.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    baLimparBtn.MouseLeave:Connect(function() TweenService:Create(baLimparBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(55,22,88),BackgroundTransparency=0.1}):Play() end)
    baLimparBtn.MouseButton1Click:Connect(function()
        ...
        if restored > 0 then
            ...
            task.delay(0.8, function() TweenService:Create(baLimparBtn,TweenInfo.new(0.3),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end)
...
local function makePlToggle(lbl_txt, desc_txt, cor, onToggle)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(pill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{BackgroundColor3=state and cor or Color3.fromRGB(55,22,88)}):Play()
        TweenService:Create(knob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
...
local function makeSliderBar(parentPage, lo_fn, lbl_txt, desc_txt, cor, minV, maxV, initVal, onChange)
    ...
    trackBg.BackgroundColor3=Color3.fromRGB(55,22,88); trackBg.BorderSizePixel=0
    ...
    knob.BackgroundColor3=Color3.fromRGB(55,22,88); knob.BorderSizePixel=0
...
local function makePlKACard(COR, titleTxt, descTxt, minV, maxV, defV, maxStr, getDescFn, initEnabled, onToggle, onSlider)
    ...
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    pill.BackgroundColor3=enabled and Color3.fromRGB(110,90,145) or Color3.fromRGB(55,22,88)

    local knob=Instance.new("Frame",pill); knob.BorderSizePixel=0
    knob.Size=UDim2.new(0,24,0,24); knob.ZIndex=9
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    knob.BackgroundColor3=enabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(pill,TweenInfo.new(0.18),{
            BackgroundColor3=enabled and Color3.fromRGB(110,90,145) or Color3.fromRGB(55,22,88)
        }):Play()
    ...
    skn.BackgroundColor3=Color3.fromRGB(55,22,88); skn.BorderSizePixel=0
...
cdPill.BackgroundColor3=Color3.fromRGB(55,22,88)
...
cdTrack.BackgroundColor3=Color3.fromRGB(55,22,88); cdTrack.BorderSizePixel=0
...
cdDot.BackgroundColor3=Color3.fromRGB(55,22,88); cdDot.BorderSizePixel=0
...
cdBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(cdPill,TweenInfo.new(0.18),{BackgroundColor3=camDistEnabled and Color3.fromRGB(110,90,145) or Color3.fromRGB(55,22,88)}):Play()
...
acsPill.BackgroundColor3=Color3.fromRGB(55,22,88)
...
acsBtnClick.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(acsPill,TweenInfo.new(0.22),{
        BackgroundColor3 = acsEnabled and ACS_COR or Color3.fromRGB(55,22,88)
    }):Play()
...
ibcPill.BackgroundColor3=Color3.fromRGB(55,22,88); ibcPill.BorderSizePixel = 0
...
ibcKnob.BackgroundColor3 = Color3.fromRGB(180,160,220); ibcKnob.BorderSizePixel = 0
...
-- ✓ checkmark
local ibcCk=Instance.new("TextLabel",ibcKnob); ibcCk.BackgroundTransparency=1
ibcCk.Size=UDim2.new(1,0,1,0); ibcCk.Text="✓"
ibcCk.TextColor3=IBC_COR; ibcCk.TextSize=12; ibcCk.Font=Enum.Font.GothamBold
ibcCk.Visible=false; ibcCk.ZIndex=11
...
ibcBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(ibcPill,TweenInfo.new(0.22),{
        BackgroundColor3 = ibcEnabled and IBC_COR or Color3.fromRGB(55,22,88)
    }):Play()
    TweenService:Create(ibcKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        ...
        BackgroundColor3 = ibcEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220),
    }):Play()
    ibcCk.Visible=ibcEnabled
...
cfgSndBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(cfgSndPill,TweenInfo.new(0.18),{BackgroundColor3=cfgSndState and Color3.fromRGB(110,90,145) or Color3.fromRGB(55,22,88)}):Play()
...
do
    ...
    -- Card toggle
    local nightRow=Instance.new("Frame",Pages["Player"]); nightRow.BackgroundColor3=Color3.fromRGB(55,22,88)
    ...
    -- Toggle pill
    local nightPill=Instance.new("Frame",nightRow); nightPill.BackgroundColor3=Color3.fromRGB(55,22,88); nightPill.BorderSizePixel=0
    ...
    local nightKnob=Instance.new("Frame",nightPill); nightKnob.BackgroundColor3=Color3.fromRGB(180,160,220); nightKnob.BorderSizePixel=0
    ...
    nightBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(nightPill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{BackgroundColor3=nightAutoEnabled and NIGHT_COR or Color3.fromRGB(55,22,88)}):Play()
        TweenService:Create(nightKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            ...
            BackgroundColor3=nightAutoEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220)
        }):Play()
...
tpChildBtn.MouseButton1Click:Connect(function()
    ...
    if target then
        ...
        TweenService:Create(chStatusDot,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(180,160,220)}):Play()
        tpChildBtnL.Text = "✓"
...
local function bpGetOrCreateGroup(biomeLabel, biomeIcon, biomeCor)
    ...
    hitBtn.MouseButton1Click:Connect(function()
        ...
        task.delay(0.1,function() TweenService:Create(btn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end)
...
makeToggle = function(page, lo_fn, lbl_txt, desc_txt, cor, onToggle)
    ...
    pill.BackgroundColor3=Color3.fromRGB(55,22,88); pill.BorderSizePixel=0
    pill.Position=UDim2.new(1,-54,0.5,-11); pill.Size=UDim2.new(0,46,0,22); pill.ZIndex=9
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",pill)
    knob.BackgroundColor3=Color3.fromRGB(180,160,220); knob.BorderSizePixel=0
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(pill,TweenInfo.new(0.22,Enum.EasingStyle.Quad),{
            BackgroundColor3=state and cor or Color3.fromRGB(55,22,88)
        }):Play()
        TweenService:Create(knob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9),
            BackgroundColor3=state and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220)
...
-- Pill toggle
local kaPill=Instance.new("Frame",kaCard); kaPill.BackgroundColor3=Color3.fromRGB(55,22,88); kaPill.BorderSizePixel=0
...
local kaKnob=Instance.new("Frame",kaPill); kaKnob.BackgroundColor3=Color3.fromRGB(180,160,220); kaKnob.BorderSizePixel=0
...
local kaMiniTrack=Instance.new("Frame",kaSliderRow); kaMiniTrack.BackgroundColor3=Color3.fromRGB(55,22,88)
...
local kaMiniDot=Instance.new("Frame",kaMiniTrack); kaMiniDot.BackgroundColor3=Color3.fromRGB(55,22,88)
...
kaBtnClick.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(kaPill,TweenInfo.new(0.22),{BackgroundColor3=kaEnabled and KA_COR or Color3.fromRGB(55,22,88)}):Play()
    TweenService:Create(kaKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        ...
        BackgroundColor3=kaEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220)
    }):Play()
...
local function stopAutoExplore(tpBack)
    ...
    pcall(function()
        if aePill   then TweenService:Create(aePill,  TweenInfo.new(0.22), {BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end
        if aeKnob   then TweenService:Create(aeKnob,  TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                            {Position=UDim2.new(0,2,0.5,-11), BackgroundColor3=Color3.fromRGB(180,160,220)}):Play() end
...
-- Toggle pill
aePill = Instance.new("Frame",aeCard); aePill.BackgroundColor3=Color3.fromRGB(55,22,88)
...
aeKnob = Instance.new("Frame",aePill); aeKnob.BackgroundColor3=Color3.fromRGB(180,160,220)
...
aeBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(aePill,TweenInfo.new(0.22),{BackgroundColor3=aeEnabled and AE_COR or Color3.fromRGB(55,22,88)}):Play()
...
local fzPill=Instance.new("Frame",freezeCard); fzPill.BackgroundColor3=Color3.fromRGB(55,22,88); fzPill.BorderSizePixel=0
...
local fzKnob=Instance.new("Frame",fzPill); fzKnob.BackgroundColor3=Color3.fromRGB(180,160,220); fzKnob.BorderSizePixel=0
...
fzBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(fzPill,TweenInfo.new(0.22),{BackgroundColor3=freezeEnabled and FREEZE_COR or Color3.fromRGB(55,22,88)}):Play()
    TweenService:Create(fzKnob,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        ...
        BackgroundColor3=freezeEnabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,160,220)
    }):Play()
...
-- Botão MINUS
local fzMinus=Instance.new("TextButton",fzRadiusCard); fzMinus.BackgroundColor3=Color3.fromRGB(55,22,88)
...
-- ── UI: Player Tab ────────────────────────────────────────────────
-- Seção
local eatSec=Instance.new("Frame",Pages["Player"]); eatSec.BackgroundColor3=Color3.fromRGB(55,22,88); eatSec.BackgroundTransparency=0; eatSec.BorderSizePixel=0
...
-- Card
local eatCard=Instance.new("Frame",Pages["Player"]); eatCard.BackgroundColor3=Color3.fromRGB(55,22,88); eatCard.BorderSizePixel=0
...
local slTrack=Instance.new("Frame",slRow); slTrack.BackgroundColor3=Color3.fromRGB(55,22,88); slTrack.BorderSizePixel=0
...
local slThumb=Instance.new("TextButton",slTrack); slThumb.BackgroundColor3=Color3.fromRGB(55,22,88); slThumb.BorderSizePixel=0
...
-- ══════════════════════════════════════════════════════════════
-- UI: Seção header + Card (estilo Fogueira Automática)
-- ══════════════════════════════════════════════════════════════
-- ── Seção Header ────────────────────────────────────────────
local afcSec = Instance.new("Frame", Pages["Farm"])
afcSec.BackgroundColor3 = Color3.fromRGB(28, 14, 4); afcSec.BorderSizePixel = 0
afcSec.Size = UDim2.new(1, 0, 0, 32); afcSec.LayoutOrder = fNextLO(); afcSec.ZIndex = 4
Instance.new("UICorner", afcSec).CornerRadius = UDim.new(0, 10)
local afcSecG = Instance.new("UIGradient", afcSec)
afcSecG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(54, 26, 4)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(14, 8, 2))
}); afcSecG.Rotation = 90
local afcSecS = Instance.new("UIStroke", afcSec)
afcSecS.Color = AFC_COR; afcSecS.Thickness = 1.5; afcSecS.Transparency = 0.7
afcSecS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local afcPill = Instance.new("Frame", afcSec); afcPill.BackgroundColor3 = AFC_COR
afcPill.BorderSizePixel = 0; afcPill.Position = UDim2.new(0, 8, 0.5, -10)
afcPill.Size = UDim2.new(0, 4, 0, 20); afcPill.ZIndex = 5
Instance.new("UICorner", afcPill).CornerRadius = UDim.new(1, 0)
local afcSecIco = Instance.new("TextLabel", afcSec); afcSecIco.BackgroundTransparency = 1
afcSecIco.Position = UDim2.new(0, 18, 0, 0); afcSecIco.Size = UDim2.new(0, 22, 1, 0)
afcSecIco.Font = Enum.Font.GothamBold; afcSecIco.Text = "🔥"; afcSecIco.TextSize = 14; afcSecIco.ZIndex = 6
local afcSecLbl = Instance.new("TextLabel", afcSec); afcSecLbl.BackgroundTransparency = 1
afcSecLbl.Position = UDim2.new(0, 40, 0, 0); afcSecLbl.Size = UDim2.new(1, -50, 1, 0)
afcSecLbl.Font = Enum.Font.GothamBlack; afcSecLbl.Text = "AUTO FEED CAMPFIRE (SEMPRE)"
afcSecLbl.TextColor3 = Color3.fromRGB(255, 228, 196); afcSecLbl.TextSize = 11
afcSecLbl.TextXAlignment = Enum.TextXAlignment.Left; afcSecLbl.ZIndex = 5
Instance.new("UIStroke", afcSecLbl).Color = Color3.fromRGB(0, 0, 0)

-- ── Card principal ───────────────────────────────────────────
local afcCard = Instance.new("Frame", Pages["Farm"])
afcCard.BackgroundColor3 = Color3.fromRGB(26, 12, 4); afcCard.BorderSizePixel = 0
...
afcCardG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(34, 14, 4)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(16, 8, 2))
}); afcCardG.Rotation = 140
local afcCardS = Instance.new("UIStroke", afcCard)
afcCardS.Color = AFC_COR; afcCardS.Thickness = 1.5; afcCardS.Transparency = 0.7
afcCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
afcRef.stroke = afcCardS

local function afcDiv(y)
    local d = Instance.new("Frame", afcCard); d.BackgroundColor3 = Color3.fromRGB(60, 30, 10)
    d.BorderSizePixel = 0; d.Position = UDim2.new(0, 0, 0, y)
    d.Size = UDim2.new(1, 0, 0, 1); d.ZIndex = 6
end

-- ROW 1: Tipo de Combustível + dropdown
local afcRow1 = Instance.new("Frame", afcCard); afcRow1.BackgroundTransparency = 1
afcRow1.BorderSizePixel = 0; afcRow1.Position = UDim2.new(0, 0, 0, 0)
afcRow1.Size = UDim2.new(1, 0, 0, 54); afcRow1.ZIndex = 6

local afcLbl1 = Instance.new("TextLabel", afcRow1); afcLbl1.BackgroundTransparency = 1
afcLbl1.Position = UDim2.new(0, 14, 0, 8); afcLbl1.Size = UDim2.new(0.45, 0, 0, 38)
afcLbl1.Font = Enum.Font.GothamBold; afcLbl1.Text = "Tipo de\nCombustível"
afcLbl1.TextColor3 = Color3.fromRGB(255, 226, 192); afcLbl1.TextSize = 12
afcLbl1.TextXAlignment = Enum.TextXAlignment.Left; afcLbl1.TextWrapped = true; afcLbl1.ZIndex = 7

local afcSelBtn = Instance.new("TextButton", afcRow1)
afcSelBtn.BackgroundColor3 = Color3.fromRGB(55,22,88); afcSelBtn.BorderSizePixel = 0
afcSelBtn.AnchorPoint = Vector2.new(1, 0.5); afcSelBtn.Position = UDim2.new(1, -10, 0.5, 0)
afcSelBtn.Size = UDim2.new(0, 152, 0, 30); afcSelBtn.Font = Enum.Font.GothamBold
afcSelBtn.Text = "Selecionar Combustível ▾"
afcSelBtn.TextColor3 = Color3.fromRGB(255, 200, 120); afcSelBtn.TextSize = 9
afcSelBtn.ZIndex = 8; afcSelBtn.AutoButtonColor = false
Instance.new("UICorner", afcSelBtn).CornerRadius = UDim.new(0, 8)
local afcSelBtnS = Instance.new("UIStroke", afcSelBtn)
afcSelBtnS.Color = AFC_COR; afcSelBtnS.Thickness = 1.5; afcSelBtnS.Transparency = 0.4
afcSelBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
afcRef.selBtn = afcSelBtn; afcRef.selBtnS = afcSelBtnS

afcDiv(54)

-- ROW 2: Status + botão ATIVAR
local afcRow2 = Instance.new("Frame", afcCard); afcRow2.BackgroundTransparency = 1
afcRow2.BorderSizePixel = 0; afcRow2.Position = UDim2.new(0, 0, 0, 56)
afcRow2.Size = UDim2.new(1, 0, 0, 54); afcRow2.ZIndex = 6

local afcStatus = Instance.new("TextLabel", afcRow2); afcStatus.BackgroundTransparency = 1
afcStatus.Position = UDim2.new(0, 14, 0, 8); afcStatus.Size = UDim2.new(0.52, 0, 0, 38)
afcStatus.Font = Enum.Font.GothamBold; afcStatus.Text = "Inativo"
afcStatus.TextColor3 = Color3.fromRGB(255, 170, 80); afcStatus.TextSize = 10
afcStatus.TextXAlignment = Enum.TextXAlignment.Left; afcStatus.TextWrapped = true; afcStatus.ZIndex = 7
afcRef.status = afcStatus

local afcBtn = Instance.new("TextButton", afcRow2)
afcBtn.BackgroundColor3 = AFC_COR; afcBtn.BorderSizePixel = 0
afcBtn.AnchorPoint = Vector2.new(1, 0.5); afcBtn.Position = UDim2.new(1, -12, 0.5, 0)
afcBtn.Size = UDim2.new(0, 110, 0, 34); afcBtn.Font = Enum.Font.GothamBlack
afcBtn.Text = "🔥  ATIVAR"; afcBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
afcBtn.TextSize = 11; afcBtn.ZIndex = 8; afcBtn.AutoButtonColor = false
...
local afcBtnShine = Instance.new("Frame", afcBtn)
afcBtnShine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); afcBtnShine.BackgroundTransparency = 0.65
afcBtnShine.BorderSizePixel = 0; afcBtnShine.Position = UDim2.new(0, 6, 0, 4)
afcBtnShine.Size = UDim2.new(0, 45, 0, 7); afcBtnShine.ZIndex = 9
Instance.new("UICorner", afcBtnShine).CornerRadius = UDim.new(1, 0)
afcRef.btn = afcBtn; afcRef.btnG = afcBtnG

afcBtn.MouseEnter:Connect(function() TweenService:Create(afcBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0}):Play() end)
afcBtn.MouseLeave:Connect(function() TweenService:Create(afcBtn, TweenInfo.new(0.12), {BackgroundTransparency = 0.2}):Play() end)
...
-- Toggle helper
local function afcDoToggle()
    if afcRunning then
        afcRunning = false
        TweenService:Create(afcSecS,TweenInfo.new(0.2),{Transparency=0.7}):Play()
        TweenService:Create(afcCardS,TweenInfo.new(0.2),{Transparency=0.7}):Play()
        ...
        task.delay(1.5,function() pcall(function() afcStatus.Text="Inativo" end) end)
    else
        ...
        TweenService:Create(afcSecS,TweenInfo.new(0.2),{Transparency=0.2}):Play()
        TweenService:Create(afcCardS,TweenInfo.new(0.2),{Transparency=0.2}):Play()
...
-- Header toggle
local afcSecTogBtn = Instance.new("TextButton", afcSec)
afcSecTogBtn.BackgroundTransparency = 1; afcSecTogBtn.Size = UDim2.new(1, 0, 1, 0)
afcSecTogBtn.Text = ""; afcSecTogBtn.ZIndex = 8
afcSecTogBtn.MouseButton1Click:Connect(afcDoToggle)
...
-- ── Seção Header (estilo Fogueira Automática) ───────────────
local afhpSec = Instance.new("Frame", Pages["Farm"])
afhpSec.BackgroundColor3 = Color3.fromRGB(28, 10, 14); afhpSec.BorderSizePixel = 0
...
afhpSecG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(56, 14, 24)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(14, 6, 10))
}); afhpSecG.Rotation = 90
local afhpSecS = Instance.new("UIStroke", afhpSec)
afhpSecS.Color = AFHP_COR; afhpSecS.Thickness = 1.5; afhpSecS.Transparency = 0.7
afhpSecS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local afhpPill = Instance.new("Frame", afhpSec); afhpPill.BackgroundColor3 = AFHP_COR
afhpPill.BorderSizePixel = 0; afhpPill.Position = UDim2.new(0, 8, 0.5, -10)
afhpPill.Size = UDim2.new(0, 4, 0, 20); afhpPill.ZIndex = 5
Instance.new("UICorner", afhpPill).CornerRadius = UDim.new(1, 0)
local afhpSecIco = Instance.new("TextLabel", afhpSec); afhpSecIco.BackgroundTransparency = 1
afhpSecIco.Position = UDim2.new(0, 18, 0, 0); afhpSecIco.Size = UDim2.new(0, 22, 1, 0)
afhpSecIco.Font = Enum.Font.GothamBold; afhpSecIco.Text = "🔥"; afhpSecIco.TextSize = 14; afhpSecIco.ZIndex = 6
local afhpSecLbl = Instance.new("TextLabel", afhpSec); afhpSecLbl.BackgroundTransparency = 1
afhpSecLbl.Position = UDim2.new(0, 40, 0, 0); afhpSecLbl.Size = UDim2.new(1, -110, 1, 0)
afhpSecLbl.Font = Enum.Font.GothamBlack; afhpSecLbl.Text = "AUTO FEED CAMPFIRE (HP BASED)"
afhpSecLbl.TextColor3 = Color3.fromRGB(255, 210, 220); afhpSecLbl.TextSize = 11
...
afhpBadge.BackgroundColor3 = Color3.fromRGB(80, 15, 30)
afhpBadge.AnchorPoint = Vector2.new(1, 0.5); afhpBadge.Position = UDim2.new(1, -8, 0.5, 0)
afhpBadge.Size = UDim2.new(0, 46, 0, 20); afhpBadge.ZIndex = 6
Instance.new("UICorner", afhpBadge).CornerRadius = UDim.new(1, 0)
local afhpBadgeS = Instance.new("UIStroke", afhpBadge)
afhpBadgeS.Color = AFHP_COR; afhpBadgeS.Thickness = 1.5; afhpBadgeS.Transparency = 0.3
afhpBadgeS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
-- ── Card ─────────────────────────────────────────────────────
local afhpCard = Instance.new("Frame", Pages["Farm"])
afhpCard.BackgroundColor3 = Color3.fromRGB(28, 10, 14); afhpCard.BorderSizePixel = 0
afhpCard.Size = UDim2.new(1, 0, 0, 170); afhpCard.LayoutOrder = fNextLO(); afhpCard.ZIndex = 5
Instance.new("UICorner", afhpCard).CornerRadius = UDim.new(0, 12)
local afhpCardG = Instance.new("UIGradient", afhpCard)
afhpCardG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(36, 10, 18)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(16, 6, 10))
}); afhpCardG.Rotation = 140
local afhpCardS = Instance.new("UIStroke", afhpCard)
afhpCardS.Color = AFHP_COR; afhpCardS.Thickness = 1.5; afhpCardS.Transparency = 0.7
afhpCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
afhpRef.stroke = afhpCardS

local function afhpDiv(y)
    local d = Instance.new("Frame", afhpCard); d.BackgroundColor3 = Color3.fromRGB(60, 20, 30)
    d.BorderSizePixel = 0; d.Position = UDim2.new(0, 0, 0, y)
    d.Size = UDim2.new(1, 0, 0, 1); d.ZIndex = 6
end

-- ROW 1: Slider de HP threshold (54px)
local afhpRow1 = Instance.new("Frame", afhpCard); afhpRow1.BackgroundTransparency = 1
afhpRow1.BorderSizePixel = 0; afhpRow1.Position = UDim2.new(0, 0, 0, 0)
afhpRow1.Size = UDim2.new(1, 0, 0, 54); afhpRow1.ZIndex = 6

local afhpLbl1 = Instance.new("TextLabel", afhpRow1); afhpLbl1.BackgroundTransparency = 1
afhpLbl1.Position = UDim2.new(0, 14, 0, 4); afhpLbl1.Size = UDim2.new(0.52, 0, 0, 46)
afhpLbl1.Font = Enum.Font.GothamBold; afhpLbl1.TextWrapped = true
afhpLbl1.TextColor3 = Color3.fromRGB(255, 210, 218); afhpLbl1.TextSize = 12
afhpLbl1.TextXAlignment = Enum.TextXAlignment.Left; afhpLbl1.ZIndex = 7
local function afhpUpdateLbl()
    afhpLbl1.Text = "Alimentar quando\nHP < "..tostring(afhpThreshold).."%"
end
afhpUpdateLbl()

local afhpTrack = Instance.new("Frame", afhpRow1)
afhpTrack.BackgroundColor3 = Color3.fromRGB(55,22,88); afhpTrack.BorderSizePixel = 0
afhpTrack.Position = UDim2.new(0.54, 0, 0.5, -2); afhpTrack.Size = UDim2.new(0.38, 0, 0, 4)
afhpTrack.ZIndex = 7
Instance.new("UICorner", afhpTrack).CornerRadius = UDim.new(1, 0)
local afhpFill = Instance.new("Frame", afhpTrack)
afhpFill.BackgroundColor3 = AFHP_COR; afhpFill.BorderSizePixel = 0
afhpFill.Size = UDim2.new(afhpThreshold/100, 0, 1, 0); afhpFill.ZIndex = 8
Instance.new("UICorner", afhpFill).CornerRadius = UDim.new(1, 0)
local afhpThumb = Instance.new("Frame", afhpTrack)
afhpThumb.Size = UDim2.new(0, 18, 0, 18); afhpThumb.BackgroundColor3 = Color3.fromRGB(180,130,230)
afhpThumb.BorderSizePixel = 0; afhpThumb.ZIndex = 9
afhpThumb.AnchorPoint = Vector2.new(0.5, 0.5)
afhpThumb.Position = UDim2.new(afhpThreshold/100, 0, 0.5, 0)
Instance.new("UICorner", afhpThumb).CornerRadius = UDim.new(1, 0)
local afhpDragBtn = Instance.new("TextButton", afhpRow1)
afhpDragBtn.BackgroundTransparency = 1; afhpDragBtn.Size = UDim2.new(1, 0, 1, 0)
afhpDragBtn.Text = ""; afhpDragBtn.ZIndex = 12
local afhpDragging = false
afhpDragBtn.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then afhpDragging = true end
end)
UserInputService.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then afhpDragging = false end
end)
UserInputService.InputChanged:Connect(function(inp)
    if afhpDragging and inp.UserInputType == Enum.UserInputType.MouseMovement then
        pcall(function()
            local tp = afhpTrack.AbsolutePosition.X
            local tsz = afhpTrack.AbsoluteSize.X
            local pct = math.clamp((inp.Position.X - tp) / tsz, 0, 1)
            afhpThreshold = math.floor(pct * 100)
            afhpFill.Size = UDim2.new(pct, 0, 1, 0)
            afhpThumb.Position = UDim2.new(pct, 0, 0.5, 0)
            afhpUpdateLbl()
            afhpBadgeLbl.Text = tostring(afhpThreshold).."%"
        end)
    end
end)
...
-- ROW 2: Tipo de Combustível + dropdown (shared with AFC popup)
local afhpRow2 = Instance.new("Frame", afhpCard); afhpRow2.BackgroundTransparency = 1
afhpRow2.BorderSizePixel = 0; afhpRow2.Position = UDim2.new(0, 0, 0, 56)
afhpRow2.Size = UDim2.new(1, 0, 0, 54); afhpRow2.ZIndex = 6

local afhpLbl2 = Instance.new("TextLabel", afhpRow2); afhpLbl2.BackgroundTransparency = 1
afhpLbl2.Position = UDim2.new(0, 14, 0, 8); afhpLbl2.Size = UDim2.new(0.45, 0, 0, 38)
afhpLbl2.Font = Enum.Font.GothamBold; afhpLbl2.Text = "Tipo de\nCombustível"
afhpLbl2.TextColor3 = Color3.fromRGB(255, 210, 218); afhpLbl2.TextSize = 12
afhpLbl2.TextXAlignment = Enum.TextXAlignment.Left; afhpLbl2.TextWrapped = true; afhpLbl2.ZIndex = 7

local afhpSelBtn = Instance.new("TextButton", afhpRow2)
afhpSelBtn.BackgroundColor3 = Color3.fromRGB(55,22,88); afhpSelBtn.BorderSizePixel = 0
afhpSelBtn.AnchorPoint = Vector2.new(1, 0.5); afhpSelBtn.Position = UDim2.new(1, -10, 0.5, 0)
afhpSelBtn.Size = UDim2.new(0, 152, 0, 30); afhpSelBtn.Font = Enum.Font.GothamBold
afhpSelBtn.Text = "Selecionar Combustível ▾"
afhpSelBtn.TextColor3 = Color3.fromRGB(255, 200, 120); afhpSelBtn.TextSize = 9
afhpSelBtn.ZIndex = 8; afhpSelBtn.AutoButtonColor = false
Instance.new("UICorner", afhpSelBtn).CornerRadius = UDim.new(0, 8)
local afhpSelBtnS = Instance.new("UIStroke", afhpSelBtn)
afhpSelBtnS.Color = AFHP_COR; afhpSelBtnS.Thickness = 1.5; afhpSelBtnS.Transparency = 0.4
afhpSelBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
afcRef.hpSelBtn = afhpSelBtn
...
-- ROW 3: Status + botão ATIVAR
local afhpRow3 = Instance.new("Frame", afhpCard); afhpRow3.BackgroundTransparency = 1
afhpRow3.BorderSizePixel = 0; afhpRow3.Position = UDim2.new(0, 0, 0, 112)
afhpRow3.Size = UDim2.new(1, 0, 0, 58); afhpRow3.ZIndex = 6

local afhpStatusLbl = Instance.new("TextLabel", afhpRow3); afhpStatusLbl.BackgroundTransparency = 1
afhpStatusLbl.Position = UDim2.new(0, 14, 0, 8); afhpStatusLbl.Size = UDim2.new(0.52, 0, 0, 42)
afhpStatusLbl.Font = Enum.Font.GothamBold; afhpStatusLbl.Text = "Inativo"
afhpStatusLbl.TextColor3 = Color3.fromRGB(255, 150, 170); afhpStatusLbl.TextSize = 10
afhpStatusLbl.TextXAlignment = Enum.TextXAlignment.Left; afhpStatusLbl.TextWrapped = true; afhpStatusLbl.ZIndex = 7
afhpRef.status = afhpStatusLbl

local afhpBtn = Instance.new("TextButton", afhpRow3)
afhpBtn.BackgroundColor3 = AFHP_COR; afhpBtn.BorderSizePixel = 0
afhpBtn.AnchorPoint = Vector2.new(1, 0.5); afhpBtn.Position = UDim2.new(1, -12, 0.5, 0)
afhpBtn.Size = UDim2.new(0, 110, 0, 34); afhpBtn.Font = Enum.Font.GothamBlack
afhpBtn.Text = "🔥  ATIVAR"; afhpBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
afhpBtn.TextSize = 11; afhpBtn.ZIndex = 8; afhpBtn.AutoButtonColor = false
Instance.new("UICorner", afhpBtn).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", afhpBtn).Color = Color3.fromRGB(8, 4, 20)
local afhpBtnG = Instance.new("UIGradient", afhpBtn); afhpBtnG.Rotation = 90
btnGrad(afhpBtnG, Color3.fromRGB(255, 100, 140), Color3.fromRGB(200, 40, 80))
local afhpBtnShine = Instance.new("Frame", afhpBtn)
afhpBtnShine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); afhpBtnShine.BackgroundTransparency = 0.65
afhpBtnShine.BorderSizePixel = 0; afhpBtnShine.Position = UDim2.new(0, 6, 0, 4)
afhpBtnShine.Size = UDim2.new(0, 45, 0, 7); afhpBtnShine.ZIndex = 9
Instance.new("UICorner", afhpBtnShine).CornerRadius = UDim.new(1, 0)
afhpRef.btn = afhpBtn; afhpRef.btnG = afhpBtnG

afhpBtn.MouseEnter:Connect(function() TweenService:Create(afhpBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0}):Play() end)
afhpBtn.MouseLeave:Connect(function() TweenService:Create(afhpBtn, TweenInfo.new(0.12), {BackgroundTransparency = 0.2}):Play() end)

-- Toggle helper
local function afhpDoToggle()
    if afhpRunning then
        afhpRunning = false
        TweenService:Create(afhpSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        TweenService:Create(afhpCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        afhpStatusLbl.Text = "⏹ Parado"; Notify.error("Auto Feed HP","⏹ Desativado")
        afhpBtn.Text="🔥  ATIVAR"; btnGrad(afhpBtnG,Color3.fromRGB(255,100,140),Color3.fromRGB(200,40,80))
        task.delay(1.5, function() pcall(function() afhpStatusLbl.Text="Inativo" end) end)
    else
        TweenService:Create(afhpSecS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        TweenService:Create(afhpCardS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        afhpBtn.Text="⏹  PARAR"; btnGrad(afhpBtnG,Color3.fromRGB(220,60,60),Color3.fromRGB(140,20,20))
        task.spawn(function()
            autoFeedHP()
            pcall(function()
                TweenService:Create(afhpSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                TweenService:Create(afhpCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                afhpBtn.Text="🔥  ATIVAR"; btnGrad(afhpBtnG,Color3.fromRGB(255,100,140),Color3.fromRGB(200,40,80))
                afhpStatusLbl.Text="Inativo"
            end)
        end)
    end
end

local afhpSecTogBtn = Instance.new("TextButton", afhpSec)
afhpSecTogBtn.BackgroundTransparency = 1; afhpSecTogBtn.Size = UDim2.new(1, 0, 1, 0)
afhpSecTogBtn.Text = ""; afhpSecTogBtn.ZIndex = 8
afhpSecTogBtn.MouseButton1Click:Connect(afhpDoToggle)
afhpBtn.MouseButton1Click:Connect(afhpDoToggle)
...
-- ── Seção Header ────────────────────────────────────────────
local ackSec = Instance.new("Frame", Pages["Farm"])
ackSec.BackgroundColor3 = Color3.fromRGB(8, 24, 10); ackSec.BorderSizePixel = 0
ackSec.Size = UDim2.new(1, 0, 0, 32); ackSec.LayoutOrder = fNextLO(); ackSec.ZIndex = 4
Instance.new("UICorner", ackSec).CornerRadius = UDim.new(0, 10)
local ackSecG = Instance.new("UIGradient", ackSec)
ackSecG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(18, 50, 22)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(6, 18, 8))
}); ackSecG.Rotation = 90
local ackSecS = Instance.new("UIStroke", ackSec)
ackSecS.Color = ACK_COR; ackSecS.Thickness = 1.5; ackSecS.Transparency = 0.7
ackSecS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local ackPill = Instance.new("Frame", ackSec); ackPill.BackgroundColor3 = ACK_COR
ackPill.BorderSizePixel = 0; ackPill.Position = UDim2.new(0, 8, 0.5, -10)
ackPill.Size = UDim2.new(0, 4, 0, 20); ackPill.ZIndex = 5
Instance.new("UICorner", ackPill).CornerRadius = UDim.new(1, 0)
local ackSecIco = Instance.new("TextLabel", ackSec); ackSecIco.BackgroundTransparency = 1
ackSecIco.Position = UDim2.new(0, 18, 0, 0); ackSecIco.Size = UDim2.new(0, 22, 1, 0)
ackSecIco.Font = Enum.Font.GothamBold; ackSecIco.Text = "🍖"; ackSecIco.TextSize = 14; ackSecIco.ZIndex = 6
local ackSecLbl = Instance.new("TextLabel", ackSec); ackSecLbl.BackgroundTransparency = 1
ackSecLbl.Position = UDim2.new(0, 40, 0, 0); ackSecLbl.Size = UDim2.new(1, -50, 1, 0)
ackSecLbl.Font = Enum.Font.GothamBlack; ackSecLbl.Text = "AUTO COOK FOOD"
ackSecLbl.TextColor3 = Color3.fromRGB(210, 255, 210); ackSecLbl.TextSize = 11
ackSecLbl.TextXAlignment = Enum.TextXAlignment.Left; ackSecLbl.ZIndex = 5
Instance.new("UIStroke", ackSecLbl).Color = Color3.fromRGB(0, 0, 0)

-- ── Card ─────────────────────────────────────────────────────
local ackCard = Instance.new("Frame", Pages["Farm"])
ackCard.BackgroundColor3 = Color3.fromRGB(8, 22, 10); ackCard.BorderSizePixel = 0
ackCard.Size = UDim2.new(1, 0, 0, 110); ackCard.LayoutOrder = fNextLO(); ackCard.ZIndex = 5
Instance.new("UICorner", ackCard).CornerRadius = UDim.new(0, 12)
local ackCardG = Instance.new("UIGradient", ackCard)
ackCardG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(10, 28, 12)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(4, 14, 6))
}); ackCardG.Rotation = 140
local ackCardS = Instance.new("UIStroke", ackCard)
ackCardS.Color = ACK_COR; ackCardS.Thickness = 1.5; ackCardS.Transparency = 0.7
ackCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
ackRef.stroke = ackCardS

local function ackDiv(y)
    local d = Instance.new("Frame", ackCard); d.BackgroundColor3 = Color3.fromRGB(24, 50, 26)
    d.BorderSizePixel = 0; d.Position = UDim2.new(0, 0, 0, y)
    d.Size = UDim2.new(1, 0, 0, 1); d.ZIndex = 6
end

-- ROW 1: Descrição + ícone
local ackRow1 = Instance.new("Frame", ackCard); ackRow1.BackgroundTransparency = 1
ackRow1.BorderSizePixel = 0; ackRow1.Position = UDim2.new(0, 0, 0, 0)
ackRow1.Size = UDim2.new(1, 0, 0, 56); ackRow1.ZIndex = 6

local ackLbl1 = Instance.new("TextLabel", ackRow1); ackLbl1.BackgroundTransparency = 1
ackLbl1.Position = UDim2.new(0, 14, 0, 8); ackLbl1.Size = UDim2.new(0.6, 0, 0, 40)
ackLbl1.Font = Enum.Font.GothamBold; ackLbl1.Text = "Cozinha carnes cruas\nna fogueira automaticamente"
ackLbl1.TextColor3 = Color3.fromRGB(200, 255, 200); ackLbl1.TextSize = 11
ackLbl1.TextXAlignment = Enum.TextXAlignment.Left; ackLbl1.TextWrapped = true; ackLbl1.ZIndex = 7

local ackIB = Instance.new("Frame", ackRow1); ackIB.BackgroundColor3 = ACK_COR
ackIB.BackgroundTransparency = 0.3; ackIB.BorderSizePixel = 0
ackIB.AnchorPoint = Vector2.new(1, 0.5); ackIB.Position = UDim2.new(1, -12, 0.5, 0)
ackIB.Size = UDim2.new(0, 38, 0, 38); ackIB.ZIndex = 7
Instance.new("UICorner", ackIB).CornerRadius = UDim.new(1, 0)
local ackIL = Instance.new("TextLabel", ackIB); ackIL.BackgroundTransparency = 1
ackIL.Size = UDim2.new(1, 0, 1, 0); ackIL.Text = "🍖"; ackIL.TextSize = 20
ackIL.Font = Enum.Font.GothamBold; ackIL.ZIndex = 8

ackDiv(56)

-- ROW 2: Status + botão
local ackRow2 = Instance.new("Frame", ackCard); ackRow2.BackgroundTransparency = 1
ackRow2.BorderSizePixel = 0; ackRow2.Position = UDim2.new(0, 0, 0, 58)
ackRow2.Size = UDim2.new(1, 0, 0, 52); ackRow2.ZIndex = 6

local ackStatus = Instance.new("TextLabel", ackRow2); ackStatus.BackgroundTransparency = 1
ackStatus.Position = UDim2.new(0, 14, 0, 8); ackStatus.Size = UDim2.new(0.52, 0, 0, 36)
ackStatus.Font = Enum.Font.GothamBold; ackStatus.Text = "Inativo"
ackStatus.TextColor3 = Color3.fromRGB(120, 220, 130); ackStatus.TextSize = 10
ackStatus.TextXAlignment = Enum.TextXAlignment.Left; ackStatus.TextWrapped = true; ackStatus.ZIndex = 7
ackRef.status = ackStatus

local ackBtn = Instance.new("TextButton", ackRow2)
ackBtn.BackgroundColor3 = ACK_COR; ackBtn.BorderSizePixel = 0
ackBtn.AnchorPoint = Vector2.new(1, 0.5); ackBtn.Position = UDim2.new(1, -12, 0.5, 0)
ackBtn.Size = UDim2.new(0, 110, 0, 34); ackBtn.Font = Enum.Font.GothamBlack
ackBtn.Text = "🍖  ATIVAR"; ackBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
ackBtn.TextSize = 11; ackBtn.ZIndex = 8; ackBtn.AutoButtonColor = false
Instance.new("UICorner", ackBtn).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", ackBtn).Color = Color3.fromRGB(8, 4, 20)
local ackBtnG = Instance.new("UIGradient", ackBtn); ackBtnG.Rotation = 90
btnGrad(ackBtnG, Color3.fromRGB(80, 220, 80), Color3.fromRGB(30, 140, 30))
local ackBtnShine = Instance.new("Frame", ackBtn)
ackBtnShine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); ackBtnShine.BackgroundTransparency = 0.65
ackBtnShine.BorderSizePixel = 0; ackBtnShine.Position = UDim2.new(0, 6, 0, 4)
ackBtnShine.Size = UDim2.new(0, 45, 0, 7); ackBtnShine.ZIndex = 9
Instance.new("UICorner", ackBtnShine).CornerRadius = UDim.new(1, 0)
ackRef.btn = ackBtn; ackRef.btnG = ackBtnG

ackBtn.MouseEnter:Connect(function() TweenService:Create(ackBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0}):Play() end)
ackBtn.MouseLeave:Connect(function() TweenService:Create(ackBtn, TweenInfo.new(0.12), {BackgroundTransparency = 0.2}):Play() end)

local function ackDoToggle()
    if ackRunning then
        ackRunning = false
        TweenService:Create(ackSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        TweenService:Create(ackCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        ackStatus.Text = "⏹ Parado"; Notify.error("Auto Cook Food","⏹ Desativado")
        ackBtn.Text = "🍖  ATIVAR"; btnGrad(ackBtnG, Color3.fromRGB(80,220,80), Color3.fromRGB(30,140,30))
        task.delay(1.5, function() pcall(function() ackStatus.Text="Inativo" end) end)
    else
        TweenService:Create(ackSecS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        TweenService:Create(ackCardS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        ackBtn.Text = "⏹  PARAR"; btnGrad(ackBtnG, Color3.fromRGB(220,60,60), Color3.fromRGB(140,20,20))
        task.spawn(function()
            autoCookFood()
            pcall(function()
                TweenService:Create(ackSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                TweenService:Create(ackCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                ackBtn.Text = "🍖  ATIVAR"; btnGrad(ackBtnG, Color3.fromRGB(80,220,80), Color3.fromRGB(30,140,30))
                ackStatus.Text = "Inativo"
            end)
        end)
    end
end

local ackSecTogBtn = Instance.new("TextButton", ackSec)
ackSecTogBtn.BackgroundTransparency = 1; ackSecTogBtn.Size = UDim2.new(1, 0, 1, 0)
ackSecTogBtn.Text = ""; ackSecTogBtn.ZIndex = 8
ackSecTogBtn.MouseButton1Click:Connect(ackDoToggle)
ackBtn.MouseButton1Click:Connect(ackDoToggle)
...
-- ══════════════════════════════════════════════════════════════
-- ── Seção Header ────────────────────────────────────────────
local amgSec = Instance.new("Frame", Pages["Farm"])
amgSec.BackgroundColor3 = Color3.fromRGB(8, 18, 32); amgSec.BorderSizePixel = 0
amgSec.Size = UDim2.new(1, 0, 0, 32); amgSec.LayoutOrder = fNextLO(); amgSec.ZIndex = 4
Instance.new("UICorner", amgSec).CornerRadius = UDim.new(0, 10)
local amgSecG = Instance.new("UIGradient", amgSec)
amgSecG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(16, 36, 66)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(6, 12, 24))
}); amgSecG.Rotation = 90
local amgSecS = Instance.new("UIStroke", amgSec)
amgSecS.Color = AMG_COR; amgSecS.Thickness = 1.5; amgSecS.Transparency = 0.7
amgSecS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local amgPill = Instance.new("Frame", amgSec); amgPill.BackgroundColor3 = AMG_COR
amgPill.BorderSizePixel = 0; amgPill.Position = UDim2.new(0, 8, 0.5, -10)
amgPill.Size = UDim2.new(0, 4, 0, 20); amgPill.ZIndex = 5
Instance.new("UICorner", amgPill).CornerRadius = UDim.new(1, 0)
local amgSecIco = Instance.new("TextLabel", amgSec); amgSecIco.BackgroundTransparency = 1
amgSecIco.Position = UDim2.new(0, 18, 0, 0); amgSecIco.Size = UDim2.new(0, 22, 1, 0)
amgSecIco.Font = Enum.Font.GothamBold; amgSecIco.Text = "⚙️"; amgSecIco.TextSize = 14; amgSecIco.ZIndex = 6
local amgSecLbl = Instance.new("TextLabel", amgSec); amgSecLbl.BackgroundTransparency = 1
amgSecLbl.Position = UDim2.new(0, 40, 0, 0); amgSecLbl.Size = UDim2.new(1, -50, 1, 0)
amgSecLbl.Font = Enum.Font.GothamBlack; amgSecLbl.Text = "AUTO MACHINE GRIND"
amgSecLbl.TextColor3 = Color3.fromRGB(190, 220, 255); amgSecLbl.TextSize = 11
amgSecLbl.TextXAlignment = Enum.TextXAlignment.Left; amgSecLbl.ZIndex = 5
Instance.new("UIStroke", amgSecLbl).Color = Color3.fromRGB(0, 0, 0)

-- ── Card principal ───────────────────────────────────────────
local amgCard = Instance.new("Frame", Pages["Farm"])
amgCard.BackgroundColor3 = Color3.fromRGB(8, 18, 32); amgCard.BorderSizePixel = 0
...
amgCardG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(10, 20, 40)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(4, 8, 18))
}); amgCardG.Rotation = 140
local amgCardS = Instance.new("UIStroke", amgCard)
amgCardS.Color = AMG_COR; amgCardS.Thickness = 1.5; amgCardS.Transparency = 0.7
amgCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
amgRef.stroke = amgCardS

local function amgDiv(y)
    local d = Instance.new("Frame", amgCard); d.BackgroundColor3 = Color3.fromRGB(20, 36, 60)
    d.BorderSizePixel = 0; d.Position = UDim2.new(0, 0, 0, y)
    d.Size = UDim2.new(1, 0, 0, 1); d.ZIndex = 6
end

-- ROW 1: Seletor de Itens + dropdown
local amgRow1 = Instance.new("Frame", amgCard); amgRow1.BackgroundTransparency = 1
amgRow1.BorderSizePixel = 0; amgRow1.Position = UDim2.new(0, 0, 0, 0)
amgRow1.Size = UDim2.new(1, 0, 0, 54); amgRow1.ZIndex = 6

local amgLbl1 = Instance.new("TextLabel", amgRow1); amgLbl1.BackgroundTransparency = 1
amgLbl1.Position = UDim2.new(0, 14, 0, 8); amgLbl1.Size = UDim2.new(0.45, 0, 0, 38)
amgLbl1.Font = Enum.Font.GothamBold; amgLbl1.Text = "Itens para\nTriturar"
amgLbl1.TextColor3 = Color3.fromRGB(190, 215, 255); amgLbl1.TextSize = 12
amgLbl1.TextXAlignment = Enum.TextXAlignment.Left; amgLbl1.TextWrapped = true; amgLbl1.ZIndex = 7

local amgSelBtn = Instance.new("TextButton", amgRow1)
amgSelBtn.BackgroundColor3 = Color3.fromRGB(55,22,88); amgSelBtn.BorderSizePixel = 0
amgSelBtn.AnchorPoint = Vector2.new(1, 0.5); amgSelBtn.Position = UDim2.new(1, -10, 0.5, 0)
amgSelBtn.Size = UDim2.new(0, 152, 0, 30); amgSelBtn.Font = Enum.Font.GothamBold
amgSelBtn.Text = "Selecionar Itens ▾"
amgSelBtn.TextColor3 = Color3.fromRGB(180, 210, 255); amgSelBtn.TextSize = 10
amgSelBtn.ZIndex = 8; amgSelBtn.AutoButtonColor = false
Instance.new("UICorner", amgSelBtn).CornerRadius = UDim.new(0, 8)
local amgSelBtnS = Instance.new("UIStroke", amgSelBtn)
amgSelBtnS.Color = AMG_COR; amgSelBtnS.Thickness = 1.5; amgSelBtnS.Transparency = 0.4
amgSelBtnS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
amgRef.selBtn = amgSelBtn; amgRef.selBtnS = amgSelBtnS

amgDiv(54)

-- ROW 2: Status + botão ATIVAR
local amgRow2 = Instance.new("Frame", amgCard); amgRow2.BackgroundTransparency = 1
amgRow2.BorderSizePixel = 0; amgRow2.Position = UDim2.new(0, 0, 0, 56)
amgRow2.Size = UDim2.new(1, 0, 0, 54); amgRow2.ZIndex = 6

local amgStatus = Instance.new("TextLabel", amgRow2); amgStatus.BackgroundTransparency = 1
amgStatus.Position = UDim2.new(0, 14, 0, 8); amgStatus.Size = UDim2.new(0.52, 0, 0, 38)
amgStatus.Font = Enum.Font.GothamBold; amgStatus.Text = "Inativo"
amgStatus.TextColor3 = Color3.fromRGB(130, 185, 255); amgStatus.TextSize = 10
amgStatus.TextXAlignment = Enum.TextXAlignment.Left; amgStatus.TextWrapped = true; amgStatus.ZIndex = 7
amgRef.status = amgStatus

local amgBtn = Instance.new("TextButton", amgRow2)
amgBtn.BackgroundColor3 = AMG_COR; amgBtn.BorderSizePixel = 0
amgBtn.AnchorPoint = Vector2.new(1, 0.5); amgBtn.Position = UDim2.new(1, -12, 0.5, 0)
amgBtn.Size = UDim2.new(0, 110, 0, 34); amgBtn.Font = Enum.Font.GothamBlack
amgBtn.Text = "⚙️  ATIVAR"; amgBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
amgBtn.TextSize = 11; amgBtn.ZIndex = 8; amgBtn.AutoButtonColor = false
...
local amgBtnShine = Instance.new("Frame", amgBtn)
amgBtnShine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); amgBtnShine.BackgroundTransparency = 0.65
amgBtnShine.BorderSizePixel = 0; amgBtnShine.Position = UDim2.new(0, 6, 0, 4)
amgBtnShine.Size = UDim2.new(0, 45, 0, 7); amgBtnShine.ZIndex = 9
Instance.new("UICorner", amgBtnShine).CornerRadius = UDim.new(1, 0)
amgRef.btn = amgBtn; amgRef.btnG = amgBtnG

amgBtn.MouseEnter:Connect(function() TweenService:Create(amgBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0}):Play() end)
amgBtn.MouseLeave:Connect(function() TweenService:Create(amgBtn, TweenInfo.new(0.12), {BackgroundTransparency = 0.2}):Play() end)
...
local function amgMakeItemRow(item, lo)
    ...
    local cb=Instance.new("Frame",row); cb.BorderSizePixel=0
    cb.Position=UDim2.new(1,-22,0.5,-9); cb.Size=UDim2.new(0,18,0,18)
    cb.BackgroundColor3=Color3.fromRGB(55,22,88); cb.ZIndex=403
    Instance.new("UICorner",cb).CornerRadius=UDim.new(0,5)
    ...
    local function refreshCB()
        ...
        TweenService:Create(cb,TweenInfo.new(0.12),{BackgroundColor3=on and AMG_COR or Color3.fromRGB(55,22,88)}):Play()
...
-- Toggle helper
local function amgDoToggle()
    if amgPopupOpen then amgClosePopup() end
    if amgRunning then
        amgRunning = false
        TweenService:Create(amgSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        TweenService:Create(amgCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        ...
        task.delay(1.5,function() pcall(function() amgStatus.Text="Inativo" end) end)
    else
        ...
        TweenService:Create(amgSecS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        TweenService:Create(amgCardS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        ...
        task.spawn(function()
            ...
            pcall(function()
                ...
                TweenService:Create(amgCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                amgBtn.Text="⚙️  ATIVAR"; btnGrad(amgBtnG,Color3.fromRGB(100,200,255),Color3.fromRGB(40,100,200))
                amgStatus.Text="Inativo"
            end)
        end)
    end
end

local amgSecTogBtn = Instance.new("TextButton", amgSec)
amgSecTogBtn.BackgroundTransparency = 1; amgSecTogBtn.Size = UDim2.new(1, 0, 1, 0)
amgSecTogBtn.Text = ""; amgSecTogBtn.ZIndex = 8
amgSecTogBtn.MouseButton1Click:Connect(amgDoToggle)
amgBtn.MouseButton1Click:Connect(amgDoToggle)
...
-- ── Seção Header ────────────────────────────────────────────
local abfSec = Instance.new("Frame", Pages["Farm"])
abfSec.BackgroundColor3 = Color3.fromRGB(8, 28, 18); abfSec.BorderSizePixel = 0
abfSec.Size = UDim2.new(1, 0, 0, 32); abfSec.LayoutOrder = fNextLO(); abfSec.ZIndex = 4
Instance.new("UICorner", abfSec).CornerRadius = UDim.new(0, 10)
local abfSecG = Instance.new("UIGradient", abfSec)
abfSecG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(16, 54, 34)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(6, 18, 10))
}); abfSecG.Rotation = 90
local abfSecS = Instance.new("UIStroke", abfSec)
abfSecS.Color = ABF_COR; abfSecS.Thickness = 1.5; abfSecS.Transparency = 0.7
abfSecS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
local abfPill = Instance.new("Frame", abfSec); abfPill.BackgroundColor3 = ABF_COR
abfPill.BorderSizePixel = 0; abfPill.Position = UDim2.new(0, 8, 0.5, -10)
abfPill.Size = UDim2.new(0, 4, 0, 20); abfPill.ZIndex = 5
Instance.new("UICorner", abfPill).CornerRadius = UDim.new(1, 0)
local abfSecIco = Instance.new("TextLabel", abfSec); abfSecIco.BackgroundTransparency = 1
abfSecIco.Position = UDim2.new(0, 18, 0, 0); abfSecIco.Size = UDim2.new(0, 22, 1, 0)
abfSecIco.Font = Enum.Font.GothamBold; abfSecIco.Text = "🧪"; abfSecIco.TextSize = 14; abfSecIco.ZIndex = 6
local abfSecLbl = Instance.new("TextLabel", abfSec); abfSecLbl.BackgroundTransparency = 1
abfSecLbl.Position = UDim2.new(0, 40, 0, 0); abfSecLbl.Size = UDim2.new(1, -50, 1, 0)
abfSecLbl.Font = Enum.Font.GothamBlack; abfSecLbl.Text = "AUTO BIOFUEL PROCESSOR"
abfSecLbl.TextColor3 = Color3.fromRGB(190, 255, 225); abfSecLbl.TextSize = 11
abfSecLbl.TextXAlignment = Enum.TextXAlignment.Left; abfSecLbl.ZIndex = 5
Instance.new("UIStroke", abfSecLbl).Color = Color3.fromRGB(0, 0, 0)

-- ── Card ─────────────────────────────────────────────────────
local abfCard = Instance.new("Frame", Pages["Farm"])
abfCard.BackgroundColor3 = Color3.fromRGB(8, 26, 16); abfCard.BorderSizePixel = 0
abfCard.Size = UDim2.new(1, 0, 0, 110); abfCard.LayoutOrder = fNextLO(); abfCard.ZIndex = 5
Instance.new("UICorner", abfCard).CornerRadius = UDim.new(0, 12)
local abfCardG = Instance.new("UIGradient", abfCard)
abfCardG.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(10, 30, 18)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(4, 14, 8))
}); abfCardG.Rotation = 140
local abfCardS = Instance.new("UIStroke", abfCard)
abfCardS.Color = ABF_COR; abfCardS.Thickness = 1.5; abfCardS.Transparency = 0.7
abfCardS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
abfRef.stroke = abfCardS

local function abfDiv(y)
    local d = Instance.new("Frame", abfCard); d.BackgroundColor3 = Color3.fromRGB(20, 50, 30)
    d.BorderSizePixel = 0; d.Position = UDim2.new(0, 0, 0, y)
    d.Size = UDim2.new(1, 0, 0, 1); d.ZIndex = 6
end

-- ROW 1: Descrição + ícone
local abfRow1 = Instance.new("Frame", abfCard); abfRow1.BackgroundTransparency = 1
abfRow1.BorderSizePixel = 0; abfRow1.Position = UDim2.new(0, 0, 0, 0)
abfRow1.Size = UDim2.new(1, 0, 0, 56); abfRow1.ZIndex = 6

local abfLbl1 = Instance.new("TextLabel", abfRow1); abfLbl1.BackgroundTransparency = 1
abfLbl1.Position = UDim2.new(0, 14, 0, 8); abfLbl1.Size = UDim2.new(0.6, 0, 0, 40)
abfLbl1.Font = Enum.Font.GothamBold; abfLbl1.Text = "Processa itens no\nBiofuel Processor"
abfLbl1.TextColor3 = Color3.fromRGB(180, 255, 218); abfLbl1.TextSize = 11
abfLbl1.TextXAlignment = Enum.TextXAlignment.Left; abfLbl1.TextWrapped = true; abfLbl1.ZIndex = 7

local abfIB = Instance.new("Frame", abfRow1); abfIB.BackgroundColor3 = ABF_COR
abfIB.BackgroundTransparency = 0.3; abfIB.BorderSizePixel = 0
abfIB.AnchorPoint = Vector2.new(1, 0.5); abfIB.Position = UDim2.new(1, -12, 0.5, 0)
abfIB.Size = UDim2.new(0, 38, 0, 38); abfIB.ZIndex = 7
Instance.new("UICorner", abfIB).CornerRadius = UDim.new(1, 0)
local abfIL = Instance.new("TextLabel", abfIB); abfIL.BackgroundTransparency = 1
abfIL.Size = UDim2.new(1, 0, 1, 0); abfIL.Text = "🧪"; abfIL.TextSize = 20
abfIL.Font = Enum.Font.GothamBold; abfIL.ZIndex = 8

abfDiv(56)

-- ROW 2: Status + botão
local abfRow2 = Instance.new("Frame", abfCard); abfRow2.BackgroundTransparency = 1
abfRow2.BorderSizePixel = 0; abfRow2.Position = UDim2.new(0, 0, 0, 58)
abfRow2.Size = UDim2.new(1, 0, 0, 52); abfRow2.ZIndex = 6

local abfStatus = Instance.new("TextLabel", abfRow2); abfStatus.BackgroundTransparency = 1
abfStatus.Position = UDim2.new(0, 14, 0, 8); abfStatus.Size = UDim2.new(0.52, 0, 0, 36)
abfStatus.Font = Enum.Font.GothamBold; abfStatus.Text = "Inativo"
abfStatus.TextColor3 = ABF_COR; abfStatus.TextSize = 10
abfStatus.TextXAlignment = Enum.TextXAlignment.Left; abfStatus.TextWrapped = true; abfStatus.ZIndex = 7
abfRef.status = abfStatus

local abfBtn = Instance.new("TextButton", abfRow2)
abfBtn.BackgroundColor3 = ABF_COR; abfBtn.BorderSizePixel = 0
abfBtn.AnchorPoint = Vector2.new(1, 0.5); abfBtn.Position = UDim2.new(1, -12, 0.5, 0)
abfBtn.Size = UDim2.new(0, 110, 0, 34); abfBtn.Font = Enum.Font.GothamBlack
abfBtn.Text = "🧪  ATIVAR"; abfBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
abfBtn.TextSize = 11; abfBtn.ZIndex = 8; abfBtn.AutoButtonColor = false
Instance.new("UICorner", abfBtn).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", abfBtn).Color = Color3.fromRGB(8, 4, 20)
local abfBtnG = Instance.new("UIGradient", abfBtn); abfBtnG.Rotation = 90
btnGrad(abfBtnG, Color3.fromRGB(100, 220, 160), Color3.fromRGB(30, 140, 80))
local abfBtnShine = Instance.new("Frame", abfBtn)
abfBtnShine.BackgroundColor3 = Color3.fromRGB(255, 255, 255); abfBtnShine.BackgroundTransparency = 0.65
abfBtnShine.BorderSizePixel = 0; abfBtnShine.Position = UDim2.new(0, 6, 0, 4)
abfBtnShine.Size = UDim2.new(0, 45, 0, 7); abfBtnShine.ZIndex = 9
Instance.new("UICorner", abfBtnShine).CornerRadius = UDim.new(1, 0)
abfRef.btn = abfBtn; abfRef.btnG = abfBtnG

abfBtn.MouseEnter:Connect(function() TweenService:Create(abfBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0}):Play() end)
abfBtn.MouseLeave:Connect(function() TweenService:Create(abfBtn, TweenInfo.new(0.12), {BackgroundTransparency = 0.2}):Play() end)

local function abfDoToggle()
    if abfRunning then
        abfRunning = false; abfPos = nil
        TweenService:Create(abfSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        TweenService:Create(abfCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
        abfStatus.Text = "⏹ Parado"; Notify.error("Auto Biofuel","⏹ Desativado")
        abfBtn.Text = "🧪  ATIVAR"; btnGrad(abfBtnG, Color3.fromRGB(100,220,160), Color3.fromRGB(30,140,80))
        task.delay(1.5, function() pcall(function() abfStatus.Text = "Inativo" end) end)
    else
        TweenService:Create(abfSecS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        TweenService:Create(abfCardS, TweenInfo.new(0.2), {Transparency=0.2}):Play()
        abfBtn.Text = "⏹  PARAR"; btnGrad(abfBtnG, Color3.fromRGB(220,60,60), Color3.fromRGB(140,20,20))
        task.spawn(function()
            autoBiofuel()
            pcall(function()
                TweenService:Create(abfSecS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                TweenService:Create(abfCardS, TweenInfo.new(0.2), {Transparency=0.7}):Play()
                abfBtn.Text = "🧪  ATIVAR"; btnGrad(abfBtnG, Color3.fromRGB(100,220,160), Color3.fromRGB(30,140,80))
                abfStatus.Text = "Inativo"
            end)
        end)
    end
end

local abfSecTogBtn = Instance.new("TextButton", abfSec)
abfSecTogBtn.BackgroundTransparency = 1; abfSecTogBtn.Size = UDim2.new(1, 0, 1, 0)
abfSecTogBtn.Text = ""; abfSecTogBtn.ZIndex = 8
abfSecTogBtn.MouseButton1Click:Connect(abfDoToggle)
abfBtn.MouseButton1Click:Connect(abfDoToggle)
...
local function makeAvSec(titleTxt, cor)
    ...
    hdr.BackgroundColor3=Color3.fromRGB(55,22,88); hdr.BorderSizePixel = 0
...
local function makeAvToggle(lbl_txt, desc_txt, cor, onToggle)
    ...
    row.BackgroundColor3=Color3.fromRGB(55,22,88); row.BorderSizePixel = 0
    ...
    td.Text = desc_txt; td.TextColor3 = Color3.fromRGB(155,135,185); td.TextSize = 9
    td.TextXAlignment = Enum.TextXAlignment.Left; td.TextWrapped = true; td.ZIndex = 7
    local pill = Instance.new("Frame",row); pill.BackgroundColor3 = Color3.fromRGB(55,22,88)
    ...
    local knob = Instance.new("Frame",pill); knob.BackgroundColor3 = Color3.fromRGB(180,160,220)
    knob.BorderSizePixel = 0; knob.Position = UDim2.new(0,3,0.5,-11); knob.Size = UDim2.new(0,22,0,22); knob.ZIndex = 10
    Instance.new("UICorner",knob).CornerRadius = UDim.new(1,0)
    -- ✓ checkmark igual à foto
    local avCk=Instance.new("TextLabel",knob); avCk.BackgroundTransparency=1
    avCk.Size=UDim2.new(1,0,1,0); avCk.Text="✓"
    avCk.TextColor3=cor; avCk.TextSize=12; avCk.Font=Enum.Font.GothamBold
    avCk.Visible=false; avCk.ZIndex=11
    local state = false
    local btn = Instance.new("TextButton",row); btn.BackgroundTransparency = 1; btn.BorderSizePixel = 0
    btn.AutoButtonColor = false; btn.Size = UDim2.new(1,0,1,0); btn.Text = ""; btn.ZIndex = 11
    btn.MouseEnter:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        task.delay(0.1,function() TweenService:Create(row,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(55,22,88)}):Play() end)
        -- Som de ligar/desligar
        pcall(function()
            local sndId = state and 6031221736 or 2544086171
            local snd = Instance.new("Sound", SoundService)
