...
local function nReflow()
    ...
    local list = isBottom and {table.unpack(nActive)} or nActive
    if isBottom then
...
local ESP_CATS = {
    ...
    {key="Kids", trLabel="espKidsLabel",trDesc="espKidsDesc",label="👶 Lost Children", cor=Color3.fromRGB(100,220,255), tipo="entity", alcance=math.huge, desc="Lost Child 1/2/3/4 (Dino/Kraken/Squid/Koala Kid)",
     nomes={"Lost Child","LostChild","Lost Child 2","LostChild2","Lost Child 3","LostChild3","Lost Child 4","LostChild4",
            ...
            -- nomes reais confirmados no workspace do jogo (2026)
...
-- Retorna a cat que bate com nameL para o tipo dado (ou nil)
...
local function buildCache()
    ...
    task.spawn(function()
        ...
        -- Mapa chars dos players para excluir
        local pchars = {}
        for _,pl in ipairs(Players:GetPlayers()) do
            if pl.Character then pchars[pl.Character] = true end
        end

        -- Coleta todos os descendentes do workspace
        local allObjs = {}
        pcall(function()
        ...
        -- Adiciona filhos diretos de workspace.Items explicitamente
        pcall(function()
            local itemsF = workspace:FindFirstChild("Items")
            if itemsF then
                for _,o in ipairs(itemsF:GetChildren()) do
                    allObjs[#allObjs+1] = o
                end
            end
        end)
        ...
        for _,obj in ipairs(allObjs) do
            ...
            -- ── ENTIDADES: Model com Humanoid vivo, não-player ────────────
                    ...
                    if hum and hum.Health > 0 then
                        ...
                        if hrp then
                            ...
                            if cat then
                                seenEnt[id] = true
                                newEnt[#newEnt+1] = {
                                    key=cat.key, cor=cat.cor, nome=obj.Name,
                                    alcance=cat.alcance, obj=obj, hrp=hrp
                                }
                            end
                        end
                    end
                ...
                if not obj:FindFirstChildWhichIsA("Humanoid") then
                    ...
                    if cat then
                        ...
                        if part then
                            newItem[#newItem+1] = {
                                key=cat.key, cor=cat.cor, nome=obj.Name,
                                alcance=cat.alcance, obj=part
                            }
                        end
                    end
                end

            -- ── ITENS: BasePart solta (não filha de NPC) ──────────────────
            elseif doItem and obj:IsA("BasePart") then
                local par = obj.Parent
                local isNPC = par and par:IsA("Model")
                           and par:FindFirstChildWhichIsA("Humanoid")
                if not isNPC and not pchars[par] then
                    -- usa nome do Model pai para o match, não da BasePart
                    ...
                    if cat then
                        newItem[#newItem+1] = {
                            key=cat.key, cor=cat.cor, nome=dispName,
                            alcance=cat.alcance, obj=obj
                        }
                    end
                end
            end
        ...
        for _,pl in ipairs(Players:GetPlayers()) do
            if pl~=Player and pl.Character then
                pcall(function()
                    ...
                    if sp.Z<=0 or sp.X<-60 or sp.X>vp.X+60 or sp.Y<-40 or sp.Y>vp.Y+40 then return end
...
do
local _sc=_nc(plNextLO); local _sy=0
_sy=_sy+_nt(_sc,_sy,"⚡ "..T("plSpeedTitle"),true,function(en)
    speedEnabled=en; applySpeed(en and playerSpeed or 16)
    Notify.info("⚡ Velocidade",en and ("Ativado — "..playerSpeed.." speed") or "Desativado")
end)
_sy=_sy+_ns(_sc,_sy,T("plSpeedDesc"),16,275,30,Color3.fromRGB(255,180,30),function(v)
    playerSpeed=v; if speedEnabled then applySpeed(v) end
end)
_sy=_sy+_nd(_sc,_sy)
_sy=_sy+_nt(_sc,_sy,"🦘 "..T("plJumpTitle"),true,function(en)
    jumpEnabled=en; applyJump(en and playerJump or 50)
    Notify.info("🦘 Pulo",en and ("Ativado — "..playerJump.." power") or "Desativado")
end)
_sy=_sy+_ns(_sc,_sy,T("plJumpDesc"),50,1285,80,Color3.fromRGB(100,220,255),function(v)
    playerJump=v; if jumpEnabled then applyJump(v) end
end)
_sc.Size=UDim2.new(1,0,0,_sy)
end

-- ── Fly ───────────────────────────────────────────────
makePlSec("plSecFly", Color3.fromRGB(100,200,255))
do
local _fc=_nc(plNextLO); local _fy=0
_fy=_fy+_nt(_fc,_fy,"✈️ "..T("plFlyToggle"),false,function(en) setFly(en) end)
_fy=_fy+_nd(_fc,_fy)
_fy=_fy+_ns(_fc,_fy,T("plFlySpeedTitle"),16,345,40,Color3.fromRGB(120,200,255),function(v)
    flySpeed=v
end)
_fy=_fy+_nd(_fc,_fy)
_fy=_fy+_nt(_fc,_fy,"🫥 "..T("plNoclipToggle"),false,function(en) setNoclip(en) end)
_fc.Size=UDim2.new(1,0,0,_fy)
end
...
do
local _uc=_nc(plNextLO); local _uy=0
_uy=_uy+_nt(_uc,_uy,"🖱️ "..T("plTpClickToggle"),false,function(en) setTpClick(en) end)
_uy=_uy+_nd(_uc,_uy)
_uy=_uy+_nt(_uc,_uy,"👁 Visão Melhorada (NoFov)",false,function(en) if setNoFov then setNoFov(en) end end)
_uy=_uy+_nd(_uc,_uy)
_uy=_uy+_nt(_uc,_uy,"♾️ Pulo Infinito",false,function(en) if setInfJump then setInfJump(en) end end)
_uc.Size=UDim2.new(1,0,0,_uy)
end
...
do
local _ac=_nc(plNextLO); local _ay=0
_ay=_ay+_nt(_ac,_ay,"🛡 Anti Desaceleração",false,function(en) setAntiSlow(en) end)
_ay=_ay+_nd(_ac,_ay)
_ay=_ay+_nt(_ac,_ay,"🕳️ Anti-Void / Anti-Traversal",false,function(en) setAntiVoid(en) end)
_ac.Size=UDim2.new(1,0,0,_ay)
end
...
do
local _gc=_nc(plNextLO); local _gy=0
_gy=_gy+_nt(_gc,_gy,"👻 God Mod",false,function(en) setGodMod(en) end)
_gc.Size=UDim2.new(1,0,0,_gy)
end
...
acsCard.BackgroundColor3 = Color3.fromRGB(10,20,52); acsCard.BackgroundTransparency = 0.28
acsCard.BorderSizePixel  = 0
acsCard.Size             = UDim2.new(1,0,0,72)
acsCard.LayoutOrder      = plNextLO()
acsCard.ZIndex           = 5
Instance.new("UICorner",acsCard).CornerRadius = UDim.new(0,12)
local acsStroke = Instance.new("UIStroke",acsCard)
acsStroke.Color = Color3.fromRGB(45,75,155); acsStroke.Thickness = 1; acsStroke.Transparency=0.5
acsStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

-- Label + toggle navy (estilo _nt manual para usar acsPill/acsKnob nas callbacks)
local acsTitLbl = Instance.new("TextLabel",acsCard); acsTitLbl.BackgroundTransparency=1
acsTitLbl.Position=UDim2.new(0,14,0,0); acsTitLbl.Size=UDim2.new(0.65,0,1,0)
acsTitLbl.Font=Enum.Font.GothamBold; acsTitLbl.Text="🎁  Baús ACS"
acsTitLbl.TextColor3=Color3.fromRGB(200,215,245); acsTitLbl.TextSize=11
acsTitLbl.TextXAlignment=Enum.TextXAlignment.Left; acsTitLbl.TextYAlignment=Enum.TextYAlignment.Center; acsTitLbl.ZIndex=7

local acsPill=Instance.new("Frame",acsCard); acsPill.BorderSizePixel=0
acsPill.AnchorPoint=Vector2.new(1,0.5); acsPill.Position=UDim2.new(1,-12,0.5,0)
acsPill.Size=UDim2.new(0,52,0,28); acsPill.ZIndex=9
Instance.new("UICorner",acsPill).CornerRadius=UDim.new(1,0)
acsPill.BackgroundColor3=Color3.fromRGB(65,72,90)
local acsKnob=Instance.new("Frame",acsPill); acsKnob.BorderSizePixel=0
acsKnob.Size=UDim2.new(0,22,0,22); acsKnob.ZIndex=10
Instance.new("UICorner",acsKnob).CornerRadius=UDim.new(1,0)
acsKnob.BackgroundColor3=Color3.fromRGB(255,255,255); acsKnob.Position=UDim2.new(0,3,0.5,-11)
local acsCk=Instance.new("TextLabel",acsKnob); acsCk.BackgroundTransparency=1
acsCk.Size=UDim2.new(1,0,1,0); acsCk.Font=Enum.Font.GothamBlack
acsCk.Text="✕"; acsCk.TextColor3=Color3.fromRGB(65,72,90); acsCk.TextSize=11; acsCk.ZIndex=11
local acsStatus=Instance.new("TextLabel",acsCard); acsStatus.BackgroundTransparency=1
acsStatus.Position=UDim2.new(0,14,0,0); acsStatus.Size=UDim2.new(0,0,0,0); acsStatus.ZIndex=1 -- oculto

local acsBtnClick=Instance.new("TextButton",acsCard); acsBtnClick.BackgroundTransparency=1
acsBtnClick.Position=UDim2.new(0,0,0,0); acsBtnClick.Size=UDim2.new(1,0,1,0)
acsBtnClick.Text=""; acsBtnClick.ZIndex=11
acsBtnClick.MouseButton1Click:Connect(function()
    acsEnabled=not acsEnabled
    TweenService:Create(acsPill,TweenInfo.new(0.18),{BackgroundColor3=acsEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)}):Play()
    TweenService:Create(acsKnob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Position=acsEnabled and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
    }):Play()
    acsCk.Text=acsEnabled and "✓" or "✕"
    acsCk.TextColor3=acsEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)
    if acsEnabled then
        pcall(function()
            if not acsRemote then acsRemote=game:GetService("ReplicatedStorage").RemoteEvents.RequestOpenItemChest end
        end)
        startACS()
        Notify.send({type="custom",icon="🎁",accent=ACS_COR,title="Baús ACS",msg="Ativado — abrindo todos os baús!",duration=4})
    else
        stopACS()
...
ibcCard.BackgroundColor3=Color3.fromRGB(10,20,52); ibcCard.BackgroundTransparency=0.28
ibcCard.BorderSizePixel=0; ibcCard.Size=UDim2.new(1,0,0,44); ibcCard.LayoutOrder=plNextLO(); ibcCard.ZIndex=5
Instance.new("UICorner",ibcCard).CornerRadius=UDim.new(0,12)
local ibcStroke=Instance.new("UIStroke",ibcCard)
ibcStroke.Color=Color3.fromRGB(45,75,155); ibcStroke.Thickness=1; ibcStroke.Transparency=0.5
ibcStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

local ibcTitLbl=Instance.new("TextLabel",ibcCard); ibcTitLbl.BackgroundTransparency=1
ibcTitLbl.Position=UDim2.new(0,14,0,0); ibcTitLbl.Size=UDim2.new(0.65,0,1,0)
ibcTitLbl.Font=Enum.Font.GothamBold; ibcTitLbl.Text="🔓  Baús Instantâneos"
ibcTitLbl.TextColor3=Color3.fromRGB(200,215,245); ibcTitLbl.TextSize=11
ibcTitLbl.TextXAlignment=Enum.TextXAlignment.Left; ibcTitLbl.TextYAlignment=Enum.TextYAlignment.Center; ibcTitLbl.ZIndex=7

local ibcPill=Instance.new("Frame",ibcCard); ibcPill.BorderSizePixel=0
ibcPill.AnchorPoint=Vector2.new(1,0.5); ibcPill.Position=UDim2.new(1,-12,0.5,0)
ibcPill.Size=UDim2.new(0,52,0,28); ibcPill.ZIndex=9
Instance.new("UICorner",ibcPill).CornerRadius=UDim.new(1,0)
ibcPill.BackgroundColor3=Color3.fromRGB(65,72,90)
local ibcKnob=Instance.new("Frame",ibcPill); ibcKnob.BorderSizePixel=0
ibcKnob.Size=UDim2.new(0,22,0,22); ibcKnob.ZIndex=10
Instance.new("UICorner",ibcKnob).CornerRadius=UDim.new(1,0)
ibcKnob.BackgroundColor3=Color3.fromRGB(255,255,255); ibcKnob.Position=UDim2.new(0,3,0.5,-11)
local ibcCk=Instance.new("TextLabel",ibcKnob); ibcCk.BackgroundTransparency=1
ibcCk.Size=UDim2.new(1,0,1,0); ibcCk.Font=Enum.Font.GothamBlack
ibcCk.Text="✕"; ibcCk.TextColor3=Color3.fromRGB(65,72,90); ibcCk.TextSize=11; ibcCk.ZIndex=11

local ibcBtn=Instance.new("TextButton",ibcCard); ibcBtn.BackgroundTransparency=1
ibcBtn.Position=UDim2.new(0,0,0,0); ibcBtn.Size=UDim2.new(1,0,1,0)
ibcBtn.Text=""; ibcBtn.ZIndex=11
ibcBtn.MouseButton1Click:Connect(function()
    ibcEnabled=not ibcEnabled
    TweenService:Create(ibcPill,TweenInfo.new(0.18),{BackgroundColor3=ibcEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)}):Play()
    TweenService:Create(ibcKnob,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
        Position=ibcEnabled and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
    }):Play()
    ibcCk.Text=ibcEnabled and "✓" or "✕"
    ibcCk.TextColor3=ibcEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)
    if ibcEnabled then
        pcall(function()
            if not ibcRemote then ibcRemote=game:GetService("ReplicatedStorage").RemoteEvents.RequestOpenItemChest end
        end)
        ibcStart()
        Notify.send({type="custom",icon="🔓",accent=IBC_COR,title="Baús Instantâneos",msg="Ativado — chegue perto de um baú e clique!",duration=4})
    else
        ibcStop()
...
pibCard.BackgroundColor3=Color3.fromRGB(10,20,52); pibCard.BackgroundTransparency=0.28
pibCard.BorderSizePixel=0; pibCard.Size=UDim2.new(1,0,0,52); pibCard.LayoutOrder=plNextLO(); pibCard.ZIndex=5
Instance.new("UICorner",pibCard).CornerRadius=UDim.new(0,12)
local pibStroke=Instance.new("UIStroke",pibCard)
pibStroke.Color=Color3.fromRGB(45,75,155); pibStroke.Thickness=1; pibStroke.Transparency=0.5
pibStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

local pibTit=Instance.new("TextLabel",pibCard); pibTit.BackgroundTransparency=1
pibTit.Position=UDim2.new(0,14,0,0); pibTit.Size=UDim2.new(0.55,0,1,0)
pibTit.Font=Enum.Font.GothamBold; pibTit.Text="🎁  Pegar Itens Baús"
pibTit.TextColor3=Color3.fromRGB(200,215,245); pibTit.TextSize=11
pibTit.TextXAlignment=Enum.TextXAlignment.Left; pibTit.TextYAlignment=Enum.TextYAlignment.Center; pibTit.ZIndex=7

local pibStatus=Instance.new("TextLabel",pibCard); pibStatus.BackgroundTransparency=1
pibStatus.Position=UDim2.new(0,14,0,0); pibStatus.Size=UDim2.new(0,0,0,0); pibStatus.ZIndex=1 -- oculto

local pibBtn=Instance.new("TextButton",pibCard)
pibBtn.BackgroundColor3=Color3.fromRGB(50,120,255); pibBtn.BackgroundTransparency=0.15
pibBtn.BorderSizePixel=0
pibBtn.AnchorPoint=Vector2.new(1,0.5); pibBtn.Position=UDim2.new(1,-10,0.5,0); pibBtn.Size=UDim2.new(0,120,0,34)
pibBtn.Font=Enum.Font.GothamBlack; pibBtn.Text="🎁  PEGAR"; pibBtn.TextColor3=Color3.fromRGB(220,235,255)
pibBtn.TextSize=10; pibBtn.ZIndex=8
Instance.new("UICorner",pibBtn).CornerRadius=UDim.new(0,8)
local pibBtnS=Instance.new("UIStroke",pibBtn); pibBtnS.Color=Color3.fromRGB(80,140,255); pibBtnS.Thickness=1.2; pibBtnS.Transparency=0.4
pibBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        pibBtn.Text="🎁  PEGAR"
        TweenService:Create(pibBtn,TweenInfo.new(0.2),{BackgroundTransparency=0.15}):Play()
    end)
end)
...
do
    ...
    -- Card toggle Auto Fogueira Anoitecer
    local nightRow=Instance.new("Frame",Pages["Player"])
    nightRow.BackgroundColor3=Color3.fromRGB(10,20,52); nightRow.BackgroundTransparency=0.28
    nightRow.BorderSizePixel=0; nightRow.Size=UDim2.new(1,0,0,44); nightRow.LayoutOrder=plNextLO(); nightRow.ZIndex=5
    Instance.new("UICorner",nightRow).CornerRadius=UDim.new(0,12)
    local nightRowS=Instance.new("UIStroke",nightRow); nightRowS.Color=Color3.fromRGB(45,75,155); nightRowS.Thickness=1; nightRowS.Transparency=0.5
    nightRowS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

    local nightTl=Instance.new("TextLabel",nightRow); nightTl.BackgroundTransparency=1
    nightTl.Position=UDim2.new(0,14,0,0); nightTl.Size=UDim2.new(0.65,0,1,0)
    nightTl.Font=Enum.Font.GothamBold; nightTl.Text="🌙 Auto Fogueira Anoitecer"
    nightTl.TextColor3=Color3.fromRGB(200,215,245); nightTl.TextSize=11
    nightTl.TextXAlignment=Enum.TextXAlignment.Left; nightTl.TextYAlignment=Enum.TextYAlignment.Center; nightTl.ZIndex=7

    local nightPill=Instance.new("Frame",nightRow); nightPill.BorderSizePixel=0
    nightPill.AnchorPoint=Vector2.new(1,0.5); nightPill.Position=UDim2.new(1,-12,0.5,0)
    nightPill.Size=UDim2.new(0,52,0,28); nightPill.ZIndex=9
    Instance.new("UICorner",nightPill).CornerRadius=UDim.new(1,0)
    nightPill.BackgroundColor3=Color3.fromRGB(65,72,90)
    local nightKnob2=Instance.new("Frame",nightPill); nightKnob2.BorderSizePixel=0
    nightKnob2.Size=UDim2.new(0,22,0,22); nightKnob2.ZIndex=10
    Instance.new("UICorner",nightKnob2).CornerRadius=UDim.new(1,0)
    nightKnob2.BackgroundColor3=Color3.fromRGB(255,255,255); nightKnob2.Position=UDim2.new(0,3,0.5,-11)
    local nightCk2=Instance.new("TextLabel",nightKnob2); nightCk2.BackgroundTransparency=1
    nightCk2.Size=UDim2.new(1,0,1,0); nightCk2.Font=Enum.Font.GothamBlack
    nightCk2.Text="✕"; nightCk2.TextColor3=Color3.fromRGB(65,72,90); nightCk2.TextSize=11; nightCk2.ZIndex=11

    local nightBtn=Instance.new("TextButton",nightRow); nightBtn.BackgroundTransparency=1
    nightBtn.Size=UDim2.new(1,0,1,0); nightBtn.Text=""; nightBtn.ZIndex=11
    nightBtn.MouseButton1Click:Connect(function()
        nightAutoEnabled=not nightAutoEnabled
        TweenService:Create(nightPill,TweenInfo.new(0.18),{BackgroundColor3=nightAutoEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)}):Play()
        TweenService:Create(nightKnob2,TweenInfo.new(0.20,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
            Position=nightAutoEnabled and UDim2.new(1,-25,0.5,-11) or UDim2.new(0,3,0.5,-11)
        }):Play()
        nightCk2.Text=nightAutoEnabled and "✓" or "✕"
        nightCk2.TextColor3=nightAutoEnabled and Color3.fromRGB(50,120,255) or Color3.fromRGB(65,72,90)
...
local function makeInlineDropdown(parent, yPos, cfg)
    local COR=cfg.color or Color3.fromRGB(148,112,220)
    local MAX_VIS=cfg.maxVisible or 5
    local ITEM_H=34; local BTN_H=30; local DROP_H=MAX_VIS*ITEM_H+10
    local dropOpen=false

    local function buildLabel()
        local sel={}
        for _,item in ipairs(cfg.items) do if cfg.sel[item.key] then table.insert(sel,item.label) end end
        local n=#sel
        if n==0 then return "Selecione ▾" end
        if n<=3 then return table.concat(sel,", ").." ▾" end
        return sel[1]..", "..sel[2]..", "..sel[3].." +".. (n-3).." ▾"
    end

    local selBtn=Instance.new("TextButton",parent)
    selBtn.BackgroundColor3=COR; selBtn.BackgroundTransparency=0.82; selBtn.BorderSizePixel=0
    selBtn.Position=UDim2.new(0,10,0,yPos); selBtn.Size=UDim2.new(1,-20,0,BTN_H)
    selBtn.Font=Enum.Font.GothamBold; selBtn.Text="Selecione ▾"
    selBtn.TextColor3=Color3.fromRGB(225,215,255); selBtn.TextSize=10
    selBtn.ZIndex=8; selBtn.AutoButtonColor=false; selBtn.TextTruncate=Enum.TextTruncate.AtEnd
    Instance.new("UICorner",selBtn).CornerRadius=UDim.new(0,8)
    local selBtnS=Instance.new("UIStroke",selBtn)
    selBtnS.Color=COR; selBtnS.Thickness=1.1; selBtnS.Transparency=0.55
    selBtnS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

    local dropFr=Instance.new("Frame",parent)
    dropFr.BackgroundColor3=Color3.fromRGB(20,14,38); dropFr.BackgroundTransparency=0.06
    dropFr.BorderSizePixel=0; dropFr.Position=UDim2.new(0,10,0,yPos+BTN_H+4)
    dropFr.Size=UDim2.new(1,-20,0,0); dropFr.ZIndex=9; dropFr.ClipsDescendants=true
    Instance.new("UICorner",dropFr).CornerRadius=UDim.new(0,8)
    local dropS=Instance.new("UIStroke",dropFr)
    dropS.Color=COR; dropS.Thickness=1; dropS.Transparency=0.5

    local scroll=Instance.new("ScrollingFrame",dropFr)
    scroll.BackgroundTransparency=1; scroll.BorderSizePixel=0
    scroll.Size=UDim2.new(1,0,1,0); scroll.ScrollBarThickness=3
    scroll.ScrollBarImageColor3=COR; scroll.CanvasSize=UDim2.new(0,0,0,0)
    scroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; scroll.ZIndex=10
    ...
    for i,item in ipairs(cfg.items) do
        ...
        row.BackgroundColor3=Color3.fromRGB(36,24,58)
        row.BackgroundTransparency=cfg.sel[item.key] and 0.25 or 0.65
        row.BorderSizePixel=0; row.Size=UDim2.new(1,0,0,ITEM_H-2)
        row.LayoutOrder=i; row.ZIndex=11
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,6)
        local rowS=Instance.new("UIStroke",row)
        rowS.Color=cfg.sel[item.key] and COR or Color3.fromRGB(60,44,90)
        rowS.Thickness=1; rowS.Transparency=cfg.sel[item.key] and 0.35 or 0.6
        rowS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

        local chkBg=Instance.new("Frame",row)
        chkBg.BackgroundColor3=cfg.sel[item.key] and COR or Color3.fromRGB(36,24,58)
        chkBg.BorderSizePixel=0; chkBg.Position=UDim2.new(0,7,0.5,-9)
        chkBg.Size=UDim2.new(0,18,0,18); chkBg.ZIndex=12
        Instance.new("UICorner",chkBg).CornerRadius=UDim.new(0,4)
        local chkS2=Instance.new("UIStroke",chkBg); chkS2.Color=COR; chkS2.Thickness=1.3
        local chkMark=Instance.new("TextLabel",chkBg); chkMark.BackgroundTransparency=1
        chkMark.Size=UDim2.new(1,0,1,0); chkMark.Font=Enum.Font.GothamBlack
        chkMark.Text=cfg.sel[item.key] and "✓" or ""; chkMark.TextColor3=Color3.fromRGB(255,255,255)
        chkMark.TextSize=10; chkMark.ZIndex=13

        local icoLbl=Instance.new("TextLabel",row); icoLbl.BackgroundTransparency=1
        icoLbl.Position=UDim2.new(0,33,0,0); icoLbl.Size=UDim2.new(0,22,1,0)
        icoLbl.Text=item.icon or "▪"; icoLbl.TextSize=13; icoLbl.Font=Enum.Font.GothamBold; icoLbl.ZIndex=12

        local nameLbl=Instance.new("TextLabel",row); nameLbl.BackgroundTransparency=1
        nameLbl.Position=UDim2.new(0,58,0,0); nameLbl.Size=UDim2.new(1,-64,1,0)
        nameLbl.Font=Enum.Font.GothamBold; nameLbl.Text=item.label
        nameLbl.TextColor3=cfg.sel[item.key] and Color3.fromRGB(255,255,255) or Color3.fromRGB(185,175,215)
        nameLbl.TextSize=10; nameLbl.TextXAlignment=Enum.TextXAlignment.Left; nameLbl.ZIndex=12

        local function refreshRow()
            local on=cfg.sel[item.key]
            TweenService:Create(row,TweenInfo.new(0.12),{BackgroundTransparency=on and 0.25 or 0.65}):Play()
            TweenService:Create(rowS,TweenInfo.new(0.12),{Color=on and COR or Color3.fromRGB(60,44,90),Transparency=on and 0.35 or 0.6}):Play()
            TweenService:Create(chkBg,TweenInfo.new(0.12),{BackgroundColor3=on and COR or Color3.fromRGB(36,24,58)}):Play()
            chkMark.Text=on and "✓" or ""; nameLbl.TextColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(185,175,215)
        end

        local rowBtn=Instance.new("TextButton",row); rowBtn.BackgroundTransparency=1
        rowBtn.Size=UDim2.new(1,0,1,0); rowBtn.Text=""; rowBtn.ZIndex=14; rowBtn.AutoButtonColor=false
        rowBtn.MouseButton1Click:Connect(function()
            cfg.sel[item.key]=not cfg.sel[item.key]; refreshRow()
            selBtn.Text=buildLabel(); if cfg.onUpdate then cfg.onUpdate() end
        end)
        itemRefs[item.key]=refreshRow
    end

    local function closeDrop()
        if not dropOpen then return end; dropOpen=false
        TweenService:Create(selBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.82}):Play()
        TweenService:Create(selBtnS,TweenInfo.new(0.15),{Transparency=0.55}):Play()
        TweenService:Create(dropFr,TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(1,-20,0,0)}):Play()
        if cfg.setDropH then cfg.setDropH(0) end
    end
    local function openDrop()
        if dropOpen then closeDrop(); return end; dropOpen=true
        TweenService:Create(selBtn,TweenInfo.new(0.15),{BackgroundTransparency=0.65}):Play()
        TweenService:Create(selBtnS,TweenInfo.new(0.15),{Transparency=0.2}):Play()
        TweenService:Create(dropFr,TweenInfo.new(0.28,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(1,-20,0,DROP_H)}):Play()
        if cfg.setDropH then cfg.setDropH(DROP_H+8) end
    end
    ...
    UserInputService.InputBegan:Connect(function(inp)
        ...
        if inp.UserInputType~=Enum.UserInputType.MouseButton1 then return end
        local mp=UserInputService:GetMouseLocation()
        local function inside(f) local ap,as=f.AbsolutePosition,f.AbsoluteSize
            return mp.X>=ap.X and mp.X<=ap.X+as.X and mp.Y>=ap.Y and mp.Y<=ap.Y+as.Y end
        if not inside(selBtn) and not inside(dropFr) then closeDrop() end
    end)

    local function selectAll()
        for _,item in ipairs(cfg.items) do cfg.sel[item.key]=true end
        for _,fn in pairs(itemRefs) do pcall(fn) end
        selBtn.Text=buildLabel(); if cfg.onUpdate then cfg.onUpdate() end
    end
    local function clearAll()
        for _,item in ipairs(cfg.items) do cfg.sel[item.key]=false end
        for _,fn in pairs(itemRefs) do pcall(fn) end
        selBtn.Text=buildLabel(); if cfg.onUpdate then cfg.onUpdate() end
    end

    return selBtn, dropFr, buildLabel, selectAll, clearAll, closeDrop
end
...
local function autoFeedCampfire()
    ...
    while afcRunning do
        ...
        if #lista==0 then pcall(function() if afcRef.status then afcRef.status.Text="⚠️ Sem combustível" end end); task.wait(5)
            ...
            ciclo+=1; pcall(function() if afcRef.status then afcRef.status.Text=string.format("🔥 Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then dropNearCampfire(e.part,e.obj,fogPos,s-1,LOTE); table.insert(loteE,e); total+=1; task.wait(0.1) end
...
local _afhpSel,_=makeInlineDropdown(_afhpCF,_y,{
    items=AFC_ALL_ITEMS,sel=afcSel,color=AFHP_COR,maxVisible=5,setDropH=_afhpDrop,
})
...
local function autoCookFood()
    ...
    while ackRunning do
        ...
        if #lista==0 then pcall(function() if ackRef.status then ackRef.status.Text="⚠️ Sem itens crus" end end); task.wait(5)
            ...
            ciclo+=1; pcall(function() if ackRef.status then ackRef.status.Text=string.format("🍖 Lote %d — %d disponíveis",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then dropNearCampfire(e.part,e.obj,fogPos,s-1,LOTE); table.insert(loteE,e); total+=1; task.wait(0.15) end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local function autoMachineGrind()
    ...
    while amgRunning do
        ...
        if #lista==0 then pcall(function() if amgRef.status then amgRef.status.Text="⚠️ Sem itens" end end); task.wait(5)
            ...
            ciclo+=1; pcall(function() if amgRef.status then amgRef.status.Text=string.format("⚙️ Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then
                    ...
                    dropAtPos(e.part,e.obj,dropPos); table.insert(loteE,e); total+=1; task.wait(0.15)
                end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local _amgSelBtn,_,_,_amgSelAll,_amgClearAll=makeInlineDropdown(_amgCF,_y,{
    items=AMG_ALL_ITEMS,sel=amgSel,color=AMG_COR,maxVisible=6,setDropH=_amgDrop,
})
...
local function autoBiofuel()
    ...
    while abfRunning do
        ...
        if #lista==0 then pcall(function() if abfRef.status then abfRef.status.Text="⚠️ Sem itens" end end); task.wait(5)
            ...
            ciclo+=1; pcall(function() if abfRef.status then abfRef.status.Text=string.format("🧪 Lote %d — %d itens",ciclo,#lista) end end)
            local loteE={}; local running={true}
            for s=1,math.min(LOTE,#lista) do local e=lista[s]
                if e and e.part and e.part.Parent then
                    local ang=(s/LOTE)*math.pi*2
                    local dropPos=Vector3.new(abfPos.X+math.cos(ang)*0.6,abfPos.Y,abfPos.Z+math.sin(ang)*0.6)
                    dropAtPos(e.part,e.obj,dropPos); table.insert(loteE,e); total+=1; task.wait(0.1)
                end
            end; waitConsumed(loteE,TIMEOUT,running)
        end
    end
...
local FBC_MODES={
    {key="jogador",label="Jogador",icon="🧍",color=Color3.fromRGB(100,200,255)},
    {key="fogueira",label="Fogueira",icon="🔥",color=Color3.fromRGB(255,120,40)},
    {key="pertofog",label="Perto da Fog.",icon="🪵",color=Color3.fromRGB(200,160,60)},
}
local fbcModeIdx=1
_nfSec("🎁  FARM BAÚS", FBC_COR)
local _fbcCard,_fbcCF,_ = makeAccordionCard(Pages["Farm"], fNextLO, {
    icon="🎁", title="Farm Baús",
    summary="Teleporta em cada baú do mapa, coleta os itens e os leva ao destino.",
    color=FBC_COR, contentH=36+9+30+8+32+8+14,
})
fbcCard=_fbcCard
local _y=36+8; _accDivLine(_fbcCF,_y,FBC_COR); _y=_y+9
-- Botão de modo
local _modeBtn=Instance.new("TextButton",_fbcCF)
_modeBtn.BackgroundColor3=Color3.fromRGB(20,14,40); _modeBtn.BackgroundTransparency=0.1; _modeBtn.BorderSizePixel=0
_modeBtn.Position=UDim2.new(0,10,0,_y); _modeBtn.Size=UDim2.new(1,-20,0,28)
_modeBtn.Font=Enum.Font.GothamBold; _modeBtn.Text="🧍  Modo: Jogador  ▸"
_modeBtn.TextColor3=Color3.fromRGB(100,200,255); _modeBtn.TextSize=10; _modeBtn.ZIndex=8; _modeBtn.AutoButtonColor=false
Instance.new("UICorner",_modeBtn).CornerRadius=UDim.new(0,8)
local _modeBtnS=Instance.new("UIStroke",_modeBtn); _modeBtnS.Color=Color3.fromRGB(100,200,255); _modeBtnS.Thickness=1.1; _modeBtnS.Transparency=0.5
_modeBtn.MouseButton1Click:Connect(function()
    fbcModeIdx=(fbcModeIdx%#FBC_MODES)+1; local m=FBC_MODES[fbcModeIdx]; fbcMode=m.key
    _modeBtn.Text=m.icon.."  Modo: "..m.label.."  ▸"; _modeBtn.TextColor3=m.color; _modeBtnS.Color=m.color
end)
_y=_y+36
        ...
        pcall(function()
            ...
            for si,e in ipairs(nearItems) do
                if e.part and e.part.Parent then
                    ...
                    dropAtPos(e.part,e.obj,target); collected+=1; task.wait(0.05)
                end
            end
        end)
