...
--  CORREÇÕES APLICADAS:
--  ✓ AnimAgressivo: +Green Frog / GreenFrog
--  ✓ Monstros: removido The Bat (não existe no jogo)
--  ✓ Cultistas: removido Shadow/Brute Cultist, +Mega Cultist
--  ✓ EspFerr: +Leather Body, Alien Armour, Frog Boots, Poison Armour
-- ════════════════════════════════════════════════════════
...
local ESP_CATS = {
    ...
    -- ✓ FIX: Adicionado Green Frog / GreenFrog
    ...
    -- ✓ FIX: Removido The Bat (não existe no jogo)
    ...
    -- ✓ FIX: Removido Shadow/Brute Cultist, adicionado Mega Cultist
    ...
    -- ✓ FIX: Adicionado Leather Body, Alien Armour, Frog Boots, Poison Armour
...
do
    local f=Instance.new("Frame",Pages["Esp"]); f.BackgroundColor3=Color3.fromRGB(40,20,10)
    f.BackgroundTransparency=0.4; f.BorderSizePixel=0; f.Size=UDim2.new(1,0,0,40); f.LayoutOrder=espLO(); f.ZIndex=5
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,8)
    local s=Instance.new("UIStroke",f); s.Color=Color3.fromRGB(200,130,30); s.Thickness=1
    local t=Instance.new("TextLabel",f); t.BackgroundTransparency=1; t.Position=UDim2.new(0,8,0,4)
    t.Size=UDim2.new(1,-16,1,-8); t.Font=Enum.Font.Gotham
    t.Text="ℹ ESP v4 Corrigido | Green Frog ✓ | Mega Cultist ✓ | Sem The Bat ✓ | Armaduras ✓ | Wiki 99 Nights 2026"
    t.TextColor3=Color3.fromRGB(220,170,80); t.TextSize=8; t.TextWrapped=true
    t.TextXAlignment=Enum.TextXAlignment.Left; t.TextYAlignment=Enum.TextYAlignment.Top; t.ZIndex=6
end

-- ════════════════════════════════════════════════════════
--  SISTEMA BRING v4
--  CORREÇÕES APLICADAS:
--  ✓ BCarcacas: +Hellephant, PolarBear, ArcticFox, Mammoth, Frog Corpse
--  ✓ BFerr: +Leather Body, Alien Armour, Frog Boots, Poison Armour
-- ════════════════════════════════════════════════════════
local BRING_CATS = {
    {key="BLog",      label="🪵 Bring Log",         cor=Color3.fromRGB(190,130,60),  desc="Só pega: Log",
     nomes={"Log"}},
    {key="BCombust",  label="🔥 Bring Combustível", cor=Color3.fromRGB(255,120,30),  desc="Coal, Biofuel, Fuel Canister, Oil Barrel…",
     nomes={"Coal","Biofuel","Oil Barrel","OilBarrel","Fuel Canister","FuelCanister","Purple Fur Tuft","PurpleFurTuft","Chair"}},
    -- ✓ FIX: Adicionado Hellephant, Polar Bear, Arctic Fox, Mammoth, Frog Corpse
...
do
    local f=Instance.new("Frame",Pages["Bring"]); f.BackgroundColor3=Color3.fromRGB(14,30,50)
    f.BackgroundTransparency=0.5; f.BorderSizePixel=0; f.Size=UDim2.new(1,0,0,52); f.LayoutOrder=bringLO(); f.ZIndex=4
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,8)
    local rs=Instance.new("UIStroke",f); rs.Color=Color3.fromRGB(50,120,200); rs.Thickness=1
    local rt=Instance.new("TextLabel",f); rt.BackgroundTransparency=1; rt.Position=UDim2.new(0,8,0,4)
    rt.Size=UDim2.new(1,-16,1,-8); rt.Font=Enum.Font.Gotham
    rt.Text="ℹ BRING v4 Corrigido | +Hellephant/PolarBear/Frog Corpse ✓ | +Armaduras ✓ | Anti-desaparecimento 8s | Wiki 2026"
    rt.TextColor3=Color3.fromRGB(90,165,255); rt.TextSize=8; rt.TextWrapped=true
    rt.TextXAlignment=Enum.TextXAlignment.Left; rt.TextYAlignment=Enum.TextYAlignment.Top; rt.ZIndex=5
end
...
local notifCount = 0
local function showNotif(msg, tipo)
    notifCount = notifCount + 1
    local cor = tipo == "erro" and Color3.fromRGB(220,60,60)
           or tipo == "aviso" and Color3.fromRGB(240,180,0)
           or tipo == "sucesso" and Color3.fromRGB(60,220,120)
           or Color3.fromRGB(88,101,242)
    local icone = tipo == "erro" and "✖" or tipo == "aviso" and "⚠" or tipo == "sucesso" and "✔" or "ℹ"
    local nf = Instance.new("Frame", NotifContainer)
    nf.Name = "Notif"..notifCount
    nf.BackgroundColor3 = Color3.fromRGB(22,24,30)
    nf.BorderSizePixel = 0
    nf.Size = UDim2.new(0, 0, 0, 44)
    nf.ClipsDescendants = true
    nf.ZIndex = 999
    nf.LayoutOrder = notifCount
    Instance.new("UICorner", nf).CornerRadius = UDim.new(0, 10)
    local ns = Instance.new("UIStroke", nf); ns.Color = cor; ns.Thickness = 1.2
    local accent = Instance.new("Frame", nf)
    accent.BackgroundColor3 = cor; accent.BorderSizePixel = 0
    accent.Position = UDim2.new(0, 0, 0, 0); accent.Size = UDim2.new(0, 3, 1, 0); accent.ZIndex = 1000
    Instance.new("UICorner", accent).CornerRadius = UDim.new(0, 3)
    local ico = Instance.new("TextLabel", nf)
    ico.BackgroundTransparency = 1; ico.Position = UDim2.new(0, 10, 0.5, -10)
    ico.Size = UDim2.new(0, 20, 0, 20); ico.Font = Enum.Font.GothamBlack
    ico.Text = icone; ico.TextColor3 = cor; ico.TextSize = 14; ico.ZIndex = 1000
    local lbl = Instance.new("TextLabel", nf)
    lbl.BackgroundTransparency = 1; lbl.Position = UDim2.new(0, 34, 0.5, -11)
    lbl.Size = UDim2.new(1, -42, 0, 22); lbl.Font = Enum.Font.GothamSemibold
    lbl.Text = msg; lbl.TextColor3 = Color3.fromRGB(220,225,240); lbl.TextSize = 10
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.TextWrapped = true; lbl.ZIndex = 1000
    -- Slide in
    TweenService:Create(nf, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 270, 0, 44)}):Play()
    -- Auto hide after 0.6s display (0.3 anim + 0.6 show + 0.3 fadeout)
    task.delay(0.6, function()
        TweenService:Create(nf, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 0, 0, 44)}):Play()
        task.delay(0.31, function() pcall(function() nf:Destroy() end) end)
    end)
end


-- ══════════════════════════════════════════════════════════
--  FUNÇÕES PLAYER — Speed, Jump, Fly, TpClick, BauANC
--  + InfinitoZoom, CameraLivre, FullBright, InfinitJump, SlopeBoost
-- ══════════════════════════════════════════════════════════
...
local camLivreEnabled = false
local fullBrightEnabled = false
local origLighting = {}
local infJumpEnabled = false
local infJumpConn
local slopeBoostEnabled = false
local slopeBoostConn
...
Player.CharacterAdded:Connect(function()
    ...
    if slopeBoostEnabled then setSlopeBoost(true) end
end)

-- INFINITO ZOOM
local function setInfZoom(state)
    infZoomEnabled = state
    pcall(function()
        local cam = workspace.CurrentCamera
        if state then
            cam.FieldOfView = 70
            -- Remove limite de zoom
            local StarterGui = game:GetService("StarterGui")
            pcall(function()
                local CameraModule = require(Players.LocalPlayer.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("CameraModule"))
                if CameraModule then
                    CameraModule.maxZoomDistance = math.huge
                end
            end)
            -- Método direto: ajusta via property no loop
            RunService:BindToRenderStep("InfZoom", Enum.RenderPriority.Camera.Value + 1, function()
                if not infZoomEnabled then RunService:UnbindFromRenderStep("InfZoom") return end
                local cam2 = workspace.CurrentCamera
                if cam2 then
                    pcall(function() cam2.MaxZoomDistance = math.huge end)
                end
            end)
        else
            RunService:UnbindFromRenderStep("InfZoom")
            pcall(function()
                local p = Players.LocalPlayer
                if p and p.CameraMaxZoomDistance then p.CameraMaxZoomDistance = 128 end
            end)
        end
    end)
end

-- CAMERA LIVRE (atravessa paredes)
local function setCamLivre(state)
    camLivreEnabled = state
    if state then
        RunService:BindToRenderStep("CamLivre", Enum.RenderPriority.Camera.Value + 1, function()
            if not camLivreEnabled then RunService:UnbindFromRenderStep("CamLivre") return end
            local cam = workspace.CurrentCamera
            if cam then cam.CameraType = Enum.CameraType.Custom end
            -- Remove colisão da câmera com objetos
            pcall(function()
                for _, part in ipairs(workspace:GetDescendants()) do
                    if part:IsA("BasePart") and not part:IsDescendantOf(Player.Character or Instance.new("Folder")) then
                        part.CastShadow = part.CastShadow -- keep alive but cam ignores
                    end
                end
            end)
        end)
        -- Hack de câmera: seta perto do personagem sem colisão
        pcall(function()
        ...
        RunService:UnbindFromRenderStep("CamLivre")
    end
end

-- FULLBRIGHT + Remove chuva/neblina
local function setFullBright(state)
    ...
    if state then
        ...
        -- Remove efeitos climáticos (Atmosphere, Clouds, etc)
        for _, obj in ipairs(Lighting:GetChildren()) do
            pcall(function()
                if obj:IsA("Atmosphere") or obj:IsA("ColorCorrectionEffect") or obj:IsA("BlurEffect") or obj:IsA("DepthOfFieldEffect") then
                    obj.Enabled = false
                end
            end)
        end
        -- Remove chuva/neblina no workspace
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                if obj:IsA("ParticleEmitter") then
                    local nm = obj.Name:lower()
                    if nm:find("rain") or nm:find("chuva") or nm:find("fog") or nm:find("snow") or nm:find("mist") or nm:find("storm") then
                        obj.Enabled = false
                    end
                end
            end)
        end
        showNotif("FullBright ativado! Noite vira dia 🌞", "sucesso")
    else
        ...
        showNotif("FullBright desativado", "info")
    end
end
...
function setInfJump(state)
    ...
    if state then
        ...
        showNotif("Infinite Jump ativado! 🚀", "sucesso")
    else
        if infJumpConn then infJumpConn:Disconnect(); infJumpConn = nil end
    end
end

-- SLOPE BOOST (imune a armadilhas de desaceleração)
function setSlopeBoost(state)
    slopeBoostEnabled = state
    if state then
        if slopeBoostConn then slopeBoostConn:Disconnect() end
        slopeBoostConn = RunService.Heartbeat:Connect(function()
            if not slopeBoostEnabled then return end
            local ch = Player.Character; if not ch then return end
            local hum = ch:FindFirstChildWhichIsA("Humanoid"); if not hum then return end
            -- Mantém velocidade constante mesmo em slopes/armadilhas
            if hum.WalkSpeed < playerSpeed then
                hum.WalkSpeed = playerSpeed
            end
            -- Remove efeito de slow de armadilhas
            pcall(function()
                for _, eff in ipairs(ch:GetDescendants()) do
                    if eff:IsA("NumberValue") and (eff.Name:lower():find("slow") or eff.Name:lower():find("speed") or eff.Name:lower():find("trap")) then
                        eff.Value = 0
                    end
                end
            end)
        end)
        showNotif("Slope Boost ativado! Armadilhas ignoradas ⚡", "sucesso")
    else
        if slopeBoostConn then slopeBoostConn:Disconnect(); slopeBoostConn = nil end
    end
end
...
local function criarIconePlayer(parent, key, cor)
    local ic=Instance.new("Frame",parent); ic.BackgroundTransparency=1; ic.BorderSizePixel=0
    ic.Position=UDim2.new(0,4,0,4); ic.Size=UDim2.new(0,28,0,28); ic.ZIndex=parent.ZIndex+1; ic.ClipsDescendants=false
    local function r(x,y,w,h,rad) local f=mkRect(ic,x,y,w,h,cor,rad); return f end
    local function c(x,y,rad) return mkCircle(ic,x,y,rad,cor) end
    if key=="Speed" then
        r(0,10,20,4,2); r(4,6,16,4,2); r(8,14,16,4,2); r(22,8,6,10,3)
    elseif key=="JumpPower" then
        c(14,18,6); r(12,4,4,14,2); r(6,4,16,4,2)
    elseif key=="Fly" then
        r(2,12,24,4,2); r(4,6,8,8,4); r(16,6,8,8,4); r(10,14,8,10,3)
    elseif key=="Noclip" then
        c(14,10,7); local inner=mkCircle(ic,14,10,5,Color3.fromRGB(28,30,38)); inner.ZIndex=ic.ZIndex+2
        r(10,6,3,3,1); r(15,6,3,3,1); r(10,11,3,3,1); r(15,11,3,3,1)
    elseif key=="TpClick" then
        r(10,0,4,28,2); r(0,10,28,4,2); c(14,14,4)
    elseif key=="BauANC" then
        r(2,8,24,16,4); r(8,4,12,6,2); r(10,12,8,4,3)
    elseif key=="InfZoom" then
        c(10,10,8); local fi=mkCircle(ic,10,10,5,Color3.fromRGB(28,30,38)); fi.ZIndex=ic.ZIndex+2
        r(16,16,12,4,2); r(24,10,4,12,2)
    elseif key=="CamLivre" then
        r(2,8,20,14,4); r(22,10,6,10,3); c(12,15,4)
    elseif key=="FullBright" then
        c(14,14,6); for a=0,315,45 do local rad=math.rad(a); r(14+math.cos(rad)*10-1,14+math.sin(rad)*10-1,3,3,1) end
    elseif key=="InfJump" then
        r(10,0,8,8,2); r(6,8,16,4,2); r(2,12,24,4,2); r(6,16,16,8,4)
    elseif key=="SlopeBoost" then
        r(0,20,28,4,2); r(4,14,12,6,2); r(12,8,12,6,2); r(18,2,10,6,2)
    else
        r(8,4,12,4,2); r(4,10,20,8,4); r(8,18,12,8,2)
    end
    return ic
end
...
--  UI PLAYER TAB
-- ══════════════════════════════════════════════════════════
local function makePlRow_Slider(page, icon_key, cor, lbl_txt, desc_txt, minV, maxV, step, initVal, onChange)
    local lo = 0
    for _, c in pairs(page:GetChildren()) do if c:IsA("Frame") or c:IsA("TextButton") then lo = lo + 1 end end
    local row=Instance.new("Frame",page); row.BackgroundColor3=Color3.fromRGB(28,30,38)
    row.BorderSizePixel=0; row.Size=UDim2.new(1,0,0,68); row.LayoutOrder=lo+10; row.ZIndex=5
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,9)
    ...
    gb.BorderSizePixel=0; gb.Size=UDim2.new(1,0,1,0); gb.ZIndex=5; Instance.new("UICorner",gb).CornerRadius=UDim.new(0,9)
    ...
    local ic=criarIconePlayer(ib,icon_key,cor); ic.Position=UDim2.new(0,4,0,4)
    ...
    tl.Position=UDim2.new(0,54,0,8); tl.Size=UDim2.new(1,-60,0,16); tl.Font=Enum.Font.GothamBold
    tl.Text=lbl_txt; tl.TextColor3=Color3.fromRGB(220,225,240); tl.TextSize=11; tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=7
    local td=Instance.new("TextLabel",row); td.BackgroundTransparency=1
    td.Position=UDim2.new(0,54,0,26); td.Size=UDim2.new(1,-60,0,14); td.Font=Enum.Font.Gotham
    ...
    local ctrl=Instance.new("Frame",row); ctrl.BackgroundColor3=Color3.fromRGB(16,18,26); ctrl.BackgroundTransparency=0.15
    ctrl.BorderSizePixel=0; ctrl.Position=UDim2.new(0,54,0,42); ctrl.Size=UDim2.new(1,-66,0,20); ctrl.ZIndex=8
    ...
    valLbl.Position=UDim2.new(0.5,-20,0,0); valLbl.Size=UDim2.new(0,40,1,0); valLbl.Font=Enum.Font.GothamBold
    ...
    for i, bd in ipairs(btns) do
        ...
        b.BorderSizePixel=0; b.Size=UDim2.new(0,30,1,0)
        b.Position=i<=2 and UDim2.new(0,(i-1)*31,0,0) or UDim2.new(1,-(4-i+1)*31,0,0)
        b.Font=Enum.Font.GothamBold; b.Text=bd[1]; b.TextColor3=Color3.fromRGB(255,255,255); b.TextSize=8; b.ZIndex=9
        Instance.new("UICorner",b).CornerRadius=UDim.new(0,5)
        local delta=bd[2]
        b.MouseButton1Click:Connect(function() upd(curV[1]+delta) end)
...
local function makePlRow_Toggle(page, icon_key, cor, lbl_txt, desc_txt, onToggle, extraSlider)
    local lo = 0
    for _, c in pairs(page:GetChildren()) do if c:IsA("Frame") or c:IsA("TextButton") then lo = lo + 1 end end
    local row=Instance.new("Frame",page); row.BackgroundColor3=Color3.fromRGB(28,30,38)
    row.BorderSizePixel=0; row.Size=UDim2.new(1,0,0,58); row.LayoutOrder=lo+10; row.ZIndex=5
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,9)
    local rowS=Instance.new("UIStroke",row); rowS.Color=Color3.fromRGB(42,46,58); rowS.Thickness=1
    local gb=Instance.new("Frame",row); gb.BackgroundColor3=cor; gb.BackgroundTransparency=0.9
    gb.BorderSizePixel=0; gb.Size=UDim2.new(1,0,1,0); gb.ZIndex=5; Instance.new("UICorner",gb).CornerRadius=UDim.new(0,9)
    ...
    bl.Position=UDim2.new(0,0,0.15,0); bl.Size=UDim2.new(0,3,0.7,0); bl.ZIndex=8
    Instance.new("UICorner",bl).CornerRadius=UDim.new(0,2)
    local pillRight = extraSlider and 118 or 60
    local tl=Instance.new("TextLabel",row); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,52,0,8); tl.Size=UDim2.new(1,-52-pillRight,0,16); tl.Font=Enum.Font.GothamBold
    tl.Text=lbl_txt; tl.TextColor3=Color3.fromRGB(220,225,240); tl.TextSize=11; tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=7
    local td=Instance.new("TextLabel",row); td.BackgroundTransparency=1
    td.Position=UDim2.new(0,52,0,26); td.Size=UDim2.new(1,-52-pillRight,0,24); td.Font=Enum.Font.Gotham
    ...
    if extraSlider then
        local esl=extraSlider
        local esf=Instance.new("Frame",row); esf.BackgroundColor3=Color3.fromRGB(16,18,26); esf.BackgroundTransparency=0.15
        esf.BorderSizePixel=0; esf.Position=UDim2.new(1,-pillRight,0.5,-11); esf.Size=UDim2.new(0,56,0,22); esf.ZIndex=9
        Instance.new("UICorner",esf).CornerRadius=UDim.new(0,6)
        Instance.new("UIStroke",esf).Color=cor
        local esm=Instance.new("TextButton",esf); esm.BackgroundTransparency=1; esm.Position=UDim2.new(0,2,0,0); esm.Size=UDim2.new(0,14,1,0)
        esm.Font=Enum.Font.GothamBlack; esm.Text="-"; esm.TextColor3=cor; esm.TextSize=13; esm.ZIndex=10
        local esv=Instance.new("TextLabel",esf); esv.BackgroundTransparency=1; esv.Position=UDim2.new(0,16,0,0); esv.Size=UDim2.new(1,-30,1,0)
        esv.Font=Enum.Font.GothamBold; esv.Text=tostring(esl.val[1]); esv.TextColor3=Color3.fromRGB(255,255,255); esv.TextSize=9; esv.ZIndex=10
        local esp=Instance.new("TextButton",esf); esp.BackgroundTransparency=1; esp.Position=UDim2.new(1,-14,0,0); esp.Size=UDim2.new(0,14,1,0)
        esp.Font=Enum.Font.GothamBlack; esp.Text="+"; esp.TextColor3=cor; esp.TextSize=13; esp.ZIndex=10
        esm.MouseButton1Click:Connect(function() esl.val[1]=math.max(esl.min,esl.val[1]-esl.step); esv.Text=tostring(esl.val[1]); if esl.onChange then esl.onChange(esl.val[1]) end end)
        esp.MouseButton1Click:Connect(function() esl.val[1]=math.min(esl.max,esl.val[1]+esl.step); esv.Text=tostring(esl.val[1]); if esl.onChange then esl.onChange(esl.val[1]) end end)
    end
    local pill=Instance.new("Frame",row); pill.BackgroundColor3=Color3.fromRGB(45,50,62); pill.BorderSizePixel=0
    pill.Position=UDim2.new(1,-54,0.5,-12); pill.Size=UDim2.new(0,46,0,24); pill.ZIndex=9
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",pill); knob.BackgroundColor3=Color3.fromRGB(160,170,185); knob.BorderSizePixel=0
    knob.Position=UDim2.new(0,2,0.5,-10); knob.Size=UDim2.new(0,20,0,20); knob.ZIndex=10
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local estado=false
    local btn=Instance.new("TextButton",row); btn.BackgroundTransparency=1; btn.Size=UDim2.new(1,0,1,0); btn.Text=""; btn.ZIndex=11
    btn.MouseEnter:Connect(function() TweenService:Create(row,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(33,36,46)}):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(row,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(28,30,38)}):Play() end)
    btn.MouseButton1Click:Connect(function()
        estado=not estado
...
local function makePlSec(page, titulo, cor)
    local lo = 0
    for _, c in pairs(page:GetChildren()) do if c:IsA("Frame") or c:IsA("TextButton") then lo = lo + 1 end end
    local hdr=Instance.new("Frame",page); hdr.BackgroundColor3=Color3.fromRGB(20,22,30)
    hdr.BackgroundTransparency=0.3; hdr.BorderSizePixel=0; hdr.Size=UDim2.new(1,0,0,22); hdr.LayoutOrder=lo+10; hdr.ZIndex=4
    Instance.new("UICorner",hdr).CornerRadius=UDim.new(0,6)
    local bar=Instance.new("Frame",hdr); bar.BackgroundColor3=cor; bar.BorderSizePixel=0
    bar.Position=UDim2.new(0,0,0,0); bar.Size=UDim2.new(0,3,1,0); bar.ZIndex=5
    Instance.new("UICorner",bar).CornerRadius=UDim.new(0,3)
...
-- ══ BUILD ABA PLAYER ══
local flySpeedRef = {val={40}, min=5, max=300, step=5}

makePlSec(Pages["Player"], "🎮 CÂMERA & MUNDO", Color3.fromRGB(180,120,255))
makePlRow_Toggle(Pages["Player"], "InfZoom", Color3.fromRGB(180,120,255), "🔭 Infinito Zoom",
    "Zoom da câmera ilimitado — vai até qualquer distância",
    function(s) setInfZoom(s) end)
makePlRow_Toggle(Pages["Player"], "CamLivre", Color3.fromRGB(140,180,255), "👁️ Câmera Livre",
    "Câmera atravessa paredes, árvores e objetos",
    function(s) setCamLivre(s) end)
makePlRow_Toggle(Pages["Player"], "FullBright", Color3.fromRGB(255,240,80), "☀️ FullBright",
    "Noite vira dia — remove chuva, neblina, escuridão",
    function(s) setFullBright(s) end)

makePlSec(Pages["Player"], "⚡ MOVIMENTO", Color3.fromRGB(255,200,50))
makePlRow_Toggle(Pages["Player"], "InfJump", Color3.fromRGB(120,255,180), "🚀 Infinite Jump",
    "Pula infinitamente no ar — sem limite de altura",
    function(s) setInfJump(s) end)
makePlRow_Toggle(Pages["Player"], "SlopeBoost", Color3.fromRGB(255,140,40), "🛡️ Slope Boost",
    "Imune a armadilhas de desaceleração — velocidade sempre constante",
    function(s) setSlopeBoost(s) end)
makePlRow_Slider(Pages["Player"], "Speed", Color3.fromRGB(255,180,30), "⚡ Speed",
    "Velocidade | padrão:16 | min:1 | max:500 | sistema inovador ±1/±5/±10/±50",
    1, 500, 1, 30, function(v) playerSpeed=v; applySpeed(v) end)
makePlRow_Slider(Pages["Player"], "JumpPower", Color3.fromRGB(100,220,255), "🦘 Jump Power",
    "Força do pulo | padrão:50 | min:1 | max:999 | sistema inovador",
    1, 999, 1, 80, function(v) playerJump=v; applyJump(v) end)

makePlSec(Pages["Player"], "✈️ VOO & NOCLIP", Color3.fromRGB(100,200,255))
makePlRow_Toggle(Pages["Player"], "Fly", Color3.fromRGB(80,180,255), "✈️ Fly",
    "W/A/S/D mover • Espaço=subir • Ctrl=descer • [-][+] = velocidade",
    function(s) setFly(s) end, flySpeedRef)
makePlRow_Toggle(Pages["Player"], "Noclip", Color3.fromRGB(140,255,140), "👻 Noclip",
    "Atravessa paredes • Não atravessa o chão • Anti-void Y=-100",
    function(s) setNoclip(s) end)

makePlSec(Pages["Player"], "🔧 UTILIDADES", Color3.fromRGB(255,210,80))
makePlRow_Toggle(Pages["Player"], "TpClick", Color3.fromRGB(255,220,60), "⚡ TP Click",
    "Clique em qualquer lugar para teleportar instantaneamente",
    function(s) setTpClick(s) end)
makePlRow_Toggle(Pages["Player"], "BauANC", Color3.fromRGB(210,160,80), "📦 Baú ANC",
    "Remove animação dos baús — abrem instantaneamente",
...
local ANIMAL_NAMES = {
    ...
    "bunny","horse","kiwi","turkey",
    "alien","elite alien","elitealien","cultist","mega cultist","megacultist","cultist king","cultistking"
}
local ANIMAL_SET = {}
...
-- Função para achar mob mais próximo dentro de um raio de tela (para FOV)
local function findNearestAnimalHrpInFOV(fovRadius)
    local ch = Player.Character; if not ch then return nil end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return nil end
    local myPos = hrp.Position
    local best, bestDist = nil, math.huge
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            ...
            local anHrp = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChildWhichIsA("BasePart")
            if not anHrp then return end
            local screenPos, onScreen = workspace.CurrentCamera:WorldToViewportPoint(anHrp.Position)
            if not onScreen then return end
            local distFromCenter = Vector2.new(screenPos.X - camCenter.X, screenPos.Y - camCenter.Y).Magnitude
            if distFromCenter > fovRadius then return end
            local d = (anHrp.Position - myPos).Magnitude
            if d < bestDist then bestDist=d; best=anHrp end
        end)
    end
...
local function createFOVCircle()
    ...
    circle.ZIndex = 500
    local size = aimbotFOVRadius * 2
    circle.Size = UDim2.new(0, size, 0, size)
    circle.Position = UDim2.new(0.5, -aimbotFOVRadius, 0.5, -aimbotFOVRadius)
    Instance.new("UICorner", circle).CornerRadius = UDim.new(1, 0)
    local stroke = Instance.new("UIStroke", circle)
    stroke.Color = Color3.fromRGB(255, 80, 80); stroke.Thickness = 1.5
    -- Pontinho central
    local dot = Instance.new("Frame", circle)
    dot.BackgroundColor3 = Color3.fromRGB(255,80,80); dot.BorderSizePixel = 0
    dot.Position = UDim2.new(0.5, -3, 0.5, -3); dot.Size = UDim2.new(0, 6, 0, 6); dot.ZIndex = 501
...
-- AIMBOT normal: redireciona projéteis
workspace.DescendantAdded:Connect(function(obj)
    if not aimbotEnabled and not aimbotFOVEnabled then return end
    task.defer(function()
        if not obj or not obj.Parent or not obj:IsA("BasePart") then return end
        ...
        local anHrp = aimbotFOVEnabled and findNearestAnimalHrpInFOV(aimbotFOVRadius) or findNearestAnimalHrp()
        if not anHrp then return end
        local steps = 0
        local conn; conn = RunService.Heartbeat:Connect(function()
            steps+=1
            if (not aimbotEnabled and not aimbotFOVEnabled) or not obj or not obj.Parent or steps>120 then conn:Disconnect(); return end
...
local RANGED_WEAPONS = {
    ...
    "laser cannon","lasercannon","flamethrower","crossbow","infernal crossbow","infernalcrossbow",
    "blowpipe","air rifle","airrifle","snowball"
}
local RANGED_SET = {}
    ...
    task.spawn(function()
        while aimbotAutoEnabled do
            ...
            pcall(function()
                ...
                pcall(function()
                    local activate = tool:FindFirstChild("Activate") or tool:FindFirstChild("Fire") or tool:FindFirstChild("Shoot")
                    if activate and activate:IsA("RemoteEvent") then activate:FireServer(anHrp.Position) end
                end)
                pcall(function()
...
-- ══════════════════════════════════════════════════════════
--  UI AVANÇADO TAB (AvancadoFuncoes) — com ícones
-- ══════════════════════════════════════════════════════════
local function criarIconeAvancado(parent, key, cor)
    local ic=Instance.new("Frame",parent); ic.BackgroundTransparency=1; ic.BorderSizePixel=0
    ic.Position=UDim2.new(0,5,0,5); ic.Size=UDim2.new(0,28,0,28); ic.ZIndex=parent.ZIndex+1
    local function r(x,y,w,h,rad) return mkRect(ic,x,y,w,h,cor,rad) end
    local function c(x,y,rad) return mkCircle(ic,x,y,rad,cor) end
    if key=="Aimbot" then
        c(14,14,10); local inner=mkCircle(ic,14,14,7,Color3.fromRGB(20,22,30)); inner.ZIndex=ic.ZIndex+2
        c(14,14,3); r(13,0,2,28,1); r(0,13,28,2,1)
    elseif key=="AimbotFOV" then
        c(14,14,12); local inner=mkCircle(ic,14,14,9,Color3.fromRGB(20,22,30)); inner.ZIndex=ic.ZIndex+2
        c(14,14,4); local inner2=mkCircle(ic,14,14,2,Color3.fromRGB(20,22,30)); inner2.ZIndex=ic.ZIndex+3
        r(13,1,2,6,1); r(13,21,2,6,1); r(1,13,6,2,1); r(21,13,6,2,1)
    elseif key=="AimbotAuto" then
        r(2,10,24,4,2); c(14,6,5); local ci=mkCircle(ic,14,6,3,Color3.fromRGB(20,22,30)); ci.ZIndex=ic.ZIndex+2
        r(8,14,12,10,4)
    else
        r(4,4,20,20,5); local fi=mkRect(ic,5,5,18,18,Color3.fromRGB(20,22,30),4); fi.ZIndex=ic.ZIndex+2
        r(10,10,8,8,3)
    end
    return ic
end
...
local function makeAvSec(titulo, cor)
    ...
    hdr.BackgroundTransparency=0.3; hdr.BorderSizePixel=0; hdr.Size=UDim2.new(1,0,0,22); hdr.LayoutOrder=avLO(); hdr.ZIndex=4
    Instance.new("UICorner",hdr).CornerRadius=UDim.new(0,6)
    local bar=Instance.new("Frame",hdr); bar.BackgroundColor3=cor; bar.BorderSizePixel=0
    bar.Position=UDim2.new(0,0,0,0); bar.Size=UDim2.new(0,3,1,0); bar.ZIndex=5
    Instance.new("UICorner",bar).CornerRadius=UDim.new(0,3)
...
local function makeAvRow_Toggle(icon_key, cor, lbl_txt, desc_txt, onToggle)
    local row=Instance.new("Frame",Pages["AvancadoFuncoes"]); row.BackgroundColor3=Color3.fromRGB(28,30,38)
    row.BorderSizePixel=0; row.Size=UDim2.new(1,0,0,68); row.LayoutOrder=avLO(); row.ZIndex=5
    Instance.new("UICorner",row).CornerRadius=UDim.new(0,9)
    local rowS=Instance.new("UIStroke",row); rowS.Color=Color3.fromRGB(42,46,58); rowS.Thickness=1
    local gb=Instance.new("Frame",row); gb.BackgroundColor3=cor; gb.BackgroundTransparency=0.9
    gb.BorderSizePixel=0; gb.Size=UDim2.new(1,0,1,0); gb.ZIndex=5; Instance.new("UICorner",gb).CornerRadius=UDim.new(0,9)
    local ib=Instance.new("Frame",row); ib.BackgroundColor3=cor; ib.BackgroundTransparency=0.78
    ib.BorderSizePixel=0; ib.Position=UDim2.new(0,8,0.5,-19); ib.Size=UDim2.new(0,38,0,38); ib.ZIndex=7
    Instance.new("UICorner",ib).CornerRadius=UDim.new(0,9)
    criarIconeAvancado(ib,icon_key,cor)
    local bl=Instance.new("Frame",row); bl.BackgroundColor3=cor; bl.BorderSizePixel=0
    bl.Position=UDim2.new(0,0,0.15,0); bl.Size=UDim2.new(0,3,0.7,0); bl.ZIndex=8
    Instance.new("UICorner",bl).CornerRadius=UDim.new(0,2)
    local tl=Instance.new("TextLabel",row); tl.BackgroundTransparency=1
    tl.Position=UDim2.new(0,56,0,6); tl.Size=UDim2.new(1,-120,0,18); tl.Font=Enum.Font.GothamBold
    tl.Text=lbl_txt; tl.TextColor3=Color3.fromRGB(225,230,248); tl.TextSize=12; tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=7
    local td=Instance.new("TextLabel",row); td.BackgroundTransparency=1
    td.Position=UDim2.new(0,56,0,26); td.Size=UDim2.new(1,-120,0,36); td.Font=Enum.Font.Gotham
    td.Text=desc_txt; td.TextColor3=Color3.fromRGB(90,100,120); td.TextSize=9
    td.TextXAlignment=Enum.TextXAlignment.Left; td.TextWrapped=true; td.ZIndex=7
    local statusDot=Instance.new("Frame",row); statusDot.BackgroundColor3=Color3.fromRGB(80,90,110)
    statusDot.BorderSizePixel=0; statusDot.Position=UDim2.new(1,-68,0.5,-4); statusDot.Size=UDim2.new(0,8,0,8); statusDot.ZIndex=9
    Instance.new("UICorner",statusDot).CornerRadius=UDim.new(1,0)
    local pill=Instance.new("Frame",row); pill.BackgroundColor3=Color3.fromRGB(45,50,62); pill.BorderSizePixel=0
    pill.Position=UDim2.new(1,-56,0.5,-12); pill.Size=UDim2.new(0,48,0,24); pill.ZIndex=9
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",pill); knob.BackgroundColor3=Color3.fromRGB(160,170,185); knob.BorderSizePixel=0
    knob.Position=UDim2.new(0,2,0.5,-10); knob.Size=UDim2.new(0,20,0,20); knob.ZIndex=10
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(rowS,TweenInfo.new(0.2),{Color=estado and cor or Color3.fromRGB(42,46,58)}):Play()
        TweenService:Create(statusDot,TweenInfo.new(0.2),{BackgroundColor3=estado and Color3.fromRGB(87,242,135) or Color3.fromRGB(80,90,110)}):Play()
        onToggle(estado)
    end)
end

local function makeAvInfoBox(txt, cor)
    local f=Instance.new("Frame",Pages["AvancadoFuncoes"]); f.BackgroundColor3=Color3.fromRGB(14,20,35)
    f.BackgroundTransparency=0.5; f.BorderSizePixel=0; f.Size=UDim2.new(1,0,0,48); f.LayoutOrder=avLO(); f.ZIndex=4
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,8)
    local fs=Instance.new("UIStroke",f); fs.Color=cor; fs.Thickness=1
    local ft=Instance.new("TextLabel",f); ft.BackgroundTransparency=1; ft.Position=UDim2.new(0,8,0,4)
    ft.Size=UDim2.new(1,-16,1,-8); ft.Font=Enum.Font.Gotham; ft.Text=txt
    ft.TextColor3=Color3.fromRGB(100,175,255); ft.TextSize=8; ft.TextWrapped=true
...
makeAvRow_Toggle("Aimbot", Color3.fromRGB(255,80,80), "🎯 Aimbot (Teleguiado)",
    "Projéteis se movem automaticamente para o animal mais próximo. 100% de acerto em qualquer direção.",
    function(s) aimbotEnabled = s end)

makeAvRow_Toggle("AimbotFOV", Color3.fromRGB(255,50,120), "🔴 Aimbot FOV (Círculo)",
    "Cria círculo no centro da tela. Projéteis miram no mob dentro do FOV. 100% precisão.",
    function(s)
        aimbotFOVEnabled = s
        if s then
            createFOVCircle()
            showNotif("Aimbot FOV ativado! Círculo visível 🔴", "sucesso")
        else
            removeFOVCircle()
        end
    end)

makeAvRow_Toggle("AimbotAuto", Color3.fromRGB(255,140,40), "🤖 Aimbot AUTO (Disparo automático)",
    "Com arma ranged equipada: atira sozinho nos animais mais próximos sem precisar clicar.",
    function(s)
        aimbotAutoEnabled = s
        if s then startAimbotAuto() end
    end)

makeAvInfoBox("ℹ️  AIMBOT: Funciona com todas as armas de projéteis (Revolver, Rifle, Crossbow, Flamethrower, etc).\nAimbot FOV: círculo pequeno no centro — só mira mobs dentro do FOV.\n⚠️  Aimbot AUTO requer VirtualInputManager.", Color3.fromRGB(80,140,255))


-- ══════════════════════════════════════════════════════════
--  ABA TELEPORTE — BIOMAS (99 Dias na Floresta 2026)
-- ══════════════════════════════════════════════════════════
-- Adicionar nova aba ao sidebar e ao sistema de pages

-- Criar page de Teleport
local tpPage = Instance.new("ScrollingFrame", ContentArea)
tpPage.Name = "TeleportPage"
tpPage.Size = UDim2.new(1,0,1,0)
tpPage.BackgroundTransparency = 1
tpPage.Visible = false
tpPage.ScrollBarThickness = 3
tpPage.ScrollBarImageColor3 = C_ACCENT
tpPage.BorderSizePixel = 0
tpPage.AutomaticCanvasSize = Enum.AutomaticSize.Y
tpPage.CanvasSize = UDim2.new(0,0,0,0)
tpPage.ZIndex = 4
local tpPad = Instance.new("UIPadding", tpPage)
tpPad.PaddingTop = UDim.new(0,14); tpPad.PaddingLeft = UDim.new(0,14)
tpPad.PaddingRight = UDim.new(0,14); tpPad.PaddingBottom = UDim.new(0,14)
local tpList = Instance.new("UIListLayout", tpPage)
tpList.Padding = UDim.new(0,10); tpList.SortOrder = Enum.SortOrder.LayoutOrder
Pages["Teleport"] = tpPage

-- Adicionar tab Teleport no sidebar (grupo EXTRA)
do
    ...
    bg.Size = UDim2.new(1,0,0,36); bg.LayoutOrder = order; bg.ZIndex = 4; bg.ClipsDescendants = true
    Instance.new("UICorner", bg).CornerRadius = UDim.new(0,7)
    local bar = Instance.new("Frame", bg); bar.BackgroundColor3 = Color3.fromRGB(255,200,80); bar.BorderSizePixel = 0
    bar.Position = UDim2.new(0,0,0.2,0); bar.Size = UDim2.new(0,3,0.6,0); bar.Visible = false; bar.ZIndex = 6
    Instance.new("UICorner", bar).CornerRadius = UDim.new(0,2)
    -- Ícone custom (bússola/mapa)
    local icCont = Instance.new("Frame", bg); icCont.BackgroundTransparency = 1; icCont.BorderSizePixel = 0
    icCont.Position = UDim2.new(0,8,0.5,-10); icCont.Size = UDim2.new(0,20,0,20); icCont.ZIndex = bg.ZIndex+2
    mkRect(icCont,8,0,4,20,C_ICON_IDLE,2); mkRect(icCont,0,8,20,4,C_ICON_IDLE,2); mkCircle(icCont,10,10,4,C_ICON_IDLE)
    local label = Instance.new("TextLabel", bg); label.BackgroundTransparency = 1
    label.Position = UDim2.new(0,37,0,0); label.Size = UDim2.new(1,-42,1,0)
    label.Font = Enum.Font.GothamSemibold; label.Text = "Teleporte"; label.TextColor3 = C_TEXT_OFF
    label.TextSize = 11; label.TextXAlignment = Enum.TextXAlignment.Left
    label.TextTruncate = Enum.TextTruncate.AtEnd; label.ZIndex = 6
    local btn = Instance.new("TextButton", bg); btn.BackgroundTransparency = 1
    btn.Size = UDim2.new(1,0,1,0); btn.Text = ""; btn.ZIndex = 7
    local tpEntry = {key="Teleport", bg=bg, iconParts={}, label=label, bar=bar}
    table.insert(allTabs, tpEntry)
    btn.MouseEnter:Connect(function()
        if currentTab~="Teleport" then TweenService:Create(bg,TweenInfo.new(0.15),{BackgroundTransparency=0.78}):Play() end
    end)
    btn.MouseLeave:Connect(function()
        if currentTab~="Teleport" then TweenService:Create(bg,TweenInfo.new(0.15),{BackgroundTransparency=1}):Play() end
    end)
    btn.MouseButton1Click:Connect(function() selectTab("Teleport") end)
end

-- ══ BIOMAS DO 99 DIAS NA FLORESTA (2026, biomas disponíveis no jogo) ══
local BIOMES = {
    {
        nome = "Floresta",
        emoji = "🌲",
        cor = Color3.fromRGB(60,180,80),
        corHex = Color3.fromRGB(20,80,30),
        desc = "Bioma inicial — floresta densa com lobos e ursos",
        -- Coordenadas aproximadas baseadas no jogo
        pos = Vector3.new(0, 10, 0),
        searchNames = {"Forest","Floresta","SpawnArea","Spawn","GreenArea"}
    },
    {
        nome = "Ártico",
        emoji = "❄️",
        cor = Color3.fromRGB(140,200,255),
        corHex = Color3.fromRGB(40,80,140),
        desc = "Bioma de neve — mamutes, raposas árticas e ursos polares",
        pos = Vector3.new(800, 10, -200),
        searchNames = {"Arctic","Artic","Snow","Ice","Tundra","Polar"}
    },
    {
        nome = "Vulcão",
        emoji = "🌋",
        cor = Color3.fromRGB(255,100,30),
        corHex = Color3.fromRGB(120,30,0),
        desc = "Bioma vulcânico — hellephants e enguias de lava",
        pos = Vector3.new(-600, 10, 600),
        searchNames = {"Volcano","Volcanic","Lava","Hell","Fire","Vulcao"}
    },
    {
        nome = "Pântano",
        emoji = "🐸",
        cor = Color3.fromRGB(100,160,80),
        corHex = Color3.fromRGB(30,60,20),
        desc = "Bioma pântano — sapos coloridos e plantas raras",
        pos = Vector3.new(400, 10, 800),
        searchNames = {"Swamp","Pantano","Frog","Marsh","Bog","Wetland"}
    },
}

local selectedBiome = nil

-- Função de TP para bioma
local function tpToBiome(biome)
    local ch = Player.Character; if not ch then showNotif("Personagem não encontrado!", "erro") return end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then showNotif("Erro de teleporte!", "erro") return end
    
    -- Tentar achar o bioma no workspace
    local foundPos = nil
    for _, searchName in ipairs(biome.searchNames) do
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                if obj.Name:lower():find(searchName:lower()) then
                    if obj:IsA("BasePart") or obj:IsA("Model") then
                        ...
                        if part then
                            ...
                            if p.Magnitude < 10000 then
                                foundPos = Vector3.new(p.X, p.Y + 5, p.Z)
                            end
                        end
                    end
                end
    ...
    if foundPos then
        hrp.CFrame = CFrame.new(foundPos)
        showNotif("Teleportado para " .. biome.emoji .. " " .. biome.nome .. "!", "sucesso")
    else
        -- Usar coordenadas padrão
        hrp.CFrame = CFrame.new(biome.pos + Vector3.new(0, 5, 0))
        showNotif("Bioma não encontrado. Explore o mapa primeiro!", "aviso")
    end
end

-- ══ UI PAINEL BIOMAS ══
local tpLO = 0
local function tplo() tpLO+=1; return tpLO end

-- Header Biomes
local biomesPanel = Instance.new("Frame", tpPage)
biomesPanel.BackgroundColor3 = Color3.fromRGB(22,24,30)
biomesPanel.BorderSizePixel = 0
biomesPanel.Size = UDim2.new(1,0,0,58)
biomesPanel.LayoutOrder = tplo()
biomesPanel.ZIndex = 5
Instance.new("UICorner", biomesPanel).CornerRadius = UDim.new(0,12)
local bpStroke2 = Instance.new("UIStroke", biomesPanel); bpStroke2.Color = Color3.fromRGB(255,200,80); bpStroke2.Thickness = 1.5

-- Título BIOMES
local biomesTitle = Instance.new("TextLabel", biomesPanel)
biomesTitle.BackgroundTransparency = 1
biomesTitle.Position = UDim2.new(0,14,0,0); biomesTitle.Size = UDim2.new(1,-14,0,28)
biomesTitle.Font = Enum.Font.GothamBlack; biomesTitle.Text = "🗺️  BIOMES"
biomesTitle.TextColor3 = Color3.fromRGB(255,200,80); biomesTitle.TextSize = 14
biomesTitle.TextXAlignment = Enum.TextXAlignment.Left; biomesTitle.ZIndex = 6

local biomesSubtitle = Instance.new("TextLabel", biomesPanel)
biomesSubtitle.BackgroundTransparency = 1
biomesSubtitle.Position = UDim2.new(0,14,0,28); biomesSubtitle.Size = UDim2.new(1,-14,0,22)
biomesSubtitle.Font = Enum.Font.Gotham; biomesSubtitle.Text = "Selecione um bioma e teleporte instantaneamente"
biomesSubtitle.TextColor3 = Color3.fromRGB(100,110,130); biomesSubtitle.TextSize = 9
biomesSubtitle.TextXAlignment = Enum.TextXAlignment.Left; biomesSubtitle.ZIndex = 6

-- Área de seleção de bioma
local selectRow = Instance.new("Frame", tpPage)
selectRow.BackgroundColor3 = Color3.fromRGB(28,30,38)
selectRow.BorderSizePixel = 0; selectRow.Size = UDim2.new(1,0,0,52); selectRow.LayoutOrder = tplo(); selectRow.ZIndex = 5
Instance.new("UICorner", selectRow).CornerRadius = UDim.new(0,10)
Instance.new("UIStroke", selectRow).Color = Color3.fromRGB(50,55,70)

local selectLabel = Instance.new("TextLabel", selectRow)
selectLabel.BackgroundTransparency = 1; selectLabel.Position = UDim2.new(0,14,0,0)
selectLabel.Size = UDim2.new(0,60,1,0); selectLabel.Font = Enum.Font.GothamSemibold
selectLabel.Text = "Selecionar"; selectLabel.TextColor3 = Color3.fromRGB(150,160,180); selectLabel.TextSize = 10
selectLabel.TextXAlignment = Enum.TextXAlignment.Left; selectLabel.ZIndex = 6

-- Botão dropdown de biomas
local dropBtn = Instance.new("TextButton", selectRow)
dropBtn.BackgroundColor3 = Color3.fromRGB(40,44,56)
dropBtn.BorderSizePixel = 0; dropBtn.Position = UDim2.new(0,78,0.5,-16); dropBtn.Size = UDim2.new(1,-86,0,32)
dropBtn.Font = Enum.Font.GothamBold; dropBtn.Text = "🗺️  Biomes ▾"; dropBtn.TextColor3 = Color3.fromRGB(200,210,255)
dropBtn.TextSize = 11; dropBtn.ZIndex = 7
Instance.new("UICorner", dropBtn).CornerRadius = UDim.new(0,8)
Instance.new("UIStroke", dropBtn).Color = Color3.fromRGB(88,101,242)

-- Dropdown popup
local dropMenu = Instance.new("Frame", ScreenGui)
dropMenu.Name = "BiomeDropdown"
dropMenu.BackgroundColor3 = Color3.fromRGB(20,22,30)
dropMenu.BorderSizePixel = 0; dropMenu.Visible = false; dropMenu.ZIndex = 600
dropMenu.Size = UDim2.new(0, 0, 0, 0); dropMenu.ClipsDescendants = true
Instance.new("UICorner", dropMenu).CornerRadius = UDim.new(0,12)
local dmStroke = Instance.new("UIStroke", dropMenu); dmStroke.Color = Color3.fromRGB(88,101,242); dmStroke.Thickness = 1.5
local dmList = Instance.new("UIListLayout", dropMenu)
dmList.Padding = UDim.new(0,4); dmList.SortOrder = Enum.SortOrder.LayoutOrder
dmList.HorizontalAlignment = Enum.HorizontalAlignment.Center
local dmPad = Instance.new("UIPadding", dropMenu)
dmPad.PaddingTop = UDim.new(0,8); dmPad.PaddingLeft = UDim.new(0,8)
dmPad.PaddingRight = UDim.new(0,8); dmPad.PaddingBottom = UDim.new(0,8)

local dropOpen = false
local function closeDropdown()
    dropOpen = false
    TweenService:Create(dropMenu, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {Size = UDim2.new(0,200,0,0)}):Play()
    task.delay(0.21, function() dropMenu.Visible = false end)
    TweenService:Create(dropBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(40,44,56)}):Play()
end
...
local function openDropdown()
    ...
    dropMenu.Position = UDim2.new(0, abs.X, 0, abs.Y + absSize.Y + 6)
    dropMenu.Size = UDim2.new(0, 200, 0, 0)
    dropMenu.Visible = true
    local targetH = #BIOMES * 50 + 24
    TweenService:Create(dropMenu, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0,200,0,targetH)}):Play()
    TweenService:Create(dropBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(60,65,90)}):Play()
end

-- Forward declaration para usar dentro do dropdown
local tpBiomeBtn

-- Criar itens do dropdown
for _, biome in ipairs(BIOMES) do
    local item = Instance.new("TextButton", dropMenu)
    item.BackgroundColor3 = Color3.fromRGB(28,32,42)
    item.BorderSizePixel = 0; item.Size = UDim2.new(1,0,0,42); item.ZIndex = 601
    Instance.new("UICorner", item).CornerRadius = UDim.new(0,8)
    -- Cor de fundo com cor do bioma
    local itemBg = Instance.new("Frame", item); itemBg.BackgroundColor3 = biome.cor
    itemBg.BackgroundTransparency = 0.88; itemBg.BorderSizePixel = 0; itemBg.Size = UDim2.new(1,0,1,0); itemBg.ZIndex = 601
    Instance.new("UICorner", itemBg).CornerRadius = UDim.new(0,8)
    local barre = Instance.new("Frame", item); barre.BackgroundColor3 = biome.cor; barre.BorderSizePixel = 0
    barre.Position = UDim2.new(0,0,0.15,0); barre.Size = UDim2.new(0,3,0.7,0); barre.ZIndex = 603
    Instance.new("UICorner", barre).CornerRadius = UDim.new(0,2)
    local emojiLbl = Instance.new("TextLabel", item)
    emojiLbl.BackgroundTransparency = 1; emojiLbl.Position = UDim2.new(0,8,0,0)
    emojiLbl.Size = UDim2.new(0,28,1,0); emojiLbl.Font = Enum.Font.GothamBold
    emojiLbl.Text = biome.emoji; emojiLbl.TextSize = 16; emojiLbl.ZIndex = 602
    emojiLbl.TextColor3 = Color3.fromRGB(255,255,255)
    local nameLbl = Instance.new("TextLabel", item)
    nameLbl.BackgroundTransparency = 1; nameLbl.Position = UDim2.new(0,38,0,4)
    nameLbl.Size = UDim2.new(1,-46,0,18); nameLbl.Font = Enum.Font.GothamBold
    nameLbl.Text = biome.nome; nameLbl.TextColor3 = biome.cor; nameLbl.TextSize = 12
    nameLbl.TextXAlignment = Enum.TextXAlignment.Left; nameLbl.ZIndex = 602
    local descLbl = Instance.new("TextLabel", item)
    descLbl.BackgroundTransparency = 1; descLbl.Position = UDim2.new(0,38,0,22)
    descLbl.Size = UDim2.new(1,-46,0,14); descLbl.Font = Enum.Font.Gotham
    descLbl.Text = biome.desc; descLbl.TextColor3 = Color3.fromRGB(100,115,135); descLbl.TextSize = 8
    descLbl.TextXAlignment = Enum.TextXAlignment.Left; descLbl.ZIndex = 602
    item.Text = ""; item.ZIndex = 605
    item.MouseEnter:Connect(function()
        TweenService:Create(item, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(35,40,55)}):Play()
    end)
    item.MouseLeave:Connect(function()
        TweenService:Create(item, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(28,32,42)}):Play()
    end)
    local biomeRef = biome
    item.MouseButton1Click:Connect(function()
        selectedBiome = biomeRef
        -- Atualiza botão dropdown com nome do bioma selecionado
        TweenService:Create(dropBtn, TweenInfo.new(0.2), {BackgroundColor3 = biomeRef.cor}):Play()
        dropBtn.Text = biomeRef.emoji .. "  " .. biomeRef.nome .. " ▾"
        dropBtn.TextColor3 = Color3.fromRGB(255,255,255)
        Instance.new("UIStroke", dropBtn).Color = biomeRef.cor
        closeDropdown()
        -- Atualiza botão de TP
        tpBiomeBtn.Text = "🚀  Tp> " .. biomeRef.nome
        TweenService:Create(tpBiomeBtn, TweenInfo.new(0.25, Enum.EasingStyle.Back), {BackgroundColor3 = biomeRef.cor}):Play()
        showNotif(biomeRef.emoji .. " " .. biomeRef.nome .. " selecionado!", "info")
    end)
end
...
UserInputService.InputBegan:Connect(function(input)
    ...
    local mp = UserInputService:GetMouseLocation()
    local ap, as = dropMenu.AbsolutePosition, dropMenu.AbsoluteSize
    local bp, bs = dropBtn.AbsolutePosition, dropBtn.AbsoluteSize
    local inDrop = mp.X>=ap.X and mp.X<=ap.X+as.X and mp.Y>=ap.Y and mp.Y<=ap.Y+as.Y
...
-- Linha de TP
local tpRow = Instance.new("Frame", tpPage)
tpRow.BackgroundColor3 = Color3.fromRGB(22,24,30)
tpRow.BorderSizePixel = 0; tpRow.Size = UDim2.new(1,0,0,52); tpRow.LayoutOrder = tplo(); tpRow.ZIndex = 5
Instance.new("UICorner", tpRow).CornerRadius = UDim.new(0,10)
Instance.new("UIStroke", tpRow).Color = Color3.fromRGB(40,45,60)

-- Botão TP> Biome
-- Criar botão TP (agora tpBiomeBtn já foi declarado acima)
tpBiomeBtn = Instance.new("TextButton", tpRow)
tpBiomeBtn.BackgroundColor3 = Color3.fromRGB(60,66,90)
tpBiomeBtn.BorderSizePixel = 0; tpBiomeBtn.Position = UDim2.new(0,10,0.5,-18); tpBiomeBtn.Size = UDim2.new(1,-20,0,36)
tpBiomeBtn.Font = Enum.Font.GothamBlack; tpBiomeBtn.Text = "🚀  Selecione um Bioma ▲"; tpBiomeBtn.TextColor3 = Color3.fromRGB(180,190,220)
tpBiomeBtn.TextSize = 12; tpBiomeBtn.ZIndex = 7
Instance.new("UICorner", tpBiomeBtn).CornerRadius = UDim.new(0,10)
Instance.new("UIStroke", tpBiomeBtn).Color = Color3.fromRGB(88,101,242)

tpBiomeBtn.MouseEnter:Connect(function()
    if selectedBiome then TweenService:Create(tpBiomeBtn, TweenInfo.new(0.15), {BackgroundTransparency=0.1}):Play() end
end)
tpBiomeBtn.MouseLeave:Connect(function()
    TweenService:Create(tpBiomeBtn, TweenInfo.new(0.15), {BackgroundTransparency=0}):Play()
end)

-- Tela de confirmação
local confirmFrame = Instance.new("Frame", ScreenGui)
confirmFrame.Name = "BiomeConfirm"
confirmFrame.BackgroundColor3 = Color3.fromRGB(0,0,0)
confirmFrame.BackgroundTransparency = 0.4
confirmFrame.Size = UDim2.new(1,0,1,0); confirmFrame.Visible = false; confirmFrame.ZIndex = 700
local confirmBox = Instance.new("Frame", confirmFrame)
confirmBox.BackgroundColor3 = Color3.fromRGB(22,24,30)
confirmBox.BorderSizePixel = 0
confirmBox.AnchorPoint = Vector2.new(0.5,0.5)
confirmBox.Position = UDim2.new(0.5,0,0.5,0); confirmBox.Size = UDim2.new(0,300,0,140); confirmBox.ZIndex = 701
Instance.new("UICorner", confirmBox).CornerRadius = UDim.new(0,14)
local cbStroke = Instance.new("UIStroke", confirmBox); cbStroke.Thickness = 1.5

local confirmTitle = Instance.new("TextLabel", confirmBox)
confirmTitle.BackgroundTransparency = 1; confirmTitle.Position = UDim2.new(0,0,0,18)
confirmTitle.Size = UDim2.new(1,0,0,24); confirmTitle.Font = Enum.Font.GothamBlack
confirmTitle.Text = "Você deseja viajar para?"; confirmTitle.TextColor3 = Color3.fromRGB(220,225,240)
confirmTitle.TextSize = 13; confirmTitle.ZIndex = 702

local confirmDest = Instance.new("TextLabel", confirmBox)
confirmDest.BackgroundTransparency = 1; confirmDest.Position = UDim2.new(0,0,0,44)
confirmDest.Size = UDim2.new(1,0,0,28); confirmDest.Font = Enum.Font.GothamBlack
confirmDest.Text = "🌋 Vulcão"; confirmDest.TextColor3 = Color3.fromRGB(255,200,80)
confirmDest.TextSize = 18; confirmDest.ZIndex = 702

local confirmYes = Instance.new("TextButton", confirmBox)
confirmYes.BackgroundColor3 = Color3.fromRGB(60,200,100)
confirmYes.BorderSizePixel = 0; confirmYes.Position = UDim2.new(0,20,1,-50); confirmYes.Size = UDim2.new(0.5,-26,0,32)
confirmYes.Font = Enum.Font.GothamBold; confirmYes.Text = "✔ Sim"; confirmYes.TextColor3 = Color3.fromRGB(255,255,255)
confirmYes.TextSize = 13; confirmYes.ZIndex = 703
Instance.new("UICorner", confirmYes).CornerRadius = UDim.new(0,8)

local confirmNo = Instance.new("TextButton", confirmBox)
confirmNo.BackgroundColor3 = Color3.fromRGB(200,60,60)
confirmNo.BorderSizePixel = 0; confirmNo.Position = UDim2.new(0.5,6,1,-50); confirmNo.Size = UDim2.new(0.5,-26,0,32)
confirmNo.Font = Enum.Font.GothamBold; confirmNo.Text = "✖ Não"; confirmNo.TextColor3 = Color3.fromRGB(255,255,255)
confirmNo.TextSize = 13; confirmNo.ZIndex = 703
Instance.new("UICorner", confirmNo).CornerRadius = UDim.new(0,8)

local function showConfirm(biome)
    confirmDest.Text = biome.emoji .. "  " .. biome.nome
    confirmDest.TextColor3 = biome.cor
    cbStroke.Color = biome.cor
    confirmTitle.Text = "Você deseja viajar para " .. biome.nome .. "?"
    confirmFrame.Visible = true
    confirmBox.Size = UDim2.new(0,0,0,0)
    TweenService:Create(confirmBox, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0,300,0,140)}):Play()
end

local function hideConfirm()
    TweenService:Create(confirmBox, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {Size = UDim2.new(0,0,0,0)}):Play()
    task.delay(0.21, function() confirmFrame.Visible = false end)
end

confirmYes.MouseButton1Click:Connect(function()
    hideConfirm()
    if selectedBiome then tpToBiome(selectedBiome) end
end)
...
tpBiomeBtn.MouseButton1Click:Connect(function()
    if not selectedBiome then
        showNotif("Selecione um bioma primeiro!", "aviso")
        return
    end
    showConfirm(selectedBiome)
end)

-- Seção info
local tpInfoF = Instance.new("Frame", tpPage)
tpInfoF.BackgroundColor3 = Color3.fromRGB(14,20,35); tpInfoF.BackgroundTransparency = 0.5
tpInfoF.BorderSizePixel = 0; tpInfoF.Size = UDim2.new(1,0,0,64); tpInfoF.LayoutOrder = tplo(); tpInfoF.ZIndex = 4
Instance.new("UICorner", tpInfoF).CornerRadius = UDim.new(0,8)
local tpInfoS = Instance.new("UIStroke", tpInfoF); tpInfoS.Color = Color3.fromRGB(255,200,80); tpInfoS.Thickness = 1
local tpInfoLbl = Instance.new("TextLabel", tpInfoF)
tpInfoLbl.BackgroundTransparency = 1; tpInfoLbl.Position = UDim2.new(0,10,0,6)
tpInfoLbl.Size = UDim2.new(1,-20,1,-12); tpInfoLbl.Font = Enum.Font.Gotham
tpInfoLbl.Text = "🗺️  BIOMAS 99 DIAS NA FLORESTA 2026:\n🌲 Floresta (inicial) • ❄️ Ártico (neve) • 🌋 Vulcão (lava) • 🐸 Pântano (sapos)\n⚠️  Se o bioma não for encontrado, explore o mapa primeiro!"
tpInfoLbl.TextColor3 = Color3.fromRGB(200,180,100); tpInfoLbl.TextSize = 8; tpInfoLbl.TextWrapped = true
tpInfoLbl.TextXAlignment = Enum.TextXAlignment.Left; tpInfoLbl.TextYAlignment = Enum.TextYAlignment.Top; tpInfoLbl.ZIndex = 5
...
print("╔══════════════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v5.0 — MEGA UPDATE  Fev 2026                     ║")
print("╠══════════════════════════════════════════════════════════════╣")
print("║  ✓ Player: InfZoom | CamLivre | FullBright | InfJump       ║")
print("║  ✓ Player: SlopeBoost | Speed | JumpPower | Fly | Noclip   ║")
print("║  ✓ Avançado: Aimbot | Aimbot FOV | Aimbot AUTO             ║")
print("║  ✓ Teleporte: 4 Biomas (Floresta/Ártico/Vulcão/Pântano)    ║")
