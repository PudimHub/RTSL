...
-- ║   FIX: Bring debounce + ESP otimizado (sem trava)               ║
-- ╚══════════════════════════════════════════════════════════════════╝
...
-- ══════════════════════════════════════════════════════════════════════════
--  ██████████  ESP SYSTEM — CORRIGIDO (sem trava)  ██████████
--  FIX: RenderStepped substituído por Heartbeat throttled a 20fps
--       Pool de labels com reuso correto, cache com intervalo maior
--       Desenho de labels apenas quando visível na tela
-- ══════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════
--  POOL DE LABELS — Pré-alocado com 80 slots
--  FIX: Sem criar/destruir frames a cada frame. Reusa sempre.
-- ══════════════════════════════════════════════════════════════════
local POOL_SIZE  = 80
local labelPool  = {}
local activeList = {}

-- Cria todos os frames do pool de uma vez
local function buildPoolFrame()
    ...
    -- Ícone placeholder (será substituído ao usar)
...
-- Cache de ícones por key para não recriar toda hora
local iconCache = {}  -- [key] -> table of Frame children data (só cor e posição)

local function getLabel(key, cor, nome, dist, posX, posY)
    local frame = table.remove(labelPool)
    if not frame then return nil end  -- pool esgotado, pula
    ...
    -- Ícone: apenas atualiza a cor dos filhos do IconHolder
    -- sem recriar frames. Cada key tem sempre o mesmo shape.
    local holder = frame:FindFirstChild("IconHolder")
    if holder then
        -- Se o holder está vazio ou tem key diferente, recria
...
-- ── Cache de objetos com intervalo maior (4s) ──
local objectCache   = {}
local lastCacheTime = 0
local CACHE_INTERVAL = 4   -- FIX: era 2.5s, agora 4s para reduzir GetDescendants
...
local function updateCache()
    ...
    -- FIX: usa task.spawn para não travar o thread principal durante GetDescendants
...
-- ── Loop de renderização — FIX: Heartbeat throttled a ~20fps ──
-- RenderStepped (60fps) causava trava. Heartbeat com acumulador de delta
-- renderiza ESP a ~20fps, suficiente para visualização e muito mais leve.
local espConn
local espDeltaAcc = 0
local ESP_RENDER_INTERVAL = 1/20  -- 20fps para ESP
...
local function iniciarESP()
    ...
    espConn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Devolver todos do frame anterior
        returnAllActive()

        -- Checar se tem algum ESP ativo
        ...
        -- Atualizar cache (só roda a cada CACHE_INTERVAL segundos)
        pcall(updateCache)
        ...
        for _, entry in ipairs(objectCache) do
            pcall(function()
                ...
                if screenPos.X < -50 or screenPos.X > vp.X + 50 then return end
                if screenPos.Y < -30 or screenPos.Y > vp.Y + 30 then return end
                -- Deduplica por célula de tela (evita labels sobrepostos)
...
local function makeEspToggleRow(cfg)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        lastCacheTime = 0  -- força reconstrução do cache na próxima rodada
    end)
end
...
-- ══════════════════════════════════════════════════════════════════════════
--  ████████  BRING SYSTEM — CORRIGIDO (sem repetição)  ████████
--  FIX: Debounce por botão — clique executa 1 vez e trava até terminar
--       Removido RunService.Heartbeat de reposicionamento (causava loop)
--       Anti-desaparecimento mantido mas apenas via task.delay simples
-- ══════════════════════════════════════════════════════════════════════════
...
local function criarIconeBring(parent, key, cor)
    ...
    elseif key == "Metais" then
        ...
        local faceta = r(5, 6, 3, 8, 1) faceta.BackgroundTransparency = 0.4
...
-- ══════════════════════════════════════════════════════════════
--  executarBring — FIX PRINCIPAL
--  - Roda UMA vez por clique (debounce externo no botão)
--  - Sem loop de Heartbeat permanente
--  - Anti-desaparecimento via task.delay simples (não loop)
-- ══════════════════════════════════════════════════════════════
local function executarBring(keywords)
    ...
    local trazidos = {}   -- lista de {obj, targetPos} desta execução
    ...
    -- ── Anti-desaparecimento via task.delay (sem loop permanente) ──
    -- Verifica a cada 1 segundo por 8 segundos se os itens sumiram longe
    -- Isso substitui o Heartbeat global que ficava rodando sempre
    if #trazidos > 0 then
        task.spawn(function()
            for i = 1, 8 do
                task.wait(1)
            ...
            trazidos = nil  -- libera memória após 8 segundos
        end)
    end
...
bringSubtitle.Text     = "Clique para teleportar os itens até você (1x por clique)"
...
local function makeBringRow(cfg)
    ...
    -- ── FIX PRINCIPAL DO BRING: debounce por botão ──
    -- 'bringRunning' bloqueia novos cliques enquanto executa
    -- Isso impede que o bring repita automaticamente
    local bringRunning = false

    btnBring.MouseButton1Click:Connect(function()
        if bringRunning then return end   -- FIX: bloqueia se já está rodando
        bringRunning = true
        ...
        task.spawn(function()
            ...
            -- FIX: Libera o debounce após tudo terminar
            -- Cooldown de 1 segundo para evitar spam acidental
            task.wait(1)
            bringRunning = false
        end)
    end)
...
print("✅ PudimHub V3 FIXED carregado!")
print("🔧 Fix Bring: debounce por botão (1x por clique + 1s cooldown)")
print("🔧 Fix ESP: Heartbeat 20fps + pool pré-alocado de 80 labels")
