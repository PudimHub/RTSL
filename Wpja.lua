...
local function makePlRow_Slider(iconKey, cor, lblTxt, descTxt, minV, maxV, stepSm, stepLg, initVal, onChange)
    ...
    local function getSliderVal()
        ...
        if absSize <= 0 then return curVal[1] end
        local relX = math.clamp(mp.X - absPos, 0, absSize)
...
local function makePlRow_Toggle(iconKey, cor, lblTxt, descTxt, onToggle, speedRef)
    ...
    if hasSpeed then
        ...
        local function getMsVal()
            local mp = UserInputService:GetMouseLocation()
            local absPos = msTrack.AbsolutePosition.X
            local absSize = msTrack.AbsoluteSize.X
            if absSize <= 0 then return sf.val[1] end
            local relX = math.clamp(mp.X - absPos, 0, absSize)
            return math.round(sf.min + relX/absSize*(sf.max-sf.min))
        end

        msDragBtn.MouseButton1Down:Connect(function()
            msDragging=true
            updateMsSlider(getMsVal())
        end)
        UserInputService.InputEnded:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.MouseButton1 and msDragging then
                msDragging=false
            end
        end)
        UserInputService.InputChanged:Connect(function(inp)
