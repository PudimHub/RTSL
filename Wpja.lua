...
-- ──────────────────────────────────────────────────
--  Row com SliderBar arrastável (Speed e JumpPower)
--  Líquido azul animado que se move ao arrastar
-- ──────────────────────────────────────────────────
local function makePlRow_Slider(iconKey, cor, lblTxt, descTxt, minV, maxV, stepSm, stepLg, initVal, onChange)
    ...
    row.Size = UDim2.new(1,0,0,80); row.LayoutOrder = plLO(); row.ZIndex = 5
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,9)
    ...
    ib.BorderSizePixel = 0; ib.Position = UDim2.new(0,8,0.5,-20); ib.Size = UDim2.new(0,36,0,36); ib.ZIndex = 7
    ...
    -- === SLIDERBAR CONTAINER ===
    local sliderContainer = Instance.new("Frame", row)
    sliderContainer.BackgroundColor3 = Color3.fromRGB(16,18,26)
    sliderContainer.BackgroundTransparency = 0.1; sliderContainer.BorderSizePixel = 0
    sliderContainer.Position = UDim2.new(0,54,0,44); sliderContainer.Size = UDim2.new(1,-66,0,22); sliderContainer.ZIndex = 8
    Instance.new("UICorner", sliderContainer).CornerRadius = UDim.new(0,11)
    local sliderStroke = Instance.new("UIStroke", sliderContainer)
    sliderStroke.Color = cor; sliderStroke.Thickness = 1; sliderStroke.Transparency = 0.6

    -- === FUNDO DO TRILHO (trilho cinza) ===
    local trackBg = Instance.new("Frame", sliderContainer)
    trackBg.BackgroundColor3 = Color3.fromRGB(28,32,44); trackBg.BorderSizePixel = 0
    trackBg.Position = UDim2.new(0,2,0,2); trackBg.Size = UDim2.new(1,-4,1,-4); trackBg.ZIndex = 9
    Instance.new("UICorner", trackBg).CornerRadius = UDim.new(0,9)

    -- === LÍQUIDO (fill) ===
    local fill = Instance.new("Frame", trackBg)
    fill.BackgroundColor3 = cor; fill.BorderSizePixel = 0
    fill.Position = UDim2.new(0,0,0,0); fill.Size = UDim2.new(0.5,0,1,0); fill.ZIndex = 10
    Instance.new("UICorner", fill).CornerRadius = UDim.new(0,9)
    -- brilho do líquido
    local shine = Instance.new("Frame", fill)
    shine.BackgroundColor3 = Color3.fromRGB(255,255,255); shine.BackgroundTransparency = 0.7
    shine.BorderSizePixel = 0; shine.Position = UDim2.new(0,2,0,2); shine.Size = UDim2.new(1,-4,0,4); shine.ZIndex = 11
    Instance.new("UICorner", shine).CornerRadius = UDim.new(0,4)
    -- ondinha líquido (frame animado que se move sutilmente)
    local wave = Instance.new("Frame", fill)
    wave.BackgroundColor3 = Color3.fromRGB(255,255,255); wave.BackgroundTransparency = 0.85
    wave.BorderSizePixel = 0; wave.Position = UDim2.new(0.3,-4,0.4,0); wave.Size = UDim2.new(0,8,0,6); wave.ZIndex = 11
    Instance.new("UICorner", wave).CornerRadius = UDim.new(1,0)

    -- === KNOB (bolinha arrastável) ===
    local knob = Instance.new("Frame", trackBg)
    knob.BackgroundColor3 = Color3.fromRGB(255,255,255); knob.BorderSizePixel = 0
    knob.Size = UDim2.new(0,18,0,18); knob.ZIndex = 12
    Instance.new("UICorner", knob).CornerRadius = UDim.new(1,0)
    local knobInner = Instance.new("Frame", knob)
    knobInner.BackgroundColor3 = cor; knobInner.BorderSizePixel = 0
    knobInner.Position = UDim2.new(0,3,0,3); knobInner.Size = UDim2.new(0,12,0,12); knobInner.ZIndex = 13
    Instance.new("UICorner", knobInner).CornerRadius = UDim.new(1,0)
    local knobGlow = Instance.new("UIStroke", knob)
    knobGlow.Color = cor; knobGlow.Thickness = 1.5; knobGlow.Transparency = 0.3

    -- === VALOR DISPLAY ===
    local valLabel = Instance.new("TextLabel", sliderContainer)
    valLabel.BackgroundTransparency = 1; valLabel.Position = UDim2.new(0.5,-22,0,0); valLabel.Size = UDim2.new(0,44,1,0)
    valLabel.Font = Enum.Font.GothamBold; valLabel.TextColor3 = Color3.fromRGB(255,255,255); valLabel.TextSize = 11
    valLabel.ZIndex = 14; valLabel.Text = tostring(initVal)
    ...
    local function updateSlider(v)
        ...
        local knobX = math.max(0, pct * (trackBg.AbsoluteSize.X - 18))
        TweenService:Create(fill, TweenInfo.new(0.12, Enum.EasingStyle.Quad), {Size = UDim2.new(pct,0,1,0)}):Play()
        TweenService:Create(knob, TweenInfo.new(0.12, Enum.EasingStyle.Quad), {Position = UDim2.new(0,knobX,0.5,-9)}):Play()
        -- ondinha de movimento
        TweenService:Create(wave, TweenInfo.new(0.15,Enum.EasingStyle.Quad),{BackgroundTransparency=0.6}):Play()
        task.delay(0.15,function() TweenService:Create(wave,TweenInfo.new(0.3),{BackgroundTransparency=0.85}):Play() end)
        TweenService:Create(rowS, TweenInfo.new(0.2),{Color=cor}):Play()
        task.delay(0.8,function() TweenService:Create(rowS,TweenInfo.new(0.4),{Color=Color3.fromRGB(42,46,58)}):Play() end)
        onChange(v)
    end
    ...
    task.spawn(function()
        task.wait(0.05)
        local pct = (initVal - minV)/(maxV - minV)
        fill.Size = UDim2.new(pct,0,1,0)
        local knobX = math.max(0, pct*(trackBg.AbsoluteSize.X-18))
        knob.Position = UDim2.new(0,knobX,0.5,-9)
    end)

    -- === ARRASTAR (drag) ===
    local dragging = false
    local dragBtn = Instance.new("TextButton", sliderContainer)
    dragBtn.BackgroundTransparency = 1; dragBtn.Size = UDim2.new(1,0,1,0); dragBtn.Text = ""; dragBtn.ZIndex = 15

    dragBtn.MouseButton1Down:Connect(function(x,y)
        dragging = true
        TweenService:Create(knob,TweenInfo.new(0.1),{Size=UDim2.new(0,22,0,22)}):Play()
        TweenService:Create(sliderStroke,TweenInfo.new(0.1),{Transparency=0}):Play()
    end)
    dragBtn.MouseButton1Up:Connect(function()
        dragging = false
        TweenService:Create(knob,TweenInfo.new(0.15,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,18,0,18)}):Play()
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if dragging and inp.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local relX = math.clamp(inp.Position.X - absPos, 0, absSize)
            local pct = relX / absSize
            local v = math.round(minV + pct*(maxV-minV))
            updateSlider(v)
        end
    end)
    -- Click direto no slider
    dragBtn.MouseButton1Click:Connect(function(x,y)
        local absPos = trackBg.AbsolutePosition.X
        local absSize = trackBg.AbsoluteSize.X
        local relX = math.clamp(x - absPos, 0, absSize)
        local pct = relX / absSize
        updateSlider(math.round(minV + pct*(maxV-minV)))
    end)
...
local function makePlRow_Toggle(iconKey, cor, lblTxt, descTxt, onToggle, speedRef)
    ...
    -- Mini SliderBar de velocidade (apenas para Fly) - estilo líquido
    if hasSpeed then
        local sf = speedRef
        local miniSlider = Instance.new("Frame", row)
        miniSlider.BackgroundColor3 = Color3.fromRGB(12,14,20)
        miniSlider.BackgroundTransparency = 0.1; miniSlider.BorderSizePixel = 0
        miniSlider.Position = UDim2.new(0,54,0,48); miniSlider.Size = UDim2.new(1,-66,0,16); miniSlider.ZIndex = 8
        Instance.new("UICorner", miniSlider).CornerRadius = UDim.new(0,8)
        local ms2 = Instance.new("UIStroke",miniSlider); ms2.Color=cor; ms2.Thickness=1; ms2.Transparency=0.6

        local msTrack = Instance.new("Frame",miniSlider)
        msTrack.BackgroundColor3 = Color3.fromRGB(28,32,44); msTrack.BorderSizePixel=0
        msTrack.Position = UDim2.new(0,1,0,1); msTrack.Size = UDim2.new(1,-2,1,-2); msTrack.ZIndex=9
        Instance.new("UICorner",msTrack).CornerRadius=UDim.new(0,7)

        local msFill = Instance.new("Frame",msTrack)
        msFill.BackgroundColor3=cor; msFill.BorderSizePixel=0
        msFill.Size=UDim2.new((sf.val[1]-sf.min)/(sf.max-sf.min),0,1,0); msFill.ZIndex=10
        Instance.new("UICorner",msFill).CornerRadius=UDim.new(0,7)
        local msShine = Instance.new("Frame",msFill)
        msShine.BackgroundColor3=Color3.fromRGB(255,255,255); msShine.BackgroundTransparency=0.7
        msShine.BorderSizePixel=0; msShine.Position=UDim2.new(0,1,0,1); msShine.Size=UDim2.new(1,-2,0,3); msShine.ZIndex=11
        Instance.new("UICorner",msShine).CornerRadius=UDim.new(0,3)

        local msKnob = Instance.new("Frame",msTrack)
        msKnob.BackgroundColor3=Color3.fromRGB(255,255,255); msKnob.BorderSizePixel=0
        msKnob.Size=UDim2.new(0,14,0,14); msKnob.ZIndex=12
        Instance.new("UICorner",msKnob).CornerRadius=UDim.new(1,0)
        local msKI = Instance.new("Frame",msKnob)
        msKI.BackgroundColor3=cor; msKI.BorderSizePixel=0
        msKI.Position=UDim2.new(0,2,0,2); msKI.Size=UDim2.new(0,10,0,10); msKI.ZIndex=13
        Instance.new("UICorner",msKI).CornerRadius=UDim.new(1,0)

        local msVal = Instance.new("TextLabel",miniSlider)
        msVal.BackgroundTransparency=1; msVal.Position=UDim2.new(0.5,-18,0,0); msVal.Size=UDim2.new(0,36,1,0)
        msVal.Font=Enum.Font.GothamBold; msVal.TextColor3=Color3.fromRGB(255,255,255); msVal.TextSize=8; msVal.ZIndex=14
        msVal.Text="Vel:"..sf.val[1]

        local function updateMsSlider(v)
            v = math.clamp(math.round(v),sf.min,sf.max); sf.val[1]=v; msVal.Text="Vel:"..v
            local pct=(v-sf.min)/(sf.max-sf.min)
            TweenService:Create(msFill,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{Size=UDim2.new(pct,0,1,0)}):Play()
            local kx=math.max(0,pct*(msTrack.AbsoluteSize.X-14))
            TweenService:Create(msKnob,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{Position=UDim2.new(0,kx,0.5,-7)}):Play()
            if sf.onChange then sf.onChange(v) end
        end
        task.spawn(function() task.wait(0.05)
            local pct=(sf.val[1]-sf.min)/(sf.max-sf.min)
            msFill.Size=UDim2.new(pct,0,1,0)
            local kx=math.max(0,pct*(msTrack.AbsoluteSize.X-14))
            msKnob.Position=UDim2.new(0,kx,0.5,-7)
        end)
        local msDragging=false
        local msDragBtn = Instance.new("TextButton",miniSlider)
        msDragBtn.BackgroundTransparency=1; msDragBtn.Size=UDim2.new(1,0,1,0); msDragBtn.Text=""; msDragBtn.ZIndex=15
        msDragBtn.MouseButton1Down:Connect(function() msDragging=true end)
        UserInputService.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then msDragging=false end end)
        UserInputService.InputChanged:Connect(function(inp)
            if msDragging and inp.UserInputType==Enum.UserInputType.MouseMovement then
                local relX=math.clamp(inp.Position.X-msTrack.AbsolutePosition.X,0,msTrack.AbsoluteSize.X)
                updateMsSlider(math.round(sf.min+relX/msTrack.AbsoluteSize.X*(sf.max-sf.min)))
            end
        end)
        msDragBtn.MouseButton1Click:Connect(function(x,y)
            local relX=math.clamp(x-msTrack.AbsolutePosition.X,0,msTrack.AbsoluteSize.X)
            updateMsSlider(math.round(sf.min+relX/msTrack.AbsoluteSize.X*(sf.max-sf.min)))
        end)
    end
...
-- Gradiente no header
local tpHGrad = Instance.new("UIGradient", tpHeader)
tpHGrad.Color = ColorSequence.new({
...
tpGrid.Size=UDim2.new(1,0,0,0); tpGrid.AutomaticSize=Enum.AutomaticSize.Y; tpGrid.LayoutOrder=tpLayoutOrder(); tpGrid.ZIndex=4
