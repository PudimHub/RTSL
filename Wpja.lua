-- ╔══════════════════════════════════════════════════════════════════╗
-- ║     PUDIM HUB — v5 FINAL (Player + Aimbot + TP)           ║
...
local TabConfig = {
    ...
    {key="Player",label="Player"},{key="Configuracoes",label="Configurações"},{key="AvancadoFuncoes",label="Avançado Funções"},
	    {key="Tp",label="Teleporte"},
}
local GroupConfig = {
    {label="GERAL",   keys={"Info","Status"}},
    {label="COMBATE", keys={"Farm","Esp","Bring","AvancadoFarm"}},
    {label="EXTRA",   keys={"Player","Configuracoes","AvancadoFuncoes","Tp"}},
}
...
local function createTabIcon(parent,key)
    ...
    elseif key=="Tp" then
        p(mkRect(cont,4,4,12,12,ic,2)); p(mkCircle(cont,10,10,4,Color3.fromRGB(255,80,80)))
        p(mkRect(cont,9,14,2,6,Color3.fromRGB(255,80,80),1))
    end
    return cont, parts
end
...
local function setInfiniteZoom(state)
    ...
    if state then Player.CameraMaxZoomDistance = 100000 else Player.CameraMaxZoomDistance = 128 end
end
local freeCamEnabled = false; local freeCamConn = nil
local function setFreeCam(state)
    freeCamEnabled = state
    if state then
        local cam = workspace.CurrentCamera; local speed = 2
        freeCamConn = RunService.RenderStepped:Connect(function()
            ...
            if dir.Magnitude > 0 then cam.CFrame = cf + (dir.Unit * speed) end
        end)
    else if freeCamConn then freeCamConn:Disconnect(); freeCamConn = nil end end
end
local fullBrightEnabled = false; local origLighting = {Brightness = Lighting.Brightness, ClockTime = Lighting.ClockTime, FogEnd = Lighting.FogEnd, GlobalShadows = Lighting.GlobalShadows, OutdoorAmbient = Lighting.OutdoorAmbient}
local function setFullBright(state)
    fullBrightEnabled = state
    if state then 
        Lighting.Brightness = 5; Lighting.ClockTime = 12; Lighting.FogEnd = 100000; Lighting.GlobalShadows = false; Lighting.OutdoorAmbient = Color3.fromRGB(255, 255, 255)
        for _,v in pairs(Lighting:GetChildren()) do if v:IsA("Atmosphere") or v:IsA("Sky") or v:IsA("Clouds") then v.Parent = nil end end
    else 
        Lighting.Brightness = origLighting.Brightness; Lighting.ClockTime = origLighting.ClockTime; Lighting.FogEnd = origLighting.FogEnd; Lighting.GlobalShadows = origLighting.GlobalShadows; Lighting.OutdoorAmbient = origLighting.OutdoorAmbient 
    end
end
local infiniteJumpEnabled = false; local infiniteJumpConn = nil
local function setInfiniteJump(state)
    infiniteJumpEnabled = state
    if state then infiniteJumpConn = UserInputService.JumpRequest:Connect(function() if infiniteJumpEnabled then local ch = Player.Character; if ch then local hum = ch:FindFirstChildOfClass("Humanoid"); if hum then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end end end)
    else if infiniteJumpConn then infiniteJumpConn:Disconnect(); infiniteJumpConn = nil end end
end
local slopeBoostEnabled = false; local slopeBoostConn = nil
local function setSlopeBoost(state)
    slopeBoostEnabled = state
    if state then slopeBoostConn = RunService.Heartbeat:Connect(function() if not slopeBoostEnabled then return end; local ch = Player.Character; if ch then local hum = ch:FindFirstChildOfClass("Humanoid"); if hum then if hum.WalkSpeed < playerSpeed then hum.WalkSpeed = playerSpeed end end end end)
    else if slopeBoostConn then slopeBoostConn:Disconnect(); slopeBoostConn = nil end end
end
local godModeEnabled = false; local godModeConn = nil
local function setGodMode(state)
    godModeEnabled = state
    if state then godModeConn = RunService.Heartbeat:Connect(function() if not godModeEnabled then return end; local ch = Player.Character; if ch then local hum = ch:FindFirstChildOfClass("Humanoid"); if hum then hum.Health = 1000000; hum.MaxHealth = 1000000 end end end)
    else if godModeConn then godModeConn:Disconnect(); godModeConn = nil end end
...
-- ──────────────────────────────────────────────────
local function makeSliderBar(parent, cor, minV, maxV, initVal, onChange)
    local sliderFrame = Instance.new("Frame", parent)
    sliderFrame.BackgroundColor3 = Color3.fromRGB(16,18,26); sliderFrame.BorderSizePixel = 0
    sliderFrame.Position = UDim2.new(0,54,0,42); sliderFrame.Size = UDim2.new(1,-66,0,22); sliderFrame.ZIndex = 8
    Instance.new("UICorner", sliderFrame).CornerRadius = UDim.new(0,6)
    local liquid = Instance.new("Frame", sliderFrame)
    liquid.BackgroundColor3 = Color3.fromRGB(88,101,242); liquid.BorderSizePixel = 0
    liquid.Position = UDim2.new(0,0,0,0); liquid.Size = UDim2.new((initVal-minV)/(maxV-minV),0,1,0); liquid.ZIndex = 9
    Instance.new("UICorner", liquid).CornerRadius = UDim.new(0,6)
    local glow = Instance.new("Frame", liquid)
    glow.BackgroundColor3 = Color3.fromRGB(255,255,255); glow.BackgroundTransparency = 0.8
    glow.BorderSizePixel = 0; glow.Position = UDim2.new(0,0,0,0); glow.Size = UDim2.new(1,0,0.3,0); glow.ZIndex = 10
    ...
    valLbl.Text = tostring(initVal); valLbl.TextColor3 = Color3.fromRGB(255,255,255); valLbl.TextSize = 13; valLbl.ZIndex = 11
    local dragging = false
    local function updateSlider(input)
        local pos = math.clamp((input.Position.X - sliderFrame.AbsolutePosition.X) / sliderFrame.AbsoluteSize.X, 0, 1)
        local val = math.round(minV + (pos * (maxV - minV)))
        TweenService:Create(liquid, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {Size = UDim2.new(pos,0,1,0)}):Play()
        valLbl.Text = tostring(val); onChange(val)
    end
    sliderFrame.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; updateSlider(input) end end)
...
local function makePlRow_Slider(iconKey, cor, lblTxt, descTxt, minV, maxV, stepSm, stepLg, initVal, onChange)
    ...
    makeSliderBar(row, cor, minV, maxV, initVal, upd))
...
makePlSec("🛠️ NOVAS FUNÇÕES", Color3.fromRGB(150,150,255))
makePlRow_Toggle("InfiniteZoom", Color3.fromRGB(100,200,255), "🔍 Infinite Zoom", "Aumenta o zoom máximo da câmera infinitamente", function(s) setInfiniteZoom(s) end)
makePlRow_Toggle("FreeCam", Color3.fromRGB(255,150,100), "🎥 Câmera Livre", "A câmera atravessa paredes e voa livremente", function(s) setFreeCam(s) end)
makePlRow_Toggle("FullBright", Color3.fromRGB(255,255,100), "☀️ FullBright", "Deixa tudo claro, remove noite, chuva e neblina", function(s) setFullBright(s) end)
makePlRow_Toggle("InfiniteJump", Color3.fromRGB(255,255,255), "🦘 Infinite Jump", "Permite pular infinitamente no ar", function(s) setInfiniteJump(s) end)
makePlRow_Toggle("SlopeBoost", Color3.fromRGB(100,255,100), "⚡ Slope Boost", "Ignora desaceleração de armadilhas e terrenos", function(s) setSlopeBoost(s) end)
makePlRow_Toggle("GodMode", Color3.fromRGB(255,50,50), "🛡️ God Mode", "Fica imune a qualquer tipo de ataque de mobs", function(s) setGodMode(s) end)
...
-- ══════════════════════════════════════════════════════
task.wait(0.05)
local biomes = {"Vulcão", "Neve", "Floresta", "Caverna", "Deserto", "Pântano", "Árvore Mãe"}
local selectedBiome = "Nenhum"
local function getBiomePosition(name)
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") or obj:IsA("BasePart") then
            if obj.Name:lower():find(name:lower()) then return obj:GetPivot().Position end
        end
    end
    return nil
end
local function teleportToBiome(name)
    local pos = getBiomePosition(name)
    if pos then local ch = Player.Character; if ch then local hrp = ch:FindFirstChild("HumanoidRootPart"); if hrp then hrp.CFrame = CFrame.new(pos + Vector3.new(0, 10, 0)) end end end
end
local function makeTpPage()
    local page = Pages["Tp"]; local tpTabLO = 0; local function tpLO() tpTabLO += 1; return tpTabLO end
    local function makeTpSec(titulo, cor)
        local hdr = Instance.new("Frame", page); hdr.BackgroundColor3 = Color3.fromRGB(20,22,30); hdr.BackgroundTransparency = 0.3; hdr.BorderSizePixel = 0; hdr.Size = UDim2.new(1,0,0,22); hdr.LayoutOrder = tpLO(); hdr.ZIndex = 4
        Instance.new("UICorner", hdr).CornerRadius = UDim.new(0,6); local bar = Instance.new("Frame", hdr); bar.BackgroundColor3 = cor; bar.BorderSizePixel = 0; bar.Position = UDim2.new(0,0,0,0); bar.Size = UDim2.new(0,3,1,0); bar.ZIndex = 5
        Instance.new("UICorner", bar).CornerRadius = UDim.new(0,3); local lbl = Instance.new("TextLabel", hdr); lbl.BackgroundTransparency = 1; lbl.Position = UDim2.new(0,10,0,0); lbl.Size = UDim2.new(1,-14,1,0); lbl.Font = Enum.Font.GothamBlack; lbl.Text = titulo; lbl.TextColor3 = cor; lbl.TextSize = 9; lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 5
    end
    makeTpSec("📍 BIOMAS", Color3.fromRGB(255,100,100))
    local tpRow = Instance.new("Frame", page); tpRow.BackgroundColor3 = Color3.fromRGB(28,30,38); tpRow.BorderSizePixel = 0; tpRow.Size = UDim2.new(1,0,0,100); tpRow.LayoutOrder = tpLO(); tpRow.ZIndex = 5
    Instance.new("UICorner", tpRow).CornerRadius = UDim.new(0,9); local tpRowS = Instance.new("UIStroke", tpRow); tpRowS.Color = Color3.fromRGB(42,46,58); tpRowS.Thickness = 1
    local selBtn = Instance.new("TextButton", tpRow); selBtn.BackgroundColor3 = Color3.fromRGB(45,50,62); selBtn.BorderSizePixel = 0; selBtn.Position = UDim2.new(0,10,0,10); selBtn.Size = UDim2.new(1,-20,0,35); selBtn.Font = Enum.Font.GothamBold; selBtn.Text = "Selecionar Bioma: " .. selectedBiome; selBtn.TextColor3 = Color3.fromRGB(255,255,255); selBtn.TextSize = 12; selBtn.ZIndex = 6
    Instance.new("UICorner", selBtn).CornerRadius = UDim.new(0,8); local popup = Instance.new("ScrollingFrame", tpRow); popup.BackgroundColor3 = Color3.fromRGB(20,22,28); popup.BorderSizePixel = 0; popup.Position = UDim2.new(0,10,0,45); popup.Size = UDim2.new(1,-20,0,0); popup.Visible = false; popup.ZIndex = 10; popup.ScrollBarThickness = 2; popup.AutomaticCanvasSize = Enum.AutomaticSize.Y
    Instance.new("UICorner", popup).CornerRadius = UDim.new(0,8); local popupList = Instance.new("UIListLayout", popup); popupList.Padding = UDim.new(0,2); local tpBtn = Instance.new("TextButton", tpRow); tpBtn.BackgroundColor3 = Color3.fromRGB(88,101,242); tpBtn.BorderSizePixel = 0; tpBtn.Position = UDim2.new(0,10,0,55); tpBtn.Size = UDim2.new(1,-20,0,35); tpBtn.Font = Enum.Font.GothamBold; tpBtn.Text = "Teleportar para: " .. selectedBiome; tpBtn.TextColor3 = Color3.fromRGB(255,255,255); tpBtn.TextSize = 12; tpBtn.ZIndex = 6
    Instance.new("UICorner", tpBtn).CornerRadius = UDim.new(0,8)
    local function togglePopup()
        local isOpen = popup.Visible
        if not isOpen then popup.Visible = true; TweenService:Create(popup, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1,-20,0,120)}):Play(); TweenService:Create(tpBtn, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0,10,0,170)}):Play(); tpRow.Size = UDim2.new(1,0,0,215)
        else TweenService:Create(popup, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {Size = UDim2.new(1,-20,0,0)}):Play(); TweenService:Create(tpBtn, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {Position = UDim2.new(0,10,0,55)}):Play(); task.delay(0.2, function() popup.Visible = false; tpRow.Size = UDim2.new(1,0,0,100) end) end
    end
    selBtn.MouseButton1Click:Connect(togglePopup)
    for _, name in ipairs(biomes) do
        local b = Instance.new("TextButton", popup); b.BackgroundColor3 = Color3.fromRGB(35,38,45); b.BorderSizePixel = 0; b.Size = UDim2.new(1,0,0,30); b.Font = Enum.Font.GothamSemibold; b.Text = name; b.TextColor3 = Color3.fromRGB(200,200,200); b.TextSize = 10; b.ZIndex = 11
        Instance.new("UICorner", b).CornerRadius = UDim.new(0,5); b.MouseButton1Click:Connect(function() selectedBiome = name; selBtn.Text = "Selecionar Bioma: " .. name; tpBtn.Text = "Teleportar para: " .. name; togglePopup() end)
    end
    tpBtn.MouseButton1Click:Connect(function() if selectedBiome ~= "Nenhum" then teleportToBiome(selectedBiome) end end)
end
makeTpPage()
selectTab("Info")
