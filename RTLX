...
local Targets = {
    ...
    Tools = {"Old Axe", "Good Axe", "Strong Axe", "Old Rod", "Good Rod", "Strong Rod", "Old Flute", "Good Flute", "Strong Flute", "Flashlight", "Strong Flashlight", "Air Rifle", "Revolver", "Crossbow", "Spear", "Morningstar", "Raygun", "Laser Sword", "Laser Cannon", "Poison Spear", "Blowpipe", "Obsidiron Hammer"},
...
for _, list in pairs({Targets.Fruits, Targets.Wood, Targets.Combustion, Targets.Seeds, Targets.Medical, Targets.Tools, Targets.Armor}) do
...
local TPCategories = {
    ...
    },
    {
        ...
        icon = "📦",
        items = {
            {id = "chest", name = "Baú Comum", icon = "📦", internalName = "Chest"},
            {id = "rare-chest", name = "Baú Raro", icon = "💎", internalName = "Rare Chest"},
            {id = "epic-chest", name = "Baú Épico", icon = "⭐", internalName = "Epic Chest"},
            {id = "legendary-chest", name = "Baú Lendário", icon = "👑", internalName = "Legendary Chest"},
            {id = "treasure-chest", name = "Baú do Tesouro", icon = "🏴‍☠️", internalName = "Treasure Chest"},
            {id = "supply-crate", name = "Caixa de Suprimentos", icon = "📫", internalName = "Supply Crate"}
        }
    },
    {
        id = "tp-children",
        ...
        icon = "👶",
        items = {
            {id = "lost-child", name = "Criança Perdida", icon = "👶", internalName = "Lost Child"}
        }
    },
    {
        id = "tp-campfire",
...
local function createAccordionContent(parent, category, categoryType)
    ...
    if categoryType == "bring" then
        ...
        for _, item in pairs(category.items) do
            ...
            itemBtn.MouseButton1Click:Connect(function()
                notify("Teleporte", "Indo para " .. item.name .. "...", 2)
                local target = findObject(item.internalName)
                if target then
                    teleportTo(target.Position)
                else
                    notify("Erro", item.name .. " não encontrado!", 3)
                end
            end)
        end
...
createSlider(farmPage, "Alcance Animal Farm", 10, 250, 50, function(v) Config.AnimalFarmRange = v end)
createSection(farmPage, "Recursos")
createToggle(farmPage, "Tree Aura UNIVERSAL", function(v) Config.TreeAura = v end)
createSlider(farmPage, "Alcance Árvores", 1, 100, 50, function(v) Config.TreeAuraRange = v end)
...
-- NOVA ABA BRING COM ACCORDION
local bringAccordionPage = createTab("Bring+", "🎯")
createSection(bringAccordionPage, "Brings - Sistema Accordion")
for _, category in pairs(BringCategories) do
    createAccordionHeader(bringAccordionPage, category, "bring")
    createAccordionContent(bringAccordionPage, category, "bring")
end
...
print("🎯 Novas abas: 'Tp' (Accordion) e 'Bring+' (Accordion)")
