-- ╔══════════════════════════════════════════════════════════════════╗
-- ║     PUDIM HUB — v5 MOBILE (Funções Novas + Compatível Delta)        ║
-- ║  InfZoom|CamLivre|FullBright|InfJump|SlopeBoost|Notificações  ║
-- ║  Fly|Noclip|TpClick|Speed|JumpPower — 99 Dias na Floresta 2026 ║
-- ║             ESP 20 cats | BRING 17 cats | Aimbot | Fev 2026     ║
-- ╚══════════════════════════════════════════════════════════════════╝
...
print("╔══════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v4 — CORRIGIDO  Fev 2026                 ║")
print("╠══════════════════════════════════════════════════════╣")
print("║  ✓ ESP: +Green Frog | Mega Cultist | sem The Bat    ║")
print("║  ✓ ESP: +Leather Body, Alien Armour, Frog Boots     ║")
print("║  ✓ BRING: +Hellephant/PolarBear/ArcticFox Corpse    ║")
print("║  ✓ BRING: +Mammoth/Frog Corpse | +Armaduras         ║")
print("║  ✓ 20 ESP cats | 17 BRING cats | Wiki 2026          ║")
...
--  BUILD ABA PLAYER
-- ══════════════════════════════════════════════════════════
local flySpeedRef = {val={40}, min=5, max=300, step=5, onChange=function(v) flySpeed=v end}
...
makePlRow_Toggle("Fly",    Color3.fromRGB(80,180,255),   "✈️ Fly",
    "W/A/S/D mover • Espaço=subir • Ctrl=descer",
...
-- ══════════════════════════════════════════════════════════
--  UI PLAYER TAB (versão definitiva com callbacks corretos)
-- ══════════════════════════════════════════════════════════
...
makePlSec(Pages["Player"], "⚡ VELOCIDADE & PULO", Color3.fromRGB(255,200,50))
makePlRow_Slider(Pages["Player"], "Speed",    Color3.fromRGB(255,180,30),  "⚡ Speed",
    "Velocidade (padrão:16 | mín:1 | máx:200)", 1, 200, 5, 30, function(v) playerSpeed=v; applySpeed(v) end)
makePlRow_Slider(Pages["Player"], "JumpPower", Color3.fromRGB(100,220,255), "🦘 Jump Power",
    "Altura do pulo (padrão:50 | mín:10 | máx:400)", 10, 400, 10, 80, function(v) playerJump=v; applyJump(v) end)
...
makePlRow_Toggle(Pages["Player"], "Fly", Color3.fromRGB(80,180,255), "✈️ Fly",
    "W/A/S/D mover • Espaço=subir • Ctrl=descer • Setas [-][+] = velocidade",
    function(s) setFly(s) end,
    flySpeedRef)
makePlRow_Toggle(Pages["Player"], "Noclip", Color3.fromRGB(140,255,140), "👻 Noclip",
    "Atravessa paredes • Anti-void: segurança abaixo de Y=-100",
    function(s) setNoclip(s) end)
...
makePlRow_Toggle(Pages["Player"], "BauANC",  Color3.fromRGB(210,160,80), "📦 Baú ANC",
    "Remove animação dos baús — abrem instantaneamente ao clicar",
    function(s) setBauANC(s) end)
...
local aimbotAutoConn

-- AIMBOT: redireciona projéteis que surgem no workspace
workspace.DescendantAdded:Connect(function(obj)
    if not aimbotEnabled then return end
    task.defer(function()
        if not obj or not obj.Parent or not obj:IsA("BasePart") then return end
        local speed = obj.AssemblyLinearVelocity.Magnitude
        if speed < 10 then return end -- não é projétil
        -- Verificar se pertence a NPC (ignora)
        ...
        for _ = 1, 3 do
            if par and par:IsA("Model") and par:FindFirstChildWhichIsA("Humanoid") then
                ...
                if not isPlayer then return end -- projétil de NPC, ignora
            end
            par = par and par.Parent
        end
        local anHrp = findNearestAnimalHrp()
        if not anHrp then return end
        -- Redirecionar continuamente enquanto projétil existe
        local steps = 0
...
-- AIMBOT AUTO: detecta arma ranged equipada e atira automaticamente
    ...
    task.spawn(function()
        while aimbotAutoEnabled do
            ...
            pcall(function()
                ...
                -- Virar câmera e personagem para o alvo
                local dir = (anHrp.Position - myHrp.Position).Unit
                local targetCF = CFrame.new(myHrp.Position, myHrp.Position + dir)
                myHrp.CFrame = CFrame.new(myHrp.Position) * CFrame.Angles(0, math.atan2(dir.X, dir.Z) + math.pi, 0)
                -- Disparar: fire o RemoteEvent de ativação da tool (se existir)
                -- Tentar via tool.Activated
                pcall(function()
                    local remote = tool:FindFirstChildOfClass("RemoteEvent")
                    if remote then remote:FireServer() end
                end)
                -- Simular ativação direto na tool
                pcall(function()
                ...
                -- Fallback: usar VirtualInputManager para mouse1
                pcall(function()
...
-- ══════════════════════════════════════════════════════════
--  UI AVANÇADO TAB (AvancadoFuncoes)
-- ══════════════════════════════════════════════════════════
local avTabLO = 0
...
local function makeAvRow_Toggle(icon_key, cor, lbl_txt, desc_txt, onToggle)
    ...
    local ic=criarIconeAvancado(ib,icon_key,cor); ic.Position=UDim2.new(0,5,0,5)
    local bl=Instance.new("Frame",row); bl.BackgroundColor3=cor; bl.BorderSizePixel=0
    bl.Position=UDim2.new(0,0,0.15,0); bl.Size=UDim2.new(0,3,0.7,0); bl.ZIndex=8
    Instance.new("UICorner",bl).CornerRadius=UDim.new(0,2)
    local tl=Instance.new("TextLabel",row); tl.BackgroundTransparency=1
...
makeAvRow_Toggle("Aimbot", Color3.fromRGB(255,80,80), "🎯 Aimbot (Teleguiado)",
    "Projéteis se movem automaticamente para o animal mais próximo. Atira em qualquer direção — 100% de acerto.",
    function(s) aimbotEnabled = s end)

makeAvRow_Toggle("AimbotAuto", Color3.fromRGB(255,140,40), "🤖 Aimbot AUTO (Disparo automático)",
    "Com arma ranged equipada: atira sozinho nos animais mais próximos sem precisar clicar. VirtualInput invisível.",
    function(s)
        aimbotAutoEnabled = s
        if s then startAimbotAuto() end
    end)

makeAvInfoBox("ℹ️  AIMBOT: Funciona com todas as armas de projéteis (Revolver, Rifle, Crossbow, Flamethrower, etc).\nAimbot AUTO: equipe a arma e ele dispara automaticamente em loop nos animais mais próximos.\n⚠️  Requer exploit com VirtualInputManager para Aimbot AUTO disparar.", Color3.fromRGB(80,140,255))

-- ══════════════════════════════════════════════════════════
--  LOG FINAL
-- ══════════════════════════════════════════════════════════
print("╔══════════════════════════════════════════════════════════════╗")
print("║  PUDIM HUB v5 MOBILE — COMPLETO  Fev 2026                        ║")
print("╠══════════════════════════════════════════════════════════════╣")
print("║  ✓ Player: Speed | JumpPower | Fly | Noclip | TpClick | ANC ║")
print("║  ✓ Avançado: Aimbot Teleguiado | Aimbot AUTO                ║")
print("║  ✓ ESP: 20 cats | BRING: 17 cats | Wiki 2026                ║")
print("║  ✓ NOVO: InfZoom|CamLivre|FullBright|InfJump|SlopeBoost    ║")
print("╚══════════════════════════════════════════════════════════════╝")
